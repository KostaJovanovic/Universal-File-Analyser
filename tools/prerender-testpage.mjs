...
const WEB = join(ROOT, 'web');   // the website source (test.html + assets live under web/)
const css = readFileSync(join(WEB, 'assets', 'css', 'analyser.css'), 'utf8');
...
const file = join(WEB, 'test.html');
if (!existsSync(file)) {
  console.log('prerender-testpage: web/test.html not present - skipping (the style-guide page is optional).');
  process.exit(0);
}
