...
async function renderSvg(file, resultsEl) {
  ...
  const svgContainer = el('div', {
    style: 'border: 1px solid var(--hairline); border-radius: 4px; padding: 12px; overflow: auto; max-height: 500px; background: repeating-conic-gradient(var(--surface) 0% 25%, var(--bg) 0% 50%) 50% / 16px 16px;',
    html: svgText
  });
  ...
  if (parseErr) {
    statsCard.appendChild(el('p', { class: 'anr-hint', style: 'color: var(--accent);' }, 'SVG parse error — stats may be incomplete'));
  }
  ...
  if (colors.size > 0) {
    ...
    for (const c of colors) {
      ...
      const swatch = el('div', {
        style: `width: 32px; height: 32px; border-radius: 4px; border: 1px solid var(--hairline); background: ${c}; cursor: pointer;`,
...
async function renderCsv(file, resultsEl) {
  ...
  const tableWrap = el('div', {
    style: 'overflow: auto; max-height: 500px; border: 1px solid var(--hairline); border-radius: 4px;'
  });
  ...
  if (hasHeader && displayRows.length > 0) {
    ...
    for (const h of displayRows[0]) {
      headerRow.appendChild(el('th', { style: 'position: sticky; top: 0; background: var(--bg); z-index: 1;' }, h));
    }
