/* Analyser - parameterized STFT/ISTFT core for MDX-Net vocal separation.
   ...
   squared. Everything is verified in scratchpad/test-stft.mjs against a naive
   DFT and round-trips, for both 6144 and 7680. */
...
export function makeStftEngine(nFft, hop) {
  ...
  function stft(signal) {
    ...
    const re = new Float32Array(frames * nBins);
    const im = new Float32Array(frames * nBins);
    const fr = new Float64Array(nFft), fi = new Float64Array(nFft);
    for (let mIdx = 0; mIdx < frames; mIdx++) {
      const off = mIdx * hop;
      for (let i = 0; i < nFft; i++) { fr[i] = padded[off + i] * window[i]; fi[i] = 0; }
      const spec = fft(fr, fi, false);
      const base = mIdx * nBins;
      for (let b = 0; b < nBins; b++) { re[base + b] = spec.re[b]; im[base + b] = spec.im[b]; }
    }
    return { re, im, frames, bins: nBins };
  }
  ...
  function istft(re, im, frames, length) {
    ...
    for (let mIdx = 0; mIdx < frames; mIdx++) {
      ...
      for (let b = 0; b < nBins; b++) { fr[b] = re[base + b]; fi[b] = im[base + b]; }
      for (let b = 1; b < nFft - nBins + 1; b++) {
        fr[nFft - b] = re[base + b];
        fi[nFft - b] = -im[base + b];
      }
      const rec = fft(fr, fi, true);   // inverse, unscaled
      const off = mIdx * hop;
