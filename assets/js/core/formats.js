...
export const FULL_ANALYSIS = [
  ...
  { label: 'Web / code', exts: 'HTML CSS JS TS TSX JSX JSON YAML XML MD PS1 PSM1 PSD1 BAT CMD', tags: 'programming development website react typescript javascript node powershell ps1 psm1 psd1 script module manifest windows automation cmdlet sysadmin shell batch bat cmd command prompt dos launcher', desc: 'Preview and inspect HTML, CSS, JavaScript, TypeScript, JSON, YAML, XML and Markdown source files, plus Windows scripts - PowerShell (PS1, PSM1, PSD1), reading their comment-based help synopsis, #Requires directives, function and parameter counts, CmdletBinding and Authenticode signing; and batch/command scripts (BAT, CMD), reading the echo state, labels, variables set and the external tools they invoke - all alongside the source.' },
...
export const IDENTIFICATION_EXTENDED = [
  ...
  { label: 'Fonts (more)', exts: 'GLIF', tags: 'ufo unified font object glif glyph interchange format robofont glyphs fontforge defcon fonttools font source outline contour type design', desc: 'Identify and inspect UFO glyph sources (.glif) - the XML one-glyph format inside Unified Font Object projects used by RoboFont, Glyphs, FontForge and fontTools: read the glyph name, Unicode, advance width and contour/point/component counts, and render the glyph outline.' },
];

export const IDENTIFICATION_CORE = [
  { label: 'Adobe',           exts: 'INDD INDT IDML AEPX PRPROJ PREL MOGRT SESX XD FLA SWF XMP LRtemplate LRcat ACV ACO ASL ABR GRD PAT', tags: 'photoshop illustrator indesign after effects premiere pro premiere elements audition xd animate flash lightroom substance motion graphics compositing', desc: 'Identify Adobe project files and read their metadata: InDesign (INDD), the After Effects XML project (AEPX), Premiere Pro and Elements (PRPROJ, PREL) with sequence and media counts, XD, Animate (FLA), and Lightroom. After Effects projects (AEP, AET), Photoshop (PSD, PSB) and Illustrator (AI) now open in full viewers - see After Effects, Photoshop and Illustrator.' },
  { label: 'Design',          exts: 'FIG Sketch afphoto afdesign afpub XCF SPP SBSAR SBS', tags: 'figma sketch affinity photo designer publisher gimp substance painter', desc: 'Identify design-app files: Figma (FIG), Sketch, Affinity Photo/Designer/Publisher, GIMP (XCF), and Substance. Krita (KRA), Procreate and Paint.NET (PDN) now show their embedded preview - see Raster art above.' },
  ...
  { label: '3D / printing',   exts: 'FBX USDZ USD USDA BLEND', tags: 'blender mesh model 3d printing prusa bambu cura slicer wavefront autodesk pixar apple unity unreal stl', desc: 'Identify 3D and 3D-printing files: FBX, USD/USDZ, and Blender (BLEND). OBJ, PLY, OFF, 3MF, AMF and glTF/GLB get a full 3D viewer - see the 3D model section.' },
  ...
  { label: 'Databases',       exts: 'SQLite SQLite3 DB DB3 SQL SQLite-WAL SQLite-SHM DB-WAL DB-SHM', tags: 'sqlite sqlite3 microsoft access database sql dump schema table query rows ddl wal shm write-ahead log shared memory wal-index checkpoint journal rollback sidecar salt frames pages', desc: 'Open SQLite databases (.sqlite/.db/.sqlite3) and read their full schema in-browser - every table with its columns and row counts, views, indexes, triggers, the CREATE-statement DDL, and a sample of the largest table. Reads the WAL-mode sidecars too: the Write-Ahead Log (-wal) - page size, salts, frame and committed-transaction counts, and the pages it changed - and the shared-memory index (-shm) - valid frame count, database size and checkpoint progress. Also parses .sql dumps (dialect, tables, columns, INSERT counts). Microsoft Access (MDB, ACCDB) now opens in a full table viewer - see Access database above.' },
  ...
  { label: 'Video editing',   exts: 'AEPX PRPROJ PREL VEG VF DRP DRT WFP WSP', tags: 'adobe after effects motion graphics premiere pro premiere elements sony vegas magix davinci resolve blackmagic wondershare filmora capcut bytedance video editing timeline sequence compositing vfx ndi draft_content.json', desc: 'Identify and read video-editing project files: the After Effects XML project (AEPX); Adobe Premiere Pro / Elements (PRPROJ, PREL) - sequence, media-item and clip counts plus media paths; Sony / MAGIX VEGAS Pro (VEG, VF); DaVinci Resolve from Blackmagic Design (DRP, DRT); Wondershare Filmora (WFP, WSP); and CapCut drafts (draft_content.json) - canvas resolution, duration, tracks and material counts. After Effects projects (AEP, AET) now open in a full timeline viewer - see After Effects.' },
...
const CAT_OF = {
  ...
  'Editing timeline': 'video', 'Subtitles': 'video',
  // Documents & ebooks
  'PDF': 'documents', 'Office docs': 'documents', 'Text & markup': 'documents', 'Apple iWork': 'documents', 'Documents': 'documents',
  'Documents / ebooks (more)': 'documents', 'eBooks': 'documents', 'Fonts': 'documents', 'Fonts (more)': 'documents',
