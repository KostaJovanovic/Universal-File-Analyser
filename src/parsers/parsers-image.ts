...
async function parseHdr(file: File) {
  ...
  if (res) {
    const dims: any = {}; dims[res[1][1]] = parseInt(res[2], 10); dims[res[3][1]] = parseInt(res[4], 10);
...
/* An EPS is meant to be placed inside another document by a layout program that
   has no PostScript interpreter, so it carries a picture of itself for that
   program to draw on screen. There are two mechanisms and a file uses one or the
   other:

   - DOS EPS (also called EPSF or TBCP) wraps the PostScript in a 30-byte binary
     header. Three sections are located by absolute offset and length: the
     PostScript itself, a Windows metafile preview, and a TIFF preview. The TIFF
     is what nearly every Windows-authored EPS carries.
   - Plain-text EPS has nowhere to put binary, so EPSI inlines the preview as
     hexadecimal inside PostScript comments - a greyscale bitmap, 1, 2, 4 or 8
     bits per pixel, written by ps2epsi and friends.

   Both are worth extracting: the stored preview is what the *authoring* program
   thought the file looked like, which is not always what rendering it produces. */

/** The three sections a DOS EPS binary header points at, by absolute offset. */
interface DosEps { wmfOff: number; wmfLen: number; tifOff: number; tifLen: number }

function dosEpsHeader(head: Uint8Array): DosEps | null {
...
/** The %%BeginPreview line's four numbers: pixel size, bits per pixel, and how
    many comment lines of hex follow. */
interface EpsiHead { w: number; h: number; depth: number; lines: number }

function epsiHeader(txt: string): EpsiHead | null {
  const m = txt.match(/^%%BeginPreview:[ \t]*(\d+)[ \t]+(\d+)[ \t]+(\d+)[ \t]+(\d+)/im);
  if (!m) return null;
  const w = +m[1], h = +m[2], depth = +m[3];
  if (!w || !h || w > 8192 || h > 8192) return null;
  if (depth !== 1 && depth !== 2 && depth !== 4 && depth !== 8) return null;
...
async function parseEps(file: File, ext: string) {
  ...
  if (epsi) out['Embedded preview'] = 'EPSI (ASCII), ' + epsi.w + ' × ' + epsi.h + ' px, ' + epsi.depth + '-bit grey';
  else if (dos && dos.tifLen) out['Embedded preview'] = 'TIFF (DOS EPS), ' + fmtBytes(dos.tifLen) + (dos.wmfLen ? ' + WMF, ' + fmtBytes(dos.wmfLen) : '');
  else if (dos && dos.wmfLen) out['Embedded preview'] = 'WMF (DOS EPS), ' + fmtBytes(dos.wmfLen);
...
async function addStoredEpsPreview(file: File, host: HTMLElement, dos: DosEps | null, epsi: EpsiHead | null, txt: string) {
  const baseName = (file.name || 'image').replace(/\.[^/.]+$/, '');
  const show = (items: EmbeddedImageItem[], hint: string) => {
  ...
  try {
    ...
    if (!dos || !dos.tifLen || dos.tifLen > PREVIEW_CARVE_MAX) return;
    if (dos.tifOff + dos.tifLen > file.size) return;
    const btn = el('button', { type: 'button', class: 'anr-btn', style: 'margin-top:10px;' }, 'Show stored preview');
    const note = el('p', { class: 'anr-hint', style: 'margin:8px 0 0;' },
      'This EPS stores a ' + fmtBytes(dos.tifLen) + ' TIFF preview of itself. Web browsers cannot show TIFF, so it is converted on demand.' +
    ...
    btn.addEventListener('click', async () => {
      ...
      const tif = new File([bytes as BlobPart], baseName + '_preview.tif', { type: 'image/tiff' });
      const { readImagesAsPngs } = await import('../renderers/photo-convert.js');
      const pages = await readImagesAsPngs(tif, host);
      if (!pages.length) { btn.disabled = false; btn.textContent = 'Could not decode the stored preview'; return; }
      btn.remove(); note.remove();
      show(pages.map((r, i) => ({
...
async function parseFig(file: File): Promise<Row|null> {
  ...
  if (magic !== 'fig-kiwi' && magic !== 'fig-jam.') return null;
  const dv = new DataView(head.buffer, head.byteOffset, head.byteLength);
  ...
  if (scan) {
    ...
    while (p + 4 <= scan.length && sizes.length < 16) {
      ...
      if (!len || len > file.size) break;
      sizes.push(len);
      p += 4 + len;
      if (p > scan.length) break;     // chunk runs past what we read - stop cleanly
    }
  }
  ...
  if (sizes.length) {
    ...
    if (sizes[1]) out['Document chunk'] = fmtBytes(sizes[1]) + ' compressed';
  }
