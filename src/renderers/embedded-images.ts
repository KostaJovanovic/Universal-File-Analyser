  ...
  imgEl.addEventListener('load', () => {
    ...
    try {
      ...
      switch (o) {
        ...
        case 4: cx.transform(1, 0, 0, -1, 0, h); break;
        case 5: cx.transform(0, 1, 1, 0, 0, 0); break;
        case 6: cx.transform(0, 1, -1, 0, h, 0); break;
        case 7: cx.transform(0, -1, -1, 0, h, w); break;
        case 8: cx.transform(0, -1, 1, 0, 0, w); break;
      }
      cx.drawImage(imgEl, 0, 0, w, h);
      cv.toBlob((b) => {
        if (!b) return;
        const u = URL.createObjectURL(b);
        if (signal) signal.addEventListener('abort', () => { try { URL.revokeObjectURL(u); } catch (_) {} });
        imgEl.src = u;
      }, 'image/png');
...
export function buildEmbeddedImagesCard({ title, hint, help, items, signal, resultsEl, sourceFile }: {
  ...
  for (const it of items) {
    ...
    if (turned) metaBits.push(el('div', { class: 'anr-hint' }, 'stored ' + turned + ', shown upright'));
