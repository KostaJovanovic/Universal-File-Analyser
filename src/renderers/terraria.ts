...
const WALL_COLOR: Record<number, number> = {
...
function tileColour(type: number) {
  ...
  if (type >= 470) return 0x9A9A9A;
  if (type >= 250) return 0x8A7A6A;
  return 0x7A7A7A;
}

interface TerrariaWorld {
  version: number;
  name: string;
  seed: string;
  width: number;
...
function parseHeader(r: Reader): TerrariaWorld | null {
  const version = r.u32();
  // 1.3.0.1 (version 135) added the "relogic" magic + revision block.
  if (version >= 135) {
    const magic = r.ascii(7);
    if (magic !== 'relogic') return null;
    r.u8();        // file type: 1 map, 2 world
    r.u32();       // revision
    r.u64();       // favourite
  }
  const numSections = r.u16();
  if (numSections < 2 || numSections > 32) return null;
  const sectionPointers: number[] = [];
  for (let i = 0; i < numSections; i++) sectionPointers.push(r.u32());
  ...
  let seed = '';
  if (version >= 179) { seed = pstring(r); r.u64(); }
  if (version >= 181) r.skip(16);            // world GUID
  r.u32();                                    // world id
  r.i32(); r.i32(); r.i32(); r.i32();          // left/right/top/bottom bounds
  const height = r.i32();
  const width = r.i32();
  let gameMode = 0;
  if (version >= 209) gameMode = r.i32();
  else if (version >= 112) gameMode = r.u8() ? 1 : 0;

  if (width <= 0 || height <= 0 || width > 20000 || height > 12000) return null;
  return {
...
function renderMap(r: Reader, w: TerrariaWorld, step: number) {
  ...
  while (x < w.width && r.pos < r.length) {
    ...
    if (flags1 & 2) {
      ...
      if (w.important[type]) { r.i16(); r.i16(); }     // frame coordinates
      if (flags3 & 8) r.u8();                          // paint
      colour = tileColour(type);
    }
    if (flags1 & 4) {
      let wall = r.u8();
      if (flags3 & 16) r.u8();                         // wall paint
      if (flags3 & 64) wall |= r.u8() << 8;            // walls above 255 (1.4)
      if (!hasTile) colour = WALL_COLOR[wall] ?? 0x2A2A2A;
    }
    const liquid = (flags1 & 24) >> 3;
    if (liquid) {
      r.u8();                                          // amount
      if (!hasTile) colour = LIQUID_COLOR[liquid] || colour;
    }
    if (flags3 & 128) r.u8();                          // 1.4.4 liquid type byte
    let rle = 0;
    const rleMode = (flags1 & 192) >> 6;
    if (rleMode === 1) rle = r.u8();
    else if (rleMode === 2) rle = r.u16();
    ...
    for (let n = 0; n <= rle; n++) {
      ...
      y++;
      if (y >= w.height) { y = 0; x++; if (x >= w.width) break; }
    }
  }
  return { img, mw, mh };
}
...
export async function renderTerraria(file: File, resultsEl: HTMLElement) {
  const loader = inlineLoader('Reading world…');
  resultsEl.appendChild(loader);

  let world: TerrariaWorld | null = null;
  ...
  if (!world || !bytes) {
    resultsEl.appendChild(el('div', { class: 'anr-info' },
      'This file has a .wld extension but does not parse as a Terraria world.'));
    return;
  }
  const wld = world;

  const info = el('div', { class: 'anr-card' });
  const [ih, ihelp] = h3help('Terraria world',
    'Read straight from the world file: its name and seed, the size and difficulty it was generated at, and the game version that last saved it.');
  info.appendChild(ih); info.appendChild(ihelp);
  const tbl = el('table', { class: 'anr-table' });
  if (wld.name) tbl.appendChild(row('World name', wld.name));
  if (wld.seed) tbl.appendChild(row('Seed', wld.seed));
  ...
  if (wld.version < 140 || wld.version > 300) {
    resultsEl.appendChild(el('div', { class: 'anr-info' },
      'The map is not drawn: this world was saved by a Terraria version outside the 1.3-1.4 range this reader covers.'));
    return;
  }

  const drawLoader = inlineLoader('Drawing map…');
  resultsEl.appendChild(drawLoader);
  let map: { img: Uint8ClampedArray; mw: number; mh: number } | null = null;
  try {
  ...
  if (!map) {
    resultsEl.appendChild(el('div', { class: 'anr-info' },
      'The world header was read, but its tile data could not be walked to the end.'));
    return;
  }

  const cv = document.createElement('canvas');
  cv.width = map.mw; cv.height = map.mh;
  const data = new Uint8ClampedArray(map.img.buffer as ArrayBuffer, map.img.byteOffset, map.img.length);
  ...
  const [h, help] = h3help('World map',
    'One pixel per tile, drawn from the world’s own tile data: terrain and ores in the foreground, walls shaded behind them, and water, lava and honey in their own colours. The palette is representative rather than an exact copy of the in-game map.');
  ...
  card.appendChild(el('div', { style: 'margin-top:10px;' }, [
    el('button', { type: 'button', class: 'anr-btn', onclick: () => {
      cv.toBlob((b) => { if (b) downloadBlob((file.name || 'world').replace(/\.[^.]+$/, '') + '_map.png', b); }, 'image/png');
    } }, 'Save map (PNG)'),
  ]));
