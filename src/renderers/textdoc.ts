...
function extractMhtmlHtml(text: string) {
  ...
  const decodePart = (raw: string) => {
    ...
    const decode = (bytes: Uint8Array) => {
      let dec;
      try { dec = new TextDecoder(cs ? cs[1] : 'utf-8'); } catch (_) { dec = new TextDecoder('utf-8'); }
    ...
    if (/content-transfer-encoding:\s*quoted-printable/.test(headers)) {
      ...
      for (let i = 0; i < rawBytes.length; i++) {
        if (rawBytes[i] === 0x3d && i + 2 < rawBytes.length && hex(rawBytes[i + 1]) && hex(rawBytes[i + 2])) {
          ...
          i += 2;
        } else out[n++] = rawBytes[i];
      }
      body = decode(out.subarray(0, n));
    } else if (/content-transfer-encoding:\s*base64/.test(headers)) {
      try { body = decode(Uint8Array.from(atob(body.replace(/\s+/g, '')), (c) => c.charCodeAt(0))); } catch (_) {}
    }
    return body;
  };
  if (mb) {
...
export async function renderTextDoc(file: File, container: HTMLElement, kind: string, ext: string) {
  ...
  try {
    ...
    let truncated = false;
    const readCapped = () => {
      if (file.size <= TEXTDOC_READ_MAX) return file.text();
      truncated = true;
