...
function channelEmbedProb(hist) {
  ...
  for (let k = 0; k < 128; k++) {
    ...
    const exp = (a + b) / 2;
    if (exp < 5) continue;                 // chi-square validity: skip sparse pairs
    chi += ((a - exp) * (a - exp)) / exp;
    df++;
  }
  if (df < 8) return null;                  // too few populated pairs to trust
...
export async function computeElaCanvas(src, { quality = 0.9, amplify = 18, maxDim = 1600 } : any = {}) {
  ...
  for (let i = 0; i < o.length; i += 4) {
    ...
    d[i] = er; d[i + 1] = eg; d[i + 2] = eb; d[i + 3] = 255;
    const m = (dr + dg + db) / 3;
...
export function parseJpegQuantTables(bytes) {
  ...
  while (p + 4 <= n) {
    ...
    while (marker === 0xFF && p + 1 < n) { p++; marker = bytes[p + 1]; }
...
export async function computeJpegGhosts(src, { qualities, maxDim = 1024, block = 4 } : any = {}) {
  ...
  for (const q of qualities) {
    ...
    for (let k = 0; k < acc.length; k++) acc[k] = cnt[k] ? acc[k] / cnt[k] : 0;
    maps.push({ q, data: acc });
  }
  if (!maps.length) return null;
  let mn = Infinity, mx = -Infinity;
  for (const m of maps) for (const v of m.data) { if (v < mn) mn = v; if (v > mx) mx = v; }
  const rng = (mx - mn) || 1;
...
export function checkIptcDigest(bytes) {
  ...
  while (c + 12 <= irb.end) {
    if (!startsWithAscii(bytes, c, '8BIM')) break;
    const id = (bytes[c + 4] << 8) | bytes[c + 5];
    let q = c + 6;
    const nameLen = bytes[q];                              // Pascal name, padded to even (incl. length byte)
    q += 1 + nameLen; if ((1 + nameLen) % 2) q += 1;
    const dataLen = (bytes[q] << 24 | bytes[q + 1] << 16 | bytes[q + 2] << 8 | bytes[q + 3]) >>> 0;
    q += 4;
    const dataStart = q, dataEnd = q + dataLen;
    if (id === 0x0404) iptcData = bytes.subarray(dataStart, dataEnd);
    else if (id === 0x0425) storedDigest = bytes.subarray(dataStart, dataStart + 16);
    c = dataEnd + (dataLen % 2);                           // data padded to even
  }
  if (!storedDigest || storedDigest.length < 16) return null;
