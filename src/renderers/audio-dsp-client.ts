...
const _jobs = new Map<number, (err: Error) => void>();
function killWorker(exceptJob: number, err: Error) {
  try { if (worker) { worker.terminate(); worker = null; } } catch (_) {}
  for (const [id, fail] of [..._jobs]) if (id !== exceptJob) fail(err);
}
...
export function runAudioDsp(audioBuffer: AudioBuffer, { needBpm, signal, onPass }: { needBpm: boolean; signal?: AbortSignal; onPass: (name: string, value: any) => void }) {
  return new Promise<void>((resolve, reject) => {
    ...
    const onErr = () => {
      cleanup();
      if (worker === w) killWorker(jobId, new Error('audio DSP worker failed to start'));
    ...
    const onAbort = () => {
      ...
      if (worker === w) killWorker(jobId, new Error('audio DSP worker was stopped by another analysis'));
    ...
    _jobs.set(jobId, (err: Error) => { cleanup(); reject(err); });
    if (signal) {
