...
function lockDownPreview(html: string) {
...
/** Where the thumbnail's pixels are and how big they are - found from the header
    bytes alone, so the pixels themselves can be read as one exact slice instead
    of pulling megabytes of scene data in to look for them. */
interface BlendThumbAt { off: number; w: number; h: number }
...
function blendThumbAt(buf: Uint8Array, little: boolean, ptr64: boolean): BlendThumbAt | null {
  ...
  for (let i = 0; i < BLEND_SCAN_BLOCKS && p + headLen <= buf.length; i++) {
    ...
    if (code === 'ENDB') break;
    if (code === 'TEST') {
      if (len <= 8 || data + 8 > buf.length) return null;
      const w = dv.getUint32(data, little);
      const h = dv.getUint32(data + 4, little);
      if (w < 1 || h < 1 || w * h * 4 > PREVIEW_CARVE_MAX || w * h * 4 + 8 > len) return null;
...
function flipRows(src: Uint8Array, w: number, h: number) {
  const stride = w * 4;
  const out = new Uint8Array(h * stride);
  for (let y = 0; y < h; y++) out.set(src.subarray((h - 1 - y) * stride, (h - y) * stride), y * stride);
  return out;
}

async function parseBlender(head: Uint8Array, file: File) {
...
function parsePe(buf: Uint8Array): Row | null {
  ...
  if (peOffset + 24 > buf.length) return { 'Format': 'PE (truncated header)' };
...
export async function extractPeIcon(file: File) {
  ...
  try {
    ...
    // rather than m^k.
    const leafMemo = new Map<number, RsrcLeaf | null>();
    const firstLeaf = (dirOff: number, depth = 0): RsrcLeaf | null => {
      if (depth >= 3) return null;
      if (leafMemo.has(dirOff)) return leafMemo.get(dirOff)!;
      leafMemo.set(dirOff, null);                     // in progress: a cycle reads as "no leaf"
      let found: RsrcLeaf | null = null;
...
async function inflateZlib(bytes: Uint8Array, origLen: number) {
  if (typeof DecompressionStream === 'undefined') throw new Error('no DecompressionStream');
  if (origLen > DECOMP_ENTRY_MAX) throw new Error('table too large');
  const out = await inflate(bytes, 'deflate', origLen);
  if (!out) throw new Error('inflate failed or table larger than declared');
  return out;
}
...
async function woffTables(buf: Uint8Array, want: string[]) {
  ...
  for (let i = 0; i < numTables; i++) {
    ...
    try { out[tag] = compLen < origLen ? await inflateZlib(comp, origLen) : comp.slice(); } catch (_) {}
  }
  return out;
}
...
async function parseTorrent(file: File) {
  try {
    ...
    function decode(): any {
      ...
      if (colonIdx >= raw.length || !/^\d+$/.test(lenStr)) throw new Error('bad bencode string length');
...
async function parseFilmora(file: File) {
  try {
...
function prettyKV(obj: any, indent?: number) {
  ...
  const push = (s: string) => {
    if (full) return;
    if (size + s.length > READABLE_TEXT_MAX) { parts.push('… (output truncated)\n'); full = true; return; }
    parts.push(s); size += s.length;
  };
  (function walk(o: any, depth: number) {
    ...
    for (const k in o) {
      ...
      if (v && typeof v === 'object') {
        if (onPath.has(v)) { push(pad + k + '  =  (circular reference)\n'); continue; }
        if (depth + 1 >= READABLE_DEPTH_MAX) { push(pad + k + '  =  (nested too deep)\n'); continue; }
        push(pad + k + '\n' + pad + '{\n');
        walk(v, depth + 1);
        push(pad + '}\n');
      } else {
...
function formatFor(ext: string): any {
  return Object.hasOwn(FORMATS, ext) ? (FORMATS as Record<string, any>)[ext] : undefined;
}
