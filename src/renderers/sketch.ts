...
function textOf(o: any): string|null {
...
function walk(o: any, doc: SketchDoc, pageName: string): SketchNode|null {
  if (!o || typeof o !== 'object') return null;
  if (doc.total >= SKETCH_LAYER_MAX) { doc.truncated = true; return null; }
  doc.total++;
  ...
  if (cls === 'symbolMaster' && node.symbolId) doc.masters.push({ id: node.symbolId, name: node.name });
  if (cls === 'symbolInstance' && node.symbolId) doc.instanceCounts[node.symbolId] = (doc.instanceCounts[node.symbolId] || 0) + 1;
  if (node.text) doc.texts.push({ page: pageName, text: node.text });
...
function treeNode(n: SketchNode, depth: number): HTMLElement {
  ...
  if (depth <= 1 || n.cls === 'artboard') det.open = true;
...
export async function renderSketch(file: File, resultsEl: HTMLElement) {
  resultsEl.hidden = false;
  resultsEl.innerHTML = '';

  let zip;
  try { zip = await openZip(file); }
  catch (e) { resultsEl.appendChild(errorCard('Could not open this .sketch file - it should be a ZIP package.')); return; }

  let meta: any = {}, document_: any = {};
  try { meta = JSON.parse((await zip.text('meta.json')) || '{}'); } catch (_) { meta = {}; }
  try { document_ = JSON.parse((await zip.text('document.json')) || '{}'); } catch (_) { document_ = {}; }

  const doc: SketchDoc = { pages: [], masters: [], instanceCounts: {}, texts: [], counts: {}, total: 0, truncated: false };
  ...
  // ---- Info card ----
  const [h, help] = h3help('Sketch document',
    'A .sketch file is a ZIP of JSON, so the whole document can be read: every page, artboard, group and layer, the components it defines and the copy it contains. Nothing is rendered from the vector data - the preview below is the picture Sketch itself saved into the file.');
  const card = el('div', { class: 'anr-card' });
  card.appendChild(h); card.appendChild(help);
  const tbl = el('table', { class: 'anr-readout' });
  tbl.appendChild(row('File', file.name));
  tbl.appendChild(row('Size', fmtBytes(file.size)));
  if (meta.appVersion) tbl.appendChild(row('Created with', 'Sketch ' + meta.appVersion + (meta.build ? ' (build ' + meta.build + ')' : '')));
  if (meta.commit) tbl.appendChild(rowHelp('Document commit', String(meta.commit).slice(0, 12),
    'Sketch versions its document format; this identifies the exact build of the file format, not the design.'));
  if (meta.variant) tbl.appendChild(row('Variant', String(meta.variant)));
  tbl.appendChild(row('Pages', String(doc.pages.length)));
  const artboards = doc.counts.artboard || 0;
  if (artboards) tbl.appendChild(row('Artboards', String(artboards)));
  tbl.appendChild(rowHelp('Layers', doc.total.toLocaleString() + (doc.truncated ? ' (stopped counting here)' : ''),
    'Every object in the document, counting groups and artboards as well as the shapes inside them.'));
  if (doc.masters.length) tbl.appendChild(rowHelp('Components', String(doc.masters.length),
    'Sketch calls these symbols: a master that other parts of the document reuse by reference rather than by copying.'));
  const instances = doc.counts.symbolInstance || 0;
  if (instances) tbl.appendChild(row('Component instances', String(instances)));
  if (doc.counts.text) tbl.appendChild(row('Text layers', String(doc.counts.text)));
  if (doc.counts.bitmap) tbl.appendChild(row('Placed images', String(doc.counts.bitmap)));

  const fonts: string[] = Array.isArray(meta.fonts) ? meta.fonts.filter((f: any) => typeof f === 'string') : [];
  if (fonts.length) tbl.appendChild(rowHelp('Fonts used', fonts.slice(0, 12).join(', ') + (fonts.length > 12 ? ' and ' + (fonts.length - 12) + ' more' : ''),
    'The typefaces the document refers to. A font listed here that is not installed will render as a substitute wherever the file is opened.'));
  const assets = (document_.assets && document_.assets.colorAssets) || document_.colorAssets;
  if (Array.isArray(assets) && assets.length) tbl.appendChild(row('Colour assets', String(assets.length)));
  const layerStyles = document_.layerStyles && document_.layerStyles.objects;
  if (Array.isArray(layerStyles) && layerStyles.length) tbl.appendChild(row('Shared layer styles', String(layerStyles.length)));
  const textStyles = document_.layerTextStyles && document_.layerTextStyles.objects;
  if (Array.isArray(textStyles) && textStyles.length) tbl.appendChild(row('Shared text styles', String(textStyles.length)));
  ...
  if (previewName) {
    try {
      ...
      if (bytes) {
        ...
        const img = el('img', { alt: 'Sketch document preview' }) as HTMLImageElement;
  ...
  // ---- Component tree ----
  {
    ...
    const [th, thelp] = h3help('Component tree',
      'Every page, artboard, group and layer in the document, in the order Sketch stores them - which is back to front, so the first child of a group sits behind the ones after it. Each row names what the object is, how many children it has, and its size in points.');
    t.appendChild(th); t.appendChild(thelp);
    const tree = el('div', { class: 'anr-tree' });
    for (const p of doc.pages) tree.appendChild(treeNode(p, 0));
    t.appendChild(tree);
    if (doc.truncated) {
      t.appendChild(el('p', { class: 'anr-hint' },
        'Stopped after ' + SKETCH_LAYER_MAX.toLocaleString() + ' layers. Everything above is real; the rest of the document was not walked.'));
    }
  ...
  if (doc.masters.length) {
    ...
    const [ch, chelp] = h3help('Components (' + doc.masters.length + ')',
      'Sketch calls these symbols. Each is defined once and referenced everywhere it appears, so the instance count is a fair measure of how much of the design actually rests on it - and a component with no instances is one nothing uses.');
    c.appendChild(ch); c.appendChild(chelp);
    ...
    for (const r of rows) t.appendChild(el('tr', {}, [el('td', {}, r.name), el('td', {}, r.n ? String(r.n) : 'unused')]));
    c.appendChild(t);
  ...
  if (doc.texts.length) {
    const c = el('div', { class: 'anr-card' });
    const [ch, chelp] = h3help('Text (' + doc.texts.length + ')',
      'Every string in the document, in document order, with the page it sits on. Reviewing a design usually means reading it, and these are otherwise several levels down inside each text layer.');
    c.appendChild(ch); c.appendChild(chelp);
    ...
    c.appendChild(list);
    if (doc.texts.length > 2000) c.appendChild(el('p', { class: 'anr-hint' }, 'Showing the first 2,000 of ' + doc.texts.length.toLocaleString() + '.'));
    resultsEl.appendChild(c);
  }
