...
function decodeCbor(b: Uint8Array, offRef?: { p: number }): any {
  const st = offRef || { p: 0 };
  const need = (n: number) => { if (!(n >= 0) || st.p + n > b.length) throw new Error('CBOR truncated'); };
  const read = (depth: number): any => {
    ...
    let len = minor;
    if (minor === 24) { need(1); len = b[st.p++]; }
    else if (minor === 25) { need(2); len = (b[st.p] << 8 | b[st.p + 1]); st.p += 2; }
    else if (minor === 26) { need(4); len = (b[st.p] * 0x1000000 + (b[st.p + 1] << 16 | b[st.p + 2] << 8 | b[st.p + 3])); st.p += 4; }
    else if (minor === 27) { // 64-bit: read as Number (fine for our field sizes)
      need(8); let v = 0; for (let k = 0; k < 8; k++) v = v * 256 + b[st.p++]; len = v;
    }
    else if (minor >= 28 && major !== 7) throw new Error('CBOR indefinite/reserved length');
    const left = b.length - st.p;
    switch (major) {
      case 0: return len;                       // uint
      case 1: return -1 - len;                  // negint
      case 2: { need(len); const s = b.subarray(st.p, st.p + len); st.p += len; return s; } // bytes
      case 3: { need(len); const s = utf8(b.subarray(st.p, st.p + len)); st.p += len; return s; } // text
      case 4: { if (len > left) throw new Error('CBOR array too long'); const a = []; for (let k = 0; k < len; k++) a.push(read(depth + 1)); return a; } // array
      case 5: { if (len * 2 > left) throw new Error('CBOR map too long'); const m: any = {}; for (let k = 0; k < len; k++) { const key = read(depth + 1); m[key] = read(depth + 1); } return m; } // map
  ...
  try { return read(0); } catch (_) { return null; }
}
