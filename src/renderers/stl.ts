...
export interface MeshGeometry {
  ...
  uvs?: Float32Array;
  textureImage?: any;
  /** Set by buildGeoFromIndexed only when it had to drop a corrupt triangle:
   *  1 per input triangle that was kept. Per-triangle attributes built from the
   *  original index list go through alignToKept() so they stay in step. */
  triKept?: Uint8Array;
}
...
export function alignToKept(geo: MeshGeometry, arr: Float32Array): Float32Array {
  const kept = geo.triKept;
  if (!kept || !kept.length) return arr;
  const stride = arr.length / kept.length;
  if (!Number.isInteger(stride) || stride <= 0) return arr;
...
export function makeResult(format: string, posArr: ArrayLike<number>, normArr: ArrayLike<number>): MeshGeometry {
  ...
  for (let i = 0; i < positions.length; i += 9) {
    ...
    if (!(isFinite(ax) && isFinite(ay) && isFinite(az) && isFinite(bx) && isFinite(by) && isFinite(bz)
      && isFinite(cx) && isFinite(cy) && isFinite(cz))) continue;
...
export function buildViewerCard(geo: MeshGeometry, title = '3D model', opts: any = {}) {
  ...
  if (viewer.ok) {
    ...
    const onDocClick = (e: MouseEvent) => {
      if (!viewCard.isConnected) { document.removeEventListener('click', onDocClick); return; }
      if (!qWrap.contains(e.target as Node)) qPanel.classList.add('is-hidden');
    };
...
export function buildGeoFromIndexed(verts: ArrayLike<number>, tris: ArrayLike<number>, format: string) {
  ...
  const drop = (t: number) => {
    if (!kept) { kept = new Uint8Array(Math.floor(tris.length / 3)); kept.fill(1, 0, t); }
  };
  for (let i = 0; i + 2 < tris.length; i += 3) {
    ...
    if (!(i0 >= 0 && i0 + 2 < vLen && i1 >= 0 && i1 + 2 < vLen && i2 >= 0 && i2 + 2 < vLen)) { drop(t); continue; }
    ...
    if (!(isFinite(ax) && isFinite(ay) && isFinite(az) && isFinite(bx) && isFinite(by) && isFinite(bz)
      && isFinite(cx) && isFinite(cy) && isFinite(cz))) { drop(t); continue; }
    if (kept) (kept as Uint8Array)[t] = 1;
    const e1x = bx - ax, e1y = by - ay, e1z = bz - az;
    const e2x = cx - ax, e2y = cy - ay, e2z = cz - az;
    let nx = e1y * e2z - e1z * e2y, ny = e1z * e2x - e1x * e2z, nz = e1x * e2y - e1y * e2x;
  ...
  const keptArr = kept as Uint8Array | null;   // assigned inside drop(), which flow analysis cannot see
  if (keptArr) geo.triKept = keptArr;
  return geo;
}
