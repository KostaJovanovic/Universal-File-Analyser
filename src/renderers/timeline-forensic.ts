...
function mvhdFromMoov(moov) {
  ...
  while (p + 8 <= moov.length) {
    ...
    let hs = 8;
    if (size === 1) { size = dv.getUint32(p + 8) * 4294967296 + dv.getUint32(p + 12); hs = 16; }
    if (size < 8) break;
...
export async function collectTimestamps(file) {
  ...
  if (file.lastModified) add('Filesystem last-modified', new Date(file.lastModified), 'filesystem');

  let head;
  try { head = new Uint8Array(await file.slice(0, 16).arrayBuffer()); } catch (_) { head = new Uint8Array(0); }
  ...
  try {
    if (head[0] === 0x89 && head[1] === 0x50) {
      ...
      if (m) { add('Movie header created (mvhd)', m.created, 'created'); add('Movie header modified (mvhd)', m.modified, 'modified'); }
...
export async function forensicTimelineCard(file) {
  ...
  if (stamps.length < 2 && !anomalies.length) return null;
