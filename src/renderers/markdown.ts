...
function isRemoteImage(raw: string) {
...
function stashCodeSpans(s: string, stash: string[]) {
  const runs: number[][] = [];            // [start, length]
  for (let i = 0; i < s.length;) {
    if (s.charCodeAt(i) !== 96) { i++; continue; }
    let j = i;
    while (j < s.length && s.charCodeAt(j) === 96) j++;
    runs.push([i, j - i]);
    i = j;
  }
  if (runs.length < 2) return s;
  const byLen = new Map<number, number[]>();   // length -> run indexes, ascending
  runs.forEach((r, k) => { let a = byLen.get(r[1]); if (!a) byLen.set(r[1], a = []); a.push(k); });
  ...
  for (let k = 0; k < runs.length; k++) {
    ...
    ptr.set(len, p);
    if (p >= list.length) continue;       // no closer: literal backticks
    const close = list[p];
...
function inlineMd(escaped: string) {
  ...
  s = s.replace(/!\[([^\]]*)\]\(([^)\s]+)(?:\s+&quot;([^)]*?)&quot;)?\)/g,
    (m, alt, src, title) => isRemoteImage(src)
...
function parseList(lines: string[], start: number): [string, number] {
  ...
  for (const it of items) {
    ...
    if (task) {
      ...
      if (!loose) rendered = unwrapP(rendered);
      html += '<li>' + rendered + '</li>';
    }
  }
  html += '</' + tag + '>';
  ...
  while (i < lines.length) {
    ...
    while (i < lines.length && !/^\s*$/.test(lines[i]) && !isBlockStart(lines[i])) {
      ...
      if (para.length && i + 1 <= lines.length &&
          (/^\s{0,3}=+\s*$/.test(lines[i]) )) break;
      para.push(lines[i]);
      i++;
      if (i < lines.length && (/^\s{0,3}=+\s*$/.test(lines[i]) || (/^\s{0,3}-+\s*$/.test(lines[i]) && para.length))) break;
    }
    if (para.length) {
...
export async function renderMarkdown(file: File, resultsEl: HTMLElement) {
  ...
  if (links) tbl.appendChild(row('Links', String(links)));
  if (images) tbl.appendChild(row('Images', String(images)));
  if (codeBlocks) tbl.appendChild(row('Code blocks', String(Math.floor(codeBlocks / 2) || codeBlocks)));
  if (tables) tbl.appendChild(row('Tables', String(tables)));
  ...
  if (blocks.length <= PREVIEW_ROWS) {
    ...
    renderCard.appendChild(el('p', { class: 'anr-hint' },
      'Showing the first ' + PREVIEW_ROWS + ' rows of ' + blocks.length + '.'));
