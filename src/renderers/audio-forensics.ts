...
export async function loudnessR128(channels: FloatBuf[], sampleRate: number, tick?: Tick) {
  ...
  for (let c = 0; c < channels.length; c++) {
    ...
    for (let i = 0; i < len; i++) f[i] += g * y[i] * y[i];
  }
  ...
  const blockMs = async (blockSec: number, hopSec: number) => {
    ...
    for (let start = 0; start + bl <= f.length; start += hop) {
      ...
      let s = 0; for (let i = start; i < start + bl; i++) s += f[i];
...
export async function truePeakDb(channels: FloatBuf[], sampleRate: number, tick?: Tick) {
  ...
  for (let p = 0; p < OS; p++) {
    ...
    if (sum) for (let k = 0; k < taps; k++) sub[k] /= sum;
    phase.push(sub);
  }
