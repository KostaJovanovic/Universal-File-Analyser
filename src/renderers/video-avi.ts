...
export async function extractAviData(file: File, aviInfo: AviHeaderInfo | null | undefined) {
  ...
  const result: { videoFrames: any[]; audioBuffer?: any } = { videoFrames };
...
/** One top-level RIFF segment's payload range. A file over 4 GB is OpenDML and
    has several, each with its own movi list. */
interface RiffSegment {
  start: number;
  end: number;
}

/** The movi list inside one segment. `fourcc` is the offset of the 'movi'
    FOURCC itself, which idx1 offsets are classically relative to. */
interface MoviList {
  fourcc: number;
  start: number;
  end: number;
...
/** Byte range of a segment's idx1 chunk payload. */
interface Idx1Range {
  start: number;
  end: number;
}

/** Growable (offset, size) list backed by typed arrays. */
interface ChunkList {
  readonly n: number;
  readonly bytes: number;
  offAt(i: number): number;
  sizeAt(i: number): number;
  push(o: number, s: number): void;
}

/** The frame surface the viewer steps through, whichever path opened the file.
    `streamed` says whether get() is a disk read (true) or an array lookup. */
export interface AviFrameSource {
  count: number;
  streamed: boolean;
  get(i: number): Promise<ArrayBuffer>;
  close(): void;
}

/** What openAviData() hands back. `audioSkipped` and `indexTruncated` are the
    two ways the streamed path deliberately gives up something, and the viewer
    surfaces both rather than silently dropping them. */
export interface AviOpenResult {
  source: AviFrameSource;
...
const str4 = (u8: Uint8Array, o: number) => (o + 4 <= u8.length)
  ? String.fromCharCode(u8[o], u8[o + 1], u8[o + 2], u8[o + 3]) : '';
...
const viewOf = (u8: Uint8Array) => new DataView(u8.buffer, u8.byteOffset, u8.byteLength);
...
function chunkList(): ChunkList {
  ...
  return {
    ...
    offAt: (i: number) => off[i],
    sizeAt: (i: number) => size[i],
    push(o: number, s: number) {
      if (n >= off.length) {
        const o2 = new Float64Array(off.length * 2); o2.set(off); off = o2;
        const s2 = new Uint32Array(size.length * 2); s2.set(size); size = s2;
      }
      off[n] = o; size[n] = s; n++; bytes += s;
    },
  };
}
...
async function riffSegments(file: File): Promise<RiffSegment[]> {
  const segs: RiffSegment[] = [];
  let pos = 0;
  while (pos + 12 <= file.size && segs.length < 64) {
    const head = await readBytes(file, pos, pos + 12);
    if (head.length < 12 || str4(head, 0) !== 'RIFF') break;
    const type = str4(head, 8);
    if (type !== 'AVI ' && type !== 'AVIX') break;
...
async function segmentLists(file: File, seg: RiffSegment) {
  const out: { movi: MoviList | null; idx1: Idx1Range | null } = { movi: null, idx1: null };
  let pos = seg.start;
  while (pos + 8 <= seg.end) {
    const head = await readBytes(file, pos, pos + 12);
...
async function indexFromIdx1(file: File, idx1: Idx1Range, movi: MoviList,
                             video: ChunkList, audio: ChunkList): Promise<boolean> {
...
async function indexByScan(file: File, movi: MoviList, video: ChunkList, audio: ChunkList,
                           onProgress?: ((frac: number) => void) | null) {
  let pos = movi.start, winStart = 0, win: Uint8Array | null = null;
  const span = Math.max(1, movi.end - movi.start);
  let lastPing = movi.start;
  const have = async (n: number) => {
    if (win && pos >= winStart && pos + n <= winStart + win.length) return true;
    winStart = pos;
    win = await readBytes(file, pos, Math.min(pos + AVI_STREAM_WINDOW, movi.end));
    return win.length >= n;
  };
  while (pos + 8 <= movi.end && video.n < AVI_INDEX_MAX) {
    if (!(await have(8))) break;
    const w = win!;
...
async function gatherPcm(file: File, audio: ChunkList, maxBytes: number): Promise<Uint8Array> {
...
function lazyFrameSource(file: File, video: ChunkList): AviFrameSource {
  const cache = new Map<number, ArrayBuffer>();   // insertion order = LRU
  let cached = 0;
  const clamp = (i: number) => Math.max(0, Math.min(video.n - 1, i | 0));
  return {
    count: video.n,
    streamed: true,
    async get(i: number) {
      i = clamp(i);
      if (cache.has(i)) { const b = cache.get(i)!; cache.delete(i); cache.set(i, b); return b; }
...
function arrayFrameSource(frames: ArrayBuffer[]): AviFrameSource {
  const clamp = (i: number) => Math.max(0, Math.min(frames.length - 1, i | 0));
  return {
    count: frames.length,
    streamed: false,
    get: (i: number) => Promise.resolve(frames[clamp(i)]),
    close() {},
  };
}
...
export async function openAviData(file: File, aviInfo: AviHeaderInfo | null | undefined,
                                  onProgress?: ((frac: number) => void) | null): Promise<AviOpenResult | null> {
  if (file.size <= AVI_EXTRACT_MAX) {
    let eager = null;
    try { eager = await extractAviData(file, aviInfo); } catch (_) { eager = null; }
  ...
  for (const seg of segs) {
    if (video.n >= AVI_INDEX_MAX) break;
    const { movi, idx1 } = await segmentLists(file, seg);
    if (!movi) continue;
    let indexed = false;
    if (idx1) {
      try { indexed = await indexFromIdx1(file, idx1, movi, video, audio); } catch (_) { indexed = false; }
    }
    if (!indexed) await indexByScan(file, movi, video, audio, onProgress);
  }
  if (!video.n) return null;
  ...
  const fmt = aviInfo && aviInfo.audioFormat;
  let audioBuffer: AudioBuffer | null = null, audioSkipped = false;
