...
export function decodeGifFrames(buffer: ArrayBuffer, budget = 120e6) {
  ...
  let cursor: { idx: number; canvas: Uint8ClampedArray } | null = null;

  const lruPut = (idx: number, data: Uint8ClampedArray) => {
    if (lru.has(idx)) lru.delete(idx);
    lru.set(idx, data);
    while (lru.size > L) lru.delete(lru.keys().next().value);   // evict oldest
  };
  ...
  const compose = (idx: number) => {
    ...
    while (s > 0 && !entry.has(s)) s -= K;                   // frame 0 starts fully transparent
    let canvas: Uint8ClampedArray;                           // canvas entering frame s
    if (cursor && cursor.idx <= idx && cursor.idx >= s) {
      s = cursor.idx; canvas = cursor.canvas; cursor = null;
    } else {
      canvas = s > 0 ? new Uint8ClampedArray(entry.get(s)) : new Uint8ClampedArray(stride);
    }
    let result = null;
    for (let k = s; k <= idx; k++) {
      if (k > 0 && k % K === 0 && !entry.has(k)) entry.set(k, new Uint8ClampedArray(canvas));
      const fr = table[k];
