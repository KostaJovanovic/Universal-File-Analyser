...
function id3FirstFrame(buf: Uint8Array, ver: number) {
  if (ver < 3 || !(buf[5] & 0x40) || buf.length < 14) return 10;
  if (ver === 4) {
  ...
  if (head[4] === 0x66 && head[5] === 0x74 && head[6] === 0x79 && head[7] === 0x70) {
    ...
    for (let i = 0; i + 8 < buf.length; i++) {
      if (buf[i] === 0x74 && buf[i + 1] === 0x6D && buf[i + 2] === 0x70 && buf[i + 3] === 0x6F) {
        ...
        if (!(buf[i + 8] === 0x64 && buf[i + 9] === 0x61 && buf[i + 10] === 0x74 && buf[i + 11] === 0x61)) continue;
