...
function jsonStats(val: any, depth: number) {
  let keys = 0, maxD = depth, arrayCount = 0, nodes = 0, partial = false;
  const arrays: number[] = [];
  const stack: [any, number][] = [[val, depth]];
  while (stack.length) {
    if (++nodes > JSON_STATS_NODES_MAX) { partial = true; break; }
    const [v, d] = stack.pop()!;
    if (d > maxD) maxD = d;
    if (v && typeof v === 'object' && !Array.isArray(v)) {
      const ks = Object.keys(v);
      keys += ks.length;
      for (let i = ks.length - 1; i >= 0; i--) stack.push([v[ks[i]], d + 1]);
    } else if (Array.isArray(v)) {
      arrayCount++;
      if (arrays.length < JSON_ARRAYS_LISTED) arrays.push(v.length);
