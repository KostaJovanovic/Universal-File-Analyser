...
function hillFormula(counts: Record<string, number>) {
...
function formatFor(ext: string) {
  if (ext === 'sdf' || ext === 'sd' || ext === 'mol' || ext === 'mdl') return 'sdf';
  if (ext === 'pdb' || ext === 'ent' || ext === 'pdb1') return 'pdb';
  if (ext === 'cif' || ext === 'mmcif' || ext === 'mcif') return 'cif';
  if (ext === 'mol2') return 'mol2';
  if (ext === 'xyz') return 'xyz';
  if (ext === 'pqr') return 'pqr';
...
function pdbHeader(text: string) {
  ...
  if (header) {
    ...
    if (id) out.push(['PDB ID', id]);
    if (cls) out.push(['Classification', cls]);
    if (date) out.push(['Deposited', date]);
  }
  grab('TITLE', 'Title');
  const exp = text.split('\n').find((l) => l.startsWith('EXPDTA'));
  if (exp) out.push(['Method', exp.slice(10).trim()]);
  const res = text.split('\n').find((l) => /^REMARK\s+2\s+RESOLUTION/.test(l));
  if (res) {
    const m = /RESOLUTION\.\s*([\d.]+)/.exec(res);
    if (m) out.push(['Resolution', m[1] + ' A']);
  }
  return out;
}

export async function renderMolecule(file: File, resultsEl: HTMLElement) {
  ...
  if (!records.length) { resultsEl.appendChild(errorCard('No structure records found in this file.')); return; }

  const infoCard = el('div', { class: 'anr-card' });
  const [h, help] = h3help('Chemical structure',
    'A structure file is a list of atoms with their coordinates, and often a list of which are bonded to which. Drawn here in your browser - the formula and molecular weight below are worked out from the atoms actually present rather than taken from any label in the file.');
  ...
  if (records.length > 1) tbl.appendChild(rowHelp('Records', records.length.toLocaleString(),
    'An SDF is a stack of separate molecules, one after another, each ending with a $$$$ line. Use the picker below the viewer to step through them.'));
  infoCard.appendChild(tbl);
  resultsEl.appendChild(infoCard);

  // ---- viewer ----
  const viewCard = el('div', { class: 'anr-card' });
  const [vh, vhelp] = h3help('Structure',
    'Drag to rotate, scroll to zoom, right-drag to pan. Ball-and-stick shows every atom and bond; the cartoon draws each protein chain as a ribbon following its backbone, with helices and sheets picked out - the only way a structure of tens of thousands of atoms is readable at all.');
  viewCard.appendChild(vh); viewCard.appendChild(vhelp);
  ...
  try { await loadScript(MOL3D_JS); }
  catch (e) { host.innerHTML = ''; host.appendChild(errorCard('The 3D viewer failed to load. Offline?')); return; }
  const $3Dmol = window.$3Dmol;
  if (!$3Dmol) { host.innerHTML = ''; host.appendChild(errorCard('The 3D viewer did not initialise.')); return; }
  ...
  const applyStyle = () => {
    ...
    viewer.render();
    for (const b of controls.querySelectorAll('button[data-style]')) b.classList.toggle('is-active', (b as HTMLElement).dataset.style === style);
    for (const b of controls.querySelectorAll('button[data-colour]')) b.classList.toggle('is-active', (b as HTMLElement).dataset.colour === colour);
    drawLegend();
  };
  ...
  const drawLegend = () => {
    ...
    if (colour === 'element') {
      if (style === 'cartoon') { legend.appendChild(el('span', { class: 'anr-legend-item' }, 'Ribbon coloured from one end of each chain to the other')); return; }
      const common: [string, string][] = [['#909090', 'Carbon'], ['#ffffff', 'Hydrogen'], ['#3050f8', 'Nitrogen'], ['#ff0d0d', 'Oxygen'], ['#ffff30', 'Sulfur'], ['#ff8000', 'Phosphorus']];
      const present = new Set(atoms.map((a) => a.elem));
      for (const [c, name] of common) if (present.has(name === 'Sulfur' ? 'S' : name === 'Phosphorus' ? 'P' : name[0])) legend.appendChild(swatch(c, name));
  ...
  const loadRecord = (index: number) => {
    ...
    if (!style) style = isMacro ? 'cartoon' : 'ball';
    applyStyle();
    viewer.zoomTo();
    viewer.render();
    describe(index);
  ...
  const describe = (index: number) => {
    ...
    const ligands: Record<string, number> = {};
    ...
    for (const a of atoms) {
      ...
      if (a.charge) charge += a.charge;
      if (a.bonds) bonds += a.bonds.length;
      if (a.chain) chains.add(a.chain);
      const resn = String(a.resn || '').toUpperCase();
      if (a.resi != null && a.chain) residues.add(a.chain + ':' + a.resi);
      if (SOLVENT.has(resn)) waters++;
      else if (resn && !AMINO.has(resn) && !NUCLEIC.has(resn)) ligands[resn] = (ligands[resn] || 0) + 1;
      if (a.ss === 'h') ss.h++; else if (a.ss === 's') ss.s++;
    }
    bonds = Math.round(bonds / 2);          // each bond is listed on both atoms

    const [dh, dhelp] = h3help('Composition',
      'Worked out from the atoms in the file. The formula is in Hill notation - carbon first, then hydrogen, then everything else alphabetically - which is how every chemistry database sorts one.');
    detailCard.appendChild(dh); detailCard.appendChild(dhelp);
    const t = el('table', { class: 'anr-readout' });
    t.appendChild(row('Atoms', atoms.length.toLocaleString()));
    if (bonds) t.appendChild(row('Bonds', bonds.toLocaleString()));
    const formula = hillFormula(counts);
    if (formula) t.appendChild(row('Formula', formula));
    if (mass) t.appendChild(rowHelp('Molecular weight', mass.toFixed(2) + ' g/mol',
      'The sum of the standard atomic weights of every atom present. If the file omits hydrogens - which many crystal structures do - this will read low by exactly those.'));
    if (charge) t.appendChild(row('Net charge', (charge > 0 ? '+' : '') + charge));
    const elements = Object.entries(counts).sort((a, b) => b[1] - a[1]);
    t.appendChild(row('Elements', elements.map(([k, v]) => k + ' x' + v).join(', ')));
    if (chains.size) t.appendChild(rowHelp('Chains', String(chains.size),
      'Separate polymer molecules in the structure - two chains often means the protein was crystallised as a pair.'));
    if (residues.size) t.appendChild(row('Residues', residues.size.toLocaleString()));
    if (ss.h || ss.s) t.appendChild(rowHelp('Secondary structure', ss.h.toLocaleString() + ' atoms in helices, ' + ss.s.toLocaleString() + ' in sheets',
      'Helices and sheets are the repeating local shapes a protein backbone folds into. They are what the cartoon view draws.'));
    const ligNames = Object.keys(ligands);
    if (ligNames.length) t.appendChild(rowHelp('Non-polymer groups', ligNames.slice(0, 20).join(', ') + (ligNames.length > 20 ? ' and ' + (ligNames.length - 20) + ' more' : ''),
      'Everything that is not part of a chain: bound drugs and cofactors, metal ions, and whatever the crystallisation buffer left behind.'));
    if (waters) t.appendChild(row('Water molecules', String(waters)));
    ...
    if (fmt === 'pdb') {
      ...
      if (hdr.length) {
        ...
        for (const [k, v] of hdr) ht.appendChild(row(k, v));
      ...
      if (title || fields.length) {
        ...
        if (title) ft.appendChild(row('Name', title));
        for (const [k, v] of fields) ft.appendChild(row(k, v));
  ...
  // ---- controls ----
  const styleBtn = (key: string, label: string) => {
  ...
  const colourBtn = (key: string, label: string) => {
    const b = el('button', { class: 'anr-seg-btn', type: 'button' }, label) as HTMLButtonElement;
  ...
  if (isMacro) styleBtn('cartoon', 'Cartoon');
  controls.appendChild(el('span', { class: 'anr-daw-zoomlabel' }, 'Colour'));
  colourBtn('element', 'Element');
  if (isMacro) { colourBtn('chain', 'Chain'); colourBtn('ss', 'Structure'); }
  const spinBtn = el('button', { class: 'anr-btn anr-btn-sm', type: 'button' }, 'Spin') as HTMLButtonElement;
  ...
  const fitBtn = el('button', { class: 'anr-btn anr-btn-sm', type: 'button' }, 'Fit') as HTMLButtonElement;
  ...
  if (records.length > 1) {
    ...
    records.slice(0, 200).forEach((r, i) => {
      const name = r.split('\n')[0].trim();
      const b = el('button', { class: 'anr-seg-btn', type: 'button', title: name || ('Record ' + (i + 1)) }, String(i + 1)) as HTMLButtonElement;
      b.addEventListener('click', () => {
        for (const other of strip.children) other.classList.remove('is-active');
    ...
    if (strip.firstElementChild) strip.firstElementChild.classList.add('is-active');
    const c = el('div', { class: 'anr-card' });
    const [rh, rhelp] = h3help('Records (' + records.length.toLocaleString() + ')',
      'An SDF holds a stack of separate molecules. Pick one to draw it; the composition below updates with it.');
    c.appendChild(rh); c.appendChild(rhelp);
    c.appendChild(strip);
    if (records.length > 200) c.appendChild(el('p', { class: 'anr-hint' }, 'Showing the first 200 of ' + records.length.toLocaleString() + '.'));
