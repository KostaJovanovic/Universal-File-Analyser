...
const DEPTH_NAME: Record<number, string> = {
...
interface AseCel {
  ...
  w: number;
  h: number;
  /** RGBA for this cel's rectangle, or null when it links to another frame. */
...
interface AseDoc {
  ...
  colorCount: number;
  /** Cels left undecoded because ASE_CEL_PIXEL_BUDGET was spent. */
  celsSkipped: number;
}

// A length-prefixed UTF-8 string: WORD length, then the bytes.
function aseString(r: Reader) {
  const n = r.u16();
...
function celToRgba(px: Uint8Array, w: number, h: number, doc: AseDoc) {
  const n = w * h;
  const out = new Uint8ClampedArray(n * 4);
  if (doc.depth === 32) {
    if (px.length < n * 4) return null;
    ...
    for (let i = 0; i < n; i++) {
      ...
      if (idx === doc.transparentIndex) { out[d + 3] = 0; continue; }
      const p = idx * 4;
      out[d] = doc.palette[p]; out[d + 1] = doc.palette[p + 1];
      out[d + 2] = doc.palette[p + 2]; out[d + 3] = doc.palette[p + 3];
    }
  }
  return out;
}
...
export async function parseAseprite(bytes: Uint8Array): Promise<AseDoc | null> {
  const r = new Reader(bytes, true);
  r.u32();                                  // declared file size
  if (r.u16() !== MAGIC) return null;
  ...
  for (let f = 0; f < frameCount; f++) {
    ...
    for (let c = 0; c < chunkCount; c++) {
      ...
      if (chunkType === 0x2004) {                       // Layer
        ...
        r.u16(); r.u16();                               // default w/h, ignored
        ...
        if (celType === 1) {
          ...
          if (w > 0 && h > 0 && w <= 65535 && h <= 65535) {
            if (celPixels + w * h > ASE_CEL_PIXEL_BUDGET) {
              ...
              let px: Uint8Array | null = r.bytes_(Math.max(0, dataEnd - r.pos));
              if (celType === 2) {
                // A cel inflates to exactly w*h*bpp bytes; never let it run past that.
                try { px = await inflate(px, 'deflate', w * h * bpp, { partial: true }); } catch (_) { px = null; }
              }
              const rgba = px ? celToRgba(px, w, h, doc) : null;
              if (rgba) celPixels += w * h;
        ...
        for (let i = first; i <= last && i < 256 && r.pos + 6 <= dataEnd; i++) {
          const flags = r.u16();
          const p = i * 4;
          doc.palette[p] = r.u8(); doc.palette[p + 1] = r.u8();
          doc.palette[p + 2] = r.u8(); doc.palette[p + 3] = r.u8();
          if (flags & 1) aseString(r);                  // entry name
        }
        if (size > doc.colorCount) doc.colorCount = size;
        ...
        for (let p = 0; p < packets && r.pos + 2 <= dataEnd; p++) {
          ...
          for (let i = 0; i < n && idx < 256 && r.pos + 3 <= dataEnd; i++, idx++) {
            ...
            doc.palette[q] = r.u8() * sc; doc.palette[q + 1] = r.u8() * sc;
            doc.palette[q + 2] = r.u8() * sc; doc.palette[q + 3] = 255;
          }
        }
        ...
        for (let t = 0; t < n && r.pos + 17 <= dataEnd; t++) {
          ...
          const repeat = r.u16();
          r.skip(6);
          r.u8(); r.u8(); r.u8();                       // deprecated tag colour
          r.u8();
    ...
    if (frameBytes > 0) r.seek(frameStart + frameBytes);
  }
...
function blendChannel(mode: number, b: number, s: number) {
  switch (mode) {
...
export function compositeFrame(doc: AseDoc, frameIndex: number) {
...
function pixelScale(w: number, h: number) {
  const longest = Math.max(w, h);
  if (!longest) return 1;
  return Math.max(1, Math.floor(PREVIEW_EDGE / longest));
}

function rgbaToCanvas(rgba: Uint8ClampedArray, w: number, h: number) {
...
function mountPlayer(sprite: AseDoc, file: File, resultsEl: HTMLElement) {
  ...
  const frameAt = (i: number) => {
    let f = cache.get(i);
    if (f) { cache.delete(i); cache.set(i, f); return f; }
  ...
  card.appendChild(spriteH); card.appendChild(spriteHelp);
  ...
  const stop = () => { if (timer !== null) { clearTimeout(timer); timer = null; } playBtn.textContent = '▶ Play'; };
  ...
  const controls = el('div', { style: 'display:flex; gap:8px; align-items:center; flex-wrap:wrap; margin-top:10px;' }, [
    ...
    el('button', { type: 'button', class: 'anr-btn', onclick: () => {
      canvas.toBlob((b) => {
        if (b) downloadBlob((file.name || 'sprite').replace(/\.[^.]+$/, '') + '_frame_' + (current + 1) + '.png', b);
      }, 'image/png');
    } }, 'Save frame (PNG)'),
    label,
  ]);
...
export async function renderAseprite(file: File, resultsEl: HTMLElement) {
  const loader = inlineLoader('Decoding sprite…');
  resultsEl.appendChild(loader);
  let doc: AseDoc | null = null;
  try {
  ...
  if (!doc) {
    resultsEl.appendChild(el('div', { class: 'anr-info' },
      'This file has an Aseprite extension but its header does not parse as one.'));
    return;
  }
  const sprite = doc;
  ...
  if (!drawable) {
    resultsEl.appendChild(el('div', { class: 'anr-info' },
      'The ' + sprite.width + ' × ' + sprite.height + ' px canvas is too large to composite on this device, so the sprite is described below but not drawn.'));
  }

  if (sprite.celsSkipped) {
    resultsEl.appendChild(el('div', { class: 'anr-info' },
      sprite.celsSkipped + ' cel' + (sprite.celsSkipped === 1 ? ' was' : 's were') +
  ...
  if (frameCount > 1) tbl.appendChild(row('Total duration', (totalMs / 1000).toFixed(2) + ' s'));
  ...
  if (sprite.depth === 8) tbl.appendChild(row('Palette colours', sprite.colorCount || 256));
  ...
  if (sprite.layers.length) {
    ...
    lc.appendChild(layerH); lc.appendChild(layerHelp);
    ...
    sprite.layers.forEach((l) => {
      ...
      if (!l.group) bits.push(Math.round(l.opacity / 255 * 100) + '% opacity');
      if (!l.visible) bits.push('hidden');
      if (NON_SEPARABLE.has(l.blendMode)) bits.push('composited as Normal here');
  ...
  if (sprite.tags.length) {
    ...
    tc.appendChild(tagH); tc.appendChild(tagHelp);
