...
export async function renderGeo(file: File, resultsEl: HTMLElement) {
  ...
  const okPt = (lat: number, lon: number) => isFinite(lat) && isFinite(lon);
  g.lines = g.lines.map((l) => l.filter((p: any[]) => okPt(p[0], p[1]))).filter((l) => l.length);
  ...
  const mapStopper = () => {
    setTimeout(() => {
      ...
      if (mapEl.isConnected || stashed) (window._anrMediaStoppers = window._anrMediaStoppers || new Set()).add(mapStopper);
      else { try { map.remove(); } catch (_) {} }
    }, 0);
  };
