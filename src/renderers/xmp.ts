...
function parseXmp(doc: Document) {
  ...
  for (const d of descs) {
    for (const a of d.attributes) {
      if (a.prefix === 'xmlns' || a.name === 'xmlns' || !a.prefix) continue;
      if (a.prefix === 'rdf') continue;
  ...
  for (const e of doc.getElementsByTagName('*')) {
    ...
    if (!p || p === 'rdf' || p === 'x' || p === 'xmlns') continue;
    ...
    if (seq) {
      ...
      if (items.length && !L.has(key)) L.set(key, items);
...
function darktableHistory(doc: Document) {
  ...
  for (const e of doc.getElementsByTagName('*')) {
    ...
    for (const a of e.attributes) {
      if (a.localName === 'operation') op = a.value;
      else if (a.localName === 'enabled') enabled = a.value !== '0';
    }
    if (op) out.push({ op, enabled });
  }
  return out;
}
...
function sliderRow(grid: HTMLDivElement, label: string, value: string, min: number, max: number, fmt?: ((n: number) => string)|null) {
  ...
  if (min < 0) bar.appendChild(el('div', { class: 'anr-xmp-bar-zero', style: `left:${(zero * 100).toFixed(2)}%` }));
...
export async function renderXmp(file: File, resultsEl: HTMLElement) {
  ...
  for (const [label, key, min, max, fmt] of (useLegacy ? BASIC_LEGACY : BASIC_2012)) {
    if (sliderRow(basicGrid, label, get('crs:' + key), min, max, fmt)) basicN++;
  }
  ...
  if (basicN) {
    ...
    const [h, help] = h3help('Develop settings',
      'The Basic panel adjustments this sidecar applies to the raw. Each bar fills from the centre - right (brighter, more) or left (darker, less) - so the shape of the edit is readable at a glance. These are settings, not the finished pixels: the raw itself is untouched.');
  ...
  if (curves.some((c) => !isIdentityCurve(c.pts))) {
    ...
    const [h, help] = h3help('Tone curve',
      'The point curve this sidecar bends light through, from blacks (bottom-left) to whites (top-right). The dashed diagonal is no change; a bump above it lifts those tones, a dip below darkens them. Any red, green or blue line is a per-channel curve on top of the main one.');
    tcCard.appendChild(h); tcCard.appendChild(help);
  ...
  if (hslRows.length) {
    ...
    const [h, help] = h3help('Colour mixer (HSL)',
      'Per-colour hue, saturation and luminance tweaks - the HSL / colour-mixer panel. Only the colours actually adjusted are shown. Each bar fills from the centre; left is a negative shift, right positive.');
    hslCard.appendChild(h); hslCard.appendChild(help);
    const grid = el('div', { class: 'anr-xmp-hsl' });
    grid.appendChild(el('div', { class: 'anr-xmp-hsl-head' }, ''));
    for (const c of ['Hue', 'Sat', 'Lum']) grid.appendChild(el('div', { class: 'anr-xmp-hsl-head' }, c));
    for (const [name, swatch, hh, ss, ll] of hslRows) {
      ...
      grid.appendChild(miniBar(hh)); grid.appendChild(miniBar(ss)); grid.appendChild(miniBar(ll));
    }
  ...
  if (isDarktable) {
    ...
    const [h, help] = h3help('darktable modules',
      'darktable stores its edit as a stack of processing modules rather than named sliders, and the pixel maths lives in an encoded block this reader can\'t re-run. So instead we list the modules the edit switches on - the closest read to what was done.');
    dtCard.appendChild(h); dtCard.appendChild(help);
