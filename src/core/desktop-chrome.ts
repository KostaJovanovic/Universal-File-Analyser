...
const SVG_GLYPH: Record<string, string> = {
...
function icon(name: string, useFont: boolean, extraClass = ''): string {
...
function button(cls: string, label: string, inner: string): HTMLButtonElement {
 ...
 *  full screen. */
function applyState(s: AnrWindowState): void {
 ...
 */
export function mountDesktopChrome(): void {
  const desk = window.anrDesktop;
  if (!desk || !desk.win || !document.body) return;

  const root = document.documentElement;
  root.classList.add('anr-desktop');
  if (desk.platform === 'darwin') root.classList.add('anr-desktop--mac');
  ...
  if (existing) {
    ...
    if (existing !== document.body.firstElementChild) document.body.prepend(existing);
    mountScrollbar();
    return;
  }
  ...
  const maxBtn = button('anr-tb-max', 'Maximise',
    icon('max', useFont, 'anr-tb-ico-max') + icon('restore', useFont, 'anr-tb-ico-restore'));
...
function mountScrollbar(): void {
  if (document.getElementById('anrScrollbar')) return;   // by ID - see mountDesktopChrome
  ...
  const measure = (): void => {
    ...
    if (range < 1 || railH < 1) { rail.classList.add('is-idle'); return; }
  ...
  let queued = false;
  const schedule = (remeasure: boolean): void => {
  ...
  thumb.addEventListener('pointermove', (e: PointerEvent) => {
    if (!thumb.hasPointerCapture(e.pointerId) || range < 1) return;
  ...
  rail.addEventListener('pointerdown', (e: PointerEvent) => {
    if (e.target !== rail || range < 1) return;
