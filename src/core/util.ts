...
export async function desktopFile<T>(f: T): Promise<T | File> {
  ...
  if (!res.ok) throw new Error('Could not open ' + stub.name + ' (' + res.status + ')');
...
export function attachViewCube(viewer: any) {
  ...
  box.addEventListener('pointermove', (e) => {
    ...
    if (!e.buttons) { en(); return; }   // released outside the box before capture began
    mv(e.clientX, e.clientY);
    if (moved && capId != null && !box.hasPointerCapture(capId)) {
      try { box.setPointerCapture(capId); } catch { /* pointer already released */ }
    }
  });
  const endPointer = (e: PointerEvent) => { if (e.pointerType !== 'touch') { en(); capId = null; } };
  ...
  let syncing = false;
  const sync = () => {
    if (!wrap.isConnected) { syncing = false; return; }
    const yd = state.yaw * R2D, pd = state.pitch * R2D;
    cube.style.transform = `rotateX(${-pd}deg) rotateY(${yd}deg)`;
    requestAnimationFrame(sync);
  };
  const startSync = () => { if (!syncing) { syncing = true; requestAnimationFrame(sync); } };
  startSync();
  new IntersectionObserver((entries) => {
    if (entries.some((x) => x.isIntersecting)) startSync();
  }).observe(box);
}
