...
// commit 151 reads "4.0", commit 173 reads "5.0", commit 195 reads "6.0", commit
// 250 reads "7.0", commit 256 reads "8.0" and commit 305 reads "9.0". To crown a
// future 10.0, append its commit number here (keep the
// list sorted ascending, and mirror the RELEASES constant in save.bat).
const RELEASE_COMMITS = [29, 60, 100, 151, 173, 195, 250, 256, 305];
...
let _dockAnims: Animation[] = [];
let _barGhost: HTMLElement | null = null;   // the bar copy dropping in during the glide
let _blurWarm = false;
...
function stopDockAnims() {
  for (const a of _dockAnims) a.cancel();
  _dockAnims = [];
...
function setDocked(header: HTMLElement | null, on: boolean) {
  if (!on) setBarMenu(header, false, false);   // the menu is the bar's; whole, the header shows every chip
  if (header) header.classList.toggle('is-bar', on);
...
function setBarMenu(header: HTMLElement | null, open: boolean, animate = true) {
  if (!header) return;
  const nav = header.querySelector<HTMLElement>('.site-mark-nav');
  const btn = header.querySelector<HTMLElement>('.bar-menu-btn');
  if (_menuAnim) { _menuAnim.cancel(); _menuAnim = null; }
  if (btn) btn.setAttribute('aria-expanded', String(open));
  if (open) header.classList.add('is-menu-open');
  ...
  _menuAnim = nav.animate([{ clipPath: open ? shut : full }, { clipPath: open ? full : shut }],
    open ? { duration: 220, easing: 'cubic-bezier(0.23, 1, 0.32, 1)' } : { duration: 160, easing: 'ease-in' });
  if (!open) _menuAnim.finished.then(() => { header.classList.remove('is-menu-open'); _menuAnim = null; }, () => {});
}
...
function dockHeader() {
  ...
  if (body.classList.contains('anr-docked') || _dockAnims.length || body.classList.contains('anr-prepaint')
      || document.documentElement.dataset.page !== 'home' || !header || !main) return;
  const dock = () => {
  ...
  window.scrollTo(0, 0);
  if (dockReduced() || typeof header.animate !== 'function') { dock(); return; }
  ...
  const opts: KeyframeAnimationOptions = { duration: DOCK_MS, easing: DOCK_EASE, fill: 'forwards' };
  ...
  requestAnimationFrame(() => requestAnimationFrame(() => {
    ...
    if (!body.classList.contains('anr-has-file')) return;   // cancelled meanwhile
    _dockAnims = [
      ...
      header.animate([{ opacity: 1 }, { opacity: 0, offset: 0.6 }, { opacity: 0 }],
...
function undockHeader() {
  stopDockAnims();
  setDocked(document.querySelector<HTMLElement>('.site-header'), false);
...
function returnHeader(reset: () => void, done: () => void) {
  const body = document.body;
  const header = document.querySelector<HTMLElement>('.site-header');
  const main = document.querySelector<HTMLElement>('.site-main');
  if (_returning) return;
  if (dockReduced() || !header || !main || typeof header.animate !== 'function'
      || document.documentElement.dataset.page !== 'home' || !body.classList.contains('anr-has-file')) { done(); return; }
  _returning = true;
  ...
  if (!body.classList.contains('anr-docked')) { stopDockAnims(); body.classList.remove('anr-prepaint'); setDocked(header, true); }
  const below = [main, document.querySelector<HTMLElement>('.site-footer')].filter(Boolean) as HTMLElement[];
  const fades = below.map((n) => n.animate([{ opacity: 1 }, { opacity: 0 }],
    { duration: RETURN_FADE_MS, easing: 'ease-out', fill: 'forwards' }));
  fades[0].finished.then(() => {
    ...
    for (const f of fades) f.cancel();   // the glide holds them at 0 from here
    // Held on the first frame for two frames, as the dock is, so the swapped-in
    // page paints before anything moves.
    for (const a of _dockAnims) a.pause();
    requestAnimationFrame(() => requestAnimationFrame(() => {
      for (const a of _dockAnims) a.play();
...
function mountTouchScrollbar() {
  ...
  const show = () => {
    ...
    raf = requestAnimationFrame(() => {
      raf = 0;
      clearTimeout(idle);
      if (!place()) { bar.classList.remove('is-on'); return; }
      bar.classList.add('is-on');
      if (!dragging) idle = window.setTimeout(() => bar.classList.remove('is-on'), TOUCH_SB_IDLE_MS);
    });
  };
  window.addEventListener('scroll', show, { passive: true });
  window.addEventListener('resize', show);
  thumb.addEventListener('pointerdown', (e) => {
    if (!bar.classList.contains('is-on') || travel <= 0) return;
    e.preventDefault();
    dragging = true;
    clearTimeout(idle);
    startY = e.clientY;
  ...
  thumb.addEventListener('pointermove', (e) => {
    ...
    window.scrollTo({ top: startScroll + (e.clientY - startY) * range / travel, behavior: 'instant' as ScrollBehavior });
  });
  const release = () => {
    if (!dragging) return;
    dragging = false;
...
function boot() {
  ...
  setDocked(document.querySelector<HTMLElement>('.site-header'), restored);
  ...
  async function handleFile(file: File, opts?: { force?: any; sidecarXmp?: any; sniffedExt?: string; [k: string]: any }) {
    ...
    if (!nested && window.anrDesktop) window.anrDesktop.setSubject(file.name);
...
window._anrReadableText = isReadableText;
  ...
  if (!(boot as typeof boot & BootGuards)._once) {
    ...
    document.addEventListener('click', (e) => {
      ...
      if (!header || !t || !t.closest) return;
      if (t.closest('.bar-menu-btn')) { setBarMenu(header, !header.classList.contains('is-menu-open')); return; }
      if (header.classList.contains('is-menu-open') && !t.closest('.nav-search')) setBarMenu(header, false);
    });
    document.addEventListener('keydown', (e) => {
      const header = document.querySelector<HTMLElement>('.site-header');
      if (e.key === 'Escape' && header && header.classList.contains('is-menu-open')) setBarMenu(header, false);
    });
    ...
    document.addEventListener('click', (e) => {
      const t = e.target as Element | null;
      if (!t || !t.closest || !t.closest('.bar-home')) return;
      if (document.documentElement.dataset.page !== 'home' || !document.body.classList.contains('anr-has-file')) return;
    ...
    window.addEventListener('drop', async (e) => {
      ...
      if (document.documentElement.dataset.page === 'home') warmBarBlur();
  ...
  if (window.anrDesktop) {
    ...
    window.anrDesktop.onOpen(async (p: any) => {
      if (!p) return;
      const goHome = () => {
        const home = new URL('/', location.href).href;
      ...
      if (p.kind === 'folder') {
        const entries: any[] & { truncated?: boolean; cancelled?: boolean } =
          (p.entries || []).map((e: any) => ({
            path: e.path,
            size: e.size,
            file: {
      ...
      } catch (_) { hideDropLoader(); return; }
      hideDropLoader();
      const away = goHome();
      if (away || !photoResults) { window._anrPendingFile = file; return; }
      handleFile(file);
    });
  }
