  ...
  for (const node of root.querySelectorAll('*')) {
    for (const attr of [...node.attributes]) {
      ...
      if (NETWORK_ATTRS.has(name)) {
        if (!allowRemote) node.removeAttribute(attr.name);
        else if (isUnsafeUrl(val)) node.removeAttribute(attr.name);
        continue;
      }
      ...
      if (name === 'style' && /url\s*\(|@import/i.test(val)) node.removeAttribute(attr.name);
    }
