...
const LIVE_MAX = 2;
const _live: { close(): void }[] = [];
function trackLive(h: { close(): void }) {
  const i = _live.indexOf(h);
  if (i >= 0) _live.splice(i, 1);
  _live.push(h);
  while (_live.length > LIVE_MAX) _live.shift()!.close();
}
...
export async function extractArchive(file: File) {
  ...
  let reopening: Promise<any> | null = null;
  const ensureOpen = async () => {
    if (archive) return archive;
    if (!reopening) {
      reopening = Archive.open(file).then((a: any) => { archive = a; trackLive(handle); return a; })
  ...
  const handle = {
    ...
    close() {
      ...
      if (i >= 0) _live.splice(i, 1);
      if (archive) { try { archive.close(); } catch (_) {} }
      archive = null;
    },
  };
  // Nothing to extract: no reason to keep the worker (and its copy) alive.
  if (!entries.length) handle.close();
  else trackLive(handle);
  return handle;
}
