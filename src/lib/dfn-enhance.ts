...
export async function enhanceAudio({ channels, runModel, onProgress }: DfnEnhanceInput) {
  ...
  for (let cc = 0; cc < nCh; cc++) {
    ...
    for (let i = 0; i < nSample; i++) padded[padFront + i] = src[i];
    ...
    for (let segStart = 0; segStart < totalFrames; segStart += SEG_CORE) {
      ...
      for (let t = 0; t < P; t++) {
        ...
        for (let i = 0; i < fftSize; i++) frame[i] = padded[off + i];
        stft.frameFwd(frame, fre, fim);
        const sb = t * nBins;
        for (let b = 0; b < nBins; b++) { specRe[sb + b] = fre[b]; specIm[sb + b] = fim[b]; }
        feat.featErb(fre, fim, eBuf);
        const eb = t * nbErb;
        for (let b = 0; b < nbErb; b++) featErb[eb + b] = eBuf[b];
        feat.featCplx(fre, fim, cRe, cIm);
        const reBase = 0 * P * nbDf + t * nbDf;          // real plane
        const imBase = 1 * P * nbDf + t * nbDf;          // imag plane
        for (let kk = 0; kk < nbDf; kk++) { featSpec[reBase + kk] = cRe[kk]; featSpec[imBase + kk] = cIm[kk]; }
      }
      ...
      for (let t = coreStart; t < coreEnd; t++) {
        ...
        for (let kk = 0; kk < nbDf; kk++) {
          ...
          enhRe[kk] = yr; enhIm[kk] = yi;
        }
        // bins nbDf..nBins-1: ERB-masked spectrum (the gained copy)
        const sb = lt * nBins;
        for (let kk = nbDf; kk < nBins; kk++) { enhRe[kk] = gainRe[sb + kk]; enhIm[kk] = gainIm[sb + kk]; }
        ...
        const off = t * hop;
        for (let i = 0; i < fftSize; i++) { outAcc[off + i] += synth[i]; wsum[off + i] += win[i] * win[i]; }
      }

      doneSegs++;
      if (onProgress) onProgress(doneSegs / totalSegs);
    }
