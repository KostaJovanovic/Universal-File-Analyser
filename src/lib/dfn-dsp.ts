...
export function erbWidths(sr = DFN.sr, fftSize = DFN.fftSize, nbBands = DFN.nbErb, minNbFreqs = DFN.minNbErbFreqs) {
  ...
  for (let i = 1; i <= nbBands; i++) {
    ...
    if (nb < minNbFreqs) { freqOver = minNbFreqs - nb; nb = minNbFreqs; }
    else freqOver = 0;
    erb[i - 1] = nb;
    prevFreq = fb;
  }
...
export function makeDfnStft(fftSize = DFN.fftSize, hop = DFN.hop) {
  ...
  function frameInv(re: FloatBuf, im: FloatBuf, out: FloatBuf) {
    ...
    for (let i = 0; i < fftSize; i++) out[i] = rec.re[i] * win[i];
  }
...
export function makeFeatureState(erb: string|any[]) {
  ...
  function featErb(re: FloatBuf, im: FloatBuf, out: FloatBuf) {
    ...
    for (let b = 0; b < nbErb; b++) {
      ...
      let s = 0;
      for (let j = 0; j < bs; j++) { const idx = bc + j; s += (re[idx] * re[idx] + im[idx] * im[idx]) * k; }
...
export function applyErbGain(re: number[]|Float32Array, im: number[]|Float32Array, gains: any[], erb: string|any[]) {
  ...
  for (let b = 0; b < erb.length; b++) {
    ...
    for (let j = 0; j < bs; j++) { re[bc + j] *= g; im[bc + j] *= g; }
    bc += bs;
  }
}
