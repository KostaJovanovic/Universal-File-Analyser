...
export interface PeSection {
  name: string;
  vsize: number;      // size once mapped into memory
  vaddr: number;      // relative virtual address
  rawSize: number;    // size on disk
  rawOff: number;     // file offset of the raw data
  flags: number;      // section characteristics
}

export interface PeMeta {
  entry: number;      // AddressOfEntryPoint (RVA)
  importDlls: number; // count from the import table, 0 if unreadable
  isDotNet: boolean;
  overlaySize: number;
}
...
export interface PackerReport {
  matches: { name: string; kind: string; why: string }[];
  signals: string[];
...
/* A signature is matched on section names, on a marker somewhere in the bytes we
   scanned, or on both. `sections` entries match a section name exactly (after
   lower-casing); `all` requires every listed name, otherwise any one hit counts.
   Markers are searched in the file head and tail only - a full-file scan would
   mean reading a 200 MB binary to identify it. */
interface Sig {
  name: string;
  kind: 'Packer'|'Protector'|'Installer'|'Runtime'|'Toolchain';
  sections?: string[];
  all?: boolean;
  marker?: RegExp;
  note?: string;
...
function flagNames(f: number) {
  const out: string[] = [];
  if (f & 0x20000000) out.push('exec');
  if (f & 0x40000000) out.push('read');
  if (f & 0x80000000) out.push('write');
  if (f & 0x00000020) out.push('code');
  if (f & 0x00000040) out.push('init-data');
  if (f & 0x00000080) out.push('uninit-data');
  return out;
}
...
/* Read a sample of each section's raw bytes and measure its entropy.
   ...
   itself the signal. */
async function measureSections(file: File, sections: PeSection[], entryRva: number): Promise<SectionReport[]> {
  ...
  for (const s of sections) {
    ...
    if (s.rawSize > 0 && s.rawOff > 0 && s.rawOff < file.size) {
      try {
        ...
        if (bytes.length) entropy = shannonEntropy(bytes);
      } catch (_) { entropy = null; }
    }
    out.push({
...
export async function analysePePacking(file: File, sections: PeSection[], meta: PeMeta): Promise<PackerReport|null> {
  ...
  let verdict: string;
  let confidence: PackerReport['confidence'];
  if (packers.length) {
