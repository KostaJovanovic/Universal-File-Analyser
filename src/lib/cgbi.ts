...
/** What a decoded CgBI PNG yields: straight (un-premultiplied) RGBA. */
export interface CgbiImage {
  width: number;
  height: number;
...
function* chunks(b: Uint8Array): Generator<{ type: string; start: number; length: number }> {
  let p = 8;
  const dv = new DataView(b.buffer, b.byteOffset, b.byteLength);
  while (p + 8 <= b.length) {
    const length = dv.getUint32(p, false);
    const type = String.fromCharCode(b[p + 4], b[p + 5], b[p + 6], b[p + 7]);
    if (length > b.length || p + 12 + length > b.length) return;
...
export function isCgbiPng(b: Uint8Array) {
  if (b.length < 16) return false;
  for (let i = 0; i < 8; i++) if (b[i] !== PNG_SIG[i]) return false;
  for (const c of chunks(b)) {
    if (c.type === 'CgBI') return true;
    if (c.type === 'IDAT' || c.type === 'IEND') return false;  // CgBI precedes IHDR
  }
  return false;
}
...
function unfilter(raw: Uint8Array, width: number, height: number, bpp: number) {
...
export async function decodeCgbiPng(b: Uint8Array): Promise<CgbiImage | null> {
  if (!isCgbiPng(b)) return null;
  ...
  if (!bpp) return null;
  if (width * height > CGBI_MAX_PIXELS) return null;
  ...
  let off = 0;
  for (const d of idat) { stream.set(d, off); off += d.length; }
  ...
  for (let i = 0, n = width * height; i < n; i++) {
    ...
    if (bpp === 4) {
      const a = px[s + 3];
      if (a === 0) { rgba[d] = rgba[d + 1] = rgba[d + 2] = rgba[d + 3] = 0; continue; }
      ...
      rgba[d] = px[s + 2]; rgba[d + 1] = px[s + 1]; rgba[d + 2] = px[s]; rgba[d + 3] = 255;
    }
  }
...
export async function cgbiToPngBlob(b: Uint8Array): Promise<Blob | null> {
  const img = await decodeCgbiPng(b);
  if (!img) return null;
  try {
    const cv = document.createElement('canvas');
    cv.width = img.width; cv.height = img.height;
    const data = new Uint8ClampedArray(img.rgba.buffer as ArrayBuffer, img.rgba.byteOffset, img.rgba.length);
    cv.getContext('2d')!.putImageData(new ImageData(data, img.width, img.height), 0, 0);
    return await new Promise<Blob | null>((res) => cv.toBlob((blob) => res(blob), 'image/png'));
  } catch (_) {
    return null;
  }
}
