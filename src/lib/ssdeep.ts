...
function fnv(h: number, c: number) { return ((Math.imul(h, HASH_PRIME) >>> 0) ^ c) >>> 0; }
...
export function ssdeepHash(buf: Uint8Array): string {
  const n = buf.length;
  if (!n) return '3::';

  // Start with a block size that would give roughly SPAMSUM_LENGTH blocks.
  let bs = MIN_BLOCKSIZE;
  while (bs * SPAMSUM_LENGTH < n) bs *= 2;

  const roll = new Roll();
  const sig: string[] = [], sig2: string[] = [];
  ...
  for (let i = 0; i < n; i++) {
    ...
    if (h3 % bs === bs - 1) {
      ...
      sig[j] = B64[h % 64];
      if (j < SPAMSUM_LENGTH - 1) { h = HASH_INIT; j++; }
    }
    if (h3 % bs2 === bs2 - 1) {
      sig2[k] = B64[h2 % 64];
      if (k < SPAMSUM_LENGTH / 2 - 1) { h2 = HASH_INIT; k++; }
    }
  }
  // Too short a signature: a smaller block size is needed.
  if (bs > MIN_BLOCKSIZE && j < SPAMSUM_LENGTH / 2) return ssdeepMultiPass(buf);
  // Whatever is left in the accumulators is a final partial block.
  if (h3 !== 0) { sig[j] = B64[h % 64]; sig2[k] = B64[h2 % 64]; }
...
/* Every block size at once, in one pass - the way ssdeep 2.13+ does it. A
   ...
   loop's. */
function ssdeepMultiPass(buf: Uint8Array): string {
  const n = buf.length;
  const bsOf = (i: number) => MIN_BLOCKSIZE * Math.pow(2, i);
  ...
  const digest: string[][] = [];
  const halfLast: string[] = [];
  h[0] = HASH_INIT; halfh[0] = HASH_INIT; digest[0] = []; halfLast[0] = '';
  let start = 0, end = 1;
  ...
  for (let p = 0; p < n; p++) {
    const c = buf[p];
    r = roll.update(c);
    for (let i = start; i < end; i++) { h[i] = fnv(h[i], c); halfh[i] = fnv(halfh[i], c); }
    ...
    for (let i = start; i < end; i++) {
      ...
      if (dlen[i] === 0 && end < NUM_BLOCKHASHES) {
        h[end] = h[end - 1]; halfh[end] = halfh[end - 1]; digest[end] = []; halfLast[end] = '';
        end++;
      }
      // The last slot is overwritten rather than appended once the signature is
      // full, which is what keeps it to SPAMSUM_LENGTH characters exactly.
      ...
      if (dlen[i] < SPAMSUM_LENGTH - 1) {
        dlen[i]++;
        h[i] = HASH_INIT;
        if (dlen[i] < SPAMSUM_LENGTH / 2) { halfh[i] = HASH_INIT; halfLast[i] = ''; }
  ...
  let bi = start;
  while (bsOf(bi) * SPAMSUM_LENGTH < n && bi < NUM_BLOCKHASHES - 1) bi++;
  if (bi >= end) bi = end - 1;
  while (bi > start && dlen[bi] < SPAMSUM_LENGTH / 2) bi--;

  // Whatever is left in the accumulators is a final partial block.
  let s1 = digest[bi].slice(0, dlen[bi]).join('');
  if (r !== 0) s1 += B64[h[bi] % 64];
  else if (digest[bi][dlen[bi]] !== undefined) s1 += digest[bi][dlen[bi]];

  let s2 = '';
  if (bi < end - 1) {
    const b2 = bi + 1;
    s2 = digest[b2].slice(0, Math.min(dlen[b2], SPAMSUM_LENGTH / 2 - 1)).join('');
    if (r !== 0) s2 += B64[halfh[b2] % 64];
    else if (halfLast[b2]) s2 += halfLast[b2];
  } else if (r !== 0) {
    s2 = B64[h[bi] % 64];
  }
...
export async function ssdeepFile(file: File, maxBytes: number): Promise<string|null> {
  if (!file || file.size > maxBytes) return null;
  try { return ssdeepHash(new Uint8Array(await file.arrayBuffer())); }
  catch (_) { return null; }
}
...
function eliminateSequences(s: string) {
  let out = '';
...
function hasCommonSubstring(a: string, b: string) {
  if (a.length < ROLLING_WINDOW || b.length < ROLLING_WINDOW) return false;
  const seen = new Set<string>();
  for (let i = 0; i + ROLLING_WINDOW <= a.length; i++) seen.add(a.substr(i, ROLLING_WINDOW));
  for (let i = 0; i + ROLLING_WINDOW <= b.length; i++) if (seen.has(b.substr(i, ROLLING_WINDOW))) return true;
  return false;
}
...
function scoreStrings(s1: string, s2: string, blockSize: number) {
...
export function ssdeepCompare(a: string, b: string): number {
  if (!a || !b) return 0;
  const pa = a.split(':'), pb = b.split(':');
  if (pa.length < 3 || pb.length < 3) return 0;
  const bs1 = parseInt(pa[0], 10), bs2 = parseInt(pb[0], 10);
  if (!isFinite(bs1) || !isFinite(bs2) || bs1 < MIN_BLOCKSIZE || bs2 < MIN_BLOCKSIZE) return 0;
  ...
  if (bs1 === bs2 * 2) return scoreStrings(a1, b2, bs1);
