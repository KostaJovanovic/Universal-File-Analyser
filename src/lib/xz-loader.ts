...
function xzDeclaredSize(b: Uint8Array) {
  const n = b.length;
  if (n < 32 || b[n - 2] !== 0x59 || b[n - 1] !== 0x5A) return -1;          // footer magic "YZ"
  const backward = (b[n - 8] | (b[n - 7] << 8) | (b[n - 6] << 16) | (b[n - 5] << 24)) >>> 0;
  const indexSize = (backward + 1) * 4;
  const idx = n - 12 - indexSize;
  if (idx < 12 || b[idx] !== 0x00) return -1;                               // index indicator
  ...
  for (let i = 0; i < count; i++) {
    if (varint() < 0) return -1;                                            // unpadded size
    const u = varint();
    if (u < 0) return -1;
    total += u;
  }
...
export async function xzDecompress(bytes: Uint8Array, maxOut: number = DECOMP_OUTPUT_MAX) {
  try {
    ...
    if (declared > maxOut) return null;
    let pre: Uint8Array | null = declared > 0 ? new Uint8Array(declared) : null;
    let preLen = 0;
    ...
    for (;;) {
      ...
      if (value && value.byteLength) {
        ...
        if (pre && preLen + value.byteLength <= pre.length) {
          ...
          if (pre) { chunks.push(pre.subarray(0, preLen)); pre = null; }
    ...
    if (pre) return preLen === pre.length ? pre : pre.subarray(0, preLen);

    const out = new Uint8Array(total);
    let off = 0;
    for (let i = 0; i < chunks.length; i++) { const c = chunks[i]!; out.set(c, off); off += c.byteLength; chunks[i] = null; }
    return out;
  } catch (_) {
    return null;
  }
