...
| `ifc.js` | IFC / BIM building models | Hand-written STEP (ISO 10303-21) parser, index-based not token-allocating. An IFC stores no hierarchy - the spatial tree is assembled by following `IfcRelAggregates` / `IfcRelContainedInSpatialStructure`. 3D reuses `stl.js`'s viewer and draws extruded area solids (ear-clipped caps), explicit meshes and mapped items (without which every window and door vanishes). Boolean results are counted and named, never evaluated - drawing the first operand would show a wall with no window in it |
| `molecule.js` | Chemical structures (MOL/SDF/MOL2/XYZ/PDB/mmCIF/CIF/PQR/GRO) | 3Dmol.js (BSD-3, vendored, lazy, Everything tier). Default style is picked from the file - ball-and-stick for a small molecule, cartoon ribbons where amino-acid/nucleotide residues are present, with ligands drawn as sticks so they don't vanish with the cartoon. Formula (Hill notation) and molecular weight are COMPUTED from the atoms present, not read from any label |
| `mlmodel.js` | ML models (ONNX, TF GraphDef, safetensors, GGUF, PyTorch, Keras) | Graphs are decoded by `lib/onnx.js` (protobuf wire reader + the documented ONNX/GraphDef field numbers) and drawn as a layered DAG - column = 1 + the DEEPEST producer, so a skip connection doesn't drag a late node forward. Protobuf stores no edge list; edges come from matching output names to input names. Weight files read through their headers only (`GGUF_HEADER_MAX`), so a 40 GB GGUF opens. `.pt`/`.pth` pickle GLOBAL opcodes are read as bytes and NEVER executed, with imports outside the expected set flagged |
