...
| Module | Handles | Notes |
|---|---|---|
| `video.js` | Video module entry point | Playback, container/codec detection, frame rate, frame capture (routed to `photo.js`), audio track extraction (waveform+spectrogram via `audio.js`), scene-change detection and a **content timeline** (movie barcode + brightness curve + black-frame/freeze flags, `buildContentTimelineCard`, sharing the scene sampler's seek loop) |
