...
- [/] **Multi-file / batch drop** (app.js) - PARTIAL: dropping several files now analyses the **first** one cleanly and ignores the rest (app.js:1041, a deliberate fix for the old render-on-top-of-each-other behaviour), and `/compare` handles exactly two. RAW+XMP pairing is the one real multi-file case. Still needed: keep each file's analysis on screen at once (tabs/stack) + a batch report.
...
- [ ] **GPX heatmap overlay** (geo.js) - Map data renders (track line + elevation profile, `elevationProfileCanvas` at geo.js:69) but does not show activity hotspots / track density.
...
- [ ] **SQLite query UI** (existing SQLite lib) - lib/sqlite.js still exposes only fixed read-only queries (schema + row counts + a 5-row sample); add a user-editable SQL box that runs arbitrary `SELECT`s.
- [/] **ASS/SSA styled subtitle rendering** (subtitles.js) - PARTIAL: override tags are now parsed and applied rather than stripped - `applyAssTags()` (subtitles.js:123) handles `\b`, `\i`, `\c`/`\1c` and `\r`, and `renderAssCue()` (:138) renders each cue from its style's base look with `\N` breaks. Still ignored: **positioning** (`\pos`, `\an`), **karaoke** (`\k`) and drawing commands - text is preserved, so it is a faithful look, not a full layout.
...
- [ ] **Fuzzy hashing (ssdeep / CTPH)** - Similarity matching against known corpora. (Still only exact hashes - MD5/SHA-1/SHA-256/SHA-512.)
- [ ] **PE packer identification** (parsers-security / dev) - Heuristic UPX/Themida detection via section names + entropy spikes. parsers-dev.js does PE container ID + section/import analysis, and `entropyProfile()` now exists to build the entropy half on - so this is cheaper than it was.
