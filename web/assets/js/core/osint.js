...
export function extractIndicators(text, opts = {}) {
    ...
    const add = (k, v) => { if (v && !seen[k].has(v) && out[k].length < cap) {
        seen[k].add(v);
        out[k].push(v);
    } };
    ...
    while ((mm = domRe.exec(work))) {
        ...
        if (FILE_TLDS.has(mm[1].toLowerCase()))
            continue;
        if (/^\d+(?:\.\d+)+$/.test(dom))
            continue;
        add('domains', dom);
...
export function osintCard(ind, opts = {}) {
    ...
    const [h, help] = h3help('Network indicators', 'Each value carries one-click links to public OSINT lookup services (such as VirusTotal, urlscan, AbuseIPDB and Shodan). '
        + 'They open in a new tab, and Analyser sends nothing until you click one - so the no-upload promise holds.');
    card.appendChild(h);
    card.appendChild(help);
    card.appendChild(el('p', { class: 'anr-hint' }, 'URLs, IPs, domains and email addresses found in this file’s text. Nothing is sent until you click a lookup link.'));
    const section = (title, items, kind) => {
        ...
        if (items.length > limit)
            card.appendChild(el('p', { class: 'anr-hint', style: 'margin-top:4px;' }, 'Showing first ' + limit + ' of ' + items.length + '.'));
    };
...
export function buildOsintCard(text, opts) {
    try {
        return osintCard(extractIndicators(text || '', opts), opts);
    }
    catch (_) {
        return null;
    }
