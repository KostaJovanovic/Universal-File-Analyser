...
(function () {
    ...
    function swap(doc, arrivingHome) {
        ...
        if (oldMain && newMain) {
            ...
            try {
                oldMain.querySelectorAll('audio, video').forEach(function (m) { try {
                    m.pause();
                }
                catch (_) { } });
            }
            ...
            if (window._anrMediaStoppers) {
                for (var stop of window._anrMediaStoppers) {
                    try {
                        stop();
                    }
                    catch (_) { }
    ...
    function navigateTo(url, push) {
        ...
        fetch(url)
            ...
            document.startViewTransition(function () {
                ...
                if (push)
                    history.pushState({ anrNav: 1 }, '', url);
    ...
    document.addEventListener('click', function (e) {
        ...
        if (link.closest('.anr-results, [data-anr-untrusted]'))
            return;
        ...
        if (u.pathname === '/docs' || u.pathname.indexOf('/docs/') === 0)
            return;
        var url = u.href;
