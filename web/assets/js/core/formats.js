/* Analyser - central format catalog
   ...
   e.g. adding SketchUp ".skp".
     1. Add it to the right IDENTIFICATION row's `exts`, and add the software
        ...
        If no existing row fits, add a new row - to IDENTIFICATION_CORE for a
        well-known proprietary format, or IDENTIFICATION_EXTENDED for the long
        tail (one row per parsers/ chunk domain). IDENTIFICATION is just the two
        concatenated; never add to it directly.
     2. In renderers/proprietary-formats.ts, add a FORMATS entry:
        `skp: { app: 'SketchUp', chunk: 'threed' }`. That file's header documents
        the four keys an entry may carry.
     3. If there is a header worth decoding, write the parser in
        parsers/parsers-<chunk>.ts and register it in that chunk's PARSERS map.
        The contract lives in parsers/parser-util.ts.
   ...
   Rare. The catalog part is still just a FULL_ANALYSIS row here; the wiring
   checklist (renderer module → classifyFile() case → ROUTES row → sw.js SHELL →
   src/CLAUDE.md inventory) is at the ROUTES table in core/app.ts.

   --- Case D: one extension, several unrelated formats ---
   .ts is TypeScript or an MPEG transport stream; .nc is CNC G-code or NetCDF.
   Declare these in EXT_VARIANTS below - the single source of truth, read by
   detectVariant() at drop time, by VARIANT_REROUTE in app.js when the wrong
   renderer would otherwise get the file, and by tools/prerender-format-pages.mjs
   which renders one /formats/<ext> page with a section per variant.

   Field reference for a catalog row { label, exts, tags, desc, note?, depth? }:
     label - category name (first column). A NEW label must also be mapped in
             CAT_OF below to a CATEGORIES key, or it silently falls back to
             'system'.
     exts  - space-separated extensions, curated casing (e.g. "WebP", "glTF")
     tags  - extra search keywords: brand/software names + synonyms. This is what
             makes searching by origin work ("solidworks" finding SLDPRT).
     desc  - one keyword-rich sentence. NOT decoration: it is the indexable SEO
             text behind "how to open a .X file" searches AND the body of the
             generated /formats/<ext> page, so name the software and say what
             Analyser actually does. Site writing convention applies - British
             spelling, no em-dashes, spaced hyphen as the separator.
     note  - optional prose shown instead of the ext list on the about page,
             where a bare list undersells the feature (e.g. PDF)
     depth - 'partial' marks a format that is read but not fully

   Changing a `desc` changes a live public page: the /formats hub, the
   per-extension pages and the sitemaps are regenerated from this catalog on
   every commit. Nothing here is internal-only.
   ============================================================================ */
...
export const FULL_ANALYSIS = [
    ...
    { label: 'XMP sidecar', exts: 'XMP', tags: 'xmp sidecar raw develop settings edit recipe non-destructive adobe camera raw acr lightroom classic darktable rawtherapee bridge crs process version tone curve hsl colour mixer color mixer white balance temperature tint exposure contrast highlights shadows whites blacks texture clarity dehaze vibrance saturation sharpening noise reduction crop straighten keywords star rating colour label metadata rdf xml dublin core', desc: 'Open the XMP sidecar (.xmp) that Lightroom, Adobe Camera Raw and darktable write next to a raw photo to keep its edits while the raw stays untouched, and see the develop recipe visualised: the tone curve drawn as a real curve, the Basic-panel sliders (exposure, contrast, highlights, shadows, whites, blacks, texture, clarity, dehaze, vibrance and saturation) as bipolar bars, the HSL colour mixer, and the crop rectangle. Reads the editor and process version, the paired raw file, white balance, star rating and colour label, the camera and lens metadata copied from the raw, and the catalog title, keywords and copyright - all in the browser. A darktable sidecar lists the processing modules its edit switches on.' },
    { label: 'Illustrator', depth: 'partial', exts: 'AI', tags: 'adobe illustrator ai vector artwork graphics design eps pdf compatible logo poster artboard postscript', desc: 'Open Adobe Illustrator artwork (.ai) in the browser. Modern Illustrator files (version 9 and later) are PDF-compatible, so Analyser renders the artwork page by page with pdf.js - the vector art, text and embedded images - and reads the document metadata. Older EPS/PostScript-based .ai files are identified from their header.' },
    { label: 'Photoshop', depth: 'partial', exts: 'PSD PSB', tags: 'adobe photoshop psd psb large document photo editing layers composite blend mode opacity raster image design', desc: 'Open Adobe Photoshop documents (PSD and PSB) in the browser: the flattened composite image Photoshop bakes in is shown, plus the full layer tree with each layer name, blend mode, opacity, visibility and a per-layer thumbnail. Reads the canvas dimensions, colour mode and bit depth. Files saved without Maximize Compatibility have no composite, so their layers are shown instead.' },
    { label: 'Raster art', depth: 'partial', exts: 'KRA Procreate PDN', tags: 'krita kra digital painting procreate ipad apple pencil brush canvas merged image preview artwork raster paint paint.net pdn layers', desc: 'Open Krita (.kra), Procreate and Paint.NET (.pdn) paintings by showing the flattened preview baked into the file - the full merged image for Krita, the QuickLook thumbnail for Procreate, the embedded PNG preview for Paint.NET - alongside the canvas size, layer count and app version. Their per-layer artwork is in a private format, so the embedded preview is the faithful render.' },
    { label: 'Tracker music', exts: 'MOD XM IT S3M MPTM MTM STM 669 667 FAR OKT MED UMX MO3 AMS C67 DBM DIGI DMF DTM ETX FC FC13 FC14 FMT FST FTM GDM GMC GTK GT2 ICE IMF IMS J2B M15 MMCMP MMS MT2 MUS NST OXM PLM PT36 PTM PUMA RTM SFX SFX2 SMOD ST26 STK STX SYMMOD TCB ULT UNIC WOW XMF XPK', tags: 'tracker module chiptune demoscene amiga protracker noisetracker soundtracker fasttracker impulse tracker scream tracker screamtracker openmpt modplug milkytracker schism octamed digibooster farandole oktalyzer ultratracker multitracker madtracker digitrakker imago orpheus graoumf future composer symphonie unreal umx mo3 compressed pattern order sample instrument channel row effect column amiga paula 4-channel demo scene keygen music libopenmpt render', desc: 'Play and analyse tracker modules - MOD, XM, IT, S3M and around sixty relatives from the demoscene and the Amiga and DOS eras. A module is not recorded audio: it is a bank of instrument samples plus a pattern score saying which sample to trigger, on which channel, at which row, so playing one means running a tracker engine. Analyser renders the song on your device with libopenmpt - the engine behind OpenMPT - and then treats the result like any other audio, so you get the full Sound section: waveform, spectrogram, transport, loudness, key and BPM. Above it sits the module itself - the tracker that wrote it, the channel, pattern and order counts, the song message, and the sample and instrument name lists, which is traditionally where authors left their greetings. The rendered song can be saved as a WAV.' },
    { label: 'Terraria world', exts: 'WLD', tags: 'terraria world save tile map seed expert master journey corruption crimson hallow dungeon jungle ocean underworld ore vein chest npc biome relogic game save sandbox re-logic', desc: 'Open a Terraria world save (.wld) and see the map drawn from its own tile data - one pixel per tile, with terrain and ore veins in the foreground, walls shaded behind them, and water, lava and honey in their own colours. Reads the world name, the seed it was generated from, its size and difficulty (Classic, Expert, Master or Journey) and the game version that last saved it, and the map can be saved as a PNG. Worlds from Terraria 1.3 and 1.4 are drawn; the .wld extension is shared with the Esri world file that georeferences an image, and the two are told apart by their bytes.' },
    { label: 'GIMP', exts: 'XCF', tags: 'gimp xcf layered image layers layer mask blend mode composite open source photo editor gnu image manipulation program indexed grayscale rle zlib tiles free software', desc: 'Open GIMP documents (.xcf) in the browser. Unlike a Photoshop file, an XCF stores no flattened image at all - it is only the layer stack - so Analyser composites the picture itself: it decodes each layer’s tiles (uncompressed, RLE or zlib), applies that layer’s mask, opacity and blend mode, and paints the stack bottom-up. Shows the full layer list with each layer’s mode, opacity, size, offset and mask, plus the canvas, colour mode, XCF version and palette, and the composite can be saved as a PNG. RGB, grayscale and indexed images are all handled; files saved at 16- or 32-bit precision are described rather than drawn.' },
    { label: 'Pixel art', exts: 'Aseprite ASE', tags: 'aseprite libresprite pixel art sprite sheet spritesheet animation frames cels layers blend mode onion skin indexed palette game art 8-bit 16-bit retro tags loop ping-pong gamedev', desc: 'Open Aseprite and LibreSprite pixel-art sprites (.aseprite / .ase) in the browser. Every frame is composited from the layer stack - these files store only the cels that changed, plus links back to earlier frames, so a frame has to be assembled rather than simply read - and the animation plays at the per-frame durations the file carries, not a single frame rate. Shows the layer tree with each layer’s blend mode, opacity and visibility, the named animation tags with their loop direction, and the canvas, colour depth and palette. Any frame can be saved as a PNG. RGBA, grayscale and indexed sprites are all decoded.' },
    { label: 'AI models', exts: 'ONNX Safetensors GGUF GGML GGJT PT PTH CKPT Keras H5 HDF5', tags: 'onnx open neural network exchange machine learning model graph computation node operator opset tensor weights safetensors huggingface gguf ggml llama.cpp quantisation quantization q4 q5 q8 llm large language model pytorch checkpoint pickle state dict keras tensorflow hdf5 layer parameters inference deep learning neural network transformer convolution', desc: 'Open machine-learning models and read what is actually in them. ONNX models and frozen TensorFlow graphs are drawn as a graph: every operation as a box, every tensor flowing between them as a line, laid out left to right by depth so parallel branches sit side by side. Neither format stores the connections - a node follows another because one of its inputs is one of the other\'s outputs - so the edges are recovered by matching those names. Alongside the picture: the operation mix that fingerprints the architecture, the input and output shapes (including the dynamic ones a model leaves open), every weight tensor with its shape, and the total parameter count. Weight files are read too - Safetensors and GGUF give up their full tensor list, precision and parameter count, and a GGUF also declares its architecture, context length and quantisation. A PyTorch checkpoint is a Python pickle, which is a program rather than a document, so nothing in it is ever run: the opcodes are read as bytes and every module it would import is listed, with anything a file of numbers has no business touching flagged. Keras models have their layer list read from the architecture JSON.' },
    { label: 'DAW project', exts: 'ALS ALP RPP RPP-BAK', tags: 'ableton live set reaper cockos daw session arrangement timeline clip track tempo bpm time signature vst vst3 plugin midi audio track group return master session music production sample media', desc: 'Open Ableton Live sets (.als/.alp) and Reaper projects (.rpp) and see the arrangement drawn out: one row per track, every clip positioned exactly where the project puts it, on a real clock with a zoom for busy sections. A project file contains no audio at all - it is an edit list - so what is shown is read from the same numbers the DAW lays the session out with. Reads the tempo, time signature, track list with each track\'s type, clip count and plugins, and the media files the project points at but does not contain, which is what tells you whether a session that has been moved will still open complete. An Ableton set is gzipped XML and times everything in beats, so the tempo is what makes a real timeline possible; a Reaper project is plain text and already stores seconds.' },
    { label: 'Sketch', exts: 'Sketch', tags: 'sketch bohemian coding design ui ux wireframe mockup artboard symbol component instance layer group shape text style design system macos vector prototype figma alternative page slice hotspot', desc: 'Open Sketch documents (.sketch) and read the whole design. The file is a ZIP of JSON, so nothing has to be guessed at: every page, artboard, group and layer appears in a browsable component tree with its type, child count and size in points. Components (Sketch calls them symbols) are listed with how many instances each one has, so you can see which parts of a design system are actually load-bearing and which are unused. All the copy in the document is pulled out into one readable list with the page each string sits on, the bitmaps placed into the design are extracted and shown, and the preview Sketch saves inside the file is displayed alongside the app version, fonts used, colour assets and shared styles.' },
    ...
    { label: 'Apple iWork', depth: 'partial', exts: 'Pages Numbers Keynote', tags: 'apple iwork pages numbers keynote key macos ios ipad icloud word processing page layout spreadsheet presentation slides quicklook preview pdf', desc: 'Open Apple iWork documents - Pages (word processing and page layout), Numbers (spreadsheets) and Keynote (presentations, .key) - by showing the QuickLook preview Apple embeds in the file: a PDF rendered page by page, or the preview image. Reads the document type and the iWork app version that wrote it. The document body itself is stored in Apple’s undocumented Snappy / Protocol-Buffer .iwa format, which is not re-rendered.' },
    { label: '3D model', exts: 'STL OBJ PLY OFF STEP STP IGES IGS BREP 3MF AMF glTF GLB MTL', tags: 'stl obj wavefront ply stanford off step stp iges igs brep 3mf amf gltf glb gltf 2.0 khronos binary gltf mtl material library texture map newmtl 3d model mesh print cad solidworks fusion catia inventor freecad opencascade tessellation assembly bambu prusa orcaslicer slicer scan point triangle viewer webgl ar augmented reality blender sketchfab wireframe topology orthographic thumbnail plate preview texture package image', desc: 'View STL, OBJ, PLY, OFF, STEP, IGES, BREP, 3MF, AMF and glTF/GLB models in an interactive WebGL viewer with orbit, orthographic or perspective projection, a wireframe topology overlay, an orientation cube, triangle count, bounding box, surface area and volume. glTF 2.0 and binary GLB scenes are parsed natively - the node graph is flattened, mesh geometry shown, and the authoring tool, version and mesh/material/animation counts read. STEP/IGES/BREP are tessellated with OpenCASCADE and STEP shows its originating CAD system, version and AP203/214/242 protocol; 3MF and AMF let you inspect each model and assembly on the build plate individually, and any pictures a 3MF package carries - its own thumbnail, the build-plate renders slicers such as Bambu Studio and OrcaSlicer write, and texture maps - are extracted and shown with it. Wavefront material libraries (MTL) are broken out into each material with its colours, shininess, opacity and the texture files it references.' },
    { label: 'Fusion 360', depth: 'partial', exts: 'F3D F3Z', tags: 'autodesk fusion 360 f3d f3z cad parametric solid model part design assembly archive shapemanager brep preview thumbnail mechanical mcad zstd', desc: 'Open an Autodesk Fusion 360 design (.f3d) or archive (.f3z) and read what it holds: the document type (part, assembly, drawing or sheet metal), the Fusion document version, the number of solid bodies and embedded appearance assets, and the thumbnail Fusion rendered into the file - shown as the model. The editable geometry is stored as Autodesk ShapeManager BREP, a proprietary format that cannot be rebuilt in the browser, so export to STL, OBJ, STEP or 3MF for the full 3D viewer.' },
    { label: 'SolidWorks', depth: 'partial', exts: 'SLDPRT SLDASM SLDDRW', tags: 'solidworks dassault systemes sldprt sldasm slddrw part assembly drawing cad parasolid shapemanager preview thumbnail metadata ole2 compound file mechanical mcad', desc: 'Open a SolidWorks part (.sldprt), assembly (.sldasm) or drawing (.slddrw). Older (pre-2015) files are OLE2 documents: Analyser reads the saved preview thumbnail and the document metadata (title, author, who last saved it and when). Files saved by SolidWorks 2015 or later are encrypted by Dassault Systemes, so they are identified but their contents cannot be read outside SolidWorks - in either case the editable Parasolid geometry stays proprietary, so export to STEP, STL or 3MF for the full 3D viewer.' },
    { label: 'Colour LUT', exts: 'CUBE LOOK', tags: 'lut colour color look up table cube 3d lut 1d lut iridas adobe resolve davinci premiere film emulation log to rec709 rec.709 look grade grading colour grade color grade teal orange cinematic trilinear domain hald clut speedgrade iridas look .look lumetri grade stack baked shaders basic correction tint wheels curves vignette secondary', desc: 'Open colour LUTs (.cube - the Adobe/Iridas/Resolve 3D and 1D look-up tables) and Adobe SpeedGrade / Iridas looks (.look) and see exactly what they do: the neutral tone-response curve that reveals the contrast and colour cast, a before/after of a hue x brightness chart and memory colours (skin, sky, foliage) pushed through the LUT, and an interactive 3D scatter of the colour cube it defines. A .look is read as the SpeedGrade grade stack (basic correction, tints, wheels, curves, secondaries, vignette and more) with the baked 3D LUT it carries visualised the same way. Reads the title, grid size, input domain and extended (HDR / scene-linear) output range, all in the browser. The same .cube extension that Gaussian uses for volumetric DFT data is detected and identified separately.' },
    { label: 'G-code', exts: 'GCODE GCO G NC NGC TAP CNC DIN', tags: 'gcode g-code 3d printing slicer prusa prusaslicer superslicer orca orcaslicer cura ideamaker bambu bambustudio simplify3d slic3r kirimoto reconstruct toolpath extrusion extruder print preview visualise visualiser cnc mill router lathe laser plasma engraver fusion 360 mastercam grbl marlin klipper reprap fanuc haas vectric carbide lightburn arc g2 g3 layer wireframe webgl din din66025 din 66025 iso 6983 heidenhain sinumerik siemens conversational nc program', desc: 'Reconstruct and visualise the printed (or machined) object straight from G-code: every extruded move is drawn as a line in an interactive WebGL viewer, height-coloured and Z-up, with a build-height scrubber to peel the print back layer by layer. Works universally across slicers (PrusaSlicer, SuperSlicer, OrcaSlicer, Cura, ideaMaker, Bambu Studio, Simplify3D) and CNC / laser CAM, handling absolute and relative moves, inch and millimetre units, and G2/G3 arcs; travel moves are separated out. Reads the slicer or CAM tool, object size, layer count and height, filament or cut-path length, feedrate range, and nozzle/bed temperatures. NC programs written to DIN 66025 (the German standard behind ISO 6983, used by Heidenhain and Siemens Sinumerik controls and commonly saved as .din) are read by the same viewer.' },
    ...
    { label: 'Data', exts: 'CSV TSV', tags: 'spreadsheet data table workbench chart pivot sort filter group by', desc: 'Open CSV and TSV tables in a full analysis workbench: a virtualised grid with sort, filter, per-column and global search, column show/hide/reorder and column splitting, a group-by summary, and a chart builder (line, bar, scatter, histogram, pie, area, box and heatmap) with PNG/JSON/CSV export.' },
    { label: 'Vector graphics', exts: 'SVG', tags: 'svg scalable vector graphics xml markup design illustration icon logo artwork', desc: 'View or rasterise SVG (Scalable Vector Graphics) - the XML-based vector format used for icons, logos and illustrations - and inspect its markup, in the browser.' },
...
export const IDENTIFICATION_EXTENDED = [
    { label: 'Developer / data', exts: 'JWT JSONL NDJSON DIFF PATCH WASM CLASS NPY MAP SQL SLN CSPROJ VBPROJ FSPROJ VCXPROJ Gradle TF TFState EditorConfig PROTO GraphQL GQL SARIF PYC PLIST', tags: 'developer code json web token jwt auth webassembly wasm java class bytecode numpy npy source map sourcemap sql dump database visual studio solution sln dotnet msbuild csproj terraform tfstate protobuf protocol buffers graphql sarif python pyc property list plist apple serialization', desc: 'Identify and read metadata from developer and data files: JWT tokens (header + claims + expiry), WebAssembly, Java class files, NumPy arrays, source maps, SQL dumps (dialect, tables, columns, INSERT counts), Visual Studio/.NET projects, Terraform, Protobuf, GraphQL, SARIF, Python bytecode, and Apple property lists (XML + binary). Machine-learning models (ONNX, Safetensors, GGUF, PyTorch, Keras) now open in a full viewer - see AI models above; Jupyter notebooks (IPYNB) and HAR captures do too - see Notebooks & data.' },
    ...
    { label: 'Archives (packages)', exts: 'CPIO A LIB WHL NUPKG CRX XPI VSIX ASAR APPX MSIX APKG CONDA DEB RPM GEM CAB WSP ACE ARJ LZH LHA ZOO ARC', tags: 'archive package installer compression cpio initramfs unix ar static library python wheel pip pypa nuget dotnet chrome extension firefox addon vs code vsix electron asar windows app package msix anki conda anaconda debian ubuntu apt dpkg redhat fedora rpm rubygems gem microsoft cabinet sharepoint solution package wsp stsadm ace arj lha lharc zoo arc lib coff import library visual studio msvc msbuild link.exe linker object file obj webview2', desc: 'Read software packages and Unix archive streams: Python wheels, NuGet, Chrome/Firefox/VS Code extensions, Electron ASAR, Windows APPX/MSIX, Debian (DEB), RPM, RubyGems, conda, Anki, Microsoft CAB, cpio, Unix ar and Microsoft COFF libraries (.lib - telling a static library apart from a DLL import library, with the target architecture and the DLLs it binds to) - showing name, version, dependencies and the file tree.' },
    { label: 'Calendar / contacts', exts: 'ICS ICAL IFB VCF VCARD VCS LDIF CONTACT MSG PST OST NSF EDB DBX', tags: 'icalendar ics calendar event meeting invite vevent vtodo rrule google calendar vcard contact address book vcf vcs photo logo embedded picture base64 ldif ldap directory pst ost exchange ese lotus notes domino nsf dbx outlook express pim personal information', desc: 'Open personal-information files: iCalendar .ics/.ical and .vcs (events, times, recurrence, organiser/attendees), vCard .vcf contacts (fields, plus every photo and company logo stored inside the file, each extracted at full size and saveable), LDIF directory exports and Windows .contact - with Outlook .msg/.pst, IBM Notes .nsf, Exchange .edb and Outlook Express .dbx identified. Email messages (.eml/.emlx/.mbox) now open in a full viewer - see Email above.' },
    { label: 'Security / keys / certs', exts: 'KEY PUB P8 CSR CRL P7B P7C PPK OVPN WG JKS KEYSTORE JCEKS MOBILECONFIG MOBILEPROVISION REG PCAP PCAPNG P12 PFX KDBX EVTX PENC ENC', tags: 'samsung secure folder knox encrypted package penc enc aes device bound android phone backup secure folder self openssl ssh openssh putty rsa ed25519 ecdsa pkcs1 pkcs8 pkcs10 pkcs7 pkcs12 pfx cms x509 certificate csr crl certbot letsencrypt keystore java jks jceks tomcat apns apple mdm provisioning mobileconfig wireguard openvpn vpn registry regedit forensics keepass encase prefetch etw wireshark tcpdump pcap fingerprint sha256 private key secret credentials', desc: 'Inspect security and crypto files: PEM private/public keys (RSA/EC/Ed25519, PKCS#1 vs PKCS#8, encryption), OpenSSH .pub with SHA-256 fingerprint, PuTTY .ppk, PKCS#10 CSR, X.509 CRL, PKCS#7 bundles, OpenVPN/WireGuard configs, Java KeyStores, Apple .mobileconfig/.mobileprovision, Windows .reg (with autorun flagging), and pcap/pcapng captures - warning when a private key or secret is present.' },
    { label: 'Game ROMs / assets', exts: 'NES GB GBC GBA SFC SMC NDS DSI Z64 N64 V64 GEN SMD IPS BPS UPS PPF WAD NBT MCWORLD ASE PCK PAK PK3 BSP VPK VTF VMT KTX KTX2 TMX TMJ LOVE PACKAGE MPQ CIA NSP XCI', tags: 'rom emulator emulation console retro nintendo nes famicom game boy gameboy gbc advance gba snes super famicom super nintendo nintendo ds dsi nintendo 64 n64 sega genesis mega drive ips bps ups ppf patch romhack doom wad slade minecraft nbt schematic litematica bedrock aseprite sprite godot quake idtech valve source engine vpk vtf vmt bsp pico-8 love2d love tiled tmx ktx ktx2 gpu texture the sims sims3 sims4 maxis dbpf package simcity spore cas mods starcraft mpq blizzard 3ds switch nsp xci citra ryujinx renpy rpg maker fceux mesen mgba snes9x project64 mednafen', desc: 'Inspect game ROMs, patches and engine assets: iNES/NES2.0, Game Boy/Color/Advance, SNES, Nintendo DS/DSi, Nintendo 64, and Sega Genesis ROM headers (title, mapper, region, checksum); IPS/BPS/UPS/PPF patches; Doom WAD lumps; Minecraft NBT/schematics and Bedrock bundles; Godot .pck; Quake/id Tech PAK/PK3; Source BSP/VPK/VMT; Valve VTF and Khronos KTX/KTX2 textures, whose block-compressed (DXT/BCn) and uncompressed surfaces are decoded to a real preview; Tiled maps; LÖVE games; PICO-8 carts - plus MPQ, 3DS/Switch and Ren’Py/RPG Maker identification.' },
    { label: 'Disk images / firmware', exts: 'OVF OVA VBOX VMX CUE CCD NRG MDS MDF HEX SREC S19 S28 S37 MOT UF2 ELF AXF O SO OUT BIN DTB DTBO UIMAGE GPT MBR EXT4 EXT SQUASHFS SFS CRAMFS ROMFS WIM SWM ESD EWF JFFS2 UBIFS YAFFS2 ISZ CDI VMSN VMEM', tags: 'disk image firmware virtual machine vm vmware virtualbox oracle ovf ova appliance vmx vbox hypervisor cue sheet clonecd nero alcohol optical cd dvd partition gpt mbr efi boot intel hex motorola s-record srec microcontroller mcu embedded uf2 raspberry pi pico micro:bit elf axf gcc clang arm risc-v avr x86 executable shared object device tree dtb u-boot uimage flash router iot openwrt ext4 ext3 squashfs cramfs romfs linux filesystem superblock wim esd swm windows imaging encase ewf forensic jffs2 ubifs yaffs2 nand bin raw binary blob bin cue sector 2352 rom dump bios cartridge mega drive genesis a.out assembler output program output log batch dump entropy hex dump generic', desc: 'Inspect virtual-machine descriptors (VMware .vmx, VirtualBox .vbox, OVF/OVA), disc images (Nero .nrg, Alcohol .mds/.mdf, CloneCD), embedded firmware (Intel HEX, Motorola S-record, UF2, ELF/AXF, Device Tree Blobs, U-Boot uImage), partition tables (MBR/GPT with GUIDs), Linux filesystem superblocks (ext2/3/4, SquashFS, cramfs, romfs) and Windows imaging (WIM/ESD) - reading headers directly, no upload. The two generic suffixes are handled on their bytes rather than their name: a .bin opens in the raw-binary inspector (magic-byte guess, entropy profile, hex and ASCII dump) and names itself when the signature proves a raw BIN/CUE disc image or a Mega Drive cartridge dump, while a .out is read as either the ELF executable a compiler left behind or the plain-text log a batch job dumped. Either one is re-routed automatically when its bytes turn out to be a real format.' },
    { label: 'Molecules', exts: 'MOL SDF SD MOL2 XYZ PDB ENT PQR GRO CIF MMCIF MCIF', tags: 'chemistry molecule structure atom bond ball and stick protein data bank pdb crystallography cif mmcif macromolecule ligand amino acid residue chain helix sheet ribbon cartoon smiles mdl molfile sdfile connection table mol2 sybyl xyz cartesian coordinates gromacs pqr rdkit avogadro pymol vmd chimera drug discovery docking cryo-em x-ray nmr molecular weight formula hill notation 3dmol webgl', desc: 'Open chemical structures and see the molecule: MOL and SDF connection tables, MOL2, plain XYZ coordinates, Protein Data Bank entries and mmCIF/CIF crystallographic files, all drawn in an interactive 3D viewer you can rotate, zoom and restyle. Small molecules come up as ball-and-stick with the standard element colours; proteins and nucleic acids get a cartoon instead, drawing each chain as a ribbon along its backbone with helices and sheets picked out and any bound ligand still shown as sticks - which is the only way a structure of tens of thousands of atoms is readable at all. Alongside the picture, the composition is worked out from the atoms actually present rather than from any label in the file: molecular formula in Hill notation, molecular weight, net charge, element counts, and for a macromolecule the chain, residue and secondary-structure counts plus every non-polymer group - bound drugs, cofactors, metal ions and crystallisation buffer. PDB headers give up the entry ID, title, experimental method and resolution; an SDF is stepped through record by record with each molecule\'s tagged data fields read out.' },
    { label: 'Science / medical / engineering', exts: 'DCM DICOM NII FIT TCX FITS FTS FASTA FA FNA FAA FASTQ FQ GBR GBL GTL DRL XLN CIR SP SPI SPICE EDF BDF JDX DX SAV DTA SAS7BDAT VTK VTU VTP VTI SEGY SGY BAM SAM BCF HEA', tags: 'dicom medical imaging ct mri x-ray pacs radiology garmin strava zwift activity fit tcx fits astronomy nasa telescope nifti neuroimaging brain fasta fastq dna rna protein genomics ncbi illumina sequencing chemistry molecule mdl sdf mol2 rdkit chemdraw cif crystallography xyz avogadro vmd gerber pcb kicad altium eagle excellon drill spice ltspice ngspice netlist eeg ecg edf bdf biosignal jcamp spectroscopy ir nmr spss stata sas statistics dataset vtk paraview kitware mesh fea cfd simulation seg-y seismic bam sam vcf variant samtools wfdb physionet', desc: 'Open scientific, medical and engineering files: DICOM scans, NIfTI brain volumes, Garmin FIT/TCX activities, FITS astronomy frames, FASTA/FASTQ sequences, Gerber/Excellon PCB data, SPICE netlists, EDF/BDF biosignals, JCAMP-DX spectra, SPSS/Stata/SAS datasets and VTK/ParaView meshes - metadata extracted entirely in-browser.' },
    { label: 'System / misc', exts: 'OPML RSS ATOM DESKTOP SERVICE CRASH AB JOB POL SCR DS_STORE THUMBSDB DSYM DWARF SDB AGC BK', tags: 'google camera gcam config android shared preferences map xml agc samsung smart switch kies backup container bk applist opml feed reader subscriptions rss atom syndication podcast enclosure freedesktop linux desktop launcher application systemd unit service daemon apple crash report ips panic exception android backup adb windows task scheduler job group policy registry.pol preg screensaver pe executable macos ds_store finder thumbs.db thumbnail dsym dwarf debug symbols shim database sdb', desc: 'Inspect OS and system files: OPML subscription lists, RSS/Atom feeds, Linux .desktop launchers and systemd .service units, Apple .crash reports, Android .ab backups, Windows Task Scheduler .job, Group Policy Registry.pol, and .scr screensaver PE headers, plus identification of .DS_Store, Thumbs.db, dSYM/DWARF and shim .sdb. Scene .nfo ASCII art now opens in a CP437 viewer - see Text art above.' },
    { label: 'Images (more)', exts: 'TGA QOI PPM PGM PBM PNM PAM PCX FF FARBFELD WBMP XBM XPM RAS SGI BW HDR DDS EXR JP2 J2K JPF JPX JPC JXR WDP HDP EPS PS WMF EMF EMZ ICNS CUR ANI MNG LOTTIE FLIF CGM PICT PCT EPSF EPSI J2C JBIG JBIG2 JB2 ICB VST', tags: 'flif free lossless image cgm computer graphics metafile apple pict pct quickdraw encapsulated postscript epsf epsi jpeg 2000 codestream j2c jbig jbig2 jb2 bi-level fax icb vst truevision targa tga game texture qoi quite ok image netpbm portable pixmap graymap bitmap pam zsoft pcx paintbrush farbfeld suckless wbmp wireless x11 xbm xpm sun raster sgi iris radiance hdr rgbe high dynamic range directdraw surface dds directx bcn dxt bc7 openexr exr ilm vfx jpeg 2000 jp2 openjpeg jpeg xr hd photo wmphoto encapsulated postscript eps ghostscript dos eps epsi stored preview thumbnail tiff preview windows metafile emf wmf apple icns icon cursor cur ani mng lottie bodymovin airbnb after effects pict flif jbig coreldraw cdr', desc: 'Decode and preview extra still-image formats in pure JavaScript - Truevision TGA, QOI, Netpbm (PPM/PGM/PBM), PCX, farbfeld, WBMP, XBM/XPM, Sun Raster and SGI are fully rendered - and read header metadata from codec-heavy formats: Radiance HDR, DirectDraw Surface (DDS) game textures, OpenEXR, JPEG 2000, JPEG XR, EPS/PostScript (including the preview image the authoring program stored inside the file - the TIFF a DOS EPS header points at, or an EPSI hexadecimal bitmap), Windows WMF/EMF metafiles, Apple ICNS icons, CUR/ANI cursors, MNG and Lottie animations.' },
    { label: 'BIM / IFC', exts: 'IFC', tags: 'ifc bim buildingsmart building information modelling revit archicad allplan tekla vectorworks solibri navisworks step iso 10303 spatial structure storey building site space wall slab beam column door window property set pset quantity take-off ifc2x3 ifc4 coordination view openbim aec construction architecture', desc: 'Open an IFC building model and see how it is put together. IFC describes a building as a graph of objects - walls, doors, spaces, each with properties - and stores no hierarchy at all: what contains what lives in separate relationship objects, so the spatial tree of project, site, building, storey and elements is assembled by following them. That tree is shown in full, with the elements on each storey grouped by type and the property sets attached to each one. The building is also drawn: shapes that are a 2D profile extruded along a direction (which is what most walls, columns and slabs are), explicit meshes, and the mapped items that place a type\'s shape at each window and door. Shapes that need a solid-modelling kernel to evaluate - a wall with an opening cut through it - are counted and named rather than drawn wrong. Alongside, the header names the authoring tool and exporter that wrote the file, which matters more than it sounds: how much a model actually contains depends heavily on which program produced it.' },
    { label: '3D / CAD / point clouds (more)', exts: 'VOX DAE ZAE USDC X3D WRL VRML LWO LWS MD2 MD3 MDL VRM JT LAS LAZ PCD PTS E57 IFCZIP SPLAT SPZ PRC VDB X3DV HIPNC', tags: 'prc 3d pdf product representation compact openvdb vdb volume fog density x3d classic vrml x3dv houdini hipnc sidefx wavefront obj stanford ply khronos collada dae blender maya magicavoxel vox lightwave newtek quake id software studiomdl lidar point cloud asprs las laz laszip leica faro pcl ros e57 bim buildingsmart ifc revit archicad siemens jt jupiter tessellation usd usdc vrm vroid avatar gaussian splat spz scaniverse niantic openvdb alembic amf additive manufacturing voxel mesh scene off', desc: 'Header and metadata extraction for 3D meshes, voxels, BIM, point clouds and Gaussian splats: MagicaVoxel VOX, COLLADA DAE/ZAE, USD crate, X3D/VRML, LightWave LWO/LWS, Quake MD2/MD3/MDL, VRM avatars, Siemens JT, LAS/LAZ/PCD/PTS/E57 LiDAR clouds, and .splat/.spz - vertex/face/point counts, bounding boxes, units and authoring tool. IFC building models now open in a full viewer - see BIM / IFC above. glTF/GLB now open in a full 3D viewer - see the 3D model section.' },
    ...
    { label: 'Game assets (more)', exts: 'Assets Bundle Resource UTOC UCAS UEXP UMD CSO CHD FSB Bank BNK WEM Spine Skel Atlas YYP YY GMX MCA MCR MCTemplate SCN CPMModel 3DSX A78 A26 LNX J64 PCE GG SMS WS WSC W3X W3M RPYC RVData2 RXData Pyxel LDtk TIC XDelta Basis SRM State DSV DSM VBM FM2 SC2Replay RPA RGSSAD RGSS3A Litematic Schem Schematic McAddon McPack PK4 OBB', tags: 'godot binary scene scn rsrc resource scene tree openttd age of empires mythology simcity 2000 scenario map orbiter space simulator behringer x32 mixer scene custom player models cpm cpmmodel minecraft skin blockbench player model avatar android obb opaque binary blob apk expansion file jobb google play asset pack aseprite sprite pixel art sc2replay starcraft 2 replay mpq renpy archive rpa rgssad rgss3a rpg maker vx ace litematica litematic minecraft schematic schem sponge mcaddon mcpack bedrock addon pack quake doom 3 pk4 id tech 4 unity asset bundle assetbundle unreal engine ue4 ue5 utoc ucas iostore cooked compressed iso ciso cso chd mame dreamcast fmod fsb5 wwise audiokinetic bnk wem sound bank spine skeleton atlas gamemaker yoyo yyp gmx minecraft region anvil mca mcr bedrock template warcraft 3 w3x w3m mpq renpy rpyc rpg maker rvdata2 rxdata marshal pyxel edit ldtk level tic-80 fantasy console xdelta vcdiff patch basis universal texture atari 2600 7800 lynx jaguar pc engine turbografx sega master system game gear wonderswan 3ds homebrew emulator save state srm savestate desmume vbm fceux fm2 movie tas speedrun', desc: 'Identify and read more game and emulator files: Unity asset bundles, Unreal cooked assets, CISO/CHD disc images, FMOD/Wwise sound banks, Spine skeletons and atlases, GameMaker/LDtk/TIC-80 projects, Minecraft Anvil regions, Warcraft III maps, Ren Py and RPG Maker data, extra console ROMs (Atari, PC Engine, Master System/Game Gear, WonderSwan, 3DS homebrew), xdelta/Basis patches and emulator saves and movies. Scene and scenario files share the .scn extension across unrelated engines, so it is resolved on its bytes: a Godot binary scene (RSRC header) is named as one, and the strategy-game scenarios (OpenTTD, Age of Empires, SimCity 2000) and plain-text scenes (Orbiter, Behringer X32) are told apart from it. Custom Player Models .cpmmodel avatars - the standalone model the Minecraft mod exports, and what the Blockbench plugin reads and writes - are identified too.' },
    ...
    { label: 'Native code, ML & misc (more)', exts: 'ONNX TRT NODE DYLIB LDB REV CFF RSP CGP CHROMA STAMP', tags: 'onnx open neural network exchange machine learning model pytorch tensorflow scikit-learn ai inference node native addon node.js electron napi dylib mach-o macos dynamic library shared object ldb leveldb rocksdb sstable key-value chrome electron discord indexeddb git pack reverse index rev ridx citation cff citation file format yaml cffinit response file rsp compiler csc msbuild razer chroma rgb lighting animation build stamp marker ninja grid pattern cgp tensorrt trt nvidia gpu cuda serialized engine plan file deployment optimised kernels fp16 int8 jetson triton', desc: 'Identify and read cross-platform binaries, ML models and build artefacts: ONNX neural-network models (the framework that exported it and the IR/model version), NVIDIA TensorRT engines (.trt - the plan file a network is compiled into for one specific GPU and TensorRT version, which is why it will not load anywhere else), native Node.js/Electron add-ons (.node) and macOS dynamic libraries (.dylib - Mach-O/PE/ELF container and architecture), LevelDB/RocksDB tables (.ldb), Git pack reverse indexes (.rev - RIDX header), Citation File Format metadata (CITATION.cff, content-gated against binary CFF fonts), compiler response files (.rsp), Razer Chroma animations (.chroma), build stamps/markers (.stamp) and pattern/grid data (.cgp).' },
...
export const IDENTIFICATION_CORE = [
    { label: 'Adobe', exts: 'INDD INDT IDML MOGRT SESX XD FLA SWF LRtemplate LRcat ACV ACO ASL ABR GRD PAT', tags: 'photoshop illustrator indesign after effects aepx premiere pro premiere elements audition xd animate flash lightroom substance motion graphics compositing', desc: 'Identify Adobe project files and read their metadata: InDesign (INDD), XD, Animate (FLA), and Lightroom. After Effects projects (AEP, AET), Premiere Pro and Elements (PRPROJ, PREL), Photoshop (PSD, PSB) and Illustrator (AI) now open in full viewers - see After Effects, Premiere Pro, Photoshop and Illustrator.' },
    { label: 'Design', exts: 'FIG afphoto afdesign afpub SPP SBSAR SBS CDR MDP', tags: 'figma affinity photo designer publisher gimp substance painter coreldraw cdr medibang paint mdp vector drawing kiwi', desc: 'Identify design-app files: Figma (FIG), Affinity Photo/Designer/Publisher, CorelDRAW (CDR), MediBang Paint (MDP) and Substance. A Figma file has its container read out - format, Kiwi version and chunk sizes - but not its contents: Figma encodes the design with Kiwi, a schema-driven binary format whose schema ships inside the file and changes with the version of Figma that wrote it, so there is no stable structure to walk. Sketch, by contrast, opens in a full viewer - see Sketch above. GIMP (XCF) does too, and Krita (KRA), Procreate and Paint.NET (PDN) show their embedded preview - see Raster art above.' },
    { label: 'CAD', exts: 'SLDREG SVAP SVPJ IPT IAM IDW 3DM SKP 3DS MAX C4D HIP ZPR ZTL MA MB CATPART CATPRODUCT PRT ASM BRD SCH KiCad_pcb GH GHX', tags: 'autocad autodesk solidworks inventor rhinoceros rhino grasshopper sketchup trimble 3ds max cinema 4d maxon houdini sidefx zbrush pixologic maya catia dassault eagle kicad electronic pcb settings wizard backup preferences registry sldreg visualize appearance material project render svap svpj', desc: 'Identify CAD files and read header metadata: AutoCAD DWG/DWT and DXF now open as a 2D drawing preview (see Diagrams), SolidWorks .sldreg Settings Wizard backups (release year, settings groups and the saved registry keys) and SOLIDWORKS Visualize .svap appearances and .svpj projects, opened as the ZIP packages they are, Inventor (IPT, IAM), Rhino (3DM) and Grasshopper (GH, GHX), SketchUp (SKP), 3ds Max, Cinema 4D, Houdini, ZBrush, Maya, CATIA, Eagle, and KiCad. SolidWorks parts/assemblies/drawings (SLDPRT, SLDASM, SLDDRW) and Autodesk Fusion 360 (F3D, F3Z) have their own viewers - see SolidWorks and Fusion 360 above.' },
    ...
    { label: '3D / printing', exts: 'FBX USDZ USD USDA BLEND IDEA ZCODE', tags: 'blender mesh model 3d printing prusa bambu cura slicer wavefront autodesk pixar apple unity unreal stl ideamaker raise3d profile filament project sliced zortrax z-suite zcode zcodex compiled toolpath thumbnail saved preview test block', desc: 'Identify 3D and 3D-printing files: FBX, USD/USDZ, Blender (BLEND), Raise3D ideaMaker projects and print profiles (IDEA) and Zortrax Z-SUITE compiled prints (ZCODE). A Blender scene saved with preview images gives up the thumbnail it stored, so you can see what the scene looked like without opening Blender. OBJ, PLY, OFF, 3MF, AMF and glTF/GLB get a full 3D viewer - see the 3D model section.' },
    { label: 'Music production', exts: 'FLP LOGIC LOGICX PTX CPR BAND', tags: 'fl studio fruity loops logic pro tools cubase garageband steinberg daw', desc: 'Identify DAW project files and read what their formats will give up: FL Studio (FLP) has its event stream walked for the FL version, tempo, title, author, genre, comment, pattern and channel names, the plugins it loads and the samples it references - but not its clip positions, which live in data events whose layout varies between FL versions. Logic Pro, Pro Tools (PTX) and Cubase (CPR) are identified. Ableton Live and Reaper projects open with a drawn arrangement - see DAW project above.' },
    { label: 'Databases', exts: 'SQLite SQLite3 DB DB3 SQLite-WAL SQLite-SHM DB-WAL DB-SHM DB3-WAL DB3-SHM SQLite3-WAL SQLite3-SHM DCB', tags: 'sqlite sqlite3 microsoft access database sql dump schema table query rows ddl wal shm write-ahead log shared memory wal-index checkpoint journal rollback sidecar salt frames pages concordance dcb lexisnexis trialdirector litigation ediscovery e-discovery legal document management review database structure', desc: 'Open SQLite databases (.sqlite/.db/.sqlite3) and read their full schema in-browser - every table with its columns and row counts, views, indexes, triggers, the CREATE-statement DDL, and a sample of the largest table. Reads the WAL-mode sidecars too: the Write-Ahead Log (-wal) - page size, salts, frame and committed-transaction counts, and the pages it changed - and the shared-memory index (-shm) - valid frame count, database size and checkpoint progress. (.sql dumps are listed under Developer / data.) Microsoft Access (MDB, ACCDB) now opens in a full table viewer - see Access database above. Concordance .dcb databases from the LexisNexis litigation review suite are identified from their bytes, and told apart from the other formats that reuse the same extension (compiled game data, speech-recognition dictionaries).' },
    ...
    { label: 'Electronics, runtime & backups (more)', exts: 'TPS Browser PYD ICU BAK OLD PDSBak', tags: 'electronics pcb unreal third party software notice tps asp.net browser definition python extension module pyd dll icu international components unicode data backup file bak old proteus design suite pdsbak', desc: 'Identify runtime, electronics and backup files: Unreal third-party software notices (.tps), ASP.NET browser definitions (.browser), Python extension modules (.pyd DLLs), ICU internationalisation data (.icu), Proteus Design Suite backups (.pdsbak) and generic backup copies (.bak / .old).' },
    { label: 'Executables', exts: 'EXE DLL MSI APK XAPK APKM APKS IPA DMG AppImage', tags: 'windows android apple mac macos linux program application installer package apk android package manifest androidmanifest permissions sdk api level versioncode versionname signing signature dex abi arm64 google play xapk apkpure apkm apkmirror apks bundletool app bundle split apk obb expansion', desc: 'Identify and read metadata from programs and installers: Windows (EXE, DLL, MSI), Android (APK and the XAPK / APKM / APKS app bundles), iOS (IPA), macOS (DMG), and Linux (AppImage). For Android APKs it decodes the binary AndroidManifest.xml - package name, version, min/target SDK, the full permission and feature list, launcher activity, signing scheme (v1/v2/v3) and the native-code ABIs. Android app bundles (XAPK from APKPure, APKM from APKMirror, APKS from bundletool) are read from their manifest - app name, package, version, SDK levels, ABIs - with the base plus split APKs and OBB expansions listed and the whole bundle browsable as an archive.' },
    { label: 'Video editing', exts: 'AEPX WFP', tags: 'adobe after effects motion graphics wondershare filmora capcut bytedance video editing timeline sequence compositing vfx ndi draft_content.json', desc: 'Identify and read video-editing project files: the After Effects XML project (AEPX); Wondershare Filmora (WFP); and CapCut drafts (draft_content.json) - canvas resolution, duration, tracks and material counts. After Effects (AEP, AET), Premiere Pro (PRPROJ, PREL), DaVinci Resolve (DRP, DRT) and VEGAS Pro (VEG, VF) projects now open in full viewers - see those entries.' },
...
// ---------- ambiguous extensions (one extension, several unrelated formats) ----------
// SINGLE SOURCE OF TRUTH for extensions that name genuinely different file types
// (e.g. .pkg = macOS installer OR Destiny package; .cube = colour LUT OR Gaussian
// volumetric grid). Two consumers read this:
//   1. tools/prerender-format-pages.mjs renders ONE /formats/<ext> page with a
//      titled section per variant (instead of the single auto capability block).
//   2. detectVariant() below sniffs the bytes/text at drop time so the in-app
//      readout names the right variant (fed to the proprietary `_app` override
//      and the renderers that route an ambiguous extension).
//
// Each entry: { summary, variants: [{ name, desc, tell, detect? }] }
//   summary - one line shown as the page lede ("the .X extension is shared by ...").
//   name    - the format's name (section heading + the in-app application label).
//   desc    - what this format is + what Analyser does with it (house style:
//             British spelling, no em-dash, keyword-rich for SEO).
//   tell    - how the two are told apart (shown as "How to tell" on the page).
//   detect  - OPTIONAL machine rule for detectVariant(): the FIRST variant whose
//             rule matches wins; a variant with { default: true } is the fallback.
//               { magic: 'xar!' }            ASCII string at offset 0
//               { hex: '35 00 02 00' }       byte sequence at offset 0
...
export const EXT_VARIANTS = {
    ...
    ase: {
        summary: 'The .ase extension names two unrelated things: an Aseprite pixel-art sprite and an Adobe Swatch Exchange colour palette.',
        variants: [
            { name: 'Aseprite sprite', desc: 'An Aseprite (or LibreSprite) pixel-art sprite - layered, animated artwork saved by the pixel editor. Analyser composites each frame from its layer stack and plays the animation at the per-frame timings the file carries, alongside the layer tree and the animation tags.', tell: 'An Aseprite sprite carries the magic number 0xA5E0 in its fifth and sixth bytes.', detect: { hexAt: [4, 'E0 A5'] } },
            { name: 'Adobe Swatch Exchange palette', desc: 'An Adobe Swatch Exchange palette - the colour swatches shared between Photoshop, Illustrator and InDesign. Analyser identifies it and reads its version and group structure.', tell: 'A swatch file begins with the ASCII signature ASEF.', detect: { default: true } },
        ],
    },
    wld: {
        summary: 'The .wld extension names two unrelated things: a Terraria game world and an Esri world file that georeferences an image.',
        variants: [
            { name: 'Esri world file', desc: 'A world file - six lines of plain text giving the pixel size, rotation and origin that place an image on a map. Analyser reads the transform and reports the ground resolution it implies.', tell: 'A world file is plain text: six numeric lines and nothing else.', detect: { default: true } },
            { name: 'Terraria world', desc: 'A Terraria world save - the whole game world as a tile grid. Analyser reads the world name, seed, size and difficulty, and draws the map one pixel per tile from the tile data, with terrain, ores, walls and liquids.', tell: 'A Terraria world is binary and carries the ASCII magic "relogic" just after its version number.', detect: { hexAt: [4, '72 65 6C 6F 67 69 63'] } },
        ],
    },
    key: {
        summary: 'The .key extension is used by both Apple Keynote presentations and PEM cryptographic keys.',
        variants: [
            { name: 'Apple Keynote presentation', desc: 'An Apple Keynote presentation (iWork). Analyser shows the QuickLook preview Apple embeds - rendered page by page - and reads the document type and iWork version.', tell: 'A Keynote file is a ZIP package (starts with the bytes PK).', detect: { hex: '50 4B 03 04' } },
            { name: 'PEM cryptographic key', desc: 'A PEM-encoded cryptographic key (OpenSSL, OpenSSH) - an RSA, EC or Ed25519 private or public key. Analyser reads the key type and PKCS encoding and warns that a private key is secret.', tell: 'A PEM key is text beginning with -----BEGIN.', detect: { default: true } },
        ],
    },
    cube: {
        summary: 'The .cube extension is shared by colour look-up tables and Gaussian volumetric data.',
        variants: [
            { name: 'Colour LUT', desc: 'An Adobe / IRIDAS / DaVinci Resolve colour look-up table. Analyser visualises exactly what it does - the tone-response curve, a before/after chart and an interactive 3D scatter of the colour cube - and reads its title, grid size and domain.', tell: 'A LUT names LUT_3D_SIZE or LUT_1D_SIZE in its header text.', detect: { textIncludes: ['LUT_3D_SIZE', 'LUT_1D_SIZE'] } },
            { name: 'Gaussian volumetric grid', desc: 'A Gaussian CUBE file - a regular 3D grid of electron density or molecular-orbital values written by Gaussian and other quantum-chemistry packages. Analyser identifies it and reads the atom and grid dimensions.', tell: 'A Gaussian cube starts with two comment lines, then an atom count and origin coordinates.', detect: { default: true } },
        ],
    },
    pkg: {
        summary: 'The .pkg extension covers macOS installer packages and Bungie Destiny game packages.',
        variants: [
            { name: 'macOS Installer Package', desc: "Apple's flat installer package - a XAR archive. Analyser lists the payload members, the checksum algorithm and the bundle version.", tell: 'A macOS installer begins with the ASCII magic xar!.', detect: { magic: 'xar!' } },
            { name: 'Destiny package (Bungie Tiger engine)', desc: "A Bungie Tiger-engine asset package from Destiny and Destiny 2 - audio, models, textures and strings in a proprietary entry/block table. Analyser reads the package id, header version, build date and platform from the header.", tell: 'A Tiger package starts with the bytes 35 00 02 00 and carries its package id at offset 0x10 (the hex id in the filename).', detect: { hex: '35 00 02 00' } },
        ],
    },
    cdp: {
        summary: 'The .cdp extension is used by two different engineering and decision-analysis tools.',
        variants: [
            { name: 'CDP4 (COMET) model', desc: 'A CDP4 / COMET concurrent-design model from the ESA systems-engineering toolset. Analyser identifies it from its bytes.', tell: 'Analyser inspects the header to tell the two apart.' },
            { name: 'Criterium DecisionPlus model', desc: 'A Criterium DecisionPlus decision-analysis model (AHP / SMART). Analyser identifies it from its bytes.', tell: 'Analyser inspects the header to tell the two apart.' },
        ],
    },
    mat: {
        summary: 'The .mat extension names both Unity material assets and MATLAB data files.',
        variants: [
            { name: 'MATLAB MAT-file', desc: 'A MathWorks MATLAB / GNU Octave MAT-file - a binary container of variables, arrays and matrices (Level 5, or the HDF5-based v7.3). Analyser identifies it and reads the header.', tell: 'A MAT-file begins with the ASCII text MATLAB.', detect: { magic: 'MATLAB' } },
            { name: 'Unity material', desc: 'A Unity engine material asset - a YAML document describing a shader and its properties. Analyser splits the object stream and reads the material.', tell: 'A Unity material is text beginning with %YAML.', detect: { default: true } },
        ],
    },
    nsf: {
        summary: 'The .nsf extension is shared by Lotus Notes databases and NES Sound Format chiptunes.',
        variants: [
            { name: 'NES Sound Format', desc: 'A NES Sound Format chiptune - ripped 6502 audio code from a Nintendo NES / Famicom game. Analyser reads the song count, load/init/play addresses and the tune metadata.', tell: 'An NSF chiptune begins with the ASCII magic NESM.', detect: { magic: 'NESM' } },
            { name: 'Lotus Notes database', desc: 'An IBM / HCL Lotus Notes and Domino database - mail, documents and design elements. Analyser identifies it from its header.', tell: 'A Notes database has its own binary header (not NESM).', detect: { default: true } },
        ],
    },
    dts: {
        summary: 'The .dts extension means both DTS surround audio and a Linux Device Tree Source.',
        variants: [
            { name: 'Device Tree Source', desc: "A Device Tree Source - the human-readable text that describes a board's hardware for the Linux kernel and compiles to a .dtb blob. Analyser opens it as source.", tell: 'A device tree source is text beginning with /dts-v1/.', detect: { textStarts: '/dts-v1/' } },
            { name: 'DTS audio', desc: 'A DTS Coherent Acoustics bitstream (Digital Theater Systems surround sound). Analyser reads the codec, channel layout and bitrate.', tell: 'DTS audio is binary, starting with a sync word (7F FE 80 01).', detect: { default: true } },
        ],
    },
    nc: {
        summary: 'The .nc extension is used by CNC G-code programs and NetCDF scientific datasets.',
        variants: [
            { name: 'NetCDF dataset', desc: 'A Unidata NetCDF dataset - self-describing array data for climate, ocean and geoscience work. Analyser identifies it and reads its dimensions and variables.', tell: 'NetCDF is binary, beginning with the magic CDF (classic) or the HDF5 signature (NetCDF-4).', detect: { magic: 'CDF' } },
            { name: 'CNC G-code', desc: 'A CNC G-code program - toolpath commands for a mill, lathe, router or laser. Analyser reconstructs and visualises the toolpath and reads the CAM tool and dimensions.', tell: 'G-code is text full of G and M commands.', detect: { default: true } },
        ],
    },
    sav: {
        summary: 'The .sav extension names both SPSS statistics datasets and video-game save files.',
        variants: [
            { name: 'SPSS dataset', desc: 'An IBM SPSS Statistics data file - cases and variables for statistical analysis. Analyser identifies it and reads the variable metadata.', tell: 'An SPSS file begins with the ASCII magic $FL2.', detect: { magic: '$FL2' } },
            { name: 'Game save file', desc: 'A video-game save file - progress, inventory and state for emulators and many PC games. The internal layout is game-specific, so Analyser identifies it and falls back to a hex view.', tell: 'Game saves use game-specific layouts (no shared magic).', detect: { default: true } },
        ],
    },
    obj: {
        summary: 'The .obj extension is shared by Wavefront 3D models and compiled object files.',
        variants: [
            { name: 'Compiled object file', desc: 'A compiled object file - relocatable machine code emitted by a C/C++ compiler (COFF on Windows, ELF on Unix) and linked into an executable. Analyser identifies it from its header.', tell: 'An object file is binary - ELF starts with 7F E L F, COFF with a machine-type word.', detect: { hex: '7F 45 4C 46' } },
            { name: 'Wavefront 3D model', desc: 'A Wavefront OBJ 3D model - text geometry (vertices, faces, normals) usually paired with a .mtl material library. Analyser opens it in the interactive WebGL 3D viewer.', tell: 'A Wavefront model is text starting with v, vn or f lines.', detect: { default: true } },
        ],
    },
    mod: {
        summary: 'The .mod extension means both tracker music modules and JVC camcorder video.',
        variants: [
            { name: 'Camcorder video', desc: 'A camcorder video clip - JVC Everio and some other cameras record MPEG-2 program streams with a .mod extension. Analyser reads the container and codec and plays it back.', tell: 'A camcorder .mod begins with an MPEG-2 pack header (00 00 01 BA).', detect: { hex: '00 00 01 BA' } },
            { name: 'Tracker module', desc: 'A tracker music module - a ProTracker / Amiga-style song with samples and patterns. Analyser reads the title, sample list, channel count and patterns.', tell: 'A tracker module carries a 4-character format tag (such as M.K.) at offset 1080.', detect: { default: true } },
        ],
    },
    map: {
        summary: 'The .map extension is reused by source maps, game level maps and linker map files.',
        variants: [
            { name: 'Source map', desc: 'A JavaScript / CSS source map - JSON that maps minified code back to its original sources for debugging. Analyser opens it as structured JSON with an expandable value tree.', tell: 'A source map is JSON beginning with a brace.', detect: { textStarts: '{' } },
            { name: 'Game level map', desc: 'A Quake / Source-engine level map - the text brush geometry a level editor (Radiant, Hammer) compiles into a BSP. Analyser opens it as source.', tell: 'A level map is plain text describing brush entities.' },
            { name: 'Linker map', desc: 'A linker map file - a text report of symbol addresses and section sizes produced by a compiler toolchain. Analyser opens it as source.', tell: 'A linker map is plain text laid out as an address/symbol table.', detect: { default: true } },
        ],
    },
    asc: {
        summary: 'The .asc extension covers Esri ASCII grids, PGP armored data and plain ASCII text.',
        variants: [
            { name: 'Esri ASCII grid', desc: 'An Esri ASCII raster grid - a small header (ncols, nrows, cellsize) followed by a grid of elevation or raster values. Analyser reads the grid header and bounds.', tell: 'An Esri grid starts with the word ncols.', detect: { textStarts: 'ncols' } },
            { name: 'PGP armored data', desc: 'An ASCII-armored OpenPGP block - a Base64-wrapped key, message or signature. Analyser reads the armor type and walks the packet structure.', tell: 'PGP armor starts with -----BEGIN PGP.', detect: { textStarts: '-----BEGIN PGP' } },
            { name: 'Plain ASCII text', desc: 'A plain ASCII text file. Analyser opens it as text.', tell: 'Anything else with a .asc extension is treated as plain text.', detect: { default: true } },
        ],
    },
    md: {
        summary: 'The .md extension means both Markdown documents and Sega Mega Drive ROMs.',
        variants: [
            { name: 'Sega Mega Drive ROM', desc: 'A Sega Mega Drive / Genesis ROM image - a cartridge dump for the 16-bit console. Analyser reads the cartridge header: title, region and checksum.', tell: 'A Mega Drive ROM is binary with the ASCII string SEGA near offset 0x100.', detect: { hexAt: [0x100, '53 45 47 41'] } },
            { name: 'Markdown document', desc: 'A Markdown document - lightweight text markup for readme files and notes. Analyser renders it and reads its structure.', tell: 'Markdown is human-readable text.', detect: { default: true } },
        ],
    },
    bin: {
        summary: 'The .bin extension is not a format at all - it is the generic label for raw binary data, and disc images, ROM dumps, firmware and application blobs all use it.',
        variants: [
            { name: 'Raw disc image', desc: 'A raw optical-disc image - the binary half of the BIN/CUE pair that disc-ripping tools write. Every 2352-byte sector is stored whole, subheader and error correction included, which is how an audio or mixed-mode disc survives a rip that an ISO could not hold. Analyser inspects the sectors and hex directly.', tell: 'A raw disc image opens with the CD sector sync pattern 00 FF FF FF FF FF FF FF FF FF FF 00.', detect: { hex: '00 FF FF FF FF FF FF FF FF FF FF 00' } },
            { name: 'Sega Mega Drive ROM', desc: 'A Sega Mega Drive / Genesis cartridge dump saved with the generic .bin suffix. Analyser reads the cartridge header - the game title, region and checksum - from the bytes.', tell: 'A Mega Drive ROM carries the ASCII string SEGA at offset 0x100.', detect: { hexAt: [0x100, '53 45 47 41'] } },
            { name: 'Firmware or BIOS image', desc: 'A firmware or BIOS image - the flash contents of a router, console or embedded board, dumped byte for byte. There is no shared header to read, so Analyser reports the entropy profile, the magic-byte guess and the raw bytes.', tell: 'Firmware images are layout-specific and share no signature.' },
            { name: 'Binary data file', desc: 'A generic binary data file: an application blob, a game data container or an installer payload that simply names itself .bin. Analyser falls back to what the bytes themselves say - the magic-byte guess, the entropy profile and a hex and ASCII dump - and re-routes the file automatically when the signature proves a real format.', tell: 'Anything with no recognised signature is treated as generic binary data.', detect: { default: true } },
        ],
    },
    out: {
        summary: 'The .out extension names both a compiled Unix executable and the plain text a program dumped to a file.',
        variants: [
            { name: 'Compiled executable', desc: 'A compiled Unix executable - "a.out" (assembler output) is the name a C compiler gives its result when no output name is set, and modern toolchains still write an ELF binary under it. Analyser reads the executable header.', tell: 'An ELF executable begins with the bytes 7F E L F.', detect: { hex: '7F 45 4C 46' } },
            { name: 'Program output log', desc: 'A program output file - the text a batch job, build or script redirected to disk as a log. Analyser opens it as text with a source preview, line count and metadata.', tell: 'An output log is human-readable text.', detect: { default: true } },
        ],
    },
    scn: {
        summary: 'The .scn extension is shared by scene and scenario files from several unrelated programs - game engines, strategy games and simulators.',
        variants: [
            { name: 'Godot binary scene', desc: "A Godot Engine binary scene - one scene tree with its nodes, properties and the paths to the external resources it pulls in, saved in the engine's compact binary resource format rather than the readable .tscn. Analyser reads the resource header.", tell: 'A Godot binary resource begins with the ASCII magic RSRC.', detect: { magic: 'RSRC' } },
            { name: 'Game scenario', desc: 'A strategy-game scenario - the starting map, players, units and victory conditions for OpenTTD, Age of Empires or SimCity 2000. Each game uses its own binary layout, so Analyser identifies it and inspects the bytes.', tell: 'Scenario files are binary and share no signature across games.' },
            { name: 'Text scene or scenario', desc: 'A plain-text scene description - an Orbiter space-simulator scenario or a Behringer X32 mixer scene, both written as readable settings blocks. Analyser opens it as text.', tell: 'These are human-readable text.', detect: { default: true } },
        ],
    },
    // ---- Tier 2: real but niche ----
    cdf: {
        summary: 'The .cdf extension is used by NetCDF datasets and Wolfram Computable Document Format files.',
        variants: [
            { name: 'NetCDF dataset', desc: 'A Unidata NetCDF classic dataset - array-based scientific data. Analyser identifies it from its CDF magic.', tell: 'NetCDF begins with the bytes CDF.', detect: { magic: 'CDF' } },
            { name: 'Wolfram CDF', desc: 'A Wolfram Computable Document Format file - an interactive Mathematica notebook or document. Analyser identifies it.', tell: 'A Wolfram CDF is a Mathematica notebook container.', detect: { default: true } },
        ],
    },
    gem: {
        summary: 'The .gem extension names both RubyGems packages and Digital Research GEM images.',
        variants: [
            { name: 'RubyGems package', desc: "A RubyGems package - a TAR of the gem's code and metadata. Analyser lists its contents and reads the gemspec.", tell: 'A RubyGem is a TAR archive (ustar at offset 257).', detect: { hexAt: [257, '75 73 74 61 72'] } },
            { name: 'GEM image / metafile', desc: 'A Digital Research GEM raster image or metafile from the Atari ST and Ventura Publisher era. Analyser identifies it.', tell: 'A GEM image has its own small binary header.', detect: { default: true } },
        ],
    },
    g: {
        summary: 'The .g extension is shared by CNC/3D-printing G-code and BRL-CAD geometry databases.',
        variants: [
            { name: 'BRL-CAD geometry', desc: "A BRL-CAD .g geometry database - solid models from the US Army's open-source CAD system. Analyser identifies it.", tell: 'A BRL-CAD database is binary with a v4/v5 header.', detect: { textStarts: 'v5' } },
            { name: 'G-code', desc: 'A CNC / 3D-printing G-code program. Analyser reconstructs and visualises the toolpath.', tell: 'G-code is text full of G and M commands.', detect: { default: true } },
        ],
    },
    prj: {
        summary: 'The .prj extension is mostly an Esri projection file, but is also reused as a generic project file.',
        variants: [
            { name: 'Esri projection (WKT)', desc: 'An Esri .prj projection file - the coordinate reference system in WKT, stored beside a shapefile. Analyser reads the CRS.', tell: 'An Esri .prj is one line of WKT beginning with GEOGCS or PROJCS.', detect: { textStarts: ['GEOGCS', 'PROJCS'] } },
            { name: 'Generic project file', desc: 'A project file for one of many applications - the extension is reused widely. Analyser opens it as text where it can.', tell: 'Anything that is not WKT is treated as a generic text project.', detect: { default: true } },
        ],
    },
    r: {
        summary: 'The .r extension is mainly R language source, but also names REBOL scripts and classic Mac Rez resources.',
        variants: [
            { name: 'R source', desc: 'An R language script - statistics and data-analysis code. Analyser opens it as source with a preview and line count.', tell: 'Analyser opens .r as source; the surrounding syntax tells the language.', detect: { default: true } },
            { name: 'REBOL / Rez resource', desc: 'Less commonly, .r is a REBOL script or a classic Macintosh resource (Rez) text file. Analyser opens it as source.', tell: 'Both are text and open as source.' },
        ],
    },
    scr: {
        summary: 'The .scr extension is both a Windows screensaver executable and, in some apps, a text script.',
        variants: [
            { name: 'Windows screensaver', desc: 'A Windows screensaver - a renamed PE executable the operating system runs as a screensaver. Analyser reads the PE header.', tell: 'A screensaver begins with the MZ PE magic.', detect: { hex: '4D 5A' } },
            { name: 'Script file', desc: 'Some applications use .scr for a text script (for example command or render scripts). Analyser opens a text .scr as source.', tell: 'A script .scr is plain text (no MZ magic).', detect: { default: true } },
        ],
    },
    rc: {
        summary: 'The .rc extension names both Windows resource scripts and Unix runcom config files.',
        variants: [
            { name: 'Windows resource script', desc: 'A Windows resource script - text defining dialogs, menus, icons and version info, compiled into a binary .res. Analyser opens it as source.', tell: 'Both are text; Analyser opens .rc as source.', detect: { default: true } },
            { name: 'Unix rc / runcom config', desc: 'A Unix "rc" configuration file (runcom) - shell or app startup settings. Analyser opens it as source.', tell: 'Both are text; Analyser opens .rc as source.' },
        ],
    },
    fs: {
        summary: 'The .fs extension is used by both F# source code and GPU fragment shaders.',
        variants: [
            { name: 'F# source', desc: 'An F# source file - .NET functional code. Analyser opens it as source.', tell: 'Both are text; Analyser opens .fs as source.', detect: { default: true } },
            { name: 'GPU fragment shader', desc: 'A GLSL / HLSL fragment (pixel) shader. Analyser opens it as source.', tell: 'Both are text; Analyser opens .fs as source.' },
        ],
    },
    din: {
        summary: 'The .din extension is overwhelmingly a CNC part program written to the DIN 66025 standard, but two legacy tools also claimed it.',
        variants: [
            { name: 'CNC G-code program', desc: 'A CNC part program written to DIN 66025 - the German standard that became ISO 6983 and defines the G and M words every controller speaks. Heidenhain and Siemens Sinumerik controls, and the CAM post-processors that feed them, commonly save the ISO output as .din. Analyser reconstructs and visualises the toolpath exactly as it does for any other G-code dialect.', tell: 'A part program is text full of N-numbered blocks of G and M words.', detect: { default: true } },
            { name: 'DataEase installation script', desc: 'An installation script for DataEase, a 1980s relational-database application builder. Analyser opens it as text.', tell: 'An installation script is text, but carries no G or M words.' },
            { name: 'Atari 8-bit picture', desc: 'A picture in one of the Atari 8-bit graphics modes (320x192), an obsolete format from the era of the Atari 400/800. Analyser identifies it and inspects the bytes.', tell: 'An Atari picture is a raw binary pixel dump with no header.' },
        ],
    },
    dcb: {
        summary: 'The .dcb extension names several unrelated binary formats, from litigation databases to compiled game data.',
        variants: [
            { name: 'Concordance database', desc: 'A database from Concordance, the LexisNexis litigation document-management suite also read by TrialDirector. The file carries the database structure - the field and index definitions - rather than the document records themselves. Analyser identifies it and reads what the header exposes.', tell: 'Analyser inspects the header to tell these apart.' },
            { name: 'Compiled game data', desc: "A compiled data container written by a game's own data compiler, packing configuration tables and asset references into one blob the engine reads straight off disk. Santa Monica Studio's God of War titles use .dcb this way. Analyser reads the header and inspects the bytes.", tell: 'A compiled data container starts with the bytes 0B 00 10 00 04 00 00 00.', detect: { hex: '0B 00 10 00 04 00 00 00' } },
            { name: 'Speech-recognition dictionary', desc: 'A binary dictionary used by voice-recognition software - a word list with the pronunciations and related entries the recogniser matches against. Analyser identifies it from its bytes.', tell: 'Analyser inspects the header to tell these apart.' },
        ],
    },
    trt: {
        summary: 'The .trt extension covers NVIDIA TensorRT inference engines, a report template format and a couple of legacy tools.',
        variants: [
            { name: 'NVIDIA TensorRT engine', desc: 'A serialised NVIDIA TensorRT engine, also called a plan file: a neural network compiled ahead of time into an optimised inference graph with its kernels, precision and memory layout already chosen. That choice is made for one specific GPU and TensorRT version, which is why an engine will not load on another machine and why it is far larger than the ONNX model it came from. Analyser identifies it and reads what the plan header exposes.', tell: 'An engine is binary and carries the TensorRT plan magic tag in its header.', detect: { default: true } },
            { name: 'Report template', desc: 'A report template - text laid out as blocks delimited by <% and %> tags, the shape the Team Sports Scheduling System uses for its printed reports. Analyser opens it as text.', tell: 'A report template is text containing <% and %> blocks.', detect: { textIncludes: ['<%'] } },
            { name: 'Legacy translation or recording', desc: 'Two obsolete uses: a DLocalizer translation table, and a video recording from an early free-to-air receiver with PVR capability. Analyser identifies what the bytes support.', tell: 'Both are legacy formats no current application writes.' },
        ],
    },
};
// Runtime variant detector: given an ambiguous extension and the file's leading
// bytes (and optionally its decoded text), return the matching variant's `name`
...
export function detectVariant(ext, bytes, text, opts) {
    ...
    const asc = (off, len) => { let s = ''; for (let i = 0; i < len && off + i < b.length; i++)
        s += String.fromCharCode(b[off + i]); return s; };
    const hexAt = (off, hex) => {
    ...
    for (const v of entry.variants) {
        ...
        if (d.magic && asc(0, d.magic.length) === d.magic)
            return v.name;
        if (d.hex && hexAt(0, d.hex))
            return v.name;
        if (d.hexAt && hexAt(d.hexAt[0], d.hexAt[1]))
            return v.name;
        if (d.tsSync && tsSync())
            return v.name;
        if (d.m2tsSync && m2tsSync())
            return v.name;
        if (head != null && d.textStarts && [].concat(d.textStarts).some((p) => head.startsWith(p.toLowerCase())))
            return v.name;
        if (head != null && d.textIncludes && [].concat(d.textIncludes).some((p) => head.includes(p.toLowerCase())))
            return v.name;
    }
    if (specificOnly)
        return null;
...
const CAT_OF = {
    ...
    // Design
    'Adobe': 'design', 'Design': 'design', 'Raster art': 'design', 'Photoshop': 'design', 'Illustrator': 'design', 'Vector graphics': 'design',
    'After Effects': 'video',
    'Premiere Pro': 'video',
    ...
    'Diagrams': 'threed', 'Text art': 'documents',
    // 3D / CAD / engineering
    '3D model': 'threed', 'Fusion 360': 'threed', 'SolidWorks': 'threed', '3D / printing': 'threed', 'CAD': 'threed', 'Altium (EDA)': 'threed', 'KiCad (EDA)': 'threed', 'IPC netlist (EDA)': 'threed',
...
// Every catalog row tagged with its depth and category key. Depth normally comes
// from which array the row lives in - 'full' = viewer + deep metadata, 'id' =
// identified + header metadata (so the two same-named 'Archives' rows keep
// distinct depths). A FULL_ANALYSIS row may opt into the in-between 'partial'
// tier with an explicit `depth: 'partial'` field: it still opens in the app and
// keeps its /formats/<ext> viewer page, but only an embedded preview/thumbnail
// plus metadata is recoverable, not the real content (e.g. Fusion 360 and
// SolidWorks thumbnails, the iWork QuickLook preview, a flattened Krita /
// Procreate / Paint.NET canvas).
function allRows() {
    const tagged = [
        ...FULL_ANALYSIS.map((r) => ({ r, depth: r.depth || 'full' })),
...
function fullExtSet() {
    if (_fullExtSet)
        return _fullExtSet;
    _fullExtSet = new Set();
    for (const r of FULL_ANALYSIS)
        for (const t of r.exts.split(/\s+/))
            if (t)
                _fullExtSet.add(t.toLowerCase());
    return _fullExtSet;
}
...
function fmtItem(r, opts = {}) {
    ...
    r.exts.split(/\s+/).forEach((t) => {
        ...
        if (opts.anchors)
            attrs.id = 'ext-' + t.toLowerCase();
        extNodes.push(el('a', attrs, t));
    });
    // Depth badge, right of the summary before the +/- glyph: FULL = opens in a
    // viewer with deep metadata; PARTIAL = opens, but only an embedded preview /
    // thumbnail + metadata is recoverable (the real content is proprietary); ID =
    // identified + header metadata only.
    const bm = DEPTH_BADGE[r.depth] || DEPTH_BADGE.id;
    const badge = el('span', {
        class: 'fmt-item-badge ' + bm.cls,
        title: bm.title,
    }, bm.label);
    ...
    if (opts.anchors)
        detailsAttrs.id = 'fmt-' + slugify(r.label);
...
function renderFmtItems(container, opts) {
    ...
    for (const c of CATEGORIES) {
        ...
        for (const r of catRows)
            list.appendChild(fmtItem(r, opts));
