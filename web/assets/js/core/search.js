...
export function initSearch() {
    ...
    if (searchInput && searchWrap && searchBtn) {
        ...
        if (initSearch._resize) {
            window.removeEventListener('resize', initSearch._resize);
        ...
        function scrollToMatch(i) {
            ...
            if (old)
                old.classList.remove('anr-search-current');
        ...
        const SYNONYMS = {
            ...
            faces: ['triangles', 'vertices', 'polygons'],
            polygons: ['triangles', 'faces'],
            mesh: ['triangles', 'vertices', 'faces', 'geometry'],
            geometry: ['geometry', 'vertices', 'triangles', 'bounding box'],
            stl: ['triangles', 'vertices', 'volume', 'surface area', 'bounding box'],
            obj: ['vertices', 'faces', 'mesh'],
            gltf: ['vertices', 'triangles', 'mesh'],
            bbox: ['bounding box'],
            boundingbox: ['bounding box'],
            area: ['surface area'],
            surfacearea: ['surface area'],
            step: ['step', 'brep', 'objects', 'assembly'],
            iges: ['iges', 'objects'],
            cad: ['step', 'iges', 'brep', 'objects', 'assembly', 'unit'],
            track: ['gpx', 'route', 'elevation'],
            ascent: ['elevation gain', 'climb'],
            // geo / GPS tracks
            gpx: ['distance', 'moving time', 'elevation', 'cadence', 'heart rate'],
            kml: ['features', 'coordinates', 'linestring'],
            geojson: ['features', 'coordinates', 'geometry'],
            route: ['distance', 'linestring', 'waypoints', 'elevation'],
            waypoint: ['waypoints', 'features', 'point'],
            distance: ['distance', 'moving time'],
            pace: ['average pace', 'average speed'],
            cadence: ['cadence'],
            hr: ['heart rate'],
            heartrate: ['heart rate'],
            // office - word processing (docx)
            doc: ['document', 'word count', 'paragraphs', 'pages'],
            docx: ['document', 'word count', 'paragraphs', 'pages'],
            word: ['word count', 'document', 'paragraphs'],
            words: ['word count', 'words'],
            wordcount: ['word count', 'words'],
            paragraphs: ['paragraphs', 'word count'],
            editingtime: ['editing time'],
            // office - spreadsheets (xlsx)
            xlsx: ['spreadsheet', 'sheets', 'named ranges', 'formulas'],
            excel: ['spreadsheet', 'sheets', 'workbook', 'formulas'],
            spreadsheet: ['sheets', 'workbook', 'named ranges'],
            workbook: ['spreadsheet', 'sheets', 'external workbook links'],
            sheet: ['sheets', 'worksheet', 'hidden sheets'],
            sheets: ['sheet', 'worksheet'],
            worksheet: ['sheets', 'hidden sheets'],
            cell: ['cells', 'formulas', 'named ranges'],
            cells: ['cell', 'formulas'],
            formula: ['formulas', 'sample formulas'],
            formulas: ['formula', 'sample formulas'],
            macro: ['macros', 'vba'],
            macros: ['macro', 'vba'],
            namedrange: ['named ranges'],
            // office - presentations (pptx)
            pptx: ['presentation', 'slides', 'speaker notes', 'slide outline'],
            powerpoint: ['presentation', 'slides', 'speaker notes'],
            presentation: ['slides', 'slide outline', 'speaker notes'],
            slide: ['slides', 'slide outline', 'hidden slides', 'slide size'],
            slides: ['slide', 'slide outline'],
            deck: ['slides', 'presentation'],
            hyperlink: ['hyperlinks', 'external links'],
            hyperlinks: ['hyperlinks', 'links'],
            // ebooks (epub)
            epub: ['ebook', 'word count', 'reading time', 'table of contents', 'chapters'],
            ebook: ['epub', 'chapters', 'reading time', 'table of contents'],
            book: ['epub', 'chapters', 'publisher', 'series'],
            chapter: ['chapters', 'toc entries', 'table of contents'],
            chapters: ['chapter', 'toc entries'],
            toc: ['table of contents', 'toc entries', 'contents'],
            isbn: ['identifier', 'isbn'],
            reading: ['reading time', 'reading direction'],
            readingtime: ['reading time'],
            // subtitles / lyrics
            subtitle: ['subtitles', 'cues', 'srt', 'webvtt'],
            subtitles: ['subtitle', 'cues'],
            srt: ['subrip', 'subtitles', 'cues'],
            vtt: ['webvtt', 'subtitles', 'cues'],
            captions: ['subtitles', 'cues'],
            cue: ['cues', 'first cue', 'last cue'],
            cues: ['cue', 'on-screen time'],
            lyrics: ['lyrics', 'lrc'],
            lrc: ['lyrics'],
            // MIDI
            midi: ['tracks', 'channels used', 'instruments', 'tempo'],
            instrument: ['instruments', 'channels used'],
            instruments: ['instrument', 'channels used'],
            // editing timelines
            timeline: ['clips', 'tracks', 'sequence', 'duration'],
            edl: ['timeline', 'clips', 'sequence'],
            fcpxml: ['timeline', 'clips', 'tracks'],
            otio: ['timeline', 'clips', 'tracks'],
            clips: ['clip', 'tracks', 'timeline'],
            sequence: ['sequence', 'clips', 'edl'],
            // folders
            folder: ['contents', 'files', 'directories', 'largest'],
            directory: ['contents', 'files', 'directories'],
            tree: ['contents', 'treemap', 'directories'],
            // comics
            issue: ['issue', 'issue count', 'series', 'number'],
            writer: ['writer', 'penciller', 'inker', 'creator'],
            penciller: ['penciller', 'inker', 'colorist'],
            // markdown
            markdown: ['headings', 'words', 'links', 'code blocks'],
            md: ['markdown', 'headings', 'words'],
        ...
        function runSearch() {
            ...
            for (const el of document.querySelectorAll('.anr-search-highlight'))
                el.classList.remove('anr-search-highlight');
            const cur = document.querySelector('.anr-search-current');
            if (cur)
                cur.classList.remove('anr-search-current');
            matches = [];
            matchIdx = -1;
            if (!q)
                return;
        ...
        searchInput.addEventListener('keydown', (e) => {
            if (e.key === 'Escape') {
                ...
                e.preventDefault();
                scrollToMatch(e.shiftKey ? matchIdx - 1 : matchIdx + 1);
            }
        });
        searchInput.addEventListener('blur', (e) => {
            if (e.relatedTarget === prevBtn || e.relatedTarget === nextBtn)
                return;
