...
export async function sniffFileType(file) {
    let b;
    try {
        b = new Uint8Array(await file.slice(0, 264).arrayBuffer());
    }
    catch (_) {
        return null;
    }
    if (!b.length)
        return null;
    const a = (s, n) => { let r = ''; for (let i = s; i < s + n && i < b.length; i++)
        r += String.fromCharCode(b[i]); return r; };
    const m = (sig, off = 0) => { for (let i = 0; i < sig.length; i++)
        if (b[off + i] !== sig[i])
            return false; return true; };
    if (a(0, 5) === '%PDF-')
        return { kind: 'pdf', ext: 'pdf', label: 'PDF document' };
    if (m([0x89, 0x50, 0x4E, 0x47]))
        return { kind: 'photo', ext: 'png', label: 'PNG image' };
    if (m([0xFF, 0xD8, 0xFF]))
        return { kind: 'photo', ext: 'jpg', label: 'JPEG image' };
    if (a(0, 3) === 'GIF')
        return { kind: 'photo', ext: 'gif', label: 'GIF image' };
    if (m([0x42, 0x4D]) && b.length > 14)
        return { kind: 'photo', ext: 'bmp', label: 'BMP image' };
    if (m([0x49, 0x49, 0x2A, 0x00]) || m([0x4D, 0x4D, 0x00, 0x2A]))
        return { kind: 'photo', ext: 'tiff', label: 'TIFF image' };
    if (m([0x38, 0x42, 0x50, 0x53]))
        return { kind: 'proprietary', ext: 'psd', label: 'Photoshop PSD' };
    if (a(0, 4) === 'RIFF') {
        const f = a(8, 4);
        if (f === 'WEBP')
            return { kind: 'photo', ext: 'webp', label: 'WebP image' };
        if (f === 'WAVE')
            return { kind: 'audio', ext: 'wav', label: 'WAV audio' };
        if (f === 'AVI ')
            return { kind: 'video', ext: 'avi', label: 'AVI video' };
    }
    if (a(4, 4) === 'ftyp') {
        const brand = a(8, 4);
        if (/heic|heix|hevc|mif1|heif/i.test(brand))
            return { kind: 'photo', ext: 'heic', label: 'HEIC image' };
        if (/avif/i.test(brand))
            return { kind: 'photo', ext: 'avif', label: 'AVIF image' };
        if (/m4a|m4b/i.test(brand))
            return { kind: 'audio', ext: 'm4a', label: 'M4A audio' };
        if (/3gp|3g2/i.test(brand))
            return { kind: 'video', ext: '3gp', label: '3GP video' };
        return { kind: 'video', ext: 'mp4', label: 'MP4 video' };
    }
    if (m([0x1A, 0x45, 0xDF, 0xA3]))
        return { kind: 'video', ext: 'mkv', label: 'Matroska / WebM video' };
    if (a(0, 4) === 'OggS')
        return { kind: 'audio', ext: 'ogg', label: 'Ogg audio' };
    if (a(0, 3) === 'ID3' || (b[0] === 0xFF && (b[1] & 0xE0) === 0xE0))
        return { kind: 'audio', ext: 'mp3', label: 'MP3 audio' };
    if (a(0, 4) === 'fLaC')
        return { kind: 'audio', ext: 'flac', label: 'FLAC audio' };
    if (m([0x50, 0x4B, 0x03, 0x04]) || m([0x50, 0x4B, 0x05, 0x06]))
        return { kind: 'zip', ext: 'zip', label: 'ZIP archive' };
    if (a(0, 4) === 'Rar!')
        return { kind: 'proprietary', ext: 'rar', label: 'RAR archive' };
    if (m([0x37, 0x7A, 0xBC, 0xAF, 0x27, 0x1C]))
        return { kind: 'proprietary', ext: '7z', label: '7-Zip archive' };
    // ar container: Unix ar (.a) and Microsoft COFF libraries (.lib) share "!<arch>\n".
    if (m([0x21, 0x3C, 0x61, 0x72, 0x63, 0x68, 0x3E, 0x0A]))
        return { kind: 'proprietary', ext: 'a', label: 'ar archive / library' };
    if (a(0, 6) === 'SQLite')
        return { kind: 'proprietary', ext: 'sqlite', label: 'SQLite database' };
    if (m([0x1F, 0x8B]))
        return { kind: 'proprietary', ext: 'gz', label: 'GZip archive' };
    if (m([0xFD, 0x37, 0x7A, 0x58, 0x5A, 0x00]))
        return { kind: 'proprietary', ext: 'xz', label: 'XZ archive' };
    if (m([0x28, 0xB5, 0x2F, 0xFD]))
        return { kind: 'proprietary', ext: 'zst', label: 'Zstandard archive' };
    if (m([0x42, 0x5A, 0x68]) && b[3] >= 0x31 && b[3] <= 0x39)
        return { kind: 'proprietary', ext: 'bz2', label: 'bzip2 archive' };
    if (m([0x04, 0x22, 0x4D, 0x18]))
        return { kind: 'proprietary', ext: 'lz4', label: 'LZ4 archive' };
    if (m([0x1F, 0x9D]))
        return { kind: 'proprietary', ext: 'z', label: 'compress (.Z) archive' };
    if (m([0x7F, 0x45, 0x4C, 0x46]))
        return { kind: 'proprietary', ext: 'elf', label: 'ELF executable' };
    if (m([0x4D, 0x5A]))
        return { kind: 'proprietary', ext: 'exe', label: 'Windows executable' };
    if (m([0xC5, 0xD0, 0xD3, 0xC7]) || a(0, 4) === '%!PS')
        return { kind: 'proprietary', ext: 'eps', label: 'PostScript / EPS' };
    if (b.length >= 132 && a(128, 4) === 'DICM')
        return { kind: 'proprietary', ext: 'dcm', label: 'DICOM medical image' };
    if (a(257, 5) === 'ustar')
        return { kind: 'proprietary', ext: 'tar', label: 'TAR archive' };
    ...
    if (b.length >= 13 && b[0] === 0x5D) {
        ...
        if (dict >= 0x1000 && dict <= 0x40000000)
            return { kind: 'proprietary', ext: 'lzma', label: 'LZMA archive' };
    }
    const start = a(0, Math.min(b.length, 220)).trimStart();
    if (start.startsWith('<svg') || (start.includes('<svg') && start.includes('xmlns')))
        return { kind: 'svg', ext: 'svg', label: 'SVG image' };
    return null;
}
...
export async function resolveByContent(file) {
    ...
    try {
        ...
        else if (lowerExt === 'cff' && (lowerName === 'citation.cff' || /^[﻿#\s]*(cff-version|#|abstract|authors|title)\b/i.test(a(0, 48)))) {
            kind = 'proprietary';
        ...
        else if (lowerExt === 'cache' && file.size >= 16 && (await file.slice(file.size - 8, file.size - 4).text().catch(() => '')) === 'RDHS') {
            kind = 'proprietary';
        ...
        else if (lowerExt === 'res' && !head.slice(0, 64).includes(0) && /^[﻿\s]*(["/]|[A-Za-z_])/.test(a(0, 64)) && /[{}]/.test(await file.slice(0, 2048).text().catch(() => ''))) {
            kind = 'proprietary';
            sniffedExt = 'valveres';
        }
        else if (a(0, 14) === 'ANDROID BACKUP') {
            kind = 'proprietary';
            sniffedExt = 'ab';
        }
        else {
        ...
        if (kind === 'unknown') {
            ...
            if (s && s.kind) {
                kind = s.kind;
                if (s.kind === 'proprietary' || s.kind === 'comic')
                    sniffedExt = s.ext;
            }
        }
        ...
        if (kind === 'unknown') {
            ...
            if (lines.length >= 2) {
                ...
                if (commaConsistent || tabConsistent)
                    kind = 'csv';
            }
        }
    }
...
export async function isReadableText(file) {
    try {
        ...
        if (!b.length)
            return true; // empty file opens (nothing to show), not a hex dump
...
//# sourceMappingURL=file-sniff.js.map