...
function live() {
    for (const v of players)
        if (!v.isConnected)
            players.delete(v); // drop detached players
    return [...players];
}
...
export function getAudioOwner() {
    ...
    if (audioOwner && !audioOwner.isConnected)
        audioOwner = null;
    return audioOwner;
}
...
export function setAudioCompanion(audioEl) {
    ...
    if (onOwner)
        ...
        catch (_) { } // let the volume system adopt it
}
export function getAudioCompanion() { return companion; }
function shadow(t, play) {
    if (!companion)
        return;
    if (isFinite(t) && Math.abs(companion.currentTime - t) > 0.15) {
        try {
            companion.currentTime = t;
        }
        catch (_) { }
    }
    if (play === true) {
        if (companion.paused) {
            const p = companion.play();
            if (p && p.catch)
                p.catch(() => { });
        }
    }
...
function pushTime(target, t) {
    if (!isFinite(t) || Math.abs(target.currentTime - t) < 0.06)
        return;
    ...
    try {
        ...
        target.__syncSeek = Math.max(0, (target.__syncSeek || 1) - 1);
    }
}
...
export function registerSyncedVideo(video) {
    ...
    video.addEventListener('play', () => {
        if (video.__syncPlay) {
            video.__syncPlay = false;
            return;
        } // echo of a synced play
        ...
        for (const v of others(video)) {
            ...
            if (!seen(v)) {
                pendingTime.set(v, video.currentTime);
                continue;
            }
    ...
    video.addEventListener('pause', () => {
        if (video.__syncPause) {
            video.__syncPause = false;
            return;
        } // echo of a synced pause
        if (audioOwner === video)
            shadow(video.currentTime, false);
        for (const v of others(video))
            if (!v.paused) {
                v.__syncPause = true;
                v.pause();
    ...
    video.addEventListener('seeking', () => {
        if (audioOwner === video)
            shadow(video.currentTime);
        if (video.__syncSeek) {
            video.__syncSeek -= 1;
            return;
        } // echo of a synced seek
    ...
    video.addEventListener('timeupdate', () => { if (audioOwner === video && !video.paused)
        shadow(video.currentTime, true); });
    video.addEventListener('ended', () => { if (audioOwner === video)
        shadow(video.currentTime, false); });
    video.addEventListener('emptied', () => { unwatch(video); players.delete(video); if (audioOwner === video)
        audioOwner = null; });
    return () => { unwatch(video); players.delete(video); if (audioOwner === video)
        audioOwner = null; };
}
