...
export function showDropLoader(file, onCancel, labelText, immediate) {
    ...
    const reveal = () => {
        ...
        requestAnimationFrame(() => { if (_dropLoaderOpen && _dropLoaderEl)
            _dropLoaderEl.classList.add('is-open'); });
    };
    if (immediate)
        reveal();
    else
        _dropLoaderTimer = setTimeout(reveal, 160);
}
...
export function setDropLoaderLabel(text) {
    ...
    if (_dropLoaderEl && _dropLoaderEl._label)
        _dropLoaderEl._label.textContent = text || '';
}
export function hideDropLoader() {
    ...
    const doHide = () => { _dropLoaderOpen = false; if (_dropLoaderEl)
    ...
    if (shownFor >= DROP_LOADER_MIN_MS)
        ...
        _dropLoaderHideTimer = setTimeout(doHide, DROP_LOADER_MIN_MS - shownFor);
}
...
export function showLinkConfirm(anchor, opts) {
    ...
    if (px + mw > window.innerWidth - 4)
        px = window.innerWidth - mw - 4;
    if (py + mh > window.innerHeight - 4)
        py = r.top - mh - 8;
