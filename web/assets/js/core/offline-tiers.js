...
export function setupOfflineTiers(COMMIT_COUNT, RELEASE_COMMITS, analyserVersion) {
    ...
    function readOfflineState() {
        try {
            return JSON.parse(localStorage.getItem('anr-offline') || '{}') || {};
        }
        catch (_) {
            return {};
        }
    ...
    function writeOfflineState(state) {
        try {
            localStorage.setItem('anr-offline', JSON.stringify(state));
        }
        catch (_) { }
    }
    ...
    function readFeatState() {
        try {
            return JSON.parse(localStorage.getItem('anr-offline-feat') || '{}') || {};
        }
        catch (_) {
            return {};
        }
    ...
    function writeFeatState(state) {
        try {
            localStorage.setItem('anr-offline-feat', JSON.stringify(state));
        }
        catch (_) { }
    }
    ...
    async function detectCachedTier() {
        try {
            ...
            if (await has(TIERS.complete[TIERS.complete.length - 1])
                && await has(TIERS.everything[TIERS.everything.length - 1]))
                return 'complete';
            if (await has(TIERS.everything[TIERS.everything.length - 1]))
                return 'everything';
            if (await has(TIERS.essentials[TIERS.essentials.length - 1]))
                return 'essentials';
        }
        catch (_) { }
        return null;
    ...
    function tierUrls(tier) {
        ...
        if (tier === 'everything' || tier === 'complete')
            urls.push(...TIERS.everything);
        if (tier === 'complete')
            urls.push(...TIERS.complete);
        return urls;
    }
    ...
    function refreshTierButtons() {
        ...
        let cachedIdx = -1;
        TIER_ORDER.forEach((t, i) => { if (state[t] != null)
            cachedIdx = Math.max(cachedIdx, i); });
        ...
        document.querySelectorAll('.offline-btn').forEach((btn) => {
            ...
            if (idx === cachedIdx) {
                ...
                if (sizeEl)
                    sizeEl.textContent = cachedIdx >= 0 ? '+~' + (TIER_MB[tier] - cachedMb) + ' MB' : TIER_SIZES[tier];
            }
        });
    ...
    async function runCacheLoop(btn, urls, force) {
        ...
        for (const url of urls) {
            if (abort.signal.aborted)
                break; // Clear storage cancelled us mid-run
            let ok = false;
            try {
    ...
    async function downloadFeature(btn, key) {
        ...
        if (FEATURE_ORDER.every(k => feats[k] != null))
            state.complete = COMMIT_COUNT;
        writeOfflineState(state);
        refreshTierButtons();
        ...
        FEATURE_ORDER.forEach((key) => {
            ...
            if (feats[key] != null) {
                ...
                if (bar)
                    bar.hidden = true;
                if (sizeEl)
                    sizeEl.textContent = '~' + FEATURES[key].mb + ' MB';
            }
        });
    }
    ...
    function refreshCompleteButton() {
        ...
        if (state.complete != null)
            return; // fully cached: keep the "Cached" badge
        const feats = readFeatState();
        const have = FEATURE_ORDER.filter(k => feats[k] != null).length;
        if (!have)
            return; // nothing yet: keep the size / upgrade label
        const sizeEl = btn.querySelector('.offline-size');
        if (sizeEl)
            sizeEl.textContent = have + ' of ' + FEATURE_ORDER.length + ' packs';
        btn.classList.remove('is-done', 'is-fading', 'is-included');
        cachedBadge(btn).hidden = true;
    }
    ...
    function buildFeaturePopup() {
        ...
        FEATURE_ORDER.forEach((key) => {
            ...
            btn.addEventListener('click', () => {
                if (btn.classList.contains('is-active') || btn.classList.contains('is-done'))
                    return;
        ...
        featPopup._onKey = (e) => { if (e.key === 'Escape')
            closeFeaturePopup(); };
        closeBtn.addEventListener('click', closeFeaturePopup);
        featPopup.addEventListener('click', (e) => { if (e.target === featPopup)
            closeFeaturePopup(); });
    }
    ...
    document.querySelectorAll('.offline-help-panel > div').forEach(d => {
        ...
        if (s && TIER_SIZES[tier])
            s.textContent = TIER_SIZES[tier];
    });
    ...
    function setOfflineOpen(open) {
        if (!offlineSection || !offlineToggle)
            return;
    ...
    if (clearBtn) {
        clearBtn.addEventListener('click', async () => {
            ...
            for (const abort of activeDownloads) {
                try {
                    abort.abort();
                }
                catch (_) { }
...
//# sourceMappingURL=offline-tiers.js.map