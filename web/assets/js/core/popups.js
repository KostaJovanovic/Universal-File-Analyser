...
async function turnstileChallenge(box, setStatus, signal) {
    ...
    return new Promise((resolve, reject) => {
        ...
        if (signal)
            signal.addEventListener('abort', () => { drop(); reject('cancelled'); }, { once: true });
        wid = ts.render(box, {
...
export function hideSuggestPopup() {
    ...
    if (_suggestPopEl)
        _suggestPopEl.classList.remove('is-open');
...
export function showSuggestPopup(ext) {
    ...
    if (!_suggestPopEl || !_suggestPopEl.isConnected) {
        ...
        cta.addEventListener('click', async () => {
            ...
            try {
                ...
                if (window.anrDesktop && !window.anrDesktop.shell) {
                    openMailto();
                    resetCta();
                    gate.hidden = true;
                    return;
    ...
    if (_suggestPopEl._gate) {
        _suggestPopEl._gate.hidden = true;
        _suggestPopEl._gate.textContent = '';
    }
...
function openContactModal() {
    ...
    turnstileChallenge(box, (t) => { status.textContent = t; }, abort.signal)
        .then(() => {
        if (settled)
            return;
...
export function wireShareButtons() {
    ...
    if (window.anrDesktop)
        document.querySelectorAll('.header-btn-app').forEach((a) => a.remove());
...
function shareNudgeAllowed() {
    try {
        ...
        if (hold && Date.now() < hold)
            return false; // within the 4-day post-share hold
        if (localStorage.getItem(NUDGE_DAY_KEY) === nudgeDayStamp())
...
function markShareNudgeShared() {
    try {
        localStorage.setItem(NUDGE_HOLD_KEY, String(Date.now() + 4 * 24 * 60 * 60 * 1000));
    }
    catch (_) { }
}
...
function markShareNudgeSeen() {
    try {
        localStorage.setItem(NUDGE_DAY_KEY, nudgeDayStamp());
    }
    catch (_) { }
}
export function hideShareNudge() {
    if (_shareNudgeTimer) {
        clearTimeout(_shareNudgeTimer);
...
export function scheduleShareNudge(ctx) {
    ...
    if (typeof document !== 'undefined' && document.body && document.body.dataset.page === 'samples')
        return;
    ...
    _shareNudgeTimer = setTimeout(() => {
        ...
        if (_suggestActive || (_suggestPopEl && _suggestPopEl.classList.contains('is-open')))
            return;
        if (_shareModalOpen)
            return;
        showShareNudge(ctx);
...
function showShareNudge(ctx) {
    ...
    if (ctx && ctx.category === 'audio')
        spectrogramFile(ctx.name).then((f) => { pendingFile = f; });
...
export async function updateNetStatus() {
    ...
    _netProbeBusy = true;
    try {
        applyNetStatus(await probeOnline());
    }
    finally {
        _netProbeBusy = false;
    }
