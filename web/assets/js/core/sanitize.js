...
export function emptySanitizeReport() {
...
function cssUnsafe(value) {
    ...
    if (s.indexOf('\\') >= 0 && s.indexOf('(') >= 0)
        return true;
    if (/@import|image-set\s*\(|\bsrc\s*\(|\bimage\s*\(|element\s*\(|expression\s*\(|-moz-binding|behavior\s*:/i.test(s))
        return true;
...
function prefixFragments(value) {
...
function cleanStyle(elt, attr, rep) {
    if (cssUnsafe(attr.value)) {
    ...
    if (/fixed|sticky/i.test(st.getPropertyValue('position')))
    ...
    if (v && /url\s*\(/i.test(v))
        elt.setAttribute('style', prefixFragments(v));
}
...
function cleanAttrs(elt, o, rep) {
    ...
    for (const attr of Array.from(elt.attributes)) {
        ...
        if (ans === XMLNS_NS || (ans === null && name === 'xmlns'))
            continue;
        if (ans === XML_NS) {
            if (name !== 'lang' && name !== 'space')
                drop();
            continue;
        }
        if (ans === XLINK_NS) {
        ...
        else if (ans !== null || name.indexOf(':') >= 0) {
            drop();
            continue;
        }
        // Event handlers.
        ...
        if (DROP_ATTRS.has(name) || name.startsWith('data-')) {
            drop();
            continue;
        }
        if (name === 'id' || name === 'name') {
            if (val)
                attr.value = ID_PREFIX + val;
            continue;
        }
        if (IDREF_ATTRS.has(name)) {
            attr.value = val.split(/\s+/).filter(Boolean).map((t) => ID_PREFIX + t).join(' ');
            continue;
        }
        if (name === 'usemap') {
            if (val.charAt(0) === '#')
                attr.value = '#' + ID_PREFIX + val.slice(1);
            continue;
        }
        if (name === 'href') {
            ...
            if (flat.charAt(0) === '#') {
                if (flat.length > 1)
                    attr.value = '#' + ID_PREFIX + flat.slice(1);
                continue;
            }
            ...
            if (!navigableEl) {
                ...
                if (scheme === 'data' && DATA_IMAGE.test(flat))
                    continue;
            }
            if (isUnsafeUrl(val)) {
                drop();
        ...
        if (NETWORK_ATTRS.has(name)) {
            if (!o.allowRemote || isUnsafeUrl(val)) {
                drop();
                rep.extRefs++;
            }
            continue;
        ...
        if (name === 'style') {
            cleanStyle(elt, attr, rep);
            continue;
        }
        // SVG presentation attributes are CSS - see cssUnsafe().
        if (isSvg && cssUnsafe(val)) {
            drop();
            rep.cssRefs++;
            continue;
        }
        if (/url\s*\(/i.test(val))
            attr.value = prefixFragments(val);
    }
    if (navigableEl) {
    ...
    else if (elt.hasAttribute('target'))
        elt.setAttribute('rel', 'noopener noreferrer');
}
...
function cleanTree(root, includeRoot, o, rep) {
    const stack = includeRoot ? [root] : Array.from(root.childNodes);
    while (stack.length) {
        ...
        if (DROP_ELEMENTS.has(local) && !(local === 'title' && ns === SVG_NS)) {
            if (local === 'script')
                rep.script++;
            else if (local === 'foreignobject')
                rep.foreignObject++;
            else if (local === 'style')
                rep.style++;
            else if (SMIL_ELEMENTS.has(local))
                rep.smil++;
            elt.remove();
            continue;
        }
        ...
        if (o.svg && ns !== SVG_NS) {
            if (ns === HTML_NS || ns === MATHML_NS)
                rep.html++;
            elt.remove();
            continue;
        }
        cleanAttrs(elt, o, rep);
        for (const c of Array.from(elt.childNodes))
            stack.push(c);
    }
}
...
/* Sanitise the <svg> of a parsed SVG document and return a clean COPY of it,
   ...
   was removed. */
export function sanitizeSvgDoc(doc, report) {
