...
function boot() {
    ...
    async function handleFile(file, opts) {
        ...
        if (stale())
            return; // a throw above skips the checks inside the try
        ...
        if (kind === 'proprietary' && /\.(exe|dll|scr)$/i.test(file.name) && photoResults) {
            ...
            extractPeIcon(file).then(async (iconFile) => {
                ...
                if (token.cancelled || _currentToken !== token)
                    return;
                mountPhotoPrompt(isScr ? 'Screensaver icon' : 'Application icon', 'The icon stored inside this program. Analyse it with the photo tools - size, colours and the rest.', 'Analyse icon', async (host) => {
                    await ensureExifr();
                    markAnalysed('photo');
                    // Returned, so the prompt's own .finally waits for the render.
                    return renderPhoto(iconFile, host, { sourceNote: (isScr ? 'Screensaver icon extracted from ' : 'Application icon extracted from ')
