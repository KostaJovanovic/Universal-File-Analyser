    ...
    if (fmtOverlay) {
        ...
        function applyFilter() {
            ...
            items.forEach((it) => {
                ...
                const text = (labelEl.textContent + ' ' + extsEl.textContent + ' ' +
                    (it.dataset.tags || '') + ' ' + descEl.textContent).toLowerCase();
                ...
                if (match) {
                    visCount++;
                    extsEl.textContent.split(/\s+/).forEach((t) => { if (t)
                        extSet.add(t.toLowerCase()); });
                }
            });
            ...
            labels.forEach((label) => {
                ...
                if (visible && !firstVisibleLabel)
                    firstVisibleLabel = label;
            });
            if (firstVisibleLabel)
                firstVisibleLabel.classList.add('is-first-visible');
            if (fmtResultCount) {
            ...
            if (fmtEmpty) {
                ...
                if (visCount === 0)
                    fmtEmpty.textContent = raw ? `No formats match “${raw}”.` : 'No formats in this category.';
            }
            syncToggleAll();
        }
        ...
        function openFmt() {
            ...
            if (wasHidden)
                fmtOverlay._backClose = openOverlayBack(hideFmt); // device Back closes it
            activeCat = 'all';
            buildChips();
            if (fmtSearch) {
                fmtSearch.value = '';
        ...
        function closeFmt() { if (fmtOverlay._backClose)
            fmtOverlay._backClose();
        else
            hideFmt(); }
        buildChips();
        ...
        if (fmtClose && !fmtClose._wired) {
            fmtClose._wired = true;
            fmtClose.addEventListener('click', () => { if (fmtCur)
                fmtCur.close(); });
        }
        if (fmtToggleAll && !fmtToggleAll._wired) {
            fmtToggleAll._wired = true;
            fmtToggleAll.addEventListener('click', () => { if (fmtCur)
                fmtCur.toggle(); });
        }
        if (!fmtOverlay._wired) {
            fmtOverlay._wired = true;
            fmtOverlay.addEventListener('click', (e) => { if (e.target === fmtOverlay && fmtCur)
                fmtCur.close(); });
        }
        ...
        if (fmtSearch && !fmtSearch._wired) {
            fmtSearch._wired = true;
            fmtSearch.addEventListener('input', debounce(() => { if (fmtCur)
                fmtCur.apply(); }, 120));
        }
    ...
    if (fmtPageSearch && !fmtPageSearch._wired) {
        ...
        const applyPageFilter = () => {
            ...
            pItems.forEach((it) => {
                ...
                const extsEl = it.querySelector('.fmt-item-exts');
                const descEl = it.querySelector('.fmt-item-desc');
                const hay = ((labelEl ? labelEl.textContent : '') + ' ' +
                    (extsEl ? extsEl.textContent : '') + ' ' +
                    (it.dataset.tags || '') + ' ' +
                    (descEl ? descEl.textContent : '')).toLowerCase();
...
//# sourceMappingURL=format-overlay.js.map