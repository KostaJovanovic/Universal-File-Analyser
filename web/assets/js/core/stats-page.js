...
export async function setupStatsPage() {
    ...
    const fEl = $('statsFiles');
    if (fEl)
        fEl.textContent = data.files.toLocaleString();
    const vEl = $('statsVisitors');
    if (vEl)
        ...
        if (!scores.length) {
            ...
            const scoreRow = (s, i) => {
                ...
                if (top) {
                    ...
                    num.addEventListener('keydown', (e) => { if (e.key === 'Enter' || e.key === ' ') {
                        e.preventDefault();
                        launchGame();
                    } });
                }
                ...
                if (Number.isFinite(waveN) && waveN > 0)
                    run.push('W' + waveN);
                if (s.cause)
                    run.push(causeText(s.cause));
                if (run.length)
                    children.push(el('span', { class: 'stats-score-run' }, run.join('  ·  ')));
                children.push(num);
    ...
    for (const e of rawExts) {
        if (e.supported)
            ...
            unsupportedTotal += e.count;
    }
    if (unsupportedTotal > 0)
        exts.push({ ext: '(unsupported)', supported: false, count: unsupportedTotal });
...
function renderStatsTrends(daily, totals) {
    ...
    let sumF = 0;
    for (const d of rows) {
        sumV += Number(d.visitors) || 0;
        sumF += Number(d.files) || 0;
    }
    const baseline = {
    ...
    if (legendEl && !legendEl._wired) {
        ...
        legendEl.addEventListener('click', (e) => {
            ...
            if (!turningOn && _TREND_SERIES.filter((k) => visible[k]).length <= 1)
                return;
    ...
    if (_trendResizeHandler)
        window.removeEventListener('resize', _trendResizeHandler);
    let resizeRaf = 0;
    const onResize = () => {
        if (resizeRaf)
            return;
...
function buildTrendChart(chartEl, daily, baseline, layout) {
    ...
    const onMove = (e) => {
        ...
        if (state.visible.visitors) {
            vFocus.setAttribute('cx', px.toFixed(1));
            vFocus.setAttribute('cy', yFor(state.series.visitors[i]).toFixed(1));
            vFocus.style.opacity = '1';
        }
        else
            vFocus.style.opacity = '0';
        if (state.visible.files) {
            fFocus.setAttribute('cx', px.toFixed(1));
            fFocus.setAttribute('cy', yFor(state.series.files[i]).toFixed(1));
            fFocus.style.opacity = '1';
        }
        else
            fFocus.style.opacity = '0';
        ...
        if (state.visible.visitors)
            kids.push(tipRow('visitors', 'Visitors'));
        if (state.visible.files)
            kids.push(tipRow('files', 'Files'));
        tip.innerHTML = '';
    ...
    hit.addEventListener('pointerleave', (e) => { if (e.pointerType !== 'touch')
        hideHover(); });
    return {
        ...
        apply(mode, scaleMax, visible) {
            ...
            for (let k = 0; k <= TICKS; k++) {
                const val = step * k;
                const y = yFor(val);
                gridLines[k].setAttribute('y1', y.toFixed(1));
                gridLines[k].setAttribute('y2', y.toFixed(1));
                yLabels[k].setAttribute('y', (y + 3.5).toFixed(1));
                yLabels[k].textContent = fmtY(val);
            }
            if (n > 1) {
            ...
            if (hoverI >= 0) { // keep the focus markers glued to the line as it rescales
                if (state.visible.visitors)
                    vFocus.setAttribute('cy', yFor(s.visitors[hoverI]).toFixed(1));
                if (state.visible.files)
                    fFocus.setAttribute('cy', yFor(s.files[hoverI]).toFixed(1));
            }
        ...
        setShown(key, on) {
            ...
            if (!on && (key === 'visitors' ? vFocus : fFocus))
                (key === 'visitors' ? vFocus : fFocus).style.opacity = '0';
        },
        // Fade the whole plot (CSS transition on the svg); `done` fires after it.
        fade(to, done) {
            if (to < 1)
                hideHover(); // don't leave a tooltip floating over a faded chart
            svg.style.opacity = String(to);
            if (done)
                setTimeout(done, 200); // matches --dur-base on .stats-trend-svg
        },
    };
}
