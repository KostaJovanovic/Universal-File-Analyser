...
export async function signatureCheck(file, sniff) {
    ...
    if (sniff && expect.sniff.includes(sniff.ext))
        return null; // signature matches the extension
    let hex = '';
    try {
...
export function signatureCard(info) {
    ...
    const [h, help] = h3help('Signature mismatch', 'Every file type opens with a distinctive pattern of bytes - its signature, or “magic number”. '
    ...
    if (info.missing)
        t.appendChild(row('Content', 'no recognised ' + info.label + ' signature'));
    else
        t.appendChild(row('Content looks like', info.sniff.label + ' (.' + info.sniff.ext + ')'));
    if (info.hex)
        t.appendChild(rowHelp('Leading bytes', info.hex, 'The raw first bytes of the file, shown in hex. Because each format begins with its own signature, '
...
async function pngLogicalEnd(file) {
    if (await endsWith(file, PNG_IEND))
        return file.size;
...
async function jpegLogicalEnd(file) {
    if (await endsWith(file, [0xFF, 0xD9]))
        return file.size;
    const b = new Uint8Array(await file.arrayBuffer());
...
async function gifLogicalEnd(file) {
    ...
    p += 7;
    if (packed & 0x80)
        p += 3 * (1 << ((packed & 7) + 1)); // global colour table
    const skipSubBlocks = () => { while (p < b.length) {
        const sz = b[p++];
        if (sz === 0)
            break;
        p += sz;
    ...
    while (p < b.length) {
        ...
        if (sep === 0x21) {
            ...
            p += 9;
            if (ipacked & 0x80)
                p += 3 * (1 << ((ipacked & 7) + 1)); // local colour table
            p++; // LZW min code size
            skipSubBlocks();
        }
        else
            return null; // unknown block - bail
        if (p > b.length)
            return null;
    }
    return null;
...
export async function trailingDataCheck(file, sniff) {
    ...
    if (logicalEnd == null || logicalEnd <= 0 || logicalEnd >= file.size)
        return null;
...
export function trailingCard(info, file) {
    ...
    const [h, help] = h3help('Trailing data', 'Content that sits after a file’s real, structural end. Normal viewers stop at that end and ignore it, '
    ...
    t.appendChild(rowHelp('Logical end', info.logicalEnd.toLocaleString() + ' bytes', 'Where the file’s own structure actually ends. Anything between here and the total size is extra data added afterwards.'));
    t.appendChild(row('Trailing data', info.trailing.toLocaleString() + ' bytes after the end'));
    if (info.sniffLabel)
        t.appendChild(row('Appended content', 'looks like ' + info.sniffLabel));
    if (info.hex)
        t.appendChild(rowHelp('First bytes', info.hex, 'The opening bytes of the appended data, in hex. Their signature hints at what the hidden content is.'));
    card.appendChild(t);
    card.appendChild(el('p', { class: 'anr-sig-flag-note' }, info.trailing.toLocaleString() + ' bytes sit after the logical end of this ' + info.fmtLabel + '.'));
    ...
    if (file) {
        ...
        const btns = [dl];
        if (info.sniffKind) {
            const an = el('button', { type: 'button', class: 'anr-btn anr-btn-sm' }, 'Analyse appended ' + (info.sniffExt ? info.sniffExt.toUpperCase() : 'data'));
            an.addEventListener('click', () => { if (_reanalyse)
                _reanalyse(new File([slice()], dlName, { type: '' })); });
            btns.push(an);
        }
...
export function findIntegrityCard(root) {
    ...
    for (const card of root.querySelectorAll('.anr-card')) {
        ...
        if (h && h.textContent.trim().startsWith('Integrity'))
            return card;
    }
    return null;
}
