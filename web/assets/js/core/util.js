...
export async function desktopFile(f) {
    const stub = f;
...
export async function probeReadable(file) {
    ...
    try {
        ...
        if (file.size > 65536)
            await file.slice(file.size - 1, file.size).arrayBuffer();
        return null;
    }
    catch (e) {
        return e;
...
export function cloudFileWarning(file) {
    ...
    box.appendChild(el('p', { style: 'margin:0 0 10px; font-weight:600;' }, 'Couldn’t read “' + ((file && file.name) || 'this file') + '”.'));
    box.appendChild(el('p', { style: 'margin:0 0 10px;' }, 'It looks like a cloud-only file (OneDrive, iCloud Drive, Google Drive, Dropbox…) whose contents aren’t on this device yet, or whose sync app isn’t running. The name and size are known, but the actual bytes couldn’t be downloaded.'));
...
export function emptyFileWarning(file) {
    ...
    box.appendChild(el('p', { style: 'margin:0 0 10px; font-weight:600;' }, '“' + ((file && file.name) || 'This file') + '” is empty - it’s 0 bytes, so there’s nothing to open or analyse.'));
    box.appendChild(el('p', { style: 'margin:0 0 10px;' }, 'This isn’t something Analyser can fix: a zero-length file contains no data at all, so it can’t be viewed or repaired. It usually means the file never finished being written.'));
    const ul = el('ul', { style: 'margin:0; padding-left:18px;' }, [
    ...
    box.appendChild(ul);
    box.appendChild(el('p', { style: 'margin:10px 0 0;' }, 'Look for another copy of the same file - a working version is often saved right alongside it (for example with a “(1)” suffix). If this is the only copy, the data is gone and can’t be recovered from an empty file.'));
    return box;
}
    ...
    bar.indeterminate = () => {
        ...
        const loop = (ts) => {
            if (bar.isConnected)
                ...
                raf = null;
                return;
            } // removed from DOM → self-stop
            if (!measured && bar.isConnected) {
                measure();
                measured = true;
            }
            if (t0 == null)
...
export function timeAnomalyCard(anomalies) {
    ...
    for (const a of anomalies)
        ul.appendChild(el('li', { style: 'margin:3px 0;' }, a));
    card.appendChild(ul);
    return card;
}
...
export function attachViewCube(viewer) {
    ...
    for (const f of FACES) {
        ...
        for (const su of [-1, 1])
            zone('anr-viewcube-edge', 50 + su * O, 50, [fx + su * f.u[0], fy + su * f.u[1], fz + su * f.u[2]]);
        for (const sv of [-1, 1])
            zone('anr-viewcube-edge', 50, 50 - sv * O, [fx + sv * f.v[0], fy + sv * f.v[1], fz + sv * f.v[2]]);
    ...
    const dn = (x, y) => { drag = true; moved = false; lx = x; ly = y; if (anim) {
        cancelAnimationFrame(anim);
        anim = 0;
    } if (viewer.setSpin)
    ...
    const mv = (x, y) => {
        if (!drag)
            return;
        if (Math.abs(x - lx) + Math.abs(y - ly) > 3)
            moved = true;
        state.yaw += (x - lx) * 0.01;
    ...
    const setHot = (hs) => {
        ...
        for (const h of hotspots)
            h.classList.toggle('is-hot', key != null && h._key === key);
    };
...
export function sha256Row(file) {
    const hashRow = rowHelp('SHA-256', '', "SHA-256 is a cryptographic fingerprint of the file’s exact contents - a short code where two identical files always match, and changing even a single byte changes it completely. That makes it a reliable way to check a file hasn’t been altered, or that it matches a known copy.");
        ...
        for (const key of sortedKeys(node)) {
            ...
            if (isDir(val)) {
                ...
                if (opts.fileAccent) {
                    ...
                    if (color)
                        lead.appendChild(el('span', { class: 'anr-tree-dot', style: 'background:' + color }));
                }
...
export function attachZoomPan(wrap, opts = {}) {
    ...
    function apply() {
        ...
        if (center)
            center.style.overflow = scale > 1 ? 'visible' : '';
    }
    function clampPan() {
...
function _initBackButton() {
    if (_backReady || typeof window === 'undefined')
        return;
    _backReady = true;
...
export function openOverlayBack(hide) {
    ...
    _ovStack.push(entry);
    try {
        history.pushState({ anrOverlay: 1 }, '');
    }
    catch (_) { }
    return function close() {
        var i = _ovStack.indexOf(entry);
        if (i === -1)
            return; // already closed (e.g. via Back)
        _ovStack.splice(i, 1);
        try {
            hide();
        }
...
function _confirmExit(onYes, onNo) {
    if (document.getElementById('anr-exit-confirm'))
    ...
    yes.type = 'button';
    yes.className = 'anr-btn';
    yes.textContent = 'Exit';
    var cancel = document.createElement('button');
    cancel.type = 'button';
    cancel.className = 'anr-btn';
