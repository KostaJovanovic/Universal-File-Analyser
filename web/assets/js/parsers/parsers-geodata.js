...
async function parseTopojson(file) {
    let j;
    try {
        j = JSON.parse(await readText(file, 16_000_000));
    }
    catch (_) {
        return null;
    }
    if (!j || j.type !== 'Topology' || typeof j.objects !== 'object')
        return null;
    ...
    if (Object.keys(byType).length)
        out['Geometry types'] = topCounts(byType);
    if (j.transform) {
        const t = j.transform;
        if (Array.isArray(t.scale))
            out['Transform scale'] = t.scale.map((n) => fc(n)).join(', ');
        if (Array.isArray(t.translate))
            out['Transform translate'] = t.translate.map((n) => fc(n)).join(', ');
        out['Quantised'] = 'yes (delta-encoded arcs)';
    }
    if (Array.isArray(j.bbox) && j.bbox.length === 4)
        out['Bounding box'] = fmtBBox(j.bbox[0], j.bbox[1], j.bbox[2], j.bbox[3]);
    return out;
}
// =====================================================================
//  OSM XML
...
async function parseOsm(file) {
    ...
    const tags = {};
    let m;
    const re = /<tag\b[^>]*\bk\s*=\s*"([^"]*)"/g;
    while ((m = re.exec(text)))
        tags[m[1]] = (tags[m[1]] || 0) + 1;
    if (Object.keys(tags).length)
        out['Top tag keys'] = topCounts(tags, 14);
...
async function parseShp(file) {
    ...
    if (h.minZ || h.maxZ)
        out['Z range'] = fc(h.minZ) + ' … ' + fc(h.maxZ);
    if (h.minM || h.maxM)
...
async function parseDbf(file) {
    ...
    if (head.length < 32)
        return null;
    const ver = head[0];
    if (!DBF_VERSIONS[ver] && (ver & 0x07) !== 0x03)
        return null; // loose validity gate
...
async function parsePrj(file) {
    ...
    if (!/^(GEOGCS|PROJCS|GEOGCRS|PROJCRS|BOUNDCRS|COMPD_CS|LOCAL_CS|VERTCS|ENGCRS)/i.test(text))
        return null;
    ...
    if (auth.length) {
        const a = auth[auth.length - 1];
        out['EPSG / authority'] = a[1] + ':' + a[2];
    }
...
async function parseCpg(file) {
    ...
    if (!enc || enc.length > 64 || /[\x00]/.test(enc))
        return null;
    return {
...
async function parseGml(file) {
    ...
    if (members)
        out['Feature members'] = members.toLocaleString();
    ...
    while ((m = re.exec(text))) {
        ...
        if (pfx === 'gml' || pfx === 'xsd' || pfx === 'xs' || pfx === 'wfs' || pfx === 'ogc' || pfx === 'xlink')
            continue;
        types[name] = (types[name] || 0) + 1;
    }
    const typeNames = Object.keys(types);
    if (typeNames.length)
        out['Feature types'] = topCounts(types, 12);
...
async function parseNmea(file) {
    ...
    for (const raw of lines) {
        ...
        if (type === 'GGA') {
            ...
            const la = nmeaCoord(f[2], f[3]);
            const lo = nmeaCoord(f[4], f[5]);
            if (la != null && lo != null) {
            ...
            if (f[7])
                sats = Math.max(sats, parseInt(f[7], 10) || 0);
            if (f[8]) {
                const h = parseFloat(f[8]);
                if (isFinite(h))
                    hdop = hdop == null ? h : Math.min(hdop, h);
            }
        }
        else if (type === 'RMC') {
            if (f[1]) {
            ...
            const la = nmeaCoord(f[3], f[4]);
            const lo = nmeaCoord(f[5], f[6]);
            if (la != null && lo != null) {
                minLat = Math.min(minLat, la);
                maxLat = Math.max(maxLat, la);
                minLon = Math.min(minLon, lo);
            ...
            if (f[3])
                sats = Math.max(sats, parseInt(f[3], 10) || 0);
        }
    }
    if (!looksNmea)
        return null;
    ...
    if (firstT)
        out['First timestamp'] = fmtTime(firstT);
    if (lastT)
        out['Last timestamp'] = fmtTime(lastT);
    if (sats)
        out['Max satellites'] = sats;
    if (hdop != null)
        out['Best HDOP'] = fc(hdop);
    if (isFinite(minLat))
        out['Bounds (lon/lat)'] = fmtBBox(minLon, minLat, maxLon, maxLat);
    if (csOk + csBad)
        out['Checksums'] = csOk + ' ok, ' + csBad + ' bad';
    if (text.length >= 4_000_000)
        out['Note'] = 'stats cover the first 4 MB only';
    return out;
}
// =====================================================================
//  IGC flight log
...
function igcCoord(d) {
    ...
    if (!m)
        return null;
    let lat = +m[1] + (+m[2] + +m[3] / 1000) / 60;
    if (m[4] === 'S')
        lat = -lat;
    let lon = +m[5] + (+m[6] + +m[7] / 1000) / 60;
    if (m[8] === 'W')
        lon = -lon;
    return { lat, lon };
}
async function parseIgc(file) {
    ...
    for (const line of lines) {
        ...
        if (c === 'H') {
            ...
            if (m)
                headers[m[1]] = (m[3] || '').trim();
        }
        else if (c === 'B' && /^B\d{6}/.test(line)) {
            bcount++;
            const t = line.slice(1, 7);
            if (!firstT)
                firstT = t;
            lastT = t;
            const co = igcCoord(line.slice(7));
            if (co) {
                minLat = Math.min(minLat, co.lat);
                maxLat = Math.max(maxLat, co.lat);
                minLon = Math.min(minLon, co.lon);
                maxLon = Math.max(maxLon, co.lon);
            }
    ...
    if (!bcount && !Object.keys(headers).length)
        return null;
    if (headers.PLT || headers.PILOTINCHARGE)
        out['Pilot'] = headers.PLT || headers.PILOTINCHARGE;
    if (headers.GTY)
        out['Glider type'] = headers.GTY;
    if (headers.GID)
        out['Glider ID'] = headers.GID;
    if (headers.DTE)
        out['Date (DDMMYY)'] = headers.DTE.replace(/[^\d]/g, '').slice(0, 6) || headers.DTE;
    if (headers.CID)
        out['Competition ID'] = headers.CID;
    out['Fix records (B)'] = bcount.toLocaleString();
    const fmtTime = (t) => t && t.length >= 6 ? t.slice(0, 2) + ':' + t.slice(2, 4) + ':' + t.slice(4, 6) + ' UTC' : '-';
    if (firstT)
        out['Time span'] = fmtTime(firstT) + ' → ' + fmtTime(lastT);
    if (isFinite(minAlt))
        out['Altitude min/max'] = minAlt + ' / ' + maxAlt + ' m';
    if (isFinite(minLat))
        out['Bounds (lon/lat)'] = fmtBBox(minLon, minLat, maxLon, maxLat);
    return out;
}
...
async function parseTab(file) {
    ...
    if (!/!table/i.test(text) && !/^\s*!version/im.test(text))
        return null;
    ...
    if (links.length)
        out['Linked files'] = Array.from(new Set(links)).slice(0, 8).join(', ');
    return out;
}
async function parseMif(file) {
    const text = await readText(file, 4_000_000);
    if (!/\bVersion\b/i.test(text) || !/\b(Columns|CoordSys|Data)\b/i.test(text))
        return null;
    ...
    if (coordsys)
        out['CoordSys'] = coordsys.trim().slice(0, 120);
    ...
    let m;
    const re = /^\s*(Point|Line|Pline|Region|Arc|Text|Rect|Roundrect|Ellipse|Multipoint|Collection)\b/gim;
    ...
    if (geom) {
        out['Geometry objects'] = geom.toLocaleString();
        out['Geometry types'] = topCounts(gt, 10);
    }
...
async function parsePmtiles(file) {
    ...
    if (b.length < 127 || ascii(b, 0, 7) !== 'PMTiles')
        return null;
    ...
    if (spec === 3) {
        ...
        const minZoom = r.u8();
        const maxZoom = r.u8();
        const minLonE7 = r.i32();
        const minLatE7 = r.i32();
        const maxLonE7 = r.i32();
        const maxLatE7 = r.i32();
        const centerZoom = r.u8();
        const centerLonE7 = r.i32();
        const centerLatE7 = r.i32();
...
async function parseDted(file) {
    ...
    if (lonOrigin != null && latOrigin != null)
        out['SW corner (lon/lat)'] = fc(lonOrigin) + ', ' + fc(latOrigin);
    if (isFinite(lonInt))
        out['Lon post spacing'] = (lonInt / 10) + ' arc-sec';
    if (isFinite(latInt))
        out['Lat post spacing'] = (latInt / 10) + ' arc-sec';
    ...
    if (isFinite(lonLines) && isFinite(latPts))
        out['Grid'] = lonLines + ' × ' + latPts + ' posts';
    // Security code at byte 32.
    const sec = String.fromCharCode(b[32]);
    if (/[USCT]/.test(sec))
        out['Security'] = { U: 'Unclassified', S: 'Secret', C: 'Confidential', T: 'Top Secret' }[sec];
    ...
    try {
        ...
        const dataStart = (() => {
            ...
            for (const l of lines) {
                if (/^\s*[-\d.]/.test(l) && !/^\s*(ncols|nrows|xll|yll|cellsize|nodata)/i.test(l))
                    break;
                off += l.length + 1;
                idx++;
            }
        ...
        if (n)
            out['Value range (sampled)'] = fc(mn) + ' … ' + fc(mx);
    }
    catch (_) { }
    return out;
}
...
async function parseHgt(file, fileObj) {
    ...
    if (m) {
        let lat = +m[2];
        if (m[1].toUpperCase() === 'S')
            lat = -lat;
        let lon = +m[4];
        if (m[3].toUpperCase() === 'W')
            lon = -lon;
    ...
    try {
        ...
        if (isFinite(mn))
            out['Elevation min/max (sampled)'] = mn + ' / ' + mx + ' m';
    }
    catch (_) { }
    return out;
}
...
async function parseGpkg(file, ext) {
    ...
    try {
        ...
        try {
            ...
            if (contents && contents.values) {
                for (const rowv of contents.values) {
                    ...
                    try {
                        ...
                        if (cr && cr[0] && cr[0].values && cr[0].values[0])
                            count = Number(cr[0].values[0][0]);
                    }
                    catch (_) { }
                    let line = (tableName || '?') + '  [' + (dataType || '?') + ']';
                    if (count != null)
                        line += '  ' + count.toLocaleString() + (dataType === 'tiles' ? ' tiles' : ' rows');
                    if (identifier && identifier !== tableName)
                        line += '  - ' + identifier;
            ...
            if (features)
                out['Feature tables'] = features;
            if (tiles)
                out['Tile matrix sets'] = tiles;
            ...
            const sections = [];
            if (layers.length)
                sections.push({ title: 'Layers (' + layers.length + ')', node: preBlock(layers.join('\n')), open: true });
            if (srsLines.length)
                sections.push({ title: 'Spatial reference systems (' + srsLines.length + ')', node: preBlock(srsLines.join('\n')) });
            if (summary.tables.length)
                sections.push({ title: 'All tables (' + summary.tables.length + ')', node: preBlock(summary.tables.map((t) => t + (summary.rowCounts[t] != null ? '  (' + summary.rowCounts[t].toLocaleString() + ')' : '')).join('\n')) });
            if (sections.length)
...
async function parseMbtiles(file, ext) {
    ...
    try {
        ...
        try {
            ...
            if (md && md.values)
                for (const rowv of md.values)
                    meta[String(rowv[0])] = rowv[1];
            const pick = (k, label) => { if (meta[k] != null && meta[k] !== '')
                out[label] = String(meta[k]); };
            pick('name', 'Name');
            ...
            try {
                ...
                if (tc && tc[0] && tc[0].values && tc[0].values[0])
                    out['Tiles'] = Number(tc[0].values[0][0]).toLocaleString();
            }
            catch (_) { }
            const sections = [];
            const metaLines = Object.entries(meta).map(([k, v]) => k + ': ' + (v == null ? '' : String(v)));
            if (metaLines.length)
                sections.push({ title: 'metadata (' + metaLines.length + ')', node: preBlock(metaLines.join('\n')), open: true });
            if (summary.tables.length)
                sections.push({ title: 'All tables (' + summary.tables.length + ')', node: preBlock(summary.tables.map((t) => t + (summary.rowCounts[t] != null ? '  (' + summary.rowCounts[t].toLocaleString() + ')' : '')).join('\n')) });
            if (sections.length)
                out._sections = sections;
...
async function parseO5m(file, ext) {
    ...
    if (magic)
        out['Header magic'] = magic + (magic === 'o5c2' ? ' (change data)' : ' (data)');
...
async function parseLyrx(file) {
    ...
    if (!Array.isArray(j.layerDefinitions) && j.type !== 'CIMLayerDocument')
        return null;
    const out = { 'Format': 'Esri ArcGIS Pro Layer (.lyrx, CIM JSON)' };
    if (j.version)
        out['CIM version'] = String(j.version);
    if (j.build)
        out['Build'] = String(j.build);
    ...
    for (const d of defs.slice(0, 60)) {
        ...
        if (d.featureTable && d.featureTable.displayField)
            line += '  display: ' + d.featureTable.displayField;
        lines.push(line);
    }
    if (lines.length)
        out._sections = [{ title: 'Layers (' + defs.length + ')', node: preBlock(lines.join('\n')), open: true }];
    return out;
}
...
async function parseQgs(file) {
    ...
    if (ver)
        out['QGIS version'] = ver;
    const title = (text.match(/<title>([^<]*)<\/title>/i) || [])[1];
    if (title && title.trim())
        out['Project title'] = title.trim();
    // Project CRS.
    ...
    if (srs) {
        ...
        if (auth || desc)
            out['Project CRS'] = [auth, desc].filter(Boolean).join(' - ');
    }
...
async function parseCpt(file) {
    ...
    if (isFinite(zMin) && isFinite(zMax))
        out['Value (z) range'] = fc(zMin) + ' … ' + fc(zMax);
    if (hasBFN)
        out['Background/Foreground/NODATA'] = 'defined (B / F / N entries)';
...
//# sourceMappingURL=parsers-geodata.js.map