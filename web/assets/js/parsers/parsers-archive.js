...
async function zstdDecompress(bytes, maxOut = DECOMP_OUTPUT_MAX) {
    try {
        ...
        let total = 0;
        const OVER = {};
        const ds = new window.fzstd.Decompress((chunk) => {
            total += chunk.length;
            if (total > maxOut)
                throw OVER;
...
async function parseGzip(file, ext) {
    ...
    if (fname)
        out['Original filename'] = fname;
    if (mtimeRaw)
        out['Modified'] = fmtDate(new Date(mtimeRaw * 1000));
    out['OS'] = GZIP_OS[osId] != null ? GZIP_OS[osId] : (osId === 255 ? 'unknown' : 'OS ' + osId);
    if (xfl === 2)
        out['Deflate level'] = 'best (slowest)';
    else if (xfl === 4)
        out['Deflate level'] = 'fastest';
    if (isize != null) {
        out['Uncompressed (ISIZE mod 4GiB)'] = fmtBytes(isize);
        const comp = file.size - 18;
        if (comp > 0 && isize > 0)
            out['Ratio'] = (isize / comp).toFixed(2) + '×';
    }
    out['Compressed size'] = fmtBytes(file.size);
    if (extraField)
        out['Extra field'] = 'present';
    if (hasCrc)
        out['Header CRC'] = 'present';
    if (comment)
        out['Comment'] = comment;
    if (ext === 'tgz')
        out['Note'] = 'Wraps a TAR archive (gzip → tar member list)';
    return out;
}
// ---------- bzip2 (.bz2) ----------
...
function parseXzHeader(head) {
    ...
    if (!matchMagic(head, [0xfd, 0x37, 0x7a, 0x58, 0x5a, 0x00]))
        return null;
...
async function parseXz(head, file, ext) {
    ...
    try {
        ...
        if (file.size > MAX_IN) {
            out['Note'] = 'inner stream not expanded (over ' + fmtBytes(MAX_IN) + ' compressed)';
            return out;
        }
        ...
        if (looksTar || nameIsTar) {
            ...
            if (members) {
                ...
                if (items.length)
                    out._sections = [fileListSection('Files (' + items.length + (members > items.length ? '+' : '') + ')', items, true)];
                return out;
            }
        }
...
async function parseZstd(head, file, ext) {
    ...
    if (out.Format !== 'Zstandard compressed')
        return out;
    try {
        const MAX_IN = 64 * 1024 * 1024;
        if (file.size > MAX_IN) {
...
function parseZstdHeader(head) {
    ...
    if (fcsLen && p + fcsLen <= head.length) {
        let v = 0n;
        for (let i = 0; i < fcsLen; i++)
            v |= BigInt(head[p + i]) << BigInt(8 * i);
        if (fcsLen === 2)
            v += 256n;
        contentSize = v;
    }
    ...
    if (windowLog != null)
        out['Window size'] = fmtBytes(windowLog);
    else if (singleSeg)
        out['Window size'] = 'single segment (= content size)';
    if (contentSize != null)
        out['Content size'] = fmtBytes(Number(contentSize));
    if (dictFlag)
        out['Dictionary ID'] = 'present';
    return out;
}
// ---------- lz4 (.lz4) ----------
...
function parseLzma(head) {
    ...
    const props = head[0];
    if (props >= 225)
        return null; // lc+lp*9+pb*45 must be < 225
    const lc = props % 9;
    ...
    let usize = 0n;
    for (let i = 0; i < 8; i++)
        usize |= BigInt(head[5 + i]) << BigInt(8 * i);
    const out = {
...
async function parseCpio(file) {
    ...
    const b = await readBytes(file, cap);
    let variant = null;
    if (b[0] === 0x30 && b[1] === 0x37 && b[2] === 0x30 && b[3] === 0x37 && b[4] === 0x30 && b[5] === 0x37)
        variant = 'odc'; // 070707
    else if (ascii(b, 0, 6) === '070701')
        variant = 'newc';
    else if (ascii(b, 0, 6) === '070702')
        variant = 'crc';
    else if ((b[0] === 0xc7 && b[1] === 0x71) || (b[0] === 0x71 && b[1] === 0xc7))
        variant = 'bin';
    if (!variant)
        return null;
    const items = [];
    ...
    try {
        while (pos + 110 <= b.length) {
            if (variant === 'newc' || variant === 'crc') {
                if (ascii(b, pos, 6) !== '070701' && ascii(b, pos, 6) !== '070702')
                    break;
                ...
                count++;
                total += filesize;
                if (items.length < 1000 && name)
                    items.push({ name, size: filesize });
                pos += headerLen + namePad + dataLen + dataPad;
            }
            else if (variant === 'odc') {
                if (ascii(b, pos, 6) !== '070707')
                    break;
    ...
    if (variant !== 'bin')
        out['Total size'] = fmtBytes(total) + (truncated ? '+' : '');
    if (items.length)
        out._sections = [fileListSection('Files (' + items.length + (truncated ? '+' : '') + ')', items, true)];
    return out;
}
...
async function parseArMembers(file, maxBytes = 16 * 1024 * 1024) {
    ...
    if (b.length < 8 || AR_MAGIC.some((c, i) => b[i] !== c))
        return null;
    const members = [];
    let pos = 8;
    while (pos + 60 <= b.length) {
        ...
        if (!(size >= 0 && size <= b.length) || members.length >= LIST_ENTRIES_MAX)
            break;
...
async function parseLib(file) {
    ...
    for (const m of objs) {
        ...
        if (sig1 === 0x0000 && sig2 === 0xffff) {
            ...
            const readStr = () => { let s = ''; const stop = Math.min(end, p + 4096); while (p < stop && b[p] !== 0)
                s += String.fromCharCode(b[p++]); p++; return s; };
    ...
    if (machines.size)
        out['Architecture'] = [...machines].map((m) => COFF_MACHINE[m] || ('0x' + m.toString(16))).join(', ');
    if (objCount)
        out['Object members'] = objCount.toLocaleString();
    if (importCount)
        out['Import entries'] = importCount.toLocaleString();
    if (dlls.size)
        out['Imports from'] = [...dlls].sort().slice(0, 12).join(', ') + (dlls.size > 12 ? ' +' + (dlls.size - 12) + ' more' : '');
    const items = objs.map((m) => ({ name: m.name, size: m.size }));
    if (items.length)
        out._sections = [fileListSection('Members (' + items.length + ')', items, true)];
    return out;
}
// ---------- deb (.deb) ----------
...
async function parseDeb(file) {
    ...
    if (verMember)
        out['Package format'] = ascii(ar.buf, verMember.dataStart, verMember.size).trim();
    ...
    if (ctrl) {
        ...
        try {
            ...
            if (/\.gz$/.test(ctrl.name))
                tarBytes = await gunzip(tarBytes, SCAN_SMALL, { partial: true });
            else if (/\.xz$/.test(ctrl.name))
                tarBytes = await xzDecompress(tarBytes); // lazy LZMA2 (null on failure)
            else if (/\.zst$/.test(ctrl.name))
                tarBytes = null; // can't decode natively
            if (tarBytes) {
...
async function parseRpm(file) {
    ...
    function readHeader(off) {
        if (!(b[off] === 0x8e && b[off + 1] === 0xad && b[off + 2] === 0xe8))
            return null;
    ...
    if (hdr) {
        ...
        for (const [k, label] of [
            ...
            if (fields[k] != null)
                out[label] = k === 'Size' ? fmtBytes(fields[k]) : fields[k];
        }
        if (fields['BuildTime'])
            out['Build time'] = fmtDate(new Date(fields['BuildTime'] * 1000));
        if (fields['Description'])
            out._sections = [{ title: 'Description', node: preBlock(String(fields['Description'])) }];
    }
    return out;
}
// ---------- gem (.gem) ----------
async function parseGem(file) {
    ...
    if (ascii(b, 257, 5) !== 'ustar' && octal(b, 124, 12) <= 0)
        return null;
    ...
    if (metaGz) {
        try {
            ...
            if (ver)
                out['Version'] = ver[1].trim();
            if (pick('summary'))
                out['Summary'] = pick('summary');
            if (pick('homepage'))
                out['Homepage'] = pick('homepage');
            const licenses = (yaml.match(/^licenses:\n((?:\s*-\s*.+\n?)+)/m) || [])[1];
            if (licenses)
                out['Licenses'] = licenses.replace(/\s*-\s*/g, '').trim().split(/\s+/).join(', ');
            const authors = (yaml.match(/^authors:\n((?:\s*-\s*.+\n?)+)/m) || [])[1];
            if (authors)
                out['Authors'] = authors.replace(/\s*-\s*/g, '').trim().split(/\n/).map((s) => s.trim()).join(', ');
...
async function parseCab(file) {
    ...
    if (flags & 0x0001)
        out['Prev cabinet'] = 'this is a continuation (multi-part set)';
    if (flags & 0x0002)
        out['Next cabinet'] = 'continues in another file';
    ...
    try {
        ...
        for (let i = 0; i < cFiles && pos + 16 < b.length && items.length < 1000; i++) {
            ...
            let name = '';
            let np = fr.tell();
            while (np < b.length && b[np] !== 0)
                name += String.fromCharCode(b[np++]);
            np++;
        ...
        if (items.length)
            out._sections = [fileListSection('Files (' + items.length + ')', items, true)];
    }
    catch (_) { }
    return out;
}
// ---------- ZIP-based packages (openZip) ----------
async function zipText(zip, name) { try {
    return await zip.text(name);
}
catch (_) {
    return null;
} }
function jsonTry(s) { try {
    return JSON.parse(s);
}
catch (_) {
    return null;
...
async function parseWhl(file) {
    ...
    if (metaEntry) {
        ...
        if (text) {
            ...
            if (f['Author'] || f['Author-email'])
                out['Author'] = f['Author'] || f['Author-email'];
            if (f['Requires-Python'])
                out['Requires Python'] = f['Requires-Python'];
    ...
    if (wheelEntry) {
        const w = parseControlFields(await zipText(zip, wheelEntry.name) || '');
        if (w['Tag'])
            out['Build tag'] = w['Tag'];
    }
...
async function parseNupkg(file) {
    ...
    if (nuspec) {
        ...
        if (xml) {
            ...
            if (pick('license'))
                out['License'] = pick('license');
            if (pick('projectUrl'))
                out['Project URL'] = pick('projectUrl');
            if (pick('description'))
                out['Description'] = pick('description');
    ...
    try {
        ...
        if (m) {
            ...
            if (m.manifest_version)
                out['Manifest version'] = m.manifest_version;
            if (m.description)
...
async function parseXpi(file) {
    ...
    if (m) {
        ...
        if (gecko?.id)
            out['Gecko ID'] = gecko.id;
        if (gecko?.strict_min_version)
            out['Min Firefox'] = gecko.strict_min_version;
...
async function parseVsix(file) {
    ...
    if (pkgEntry) {
        ...
        if (p) {
            if (p.engines?.vscode)
                out['VS Code engine'] = p.engines.vscode;
            if (Array.isArray(p.activationEvents))
                out['Activation events'] = p.activationEvents.length;
            if (p.contributes?.commands)
                out['Commands'] = p.contributes.commands.length;
        }
    }
    return Object.keys(out).length > 1 ? out : null;
}
...
function walkAsarTree(node, prefix, out, sizeRef) {
    ...
    for (const [name, info] of Object.entries(node.files)) {
        ...
        if (info.files)
            walkAsarTree(info, path + '/', out, sizeRef);
        else {
            const size = info.size || 0;
            sizeRef.total += size;
            sizeRef.count++;
            if (out.length < 1000)
                out.push({ name: path + (info.executable ? '  [exec]' : ''), size });
        }
    }
}
    ...
    if (json.files && json.files['package.json']) {
        ...
        if (info.offset != null && info.size) {
            try {
                ...
                if (pj) {
                    if (pj.name)
                        out['App name'] = pj.name;
                    if (pj.version)
                        out['App version'] = pj.version;
                }
            }
            catch (_) { }
        }
...
async function parseAppx(file) {
    ...
    if (zip.has('AppxBundleManifest.xml') || zip.entries.some((e) => /AppxBundleManifest/i.test(e.name)))
        out['Bundle'] = 'yes (.appxbundle / .msixbundle)';
    return Object.keys(out).length > 1 ? out : null;
}
// .apkg (Anki deck)
async function parseApkg(file) {
    ...
    if (mediaText) {
        ...
        if (m)
            out['Media files'] = Object.keys(m).length;
    }
    const mediaFiles = zip.entries.filter((e) => /^\d+$/.test(e.name)).length;
    if (mediaFiles && !out['Media files'])
        out['Media files'] = mediaFiles;
    return out;
}
...
async function parseXapk(file, ext) {
    ...
    if (pkg)
        out['Package'] = pkg;
    if (verName || verCode)
        out['Version'] = [verName, verCode ? '(code ' + verCode + ')' : ''].filter(Boolean).join(' ');
    const minSdk = m.min_sdk_version || m.min_api;
    const tgtSdk = m.target_sdk_version;
    if (minSdk)
        out['Min SDK'] = String(minSdk);
    if (tgtSdk)
        out['Target SDK'] = String(tgtSdk);
    if (Array.isArray(m.arches) && m.arches.length)
        out['ABIs'] = m.arches.join(', ');
    if (typeof m.total_size === 'number' && m.total_size > 0)
        out['Declared size'] = fmtBytes(m.total_size);
    ...
    if (apkCount)
        out['APKs inside'] = apkCount + (apkCount > 1 ? ' (base + splits)' : '');
    ...
    if (expCount)
        out['OBB expansions'] = expCount;
    if (Array.isArray(m.permissions) && m.permissions.length)
        out['Permissions'] = m.permissions.length;
    ...
    if (rows.length)
        out._sections = [fileListSection('Packages (' + rows.length + ')', rows, true)];
    if (Array.isArray(m.permissions) && m.permissions.length) {
    ...
    if (!appName && !pkg && !apkCount)
        out['Note'] = 'No manifest.json / info.json and no APK members found - browse the archive tree below.';
    return out;
}
...
async function parseConda(file) {
    ...
    if (meta) {
        const m = jsonTry(meta);
        if (m && m.conda_pkg_format_version)
            out['Format version'] = m.conda_pkg_format_version;
    }
            ...
            if (zstBytes && zstBytes.length) {
                ...
                if (tarBytes && tarBytes.length) {
                    ...
                    if (idxBytes) {
                        ...
                        if (idx) {
                            if (idx.name)
                                out['Package'] = idx.name;
                            if (idx.version)
                                out['Version'] = idx.version;
                            if (idx.build)
                                out['Build'] = idx.build;
                            if (idx.subdir || idx.platform)
                                out['Platform'] = idx.subdir || idx.platform;
    ...
    if (!out['Package'])
        out['Note'] = 'info/index.json lives inside a .tar.zst member; could not be decompressed.';
    return out;
}
// ---------- XAR (.xar, .pkg, .mpkg) ----------
    ...
    try {
        if (tocComp > 0 && tocComp <= 32 * 1024 * 1024) {
            ...
            if (xmlBytes && xmlBytes.length) {
                ...
                if (fileEls)
                    out['Members'] = fileEls.toLocaleString();
                ...
                if (total)
                    out['Total uncompressed'] = fmtBytes(total);
                if (items.length)
                    out._sections = [fileListSection('Files (' + items.length + (fileEls > items.length ? '+' : '') + ')', items, true)];
                else if (names.length)
                    out._sections = [{ title: 'TOC', node: preBlock(names.slice(0, 500).join('\n')) }];
            }
            else {
...
function parseTigerPkg(head, file) {
    ...
    if (version === 0 || version >= 0x100 || tag2 !== 0x0002)
        return null;
    ...
    if (nameId !== null && nameId !== pkgId)
        return null; // not a Tiger package
    const out = {
    ...
    if (m) {
        ...
        if (TIGER_PLATFORM[plat])
            out['Platform'] = TIGER_PLATFORM[plat];
    }
    ...
    if (ts > 0x40000000 && ts < 0x90000000)
        out['Build date'] = fmtDate(new Date(ts * 1000));
...
async function parseLzo(file) {
    ...
    if (!matchMagic(b, [0x89, 0x4c, 0x5a, 0x4f, 0x00, 0x0d, 0x0a, 0x1a, 0x0a]))
        return null;
    const r = new Reader(b); // big-endian
    r.seek(9);
    const version = r.u16();
    ...
    if (mtimeLow)
        out['Modified'] = fmtDate(new Date(mtimeLow * 1000));
    ...
    try {
        ...
        if (nameLen > 0 && nameLen < 255) {
            let nm = '';
            const at = r.tell();
            for (let i = 0; i < nameLen && at + i < b.length; i++)
                nm += String.fromCharCode(b[at + i]);
            if (nm)
                out['Original filename'] = nm;
        }
    }
    catch (_) { }
    if (flags & 0x00000001)
        out['Adler32'] = 'header checksum present';
    if (flags & 0x00001000)
        out['CRC32'] = 'header checksum present';
    out['Compressed size'] = fmtBytes(file.size);
...
async function parseSnap(file, ext) {
    ...
    let little;
    if (b[0] === 0x68 && b[1] === 0x73 && b[2] === 0x71 && b[3] === 0x73)
        little = true; // 'hsqs' LE
    else if (b[0] === 0x73 && b[1] === 0x71 && b[2] === 0x73 && b[3] === 0x68)
        little = false; // 'sqsh' BE
    else
        return null;
    ...
    if (bytesUsed)
        out['Bytes used'] = fmtBytes(bytesUsed);
    if (modTime)
        out['Modified'] = fmtDate(new Date(modTime * 1000));
    if (ext === 'snap')
        out['Note'] = 'meta/snap.yaml (name/version/confinement) lives inside the compressed image; not extracted in-browser.';
    return out;
}
// ---------- StuffIt (.sit, .sitx) ----------
...
async function parseBr(file) {
    ...
    try {
        if (typeof DecompressionStream !== 'undefined' && file.size <= 64 * 1024 * 1024) {
            ...
            const decoded = await inflate(file, 'br', DECOMP_OUTPUT_MAX);
            if (decoded && decoded.length) {
                out['Decompressed size'] = fmtBytes(decoded.length);
                if (file.size > 0)
                    out['Ratio'] = (decoded.length / file.size).toFixed(2) + '×';
            }
        }
    }
    catch (_) { }
    if (!out['Decompressed size'])
        out['Note'] = 'Raw Brotli has no header; decode unavailable in this browser (size-only).';
    return out;
}
...
async function parseObb(file) {
    ...
    if (head[0] === 0x50 && head[1] === 0x4B) {
        ...
        try {
            ...
            if (zip && zip.entries) {
                ...
                if (items.length)
                    out._sections = [fileListSection('Files (' + items.length + (zip.entries.length > items.length ? '+' : '') + ')', items, true)];
            }
        }
        catch (_) {
            out['Note'] = 'ZIP-based OBB; member list could not be read.';
        }
        return out;
    }
    ...
    if (printable)
        out['Container signature'] = fourcc.replace(/[^\x20-\x7e]/g, '.') + (fourcc === 'WBND' ? ' (game-specific bundle)' : '');
...
//# sourceMappingURL=parsers-archive.js.map