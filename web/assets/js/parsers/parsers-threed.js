...
async function parseObj(file) {
    ...
    for (const line of lines) {
        ...
        if (l.startsWith('v ')) {
            ...
            l.slice(6).trim().split(/\s+/).forEach((m) => m && mtllibs.add(m));
    ...
    if (usemtl)
        out['Material refs (usemtl)'] = usemtl;
    if (mtllibs.size)
        out['Material libraries'] = Array.from(mtllibs).join(', ');
    const box = fmtBbox(bb);
    if (box)
        out['Bounding box'] = box;
    return out;
}
// ---------- PLY (Stanford) ----------
async function parsePly(file) {
    ...
    for (const raw of lines) {
        ...
        if (l.startsWith('format ')) {
            ...
            comments.push(l.slice(8).trim());
        else if (l.startsWith('element ')) {
            const p = l.split(/\s+/);
            cur = { name: p[1], count: parseInt(p[2], 10) || 0 };
            elements.push(cur);
        }
    ...
    if (vtx)
        out['Vertices'] = vtx.count.toLocaleString();
    if (face)
        out['Faces'] = face.count.toLocaleString();
    const others = elements.filter((e) => e.name !== 'vertex' && e.name !== 'face');
    if (others.length)
        out['Other elements'] = others.map((e) => e.name + ' (' + e.count + ')').join(', ');
    ...
    if (software)
        out['Comment'] = comments.slice(0, 4).join(' | ');
    return out;
}
// ---------- OFF ----------
async function parseOff(file) {
    ...
    const first = () => { while (i < lines.length) {
    ...
    if (nums.length < 2 || !isFinite(nums[0]))
        return null;
    const [nv, nf, ne] = nums;
    const out = {
    ...
    if (ne)
        out['Edges'] = ne.toLocaleString();
    out['Per-vertex colour'] = colored ? 'yes' : 'no';
    if (hasNormals)
        out['Per-vertex normals'] = 'yes';
    return out;
}
// ---------- glTF (JSON) ----------
async function parseGltf(file) {
    let j;
    try {
        j = JSON.parse(await file.slice(0, Math.min(file.size, PARSE_TEXT_MAX)).text());
    }
    catch (_) {
        return null;
    }
    ...
    if (a.version)
        out['glTF version'] = a.version;
    if (a.generator)
        out['Generator'] = a.generator;
    if (a.copyright)
        out['Copyright'] = a.copyright;
    ...
    if (Array.isArray(j.meshes)) {
        ...
        if (prims)
            out['Mesh primitives'] = prims;
    }
    const exts = new Set([...(j.extensionsUsed || []), ...(j.extensionsRequired || [])]);
    if (exts.size)
        out['Extensions'] = Array.from(exts).join(', ');
    return out;
}
// ---------- 3MF ----------
...
async function parseAmf(file) {
    ...
    if (title)
        out['Title'] = title;
    const author = meta('Author') || meta('Designer');
    if (author)
        out['Author'] = author;
    out['Objects'] = (xml.match(/<object\b/gi) || []).length;
...
async function parseVox(file) {
    ...
    const sizes = [];
    let voxels = 0, models = 0, paletteCustom = false;
    try {
        // expect MAIN chunk
    ...
    if (sizes.length)
        out['Model dimensions'] = sizes.slice(0, 4).map((s) => s.join('x')).join(', ') + (sizes.length > 4 ? ', ...' : '');
    out['Models'] = models || sizes.length || 1;
    if (voxels)
        out['Voxels'] = voxels.toLocaleString();
...
function parseDaeXml(xml) {
    ...
    if (unitM) {
        ...
        if (name || meter)
            out['Units'] = (name || '') + (meter ? ' (' + meter + ' m)' : '');
    }
    const ver = (xml.match(/<COLLADA[^>]*\bversion="([^"]+)"/i) || [])[1];
    if (ver)
        out['COLLADA version'] = ver;
...
async function parseZae(file) {
    ...
    if (assets.length)
        out['_sections'] = [{ title: 'Bundled assets (' + assets.length + ')', node: preBlock(assets.join('\n')) }];
    return out;
}
// ---------- USD Crate (.usdc binary) ----------
async function parseUsdc(file) {
    const b = await head(file, 64);
    if (ascii(b, 0, 8) !== 'PXR-USDC')
        return null;
    const r = new Reader(b, true);
    r.seek(8);
...
async function parseX3d(file, ext) {
    ...
    if (file.size > SAMPLE)
        out['Note'] = 'counts from first 2 MB';
    return out;
}
// ---------- LightWave LWO / LWS ----------
async function parseLwo(file, ext) {
    ...
    if (ext === 'lws') {
        ...
        if (fr || lr)
            out['Frame range'] = (fr || '?') + ' - ' + (lr || '?');
        return out;
    }
    // LWO: IFF FORM container
    if (ascii(b, 0, 4) !== 'FORM')
        return null;
    const r = new Reader(b, false);
    r.seek(4);
    const formLen = r.u32();
    const formType = r.ascii(4);
    if (!/^(LWO2|LWOB|LWO3|LWLO)$/.test(formType))
        return null;
    const out = {
    ...
    try {
        while (r.remaining() >= 8) {
            ...
            if (id === 'LAYR')
                ...
                points += Math.floor(len / 12);
            else if (id === 'POLS')
                polys++;
            else if (id === 'SURF')
                surfaces++;
            if (next <= r.tell() || next > b.length)
                break;
            r.seek(next);
        }
    }
    ...
    out['Layers'] = layers || 1;
    if (points)
        out['Points'] = points.toLocaleString();
    if (polys)
        out['Polygon chunks'] = polys;
    if (surfaces)
        out['Surfaces'] = surfaces;
    if (file.size > SAMPLE)
        out['Note'] = 'partial (first 2 MB)';
    return out;
}
...
async function parseLas(file, ext) {
    ...
    if (verMajor === 1 && verMinor >= 4 && b.length >= 255) {
        try {
            const r2 = new Reader(b, true);
            r2.seek(247);
            const big = Number(r2.u64());
            if (big > 0)
                pointCount = big;
        }
        catch (_) { }
    ...
    if (genSoft)
        out['Generating software'] = genSoft;
    if (systemId)
        out['System ID'] = systemId;
    if (creationYear)
        out['Created'] = 'day ' + creationDay + ', ' + creationYear;
    ...
    if (ext === 'laz')
        out['Compression'] = 'LASzip (LAZ)';
    return out;
}
// ---------- PCD (Point Cloud Data) ----------
async function parsePcd(file) {
    const text = await headText(file, 8192);
    if (!/(^|\n)\s*(#|VERSION|FIELDS)/i.test(text.slice(0, 200)) && !/POINTS/.test(text))
        return null;
    ...
    if (size)
        out['Field sizes'] = size;
    const type = line('TYPE');
    if (type)
        out['Field types'] = type;
    const width = line('WIDTH'), height = line('HEIGHT');
    if (width || height)
        out['Dimensions'] = (width || '?') + ' x ' + (height || '?');
    if (points)
        out['Points'] = (parseInt(points, 10) || points).toLocaleString();
    const vp = line('VIEWPOINT');
    if (vp)
        out['Viewpoint'] = vp;
    const data = line('DATA');
    if (data)
        out['Data encoding'] = data;
    return out;
}
...
async function parsePtsPtx(file, ext) {
    ...
    const lines = text.split(/\r?\n/);
    let i = 0;
    const nextNonEmpty = () => { while (i < lines.length) {
        const l = lines[i++].trim();
        if (l)
            return l;
    } return null; };
    if (ext === 'ptx') {
        ...
        if (first)
            out['Columns/point'] = describePtsCols(first.split(/\s+/).length);
        return out;
    }
    ...
    let dataLine = first;
    if (/^\d+$/.test(first)) {
        out['Declared points'] = parseInt(first, 10).toLocaleString();
        dataLine = nextNonEmpty();
    }
    if (!dataLine)
        return null;
    ...
    if (xmlOffset > 0 && xmlLength > 0 && xmlOffset < file.size && pageSize > 4 && pageSize <= 1 << 20) {
        try {
            ...
            for (let i = 0; i < phys.length && n < want; i++) {
                if ((xmlOffset + i) % pageSize < pageSize - 4)
                    logical[n++] = phys[i];
            }
            ...
            if (scans)
                out['Scans (data3D)'] = scans;
            if (records)
                out['Total point records'] = records.toLocaleString();
            if (sensorVendor || sensorModel)
                out['Sensor'] = [sensorVendor, sensorModel].filter(Boolean).join(' ');
            if (ts)
                out['Timestamp'] = ts;
            if (guid)
                out['GUID'] = guid;
...
async function parseIfc(file) {
    const text = await headText(file, Math.min(file.size, SAMPLE));
    if (!/ISO-10303-21|FILE_SCHEMA/i.test(text))
        return null;
    const out = parseIfcText(text);
    if (file.size > SAMPLE)
        out['Note'] = 'entity counts from first 2 MB';
    return out;
}
...
async function parseVrm(file) {
    ...
    if (j.asset && j.asset.generator)
        out['Generator'] = j.asset.generator;
    const meta = ext && (ext.meta || ext);
    if (meta) {
        const title = meta.title || meta.name;
        if (title)
            out['Title'] = title;
        const author = meta.author || (Array.isArray(meta.authors) ? meta.authors.join(', ') : null);
        if (author)
            out['Author'] = author;
        const license = meta.licenseName || meta.licenseUrl || meta.commercialUssageName || meta.avatarPermission;
        if (license)
    ...
    if (ext && ext.humanoid && Array.isArray(ext.humanoid.humanBones))
        out['Humanoid bones'] = ext.humanoid.humanBones.length;
    else if (ext && ext.humanBones)
        out['Humanoid bones'] = Array.isArray(ext.humanBones) ? ext.humanBones.length : Object.keys(ext.humanBones).length;
    const bs = ext && (ext.blendShapeMaster || ext.expressions || ext.blendShape);
    if (bs && bs.blendShapeGroups)
        out['Blendshapes'] = bs.blendShapeGroups.length;
    else if (bs && bs.preset)
        out['Blendshapes'] = Object.keys(bs.preset).length;
...
async function parseJt(file) {
    ...
    try {
        ...
        if (tocOffset > 0 && tocOffset < file.size) {
            ...
            if (entryCount > 0 && entryCount < 1e6)
                out['TOC segments'] = entryCount;
        }
    }
    catch (_) { }
...
async function parseSplat(file) {
    ...
    if (file.size < 32 || file.size % 32 !== 0) {
        ...
        if (file.size % 32 !== 0)
            return { 'Format': 'Gaussian splat (.splat)', 'Note': 'size not a multiple of 32 - non-standard layout', 'File size': fmtBytes(file.size) };
    }
    ...
    try {
        ...
        const box = fmtBbox(bb);
        if (box)
            out['Bounding box' + (n < count ? ' (sampled)' : '')] = box;
    }
    catch (_) { }
    return out;
}
...
async function parseDraco(file) {
    ...
    if (encoderType === 1)
        out['Method'] = encoderMethod === 0 ? 'edgebreaker' : encoderMethod === 1 ? 'sequential' : 'method ' + encoderMethod;
...
async function parseQb(file) {
    ...
    if (numMatrices === 0 || numMatrices > 100000)
        return null;
    const out = {
    ...
    try {
        for (let i = 0; i < numMatrices && r.remaining() > 16; i++) {
            ...
            if (compressed)
                break; // can't skip RLE voxel data cheaply
            const voxBytes = sx * sy * sz * 4;
            if (voxBytes > r.remaining())
                break;
            r.skip(voxBytes);
        }
    }
    ...
    if (names.length)
        out['Matrix list'] = names.slice(0, 8).join(', ') + (names.length > 8 ? ', ...' : '');
    if (!compressed && totalVoxels)
        out['Grid voxels'] = totalVoxels.toLocaleString();
    return out;
}
...
async function parseU3d(file) {
    ...
    if (blockType !== 0x00443355)
        return null; // 'U3D\0' file-header marker
    ...
    if (profile & 0x2)
        profiles.push('extensible');
    if (profile & 0x4)
        profiles.push('no compression');
    if (profile & 0x8)
        profiles.push('defined units');
    const out = {
    ...
    if (profiles.length)
        out['Profile flags'] = profiles.join(', ');
...
async function parseWings(file) {
    ...
    if (verM)
        out['File tag version'] = verM[1];
    // Locate the gzip stream after the tag and inflate.
    let gzStart = -1;
    for (let i = 0; i < raw.length - 1; i++) {
        if (raw[i] === 0x1f && raw[i + 1] === 0x8b) {
            gzStart = i;
            break;
        }
    }
    if (gzStart >= 0) {
        try {
            ...
            if (inf && inf.length) {
                ...
                if (objects)
                    out['Objects (approx)'] = objects;
                if (mats)
                    out['Material refs (approx)'] = mats;
            }
        }
        catch (_) { }
    }
...
async function parseRevit(file, ext) {
    ...
    let bytes = null;
    try {
        bytes = cfbf.readStream((c) => /BasicFileInfo/i.test(c.name));
    }
    catch (_) { }
    if (bytes && bytes.length) {
        ...
        if (central && central[1])
            out['Central model'] = central[1].trim();
        if (/IsSingleUserCloudModel|IsLocal/i.test(txt)) {
            const wsm = /Worksharing:\s*([^\r\n]+)/i.exec(txt);
            if (wsm)
                out['Worksharing'] = wsm[1].trim();
        }
    }
    out['OLE streams'] = cfbf.entries.filter((e) => e.type === 2).length;
    if (!out['Version'] && !out['Revit build'])
        out['Note'] = 'Autodesk Revit OLE2 document - version blob not located.';
    return out;
}
...
function parseIdeaMaker(c) {
    ...
    if (sig.startsWith('IDEA - MAKER')) {
        format = 'ideaMaker project';
        app = 'Raise3D ideaMaker project';
        signature = 'IDEA - MAKER';
    }
    else if (sig.startsWith('IEDA - PROFILE')) {
        format = 'ideaMaker print profile';
        app = 'Raise3D ideaMaker print profile';
        signature = 'IEDA - PROFILE';
    }
    else {
        return null;
    }
...
async function parseAbc(file) {
    ...
    let variant = 'unknown';
    if (ascii(b, 0, 5) === 'Ogawa')
        variant = 'Ogawa';
    else if (b[0] === 0x89 && ascii(b, 1, 3) === 'HDF')
        variant = 'HDF5 (legacy)';
    return {
        'Format': 'Alembic (ABC)',
        'Container': variant,
...
//# sourceMappingURL=parsers-threed.js.map