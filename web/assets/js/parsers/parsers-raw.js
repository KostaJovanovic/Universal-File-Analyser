...
async function parseAae(file) {
    ...
    if (v.adjustmentFormatVersion != null)
        out['Format version'] = v.adjustmentFormatVersion;
    if (v.adjustmentBaseVersion != null)
        out['Base version'] = v.adjustmentBaseVersion;
    if (v.adjustmentFormatIdentifier)
        out['Adjustment type'] = v.adjustmentFormatIdentifier;
    if (v.adjustmentEditorBundleID)
        out['Editor'] = v.adjustmentEditorBundleID;
    if (v.adjustmentData)
        out['Edits'] = 'present (encoded adjustmentData)';
    return out;
}
// RawTherapee .pp3 processing profile (INI-style).
async function parsePp3(file) {
    const text = await file.text();
    if (!/\[(Version|General|Exposure|RAW|White Balance)\]/i.test(text))
        return null;
...
//# sourceMappingURL=parsers-raw.js.map