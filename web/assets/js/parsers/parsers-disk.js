...
function parseOvfXml(xml) {
    ...
    if (osId || osDesc)
        out['Guest OS'] = [osDesc, osId ? 'id ' + osId : ''].filter(Boolean).join(' ');
    ...
    for (const it of items) {
        ...
        if (rt === '3' && qty)
            ...
            mem = qty; // MB (per AllocationUnits, usually)
        else if (rt === '10')
            nics++; // ethernet adapter
    }
    if (vcpu)
        out['vCPU'] = vcpu;
    if (mem)
    ...
    for (const d of diskTags) {
        ...
        if (cap)
            disks.push((id ? id + ': ' : '') + cap + (units ? ' ' + units : ' bytes'));
    }
    if (diskTags.length)
        out['Disks'] = diskTags.length;
    const hw = xmlAttr(xml, 'System', 'vssd:VirtualSystemType') || xmlText(xml, 'VirtualSystemType');
    if (hw)
        out['Hardware type'] = hw;
    const eula = xmlText(xml, 'License') || xmlText(xml, 'EulaSection');
    const sections = [];
    if (disks.length)
        sections.push({ title: 'Disk capacities (' + disks.length + ')', node: preBlock(disks.join('\n')) });
    if (eula)
        sections.push({ title: 'EULA / License', node: preBlock(eula.slice(0, 8000)) });
    if (sections.length)
...
async function parseOvf(file) {
    ...
    if (!/<\s*(?:\w+:)?Envelope/i.test(text))
        return null;
    try {
        return parseOvfXml(text);
    }
    catch (_) {
        return null;
    }
...
async function parseVbox(file) {
    const text = await file.slice(0, Math.min(file.size, 2 * 1024 * 1024)).text();
    if (!/<VirtualBox\b/i.test(text) && !/<Machine\b/i.test(text))
        return null;
    ...
    const sections = [];
    if (disks.length)
        sections.push({ title: 'Attached disks (' + disks.length + ')', node: preBlock(disks.join('\n')) });
    if (snaps) {
        const names = (text.match(/<Snapshot\b[^>]*\bname="([^"]+)"/gi) || [])
            .map((m) => (m.match(/name="([^"]+)"/i) || [])[1]).filter(Boolean);
        if (names.length)
            sections.push({ title: 'Snapshot tree (' + names.length + ')', node: preBlock(names.join('\n')) });
    }
    if (sections.length)
        out._sections = sections;
    return out;
...
async function parseVmx(file) {
    ...
    if (!('config.version' in kv) && !('virtualhw.version' in kv) && !('displayname' in kv))
        return null;
    const out = { 'Format': 'VMware VM config (.vmx)' };
    if (kv['displayname'])
        out['Display name'] = kv['displayname'];
    if (kv['guestos'])
        out['Guest OS'] = kv['guestos'];
    if (kv['memsize'])
        out['Memory'] = kv['memsize'] + ' MB';
    if (kv['numvcpus'])
        out['vCPU'] = kv['numvcpus'];
    if (kv['cpuid.corespersocket'])
        out['Cores/socket'] = kv['cpuid.corespersocket'];
    if (kv['virtualhw.version'])
        out['Hardware version'] = kv['virtualhw.version'];
    if (kv['firmware'])
        out['Firmware'] = kv['firmware'];
    if (kv['uuid.bios'])
        out['BIOS UUID'] = kv['uuid.bios'].trim();
    // Disks: <ctrl><n>:<m>.fileName referencing a vmdk
    const disks = [];
    for (const [k, v] of Object.entries(kv)) {
        if (/\.filename$/.test(k) && /\.vmdk$/i.test(v))
            disks.push(v);
    }
    if (disks.length)
        out['Disks'] = disks.length;
    // NICs: ethernetN.present = TRUE
    let nics = 0;
    for (const [k, v] of Object.entries(kv))
        if (/^ethernet\d+\.present$/.test(k) && /true/i.test(v))
            nics++;
    if (nics)
        out['Network adapters'] = nics;
    const sections = [];
    if (disks.length)
        sections.push({ title: 'Virtual disks (' + disks.length + ')', node: preBlock(disks.join('\n')) });
    if (sections.length)
        out._sections = sections;
    return out;
}
...
async function parseCue(file) {
    ...
    if (!/^\s*(FILE|REM|TRACK)\b/im.test(text))
        return null;
    const files = [];
    const modes = {};
    let tracks = 0;
    ...
    for (const raw of text.split(/\r?\n/)) {
        const line = raw.trim();
        let m;
        if ((m = line.match(/^FILE\s+"?(.+?)"?\s+(\w+)\s*$/i)))
            files.push(m[1] + ' (' + m[2] + ')');
        else if ((m = line.match(/^TRACK\s+(\d+)\s+(\S+)/i))) {
            tracks++;
            modes[m[2]] = (modes[m[2]] || 0) + 1;
        }
        else if ((m = line.match(/^INDEX\s+(\d+)\s+(\d+:\d+:\d+)/i)))
            indexes.push('idx ' + m[1] + ' @ ' + m[2]);
        else if ((m = line.match(/^(TITLE|PERFORMER|SONGWRITER)\s+"?(.+?)"?\s*$/i)))
            cdtext.push(m[1] + ': ' + m[2]);
    }
    if (!files.length && !tracks)
        return null;
    const out = {
    ...
    const sections = [];
    if (cdtext.length)
        sections.push({ title: 'CD-TEXT', node: preBlock(cdtext.join('\n')) });
    if (indexes.length)
        sections.push({ title: 'Index points (' + indexes.length + ')', node: preBlock(indexes.join('\n')) });
    if (sections.length)
        out._sections = sections;
    return out;
}
...
async function parseCcd(file) {
    const text = await file.slice(0, Math.min(file.size, 512 * 1024)).text();
    if (!/\[CloneCD\]/i.test(text) && !/\[Disc\]/i.test(text))
        return null;
    ...
    if (tocEntries)
        out['TOC entries'] = tocEntries;
    const dataTracks = get('DataTracksScrambled');
    if (dataTracks != null)
        out['Data tracks scrambled'] = dataTracks;
    const entries = (text.match(/\[Entry \d+\]/gi) || []).length;
    if (entries)
        out['Track/TOC entries'] = entries;
    return out;
}
...
async function parseMds(file, ext) {
    ...
    if (ascii(b, 0, 16) !== 'MEDIA DESCRIPTOR')
        return null;
...
async function parseIntelHex(file) {
    ...
    if (!/^:[0-9A-Fa-f]{8}/.test(lines.find((l) => l.trim()) || ''))
        return null;
    const types = {};
    let dataBytes = 0, records = 0, badCksum = 0;
    let base = 0, minAddr = Infinity, maxAddr = -Infinity;
    for (const line of lines) {
        if (line[0] !== ':')
            continue;
        const bytes = line.slice(1).trim();
        if (bytes.length < 10 || bytes.length % 2)
            continue;
        ...
        for (let i = 0; i < bytes.length; i += 2)
            sum += parseInt(bytes.slice(i, i + 2), 16);
        if ((sum & 0xff) !== 0)
            badCksum++;
        if (type === 0x00) {
            dataBytes += len;
    ...
    if (isFinite(minAddr))
        out['Address range'] = hex(minAddr) + ' – ' + hex(maxAddr);
    return out;
}
...
async function parseSrec(file) {
    ...
    if (!first || first[0] !== 'S' || !/^S[0-9]/.test(first))
        return null;
    const counts = {};
    let dataBytes = 0, records = 0, badCksum = 0;
    ...
    for (const line of lines) {
        ...
        for (let i = 2; i < l.length; i += 2) {
            const v = parseInt(l.slice(i, i + 2), 16);
            if (!isNaN(v))
                sum += v;
        }
        if ((sum & 0xff) !== 0xff)
        ...
        if (type === '0') {
            ...
            if (addr < minAddr)
                minAddr = addr;
            if (addr + dataLen > maxAddr)
                maxAddr = addr + dataLen;
        }
        if (records > 2000000)
            break;
    }
    ...
    if (isFinite(minAddr))
        out['Address span'] = hex(minAddr) + ' – ' + hex(maxAddr);
    return out;
}
...
async function parseUf2(file) {
    ...
    if (!(head[0] === 0x55 && head[1] === 0x46 && head[2] === 0x32 && head[3] === 0x0a))
        return null;
    if (!(head[4] === 0x57 && head[5] === 0x51 && head[6] === 0x5d && head[7] === 0x9e))
        return null;
    ...
    if (flags & 0x00002000)
        out['Family ID'] = hex(familyId);
    if (flags & 0x00000001)
        out['Note'] = 'block(s) not for flashing (NOFLASH)';
...
async function parseDtb(file) {
    ...
    if (!(head[0] === 0xd0 && head[1] === 0x0d && head[2] === 0xfe && head[3] === 0xed))
        return null;
    ...
    try {
        ...
        if (printable.length > 4)
            out['Root strings'] = printable.slice(0, 120);
    }
    catch (_) { }
    return out;
}
...
async function parseUImage(file) {
    ...
    if (!(head[0] === 0x27 && head[1] === 0x05 && head[2] === 0x19 && head[3] === 0x56))
        return null;
...
function parseMbrEntries(b, out) {
    ...
    if (parts.length) {
        const lines = parts.map((p, i) => '#' + (i + 1) + '  ' + (p.boot ? '* ' : '  ') + p.label.padEnd(16) +
...
async function parseMbr(file) {
    ...
    if (sig)
        out['Disk signature'] = hex(sig);
    const parts = parseMbrEntries(b, out);
    if (parts.some((p) => p.type === 0xee))
        out['Note'] = 'Protective MBR - disk is GPT-partitioned (see .gpt).';
    return out;
}
async function parseGpt(file) {
    ...
    let hdrOff = -1;
    if (ascii(b, 512, 8) === 'EFI PART')
        hdrOff = 512;
    else if (ascii(b, 0, 8) === 'EFI PART')
        hdrOff = 0;
    if (hdrOff < 0) {
        // Maybe a plain MBR dump without GPT.
        if (b.length >= 512 && b[510] === 0x55 && b[511] === 0xaa)
            return parseMbr(file);
        return null;
    }
    const r = new Reader(b, true);
    r.seek(hdrOff + 8);
    ...
    if (parts.length) {
        const lines = parts.map((p, i) => '#' + (i + 1) + '  ' + (p.name || '(unnamed)').padEnd(20) +
...
async function parseExt(file) {
    ...
    let type = 'ext2';
    if (featIncompat & 0x40 || featRoCompat & 0x8)
        type = 'ext4';
    else if (featCompat & 0x4)
        type = 'ext3';
    const out = {
    ...
    if (mountPath)
        out['Last mount path'] = mountPath;
    if (wtime)
        out['Last written'] = fmtDate(new Date(wtime * 1000));
    if (mtime)
        out['Last mounted'] = fmtDate(new Date(mtime * 1000));
    return out;
}
// ---------- SquashFS ----------
...
async function parseCramfs(file) {
    ...
    let little;
    if (b[0] === 0x45 && b[1] === 0x3d && b[2] === 0xcd && b[3] === 0x28)
        little = true;
    else if (b[0] === 0x28 && b[1] === 0xcd && b[2] === 0x3d && b[3] === 0x45)
        little = false;
    else
        return null;
    const r = new Reader(b, little);
    ...
    if (!/Compressed ROMFS/i.test(sig))
        return null;
    return {
...
async function parseWim(file, ext) {
    ...
    if (totalParts > 1)
        out['Part'] = partNumber + ' of ' + totalParts + ' (split set)';
    if (flags & 0x00000001)
        out['Reserved flag'] = 'set';
    if (ext === 'esd' || (flags & 0x00080000))
        out['Note'] = 'LZMS-compressed (ESD); image XML not decoded (decoder not bundled).';
    return out;
}
// ===================================================================
...
async function parseTrx(file) {
    ...
    if (!(b[0] === 0x48 && b[1] === 0x44 && b[2] === 0x52 && b[3] === 0x30))
        return null;
    const r = new Reader(b, true);
    r.seek(4);
    const length = r.u32();
    ...
    for (let i = 0; i < nOff; i++) {
        const o = r.u32();
        if (o)
            offs.push('partition ' + i + ' @ ' + hex(o));
    }
    if (flags)
        out['Flags'] = hex(flags, 4);
    if (offs.length)
        out._sections = [{ title: 'Partition offsets (' + offs.length + ')', node: preBlock(offs.join('\n')) }];
    return out;
}
...
async function parseDfu(file) {
    ...
    try {
        ...
        if (tail.length === 16 && tail[10] === 0x55 && tail[9] === 0x46 && tail[8] === 0x44) {
            ...
            if (bcdDevice !== 0xffff)
                out['Device release'] = hex(bcdDevice, 4);
...
async function parseUefiFv(file) {
    ...
    if (ascii(b, 40, 4) !== '_FVH') {
        ...
        if (!(fd.length === 4 && fd[0] === 0x5a && fd[1] === 0xa5 && fd[2] === 0xf0 && fd[3] === 0x0f))
            return null;
        return {
...
async function parseAndroidSparse(file) {
    ...
    if (!(b[0] === 0x3a && b[1] === 0xff && b[2] === 0x26 && b[3] === 0xed))
        return null;
    const r = new Reader(b, true);
    r.seek(4);
    const major = r.u16();
...
async function parseFloppy(file, ext) {
    ...
    if (FLOPPY_GEOMETRY[file.size])
        out['Geometry (by size)'] = FLOPPY_GEOMETRY[file.size];
    out['Image size'] = fmtBytes(file.size);
    if (b.length < 512)
        return out;
    ...
    if (jump === 0xeb || jump === 0xe9) {
        ...
        if (bytesPerSector === 512 || bytesPerSector === 1024 || bytesPerSector === 2048 || bytesPerSector === 4096) {
            ...
            if (FAT_MEDIA[mediaType])
                out['Media descriptor'] = FAT_MEDIA[mediaType] + ' (' + hex(mediaType, 2) + ')';
            const total = totalSectors16 || totalSectors32;
            if (total)
                out['Total sectors'] = total.toLocaleString();
...
async function parseVmsd(file) {
    ...
    if (lines < 1 || !('snapshot.numsnapshots' in kv) && !('snapshot.current' in kv) && !Object.keys(kv).some((k) => /^snapshot\d+\./.test(k)))
        return null;
    const out = { 'Format': 'VMware snapshot metadata (.vmsd)' };
    if (kv['snapshot.numsnapshots'] != null)
        out['Snapshots'] = kv['snapshot.numsnapshots'];
    if (kv['snapshot.current'] != null)
        out['Current snapshot UID'] = kv['snapshot.current'];
    ...
    if (names.length)
        out._sections = [{ title: 'Snapshot names (' + names.length + ')', node: preBlock(names.join('\n')) }];
    return out;
}
...
async function parseParallels(file, ext) {
    ...
    if (/<Parallels_disk_image/i.test(text) || /<ParallelsSavedStates/i.test(text) || /<ParallelsVirtualMachine/i.test(text)) {
        ...
        if (cyl && heads && sectors) {
            ...
            if (size > 0)
                out['Virtual size'] = fmtBytes(size);
        }
        if (blockSize)
            out['Block size'] = blockSize + ' sectors';
        return out;
    }
...
async function parseDjiFirmware(file) {
    ...
    let model = '';
    for (let i = 0x1c; i < 0x2c && b[i]; i++)
        model += String.fromCharCode(b[i]);
...
//# sourceMappingURL=parsers-disk.js.map