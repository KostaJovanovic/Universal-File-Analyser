...
async function parseCbz(file) {
    ...
    if (!imgEntries.length && !zip.has('ComicInfo.xml'))
        return null;
    ...
    for (const e of imgEntries) {
        const ex = (e.name.match(/\.([^.]+)$/) || [])[1].toLowerCase();
        byFmt[ex] = (byFmt[ex] || 0) + 1;
    }
    if (Object.keys(byFmt).length)
    ...
    if (ciName) {
        ...
        if (ci) {
            for (const [k, v] of Object.entries(ci.fields))
                out[k] = v;
            if (ci.summary)
                out._sections = [{ title: 'Summary', node: preBlock(ci.summary) }];
        }
    }
    ...
    if (first && NATIVE_IMG.test(first.name)) {
        try {
            ...
            if (bytes) {
                const node = imgPreview(bytes, imgMime(first.name));
                if (node)
...
function tarMembers(b) {
    ...
    while (pos + 512 <= b.length) {
        ...
        if (typeChar === 'L') {
            ...
            if (name)
                items.push({ name, size, dataStart });
        }
...
async function parseCbt(file) {
    ...
    if (ascii(b, 257, 5) !== 'ustar' && tarOctal(b, 124, 12) <= 0)
        return null;
    let members;
    try {
        members = tarMembers(b);
    }
    catch (_) {
        return null;
    }
    ...
    const byFmt = {};
    for (const m of imgs) {
        const ex = (m.name.match(/\.([^.]+)$/) || [])[1].toLowerCase();
        byFmt[ex] = (byFmt[ex] || 0) + 1;
    }
    out['Page formats'] = Object.entries(byFmt).map(([k, v]) => k + ' (' + v + ')').join(', ');
    ...
    if (ci) {
        ...
        if (parsed) {
            for (const [k, v] of Object.entries(parsed.fields))
                out[k] = v;
            if (parsed.summary)
                out._sections = [{ title: 'Summary', node: preBlock(parsed.summary) }];
        }
    }
...
async function parseXps(file, ext) {
    ...
    if (coreName)
        opcCore(out, await zip.text(coreName));
    if (names.some((n) => /\.wdp$|\.jxr$/i.test(n)))
        out['Image codec'] = 'JPEG XR (HD Photo) present';
    return out;
}
// ---------- HWPX (Hangul OWPML) ----------
...
async function parseIbooks(file) {
    ...
    if (opfName) {
        ...
        if (creators.length)
            out['Author'] = creators.join('; ');
        const pub = tag(opf, 'publisher');
        if (pub)
            out['Publisher'] = pub;
        const lang = tag(opf, 'language');
        if (lang)
            out['Language'] = lang;
        out['Manifest items'] = countRe(opf, /<item\b/gi);
        out['Spine items'] = countRe(opf, /<itemref\b/gi);
    }
    if (zip.names().some((n) => /com\.apple\.ibooks/i.test(n)))
        out['Apple extensions'] = 'present';
...
async function parseScriv(file, ext) {
    ...
    if (!/<ScrivenerProject/i.test(text) && !/<Binder\b/i.test(text))
        return null;
    ...
    const byType = {};
    for (const m of text.matchAll(/<BinderItem[^>]*\bType="([^"]+)"/gi))
        byType[m[1]] = (byType[m[1]] || 0) + 1;
    if (byType['Text'])
        out['Documents'] = byType['Text'];
    if (Object.keys(byType).length)
        out['Item types'] = Object.entries(byType).map(([k, v]) => k + ' (' + v + ')').join(', ');
    // Title outline (first 60 titles).
    const titles = Array.from(text.matchAll(/<Title>([^<]+)<\/Title>/gi)).map((m) => xmlText(m[1])).slice(0, 60);
    if (titles.length)
        out._sections = [{ title: 'Binder titles (first ' + titles.length + ')', node: preBlock(titles.join('\n')) }];
    return out;
}
// ---------- AbiWord (.abw / .zabw) ----------
...
function odfMeta(out, meta) {
    const t = tag(meta, 'title');
    if (t)
        out['Title'] = t;
    const creator = tag(meta, 'creator') || tag(meta, 'initial-creator');
    if (creator)
        out['Author'] = creator;
    const gen = tag(meta, 'generator');
    if (gen)
        out['Generator'] = gen;
    const subj = tag(meta, 'subject');
    if (subj)
        out['Subject'] = subj;
    const kw = tag(meta, 'keyword');
    if (kw)
        out['Keywords'] = kw;
    const date = tag(meta, 'date') || tag(meta, 'creation-date');
    if (date)
        out['Modified'] = date;
...
async function parseFodt(file, ext) {
    const text = await fileText(file, 8 * 1024 * 1024);
    if (!/office:document\b/i.test(text))
        return null;
...
async function parseDotx(file, ext) {
    ...
    if (app) {
        const pages = tag(app, 'Pages');
        if (pages)
            out['Pages'] = pages;
        const words = tag(app, 'Words');
        if (words)
            out['Words'] = words;
...
async function parseTex(file, ext) {
    ...
    if (docclass)
        out['Document class'] = docclass;
    if (ext === 'cls') {
        const p = pick(text, /\\ProvidesClass\{([^}]+)\}(?:\[([^\]]*)\])?/);
        if (p)
            out['Provides class'] = p;
    }
    if (ext === 'sty') {
        const p = pick(text, /\\ProvidesPackage\{([^}]+)\}(?:\[([^\]]*)\])?/);
        if (p)
            out['Provides package'] = p;
    }
    const title = pick(text, /\\title\{([^}]*)\}/);
    if (title)
        out['Title'] = title.replace(/\\\w+\{?|\}/g, '').trim();
    const author = pick(text, /\\author\{([^}]*)\}/);
    if (author)
        out['Author'] = author.replace(/\\\w+\{?|\}|\\and/g, ' ').replace(/\s+/g, ' ').trim();
    const pkgs = Array.from(text.matchAll(/\\usepackage(?:\[[^\]]*\])?\{([^}]+)\}/g)).flatMap((m) => m[1].split(',').map((s) => s.trim())).filter(Boolean);
    if (pkgs.length)
        out['Packages'] = pkgs.length + ': ' + Array.from(new Set(pkgs)).slice(0, 20).join(', ');
    ...
    if (includes.length)
        out['Includes'] = includes.length;
    const bib = pick(text, /\\bibliography\{([^}]+)\}/) || (text.match(/\\addbibresource\{([^}]+)\}/) || [])[1];
    if (bib)
        out['Bibliography'] = bib;
    if (includes.length)
        out._sections = [{ title: 'Includes (' + includes.length + ')', node: preBlock(includes.join('\n')) }];
    return out;
}
// ---------- BibTeX (.bib) ----------
async function parseBib(file) {
    ...
    for (const m of entries) {
        ...
        if (/^(string|preamble|comment)$/.test(type))
            continue;
    ...
    if (years.length)
        out['Year range'] = Math.min(...years) + '–' + Math.max(...years);
    if (dupKeys.size)
        out['⚠ Duplicate keys'] = dupKeys.size + ': ' + Array.from(dupKeys).slice(0, 10).join(', ');
...
async function parseRst(file) {
    ...
    for (let i = 1; i < lines.length; i++) {
        if (RST_UNDERLINE.test(lines[i]) && lines[i - 1].trim() && lines[i].trim()[0] && lines[i].length >= lines[i - 1].trim().length - 2) {
            ...
            if (txt && !/^[=\-`:.'"~^_*+#]+$/.test(txt))
                headings.push(lines[i].trim()[0] + ' ' + txt);
        }
    }
    ...
    if (headings.length)
        out._sections = [{ title: 'Outline (' + headings.length + ')', node: preBlock(headings.slice(0, 200).join('\n')) }];
    return out;
}
...
async function parseAdoc(file) {
    ...
    if (titleLine) {
        ...
        if (a && !/^[:=]/.test(a) && !RST_UNDERLINE.test(a))
            out['Author'] = a.split('<')[0].trim();
...
async function parseOrg(file) {
    ...
    const title = pick(text, /^#\+TITLE:\s*(.+)$/im);
    if (title)
        out['Title'] = title;
    const author = pick(text, /^#\+AUTHOR:\s*(.+)$/im);
    if (author)
        out['Author'] = author;
    const date = pick(text, /^#\+DATE:\s*(.+)$/im);
    if (date)
        out['Date'] = date;
    ...
    if (outline.length)
        out._sections = [{ title: 'Outline (' + headings.length + ')', node: preBlock(outline.join('\n')) }];
    return out;
}
// ---------- Textile (.textile) ----------
async function parseTextile(file) {
    ...
    out['Words'] = (text.match(/\S+/g) || []).length;
    if (headings.length)
        out._sections = [{ title: 'Outline', node: preBlock(headings.slice(0, 200).map((h) => h.replace(/^h(\d)\.\s+/, (_, n) => '  '.repeat(n - 1) + '• ')).join('\n')) }];
    return out;
}
// ---------- TEI XML (.tei) ----------
async function parseTei(file) {
    const text = await fileText(file, 6 * 1024 * 1024);
    if (!/<TEI[\s>]/i.test(text) && !/<teiHeader/i.test(text))
        return null;
    const out = { 'Format': 'TEI XML (Text Encoding Initiative)' };
    const t = tag(text, 'title');
    if (t)
        out['Title'] = t;
    const author = tag(text, 'author');
    if (author)
        out['Author'] = author;
    const pub = tag(text, 'publisher');
    if (pub)
        out['Publisher'] = pub;
...
async function parseRmd(file, ext) {
    ...
    if (ym) {
        const y = ym[1];
        const title = pick(y, /^title:\s*["']?(.+?)["']?\s*$/im);
        if (title)
            out['Title'] = title;
        const author = pick(y, /^author:\s*["']?(.+?)["']?\s*$/im);
        if (author)
            out['Author'] = author;
        const fmt = pick(y, /^format:\s*(.+)$/im) || pick(y, /^output:\s*(.+)$/im);
        if (fmt)
            out['Output / format'] = fmt;
        const engine = pick(y, /^engine:\s*(.+)$/im);
        if (engine)
            out['Engine'] = engine;
    }
...
async function parseRtfd(file) {
    ...
    if (/\{\\rtf/i.test(text)) {
        ...
        const ansicpg = pick(text, /\\ansicpg(\d+)/i);
        if (ansicpg)
            out['Code page'] = 'cp' + ansicpg;
        const cocoa = pick(text, /\\cocoartf(\d+)/i);
        if (cocoa)
            out['Cocoa RTF version'] = cocoa;
    }
    else {
...
async function parseMht(file) {
    ...
    if (!/Content-Type:\s*multipart\/related/i.test(text) && !/^From:\s*<Saved by/im.test(text)) {
        if (!/MIME-Version:/i.test(text))
            return null;
    }
    const out = { 'Format': 'MHTML web archive (.mht / .mhtml)' };
    const subject = pick(text, /^Subject:\s*(.+)$/im);
    if (subject)
        out['Title'] = subject.trim();
    const date = pick(text, /^Date:\s*(.+)$/im);
    if (date)
        out['Saved'] = date.trim();
    const snapshot = pick(text, /^Snapshot-Content-Location:\s*(.+)$/im);
    if (snapshot)
        out['Original URL'] = snapshot.trim();
...
async function parseWarc(file, ext) {
    ...
    const hosts = new Set();
    const dates = [];
    for (const m of text.matchAll(/^WARC-Type:\s*(\S+)/gim))
        types[m[1].toLowerCase()] = (types[m[1].toLowerCase()] || 0) + 1;
    ...
    for (const m of text.matchAll(/^WARC-Date:\s*(\S+)/gim))
        dates.push(m[1]);
    ...
    if (dates.length) {
        dates.sort();
        out['Date range'] = dates[0] + ' → ' + dates[dates.length - 1];
    }
    if (hosts.size)
        out._sections = [{ title: 'Hosts (' + hosts.size + ')', node: preBlock(Array.from(hosts).slice(0, 200).join('\n')) }];
    return out;
}
// ---------- MAFF (.maff) ----------
...
async function parseJats(file) {
    const text = await fileText(file, 6 * 1024 * 1024);
    if (!/<article\b/i.test(text) && !/JATS|journalpublishing/i.test(text))
        return null;
    const out = { 'Format': 'JATS journal article XML' };
    const meta = (text.match(/<article-meta>([\s\S]*?)<\/article-meta>/i) || [])[1] || text;
    const title = tag(meta, 'article-title');
    if (title)
        out['Title'] = title;
    const journal = tag(text, 'journal-title');
    if (journal)
        out['Journal'] = journal;
    const doi = (meta.match(/<article-id[^>]*pub-id-type="doi"[^>]*>([^<]+)</i) || [])[1];
    if (doi)
        out['DOI'] = doi.trim();
    ...
    const authors = Array.from(meta.matchAll(/<contrib\b[^>]*>([\s\S]*?)<\/contrib>/gi))
        ...
        .filter(Boolean);
    if (authors.length)
        out['Authors'] = authors.slice(0, 12).join('; ') + (authors.length > 12 ? '; …' : '');
    ...
    if (abstract)
        out._sections = [{ title: 'Abstract', node: preBlock(abstract.replace(/\s+/g, ' ').trim()) }];
    return out;
}
// ---------- DVI (.dvi) ----------
...
async function parseDita(file, ext) {
    ...
    if (rootM)
        out['Root type'] = rootM[1];
    const dt = text.match(/<!DOCTYPE\s+\S+\s+PUBLIC\s+"([^"]+)"/i);
    if (dt)
        out['DOCTYPE'] = dt[1];
    const t = tag(text, 'title') || tag(text, 'navtitle');
    if (t)
        out['Title'] = t;
    ...
    if (isMap) {
        ...
        if (hrefs.length)
            out._sections = [{ title: 'References (' + hrefs.length + ')', node: preBlock(hrefs.slice(0, 60).join('\n')) }];
    }
    else {
...
async function parseScribus(file, ext) {
    ...
    if (!text)
        text = latin1(bytes.subarray(0, 4 * 1024 * 1024));
    if (!/<SCRIBUSUTF8NEW\b/i.test(text) && !/<SCRIBUS\b/i.test(text))
        return null;
    const out = { 'Format': 'Scribus document (.' + ext + ')' };
    const ver = pick(text, /<SCRIBUSUTF8NEW[^>]*\bVersion="([^"]+)"/i) || pick(text, /<SCRIBUS[^>]*\bVersion="([^"]+)"/i);
    if (ver)
        out['Scribus version'] = ver;
    // DOCUMENT element carries author/title/page geometry.
    const doc = (text.match(/<DOCUMENT\b([^>]*)>/i) || [])[1] || '';
    const title = pick(doc, /\bTITLE="([^"]*)"/i);
    if (title)
        out['Title'] = title;
    const author = pick(doc, /\bAUTHOR="([^"]*)"/i);
    if (author)
        out['Author'] = author;
    const kw = pick(doc, /\bKEYWORDS="([^"]*)"/i);
    if (kw)
        out['Keywords'] = kw;
    const w = pick(doc, /\bPAGEWIDTH="([^"]+)"/i);
    const h = pick(doc, /\bPAGEHEIGHT="([^"]+)"/i);
    if (w && h)
        out['Page size'] = Math.round(parseFloat(w)) + ' × ' + Math.round(parseFloat(h)) + ' pt';
...
function oleSummary(out, bytes) {
    ...
    try {
        ...
        if (dv.getUint16(0, true) !== 0xfffe)
            return;
        const secOff = dv.getUint32(0x2c, true);
        if (secOff + 8 > bytes.length)
            return;
        ...
        for (let i = 0; i < numProps; i++) {
            ...
            if (!PID[pid] || off + 8 > bytes.length)
                continue;
            const type = dv.getUint32(off, true);
            if (type !== 30 && type !== 31)
                continue; // VT_LPSTR / VT_LPWSTR
            const len = dv.getUint32(off + 4, true);
            if (len <= 0 || len > 2048 || off + 8 + len > bytes.length)
                continue;
            let val;
            if (type === 31)
                val = utf16(bytes.subarray(off + 8, off + 8 + len * 2), true).replace(/\0+$/, '');
            else
                val = latin1(bytes.subarray(off + 8, off + 8 + len)).replace(/\0+$/, '');
            val = val.trim();
            if (val && !out[PID[pid]])
                out[PID[pid]] = val;
        }
...
async function parseOleDoc(file, ext) {
    ...
    let cf;
    try {
        ({ openCfbf } = await import('../lib/cfbf.js'));
        cf = await openCfbf(file);
    }
    catch (_) {
        return null;
    }
    ...
    if (has(/^WksSSWorkBook$/i))
        out['Works module'] = 'Spreadsheet';
    else if (has(/^CONTENTS$/i) && ext.startsWith('wp'))
        out['Works module'] = 'Word processor';
    if (has(/^StarWriterDocument$|StarWriter/i))
        out['StarOffice module'] = 'Writer';
    else if (has(/^StarCalcDocument$|StarCalc/i))
        out['StarOffice module'] = 'Calc';
    else if (has(/^StarDrawDocument|StarImpress/i))
        out['StarOffice module'] = 'Draw / Impress';
    if (has(/^WordDocument$/i))
        out['Word stream'] = 'WordDocument present';
    if (has(/^Workbook$|^Book$/i))
        out['Excel stream'] = 'Workbook present';
...
async function parseSnb(file) {
    ...
    if (bookName) {
        ...
        const t = tag(xml, 'book-name') || tag(xml, 'title');
        if (t)
            out['Title'] = t;
        const a = tag(xml, 'author');
        if (a)
            out['Author'] = a;
        const pub = tag(xml, 'publisher');
        if (pub)
            out['Publisher'] = pub;
        const lang = tag(xml, 'language');
        if (lang)
            out['Language'] = lang;
    }
...
async function parseLrf(file, ext) {
    ...
    if (!sigOk && !(b[0] === 0x4c && b[1] === 0x52 && b[2] === 0x46))
        return null;
    ...
    if (version)
        out['BBeB version'] = version;
    out['Object count'] = dv.getUint32(0x10, true) >>> 0 || undefined;
    if (ext === 'lrx')
        out['DRM'] = 'encrypted (Sony Marlin DRM)';
...
async function parseFrame(file, ext) {
    ...
    if (/^<MIFFile/i.test(headStr) || ext === 'mif') {
        const text = await fileText(file, 4 * 1024 * 1024);
        if (!/<MIFFile/i.test(text))
            return null;
        const out = { 'Format': 'FrameMaker Interchange Format (.mif)' };
        const ver = pick(text, /<MIFFile\s+([\d.]+)/i);
        if (ver)
            out['MIF version'] = ver;
    ...
    if (/^<MakerFile/i.test(headStr) || /^<MakerDictionary/i.test(headStr) || /^<MakerScreenFont/i.test(headStr)) {
        ...
        const v = verStr.match(/<Maker\w+\s+([\d.]+)/i);
        if (v)
            out['Version'] = v[1];
...
//# sourceMappingURL=parsers-docs.js.map