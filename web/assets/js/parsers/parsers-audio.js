...
async function readApev2(file) {
    try {
        ...
        for (let i = 0; i < count && i < 200; i++) {
            if (p + 8 > tail.length)
                break;
            const r2 = new Reader(tail, true);
            r2.seek(p);
            const vlen = r2.u32();
            const iflags = r2.u32();
            p += 8;
            let key = '';
            while (p < tail.length && tail[p] !== 0) {
                key += String.fromCharCode(tail[p]);
                p++;
            }
            p++; // skip NUL
            if (p + vlen > tail.length)
                break;
            const isText = ((iflags >> 1) & 0x03) === 0;
            if (isText && vlen < 4096)
                out[key] = utf8(tail.subarray(p, p + vlen)).replace(/\0/g, '');
            p += vlen;
        }
...
function id3v2Size(head, off) {
    ...
    if (!(head[off] === 0x49 && head[off + 1] === 0x44 && head[off + 2] === 0x33))
        return 0;
...
async function parseApe(file) {
    ...
    if (version >= 3980) {
        ...
        rate = r.u32();
        r.u32();
        r.u32(); // header/terminating bytes
        totalFrames = r.u32();
        finalBlocks = r.u32();
        bits = 16;
...
async function parseWavpack(file) {
    ...
    if (rate) {
        out['Sample rate'] = fmtRate(rate);
        if (totalSamples && totalSamples !== 0xFFFFFFFF)
            out['Duration'] = fmtDuration(totalSamples / rate);
    }
    ...
    try {
        ...
        for (let guard = 0; guard < 64 && p + 4 < buf.length; guard++) {
            ...
            if (type === 1 && p + 10 <= buf.length) {
                ...
                if (sampleRate >= 6000 && sampleRate <= 384000)
                    out['Sample rate'] = fmtRate(sampleRate);
                break;
            }
...
function parseOptimfrog(head, ext) {
    if (ascii(head, 0, 4) !== 'OFR ' && ascii(head, 0, 3) !== 'OFR')
        return null;
    return {
...
async function parseDff(file) {
    ...
    if (ascii(head, 0, 4) !== 'FRM8')
        return null;
    if (ascii(head, 12, 4) !== 'DSD ')
        return null;
...
function walkRiff(buf, start, cb) {
    ...
    for (let g = 0; g < 256 && p + 8 <= buf.length; g++) {
        ...
        cb(id, body, sz);
        if (sz === 0xFFFFFFFF)
            break; // RF64 placeholder handled by caller
        p = body + sz + (sz & 1);
    }
}
...
async function parseCaf(file) {
    ...
    for (let g = 0; g < 32 && p + 12 <= head.length; g++) {
        ...
        if (type === 'desc' && body + 32 <= head.length) {
            const dr = new Reader(head, false);
            dr.seek(body);
            rate = dr.f64();
            ...
            if (rate)
                out['Sample rate'] = fmtRate(Math.round(rate));
            out['Channels'] = channels;
            if (bitsPerChannel)
                out['Bit depth'] = bitsPerChannel + '-bit';
        }
...
async function parseRf64(file) {
    ...
    if (codec)
        out['Codec'] = WAVE_CODECS[codec] || ('0x' + codec.toString(16));
    if (rate)
        out['Sample rate'] = fmtRate(rate);
    if (bits)
        out['Bit depth'] = bits + '-bit';
    if (channels)
        out['Channels'] = channels;
    if (rate && sampleCount64)
        out['Duration'] = fmtDuration(Number(sampleCount64) / rate);
    return out;
}
// ---------- Wave64 (Sony) ----------
...
async function parseWave64(file) {
    ...
    if (!(head[0] === 0x72 && head[1] === 0x69 && head[2] === 0x66 && head[3] === 0x66))
        return null;
...
async function parseAu(file) {
    ...
    if (dataOffset > 24)
        out['Annotation'] = cleanAscii(head, 24, Math.min(dataOffset - 24, 40)) || '-';
    return out;
}
// ---------- VOC (Creative Voice) ----------
async function parseVoc(file) {
    const head = await readSlice(file, 0, 32);
    if (!startsWithAscii(head, 'Creative Voice File'))
        return null;
    const r = new Reader(head, true);
    r.seek(20);
...
async function parseBwf(file) {
    ...
    walkRiff(head, 12, (cid, off, sz) => {
        if (cid === 'fmt ' && off + 16 <= head.length) {
            ...
            if (rate && samples)
                out['SMPTE timecode start'] = fmtDuration(samples / rate) + ' (' + samples + ' samples)';
            const ver = r.u16();
            out['BWF version'] = ver;
        }
    });
    if (!hasBext)
        return null; // plain WAV, not BWF
    if (codec)
        out['Codec'] = WAVE_CODECS[codec] || ('0x' + codec.toString(16));
    if (rate)
        out['Sample rate'] = fmtRate(rate);
...
async function parseSpeex(file) {
    ...
    out['Channels'] = channels;
    if (bitrate > 0)
        out['Bitrate'] = (bitrate / 1000).toFixed(1) + ' kbps';
    return out;
}
// ---------- AMR-WB ----------
...
async function parseIsoAudio(file, ext, label) {
    ...
    try {
        ...
        for (const cc of ['mp4a', 'alac', 'Opus', 'samr', 'sawb', 'ac-3']) {
            if (findBytes(head, Array.from(cc).map((ch) => ch.charCodeAt(0)), 0) >= 0) {
                out['Codec'] = cc;
                break;
            }
        }
...
async function parseMpeg12(file, ext) {
    ...
    if (bitrate)
        out['Bitrate'] = bitrate + ' kbps';
    const rate = (MP_RATES[verBits] || [])[srField];
    if (rate)
        out['Sample rate'] = fmtRate(rate);
    out['Channel mode'] = MP_CHANMODE[channelMode];
    // Estimate duration from CBR.
    if (bitrate && file.size)
        out['Duration (est, CBR)'] = fmtDuration((file.size - off) * 8 / (bitrate * 1000));
    if (id3)
        out['ID3v2 tag'] = 'present (' + fmtBytes(id3) + ')';
    return out;
}
// =====================================================================
...
async function parseSf2(file, ext) {
    ...
    if (ascii(head, 0, 4) !== 'RIFF' || ascii(head, 8, 4) !== 'sfbk')
        return null;
            ...
            if (listType === 'INFO') {
                walkRiff(head, off + 4, (sid, soff, ssz) => {
                    if (ssz > 0 && ssz < 512) {
                        ...
                        if (map[sid])
                            info[map[sid]] = cleanAscii(head, soff, ssz);
                        if (sid === 'ifil' && soff + 4 <= head.length) {
                            const r = new Reader(head, true);
                            r.seek(soff);
                            info['SoundFont version'] = r.u16() + '.' + r.u16();
                        }
                    }
                });
            }
...
async function parseSfz(file) {
    ...
    if (!/<(region|group|global|control|master)>/i.test(text))
        return null;
    ...
    if (keys.length)
        out['Key references'] = keys.length;
    const uniq = Array.from(new Set(samples)).slice(0, 200);
    if (uniq.length)
        out._sections = [{ title: 'Samples (' + uniq.length + ')', node: preBlock(uniq.join('\n')) }];
    return out;
}
// ---------- DLS ----------
async function parseDls(file) {
    const head = await readSlice(file, 0, 65536);
    if (ascii(head, 0, 4) !== 'RIFF' || ascii(head, 8, 4) !== 'DLS ')
        return null;
    const out = { 'Format': 'Downloadable Sounds (.dls)' };
    let instruments = 0;
    walkRiff(head, 12, (cid, off, sz) => {
        if (cid === 'colh' && off + 4 <= head.length) {
            const r = new Reader(head, true);
            r.seek(off);
            instruments = r.u32();
        }
        if (cid === 'LIST' && ascii(head, off, 4) === 'INFO') {
            walkRiff(head, off + 4, (sid, soff, ssz) => {
                if (ssz > 0 && ssz < 512) {
                    ...
                    if (map[sid])
                        out[map[sid]] = cleanAscii(head, soff, ssz);
                }
            });
        }
    });
...
async function parseRmi(file) {
    const head = await readSlice(file, 0, 4096);
    if (ascii(head, 0, 4) !== 'RIFF' || ascii(head, 8, 4) !== 'RMID')
        return null;
    ...
    walkRiff(head, 12, (cid, off, sz) => {
        ...
        if (cid === 'DLS ' || (cid === 'LIST' && ascii(head, off, 4) === 'DLS '))
            out['Embedded DLS'] = 'yes';
    });
    if (findBytes(head, [0x73, 0x66, 0x62, 0x6B], 0) >= 0)
        out['Embedded SoundFont'] = 'yes (sfbk)';
    return out;
}
// ---------- SMAF (.mmf) ----------
async function parseSmaf(file) {
    const head = await readSlice(file, 0, 256);
    if (ascii(head, 0, 4) !== 'MMMD')
        return null;
    return {
...
async function parseSap(file) {
    ...
    if (ascii(head, 0, 4) !== 'SAP\r' && !startsWithAscii(head, 'SAP'))
        return null;
...
async function parseMod(file) {
    ...
    if (channels == null) {
        ...
        if (m)
            channels = parseInt(m[1], 10);
    }
    if (channels == null)
        return null; // not a 31-sample MOD
    ...
    if (names.length)
        out._sections = [{ title: 'Sample names (' + names.length + ' of 31)', node: preBlock(names.join('\n')) }];
    return out;
}
// ---------- XM (FastTracker 2) ----------
async function parseXm(file) {
    const head = await readSlice(file, 0, 80);
    if (!startsWithAscii(head, 'Extended Module: '))
        return null;
    ...
    r.u16();
    const channels = r.u16();
    const patterns = r.u16();
    const instruments = r.u16();
    out['Channels'] = channels;
...
async function parseIt(file) {
    ...
    try {
        ...
        if (msgLen > 0 && msgLen < 8192) {
            ...
            if (msg)
                out._sections = [{ title: 'Song message', node: preBlock(msg) }];
        }
    }
    catch (_) { }
    return out;
...
async function parseStm(file) {
    ...
    if (!/!Scream!|BMOD2STM|!SCREAM!/i.test(id) && head[28] !== 0x1A)
        return null;
...
async function parseFar(file) {
    ...
    if (!(ascii(head, 0, 3) === 'FAR' && head[3] === 0xFE))
        return null;
...
async function parseNsf(file, ext) {
    ...
    if (!(ascii(head, 0, 4) === 'NESM' && head[4] === 0x1A))
        return null;
    ...
    if (chip & 0x08)
        chips.push('MMC5');
    if (chip & 0x10)
        chips.push('Namco 163');
    if (chip & 0x20)
        chips.push('Sunsoft 5B');
...
async function parseSpc(file) {
    ...
    if (!startsWithAscii(head, 'SNES-SPC700 Sound File Data'))
        return null;
...
async function parseVgm(file, ext) {
    ...
    r.seek(4);
    const eofOffset = r.u32();
    r.seek(0x18);
    const totalSamples = r.u32();
    if (totalSamples)
        out['Duration'] = fmtDuration(totalSamples / 44100);
    r.seek(0x14);
    const gd3Rel = r.u32();
    ...
    for (const [off, name] of VGM_CHIPS) {
        if (off + 4 <= head.length) {
            ...
            if (clk)
                chips.push(name + ' @ ' + (clk / 1000000).toFixed(3).replace(/\.?0+$/, '') + ' MHz');
        }
    }
    if (chips.length)
        out['Sound chips'] = chips.join('; ');
    // GD3 tag.
...
async function parseAup(file) {
    ...
    if (labeltracks)
        out['Label tracks'] = labeltracks;
    const tags = Array.from(text.matchAll(/<tag\s+name="([^"]+)"\s+value="([^"]*)"/gi)).map((m) => m[1] + ': ' + m[2]);
    if (tags.length)
        out._sections = [{ title: 'Metadata tags', node: preBlock(tags.join('\n')) }];
    return out;
}
...
async function parseAup3(file) {
    const head = await readSlice(file, 0, 16);
    if (!startsWithAscii(head, 'SQLite format 3'))
        return null;
    let summary = null;
    try {
        ...
        try {
            ...
            if (counts.sampleblocks != null)
                out['Sample blocks'] = counts.sampleblocks.toLocaleString();
            if (counts.autosave != null)
                out['Autosave rows'] = counts.autosave;
            if (counts.tags != null && out['Tags'] == null)
                out['Tag rows'] = counts.tags;
            const sections = [];
            if (out._aup3Tags) {
                sections.push({ title: 'Metadata tags (' + out._aup3Tags.length + ')', node: preBlock(out._aup3Tags.join('\n')) });
...
//# sourceMappingURL=parsers-audio.js.map