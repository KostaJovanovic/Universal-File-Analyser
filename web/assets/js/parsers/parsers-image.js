...
import { SCAN_SMALL, PREVIEW_CARVE_MAX, PARSE_IMAGE_READ_MAX, PARSE_NETPBM_READ_MAX, PARSE_TEXTURE_READ_MAX, PARSE_TEXT_MAX, PARSE_JP2_READ_MAX } from '../core/limits.js';
import { decodeBcn } from '../lib/bcn.js';
import { canvasFromRGBA } from './parser-util.js';
// ---------- shared helpers ----------
async function readAll(file, cap = PARSE_IMAGE_READ_MAX) {
...
async function parseTga(file) {
    ...
    if (xOrigin || yOrigin)
        out['Origin offset'] = xOrigin + ', ' + yOrigin;
    if (colorMapType)
        out['Palette'] = cmapLen + ' entries, ' + cmapDepth + '-bit';
...
function decodeTga(b, h) {
    ...
    for (let i = 0; i < px; i++) {
        ...
        if (h.indexed) {
            ...
            g = ((v >> 5) & 31) * 8;
            bl = (v & 31) * 8;
            if (pixelDepth === 16 && (h.descriptor & 0x0f))
                a = (v & 0x8000) ? 255 : 0;
        }
        else { // BGR(A)
            bl = raw[s];
...
async function parseQoi(file) {
    const b = await readAll(file);
    if (b.length < 14 || ascii(b, 0, 4) !== 'qoif')
        return null;
    ...
    if (!width || !height || width > 60000 || height > 60000)
        return null;
    const out = {
...
async function parseNetpbm(file) {
    ...
    if (b.length < 3 || b[0] !== 0x50)
        return null; // 'P'
    const t = b[1];
    if (t === 0x37)
        return parsePam(b); // P7 = PAM
    if (t < 0x31 || t > 0x36)
        return null; // P1..P6
    const type = t - 0x30;
    const ascii_ = type <= 3;
...
function decodeNetpbm(b, pos, h) {
    ...
    if (ascii_) {
        ...
        let p = pos;
        const nextInt = () => {
            while (p < b.length && (b[p] === 0x20 || b[p] === 0x09 || b[p] === 0x0a || b[p] === 0x0d))
                p++;
            if (b[p] === 0x23) {
                while (p < b.length && b[p] !== 0x0a)
                    p++;
                return nextInt();
            }
            let v = 0, got = false;
            while (p < b.length && b[p] >= 0x30 && b[p] <= 0x39) {
                v = v * 10 + (b[p] - 0x30);
                p++;
                got = true;
            }
        ...
        const nextBit = () => {
            for (;;) {
                ...
                if (p < b.length && (b[p] === 0x30 || b[p] === 0x31))
                    return b[p++] - 0x30;
                return null;
            }
        };
    ...
    try {
        ...
        if (px > 0 && px <= 48_000_000 && maxval <= 255) {
            ...
            for (let i = 0; i < px; i++) {
                const d = i * 4;
                if (depth === 1) {
                    const g = Math.round((b[p++] || 0) * scale);
                    rgba[d] = rgba[d + 1] = rgba[d + 2] = g;
                    rgba[d + 3] = 255;
                }
                else if (depth === 2) {
                    const g = Math.round((b[p++] || 0) * scale);
                    rgba[d] = rgba[d + 1] = rgba[d + 2] = g;
                    rgba[d + 3] = Math.round((b[p++] || 0) * scale);
                }
                else if (depth === 3) {
                    rgba[d] = Math.round((b[p++] || 0) * scale);
                    rgba[d + 1] = Math.round((b[p++] || 0) * scale);
                    rgba[d + 2] = Math.round((b[p++] || 0) * scale);
                    rgba[d + 3] = 255;
                }
                else {
                    rgba[d] = Math.round((b[p++] || 0) * scale);
...
function decodePcx(b, h) {
    ...
    if (!(rowBytes > 0) || rowBytes > 256_000_000)
        return null;
    ...
    if (planes === 3 && bpp === 8) { // 24-bit RGB
        ...
        let pal = null;
        if (b.length >= 769 && b[b.length - 769] === 0x0c)
            pal = b.subarray(b.length - 768);
        for (let y = 0; y < height; y++) {
            const base = y * totalPerRow;
            for (let x = 0; x < width; x++) {
                ...
                if (pal) {
                    rgba[d] = pal[idx * 3];
                    rgba[d + 1] = pal[idx * 3 + 1];
                    rgba[d + 2] = pal[idx * 3 + 2];
                }
                else {
                    rgba[d] = rgba[d + 1] = rgba[d + 2] = idx;
                }
                rgba[d + 3] = 255;
            }
        }
        ...
        for (let y = 0; y < height; y++) {
            ...
            for (let x = 0; x < width; x++) {
                ...
                if (planes === 1) {
                    ...
                    rgba[d] = hdrPal[idx * 3];
                    rgba[d + 1] = hdrPal[idx * 3 + 1];
                    rgba[d + 2] = hdrPal[idx * 3 + 2];
                }
                rgba[d + 3] = 255;
            }
        }
...
async function parseFarbfeld(file) {
    const b = await readAll(file);
    if (b.length < 16 || ascii(b, 0, 8) !== 'farbfeld')
        return null;
    const r = new Reader(b); // big-endian
    r.seek(8);
    const width = r.u32();
    ...
    try {
        ...
        if (px <= 16_000_000 && 16 + px * 8 <= b.length + 8) {
            ...
            for (let i = 0; i < px; i++) {
                ...
                rgba[d + 1] = b[p + 2];
                rgba[d + 2] = b[p + 4];
                rgba[d + 3] = b[p + 6]; // high byte of each 16-bit BE sample
                p += 8;
            }
            const preview = canvasFromRGBA(rgba, width, height);
            if (preview)
...
async function parseWbmp(file) {
    ...
    if (b.length < 4)
        return null;
    if (b[0] !== 0x00)
        return null; // type 0 = B/W, no compression
    ...
    const mb = () => { let v = 0; let c; do {
        c = b[p++];
        v = (v << 7) | (c & 0x7f);
    } while (c & 0x80); return v; };
    const width = mb();
    const height = mb();
    if (!width || !height || width > 30000 || height > 30000)
        return null;
...
async function parseXbm(file) {
    ...
    if (!wm || !hm || !/\{[\s\S]*0x[0-9a-f]/i.test(text))
        return null;
    ...
    try {
        ...
        while ((m = re.exec(arr)) && bytes.length < need)
...
async function parseXpm(file) {
    const text = await readText(file, PARSE_TEXT_MAX);
    if (!/XPM/.test(text) && !/static\s+char/.test(text))
        return null;
    // Collect the quoted strings forming the data array.
    const strings = [];
    const re = /"((?:[^"\\]|\\.)*)"/g;
    let m;
    while ((m = re.exec(text)) && strings.length < 200000)
        strings.push(m[1]);
    if (!strings.length)
        return null;
    const vals = strings[0].trim().split(/\s+/).map(Number);
    const [width, height, ncolors, cpp] = vals;
    if (!width || !height || !ncolors || !cpp || width > 30000 || height > 30000)
        return null;
    const out = {
        'Format': 'XPM (X PixMap)',
        'Dimensions': width + ' × ' + height,
    ...
    try {
        ...
        const parseColor = (name) => {
            ...
            if (/^none$/i.test(name))
                return [0, 0, 0, 0];
            let mm;
            if ((mm = name.match(/^#([0-9a-fA-F]{6})$/))) {
                const v = parseInt(mm[1], 16);
                return [(v >> 16) & 255, (v >> 8) & 255, v & 255, 255];
            }
            if ((mm = name.match(/^#([0-9a-fA-F]{12})$/)))
                return [parseInt(mm[1].slice(0, 2), 16), parseInt(mm[1].slice(4, 6), 16), parseInt(mm[1].slice(8, 10), 16), 255];
...
async function parseSunRaster(file) {
    ...
    if (type === 2)
        out['Note'] = 'Byte-RLE encoded; preview decoder not bundled.';
    try {
...
async function parseSgi(file) {
    ...
    if (!xsize || !ysize || xsize > 30000 || ysize > 30000)
        return null;
    const out = {
    ...
    if (name)
        out['Image name'] = name;
    if (storage === 1)
        out['Note'] = 'RLE-encoded; preview decoder not bundled.';
    try {
        if (storage === 0 && bpc === 1) {
...
async function parseHdr(file) {
    ...
    if (fmt)
        out['Pixel format'] = fmt;
    const exp = get('EXPOSURE');
    if (exp)
        out['Exposure'] = exp;
    ...
    if (res) {
        const dims = {};
        dims[res[1][1]] = parseInt(res[2], 10);
        dims[res[3][1]] = parseInt(res[4], 10);
        if (dims.X && dims.Y)
            out['Dimensions'] = dims.X + ' × ' + dims.Y;
    }
...
async function parseDds(file) {
    ...
    if (depth > 1 && (caps2 & 0x200000))
        out['Volume depth'] = depth;
    ...
    if (pfFlags & 0x4) { // DDPF_FOURCC
        ...
        if ((pfFlags & 0x40) && rgbBitCount === 32)
            kind = 'rgba';
    }
    out['Pixel format'] = compression;
    if (caps2 & 0x200)
        out['Type'] = 'cubemap';
    else if (caps2 & 0x200000)
        out['Type'] = 'volume texture';
    ...
    try {
        if (kind && width > 0 && height > 0 && width <= 16384 && height <= 16384) {
            ...
            if (kind === 'rgba' && compression === 'DX10') {
                ...
                rgba = decodeDdsUncompressed(b, dataOff, width, height, bgra ? 0x00ff0000 : 0x000000ff, 0x0000ff00, bgra ? 0x000000ff : 0x00ff0000, dxgi === 88 ? 0 : 0xff000000);
            }
            else if (kind === 'rgba') {
    ...
    if (!decoded) {
        ...
        if (kind === null) {
            if (compression === 'DX10')
                why = 'Preview: ' + (dxgiName(dxgi).replace(/^[\s(]+|[)\s]+$/g, '') || ('DXGI ' + dxgi)) + ' not decoded';
            else if (pfFlags & 0x4)
                why = 'Preview: ' + (fourCC.replace(/\0/g, '') || 'FourCC') + ' not decoded';
            else
                why = 'Preview: uncompressed ' + rgbBitCount + '-bit layout not decoded';
        }
        else {
...
function fourCCKind(fourCC) {
    ...
    if (f === 'DXT4' || f === 'DXT5')
        return 'bc3';
    if (f === 'ATI1' || f === 'BC4U' || f === 'BC4S')
        return 'bc4';
    if (f === 'ATI2' || f === 'BC5U' || f === 'BC5S')
        return 'bc5';
    return null;
}
...
function dxgiKind(n) {
    if (n === 70 || n === 71 || n === 72)
        return 'bc1'; // BC1 typeless/UNORM/sRGB
    if (n === 73 || n === 74 || n === 75)
        return 'bc2'; // BC2
    if (n === 76 || n === 77 || n === 78)
        return 'bc3'; // BC3
    if (n === 79 || n === 80 || n === 81)
        return 'bc4'; // BC4 (UNORM/SNORM)
    if (n === 82 || n === 83 || n === 84)
        return 'bc5'; // BC5
    if (n === 28 || n === 29 || n === 87 || n === 88)
        return 'rgba'; // R8G8B8A8 / B8G8R8A8 family
    return null;
}
function dxgiName(n) {
        ...
        while (p < b.length - 1) {
            ...
            if (name === 'channels') {
                ...
                while (q < valStart + size && b[q]) {
                    let cn = '';
                    while (q < b.length && b[q])
                        cn += String.fromCharCode(b[q++]);
                    q++;
...
async function parseJp2(file, ext) {
    ...
    try {
        ...
        while (p + 8 <= b.length) {
            ...
            let hdr = 8;
            if (len === 1) {
                len = Number(dv.getBigUint64(p + 8, false));
                hdr = 16;
            }
            if (len === 0)
                len = b.length - p;
                ...
                if (type === 'jp2h') {
                    ...
                    while (q + 8 <= p + len && q + 8 <= b.length) {
                        ...
                        if (btype === 'ihdr') {
                            ...
                            const meth = b[q + 8];
                            if (meth === 1) {
                                const cs = dv.getUint32(q + 11, false);
                                out['Color space'] = cs === 16 ? 'sRGB' : cs === 17 ? 'grayscale' : cs === 18 ? 'sYCC' : ('enum ' + cs);
                            }
                            else
                                out['Color space'] = 'ICC profile';
                        }
                        if (blen <= 0)
                            break;
                        q += blen;
    ...
    try {
        ...
        if (res && res.rgba) {
            ...
            if (preview) {
                out._previewNode = preview;
                if (!out['Dimensions'])
                    out['Dimensions'] = res.width + ' × ' + res.height;
                return;
            }
        }
    }
...
async function parseJxr(file) {
    ...
    try {
        if (ifdOff && ifdOff + 2 <= b.length) {
            ...
            if (pfGuid) {
                out['Pixel format GUID'] = pfGuid;
                const pn = JXR_PF[pfGuid];
                if (pn)
                    out['Pixel format'] = pn;
            }
        }
...
const JXR_PF = Object.fromEntries([
...
].map(([k, v]) => ['6FDDC324-4E03-4BFE-B185-3D77768DC9' + k, v]));
...
async function parseEps(file, ext) {
    ...
    if (m1) {
        out['PostScript level'] = 'Adobe ' + m1[1];
        if (m1[2])
            out['EPSF version'] = m1[2];
    }
    ...
    if (bbox && !/atend/i.test(bbox)) {
        ...
        if (n.length === 4)
            out['Dimensions'] = (n[2] - n[0]) + ' × ' + (n[3] - n[1]) + ' pt';
    }
    const creator = dsc('Creator');
    if (creator)
        out['Creator'] = creator;
    const title = dsc('Title');
    if (title)
        out['Title'] = title;
    const cdate = dsc('CreationDate');
    if (cdate)
        out['Creation date'] = cdate;
    const forr = dsc('For');
    if (forr)
        out['Author'] = forr;
    const pages = dsc('Pages');
    if (pages)
        out['Pages'] = pages;
    ...
    try {
        ...
        (async () => {
            ...
            try {
                ...
                img.addEventListener('load', () => { if (url) {
                    URL.revokeObjectURL(url);
                    url = null;
                } }, { once: true });
                img.addEventListener('error', () => { if (url) {
                    URL.revokeObjectURL(url);
                    url = null;
                } stage.remove(); }, { once: true });
    ...
    if (ascii(b, 40, 4) === ' EMF') {
        ...
        if (descLen && descOff && descOff + descLen * 2 <= b.length) {
            ...
            for (let i = 0; i < descLen; i++) {
                const c = b[descOff + i * 2] | (b[descOff + i * 2 + 1] << 8);
                if (c)
                    s += String.fromCharCode(c);
                else
                    s += ' ';
...
async function parseIcns(file) {
    ...
    if (entries.length)
        out._sections = [{ title: 'Embedded icons (' + entries.length + ')', node: preBlock(entries.join('\n')) }];
...
async function parseCur(file) {
    ...
    if (list.length)
        out._sections = [{ title: 'Cursor images', node: preBlock(list.join('\n')) }];
    return out;
}
async function parseAni(file) {
    const b = await readAll(file, 1024 * 1024);
    if (ascii(b, 0, 4) !== 'RIFF' || ascii(b, 8, 4) !== 'ACON')
        return null;
    ...
    try {
        ...
        out['Frames'] = frames;
        if (steps)
            out['Animation steps'] = steps;
        if (jifRate)
            out['Default rate'] = jifRate + ' jiffies (' + (jifRate / 60).toFixed(2) + ' s/frame)';
    }
    catch (_) { }
    return out;
}
...
async function parseMng(file) {
    const b = await readAll(file, 64 * 1024);
    if (!(b[0] === 0x8a && b[1] === 0x4d && b[2] === 0x4e && b[3] === 0x47))
        return null; // \212 MNG
    ...
    if (type !== 'MHDR')
        return { 'Format': 'MNG (Multiple-image Network Graphics)' };
    r.seek(16);
    const width = r.u32();
    const height = r.u32();
    const ticks = r.u32();
    ...
    if (playTime && playTime !== 0x7fffffff && ticks)
        out['Play time'] = (playTime / ticks).toFixed(2) + ' s';
...
async function parseLottie(file) {
    ...
    let j;
    try {
        j = JSON.parse(await readText(file, 16 * 1024 * 1024));
    }
    catch (_) {
        return null;
    }
    if (!j || (j.v == null && !Array.isArray(j.layers)))
        return null; // not a Bodymovin doc
    const out = { 'Format': 'Lottie (Bodymovin JSON)' };
    if (j.v)
        out['Bodymovin version'] = j.v;
    if (j.nm)
        out['Name'] = j.nm;
    if (j.w && j.h)
        out['Composition size'] = j.w + ' × ' + j.h;
    const fr = j.fr || 0;
    if (fr)
        out['Frame rate'] = fr + ' fps';
    if (j.ip != null && j.op != null) {
        out['In / out point'] = j.ip + ' → ' + j.op;
        if (fr)
            out['Duration'] = ((j.op - j.ip) / fr).toFixed(2) + ' s';
        out['Frames'] = (j.op - j.ip);
    }
    const layers = Array.isArray(j.layers) ? j.layers : [];
    out['Layers'] = layers.length;
    if (Array.isArray(j.assets))
        out['Assets'] = j.assets.length;
    if (Array.isArray(j.markers) && j.markers.length)
        out['Markers'] = j.markers.length;
    // Layer type breakdown
    const LTYPE = { 0: 'precomp', 1: 'solid', 2: 'image', 3: 'null', 4: 'shape', 5: 'text', 6: 'audio' };
    const byType = {};
    for (const ly of layers) {
        const t = LTYPE[ly.ty] || ('type ' + ly.ty);
        byType[t] = (byType[t] || 0) + 1;
    }
    const sects = [];
    if (Object.keys(byType).length)
        out['Layer types'] = Object.entries(byType).map(([k, v]) => k + ' (' + v + ')').join(', ');
    if (Array.isArray(j.markers) && j.markers.length) {
        ...
        (async () => {
            try {
                if (!(window.lottie && window.lottie.loadAnimation))
                    await loadScript('assets/vendor/lottie/lottie.min.js');
                if (window.lottie && window.lottie.loadAnimation) {
...
//# sourceMappingURL=parsers-image.js.map