...
function wrapPreview(canvas, w, h) {
    ...
    wrap.appendChild(el('div', { style: 'font-size:11px;opacity:.6;margin-top:4px;' }, 'Decoded preview · ' + w + ' × ' + h + (Math.max(w, h) > MAX_EDGE ? ' (scaled)' : '')));
    return wrap;
}
// =====================================================================
...
function decodeAlphaBlock(b, off, bx, by, dstW, dstH, write) {
    ...
    if (a0 > a1) {
        for (let i = 1; i < 7; i++)
            a[i + 1] = (((7 - i) * a0 + i * a1) / 7) | 0;
    }
    else {
        for (let i = 1; i < 5; i++)
            a[i + 1] = (((5 - i) * a0 + i * a1) / 5) | 0;
        a[6] = 0;
        a[7] = 255;
    }
