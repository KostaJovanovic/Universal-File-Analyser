...
function pemKind(banner) {
    ...
    if (b.includes('OPENSSH PRIVATE'))
        return { type: 'OpenSSH', scheme: 'OpenSSH', priv: true };
    if (b.includes('RSA PRIVATE'))
        return { type: 'RSA', scheme: 'PKCS#1', priv: true };
    if (b.includes('EC PRIVATE'))
        return { type: 'EC', scheme: 'SEC1', priv: true };
    if (b.includes('DSA PRIVATE'))
        return { type: 'DSA', scheme: 'PKCS#1', priv: true };
    if (b.includes('ENCRYPTED PRIVATE'))
        return { type: 'PKCS#8 (encrypted)', scheme: 'PKCS#8', priv: true };
    if (b.includes('PRIVATE'))
        return { type: 'PKCS#8', scheme: 'PKCS#8', priv: true };
    if (b.includes('RSA PUBLIC'))
        return { type: 'RSA', scheme: 'PKCS#1', priv: false };
    if (b.includes('PUBLIC'))
        return { type: 'public key', scheme: 'SubjectPublicKeyInfo', priv: false };
...
async function parsePemKey(file, ext) {
    ...
    if (!PEM_RE.test(text)) {
        if (ext === 'pub')
            return await parseOpenSshPub(text);
        return null;
    }
    ...
    if (objs > 1)
        out['PEM objects'] = objs;
    if (kind.priv)
        out['⚠ Warning'] = 'Contains a PRIVATE key - keep secret, do not share';
    return out;
}
// ---------- .p8 (PKCS#8) ----------
...
async function parsePemIdentify(file, label, banners, note) {
    ...
    if (!banners.some((b) => banner.includes(b)))
        return null;
...
async function parsePpk(file) {
    ...
    if (enc)
        out['Encryption'] = enc[1].trim();
    const comment = text.match(/^Comment:\s*(.+)$/m);
    if (comment)
        out['Comment'] = comment[1].trim();
    const pubLines = text.match(/^Public-Lines:\s*(\d+)$/m);
    if (pubLines)
        out['Public-key lines'] = pubLines[1];
...
async function parseOvpn(file) {
    ...
    if (!/^\s*(client|remote|dev\s+tun|dev\s+tap|tls-auth|proto)\b/m.test(text))
        return null;
    ...
    if (remotes.length)
        out['Remote endpoints'] = remotes.length;
    ...
    if (cipher)
        out['Cipher'] = cipher.trim();
    const auth = (text.match(/^\s*auth\s+(\S+)/m) || [])[1];
    if (auth)
        out['Auth digest'] = auth;
    if (/^\s*(comp-lzo|compress)\b/m.test(text))
        out['Compression'] = 'enabled';
    const inline = [];
    ...
    if (remotes.length)
        out._sections = [{ title: 'Remotes (' + remotes.length + ')', node: preBlock(remotes.join('\n')) }];
    return out;
}
// ---------- WireGuard: .wg / .conf ----------
    ...
    try {
        for (const rdn of derChildren(b, nameNode.content, nameNode.end)) { // SET OF
            for (const atv of derChildren(b, rdn.content, rdn.end)) { // SEQUENCE {OID, value}
                ...
                if (kids.length < 2 || kids[0].tag !== 0x06)
                    continue;
                const key = X509_DN_ATTR[derOid(b, kids[0])];
                if (key)
                    parts.push(key + '=' + utf8(b.subarray(kids[1].content, kids[1].end)));
            }
        }
    }
    catch (_) { }
...
function derTime(b, n) {
    try {
        ...
        if (n.tag === 0x17) {
            ...
            return null;
        return new Date(Date.UTC(year, parseInt(rest.slice(0, 2), 10) - 1, parseInt(rest.slice(2, 4), 10), parseInt(rest.slice(4, 6), 10) || 0, parseInt(rest.slice(6, 8), 10) || 0, parseInt(rest.slice(8, 10), 10) || 0));
    }
    catch (_) {
        return null;
    }
...
async function parseJks(file) {
        ...
        try {
            for (let i = 0; i < count; i++) {
                ...
                if (tag === 1) { // KEY entry: encrypted key + cert chain
                    ...
                    e.kind = 'TrustedCertEntry';
                    trusted++;
                    if (version === 2)
                        r.bytes_(r.u16()); // cert type string (e.g. "X.509")
                    e.certs.push(r.bytes_(r.u32()));
                }
                else {
                    break;
                } // unknown tag: stop cleanly
                entries.push(e);
            }
        }
        catch (_) {
    ...
    if (keys)
        out['Private-key entries'] = keys;
    if (trusted)
        out['Trusted-cert entries'] = trusted;
    ...
    for (let i = 0; i < entries.length; i++) {
        ...
        for (const der of e.certs) {
            ...
            if (x) {
                ...
                if (x.issuer)
                    lines.push('    Issuer:  ' + x.issuer + (x.subject && x.subject === x.issuer ? ' (self-signed)' : ''));
                if (x.notBefore && x.notAfter)
                    lines.push('    Valid:   ' + fmtDate(x.notBefore) + '  to  ' + fmtDate(x.notAfter) + (x.notAfter < now ? '  (EXPIRED)' : ''));
            }
            const fp = await sha256hex(der);
            if (fp)
                lines.push('    SHA-256: ' + fp);
        }
        lines.push('');
    }
    if (lines.length)
        out._sections = [{ title: 'Entries (' + entries.length + ')', node: preBlock(lines.join('\n').trimEnd()), open: true }];
    if (truncated)
        out['Note'] = 'File appears truncated - listed the entries that could be read.';
    out['⚠ Warning'] = keys
...
async function parseSshKeyDb(file, ext) {
    ...
    for (const l of lines) {
        ...
        if (m)
            algos[m[1]] = (algos[m[1]] || 0) + 1;
        if (/-cert-v01@openssh\.com/.test(l))
            certs++;
    }
    const out = {
    ...
    if (hashed)
        out['Hashed host names'] = hashed + ' entr' + (hashed === 1 ? 'y' : 'ies');
    if (certs)
...
async function parseMobileconfig(file) {
    ...
    if (v.PayloadDisplayName)
        out['Name'] = String(v.PayloadDisplayName);
    if (v.PayloadOrganization)
        out['Organization'] = String(v.PayloadOrganization);
    if (v.PayloadIdentifier)
        out['Identifier'] = String(v.PayloadIdentifier);
    if (v.PayloadUUID)
        out['UUID'] = String(v.PayloadUUID);
    ...
    if (types.length)
        out['Payload types'] = Array.from(new Set(types)).join(', ');
    out['Signed'] = signed ? 'yes (CMS / PKCS#7 wrapper)' : res.format === 'binary' ? 'no (binary plist)' : 'no (plain plist)';
    if (types.length)
        out._sections = [{ title: 'Payload types', node: preBlock(types.join('\n')) }];
    return out;
}
...
async function parseMobileprovision(file) {
    ...
    if (v.AppIDName)
        out['App ID name'] = String(v.AppIDName);
    if (v.TeamName)
        out['Team name'] = String(v.TeamName);
    if (Array.isArray(v.TeamIdentifier))
        out['Team ID'] = v.TeamIdentifier.join(', ');
    if (v.CreationDate)
        out['Created'] = fmtDate(v.CreationDate);
    if (v.ExpirationDate) {
    ...
    if (v.ProvisionsAllDevices)
        out['Provisions all devices'] = 'yes';
    if (Array.isArray(v.ProvisionedDevices))
        out['Provisioned UDIDs'] = v.ProvisionedDevices.length;
    const ent = v.Entitlements;
    if (ent && typeof ent === 'object') {
        if (ent['application-identifier'])
            out['Application identifier'] = String(ent['application-identifier']);
        if (ent['get-task-allow'] != null)
            out['get-task-allow'] = String(ent['get-task-allow']);
        if (ent['aps-environment'])
            out['APS environment'] = String(ent['aps-environment']);
    }
    return out;
}
...
async function parseReg(file) {
    ...
    if (!/^\s*(Windows Registry Editor Version|REGEDIT4)/m.test(text))
        return null;
    ...
    if (deletions)
        out['Key deletions'] = deletions;
    if (Object.keys(types).length)
        out['Value types'] = Object.entries(types).map(([k, v]) => k + ': ' + v).join('  ');
    const autorun = keys.filter((m) => /\\(Run|RunOnce|RunServices|Winlogon|Userinit|Shell)\b/i.test(m[2]));
    if (autorun.length)
        out['⚠ Autorun / persistence keys'] = autorun.length + ' (Run/RunOnce/Winlogon/...)';
    const keyList = keys.map((m) => (m[1] === '-' ? '[DEL] ' : '') + m[2]);
    if (keyList.length)
        out._sections = [{ title: 'Keys (' + keyList.length + ')', node: preBlock(keyList.slice(0, 500).join('\n')) }];
    return out;
}
...
async function parseSldreg(file) {
    ...
    if (!/^\s*REGEDIT4/.test(text))
        return null;
    const keys = Array.from(text.matchAll(/^\s*\[(-?)(HKEY[^\]]+)\]/gm));
    const values = (text.match(/^\s*(?:"[^"]*"|@)\s*=/gm) || []).length;
    // Product release years from keys like ...\SolidWorks\SOLIDWORKS 2024.
    const versions = Array.from(new Set(Array.from(text.matchAll(/SOLIDWORKS\s+(\d{4})/g)).map((m) => m[1]))).sort();
    ...
    if (groups.size)
        out['Settings groups'] = groups.size;
    const sections = [];
    if (groups.size) {
    ...
    const keyList = keys.map((m) => (m[1] === '-' ? '[DEL] ' : '') + m[2]);
    if (keyList.length)
        sections.push({ title: 'Registry keys (' + keyList.length + ')', node: preBlock(keyList.slice(0, 500).join('\n')) });
    if (sections.length)
...
function parsePcap(head) {
    ...
    else if (be === 0xD4C3B2A1) {
        little = true;
    }
    else if (be === 0xA1B23C4D) {
        little = false;
        nano = true;
    }
    else if (be === 0x4D3CB2A1) {
        little = true;
        nano = true;
    }
    else
...
function derRead(b, pos) {
    ...
    if (p >= b.length)
        throw new Error('der: len');
    let len = b[p++];
    if (len & 0x80) { // long form
        const n = len & 0x7f;
        if (n === 0 || n > 4)
            throw new Error('der: indefinite/oversized length');
        len = 0;
        for (let i = 0; i < n; i++) {
            if (p >= b.length)
                throw new Error('der: len');
...
function derOid(b, n) {
    ...
    if (started)
        parts.push(val); // tolerate truncation
    return parts.join('.');
}
...
async function parseP12(file) {
    ...
    try {
        ...
        try {
            ...
            if (ci.length && ci[0].tag === 0x06)
                authType = derOid(b, ci[0]);
        }
        catch (_) { }
        ...
        if (top.length >= 3 && top[2].tag === 0x10 && top[2].constructed) {
            try {
                ...
                for (let i = mac.length - 1; i >= 1; i--) {
                    if (mac[i].tag === 0x02) {
                        iterations = derInt(b, mac[i]);
                        break;
                    }
                }
                if (iterations == null)
                    iterations = 1; // ASN.1 DEFAULT 1
        ...
        walkOids(b, 0, pfx.end, (oid) => {
            ...
            if (bag)
                bagCounts[bag] = (bagCounts[bag] || 0) + 1;
            if (ENC_OIDS[oid] && !encAlgos.includes(ENC_OIDS[oid]))
                encAlgos.push(ENC_OIDS[oid]);
        }, 0);
        ...
        if (shrouded)
            out['Private key bag'] = 'pkcs8ShroudedKeyBag (encrypted)';
        else if (bagCounts['keyBag'])
            out['Private key bag'] = 'keyBag (unencrypted PKCS#8)';
        if (encAlgos.length)
            out['Encryption'] = encAlgos.join(', ');
...
function walkOids(b, start, end, cb, depth) {
    ...
    while (p < end) {
        ...
        if (n.end <= n.start)
            return; // guard against zero-progress
        ...
        if (n.constructed) {
            try {
                walkOids(b, n.content, n.end, cb, depth + 1);
            }
            catch (_) { }
        }
            ...
            let inner = n.content;
            if (n.tag === 0x03)
                inner += 1; // BIT STRING leading "unused bits" octet
            try {
                const first = b[inner]; // 0x30 = SEQUENCE, 0x31 = SET
                if (first === 0x30 || first === 0x31)
                    walkOids(b, inner, n.end, cb, depth + 1);
            }
            catch (_) { }
        }
        p = n.end;
...
function parseEvt(head) {
    ...
    if (hdrLen !== 0x30)
        return null;
    if (ascii(head.subarray(4, 8)) !== 'LfLe')
        return null;
    r.seek(8);
    const major = r.u32();
    const minor = r.u32();
    ...
    if (count >= 0)
        out['Records (approx)'] = count;
    if (maxSize)
        out['Configured max size'] = fmtBytes(maxSize);
...
async function parseYara(file) {
    ...
    if (imports.length)
        out['Imports'] = imports.join(', ');
    if (includes)
        out['Includes'] = includes;
    if (metaAuthors.length)
        out['Authors'] = metaAuthors.slice(0, 10).join(', ');
...
async function parseRules(file) {
    ...
    for (let line of lines) {
        ...
        if (!m)
            continue;
        if (!/\bsid\s*:/.test(l))
            continue; // a real IDS rule carries a sid
        if (off)
            disabled++;
    ...
    for (const r of rules) {
        ...
        if (ct)
            classtypes[ct.trim()] = (classtypes[ct.trim()] || 0) + 1;
    }
    ...
    if (disabled)
        out['Disabled (commented)'] = disabled;
    if (Object.keys(classtypes).length)
        out['Top classtypes'] = fmtTally(classtypes).split(', ').slice(0, 8).join(', ');
...
async function parseStix(file) {
    ...
    let objects;
    if (j && j.type === 'bundle' && Array.isArray(j.objects))
        objects = j.objects;
    else if (Array.isArray(j))
        objects = j;
    else if (j && typeof j.type === 'string')
        objects = [j];
    else
        return null;
    ...
    if (!looksStix && !(j && j.spec_version))
        return null;
    const out = { 'Format': 'STIX threat intelligence' };
    if (j.spec_version)
        out['Spec version'] = String(j.spec_version);
    else
        out['Spec version'] = '2.0 (inferred)';
    ...
    if (indicators.length)
        out['Indicators'] = indicators.length;
    const actors = objects.filter((o) => o && o.type === 'threat-actor').map((o) => o.name).filter(Boolean);
    if (actors.length)
        out['Threat actors'] = Array.from(new Set(actors)).slice(0, 10).join(', ');
    ...
    if (patterns.length)
        out._sections = [{ title: 'Indicator patterns (' + patterns.length + ')', node: preBlock(patterns.slice(0, 200).join('\n')) }];
    return out;
}
// ---------- OpenIOC: .ioc (XML) ----------
async function parseIoc(file) {
    const text = await readText(file, 4_000_000);
    if (!/<ioc\b/i.test(text))
        return null;
    ...
    if (items)
        out['Indicator items'] = items;
    const logic = (text.match(/<Indicator\b[^>]*\boperator\s*=\s*"([^"]+)"/i) || [])[1];
    if (logic)
        out['Top operator'] = logic.toUpperCase();
    ...
    if (searches.length)
        out._sections = [{ title: 'Searched terms (' + searches.length + ')', node: preBlock(searches.slice(0, 300).join('\n')) }];
    return out;
}
...
async function parseSaz(file) {
    let z;
    try {
        z = await openZip(file, 64 * 1024 * 1024);
    }
    catch (_) {
        return null;
    }
    ...
    if (!requests.length && !z.has('_index.htm'))
        return null;
    ...
    for (const e of requests.slice(0, 400)) {
        ...
        if (m) {
            ...
            try {
                hosts.add(new URL(m[2], 'http://x/').host || (t.match(/^Host:\s*(\S+)/im) || [])[1]);
            }
            catch (_) { }
        }
    ...
    if (Object.keys(methods).length)
        out['Methods'] = fmtTally(methods);
    if (Object.keys(statuses).length)
        out['Status codes'] = fmtTally(statuses);
...
async function parse1pux(file) {
    ...
    if (!z.has('export.data') && !z.has('export.attributes'))
        return null;
    ...
    if (attrText) {
        try {
            ...
            if (a.version != null)
                out['Export version'] = String(a.version);
            if (a.description)
    ...
    if (dataText) {
        try {
            ...
            for (const acc of accounts) {
                if (acc.attrs && acc.attrs.name)
                    acctNames.push(acc.attrs.name);
            ...
            out['Items'] = items;
            if (acctNames.length)
                out['Account names'] = acctNames.slice(0, 10).join(', ');
        }
        catch (_) { }
    }
    const attachments = z.match(/^files\//i).length;
    if (attachments)
        out['Attachment files'] = attachments;
...
async function parseOpvault(file, ext, name) {
    ...
    if (/profile\.js$/.test(fname))
        out['Component'] = 'profile.js (vault profile / KDF parameters)';
    else if (/band_.*\.js$/.test(fname))
        out['Component'] = 'band file (encrypted item batch)';
    else if (/\.opvault$/.test(fname))
        out['Component'] = 'vault bundle';
...
function pgpWalk(b, limit) {
    ...
    while (p < end) {
        const c = b[p];
        if (!(c & 0x80))
            break; // not a valid packet tag octet
        ...
        if (c & 0x40) { // new-format packet
            ...
            if (l0 < 192) {
                ...
                len = ((l0 - 192) << 8) + b[p + 2] + 192;
                headerLen = 3;
            }
            else if (l0 === 255) {
                len = (b[p + 2] << 24) | (b[p + 3] << 16) | (b[p + 4] << 8) | b[p + 5];
                headerLen = 6;
            }
            else {
                break;
            } // partial body lengths - stop
        }
            ...
            if (lt === 0) {
                ...
                len = (b[p + 1] << 24) | (b[p + 2] << 16) | (b[p + 3] << 8) | b[p + 4];
                headerLen = 5;
            }
            else {
                break;
            } // indeterminate length - stop
        }
        if (len < 0 || headerLen == null)
            break;
        ...
        if (tag === 13 && !info.userId) { // User ID packet body is UTF-8 text
            try {
                info.userId = utf8(b.subarray(body, body + Math.min(len, 200)));
            }
            catch (_) { }
        }
        p = body + len;
...
async function parsePgp(file, ext) {
    ...
    if (armorMatch) {
        ...
        const b64 = body.split(/-----END/)[0]
            ...
            .replace(/\s+/g, '');
        try {
            walked = pgpWalk(b64ToBytes(b64), 1_000_000);
        }
        catch (_) { }
    }
    else if (head[0] & 0x80) {
    ...
    if (!armored && !walked)
        return null; // not OpenPGP
    out['Format'] = 'OpenPGP ' + (ext === 'sig' ? 'signature' :
    ...
    if (walked) {
        ...
        if (walked.info.keyAlgo)
            out['Key algorithm'] = walked.info.keyAlgo;
        if (walked.info.created)
            out['Key created'] = fmtDate(walked.info.created);
        if (walked.info.userId)
            out['User ID'] = walked.info.userId.replace(/[^\x20-\x7e].*$/, '').trim();
...
function parseKdb(head) {
    ...
    if (sig2 === 0xB54BFB65)
        out['Variant'] = 'KeePass 1.x (KDB)';
    else
        out['Variant'] = 'KeePass (signature 2 = 0x' + sig2.toString(16).toUpperCase() + ')';
    const flags = r.u32();
    const enc = [];
    if (flags & 0x02)
        enc.push('AES (Rijndael)');
    if (flags & 0x08)
        enc.push('Twofish');
    if (enc.length)
        out['Cipher'] = enc.join(' / ');
...
function parsePvk(head) {
    ...
    if (saltLen)
        out['Salt length'] = saltLen + ' bytes';
    if (keyLen)
        out['Key blob length'] = fmtBytes(keyLen);
...
//# sourceMappingURL=parsers-security.js.map