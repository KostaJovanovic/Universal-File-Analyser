...
import { buildEmbeddedImagesCard } from '../renderers/embedded-images.js';
            ...
            if (enc.toLowerCase() === 'b') {
                ...
                for (let i = 0; i < txt.length; i++) {
                    if (txt[i] === '=' && i + 2 < txt.length) {
                        arr.push(parseInt(txt.substr(i + 1, 2), 16));
                        i += 2;
                    }
                    else
                        arr.push(txt.charCodeAt(i));
                }
...
function analyseMime(raw) {
    ...
    if (subj)
        out['Subject'] = subj;
    if (map['from'])
        out['From'] = decodeRfc2047(map['from']);
    if (map['to'])
        out['To'] = decodeRfc2047(map['to']);
    if (map['cc'])
        out['Cc'] = decodeRfc2047(map['cc']);
    if (map['date'])
        out['Date'] = map['date'];
    if (map['message-id'])
        out['Message-ID'] = map['message-id'];
    ...
    if (/spf=pass/.test(ar))
        ...
        auth.push('SPF ' + (ar.match(/spf=(\w+)/) || [])[1]);
    else if (map['received-spf'])
        auth.push('SPF (' + (map['received-spf'].split(/\s/)[0] || '?') + ')');
    if (map['dkim-signature'] != null || /dkim=pass/.test(ar))
        auth.push(/dkim=pass/.test(ar) ? 'DKIM pass' : 'DKIM signed');
    if (/dmarc=/.test(ar))
        auth.push('DMARC ' + (ar.match(/dmarc=(\w+)/) || [])[1]);
    ...
    if (ct.type.startsWith('multipart/') && ct.params.boundary) {
        ...
        for (const chunk of chunks) {
            ...
            const fname = decodeRfc2047((pct.params.name) ||
            ...
            if (isAttach)
                attachments.push((fname || '(unnamed)') + ' [' + (pct.type || '?') + ', ~' + fmtBytes(sizeApprox) + ']');
            // First text/plain part -> preview.
            if (!bodyPreview && pct.type === 'text/plain') {
                let txt = pbRaw;
                if (/quoted-printable/i.test(pm['content-transfer-encoding'] || ''))
                    txt = decodeQuotedPrintable(txt);
        ...
        let txt = body;
        if (/quoted-printable/i.test(map['content-transfer-encoding'] || ''))
            txt = decodeQuotedPrintable(txt);
        if (/text\//.test(ct.type || 'text/plain'))
            bodyPreview = txt.trim().slice(0, 4000);
    }
    ...
    if (parts.length)
        sections.push({ title: 'MIME parts (' + parts.length + ')', node: preBlock(parts.join('\n')) });
    if (attachments.length)
        sections.push({ title: 'Attachments (' + attachments.length + ')', node: preBlock(attachments.join('\n')) });
    if (bodyPreview)
        sections.push({ title: 'Body preview', node: preBlock(bodyPreview), open: true });
    return { out, sections };
}
...
async function parseMbox(file) {
    ...
    for (const line of lines) {
        ...
        if (inHeaders) {
            ...
            if (m) {
                ...
                if (k === 'from' && !cur.from)
                    cur.from = decodeRfc2047(m[2]);
                else if (k === 'subject' && !cur.subject)
                    cur.subject = decodeRfc2047(m[2]);
                else if (k === 'date' && !cur.date)
                    cur.date = m[2];
            }
        }
    }
    ...
    const sample = msgs.slice(0, 200).map((m, i) => (i + 1) + '. ' + (m.subject || '(no subject)') + '\n    ' + (m.from || '?') + (m.date ? '  ·  ' + m.date : '')).join('\n');
    out._sections = [
...
function icalLine(line) {
    ...
    for (const s of segs) {
        const i = s.indexOf('=');
        if (i > 0)
            params[s.slice(0, i).toUpperCase()] = s.slice(i + 1);
    }
...
async function parseIcs(file) {
    ...
    for (const l of lines) {
        if (l.name === 'PRODID')
            prodid = l.value;
        else if (l.name === 'VERSION' && !version)
            version = l.value;
            ...
            if (counts[t] != null)
                counts[t]++;
            if ((t === 'VEVENT' || t === 'VTODO') && !cur) {
                cur = { type: t, attendees: 0 };
                curType = t;
            }
        }
        else if (l.name === 'END') {
            ...
            switch (l.name) {
                ...
                case 'DTEND':
                    ...
                    break;
                case 'LOCATION':
                    cur.location = l.value;
                    break;
                case 'ORGANIZER':
                    cur.organizer = (l.params.CN || l.value).replace(/^mailto:/i, '');
                    break;
                case 'ATTENDEE':
                    cur.attendees++;
                    break;
    ...
    if (counts.VJOURNAL)
        out['Journals'] = counts.VJOURNAL;
    if (counts.VFREEBUSY)
        out['Free/busy'] = counts.VFREEBUSY;
    if (counts.VALARM)
        out['Alarms'] = counts.VALARM;
    if (counts.VTIMEZONE)
        out['Timezones'] = counts.VTIMEZONE;
    if (events.length) {
        const lines2 = events.map((e, i) => {
            const bits = [(i + 1) + '. ' + (e.summary || '(no title)')];
            if (e.dtstart)
                bits.push('   start: ' + e.dtstart);
            if (e.dtend)
                bits.push('   end:   ' + e.dtend);
            if (e.location)
                bits.push('   where: ' + e.location);
            if (e.organizer)
                bits.push('   organizer: ' + e.organizer);
            if (e.attendees)
                bits.push('   attendees: ' + e.attendees);
            if (e.rrule)
                bits.push('   repeats: ' + e.rrule);
            return bits.join('\n');
        }).join('\n\n');
...
async function parseVcs(file) {
    ...
    for (const l of lines) {
        if (l.name === 'VERSION' && !version)
            version = l.value;
        else if (l.name === 'BEGIN' && l.value.toUpperCase() === 'VEVENT') {
            events++;
            if (!cur)
                cur = {};
        }
        else if (cur) {
            if (l.name === 'SUMMARY' && !cur.summary)
                cur.summary = l.value;
            else if (l.name === 'DTSTART' && !cur.dtstart)
                cur.dtstart = l.value;
            else if (l.name === 'DTEND' && !cur.dtend)
                cur.dtend = l.value;
            else if (l.name === 'LOCATION' && !cur.location)
                cur.location = l.value;
        }
    }
    if (version)
    ...
    if (cur) {
        if (cur.summary)
            out['First event'] = cur.summary;
        if (cur.dtstart)
            out['Start'] = cur.dtstart;
        if (cur.dtend)
            out['End'] = cur.dtend;
        if (cur.location)
...
function vcardImage(prop, who) {
    ...
    if (dataUri) {
        if (!dataUri[2])
            return null; // percent-encoded data: URI, not base64
        if (dataUri[1])
            declared = dataUri[1];
        b64 = dataUri[3];
    }
...
async function parseVcf(file) {
    ...
    cards.forEach((card, idx) => {
        ...
        if (idx < 5) {
            ...
            if (get('ORG'))
                bits.push('   org: ' + get('ORG').replace(/;+$/, ''));
            for (const e of all('EMAIL'))
                bits.push('   email: ' + e.value);
            for (const t of all('TEL'))
                bits.push('   tel: ' + t.value);
            for (const a of all('ADR'))
                bits.push('   adr: ' + a.value.split(';').filter(Boolean).join(', '));
            if (get('BDAY'))
                bits.push('   bday: ' + get('BDAY'));
            detail.push(bits.join('\n'));
        }
        const ver = get('VERSION');
        if (ver && idx === 0)
            out['vCard version'] = ver;
        for (const prop of props) {
            if (prop.name !== 'PHOTO' && prop.name !== 'LOGO')
                continue;
            if (photos.length >= EMBEDDED_IMAGES_MAX)
                break;
    ...
    if (photos.length)
        out['Embedded pictures'] = String(photos.length);
    if (linked)
        out['Linked pictures'] = linked + ' (stored as a link, not inside this file)';
    if (photos.length) {
        try {
            const items = photos.map((ph, i) => {
                const blob = new Blob([ph.bytes], { type: ph.mime });
...
async function parseLdif(file) {
    ...
    for (const line of logical) {
        ...
        if (attr === 'dn')
            curDn = val;
        else if (attr === 'cn' && !curCn)
            curCn = val;
        else if (attr === 'mail' && !curMail)
            curMail = val;
        else if (attr === 'objectclass')
            objectClasses[val] = (objectClasses[val] || 0) + 1;
        else if (attr === 'changetype')
            changes++;
    }
    flush();
    if (!entries)
    ...
    if (changes)
        out['changetype records'] = changes;
    const ocList = Object.entries(objectClasses).sort((a, b) => b[1] - a[1]);
    out['objectClasses'] = ocList.length;
    out._sections = [];
    if (ocList.length)
        out._sections.push({ title: 'objectClass breakdown', node: preBlock(ocList.map(([k, v]) => v + '  ' + k).join('\n')) });
    if (sample.length)
        out._sections.push({ title: 'Sample entries', node: preBlock(sample.join('\n\n')), open: true });
    return out;
}
// ---------- Windows .contact (XML) ----------
async function parseContact(file) {
    const text = await readText(file, 1024 * 1024);
    if (!text || !/<c:contact|<contact/i.test(text))
        return null;
    let doc;
    try {
    ...
    const textOf = (localName) => {
        ...
        for (const e of els)
            if (e.localName === localName && e.textContent.trim())
    ...
    if (phones.length)
        out['Phone numbers'] = phones.length;
    if (bday)
        out['Birthday'] = bday;
    const sect = [];
    if (emails.length)
    ...
    if (phones.length)
        sect.push('Phones:\n' + phones.map((p) => '  ' + p).join('\n'));
    if (sect.length)
        out._sections = [{ title: 'Contact details', node: preBlock(sect.join('\n\n')), open: true }];
    return out;
}
...
function decodeMapiString(bytes, type) {
    if (!bytes || !bytes.length)
        return '';
    try {
        if (type === '001f' || type === '101f')
            return utf16(bytes, true).replace(/\0+$/, '');
...
async function parseMsg(file) {
    let cfbf;
    try {
        cfbf = await openCfbf(file);
    }
    catch (_) {
        return null;
    }
    if (!cfbf || !cfbf.rawEntries)
        return null;
    ...
    const looksLikeMsg = cfbf.rawEntries.some((e) => e.name && /^__(substg1\.0_|properties_version1\.0|recip_version1\.0_|attach_version1\.0_)/i.test(e.name));
    if (!looksLikeMsg)
        return null;
    ...
    const attachStorages = cfbf.rawEntries.filter((e) => e.type === 1 && e.name && /^__attach_version1\.0_/i.test(e.name));
    const attachNames = [];
    ...
    if (bcc)
        out['Bcc'] = bcc;
    if (sentDate)
        out['Date'] = sentDate.toLocaleString();
    if (messageId)
        out['Message-ID'] = messageId;
    out['Attachments'] = attachNames.length;
    const sections = [];
    if (headers)
        sections.push({ title: 'Transport headers', node: preBlock(headers.slice(0, 8000)) });
    if (attachNames.length)
        sections.push({ title: 'Attachments (' + attachNames.length + ')', node: preBlock(attachNames.join('\n')) });
    if (body)
        sections.push({ title: 'Body preview', node: preBlock(body.trim().slice(0, 4000)), open: true });
    if (sections.length)
...
async function parseOlm(file) {
    ...
    if (totalUncomp)
        out['Uncompressed size'] = fmtBytes(totalUncomp);
    const sections = [];
    if (folderList)
        sections.push({ title: 'Top-level entries', node: preBlock(folderList) });
    const sampleNames = msgXml.slice(0, 40).map((e) => e.name).join('\n') ||
        ents.slice(0, 40).map((e) => e.name).join('\n');
    if (sampleNames)
        sections.push({ title: 'Member sample', node: preBlock(sampleNames), open: true });
    out._sections = sections;
    return out;
}
...
async function parseP7(file, ext) {
    ...
    for (const [hex, name] of Object.entries(CMS_OIDS)) {
        ...
        for (let i = 0; i < hex.length; i += 2)
            needle.push(parseInt(hex.substr(i, 2), 16));
        if (findBytes(der, needle) >= 0) {
            contentType = name;
            if (name !== 'data')
                break;
        }
    ...
    if (contentType)
        out['CMS content type'] = CMS_OID_LABEL[contentType] || contentType;
    ...
    if (dnCount)
        out['Directory-name attributes'] = dnCount + ' (issuer/subject DN components)';
...
async function parseMsf(file) {
    ...
    if (!text)
        return null;
    if (!/<!--\s*<mdb:mork|^\/\/\s*<!--\s*<mdb:mork|BeMs/m.test(text.slice(0, 4000)) && text.indexOf('mork') < 0)
        return null;
    ...
    if (tableCount)
        out['Tables'] = tableCount.toLocaleString();
    if (rowCount)
        out['Rows (cached headers)'] = rowCount.toLocaleString();
    if (emails.length)
        out['Distinct addresses'] = emails.length.toLocaleString();
    const sections = [];
    // Cached subjects/senders are the longer textual atoms.
    const sample = texty.filter((v) => v.length >= 3).slice(0, 60).map((v) => '  ' + v).join('\n');
    if (sample)
        sections.push({ title: 'Cached strings (subjects / senders)', node: preBlock(sample), open: true });
    if (emails.length)
        sections.push({ title: 'Addresses', node: preBlock(emails.slice(0, 80).map((e) => '  ' + e).join('\n')) });
    if (sections.length)
        out._sections = sections;
    return out;
}
async function parseMab(file) {
    ...
    if (!text)
        return null;
    if (text.indexOf('mork') < 0 && !/<!--\s*<mdb:mork/.test(text.slice(0, 4000)))
        return null;
    ...
    if (rowCount)
        out['Rows (contacts/cards)'] = rowCount.toLocaleString();
    if (emails.length)
        out['Distinct emails'] = emails.length.toLocaleString();
    if (phones.length)
        out['Phone-like values'] = phones.length.toLocaleString();
    const sections = [];
    if (emails.length)
        sections.push({ title: 'Emails', node: preBlock(emails.slice(0, 80).map((e) => '  ' + e).join('\n')), open: true });
    const nameSample = names.slice(0, 60).map((v) => '  ' + v).join('\n');
    if (nameSample)
        sections.push({ title: 'Cached names / fields', node: preBlock(nameSample) });
    if (sections.length)
        out._sections = sections;
    return out;
}
...
async function parseMbx(file) {
    ...
    if (!/^From [^\n]*\r?\n/m.test(raw) && !/^[\w-]+:\s/m.test(raw.slice(0, 2000)))
        return null;
...
async function parseToc(file) {
    ...
    if (printable.length)
        out['Folder / label'] = printable[0].slice(0, 80);
...
async function parseVmg(file) {
    const text = await readText(file, 1024 * 1024);
    if (!text || !/BEGIN:VMSG/i.test(text))
        return null;
    ...
    if (sender)
        out['Sender / number'] = sender;
    if (date)
        out['Date'] = date;
    if (status)
        out['Status / type'] = status;
    ...
    if (bodyM) {
        ...
        msg = msg.replace(/^[\s\r\n]+|[\s\r\n]+$/g, '');
        if (msg)
            out._sections = [{ title: 'Message text', node: preBlock(msg.slice(0, 4000)), open: true }];
    }
    return out;
}
async function parseVnt(file) {
    const text = await readText(file, 1024 * 1024);
    if (!text || !/BEGIN:VNOTE/i.test(text))
        return null;
    const props = parseVobjectText(text);
    const get = (n) => { const p = props.find((x) => x.name === n); return p ? maybeQp(p.value, p.params) : ''; };
...
async function parseXcal(file) {
    ...
    if (tag('vjournal'))
        out['Journals'] = tag('vjournal');
    if (tag('vfreebusy'))
        out['Free/busy'] = tag('vfreebusy');
    const prod = [...doc.getElementsByTagName('*')].find((e) => e.localName === 'prodid');
    if (prod && prod.textContent.trim())
        out['Product (PRODID)'] = prod.textContent.trim();
    const summaries = [...doc.getElementsByTagName('*')].filter((e) => e.localName === 'summary').map((e) => e.textContent.trim()).filter(Boolean).slice(0, 20);
    if (summaries.length)
        out._sections = [{ title: 'Event summaries', node: preBlock(summaries.map((s) => '  ' + s).join('\n')), open: true }];
    return out;
}
    ...
    const walk = (comp) => {
        ...
        if (Array.isArray(props))
            for (const p of props)
                if (Array.isArray(p) && p[0] === 'summary' && summaries.length < 20)
                    summaries.push(String(p[3]));
...
async function parseXcard(file) {
    const text = await readText(file, 4 * 1024 * 1024);
    if (!text || !/<vcards\b|<vcard\b/i.test(text))
        return null;
    let doc;
    try {
        doc = new DOMParser().parseFromString(text, 'application/xml');
    ...
    cards.slice(0, 5).forEach((card, i) => {
        ...
        for (const e of allText('email'))
            bits.push('   email: ' + e);
        for (const t of allText('tel'))
            bits.push('   tel: ' + t);
        detail.push(bits.join('\n'));
    });
    out._sections = [{ title: 'First cards', node: preBlock(detail.join('\n\n')), open: true }];
...
//# sourceMappingURL=parsers-email.js.map