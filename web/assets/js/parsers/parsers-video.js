...
function parseHls(text, ext) {
    ...
    if (!isHls)
        return null; // basic .m3u handled elsewhere
    ...
    if (isMaster) {
        ...
        if (audioGroups)
            out['Alternate audio renditions'] = audioGroups;
        if (subGroups)
            out['Subtitle renditions'] = subGroups;
        const bws = variants.map((v) => v.bw).filter(Boolean);
        if (bws.length)
            out['Bandwidth range'] = Math.round(Math.min(...bws) / 1000) + ' - ' + Math.round(Math.max(...bws) / 1000) + ' kbps';
        if (variants.length) {
            const rows = variants.sort((a, b) => a.bw - b.bw).map((v) => {
                ...
                if (v.res)
                    parts.push(v.res);
                if (v.bw)
                    parts.push(Math.round(v.bw / 1000) + ' kbps');
                if (v.fr)
                    parts.push(v.fr + ' fps');
                if (v.codecs)
                    parts.push(v.codecs);
...
function parseDash(text) {
    ...
    for (const a of adaptSets) {
        ...
        if (/video/.test(ct))
            ...
            audio++;
        else if (/text|application/.test(ct))
            text2++;
        const lng = a.getAttribute('lang');
        if (lng)
            langs.add(lng);
    }
    out['Tracks'] = video + ' video, ' + audio + ' audio' + (text2 ? ', ' + text2 + ' text' : '');
    if (langs.size)
        out['Languages'] = Array.from(langs).join(', ');
    ...
    if (cps.length) {
        ...
        for (const cp of cps) {
            ...
            if (scheme.includes('edef8ba9'))
                systems.add('Widevine');
            else if (scheme.includes('9a04f079'))
                systems.add('PlayReady');
            else if (scheme.includes('94ce86fb'))
                systems.add('FairPlay');
            else if (scheme.includes('mp4protection'))
                systems.add('Common Encryption (cenc)');
            else {
                const v = cp.getAttribute('value');
                if (v)
                    systems.add(v);
            }
        }
    ...
    const lines = reps.slice(0, 60).map((r) => {
        ...
        if (w && h)
            parts.push(w + 'x' + h);
        if (bw)
            parts.push(Math.round(parseInt(bw, 10) / 1000) + ' kbps');
        if (fr)
            parts.push((/\//.test(fr) ? (eval2(fr)) : fr) + ' fps');
        if (codec)
            parts.push(codec);
        return parts.join('  ');
    });
    if (lines.length)
        out._sections = [{ title: 'Representations (' + reps.length + ')', node: preBlock(lines.join('\n')), open: true }];
    return out;
}
...
function parseSmooth(text) {
    ...
    if (!ssm && !/SmoothStreamingMedia|<smil/i.test(text))
        return null;
    ...
    if (ssm) {
        ...
        for (const si of indexes) {
            ...
            const lng = si.getAttribute('Language');
            if (lng)
                langs.add(lng);
            ...
            for (const q of qualities.slice(0, 12)) {
                ...
                if (w && h)
                    parts.push(w + 'x' + h);
                if (br)
                    parts.push(Math.round(parseInt(br, 10) / 1000) + ' kbps');
                if (fourcc)
                    parts.push(fourcc);
        ...
        if (lines.length)
            out._sections = [{ title: 'Quality levels', node: preBlock(lines.join('\n')), open: true }];
    }
    else {
        ...
        if (srcs.length)
            out._sections = [{ title: 'Sources', node: preBlock(srcs.join('\n')) }];
    }
    return out;
}
// ---------- Adobe HDS .f4m ----------
function parseF4m(text) {
    ...
    if (id)
        out['Stream ID'] = id.textContent.trim();
    const dur = doc.querySelector('duration');
    if (dur) {
        const d = parseFloat(dur.textContent);
        if (d)
            out['Duration'] = fmtDuration(d);
    }
    const stype = doc.querySelector('streamType');
    if (stype)
        out['Stream type'] = stype.textContent.trim();
    ...
    if (lines.length)
        out._sections = [{ title: 'Media renditions', node: preBlock(lines.join('\n')) }];
    return out;
}
...
function parsePlaylist(text, ext) {
    if (ext === 'pls') {
        ...
        if (lines.length)
            out._sections = [{ title: 'Entries', node: preBlock(lines.join('\n')) }];
        return out;
    }
    ...
    if (ext === 'asx') {
        ...
        if (titles.length)
            out['Titles'] = titles.slice(0, 5).join(' | ') + (titles.length > 5 ? ' …' : '');
        if (refs.length)
            out._sections = [{ title: 'Stream URLs', node: preBlock(refs.join('\n')) }];
        return out;
    }
    const doc = parseXml(text);
    if (ext === 'wpl') {
        if (!doc || !/<smil/i.test(text))
            return null;
        ...
        if (media.length)
            out._sections = [{ title: 'Tracks', node: preBlock(media.join('\n')) }];
        return out;
    }
    if (ext === 'xspf') {
        if (!doc || !/<playlist/i.test(text))
            return null;
        ...
        if (lines.length)
            out._sections = [{ title: 'Tracks', node: preBlock(lines.join('\n')) }];
        return out;
    }
    return null;
}
...
async function parseMxf(file) {
    ...
    const STATUS = { 1: 'open, incomplete', 2: 'closed, incomplete', 3: 'open, complete', 4: 'closed, complete' };
    ...
    if (findBytes(head, hex([0x04, 0x01, 0x02, 0x02, 0x01])) >= 0)
        codecHints.push('MPEG');
    if (findBytes(head, hex([0x0A, 0x0E, 0x10, 0x00])) >= 0 || findBytes(head, hex([0x6A, 0x70, 0x65, 0x67])) >= 0)
        codecHints.push('JPEG2000');
...
async function parseGxf(file) {
    ...
    if (!(head[0] === 0 && head[1] === 0 && head[2] === 0 && head[3] === 0 && head[4] === 0x01))
        return null;
...
async function parseDv(file) {
    ...
    const sct = head[0] >> 5;
    if (sct !== 0)
        return null; // not a header section
...
async function parseAsf(file, ext) {
    ...
    if (fpIdx >= 0) {
        ...
        if (seconds > 0)
            out['Duration'] = fmtDuration(seconds);
        if (maxBitrate)
            out['Max bitrate'] = Math.round(maxBitrate / 1000) + ' kbps';
    ...
    for (let n = 0; n < numObjects && pos + 24 <= head.length; n++) {
        ...
        if (objSize <= 0 || objSize > headerSize)
            break;
        if (matchMagic(guid, ASF_STREAM_PROPS, 0)) {
            ...
            let kind = 'unknown';
            if (matchMagic(stType, ASF_VIDEO_MEDIA, 0))
                kind = 'video';
            else if (matchMagic(stType, ASF_AUDIO_MEDIA, 0))
                kind = 'audio';
            streams.push(kind);
        }
        pos += objSize;
    ...
    if (ext === 'dvr-ms')
        out['Note'] = 'Windows Media Center recording (ASF with DVR extension objects).';
    return out;
}
// ---------- RealMedia .rm / .rmvb ----------
async function parseReal(file) {
    const head = await readSlice(file, 0, Math.min(file.size, 65536));
    if (ascii(head, 0, 4) !== '.RMF')
        return null;
    ...
    try {
        while (pos + 8 <= head.length) {
            ...
            if (id === 'PROP') {
                ...
                if (avgBitrate)
                    out['Average bitrate'] = Math.round(avgBitrate / 1000) + ' kbps';
                if (duration)
                    out['Duration'] = fmtDuration(duration / 1000);
                if (numPackets)
                    out['Packets'] = numPackets.toLocaleString();
            }
            else if (id === 'MDPR') {
                // Media properties: stream name, mime type, type-specific data with FourCC.
                const mr = new Reader(head, false);
                mr.seek(pos + 10);
                ...
                let codec = null;
                if (/VIDORV/.test(bs))
                    codec = 'RealVideo (' + (bs.match(/VIDORV(\w+)/) || [])[0] + ')';
                else if (/RV40|RV30|RV20|RV10/.test(bs))
                    codec = 'RealVideo ' + (bs.match(/RV\d0/) || [])[0];
                else if (/cook|raac|sipr|atrc|dnet|RAAC|COOK/i.test(bs))
                    codec = 'RealAudio (' + (bs.match(/cook|raac|sipr|atrc|dnet/i) || [])[0] + ')';
                streams.push(codec || 'stream');
            }
            else if (id === 'CONT') {
                // Content description: title/author/copyright/comment (length-prefixed).
                try {
                    const cr = new Reader(head, false);
                    cr.seek(pos + 10);
    ...
    if (streams.length)
        out['Streams'] = streams.length + ': ' + streams.join(', ');
...
function readMp4Boxes(buf) {
    ...
    while (r.pos + 8 <= buf.length) {
        ...
        if (size < 8 || start + size > buf.length + 0x7fffffff)
            break;
        if (type === 'ftyp')
            ftyp = ascii(buf, start + 8, Math.min(size - 8, 4));
...
function mp4TrackInfo(buf, moovStart, moovSize) {
    ...
    if (mvhd >= 0) {
        ...
        if (ver === 1) {
            r.skip(16);
            info.timescale = r.u32();
            info.durationSec = Number(r.u64()) / (info.timescale || 1);
        }
        else {
            r.skip(8);
            info.timescale = r.u32();
            info.durationSec = r.u32() / (info.timescale || 1);
        }
    }
...
async function parseMp4Wrapper(file, label, note) {
    ...
    if (!top.length || !top.some((b) => b.type === 'ftyp'))
        return null;
    ...
    if (moov) {
        ...
        if (info.width && info.height)
            out['Resolution'] = info.width + ' x ' + info.height;
        if (info.durationSec)
            out['Duration'] = fmtDuration(info.durationSec);
        if (info.codecs.length)
            out['Codecs'] = Array.from(new Set(info.codecs)).join(', ');
        if (info.durationSec && file.size)
            out['Overall bitrate'] = Math.round((file.size * 8) / info.durationSec / 1000) + ' kbps';
    }
    else {
...
async function parseDivx(file) {
    ...
    if (avih >= 0) {
        ...
        if (usPerFrame) {
            ...
            out['Frame rate'] = (Math.round(fps * 100) / 100) + ' fps';
            if (totalFrames)
                out['Duration'] = fmtDuration(totalFrames / fps);
        }
        if (totalFrames)
            out['Total frames'] = totalFrames.toLocaleString();
        out['Streams'] = streams;
    }
...
async function parseIvf(file) {
    ...
    if (rateDen) {
        ...
        out['Frame rate'] = (Math.round(fps * 1000) / 1000) + ' fps';
        out['Header time base'] = rateNum + '/' + rateDen;
        if (frameCount)
            out['Duration'] = fmtDuration(frameCount / fps);
    }
    if (frameCount)
        out['Frame count'] = frameCount.toLocaleString();
...
async function parseY4m(file) {
    ...
    if (ascii(head, 0, 9) !== 'YUV4MPEG2')
        return null;
    // Header is one ASCII line ending in 0x0A.
    let end = 9;
    while (end < head.length && head[end] !== 0x0A)
        end++;
    ...
    if (fr[1]) {
        const fps = +fr[1] / (+fr[2] || 1);
        out['Frame rate'] = (Math.round(fps * 1000) / 1000) + ' fps (' + fr[1] + ':' + fr[2] + ')';
    }
    const il = (line.match(/\sI(\w)/) || [])[1];
    const ilMap = { p: 'Progressive', t: 'Top-field-first', b: 'Bottom-field-first', m: 'Mixed' };
    if (il)
        out['Interlacing'] = ilMap[il] || il;
    const ar = (line.match(/\sA(\d+):(\d+)/) || []);
    if (ar[1])
        out['Pixel aspect ratio'] = ar[1] + ':' + ar[2];
    const cs = (line.match(/\sC(\S+)/) || [])[1];
    if (cs)
        out['Chroma subsampling'] = cs;
    return out;
}
...
async function parseMpegVideo(file, ext) {
    ...
    if (MPEG_FR[frCode])
        out['Frame rate'] = MPEG_FR[frCode] + ' fps';
    if (bitrate && bitrate !== 0x3FFFF)
        out['Bitrate'] = Math.round(bitrate * 400 / 1000) + ' kbps';
...
class BitReader {
    ...
    bits(n) { let v = 0; for (let i = 0; i < n; i++)
        v = (v << 1) | this.bit(); return v >>> 0; }
    ue() { let zeros = 0; while (this.pos < this.b.length * 8 && this.bit() === 0)
        zeros++; let v = 0; for (let i = 0; i < zeros; i++)
        v = (v << 1) | this.bit(); return v + (1 << zeros) - 1; }
...
function findNals(buf, hevc) {
    ...
    for (let k = 0; k < nals.length; k++)
        nals[k].end = k + 1 < nals.length ? nals[k + 1].start - 3 : buf.length;
    return nals;
}
    ...
    if ([100, 110, 122, 244, 44, 83, 86, 118, 128].includes(profileIdc)) {
        ...
        if (br.bit()) { // seq_scaling_matrix_present_flag
            for (let i = 0; i < (chroma !== 3 ? 8 : 12); i++) {
                if (!br.bit())
                    continue; // seq_scaling_list_present_flag[i]
...
async function parseH265(file) {
    ...
    for (let i = 0; i < maxSubLayersM1; i++) {
        subProfile.push(br.bit());
        subLevel.push(br.bit());
    }
    if (maxSubLayersM1 > 0)
        for (let i = maxSubLayersM1; i < 8; i++)
            br.bits(2);
    for (let i = 0; i < maxSubLayersM1; i++) {
        if (subProfile[i])
            br.bits(88);
    ...
    while (pos + 1 < buf.length && guard++ < 32) {
        ...
        if (obuType === 1) {
            ...
            if (reducedHdr) {
                ...
                if (timingInfo) {
                    sb.bits(32);
                    sb.bits(32); // num_units_in_display_tick, time_scale
                    if (sb.bit()) { // equal_picture_interval: uvlc num_ticks_per_picture_minus_1
                        let lz = 0;
                        while (lz < 32 && sb.bit() === 0)
                            lz++;
                        if (lz < 32)
                            sb.bits(lz);
                    }
                    ...
                    if (decoderModel) {
                        ...
                        sb.bits(5);
                        sb.bits(5); // buffer_removal_time / frame_presentation_time lengths
                    }
                }
    ...
    for (const [sz, lead] of [[188, 0], [192, 4], [204, 0]]) {
        ...
        for (let o = lead; o <= limit; o++) {
            if (buf[o] !== 0x47)
                continue;
            let runs = 1;
            while (o + runs * sz < buf.length && buf[o + runs * sz] === 0x47 && runs < 4)
                runs++;
...
async function parseMpegPs(file, ext) {
    ...
    for (let i = packIdx; i + 4 < buf.length && i < 262144; i++) {
        if (buf[i] === 0 && buf[i + 1] === 0 && buf[i + 2] === 1) {
            const id = buf[i + 3];
            if (id >= 0xC0 && id <= 0xDF)
                streams.add('Audio (MPEG)');
            else if (id >= 0xE0 && id <= 0xEF)
                streams.add('Video (MPEG)');
            else if (id === 0xBD)
                streams.add('Private 1 (AC-3/DTS/subtitles)');
            else if (id === 0xBF)
                streams.add('Private 2');
        }
    }
    if (streams.size)
        out['Elementary streams'] = Array.from(streams).join(', ');
    return out;
}
// ============================================================================
...
async function parseWtv(file) {
    ...
    if (channel)
        out['Channel'] = channel.slice(0, 60);
    if (desc)
        out['Description'] = desc.slice(0, 200);
...
async function parseOgm(file) {
    ...
    while (pos + 27 < buf.length && pages < 200) {
        if (ascii(buf, pos, 4) !== 'OggS')
            break;
        const segCount = buf[pos + 26];
        let pageDataLen = 0;
        for (let i = 0; i < segCount; i++)
            pageDataLen += buf[pos + 27 + i];
        ...
        if (/video/.test(tag) || /\x01video/.test(latin1(data))) {
            video++;
            const cc = ascii(data, 9, 4);
            if (cc)
                codecs.add(cc);
        }
        else if (/audio/.test(tag)) {
            audio++;
        }
        else if (/text/.test(tag)) {
            text2++;
        }
        else if (/vorbis/.test(latin1(data))) {
            audio++;
            codecs.add('Vorbis');
        }
        else if (/theora/.test(latin1(data))) {
            video++;
            codecs.add('Theora');
        }
    ...
    if (codecs.size)
        out['Codecs'] = Array.from(codecs).join(', ');
...
async function parseCin(file) {
    ...
    if (!(head[0] === 0x80 && head[1] === 0x2A && head[2] === 0x5F && head[3] === 0xD7))
        return null;
    return {
...
//# sourceMappingURL=parsers-video.js.map