...
function uleb(b, cur) { let r = 0, sh = 0, x; do {
    x = b[cur.i++];
    r += (x & 0x7f) * Math.pow(2, sh);
    sh += 7;
} while (x & 0x80); return r; }
// ---------- JSON Web Token ----------
async function parseJwt(file) {
    ...
    if (header.alg)
        out['Algorithm'] = header.alg;
    if (header.typ)
        out['Header typ'] = header.typ;
    if (header.kid)
        out['Key ID (kid)'] = header.kid;
    ...
    if (payload) {
        ...
        for (const [k, label] of Object.entries(map))
            if (payload[k] != null)
                out[label] = Array.isArray(payload[k]) ? payload[k].join(', ') : String(payload[k]);
        for (const [k, label] of [['iat', 'Issued at'], ['nbf', 'Not before'], ['exp', 'Expires']]) {
            if (typeof payload[k] === 'number')
                out[label] = new Date(payload[k] * 1000).toLocaleString();
        }
        if (typeof payload.exp === 'number')
            out['Status'] = (payload.exp * 1000 < Date.now()) ? 'EXPIRED' : 'valid';
        for (const [k, v] of Object.entries(payload))
            claims.push([k, typeof v === 'object' ? JSON.stringify(v) : String(v)]);
    }
    const warn = [];
    if (String(header.alg).toLowerCase() === 'none')
        warn.push('alg: none - token is unsigned (accept with caution)');
    if (out['Status'] === 'EXPIRED')
        warn.push('Token is expired');
    if (warn.length)
        out['⚠ Warning'] = warn.join('; ');
    const sections = [{ title: 'Header', node: preBlock(JSON.stringify(header, null, 2)) }];
    if (payload)
        sections.push({ title: 'Payload claims (' + claims.length + ')', node: preBlock(JSON.stringify(payload, null, 2)), open: true });
...
async function parseHar(file) {
    ...
    let j;
    try {
        j = JSON.parse(await readText(file, SCAN_LARGE));
    }
    catch (_) {
        return null;
    }
    ...
    if (log.creator)
        out['Creator'] = (log.creator.name || '') + ' ' + (log.creator.version || '');
    ...
    for (const e of entries) {
        ...
        const ct = ((r.content && r.content.mimeType) || '').split(';')[0];
        if (ct)
            types[ct] = (types[ct] || 0) + 1;
        const hdrs = ((e.request && e.request.headers) || []).concat((r.headers) || []);
        if (hdrs.some((h) => /^(authorization|cookie|set-cookie)$/i.test(h.name || '')))
            secrets++;
    }
    out['Total content size'] = fmtBytes(bytes);
    out['Slow requests (>1s)'] = slow;
    if (secrets)
        out['⚠ Auth/cookie headers'] = secrets + ' request(s) carry credentials';
...
async function parseIpynb(file) {
    ...
    if (ks.display_name || ks.name)
        out['Kernel'] = ks.display_name || ks.name;
    if (li.name)
        out['Language'] = li.name + (li.version ? ' ' + li.version : '');
    const counts = {};
    let codeLines = 0, outputs = 0;
...
async function parseJsonl(file) {
    ...
    if (cut)
        lines.pop(); // the last line is probably cut mid-record
    let valid = 0;
    const keys = new Set();
...
async function parseWasm(file) {
    ...
    try {
        while (cur.i < b.length) {
            ...
            if (id === 0) { // Custom section - grab its name
                const c2 = { i: cur.i };
                const nlen = uleb(b, c2);
                const nm = ascii(b, c2.i, nlen);
                if (/producers|name/.test(nm))
                    producer = nm;
            }
            cur.i = end;
            if (found.length > 200)
    ...
    if (secCounts['Import'] != null)
        out['Imports'] = secCounts['Import'];
    if (secCounts['Function'] != null)
        out['Functions'] = secCounts['Function'];
    if (secCounts['Export'] != null)
        out['Exports'] = secCounts['Export'];
    out['Sections'] = found.join(', ');
    if (producer)
        out['Custom section'] = producer;
    return out;
}
// ---------- Java .class ----------
...
async function parseNpy(file) {
    ...
    if (!(b[0] === 0x93 && b[1] === 0x4e && b[2] === 0x55 && b[3] === 0x4d && b[4] === 0x50 && b[5] === 0x59))
        return null;
    const major = b[6];
...
async function parseSafetensors(file) {
    ...
    if (n <= 0 || n > 100_000_000 || n + 8 > file.size)
        return null;
    let meta;
    try {
        meta = JSON.parse(new TextDecoder('utf-8').decode(new Uint8Array(await file.slice(8, 8 + n).arrayBuffer())));
    }
    catch (_) {
        return null;
    }
    ...
    if (meta.__metadata__)
        out._sections = [{ title: 'Metadata', node: preBlock(JSON.stringify(meta.__metadata__, null, 2)) }];
    return out;
}
// ---------- GGUF (llama.cpp) ----------
...
async function parseSql(file) {
    ...
    if (/ENGINE=|AUTO_INCREMENT|`/.test(text))
        dialect = 'MySQL / MariaDB';
    else if (/SERIAL\b|pg_catalog|OWNER TO|::|^COPY\s+/im.test(text))
        dialect = 'PostgreSQL';
    else if (/PRAGMA|sqlite_sequence|AUTOINCREMENT/.test(text))
        dialect = 'SQLite';
    else if (/\bGO\s*$|nvarchar|\[dbo\]|IDENTITY\(/im.test(text))
    ...
    while ((m = reTable.exec(text)) && tables.length < 300 && attempts++ < 1000) {
        ...
        if (close < 0 || !reTail.test(text.slice(close + 1, close + 64)))
            continue;
...
async function parseDotnetProj(file) {
    ...
    if (pkgs.length)
        out._sections = [{ title: 'NuGet packages', node: preBlock(pkgs.join('\n')) }];
    return out;
}
// ---------- Gradle build ----------
...
async function parseTerraform(file, ext) {
    ...
    if (ext === 'tfstate') {
        ...
        for (const r of (j.resources || []))
            byType[r.type] = (byType[r.type] || 0) + (r.instances ? r.instances.length : 1);
        return {
...
async function parseSarif(file) {
    ...
    for (const r of j.runs) {
        const td = r.tool && r.tool.driver;
        if (td)
            tools.add(td.name + (td.version ? ' ' + td.version : ''));
        for (const res of (r.results || [])) {
            results++;
            const s = res.level || 'none';
            sev[s] = (sev[s] || 0) + 1;
        }
    }
...
async function parsePlistRows(file) {
    ...
    let json;
    try {
        json = JSON.stringify(v, (key, val) => (val instanceof Uint8Array ? '<' + val.length + ' bytes>' : val), 2);
    }
    catch (_) {
        json = null;
    }
    if (json)
        out._sections = [{ title: 'Contents', node: preBlock(json.length > 20000 ? json.slice(0, 20000) + '\n…' : json) }];
    return out;
}
...
async function parseLock(file, ext, name) {
    ...
    if (fname === 'package-lock.json' || (ext === 'json' && /"lockfileVersion"/.test(text))) {
        ...
        if (j) {
            ...
            if (j.packages)
                deps = Object.keys(j.packages).filter((k) => k).length; // v2/v3: keys are paths, '' = root
            else if (j.dependencies)
                deps = Object.keys(j.dependencies).length; // v1
        }
    }
        ...
        if (j)
            deps = (j.packages || []).length + (j['packages-dev'] || []).length;
    }
        ...
        if (j && j.nodes)
            deps = Object.keys(j.nodes).length - 1; // minus the synthetic root node
    }
    const out = { 'Format': 'Dependency lockfile' + (truncated ? ' (first 8 MB scanned)' : '') };
    out['Ecosystem'] = ecosystem;
    if (lockVer != null)
        out['Lockfile version'] = String(lockVer);
    if (deps != null)
        out['Locked packages'] = deps.toLocaleString();
    const hasIntegrity = /integrity|sha512-|sha256:|narHash|checksum\s*=/.test(text);
    if (hasIntegrity)
        out['Integrity hashes'] = 'present';
    return out;
}
...
async function parseMsgpack(file) {
    ...
    function val() {
        ...
        if (c >= 0x80 && c <= 0x8f) {
            const len = c & 0x0f;
            for (let i = 0; i < len && !r.eof && n <= 20000; i++) {
                val();
                val();
            }
            return;
    ...
    if (hist.length)
        out._sections = [{ title: 'Type histogram', node: preBlock(hist.join('\n')) }];
    return out;
}
// ---------- CBOR ----------
async function parseCbor(file) {
    ...
    function val() {
        ...
        if (major === 2 || major === 3) {
            if (len >= 0)
                ...
                while (!r.eof && n <= 20000 && r.bytes[r.pos] !== 0xff)
                    val();
            if (len < 0)
                r.skip(1);
            return;
    ...
    if (isSelfDesc)
        out['Self-describe tag'] = 'present (0xd9d9f7)';
    const hist = Object.entries(counts).sort((a, b) => b[1] - a[1]).map(([k, v]) => k + ' (' + v + ')');
    const tagList = Object.entries(tags).sort((a, b) => b[1] - a[1]).map(([k, v]) => 'tag ' + k + ' (' + v + ')');
    const secs = [];
    if (hist.length)
        secs.push({ title: 'Major-type histogram', node: preBlock(hist.join('\n')) });
    if (tagList.length)
        secs.push({ title: 'Tags', node: preBlock(tagList.join('\n')) });
    if (secs.length)
        out._sections = secs;
    return out;
}
...
async function parseBson(file) {
    ...
    if (docLen < 5 || (docLen > file.size && docLen > b.length))
        return null;
    r.seek(4);
    const types = {};
    const keys = [];
    ...
    if (keys.length)
        out._sections = [{ title: 'Elements', node: preBlock(keys.join('\n')) }];
    return out;
}
...
function pbWalk(b, start, end, depth, lines, stats) {
    ...
    while (cur.i < end && stats.fields < 4000) {
        ...
        if (wire === 0) {
            ...
            let printable = 0;
            for (let i = s; i < Math.min(e, s + 64); i++) {
                const c = b[i];
            ...
            if (depth < 6 && len > 1 && ratio < 0.85) {
                ...
                if (lines.length < 400)
                    lines.push('  '.repeat(depth) + 'field ' + field + ' { (nested ' + len + 'B)');
                const ok = pbWalk(b, s, e, depth + 1, lines, stats);
                if (lines.length < 400)
                    lines.push('  '.repeat(depth) + '}');
                if (!ok) {
                    stats.fields = before;
                    detail = 'len=' + len + (ratio >= 0.5 ? ' "' + ascii(b, s, Math.min(len, 40)) + '"' : ' (bytes)');
                    cur.i = e;
                }
                else {
                    cur.i = e;
        ...
        if (lines.length < 400 && wire !== 2)
            lines.push('  '.repeat(depth) + 'field ' + field + ' ' + WIRE[wire] + ' ' + detail);
        else if (lines.length < 400 && wire === 2 && detail)
            lines.push('  '.repeat(depth) + 'field ' + field + ' ' + detail);
    }
    return true;
}
async function parsePb(file, ext) {
    ...
    if (!b.length)
        return null;
    const lines = [];
    const stats = { fields: 0, wires: {} };
    const ok = pbWalk(b, 0, b.length, 0, lines, stats);
    if (!ok || stats.fields === 0)
        return null;
    const out = {
    ...
    if (ext === 'desc') {
        ...
        if (protos.length)
            out['.proto files'] = protos.length;
        out._sections = [];
        if (protos.length)
            out._sections.push({ title: 'Referenced .proto files', node: preBlock(protos.join('\n')) });
...
async function parsePickle(file) {
    ...
    let proto = 0;
    if (b[0] === 0x80)
        proto = b[1]; // PROTO opcode carries the version
    ...
    const line = () => {
        let s = '';
        while (cur.i < b.length && b[cur.i] !== 0x0a && s.length < 256)
            s += String.fromCharCode(b[cur.i++]);
        while (cur.i < b.length && b[cur.i] !== 0x0a)
            cur.i++;
        cur.i++;
        return s;
    };
    const str = (n) => { const s = new TextDecoder('utf-8').decode(b.subarray(cur.i, Math.min(b.length, cur.i + Math.min(n, 256)))); cur.i += n; return s; };
    ...
    const addGlobal = (g) => { if (globals.length < 60)
        globals.push(g); };
    ...
    try {
        while (cur.i < b.length && scanned < 50000) {
            ...
            if (op === 0x63 || op === 0x69) { // GLOBAL / INST: 'module\nname\n'
                ...
                break; // not a pickle opcode: stop rather than guess
        }
    }
    catch (_) { }
    const out = {
    ...
    if (globals.length)
        out['GLOBAL imports'] = globals.length;
    const secs = [];
    if (globals.length)
        secs.push({ title: 'GLOBAL imports (module.name)', node: preBlock(Array.from(new Set(globals)).join('\n')), open: true });
    const hist = Object.entries(opHist).sort((a, b) => b[1] - a[1]).slice(0, 30).map(([k, v]) => k + ' (' + v + ')');
    if (hist.length)
        secs.push({ title: 'Opcode histogram', node: preBlock(hist.join('\n')) });
    if (secs.length)
        out._sections = secs;
    return out;
}
...
async function parseJavaArchive(file, ext) {
    ...
    for (const e of classes) {
        ...
        if (seg)
            pkgs.add(seg.replace(/\.class$/i, ''));
    }
    if (pkgs.size)
        out['Top packages'] = pkgs.size;
    const manifest = await zip.text('META-INF/MANIFEST.MF');
    const secs = [];
    if (manifest) {
        const get = (k) => (manifest.match(new RegExp('^' + k + ':\\s*(.+)$', 'mi')) || [])[1];
        const main = get('Main-Class');
        if (main)
            out['Main-Class'] = main.trim();
        const sbc = get('Start-Class');
        if (sbc)
            out['Spring Boot Start-Class'] = sbc.trim();
        const jdk = get('Build-Jdk') || get('Build-Jdk-Spec');
        if (jdk)
            out['Build-Jdk'] = jdk.trim();
        const ver = get('Implementation-Version') || get('Bundle-Version');
        if (ver)
            out['Version'] = ver.trim();
        if (/Spring-Boot-Version/i.test(manifest))
            out['Spring Boot'] = (get('Spring-Boot-Version') || 'yes').trim();
        secs.push({ title: 'MANIFEST.MF', node: preBlock(manifest.slice(0, 4000)) });
    }
    if (zip.entries.some((e) => /^META-INF\/.*\.(RSA|DSA|EC|SF)$/i.test(e.name)))
        out['Signed'] = 'yes (signature in META-INF)';
    if (pkgs.size)
        secs.unshift({ title: 'Top-level packages', node: preBlock(Array.from(pkgs).slice(0, 80).join('\n')) });
    if (secs.length)
        out._sections = secs;
    return out;
}
...
async function parseHcl(file) {
    ...
    for (const m of text.matchAll(/^\s*([a-zA-Z_]\w*)\s+(?:"[^"]*"\s*)*\{/gm))
        blocks[m[1]] = (blocks[m[1]] || 0) + 1;
    const total = Object.values(blocks).reduce((a, b) => a + b, 0);
    let tool = 'Generic HCL';
    if (/\bresource\s+"|\bprovider\s+"|\bterraform\s*\{/.test(text))
        tool = 'Terraform';
    else if (/\bjob\s+"|\btask\s+"|\bgroup\s+"/.test(text))
        tool = 'Nomad';
    else if (/\bpath\s+"|\bsecret\s+"/.test(text))
        tool = 'Vault / Consul';
    else if (/\bbuild\s*\{|\bsource\s+"/.test(text))
        tool = 'Packer';
    const out = {
    ...
    if (list.length)
        out._sections = [{ title: 'Block types', node: preBlock(list.join('\n')) }];
    return out;
}
// ---------- MATLAB MAT-file ----------
async function parseMat(file) {
    ...
    if (/HDF5/i.test(headText))
        out['Format'] = 'MATLAB MAT-file v7.3 (HDF5)';
    return out;
}
// ---------- Redis RDB dump ----------
async function parseRdb(file) {
    const b = new Uint8Array(await file.slice(0, 256).arrayBuffer());
    if (ascii(b, 0, 5) !== 'REDIS')
        return null;
    ...
    function rstr() {
        ...
        if (type === 0)
            ...
            len = ((first & 0x3f) << 8) | b[cur.i++];
        }
        else if (type === 2) {
            len = (b[cur.i] << 24) | (b[cur.i + 1] << 16) | (b[cur.i + 2] << 8) | b[cur.i + 3];
            cur.i += 4;
        }
        else {
            cur.i += 1;
            return '(int-encoded)';
        } // special encoding, skip 1 byte approx
        const s = ascii(b, cur.i, Math.min(len, 64));
        cur.i += len;
        return s;
    }
    try {
        ...
        while (cur.i < b.length && guard++ < 50) {
            ...
            if (op === 0xfa) {
                ...
                const k = rstr();
                const v = rstr();
                if (k != null && aux.length < 20)
                    aux.push(k + ' = ' + v);
            }
            else if (op === 0xfe) {
                cur.i++;
                dbs++;
                const num = b[cur.i] < 0xc0 ? (b[cur.i] & 0x3f) : 0;
                cur.i++;
            }
            else
                break;
    ...
    if (aux.length)
        out['Aux fields'] = aux.length;
    if (aux.length)
        out._sections = [{ title: 'Aux metadata', node: preBlock(aux.join('\n')) }];
    return out;
}
...
async function parseParquet(file) {
    ...
    if (ascii(head, 0, 4) !== 'PAR1' && ascii(head, 0, 4) !== 'PARE')
        return null;
...
async function parseOrc(file) {
    ...
    if (tailMagic !== 'ORC' && headMagic !== 'ORC')
        return null;
...
async function parsePowerShell(file, ext) {
    ...
    if (psVer)
        out['Min PowerShell'] = psVer;
    if (/#Requires\s+-RunAsAdministrator/i.test(text))
        out['Elevation'] = 'Requires administrator';
    const reqModules = [...text.matchAll(/#Requires\s+-Modules\s+([^\r\n]+)/gi)].map((m) => m[1].trim());
    if (reqModules.length)
        out['Required modules'] = reqModules.join('; ');
    ...
    if (synMatch) {
        ...
        if (syn)
            out['Synopsis'] = syn.length > 200 ? syn.slice(0, 200) + '…' : syn;
    }
    // Functions, parameters, advanced-function marker.
    const funcs = [...text.matchAll(/^\s*function\s+([A-Za-z_][\w-]*)/gim)].map((m) => m[1]);
    if (funcs.length)
        out['Functions'] = String(funcs.length);
    const paramAttrs = (text.match(/\[Parameter[^\]]*\]/gi) || []).length;
    if (paramAttrs)
        out['Parameters'] = String(paramAttrs);
    if (/\[CmdletBinding\s*\(/i.test(text))
        out['Advanced function'] = 'Yes (CmdletBinding)';
    if (/#\s*SIG\s*#\s*Begin signature block/i.test(text))
        out['Digitally signed'] = 'Yes (Authenticode)';
    // .psd1 manifest well-known keys.
    if (ext === 'psd1') {
        const key = (k) => (text.match(new RegExp(k + "\\s*=\\s*'([^']+)'", 'i')) || [])[1];
        const mv = key('ModuleVersion');
        if (mv)
            out['Module version'] = mv;
        const au = key('Author');
        if (au)
            out['Author'] = au;
        const rm = key('RootModule') || key('ModuleToProcess');
        if (rm)
            out['Root module'] = rm;
        const gd = key('GUID');
        if (gd)
...
async function parseBatch(file, ext) {
    ...
    for (const l of text.replace(/\r/g, '').split('\n')) {
        ...
        if (m) {
            const d = m[1].trim();
            out['Description'] = d.length > 160 ? d.slice(0, 160) + '…' : d;
            break;
        }
    }
    out['Echo'] = /^\s*@?echo\s+off\b/im.test(text) ? 'Off' : 'On';
    if (/^\s*setlocal\b/im.test(text))
        out['Setlocal'] = 'Yes';
    const labels = [...text.matchAll(/^\s*:([A-Za-z0-9_.-]+)/gm)].map((m) => m[1]).filter((l) => l.toLowerCase() !== 'eof');
    if (labels.length)
        out['Labels'] = String(labels.length);
    const sets = (text.match(/^\s*set\s+["/]?\w/gim) || []).length;
    if (sets)
        out['Variables set'] = String(sets);
    if (/%ERRORLEVEL%|\berrorlevel\b/i.test(text))
        out['Checks errorlevel'] = 'Yes';
...
async function parseTypeScript(file, ext) {
    ...
    out['Format'] = /\.d\.[cm]?ts$/.test(base)
        ...
        : ext === 'mts'
            ...
            : 'TypeScript source';
    if (file.size > text.length)
        out['Read'] = 'First ' + fmtBytes(text.length) + ' of ' + fmtBytes(file.size) + ' - counts below cover that much';
    ...
    const exported = [];
    for (const m of bare.matchAll(/\bexport\s+(?:default\s+)?(?:declare\s+)?(?:abstract\s+)?(?:async\s+)?(?:function\s*\*?|class|const|let|var|interface|type|enum|namespace|module)\s+([A-Za-z_$][\w$]*)/g))
        exported.push(m[1]);
    for (const m of bare.matchAll(/\bexport\s*\{([^}]*)\}/g)) {
        for (const part of m[1].split(',')) {
            const nm = part.trim().split(/\s+as\s+/).pop();
            if (nm && /^[A-Za-z_$][\w$]*$/.test(nm) && nm !== 'default')
                exported.push(nm);
        }
    }
    ...
    if (names.length || stars || hasDefault) {
        ...
        if (hasDefault)
            bits.push('a default');
        if (stars)
            bits.push(plural(stars, 'wildcard re-export'));
    ...
    if (decls.length)
        out['Declares'] = decls.map(([n, s, p]) => plural(n, s, p)).join(', ');
    const asyncs = count(/\basync\b/g), awaits = count(/\bawait\b/g);
    if (asyncs || awaits)
        out['Asynchronous'] = asyncs + ' async, ' + awaits + ' await';
    ...
    const esc = [];
    if (anys)
        esc.push(plural(anys, 'any'));
    if (casts)
        esc.push(plural(casts, '`as` cast'));
    if (bangs)
        esc.push(plural(bangs, 'non-null `!`'));
    if (suppress)
        esc.push(plural(suppress, '@ts- suppression'));
    out['Type escapes'] = esc.length ? esc.join(', ') : 'None found';
    if (docBlocks)
        out['Doc comments'] = plural(docBlocks, 'JSDoc block');
    const decorators = count(/(^|\n)\s*@[A-Za-z_$][\w$]*\s*[(\r\n]/g);
    if (decorators)
        out['Decorators'] = String(decorators);
    const marks = (text.match(/\b(?:TODO|FIXME|HACK|XXX)\b/g) || []).length;
    if (marks)
        out['Markers'] = plural(marks, 'TODO / FIXME / HACK note');
    const sections = [];
    if (uses.length) {
        sections.push({
    ...
    if (names.length)
        sections.push({ title: `Exported names (${names.length})`, node: preBlock(names.join('\n')) });
    if (sections.length)
        out._sections = sections;
    return out;
}
...
async function parseOnnx(file) {
    ...
    let p = 0;
    const varint = () => { let shift = 0, v = 0n; while (p < buf.length) {
        const b = buf[p++];
    ...
    try {
        while (p < buf.length) {
            ...
            if (wire === 0) { // varint
                const v = varint();
                if (field === 1)
                    out['IR version'] = ONNX_IR[Number(v)] ? ONNX_IR[Number(v)] + ' (ir ' + v + ')' : 'ir ' + v;
                if (field === 5)
                    out['Model version'] = String(v);
            }
                ...
                p += len;
                if (field === 2)
                    out['Exported by'] = ascii(slice, 0, slice.length);
                else if (field === 3)
                    out['Producer version'] = ascii(slice, 0, slice.length);
                else if (field === 4 && slice.length)
                    out['Domain'] = ascii(slice, 0, slice.length);
                else if (field === 8)
                    opsets++; // opset_import (count them)
            }
            else if (wire === 5) {
                p += 4;
            }
    ...
    if (opsets)
        out['Opset imports'] = opsets + (opsets >= 1 ? '+ (in header)' : '');
...
async function parseNativeBinary(file, ext) {
    ...
    if (machoLE || machoBE) {
        ...
        if (MACHO_CPU[cpu])
            out['Architecture'] = MACHO_CPU[cpu];
        out['Platform'] = 'macOS';
    }
...
async function parseLevelDb(file) {
    ...
    if (!ok)
        return { 'Format': 'LevelDB table (.ldb)', 'Note': 'Expected LevelDB footer magic not found - this .ldb may be an Access lock file or a different store.' };
    return {
...
async function parseIdl(file) {
    ...
    if (!/\b(interface|coclass|library|dispinterface|import|importlib|module|typedef)\b/.test(text))
        return null;
    ...
    if (coclasses.length)
        out['Coclasses'] = coclasses.length;
    if (libs.length)
        out['Type library'] = [...new Set(libs)].join(', ');
    if (uuids)
        out['GUIDs declared'] = uuids;
    if (methods)
        out['HRESULT methods'] = methods;
    if (imports.length)
        out['Imports'] = [...new Set(imports)].slice(0, 12).join(', ');
    const secs = [];
    if (interfaces.length)
        secs.push({ title: 'Interfaces (' + interfaces.length + ')', node: preBlock([...new Set(interfaces)].slice(0, 200).join('\n')) });
    if (coclasses.length)
        secs.push({ title: 'Coclasses (' + coclasses.length + ')', node: preBlock([...new Set(coclasses)].join('\n')) });
    if (secs.length)
        out._sections = secs;
    return out;
}
...
async function parseAsp(file) {
    ...
    if (!codeBlocks && !exprBlocks && !serverScripts && !includes.length && !/<%@/.test(text))
        return null;
    const dir = text.match(/<%@\s*([^%]*?)%>/);
    let lang = dir && (dir[1].match(/Language\s*=\s*["']?([A-Za-z]+)/i) || [])[1];
    if (!lang) {
        const sl = text.match(/<script[^>]*language\s*=\s*["']?([A-Za-z]+)/i);
        if (sl)
            lang = sl[1];
    }
    const out = {
    ...
    if (codeBlocks)
        out['Server code blocks'] = codeBlocks;
    if (exprBlocks)
        out['Inline expressions (<%=)'] = exprBlocks;
    if (serverScripts)
        out['Server <script> blocks'] = serverScripts;
    if (includes.length)
        out['Server-side includes'] = includes.length + ': ' + includes.slice(0, 8).join(', ');
    return out;
}
// ---------- pkg-config (.pc) ----------
...
async function parsePkgConfig(file) {
    ...
    if (!/^\s*(Name|Description|Version|Cflags|Libs)\s*:/mi.test(text))
        return null;
    ...
    if (vars.length)
        out['Variables'] = [...new Set(vars)].join(', ');
    return out;
}
// ---------- DraStic shader (.dsd) ----------
...
async function parseTemplate(file) {
    ...
    if (ph.length)
        out['Placeholders'] = ph.length + ': ' + ph.slice(0, 16).join(', ');
    return out;
}
...
//# sourceMappingURL=parsers-dev.js.map