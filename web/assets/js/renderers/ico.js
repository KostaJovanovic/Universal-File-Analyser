...
export function parseIcoEntries(buf) {
    ...
    if (reserved !== 0 || (type !== 1 && type !== 2) || count === 0 || count > 2000)
        return null;
    if (buf.byteLength < 6 + count * 16)
        return null;
    const entries = [];
    for (let i = 0; i < count; i++) {
        ...
        if (w === 0)
            w = 256; // 0 in the directory means 256
        if (h === 0)
            h = 256;
        ...
        if (!bytesInRes || imageOffset + bytesInRes > buf.byteLength)
            continue;
...
export async function buildIcoImagesCard(file, signal, resultsEl) {
    let parsed = null;
    try {
        parsed = parseIcoEntries(await file.arrayBuffer());
    }
    catch (_) {
        parsed = null;
    }
