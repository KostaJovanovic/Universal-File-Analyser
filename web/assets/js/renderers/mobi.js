...
function metaStr(v) {
    ...
    if (typeof v === 'string')
        return v;
    if (Array.isArray(v))
        return v.map(metaStr).filter(Boolean).join(', ');
    if (typeof v === 'object')
...
export async function renderMobi(file, resultsEl) {
    ...
    if (title)
        tbl.appendChild(row('Title', title));
    const author = metaStr(md.author);
    if (author)
        tbl.appendChild(row('Author', author));
    const pub = metaStr(md.publisher);
    if (pub)
        tbl.appendChild(row('Publisher', pub));
    const lang = metaStr(md.language);
    if (lang)
        tbl.appendChild(row('Language', lang));
    const published = metaStr(md.published);
    if (published)
        tbl.appendChild(row('Published', published.replace('T', ' ').replace(/\..*$/, '')));
    if (book.sections)
        tbl.appendChild(row('Sections', String(book.sections.length)));
    ...
    async function show(n) {
        ...
        busy = false;
        prev.disabled = cur <= 0;
        next.disabled = cur >= sections.length - 1;
    }
