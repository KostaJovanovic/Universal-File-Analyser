...
function tarPages(buf) {
    ...
    while (p + 512 <= buf.length) {
        ...
        if (!name)
            break; // run of zero blocks = end
        const size = oct(dec.decode(buf.subarray(p + 124, p + 136)));
        const type = buf[p + 156];
        const start = p + 512;
        if ((type === 48 || type === 0) && size > 0) {
            if (IMG_RE.test(name))
                pages.push({ name, getBytes: async () => buf.subarray(start, start + size) });
            else if (/comicinfo\.xml$/i.test(name)) {
                try {
                    comicInfo = new TextDecoder('utf-8').decode(buf.subarray(start, start + size));
                }
                catch (_) { }
            }
        }
...
export async function renderComic(file, resultsEl, extOverride) {
    ...
    function openReader(start) {
        ...
        prevBtn.addEventListener('click', (e) => { e.stopPropagation(); if (current > 0)
            show(current - 1); });
        const nextBtn = el('button', { type: 'button', class: 'lightbox-tool-btn' }, 'Next →');
        nextBtn.addEventListener('click', (e) => { e.stopPropagation(); if (current < pages.length - 1)
            show(current + 1); });
        overlay._prev = () => { if (current > 0)
            show(current - 1); };
        overlay._next = () => { if (current < pages.length - 1)
            show(current + 1); };
    ...
    if (pages.length > shown) {
        thumbCard.appendChild(el('p', { class: 'anr-hint', style: 'margin-top:12px;' }, '+ ' + (pages.length - shown) + ' more pages - open the reader to view them all.'));
    }
    resultsEl.appendChild(integrityCard(file));
}
