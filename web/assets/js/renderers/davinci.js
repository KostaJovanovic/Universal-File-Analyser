...
const kids = (e, tag) => { const out = []; if (e)
    for (const c of e.children)
        if (c.tagName === tag)
            out.push(c); return out; };
const kid = (e, tag) => { if (e)
    for (const c of e.children)
        if (c.tagName === tag)
...
function hexDouble(h) {
    ...
    for (let i = 0; i < 8; i++)
        b.setUint8(i, parseInt(h.substr(i * 2, 2), 16));
...
function pbStrings(buf, depth, path, out) {
    ...
    while (p < buf.length) {
        ...
        if (wt === 0) {
            ...
            if (printable)
                out.push({ path: path + '/' + field, str: String.fromCharCode.apply(null, sub) });
            else if (depth < 10)
                pbStrings(sub, depth + 1, path + '/' + field, out);
        }
        else
            return;
    }
...
function decodeGrade(hex) {
    ...
    if (z < 0 || !(window.fzstd && window.fzstd.decompress))
        return null;
    let raw;
    try {
        raw = window.fzstd.decompress(bytes.subarray(z));
    }
    catch (_) {
        return null;
    }
    ...
    for (const o of found) {
        ...
        if (LUT_RE.test(o.str)) {
            const clean = o.str.replace(/^[^A-Za-z0-9]+/, '').trim();
            if (clean)
                luts.add(basename(clean) || clean);
        }
        const fx = o.str.match(/com\.blackmagicdesign\.resolvefx\.([a-z0-9]+)/i);
        if (fx)
            effects.add(prettyFx(fx[1].toLowerCase()));
    }
    if (!nodes.length && !luts.size && !effects.size)
        return null;
...
function nodeChipsHtml(nodes) {
    if (!nodes.length)
        return '<span class="anr-hint">grade present, no labelled nodes</span>';
...
function trackLanesSvg(tl, H, trackW, ppf) {
    ...
    tracks.forEach((t, i) => {
        ...
        t.clips.forEach((c) => {
            ...
            if (gi && bw > 10)
                bars += `<circle cx="${bx + bw - 6}" cy="${y + 9}" r="2.6" fill="#f0a830"><title>${esc((gi.nodes.length || '?') + ' grade nodes')}</title></circle>`;
            if (bw > 42)
                bars += `<text x="${bx + 5}" y="${y + LH / 2 + 4}" fill="#fff" font-size="9.5" opacity=".95" pointer-events="none">${esc(c.name.slice(0, Math.max(3, Math.floor(bw / 6.5))))}</text>`;
        });
    });
...
function buildTimelineCard(tl, title) {
    ...
    scroller.addEventListener('pointermove', (e) => { if (dragging)
...
export async function renderDavinci(file, resultsEl) {
    ...
    let project = {};
    try {
        const pe = get('project.xml');
        if (pe)
            project = parseProject(await readEntryText(file, pe));
    }
    catch (_) { }
    const mediaPool = [];
    for (const e of byPrefix('MediaPool/', 'MpFolder.xml')) {
        try {
            const bin = parseMediaPool(await readEntryText(file, e));
            if (bin && (bin.clips.length || bin.timelines.length))
                mediaPool.push(bin);
        }
        catch (_) { }
    }
    ...
    for (const bin of mediaPool)
        for (const t of bin.timelines)
            if (t.seqId)
                tlNameBySeq.set(t.seqId, t.name);
    const timelines = [];
    for (const e of byPrefix('SeqContainer/', '.xml')) {
        try {
            const tl = parseTimeline(await readEntryText(file, e));
            if (tl)
                timelines.push(tl);
        }
        catch (_) { }
    ...
    if (gradedClips.length) {
        try {
            ...
            for (const c of gradedClips) {
                const g = decodeGrade(c.grade.bodyHex);
                if (g)
                    c.gradeInfo = g;
            }
        }
    ...
    tbl.appendChild(rowHelp('Format', isDrt ? 'Resolve timeline (.drt)' : 'Resolve project (.drp)', 'A Resolve project (.drp) is a ZIP archive of XML documents exported from Resolve’s project database. Analyser unpacks it and reads the track and clip objects (SeqContainer) to rebuild each timeline.'));
    if (project.name)
        tbl.appendChild(row('Project name', project.name));
    if (project.appVer)
        tbl.appendChild(rowHelp('Resolve version', project.appVer, 'The version of DaVinci Resolve that last saved this project (stored as DbAppVer).'));
    if (project.dbVer)
        tbl.appendChild(rowHelp('Database version', project.dbVer, 'The version of Resolve’s internal project-database layout (its schema), stored as DbPrjVer.'));
    if (project.modified && !isNaN(project.modified))
        tbl.appendChild(row('Last modified', project.modified.toLocaleString()));
    if (project.ageMs > 0)
        tbl.appendChild(rowHelp('Project age', (project.ageMs / 86400000).toFixed(1) + ' days', 'The total time this project has existed, added up by Resolve across all sessions (stored as ProjectAgeInMs).'));
        ...
        for (const t of tl.tracks)
            for (const c of t.clips)
                if (c.gradeInfo)
                    graded.push(c);
    if (graded.length) {
        const totalVideo = timelines.reduce((s, tl) => s + tl.tracks.reduce((a, t) => a + (t.kind === 'video' ? t.clips.length : 0), 0), 0);
        const card = el('div', { class: 'anr-card' });
        ...
        card.appendChild(el('p', { class: 'anr-hint', style: 'margin:0 0 12px' }, graded.length + ' of ' + totalVideo + ' video clips carry a colour grade.'));
        ...
        [...chains.values()].sort((a, b) => b.n - a.n).slice(0, 12).forEach((ch) => {
            card.appendChild(el('div', { style: 'margin:0 0 9px' }, [
                el('div', { style: 'font-size:12px;opacity:.6;margin-bottom:3px' }, '×' + ch.n + ' clip' + (ch.n === 1 ? '' : 's') + '  ·  ' + ch.nodes.length + ' node' + (ch.nodes.length === 1 ? '' : 's')),
        ...
        for (const c of graded) {
            c.gradeInfo.luts.forEach((l) => luts.add(l));
            c.gradeInfo.effects.forEach((e) => fx.add(e));
        }
        if (luts.size || fx.size) {
            const tbl2 = el('table', { class: 'anr-readout', style: 'margin-top:6px' });
            if (fx.size)
                tbl2.appendChild(rowHelp('ResolveFX', [...fx].join(', '), 'Add-on visual effects (Blackmagic ResolveFX or OFX plugins) applied during colour grading - on grade nodes, the steps in Resolve’s colour pipeline.'));
            if (luts.size)
                tbl2.appendChild(rowHelp('LUTs', [...luts].join(', '), 'Look-up tables (LUTs) - colour presets that remap the image’s colours - loaded during colour grading.'));
    ...
    // ---- Legend ----
    if (timelines.length) {
        const [legH, legP] = h3help('Legend', 'Hover a clip for its source path, frame rate and timecode. '
        ...
        resultsEl.appendChild(el('div', { class: 'anr-card' }, [
            legH, legP,
            el('p', { class: 'anr-hint', html: 'Each bar is a clip, positioned by its Start and Duration (in frames) on the timeline. '
    ...
    if (mediaPool.length) {
        ...
        for (const bin of mediaPool) {
            if (!bin.clips.length && !bin.timelines.length)
                continue;
            card.appendChild(el('p', { style: 'margin:10px 0 2px;font-weight:600;font-size:13px' }, `${bin.name}  (${bin.clips.length})`));
    ...
    if (allPaths.length) {
        ...
        if (allPaths.length > 250)
            ul.appendChild(el('li', { class: 'anr-hint' }, '… and ' + (allPaths.length - 250) + ' more'));
        card.appendChild(ul);
        resultsEl.appendChild(card);
    }
