...
function jsonTree(value, key) {
            ...
            if (det.open && !filled) {
                ...
                const addBatch = () => {
                    ...
                    if (next >= count)
                        ...
                        moreBtn.textContent = 'Show more (' + next.toLocaleString() + ' of ' + count.toLocaleString() + ')';
                };
    ...
    if (key != null)
        line.appendChild(el('span', { class: 'anr-json-key' }, String(key) + ': '));
...
export async function renderHar(file, container) {
    ...
    try {
        ...
        container.innerHTML = '';
        container.appendChild(errorCard('This .har file is not valid JSON: ' + (e && e.message || 'parse error')));
        return;
    }
    const log = har.log || {};
    ...
    if (!entries.length) {
        container.appendChild(el('div', { class: 'anr-card' }, [el('h3', {}, 'Requests'), el('p', { class: 'anr-hint' }, 'No requests in this capture.')]));
        return;
    }
    ...
    function reveal(upTo) {
        for (; shown < upTo && shown < entries.length; shown++)
            addRow(shown);
        if (shown >= entries.length)
            btnRow.hidden = true;
        else
            moreBtn.textContent = 'Show more (' + shown + '/' + entries.length + ')';
    }
...
export async function renderJsonData(file, container) {
    ...
    try {
        ...
        info.appendChild(buildReadout([
            ...
            rowHelp('Parses', parsed != null ? 'Yes (after comment / trailing-comma stripping)' : 'No - shown as source only', 'JSON is a common data format with strict rules; these relaxed versions (JSON5, Hjson) allow extras like comments and stray commas. Analyser tries to read them by stripping those out, but some looser habits (single quotes, names left without quotes) may still not read, in which case it just shows the raw text.'),
        ]));
        container.appendChild(info);
