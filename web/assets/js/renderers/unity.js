...
function splitDocs(text) {
    ...
    while ((m = re.exec(text)))
        heads.push({ classId: +m[1], fileId: m[2], at: m.index, end: re.lastIndex });
    const docs = [];
...
function parseUnity(text, ext, name) {
    ...
    for (const d of docs) {
        ...
        if (d.className === 'GameObject') {
            if (nm)
                gameObjects.push(nm);
        }
        else if (nm)
            named.push({ className: d.className, name: nm });
    }
...
export async function renderUnity(file, resultsEl) {
    ...
    let text;
    try {
        text = await file.slice(0, MAX_BYTES).text();
    }
    catch (e) {
        resultsEl.innerHTML = '';
    ...
    if (data.kind === 'meta') {
        ...
        tbl.appendChild(rowHelp('Format', 'Unity .meta importer record', 'A companion file Unity keeps beside every asset (image, model, sound) in a game project. It stores a fixed ID Unity uses to track that asset, plus its import settings, written as plain-text YAML.'));
        if (data.guid)
            tbl.appendChild(rowHelp('Asset GUID', data.guid, 'A fixed 32-character ID Unity uses to refer to this asset throughout the project. It stays the same even if the file is renamed or moved.'));
        if (data.importer)
            tbl.appendChild(rowHelp('Importer', data.importer.replace(/([a-z])([A-Z])/g, '$1 $2'), 'The part of Unity that processes this asset when it is brought into the project - for example a texture importer for an image, or a model importer for a 3D model.'));
        if (data.isFolder)
            tbl.appendChild(row('Folder asset', 'yes'));
        ...
        if (data.licenseType)
            tbl.appendChild(row('License', data.licenseType));
        if (data.fileFormatVersion)
            tbl.appendChild(row('Format version', data.fileFormatVersion));
    ...
    const tbl = el('table', { class: 'anr-readout' });
    tbl.appendChild(row('Application', 'Unity (game engine)'));
    tbl.appendChild(rowHelp('Format', unityFormatName(data.ext, data.label), 'Unity saves this asset as plain text in its own flavour of the YAML format - one section per object, each labelled with the kind of object it is (!u!<classID>). Analyser separates those sections and reads the important fields.'));
    if (data.sceneLike) {
    ...
    if (data.gameObjects.length) {
        ...
        if (data.gameObjects.length > 400)
            ul.appendChild(el('li', { class: 'anr-hint' }, '… and more'));
        card.appendChild(ul);
        resultsEl.appendChild(card);
    }
