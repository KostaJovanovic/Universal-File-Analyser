...
import { parseOnnxModel, parseTfGraphDef } from '../lib/onnx.js';
import { ML_MODEL_MAX, ML_GRAPH_NODES, ML_NODE_MAX, GGUF_HEADER_MAX } from '../core/limits.js';
const fmtParams = (n) => n >= 1e9 ? (n / 1e9).toFixed(2) + ' B' : n >= 1e6 ? (n / 1e6).toFixed(1) + ' M' : n >= 1e3 ? (n / 1e3).toFixed(1) + ' K' : String(n);
const shapeText = (dims) => dims.length ? dims.join(' x ') : 'scalar';
/* ---- the graph diagram ----
...
function buildGraphDiagram(nodes) {
    const producer = new Map();
    nodes.forEach((n) => n.outputs.forEach((o) => { if (o && !producer.has(o))
        producer.set(o, n.index); }));
    ...
    for (let i = 0; i < nodes.length; i++) {
        ...
        while (stack.length) {
            ...
            for (const inp of nodes[cur].inputs) {
                ...
                if (visiting[p])
                    continue; // cycle - treat as already placed
                visiting[p] = 1;
                stack.push(p);
                waiting = true;
            }
...
function tensorTable(tensors, limit = 500) {
    const t = el('table', { class: 'anr-readout' });
...
/* ---- ONNX / TensorFlow ---- */
async function renderGraphModel(file, resultsEl) {
    ...
    const nodes = model && model.graph ? model.graph.nodes : (tf ? tf.nodes : []);
    ...
    const [h, help] = h3help(framework === 'ONNX' ? 'ONNX model' : 'TensorFlow graph', 'A trained model is a computation graph: a list of operations, each taking named tensors in and producing named tensors out. The connections are not stored anywhere - they are recovered by matching those names - and everything below is read from the file in your browser. The weights themselves are not loaded.');
    card.appendChild(h);
    ...
    if (model) {
        if (model.irVersion != null)
            tbl.appendChild(rowHelp('IR version', String(model.irVersion), 'The version of the ONNX intermediate representation, which decides what a runtime needs to support to load this at all.'));
        if (model.producer)
            tbl.appendChild(row('Exported by', model.producer + (model.producerVersion ? ' ' + model.producerVersion : '')));
        if (model.domain)
            tbl.appendChild(row('Domain', model.domain));
        if (model.modelVersion)
            tbl.appendChild(row('Model version', model.modelVersion));
        if (model.opsets.length)
            tbl.appendChild(rowHelp('Opset', model.opsets.map((o) => o.domain + ' v' + o.version).join(', '), 'The operator-set version the model was written against. A runtime older than this will not have every operation the graph uses.'));
    }
    ...
    if (inits.length) {
        ...
        if (totalParams)
            tbl.appendChild(rowHelp('Parameters', fmtParams(totalParams) + ' (' + totalParams.toLocaleString() + ')', 'Every learned number in the model, added up across its weight tensors. This is the figure people mean by a "7B" model.'));
    }
    ...
    if (model && model.graph && (model.graph.inputs.length || model.graph.outputs.length)) {
        ...
        const [ih, ihelp] = h3help('Inputs and outputs', 'What the model expects and what it returns, with the shape of each. A dimension shown as a name rather than a number is dynamic - "batch_size" means the model will take any number of items at once.');
        c.appendChild(ih);
        c.appendChild(ihelp);
    ...
    {
        const c = el('div', { class: 'anr-card' });
        const [gh, ghelp] = h3help('Graph', 'The computation drawn out, left to right. Each box is one operation and each line an output feeding the next input. A node sits one column further right than the deepest thing it depends on, so parallel branches appear side by side and merge where they rejoin. Hover a box for its full name and settings.');
        c.appendChild(gh);
        c.appendChild(ghelp);
        if (nodes.length > ML_GRAPH_NODES) {
            c.appendChild(el('p', { class: 'anr-hint' }, 'This graph has ' + nodes.length.toLocaleString() + ' nodes - too many to draw legibly, so the first ' + ML_GRAPH_NODES.toLocaleString() + ' are shown. The tables below cover all of them.'));
        }
    ...
    // Op histogram
    {
        ...
        const c = el('div', { class: 'anr-card' });
        const [oh, ohelp] = h3help('Operations (' + hist.distinct + ' kinds)', 'What the model is actually made of. The mix is a fingerprint of the architecture: Conv everywhere is a vision model, MatMul and Softmax in a repeating block is a transformer, and a wall of Gemm is a plain fully-connected network.');
        c.appendChild(oh);
        c.appendChild(ohelp);
    ...
    if (inits.length) {
        const c = el('div', { class: 'anr-card' });
        const [wh, whelp] = h3help('Weights (' + inits.length.toLocaleString() + ')', 'The learned tensors baked into the file, with the shape and parameter count of each. Sorted largest first, so the layers carrying the model\'s size come to the top.');
        c.appendChild(wh);
        c.appendChild(whelp);
        c.appendChild(tensorTable(inits.slice().sort((a, b) => b.params - a.params)));
        if (inits.length > 500)
            c.appendChild(el('p', { class: 'anr-hint' }, 'Showing the 500 largest of ' + inits.length.toLocaleString() + '.'));
        resultsEl.appendChild(c);
    }
    // Node list
    {
        ...
        if (nodes.length > 1000)
            c.appendChild(el('p', { class: 'anr-hint' }, 'Showing the first 1,000.'));
        resultsEl.appendChild(c);
    }
    if (model && model.metadata.length) {
        ...
        const t = el('table', { class: 'anr-readout' });
        for (const [k, v] of model.metadata)
            t.appendChild(row(k, v.length > 300 ? v.slice(0, 300) + '…' : v));
        c.appendChild(t);
        resultsEl.appendChild(c);
    }
...
async function renderSafetensors(file, resultsEl) {
    const head = new Uint8Array(await file.slice(0, 8).arrayBuffer());
    const n = Number(new DataView(head.buffer).getBigUint64(0, true));
    if (!n || n > 200_000_000 || n + 8 > file.size) {
        resultsEl.appendChild(errorCard('This is not a valid safetensors file.'));
        return;
    }
    let meta;
    try {
        meta = JSON.parse(new TextDecoder().decode(new Uint8Array(await file.slice(8, 8 + n).arrayBuffer())));
    }
    catch (_) {
        resultsEl.appendChild(errorCard('The safetensors header is not readable JSON.'));
        return;
    }
    const tensors = [];
    ...
    const dtypes = Object.create(null);
    const groups = Object.create(null);
    let params = 0;
    for (const [name, t] of Object.entries(meta)) {
        if (name === '__metadata__' || !t || !t.shape)
            continue;
        const dims = t.shape;
    ...
    const card = el('div', { class: 'anr-card' });
    const [h, help] = h3help('Safetensors weights', 'A weight file, not a model: it holds the learned numbers but nothing about how they are wired together. The format was designed as a safe replacement for pickled PyTorch checkpoints - the header is plain JSON with a length in front of it, so what is in the file can be read without running anything from it.');
    card.appendChild(h);
    card.appendChild(help);
    const tbl = el('table', { class: 'anr-readout' });
    tbl.appendChild(row('File', file.name));
    ...
    tbl.appendChild(rowHelp('Parameters', fmtParams(params) + ' (' + params.toLocaleString() + ')', 'Every learned number in the file. This is the figure a model name like "7B" refers to.'));
    tbl.appendChild(row('Precision', Object.entries(dtypes).sort((a, b) => b[1] - a[1]).map(([k, v]) => k + ' (' + v + ')').join(', ')));
    if (params)
        tbl.appendChild(rowHelp('Bytes per parameter', (file.size / params).toFixed(2), 'File size divided by parameter count - which is effectively the precision the weights are stored at: about 4 for float32, 2 for float16 or bfloat16, 1 for 8-bit.'));
    card.appendChild(tbl);
    resultsEl.appendChild(card);
    if (meta.__metadata__) {
        ...
        c.appendChild(el('h3', {}, 'Metadata'));
        const t = el('table', { class: 'anr-readout' });
        for (const [k, v] of Object.entries(meta.__metadata__))
            t.appendChild(row(k, String(v).slice(0, 400)));
        c.appendChild(t);
        resultsEl.appendChild(c);
    }
    ...
    if (groupRows.length > 1 && groupRows.length < tensors.length) {
        const c = el('div', { class: 'anr-card' });
        const [gh, ghelp] = h3help('By block (' + groupRows.length + ')', 'Tensor names in a trained model are paths - "model.layers.12.self_attn.q_proj.weight" - so grouping by the path above the last two parts collapses a repeated transformer block into one row and shows where the parameters actually sit.');
        c.appendChild(gh);
        c.appendChild(ghelp);
        const t = el('table', { class: 'anr-readout' });
    ...
    if (tensors.length > 500)
        c.appendChild(el('p', { class: 'anr-hint' }, 'Showing the 500 largest of ' + tensors.length.toLocaleString() + '.'));
    resultsEl.appendChild(c);
}
/* ---- GGUF ---- */
const GGML_TYPE = {
...
async function renderGguf(file, resultsEl) {
    ...
    const kvCount = u64();
    const str = () => { const n = u64(); if (p + n > buf.length)
        throw new Error('string runs past the header'); const s = new TextDecoder().decode(buf.subarray(p, p + n)); p += n; return s; };
    ...
    const GGUF_MIN_SIZE = { 0: 1, 1: 1, 2: 2, 3: 2, 4: 4, 5: 4, 6: 4, 7: 1, 8: 8, 9: 12, 10: 8, 11: 8, 12: 8 };
    ...
    const readValue = (type) => {
        switch (type) {
            case 0:
                if (p >= buf.length)
                    throw new Error('value runs past the header');
                return String(buf[p++]);
            ...
            case 7:
                if (p >= buf.length)
                    throw new Error('value runs past the header');
                return buf[p++] ? 'true' : 'false';
            case 8: return str();
            case 9: {
                const elem = dv.getUint32(p, true);
                p += 4;
                const n = u64();
                ...
                if (n * min > buf.length - p)
                    throw new Error('array runs past the header');
                const preview = [];
                for (let i = 0; i < n; i++) {
            ...
            case 11: {
                const v = Number(dv.getBigInt64(p, true));
                p += 8;
                return String(v);
            }
            case 12: {
    ...
    const find = (k) => { const e = kv.find(([key]) => key === k || key.endsWith('.' + k)); return e ? e[1] : null; };
    const arch = find('general.architecture') || find('architecture');
    const card = el('div', { class: 'anr-card' });
    const [h, help] = h3help('GGUF model', 'The container llama.cpp uses for quantised models: a self-describing header of key/value metadata followed by the tensors. Everything a runtime needs to load the model - the architecture, the context length, the tokenizer, the chat template - is in that header rather than in a separate config file, which is what makes a GGUF a single portable file.');
    card.appendChild(h);
    card.appendChild(help);
    const tbl = el('table', { class: 'anr-readout' });
    tbl.appendChild(row('File', file.name));
    ...
    if (arch)
        tbl.appendChild(row('Architecture', arch));
    const nameKv = find('general.name');
    if (nameKv)
        tbl.appendChild(row('Model name', nameKv));
    const ctx = arch ? find(arch + '.context_length') : null;
    if (ctx)
        tbl.appendChild(rowHelp('Context length', Number(ctx).toLocaleString() + ' tokens', 'How much text the model can attend to at once - the longest prompt plus reply it can hold in mind.'));
    const layers = arch ? find(arch + '.block_count') : null;
    if (layers)
        tbl.appendChild(row('Layers', layers));
    const embd = arch ? find(arch + '.embedding_length') : null;
    if (embd)
        tbl.appendChild(row('Embedding size', embd));
    tbl.appendChild(row('Tensors', tensorCount.toLocaleString()));
    tbl.appendChild(row('Metadata entries', kvCount.toLocaleString()));
    if (params) {
        tbl.appendChild(rowHelp('Parameters', fmtParams(params) + ' (' + params.toLocaleString() + ')', 'Every learned number in the model, added up across its tensors.'));
        tbl.appendChild(rowHelp('Bits per parameter', ((file.size * 8) / params).toFixed(2), 'File size divided by parameter count. It is the practical measure of how hard the model has been quantised: 16 is unquantised half precision, and a Q4 model lands near 4.5 once its metadata is counted.'));
    }
    const quants = {};
    for (const t of tensors)
        quants[t.dtype] = (quants[t.dtype] || 0) + 1;
    if (Object.keys(quants).length) {
        tbl.appendChild(rowHelp('Quantisation', Object.entries(quants).sort((a, b) => b[1] - a[1]).map(([k, v]) => k + ' (' + v + ')').join(', '), 'The formats the individual tensors are stored in. A mixed list is normal and deliberate: llama.cpp keeps the layers that matter most at higher precision.'));
    }
    card.appendChild(tbl);
    resultsEl.appendChild(card);
    if (!readOk) {
        resultsEl.appendChild(el('p', { class: 'anr-hint' }, 'The header runs past the first ' + fmtBytes(cap) + ' of the file, so the listings below are partial. The counts above come from the file\'s own header and are complete.'));
    }
    if (kv.length) {
        const c = el('div', { class: 'anr-card' });
        const [mh, mhelp] = h3help('Metadata (' + kv.length + ')', 'Everything the model declares about itself. The tokenizer entries are the large ones - a vocabulary is a list of every token the model knows, so it is summarised by size rather than printed.');
        c.appendChild(mh);
        c.appendChild(mhelp);
        const t = el('table', { class: 'anr-readout' });
        for (const [k, v] of kv)
            t.appendChild(row(k, v.length > 400 ? v.slice(0, 400) + '…' : v));
        c.appendChild(el('div', { class: 'anr-table-wrap' }, [t]));
        resultsEl.appendChild(c);
    }
    if (tensors.length) {
...
// reports what it can.
function pickleGlobalsWalk(b, found) {
    const dv = new DataView(b.buffer, b.byteOffset, b.byteLength);
    let i = 0;
    const line = () => {
        let s = '';
        while (i < b.length && b[i] !== 0x0a && s.length < 256)
            s += String.fromCharCode(b[i++]);
        while (i < b.length && b[i] !== 0x0a)
            i++;
        i++;
        return s;
    };
    const str = (n) => { const s = new TextDecoder('utf-8').decode(b.subarray(i, Math.min(b.length, i + Math.min(n, 256)))); i += n; return s; };
    ...
    const push = (v) => { prev = last; last = v; };
    const memo = new Map();
    let memoNext = 0;
    const FIXED = { 0x4a: 4, 0x4b: 1, 0x4d: 2, 0x47: 8, 0x82: 1, 0x83: 2, 0x84: 4 };
        ...
        while (i < b.length) {
            ...
            if (op === 0x63 || op === 0x69) {
                ...
                if (b[i] !== 0x80)
                    return true;
                prev = last = null;
            } // STOP - a legacy torch file chains several
            else if (op === 0x80)
                i++; // PROTO
            else if (op === 0x95)
                i += 8; // FRAME
            else if (op === 0x94)
                memo.set(memoNext++, last); // MEMOIZE
            else if (op === 0x70)
                memo.set(parseInt(line(), 10), last); // PUT
            else if (op === 0x71)
                memo.set(b[i++], last); // BINPUT
            else if (op === 0x72)
                memo.set(u32(), last); // LONG_BINPUT
            else if (op === 0x67)
                push(memo.get(parseInt(line(), 10)) ?? null); // GET
            else if (op === 0x68)
                push(memo.get(b[i++]) ?? null); // BINGET
            else if (op === 0x6a)
                push(memo.get(u32()) ?? null); // LONG_BINGET
            else if (op === 0x8c || op === 0x55)
                push(str(b[i++])); // SHORT_BINUNICODE / SHORT_BINSTRING
            else if (op === 0x58 || op === 0x54)
                push(str(u32())); // BINUNICODE / BINSTRING
            else if (op === 0x8d)
                push(str(u64())); // BINUNICODE8
            else if (op === 0x56 || op === 0x53)
                push(line().replace(/^'|'$/g, '')); // UNICODE / STRING (text)
            else if (op === 0x49 || op === 0x4c || op === 0x46 || op === 0x50) {
                line();
                push(null);
            } // INT/LONG/FLOAT/PERSID
            else if (op === 0x43 || op === 0x8a) {
                i += 1 + b[i];
                push(null);
            } // SHORT_BINBYTES / LONG1
            else if (op === 0x42 || op === 0x8b) {
                const n = u32();
                i += n;
                push(null);
            } // BINBYTES / LONG4
            else if (op === 0x8e || op === 0x96) {
                const n = u64();
                i += n;
                push(null);
            } // BINBYTES8 / BYTEARRAY8
            else if (FIXED[op] != null) {
                i += FIXED[op];
                push(null);
            }
            else if (NOARG.has(op))
                push(null);
            else
                return false; // not a pickle opcode
        }
    }
    catch (_) {
        return false;
...
function pickleGlobals(buf) {
    const found = new Set();
    if (pickleGlobalsWalk(buf, found))
        return [...found].sort();
    ...
    for (let i = 0; i < buf.length - 2; i++) {
        ...
        if (strs.length === 2)
            found.add(strs[0] + '.' + strs[1]);
    }
    return [...found].sort();
}
async function renderPyTorch(file, resultsEl) {
    let zip;
    try {
    ...
    const dataEntries = zip.entries.filter((e) => /\/data\/\d+$/.test(e.name) || /^data\/\d+$/.test(e.name));
    const pkl = zip.entries.find((e) => /data\.pkl$/.test(e.name));
    const versionText = zip.has('version') ? (await zip.text('version')) : null;
    tbl.appendChild(row('Container', 'ZIP (PyTorch 1.6+ format)'));
    if (versionText)
        tbl.appendChild(row('Serialisation version', versionText.trim()));
    tbl.appendChild(rowHelp('Tensor storages', dataEntries.length.toLocaleString(), 'The raw blocks of numbers. The pickle beside them says which tensor each block is, what shape it has and what dtype to read it as.'));
    const storageBytes = dataEntries.reduce((s, e) => s + (e.size || e.uncompressedSize || 0), 0);
    if (storageBytes)
        tbl.appendChild(row('Weight data', fmtBytes(storageBytes)));
    card.appendChild(tbl);
    resultsEl.appendChild(card);
    if (pkl) {
        ...
        if (bytes) {
            ...
            if (keys.length) {
                const c = el('div', { class: 'anr-card' });
                const [kh, khelp] = h3help('Tensor names (' + keys.length + ')', 'The keys of the checkpoint\'s state dictionary - the names the model gives its own weights. They are plain strings in the pickle, so they read out without interpreting any of it.');
                c.appendChild(kh);
                c.appendChild(khelp);
...
function appendPickleFindings(resultsEl, globals) {
    ...
    const c = el('div', { class: 'anr-card' });
    const [h, help] = h3help('What this pickle would import', 'Unpickling calls whatever the file names. That is the whole of the risk: a checkpoint downloaded from anywhere can name os.system as easily as torch.FloatStorage, and torch.load will call it. These are the module.function pairs found in the opcodes, read as bytes and never run.');
    c.appendChild(h);
    c.appendChild(help);
    if (risky.length) {
        c.appendChild(el('div', { class: 'anr-pack-signal' }, 'This file names ' + risky.join(', ') + '. A file of numbers has no reason to reach for these, and unpickling it would call them. Treat it as untrusted code, not as data.'));
    }
    if (unusual.length) {
        c.appendChild(el('div', { class: 'anr-pack-signal' }, 'Outside the usual set: ' + unusual.slice(0, 24).join(', ') + (unusual.length > 24 ? ' and ' + (unusual.length - 24) + ' more' : '') +
    ...
    if (globals.length)
        c.appendChild(preBlock(globals.join('\n')));
    resultsEl.appendChild(c);
}
/* ---- Keras ---- */
async function renderKeras(file, resultsEl, ext) {
    let config = null, meta = null, source = '';
    if (ext === 'keras') {
        const zip = await openZip(file);
        try {
            config = JSON.parse((await zip.text('config.json')) || 'null');
        }
        catch (_) { }
        try {
            meta = JSON.parse((await zip.text('metadata.json')) || 'null');
        }
        catch (_) { }
        ...
        if (at >= 0) {
            ...
            if (endIndex > 0) {
                try {
                    config = JSON.parse(text.slice(at, endIndex));
                }
                catch (_) { }
            }
        }
    ...
    const card = el('div', { class: 'anr-card' });
    const [h, help] = h3help('Keras model', 'Keras stores the architecture as JSON and the weights beside it, so the shape of the network reads out directly - read here from ' + source + '.');
    card.appendChild(h);
    card.appendChild(help);
    const tbl = el('table', { class: 'anr-readout' });
    tbl.appendChild(row('File', file.name));
    ...
    if (meta && meta.keras_version)
        tbl.appendChild(row('Keras version', meta.keras_version));
    if (meta && meta.date_saved)
        tbl.appendChild(row('Saved', meta.date_saved));
    if (config && config.class_name)
        tbl.appendChild(row('Model type', config.class_name));
    const layers = (config && config.config && config.config.layers) || [];
    if (layers.length)
        tbl.appendChild(row('Layers', String(layers.length)));
    card.appendChild(tbl);
    resultsEl.appendChild(card);
    if (!config) {
    ...
    if (layers.length) {
        const c = el('div', { class: 'anr-card' });
        const [lh, lhelp] = h3help('Layers (' + layers.length + ')', 'The network as Keras describes it, in order. Each row is one layer with the settings that define its shape - units, filters, kernel size, activation.');
        c.appendChild(lh);
        c.appendChild(lhelp);
        const t = el('table', { class: 'anr-readout' });
...
/* ---- dispatch ---- */
export async function renderMlModel(file, resultsEl) {
    resultsEl.hidden = false;
    resultsEl.innerHTML = '';
    const ext = (file.name.split('.').pop() || '').toLowerCase();
    try {
        if (ext === 'safetensors')
            return await renderSafetensors(file, resultsEl);
        if (ext === 'gguf' || ext === 'ggml' || ext === 'ggjt')
            return await renderGguf(file, resultsEl);
        if (ext === 'pt' || ext === 'pth' || ext === 'ckpt')
            return await renderPyTorch(file, resultsEl);
        if (ext === 'keras' || ext === 'h5' || ext === 'hdf5')
            return await renderKeras(file, resultsEl, ext);
        ...
        resultsEl.appendChild(errorCard('Could not read this model file.' + (e && e.message ? ' ' + e.message : '')));
    }
}
