...
function findJumbfStore(b, from, to) {
...
function extractFromIsobmff(b) {
    ...
    while (p + 8 <= b.length) {
        ...
        if (size === 1) { // 64-bit largesize
            ...
            size = b.length - p; // "to end of file"
        if (size < head)
            break;
        ...
        if (type === 'uuid' && p + head + 16 <= b.length) {
            let match = true;
            for (let i = 0; i < 16; i++)
                if (b[p + head + i] !== BMFF_C2PA_UUID[i]) {
                    match = false;
                    break;
                }
            if (match) {
...
function extractC2pa(b) {
    ...
    if (b[0] === 0x89 && b[1] === 0x50)
        return extractFromPng(b);
    if (looksLikeJumbf(b))
        return b; // a bare .c2pa manifest
...
function* derKids(b, s, e) { let p = s; while (p < e) {
    const n = derRead(b, p);
    yield n;
    p = n.end;
} }
function derOid(b, n) {
    const by = b.subarray(n.content, n.end);
    if (!by.length)
        return '';
    const parts = [Math.floor(by[0] / 40), by[0] % 40];
    let v = 0;
    ...
    try {
        for (const rdn of derKids(b, node.content, node.end))
            for (const atv of derKids(b, rdn.content, rdn.end)) {
                ...
                if (k.length >= 2 && k[0].tag === 0x06) {
                    const key = DN[derOid(b, k[0])];
                    if (key)
                        parts.push(key + '=' + utf8(b.subarray(k[1].content, k[1].end)));
                }
            }
    }
    catch (_) { }
...
function findByTag(nodes, tag, label) {
    for (const n of nodes || []) {
        if (n.type === 'jumb' && (n.tag === tag || baseLabel(n.label) === label))
            return n;
    }
    return null;
}
...
function certsFromCose(cose) {
    if (!Array.isArray(cose) || cose.length < 2)
        return [];
    const headers = [];
    if (cose[0] instanceof Uint8Array && cose[0].length) {
        const h = decodeCbor(cose[0]);
        if (h)
            headers.push(h);
    }
    if (cose[1] && typeof cose[1] === 'object')
        headers.push(cose[1]);
    for (const h of headers) {
        ...
        if (x5 instanceof Uint8Array)
            return [x5];
        if (Array.isArray(x5))
            return x5.filter((c) => c instanceof Uint8Array);
    }
    return [];
}
function parseManifest(node) {
    ...
    if (sigBox) {
        ...
        if (certs.length) {
            out.signer = x509Summary(certs[0]);
            out.certCount = certs.length;
        }
    }
    return out;
}
function safeJson(bytes) { try {
...
export function readC2paFromBytes(b) {
    const store = extractC2pa(b);
    if (!store)
        return null;
    try {
        return parseC2paStore(store);
    }
    catch (_) {
        return null;
    }
...
export async function buildC2paCard(file, manifests) {
    ...
    manifests.forEach((m, idx) => {
        if (manifests.length > 1)
            card.appendChild(el('div', { class: 'anr-readout-section' }, 'Manifest ' + (idx + 1)));
        const t = el('table', { class: 'anr-readout' });
        if (m.generator)
            t.appendChild(row('Created with', m.generator));
        if (m.title)
            t.appendChild(row('Title', m.title));
        if (m.format)
            t.appendChild(row('Format', m.format));
        if (m.alg)
            t.appendChild(rowHelp('Claim hash alg', m.alg, 'The type of cryptographic fingerprint used to seal this record - ‘sha256’ is a common one. It is the method that would reveal any tampering if the signature were checked.'));
        if (m.signer) {
            if (m.signer.subject)
                t.appendChild(row('Signed by', m.signer.subject));
            if (m.signer.issuer)
                t.appendChild(rowHelp('Certificate issuer', m.signer.issuer + (m.signer.subject === m.signer.issuer ? '  (self-signed)' : ''), 'Who issued the digital certificate that signed this record. ‘Self-signed’ means the signer vouched for themselves rather than a recognised authority, which is weaker evidence.'));
            const fmtD = (d) => d ? d.toISOString().replace('T', ' ').replace(/\..*$/, '') : null;
            if (m.signer.notBefore || m.signer.notAfter)
                t.appendChild(rowHelp('Certificate validity', (fmtD(m.signer.notBefore) || '?') + '  to  ' + (fmtD(m.signer.notAfter) || '?'), 'The dates the signing certificate itself is valid between - this is about the certificate, not when the photo was taken or edited.'));
        }
        else {
        ...
        if (m.ai.length) {
            ...
            for (const a of m.ai)
                card.appendChild(el('p', { style: 'color:var(--accent);font-weight:600;margin:4px 0;' }, a));
        }
        if (m.actions.length) {
        ...
        if (m.ingredients.length) {
            ...
            for (const g of m.ingredients)
                it.appendChild(row(g.title || '(untitled)', [g.relationship, g.format].filter(Boolean).join(' · ') || '-'));
            card.appendChild(it);
        }
