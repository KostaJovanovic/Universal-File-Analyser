...
export async function collectAiSignals(file, exif, preManifests) {
    ...
    if (exif) {
        for (const [k, v] of Object.entries(exif)) {
            ...
            if (DIGITAL_SOURCE_AI.test(v))
                pushUnique(signals, { label: 'Digital Source Type (' + k + ')', detail: v + '  - a standard "AI-generated" marker', strong: true, help: DST_HELP });
    ...
    if (xmp) {
        if (DIGITAL_SOURCE_AI.test(xmp))
            pushUnique(signals, { label: 'Digital Source Type (XMP)', detail: (xmp.match(DIGITAL_SOURCE_AI) || [])[0] + '  - a standard "AI-generated" marker', strong: true, help: DST_HELP });
    ...
    try {
        ...
        for (const m of manifests || []) {
            if (m.ai && m.ai.length)
                pushUnique(signals, { label: 'Content Credentials declare an AI source', detail: 'see the C2PA card above', strong: true, c2pa: true, help: CC_AI_HELP });
            else if (m.generator && AI_TOOLS.test(m.generator))
