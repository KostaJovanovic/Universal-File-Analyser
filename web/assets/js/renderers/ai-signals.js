...
function pushUnique(list, sig) {
    if (!list.some((s) => s.label === sig.label && s.detail === sig.detail))
        list.push(sig);
}
...
async function readForSignals(file) {
...
export async function collectAiSignals(file, exif, preManifests) {
    ...
    if (exif) {
        for (const [k, v] of Object.entries(exif)) {
            if (typeof v !== 'string' || !v)
                continue;
            if (DIGITAL_SOURCE_AI.test(v))
                pushUnique(signals, { label: 'Digital Source Type (' + k + ')', detail: v + '  - a standard "AI-generated" marker', strong: true, dst: true, help: DST_HELP });
            else if (SD_PARAMS.test(v)) {
                const sd = sdSummary(v);
                pushUnique(signals, { label: 'Generation parameters (' + k + ')', detail: sd.params || v.slice(0, 200), prompt: sd.prompt, strong: true });
            }
            else if (AI_NAMED.test(v))
                pushUnique(signals, { label: 'Generator tool in metadata (' + k + ')', detail: v, strong: true });
        }
    }
    // 2) Raw XMP packet (covers fields exifr may not expose, e.g. GenAI extensions).
    const xmp = extractXmp(b);
    if (xmp) {
        if (DIGITAL_SOURCE_AI.test(xmp))
            pushUnique(signals, { label: 'Digital Source Type (XMP)', detail: (xmp.match(DIGITAL_SOURCE_AI) || [])[0] + '  - a standard "AI-generated" marker', strong: true, dst: true, help: DST_HELP });
        const tool = xmp.match(AI_NAMED);
        if (tool)
            pushUnique(signals, { label: 'Generator tool named in XMP', detail: tool[0], strong: true });
    }
    ...
    try {
        ...
        for (const m of manifests || []) {
            ...
            const lead = m.ai && (m.ai.find((s) => DIGITAL_SOURCE_AI.test(s)) || m.ai[0]);
            if (lead)
                pushUnique(signals, { label: 'Content Credentials declare an AI source', detail: lead + '  - see the C2PA card above', strong: true, c2pa: true, dst: true, help: CC_AI_HELP });
            else if (m.generator && AI_NAMED.test(m.generator))
                pushUnique(signals, { label: 'Content Credentials generator is an AI tool', detail: m.generator, strong: true, c2pa: true, help: CC_AI_HELP });
        }
    }
    ...
    if (!signals.some((s) => s.dst)) {
        ...
        if (at >= 0) {
            ...
            if (DIGITAL_SOURCE_AI.test(code))
                pushUnique(signals, { label: 'Digital Source Type (embedded metadata)', detail: code + '  - a standard "AI-generated" marker', strong: true, dst: true, help: DST_HELP });
        }
    }
    return signals;
}
...
export async function buildAiSignalsCard(file, exif, manifests) {
    let signals;
    try {
        signals = await collectAiSignals(file, exif, manifests);
    }
    catch (_) {
        return null;
    }
    ...
    for (const s of signals)
        tbl.appendChild(s.help ? rowHelp(s.label, s.detail, s.help) : row(s.label, s.detail));
