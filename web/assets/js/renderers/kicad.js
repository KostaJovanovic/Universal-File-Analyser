...
const kid = (node, tag) => { if (isNode(node))
    for (const x of node)
        if (isNode(x) && x[0] === tag)
...
function propVal(node, name) {
    for (const x of node || [])
        if (isNode(x) && x[0] === 'property' && x[1] === name)
            return x[2];
    return null;
}
    ...
    (function walk(node) {
        for (const x of node) {
            ...
            if (t === 'rectangle') {
                ...
                if (a && b)
                    prims.push({ kind: 'rect', x1: numAt(a, 0), y1: numAt(a, 1), x2: numAt(b, 0), y2: numAt(b, 1), fill: fillType(x) });
            }
                ...
                if (a && m && e)
                    prims.push({ kind: 'arc', x1: numAt(a, 0), y1: numAt(a, 1), xm: numAt(m, 0), ym: numAt(m, 1), x2: numAt(e, 0), y2: numAt(e, 1) });
            }
            else if (t === 'pin') {
                const p = atOf(x);
...
function parseSchematic(rootNode) {
    ...
    if (lib)
        for (const sym of kids(lib, 'symbol'))
            libs[sym[1]] = symbolGraphics(sym);
    ...
    for (const x of rootNode) {
        ...
        if (t === 'symbol') {
            ...
            const p = atOf(x);
            if (p)
                labels.push({ x: p.x, y: p.y, rot: p.rot, text: x[1], kind: t });
        }
        else if (t === 'no_connect') {
            const p = atOf(x);
            if (p)
                noconns.push(p);
        }
            ...
            const p = atOf(x);
            if (p)
                texts.push({ x: p.x, y: p.y, rot: p.rot, text: x[1] });
        }
...
function schShape(x, t) {
    if (t === 'rectangle') {
        const a = kid(x, 'start'), b = kid(x, 'end');
        return (a && b) ? { kind: 'rect', x1: numAt(a, 0), y1: numAt(a, 1), x2: numAt(b, 0), y2: numAt(b, 1), fill: fillType(x) } : null;
    }
    if (t === 'polyline' || t === 'bezier')
        return { kind: 'poly', pts: ptsOf(x), fill: fillType(x) };
    if (t === 'circle') {
        const c = kid(x, 'center');
        return { kind: 'circle', cx: numAt(c, 0), cy: numAt(c, 1), r: numAt(kid(x, 'radius'), 0), fill: fillType(x) };
    }
    if (t === 'arc') {
        const a = kid(x, 'start'), m = kid(x, 'mid'), e = kid(x, 'end');
        return (a && m && e) ? { kind: 'arc', x1: numAt(a, 0), y1: numAt(a, 1), xm: numAt(m, 0), ym: numAt(m, 1), x2: numAt(e, 0), y2: numAt(e, 1) } : null;
    }
    return null;
}
...
function paperSize(raw) {
    ...
    const name = raw[0];
    if (name === 'User') {
        const w = parseFloat(raw[1]), h = parseFloat(raw[2]);
...
function drawSheetFrame(g, W, H, parsed, b) {
    ...
    const rows = [];
    if (parsed.title)
        rows.push(['Title', parsed.title]);
    if (parsed.company)
        rows.push(['', parsed.company]);
    if (parsed.date)
        rows.push(['Date', parsed.date]);
    ...
    rows.forEach((r, i) => {
        const ry = y0 + i * rowH;
        if (i > 0)
            g.appendChild(svg('line', { x1: x0, y1: ry, x2: x0 + tbw, y2: ry, stroke: ink, 'stroke-width': 0.12 }));
...
function drawSchGraphic(g, gr, b) {
    ...
    if (gr.kind === 'rect') {
        ...
        grow(b, gr.cx - gr.r, gr.cy - gr.r);
        grow(b, gr.cx + gr.r, gr.cy + gr.r);
    }
    else if (gr.kind === 'arc') {
        const arc = arc3(gr.x1, gr.y1, gr.xm, gr.ym, gr.x2, gr.y2);
        if (arc) {
            g.appendChild(svg('path', { d: `M ${gr.x1} ${gr.y1} A ${arc.r} ${arc.r} 0 ${arc.large} ${arc.sweep} ${gr.x2} ${gr.y2}`, fill: 'none', stroke: col, 'stroke-width': W }));
            grow(b, gr.x1, gr.y1);
            grow(b, gr.x2, gr.y2);
        }
    }
...
function drawSchField(g, f, b) {
    if (f.hide || f.value == null || f.value === '' || f.value === '~')
        return;
    if (f.key === 'Reference' && /^#/.test(f.value))
        return; // power/hidden pseudo-refs
    if (f.key !== 'Reference' && f.key !== 'Value')
        return; // KiCad hides the rest by default
    ...
    if (f.rot === 90 || f.rot === 270)
        a.transform = `rotate(${-f.rot} ${f.x} ${f.y})`;
    const t = svg('text', a);
    t.textContent = f.value;
    g.appendChild(t);
    grow(b, f.x, f.y);
}
...
function schView(parsed) {
    const v = buildViewer((g) => {
        ...
        if (page)
            drawSheetFrame(g, page[0], page[1], parsed, b);
        // root-level sheet graphics (grouping boxes) - behind the circuit
        for (const gr of parsed.graphics)
            drawSchGraphic(g, gr, b);
        // wires + buses
        ...
        for (const j of parsed.junctions) {
            g.appendChild(svg('circle', { cx: j.x, cy: j.y, r: 0.5, fill: SCH.junction }));
            grow(b, j.x, j.y);
        }
        // labels
        for (const l of parsed.labels) {
            const col = l.kind === 'global_label' ? SCH.glabel : l.kind === 'hierarchical_label' ? SCH.hlabel : SCH.label;
            const t = svg('text', { x: l.x, y: l.y - 0.4, 'font-size': 1.6, fill: col });
        ...
        // free text
        for (const tx of parsed.texts) {
            const t = svg('text', { x: tx.x, y: tx.y, 'font-size': 1.6, fill: SCH.text });
            t.textContent = (tx.text || '').split('\n')[0];
            g.appendChild(t);
            grow(b, tx.x, tx.y);
        }
    ...
    for (const inst of parsed.instances)
        if (inst.ref && !/^#/.test(inst.ref))
            at.set(inst.ref.toUpperCase(), { x: inst.x, y: inst.y });
    v.focus = (ref) => { const c = at.get(String(ref).toUpperCase()); if (!c)
        return false; v.centerOn(c.x, c.y, Math.max(v.home.w * 0.22, 30)); v.flash(c.x, c.y); return true; };
    return v;
}
function drawSchPrim(g, inst, pr, b) {
    if (pr.kind === 'rect') {
        ...
        if (arc) {
            g.appendChild(svg('path', { d: `M ${a[0]} ${a[1]} A ${arc.r} ${arc.r} 0 ${arc.large} ${arc.sweep} ${e[0]} ${e[1]}`, fill: 'none', stroke: SCH.body, 'stroke-width': 0.15 }));
            grow(b, a[0], a[1]);
            grow(b, e[0], e[1]);
            grow(b, m[0], m[1]);
        }
        ...
        if (pr.hidden)
            return; // hidden pins (e.g. power) draw nothing in KiCad
    ...
    for (const x of fpNode) {
        ...
        if (t === 'pad') {
            ...
            if (drillNode) { // round: (drill D)  slot: (drill oval X Y)
                ...
                if (da[0] === 'oval') {
                    drillX = parseFloat(da[1]) || 0;
                    drillY = parseFloat(da[2]) || 0;
                }
                else
                    drillX = parseFloat(da[0]) || 0;
            }
...
function parsePcb(rootNode) {
    ...
    for (const x of rootNode) {
        ...
        if (t === 'footprint') {
            ...
            for (const pr of fp.prims) {
                prims.push(pr);
                layersUsed.add(pr.layer);
            }
            for (const pd of fp.pads) {
                pads.push(pd);
                for (const ly of pd.layers)
                    if (ly !== '*.Mask' && ly !== '*.Paste')
                        layersUsed.add(ly === '*.Cu' ? 'F.Cu' : ly);
            }
            if (fp.ref)
                footprints.push({ ref: fp.ref, value: fp.value, cx: p ? p.x : 0, cy: p ? p.y : 0 });
        }
            ...
            if (prims.length > before)
                layersUsed.add(prims[prims.length - 1].layer);
        }
        else if (t === 'segment') {
            ...
            const p = atOf(x);
            vias.push({ cx: p ? p.x : 0, cy: p ? p.y : 0, r: numAt(kid(x, 'size'), 0, 0.8) / 2, drill: numAt(kid(x, 'drill'), 0, 0.4) });
        }
        else if (t === 'zone') {
            zones++;
            for (const fp of kids(x, 'filled_polygon')) {
                const pts = ptsOf(fp);
                if (pts.length) {
                    prims.push({ kind: 'poly', layer: args(kid(fp, 'layer'))[0] || args(kid(x, 'layer'))[0] || 'F.Cu', pts, w: 0, filled: true, zone: true });
                    layersUsed.add(args(kid(fp, 'layer'))[0] || 'F.Cu');
                }
            }
        }
    }
...
function boardOutline(prims) {
    ...
    if (circ) {
        ...
        for (let i = 0; i < 72; i++) {
            ...
            pts.push([circ.cx + circ.r * Math.cos(t), circ.cy + circ.r * Math.sin(t)]);
        }
    ...
    for (const p of edges) {
        if (p.kind === 'line')
            segs.push({ a: [p.x1, p.y1], b: [p.x2, p.y2], arc: false });
        else if (p.kind === 'arc')
            segs.push({ a: [p.x1, p.y1], b: [p.x2, p.y2], mid: [p.xm, p.ym], arc: true });
        else if (p.kind === 'rect') {
    ...
    for (let guard = 0; guard < segs.length + 2 && !eq(pt, start); guard++) {
        ...
        if (found < 0)
            break; // open outline - bail (caller falls back)
        used[found] = true;
        const s = segs[found];
        pts.push(...stepPts(s, rev));
        pt = rev ? s.a : s.b;
    }
    if (!eq(pt, start))
        return null; // didn't close - not a usable silhouette
...
function paintBoard(g, pcb, opts = {}) {
    ...
    if (opts.substrate && opts.outline) {
        ...
        cp.appendChild(svg('polygon', { points: ptsStr }));
        defs.appendChild(cp);
        g.appendChild(defs);
        g.appendChild(svg('polygon', { points: ptsStr, fill: '#f4f0e6', stroke: 'rgba(40,60,40,0.55)', 'stroke-width': 0.25 }));
        host = svg('g', { 'clip-path': `url(#${clip})` });
        g.appendChild(host);
    }
    for (const p of pcb.prims)
        if (p.kind === 'poly' && p.zone)
            drawPrim(host, p, b);
    for (const p of pcb.prims)
        if (!(p.kind === 'poly' && p.zone))
            drawPrim(host, p, b);
    for (const tk of pcb.tracks)
        drawTrack(host, tk, b);
    for (const pd of pcb.pads)
        drawPad(host, pd, b, holes, labels);
    for (const vi of pcb.vias) {
        ...
        grow(b, vi.cx - vi.r, vi.cy - vi.r);
        grow(b, vi.cx + vi.r, vi.cy + vi.r);
    }
    for (const h of holes)
        drawHole(host, h); // punch holes through all copper
    for (const t of labels)
        host.appendChild(t); // pad numbers sit on top of the holes
...
function drawHole(g, h) {
    if (h.ry > 0 && Math.abs(h.ry - h.rx) > 1e-6) { // slot (oval drill)
        ...
        if (h.rot)
            n.setAttribute('transform', `rotate(${h.rot} ${h.cx} ${h.cy})`);
        g.appendChild(n);
    }
    else if (h.rx > 0) {
...
function pcbView(pcb) {
    ...
    for (const fp of pcb.footprints)
        at.set(fp.ref.toUpperCase(), { x: fp.cx, y: fp.cy });
    v.focus = (ref) => { const c = at.get(String(ref).toUpperCase()); if (!c)
        return false; v.centerOn(c.x, c.y, Math.max(v.home.w * 0.18, 12)); v.flash(c.x, c.y); return true; };
    return v;
}
...
function padSide(pd) {
    if (pd.drill > 0)
        return 'both'; // plated through-hole
    const L = pd.layers || [];
    if (L.includes('*.Cu'))
        return 'both';
    if (L.some((l) => /^B\./.test(l)) && !L.some((l) => /^F\./.test(l)))
        return 'bottom';
    return 'top';
}
...
function sidePcb(pcb, side) {
    ...
    for (const p of prims)
        layersUsed.add(p.layer);
    for (const t of tracks)
        layersUsed.add(t.layer);
    for (const pd of pads)
        for (const ly of pd.layers || [])
            if (ly !== '*.Mask' && ly !== '*.Paste')
                layersUsed.add(ly === '*.Cu' ? (side === 'bottom' ? 'B.Cu' : 'F.Cu') : ly);
...
function buildBoard3D(pcb, opts = {}) {
    ...
    for (const f of [front, back]) {
        ...
        grow(b, ob.minx, ob.miny);
        grow(b, ob.maxx, ob.maxy);
    }
    ...
    for (const d of [cam, scene, board]) {
        d.style.width = W + 'px';
        d.style.height = H + 'px';
    }
    ...
    for (let i = 0; i < rel.length; i++)
        wallSeg(rel[i], rel[(i + 1) % rel.length]);
    ...
    const spinTick = () => {
        if (!view.spin || !stage.isConnected) {
            spinRaf = 0;
            return;
        }
    ...
    stage.addEventListener('pointerdown', (e) => {
        ...
        if (e.target.closest('.anr-viewcube'))
            return;
        ...
        if (pts.size >= 2) {
            ...
            mode = (e.button === 2 || e.button === 1) ? 'pan' : 'rotate';
    });
    stage.addEventListener('pointermove', (e) => {
        ...
        if (mode === 'pinch' && pts.size >= 2) {
            ...
            view.panX += t.cx - pinch.cx;
            view.panY += t.cy - pinch.cy;
            pinch = t;
            applyCam();
        }
        else if (mode === 'pan') {
            view.panX += e.clientX - px;
            view.panY += e.clientY - py;
            applyCam();
        }
        else if (mode === 'rotate') {
            view.ry += (e.clientX - px) * 0.5;
            view.rx = Math.max(-90, Math.min(90, view.rx - (e.clientY - py) * 0.5));
            applyBoard();
        }
    });
    const dropPointer = (e) => {
        ...
        if (pts.size === 0) {
            ...
            mode = 'rotate';
            pinch = null;
        } // back to one finger: resume rotating, no jump
    };
    ...
    stage.addEventListener('wheel', (e) => { if (!wheelZoom.enabled())
        return; e.preventDefault(); view.zoom = Math.max(ZMIN, Math.min(ZMAX, view.zoom * (e.deltaY < 0 ? 1.1 : 1 / 1.1))); applyCam(); }, { passive: false });
    stage.addEventListener('dblclick', (e) => { if (e.target.closest('.anr-viewcube'))
        return; resetView(); });
    ...
    attachViewCube({
        ...
        markDirty: () => {
            ...
            applyBoard();
            if (cubeSettle)
                clearTimeout(cubeSettle);
            cubeSettle = setTimeout(() => { if (!pts.size)
                board.classList.remove('is-dragging'); }, 90);
        },
    });
    ...
    ssBtn.addEventListener('click', () => { if (opts.onToggleSS)
        opts.onToggleSS(); });
...
function boardView(pcb) {
    ...
    const build = (mode) => {
        ...
        let node;
        if (mode === 'top') {
            topV = pcbView(sidePcb(pcb, 'top'));
            node = topV.wrap;
        }
        else if (mode === 'bottom')
            node = pcbView(mirrorXPcb(sidePcb(pcb, 'bottom'))).wrap;
        else
            node = buildBoard3D(pcb, { ss: ss3d, view: view3d, onToggleSS: () => { ss3d = !ss3d; cache['3d'] = null; show('3d'); } });
...
function drawPrim(g, p, b) {
    ...
    if (p.kind === 'line') {
        ...
        grow(b, p.cx - p.r, p.cy - p.r);
        grow(b, p.cx + p.r, p.cy + p.r);
    }
    else if (p.kind === 'arc') {
        const arc = arc3(p.x1, p.y1, p.xm, p.ym, p.x2, p.y2);
        if (arc) {
            g.appendChild(svg('path', { d: `M ${p.x1} ${p.y1} A ${arc.r} ${arc.r} 0 ${arc.large} ${arc.sweep} ${p.x2} ${p.y2}`, fill: 'none', stroke: col, 'stroke-width': W, 'stroke-linecap': 'round', 'data-layer': p.layer }));
            grow(b, p.x1, p.y1);
            grow(b, p.x2, p.y2);
            grow(b, p.xm, p.ym);
        }
...
function drawTrack(g, tk, b) {
    ...
    if (tk.kind === 'line') {
        ...
        if (arc) {
            g.appendChild(svg('path', { d: `M ${tk.x1} ${tk.y1} A ${arc.r} ${arc.r} 0 ${arc.large} ${arc.sweep} ${tk.x2} ${tk.y2}`, fill: 'none', stroke: col, 'stroke-width': W, 'stroke-linecap': 'round', 'data-layer': tk.layer }));
            grow(b, tk.x1, tk.y1);
            grow(b, tk.x2, tk.y2);
        }
    }
...
function drawPad(g, pd, b, holes, labels) {
    ...
    if (copper) {
        ...
        if (/circle/.test(pd.shape)) {
            ...
            if (pd.rot)
                n.setAttribute('transform', `rotate(${pd.rot} ${pd.cx} ${pd.cy})`);
        }
        n.setAttribute('data-layer', layerKey);
        g.appendChild(n);
    }
    grow(b, pd.cx - pd.sx / 2, pd.cy - pd.sy / 2);
    grow(b, pd.cx + pd.sx / 2, pd.cy + pd.sy / 2);
    if (pd.drill > 0)
        holes.push({ cx: pd.cx, cy: pd.cy, rx: pd.drill / 2, ry: (pd.drillY || pd.drill) / 2, rot: pd.rot });
    if (pd.num && copper) {
...
function metaCard(title, help, rows, file, extra) {
    ...
    for (const [k, val, htxt] of rows)
        if (val != null && val !== '')
            tbl.appendChild(htxt ? rowHelp(k, String(val), htxt) : row(k, String(val)));
...
function helpTh(label, help) {
    ...
    if (!_thTipInit) {
        _thTipInit = true;
        document.addEventListener('click', () => document.querySelectorAll('.anr-tip.is-active').forEach((t) => t.classList.remove('is-active')));
    }
    th.appendChild(document.createTextNode(label + ' '));
    const btn = el('button', { type: 'button', class: 'anr-tip-btn', title: 'Info' }, '[?]');
    const tip = el('div', { class: 'anr-tip' }, help);
    btn.addEventListener('click', (e) => { e.stopPropagation(); const on = tip.classList.contains('is-active'); document.querySelectorAll('.anr-tip.is-active').forEach((t) => t.classList.remove('is-active')); if (!on)
...
export async function renderKicad(file, resultsEl) {
    ...
    resultsEl.innerHTML = '';
    try {
        if (ext === 'kicad_pro' || ext === 'kicad_prl')
            return renderProjectJson(file, text, resultsEl);
        if (lower === 'fp-info-cache')
            return renderFpCache(file, text, resultsEl);
        if (lower === 'fp-lib-table' || lower === 'sym-lib-table')
            return renderLibTable(file, text, resultsEl);
        if (ext === 'wbk')
            return renderWbk(file, text, resultsEl);
        const node = parseSexpr(text);
        const rootTag = tagOf(node);
        if (ext === 'kicad_sch' || rootTag === 'kicad_sch')
            return renderSchDoc(file, node, resultsEl);
        if (ext === 'kicad_pcb' || rootTag === 'kicad_pcb')
            return renderPcbDoc(file, node, resultsEl);
        if (ext === 'kicad_mod' || rootTag === 'footprint')
            return renderMod(file, node, resultsEl);
        if (ext === 'kicad_sym' || rootTag === 'kicad_symbol_lib' || ext === 'bak')
            return renderSymLib(file, node, resultsEl);
...
function renderSchDoc(file, node, resultsEl) {
    ...
    if (bom.length)
        resultsEl.appendChild(bomCard(bom, null, null));
}
...
function renderMod(file, node, resultsEl) {
    ...
    if (fp.pads.length) {
        ...
        c.appendChild(el('h3', {}, 'Pads'));
        c.appendChild(padsTable(fp.pads));
...
function renderSymLib(file, node, resultsEl) {
    ...
    if (!syms.length) {
        resultsEl.appendChild(el('div', { class: 'anr-info' }, 'No symbols found.'));
        return;
    }
    const card = el('div', { class: 'anr-card' });
    ...
    for (const sym of syms)
        sel.appendChild(el('option', { value: sym[1] }, sym[1]));
...
function renderProjectJson(file, text, resultsEl) {
    ...
    if (nets && nets.length) {
        ...
        for (const nc of nets)
            tbl.appendChild(el('tr', {}, [el('td', {}, nc.name || '—'), el('td', {}, fmtNum(nc.track_width)), el('td', {}, fmtNum(nc.clearance)), el('td', {}, fmtNum(nc.via_diameter))]));
        card.appendChild(tbl);
...
function renderFpCache(file, text, resultsEl) {
    ...
    if (items.length) {
        ...
        for (const it of items.slice(0, 200))
            tbl.appendChild(el('tr', {}, [el('td', {}, it.nick || '—'), el('td', {}, it.name || '—'), el('td', {}, /^\d+$/.test(it.pads || '') ? it.pads : '—')]));
        card.appendChild(tbl);
        if (items.length > 200)
            card.appendChild(el('p', { class: 'anr-hint' }, `… and ${items.length - 200} more.`));
        resultsEl.appendChild(card);
    }
}
function renderLibTable(file, text, resultsEl) {
    ...
    if (libs.length) {
        ...
        for (const lib of libs)
            tbl.appendChild(el('tr', {}, [el('td', {}, args(kid(lib, 'name'))[0] || '—'), el('td', {}, args(kid(lib, 'type'))[0] || '—'), el('td', {}, args(kid(lib, 'uri'))[0] || '—')]));
        card.appendChild(tbl);
        resultsEl.appendChild(card);
    }
}
...
function bomCard(bom, onSch, onPcb) {
    ...
    for (const inst of bom) {
        ...
        if (onSch) {
            desigBtn.title = 'Show ' + inst.ref + ' on the schematic';
            desigBtn.addEventListener('click', () => onSch(inst.ref));
        }
        else
            desigBtn.disabled = true;
        const cells = [
            el('td', {}, desigBtn),
            el('td', {}, inst.value || '—'),
...
export async function buildKicadProjectCard(kiFiles, folderName) {
    ...
    for (const f of kiFiles) {
        ...
        if (ext === 'kicad_pro')
            ...
            if (!schFile || f.path.length < schFile.path.length)
                schFile = f;
        }
            ...
            modFiles.push(f);
        else if (lower === 'fp-lib-table' || lower === 'sym-lib-table')
            tableFiles.push(f);
        else
            otherFiles.push(f);
    }
    ...
    if (schFile) {
        try {
            sch = parseSchematic(parseSexpr(await schFile.file.text()));
        }
        catch (_) { }
    }
    if (pcbFile) {
        try {
            pcb = parsePcb(parseSexpr(await pcbFile.file.text()));
        }
        catch (_) { }
    }
    if (proFile) {
        try {
            pro = JSON.parse(await proFile.file.text());
        }
        catch (_) { }
    }
    ...
    card.appendChild(el('p', { class: 'anr-hint', style: 'margin:0 0 12px;' }, 'Every document in this project, opened together.'));
    ...
    function setActive(i) {
        ...
        if (t && !t.built) {
            t.built = true;
            t.view = t.build(t.panel) || null;
        }
    }
    const overviewIdx = addTab('Overview', (panel) => buildOverview(panel));
    let schTab = -1, pcbTab = -1;
    if (sch)
        schTab = addTab(schFile ? base(schFile.path) : 'Schematic', (panel) => {
    ...
    if (pcb)
        pcbTab = addTab(pcbFile ? base(pcbFile.path) : 'PCB', (panel) => {
            ...
            const v = boardView(pcb);
            panel.appendChild(v.wrap);
            return v;
        });
    if (symFile)
        addTab(base(symFile.path), (panel) => { buildSymPanel(panel, symFile); });
    if (modFiles.length)
        addTab('Footprints (' + modFiles.length + ')', (panel) => buildModPanel(panel, modFiles));
    function crossSch(ref) { if (schTab < 0)
        return; setActive(schTab); const v = tabs[schTab].view; if (v && v.focus)
        v.focus(ref); tabsBar.scrollIntoView({ block: 'nearest' }); }
    function crossPcb(ref) { if (pcbTab < 0)
        return; setActive(pcbTab); const v = tabs[pcbTab].view; if (v && v.focus)
        v.focus(ref); tabsBar.scrollIntoView({ block: 'nearest' }); }
    function buildOverview(panel) {
        ...
        if (bom.length)
            panel.appendChild(bomCard(bom, schTab >= 0 ? crossSch : null, pcbTab >= 0 ? crossPcb : null));
        else
            panel.appendChild(el('div', { class: 'anr-info' }, 'No BOM components were found in the schematic.'));
        // library tables
        ...
        for (const f of kiFiles)
            filesT.appendChild(row(roleOf(base(f.path)), base(f.path)));
    }
    function buildSymPanel(panel, sf) {
        ...
        sf.file.text().then((t) => {
            ...
            const draw = (nm) => { const sym = syms.find((s) => s[1] === nm); host.innerHTML = ''; if (!sym)
                return; const parsed = { libs: { [nm]: symbolGraphics(sym) }, instances: [{ libId: nm, x: 0, y: 0, rot: 0, mirror: '', ref: '' }], wires: [], buses: [], junctions: [], labels: [], noconns: [], texts: [] }; host.appendChild(schView(parsed).wrap); };
    ...
    function buildModPanel(panel, mods) {
        const sel = el('select', { class: 'anr-btn anr-kicad-select' });
        for (const m of mods)
            sel.appendChild(el('option', { value: m.path }, base(m.path).replace(/\.kicad_mod$/i, '')));
        panel.appendChild(sel);
        const host = el('div', { class: 'anr-kicad-symhost' });
        panel.appendChild(host);
        const draw = (p) => {
            ...
            mf.file.text().then((t) => {
                ...
                if (fp.pads.length)
                    host.appendChild(padsTable(fp.pads));
...
function roleOf(n) {
    ...
    if (ext === 'stp' || ext === 'step')
        return '3D model';
    if (lower === 'fp-lib-table')
        return 'Footprint lib table';
    if (lower === 'sym-lib-table')
        return 'Symbol lib table';
    if (lower === 'fp-info-cache')
