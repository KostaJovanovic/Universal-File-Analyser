...
export function categorizeExt(ext) {
    if (!ext)
        return 'other';
    ext = ext.toLowerCase();
    if (PHOTO_EXTS.has(ext) || ext === 'psd' || ext === 'svg')
        return 'photo';
...
export function renderBreakdownCards(items, resultsEl, extraSummaryRows) {
    ...
    if (extraSummaryRows) {
        for (const r of extraSummaryRows)
            tbl.appendChild(r);
    }
    ...
    for (const cat of CATEGORIES) {
        ...
        if (d.count)
            catParts.push(d.count + ' ' + CATEGORY_LABELS[cat].toLowerCase());
    }
    tbl.appendChild(rowHelp('Categories', catParts.join(', ') || '-', 'A breakdown of the contents grouped by media kind: photos, audio, video, documents, archives, and other.'));
    tbl.appendChild(rowHelp('Unique extensions', String(breakdown.sorted.length), 'How many different file types appear, counted by their extension - the bit after the dot in a name, like .jpg or .pdf.'));
    ...
    if (breakdown.sorted.length) {
        ...
        for (const [ext, data] of visible)
            extTbl.appendChild(fmtExtRow(ext, data));
        ...
        if (hidden.length) {
            ...
            for (const [ext, data] of hidden)
                hiddenTbl.appendChild(fmtExtRow(ext, data));
...
export function renderViewToggle(container, items, treeObj, treeOpts, onFileClick, opts) {
    ...
    const place = (node) => { if (slot && slot.parentNode === container)
        container.insertBefore(node, slot);
    else
    ...
    function openExtFilterModal() {
        ...
        for (const [ext, data] of bk.sorted) {
            chipRow.appendChild(mkChip(ext, [fmtExtLabel(ext), el('span', { class: 'anr-extchip-n' }, String(data.count))], data.count + (data.count === 1 ? ' file' : ' files') + ' · ' + fmtBytes(data.size)));
        }
...
//# sourceMappingURL=folder-archive-shared.js.map