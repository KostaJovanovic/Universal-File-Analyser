...
function parseRaw(buf) {
    ...
    if (!(nVarsClaimed > 0) || !(nPoints > 0))
        throw new Error('missing variable/point count');
    ...
    if (nVarsClaimed > SPICE_VARS_MAX || nVarsClaimed > Math.floor(buf.length / 4))
        throw new Error('the header claims ' + nVarsClaimed.toLocaleString() + ' variables, more than this file could hold');
    ...
    if (nVars * nPoints > maxValues)
        nPoints = Math.max(1, Math.floor(maxValues / nVars));
    ...
    if (varsStart < 0 || !mm)
        throw new Error('no variables/data marker');
    ...
    for (const line of varsBlock.split('\n')) {
        ...
        const parts = t.split(/[ \t]+/);
        if (parts.length >= 3)
            vars.push({ idx: +parts[0], name: parts[1], type: parts[2].toLowerCase() });
        else if (parts.length === 2)
            vars.push({ idx: +parts[0], name: parts[1], type: '' });
    }
    while (vars.length < nVars)
        vars.push({ idx: vars.length, name: 'v' + vars.length, type: '' });
    ...
    if (isBinary) {
        ...
        if (nPoints < 1)
            throw new Error('no sample data after the header');
        ...
        for (let p = 0; p < nPoints; p++) {
            ...
            if (scheme === 'cplx-double') {
                ...
                for (let v = 1; v < nVars; v++)
                    data[v][p] = dv.getFloat32(o + 8 + (v - 1) * 4, true);
            }
            else if (scheme === 'real-float') {
                o = p * nVars * 4;
                for (let v = 0; v < nVars; v++)
                    data[v][p] = dv.getFloat32(o + v * 4, true);
            }
            else {
                o = p * nVars * 8;
                for (let v = 0; v < nVars; v++)
                    data[v][p] = dv.getFloat64(o + v * 8, true);
            }
        }
    }
    else {
        ...
        const readNum = (s) => {
            ...
            if (complex) {
                const c = s.split(',');
                return Math.hypot(parseFloat(c[0]), parseFloat(c[1] || '0'));
            }
            return parseFloat(s);
        };
        while (p < nPoints && li < toks.length) {
            ...
            for (let v = 1; v < nVars; v++) {
                while (li < toks.length && !toks[li].trim())
                    li++;
        ...
        if (p < nPoints && p > 0) {
            nPoints = p;
            data = data.map((d) => d.subarray(0, p));
        }
    }
...
function buildPlot(parsed) {
    ...
    if (!traces.some((t) => t.on))
        traces.forEach((t, i) => { t.on = i < 4; });
    ...
    wrap.appendChild(el('p', { class: 'anr-hint' }, 'X axis: ' + (xType || 'sweep') + (xUnit ? ' (' + xUnit + ')' : '') + '. Click a name to show/hide a trace; hover the plot to read off values.'));
...
export async function renderSpiceRaw(file, resultsEl) {
    ...
    const [h, help] = h3help('SPICE simulation (.raw)', 'The saved results of a circuit simulation (a SPICE ".raw" waveform dump), produced by tools such as KiCad / ngspice or LTspice - it records how the voltages and currents change during the simulated run. '
    ...
    if (m.plotname)
        tbl.appendChild(rowHelp('Analysis', m.plotname, 'The kind of simulation run that produced this data - for example transient (how signals change over time), AC (frequency response), DC sweep (stepping a source through a range of values) or operating point (the steady resting state).'));
    if (m.title)
        tbl.appendChild(row('Title', m.title));
    if (m.date)
        tbl.appendChild(row('Date', m.date));
    if (m.flags)
        tbl.appendChild(rowHelp('Flags', m.flags, 'Whether each recorded value is a single ("real") number, used for time and DC data, or a pair of numbers ("complex", a real part and an imaginary part), used for AC frequency data.'));
    ...
    if (parsed.truncated) {
        tbl.appendChild(rowHelp('Data points', parsed.nPoints.toLocaleString() + '  (header declares ' + parsed.pointsClaimed.toLocaleString() + ')', 'The file says the simulation recorded more points than the file itself actually holds, so the run was cut short or the file is damaged. Analyser plots the points that are really there.'));
    }
    else {
    ...
    if (!isOp && parsed.x && parsed.x.length > 1) {
        ...
        tbl.appendChild(row(xt === 'frequency' ? 'Frequency range' : xt === 'time' ? 'Duration' : 'Sweep span', fmtSI(parsed.x[0], unitFor(xt)) + ' → ' + fmtSI(parsed.x[parsed.x.length - 1], unitFor(xt))
    ...
    if (isOp) {
        ...
        opCard.appendChild(oph);
        opCard.appendChild(ophp);
