...
function decodeThumb(thumb) {
    ...
    return new Promise((done) => {
        ...
        im.onload = () => {
            ...
            try {
                frac = emptyFraction(ctx, cv.width, cv.height);
            }
            catch (_) { }
            ...
            if (isJpeg && frac < 0.99) {
                try {
                    corrupt = detectCorruptCut(ctx.getImageData(0, 0, cv.width, cv.height).data, cv.width, cv.height) >= 0;
                }
                catch (_) { }
            }
            cleanup();
            if (isJpeg && frac >= 0.99) {
                salvage();
                return;
            } // browser drew nothing - salvage a strip or the thumbnail
...
export function emptyFraction(ctx, w, h) {
    try {
        ...
        for (let i = 0; i < d.length; i += 4) {
            ...
            if (Math.abs(d[i] - 128) <= 2 && Math.abs(d[i + 1] - 128) <= 2 && Math.abs(d[i + 2] - 128) <= 2)
                fill++;
        }
        return fill / total;
    }
        ...
        return 0;
    } // tainted/unreadable - assume it drew
}
...
export function createCarveGallery() {
    ...
    function add({ file, format, width, height, complete, onAnalyse }) {
        ...
        const thumb = el('div', { class: 'anr-carve-thumb is-plain' }, el('span', { class: 'anr-hint', style: 'margin:0;' }, fmt));
        ...
        cell.appendChild(el('div', { class: 'anr-carve-cap' }, fmt + (dims ? '  ' + dims : '') + (complete === false ? '  · partial' : '')));
        io.observe(thumb);
...
//# sourceMappingURL=carve-gallery.js.map