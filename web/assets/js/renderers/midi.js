...
export async function renderMidi(file, resultsEl) {
    ...
    try {
        ...
        resultsEl.appendChild(errorCard('Could not read this MIDI file.'));
        return;
    }
    ...
    if (division & 0x8000)
        smpte = { fps: 256 - (division >> 8), tpf: division & 0xFF };
    else
        ppq = division;
    ...
    for (let t = 0; t < ntrks && pos + 8 <= buf.length; t++) {
        ...
        while (p < endTrk) {
            ...
            if (status === 0xFF) {
                ...
                p += mlen;
                if (type === 0x51 && mlen === 3)
                    tempos.push({ tick, us: (data[0] << 16) | (data[1] << 8) | data[2] });
                else if (type === 0x58 && mlen >= 2 && !timeSig)
                    timeSig = data[0] + '/' + (1 << data[1]);
                else if (type === 0x59 && mlen >= 2 && !keySig)
                    keySig = data;
                else if (type === 0x03) {
                    const n = dec.decode(data).trim();
                    if (n)
                        trackNames.push(n);
                }
                else if (type === 0x04) {
    ...
    // ---- Info card ----
    const [h, help] = h3help('MIDI file', 'A Standard MIDI File stores a musical score (notes, tempo, instruments) - not sampled audio - so there is no waveform or spectrogram to play.');
    ...
    tbl.appendChild(rowHelp('Division', smpte ? (smpte.fps + ' fps · ' + smpte.tpf + ' ticks/frame') : (ppq + ' ticks/quarter'), 'How finely the timing is measured - the number of ticks (tiny time steps) in each quarter-note, or SMPTE film frames.'));
    tbl.appendChild(rowHelp('Tempo', Math.round(initialBpm) + ' BPM' + (tempos.length > 1 ? ' (' + tempos.length + ' changes)' : ''), 'The starting tempo, taken from the first tempo instruction in the file (defaults to 120 BPM if none is given).'));
    if (timeSig)
        tbl.appendChild(row('Time signature', timeSig));
    ...
    if (hasDrums)
        instrs.push('Drum kit (channel 10)');
    if (instrs.length) {
