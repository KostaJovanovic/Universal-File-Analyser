...
function decodeLpwstr(bytes) { try {
    return new TextDecoder('utf-16le').decode(bytes).replace(/\0+$/, '').trim();
}
catch (_) {
    return '';
} }
function decodeLpstr(bytes) { try {
    ...
    try {
        ...
        for (let i = 0; i < numProps; i++) {
            ...
            if (PID[pid] && (type === 30 || type === 31)) { // VT_LPSTR / VT_LPWSTR
                ...
                if (val && !out.fields[PID[pid]])
                    out.fields[PID[pid]] = val;
            }
...
function appendTail(file, resultsEl, encrypted) {
    ...
    note.appendChild(nh);
    note.appendChild(nhelp);
    note.appendChild(el('p', { class: 'anr-hint', style: 'margin:0;' }, 'To open the actual model, export it from SolidWorks as STEP, STL or 3MF and drop that here for the full 3D viewer.'));
...
export async function renderSolidworks(file, resultsEl) {
    ...
    if (!cf) {
        ...
        t.appendChild(rowHelp('Container', 'Encrypted (SolidWorks 2015+)', 'The underlying file structure that stores the SolidWorks document. Versions from 2015 onward encrypt it, so the file can be identified but not opened here.'));
    ...
    t.appendChild(rowHelp('Container', 'OLE2 / CFBF v' + cf.version, 'The underlying file structure that stores the SolidWorks document - an OLE2 / Compound File Binary container, the same wrapper older Microsoft Office files use.'));
    ...
    for (const [k, v] of Object.entries(sum.fields))
    ...
    if (pngStream && isPng(pngStream))
        preview = { bytes: pngStream, mime: 'image/png' };
    else if (sum.thumb)
        preview = sum.thumb;
    if (preview) {
