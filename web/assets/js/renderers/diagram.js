...
function labelOf(cell) {
    ...
    if (/[<&]/.test(v)) {
        v = new DOMParser().parseFromString(v, 'text/html').body.textContent || '';
    }
...
function renderMxModel(model) {
    ...
    if (!verts.length)
        return { svg: null, verts: 0, edges: edges.length };
    ...
    for (const e of edges) {
        ...
        if (e.label)
            body += '<text x="' + ((s.cx + t.cx) / 2) + '" y="' + ((s.cy + t.cy) / 2) + '" class="anr-dg-elabel">' + esc(e.label) + '</text>';
    }
    for (const v of verts) {
        const st = v.style;
        if (/ellipse/.test(st))
            body += '<ellipse cx="' + v.cx + '" cy="' + v.cy + '" rx="' + (v.w / 2) + '" ry="' + (v.h / 2) + '" class="anr-dg-shape"/>';
        else if (/rhombus/.test(st))
            body += '<polygon points="' + [[v.cx, v.y], [v.x + v.w, v.cy], [v.cx, v.y + v.h], [v.x, v.cy]].map((p) => p.join(',')).join(' ') + '" class="anr-dg-shape"/>';
        else {
            const rnd = /rounded=1/.test(st) ? ' rx="8"' : '';
            body += '<rect x="' + v.x + '" y="' + v.y + '" width="' + v.w + '" height="' + v.h + '"' + rnd + ' class="anr-dg-shape"/>';
        }
        if (v.label)
            body += '<text x="' + v.cx + '" y="' + v.cy + '" class="anr-dg-label">' + esc(v.label.slice(0, 60)) + '</text>';
    }
...
export async function renderDrawio(file, container) {
    ...
    try {
        ...
        if (!diagrams.length) {
            container.appendChild(errorCard('No diagram pages found in this file.'));
            return;
        }
        let drewAny = false;
        ...
        for (let i = 0; i < diagrams.length; i++) {
            ...
            if (!model) {
                container.insertBefore(el('div', { class: 'anr-card' }, [el('h3', {}, name), el('p', { class: 'anr-hint' }, 'Could not decode this page.')]), _renderAnchor);
                continue;
            }
...
function parseDxfEntities(text) {
    ...
    for (let i = 0; i < pairs.length; i++) {
        if (pairs[i][0] === 2 && (pairs[i][1] || '').trim() === 'ENTITIES') {
            start = i + 1;
            break;
        }
    }
    ...
    for (let i = start; i < pairs.length; i++) {
        if (pairs[i][0] === 0 && (pairs[i][1] || '').trim() === 'ENDSEC') {
            end = i;
            break;
        }
    }
    ...
    for (let i = start; i < end; i++) {
        ...
        if (code === 0) {
            ...
            const v = (val || '').trim();
            if (code === 10)
                cur.xs.push(parseFloat(v));
            else if (code === 20)
                cur.ys.push(parseFloat(v));
            else if (code === 11) {
                cur.x2 = parseFloat(v);
            }
            else if (code === 21) {
...
function dxfToSvg(entities) {
    let minX = Infinity, minY = Infinity, maxX = -Infinity, maxY = -Infinity;
    const see = (x, y) => { if (Number.isFinite(x) && Number.isFinite(y)) {
    ...
    for (const e of entities) {
        ...
        if (t === 'LINE' && e.xs.length && e.x2 != null) {
            ...
            for (let s = 0; s <= steps; s++) {
                ...
                const x = e.xs[0] + r * Math.cos(a), y = e.ys[0] + r * Math.sin(a);
                see(x, y);
                pts.push(x + ',' + Y(y));
            }
            ...
            const pts = [];
            for (let k = 0; k < e.xs.length && k < e.ys.length; k++) {
                see(e.xs[k], e.ys[k]);
                pts.push(e.xs[k] + ',' + Y(e.ys[k]));
            }
            const closed = (parseInt(e.codes[70], 10) || 0) & 1;
            if (pts.length)
                draw.push('<' + (closed ? 'polygon' : 'polyline') + ' points="' + pts.join(' ') + '" class="anr-dxf-line"/>');
        }
        else if ((t === 'TEXT' || t === 'MTEXT') && e.xs.length) {
            const h = parseFloat(e.codes[40]) || 4;
            see(e.xs[0], e.ys[0]);
            const txt = (e.codes[1] || '').replace(/\\[A-Za-z][^;]*;?/g, '').slice(0, 80);
            if (txt)
                draw.push('<text x="' + e.xs[0] + '" y="' + Y(e.ys[0]) + '" font-size="' + h + '" class="anr-dxf-text">' + esc(txt) + '</text>');
        }
    ...
    if (!Number.isFinite(minX) || !draw.length)
        return { svg: null, counts };
...
export async function renderDxf(file, container) {
    ...
    try {
        ...
        container.appendChild(info);
        if (r.svg)
            container.insertBefore(svgCard('Drawing preview', r.svg), container.firstChild);
        else
            container.appendChild(el('div', { class: 'anr-card' }, [el('h3', {}, 'Preview'), el('p', { class: 'anr-hint' }, entities.length ? 'No drawable geometry in the ENTITIES section (it may use blocks/inserts only).' : 'No entities found to draw.')]));
        if (file.size <= HASH_FILE_MAX)
            container.appendChild(integrityCard(file));
    }
    catch (e) {
