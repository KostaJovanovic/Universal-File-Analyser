...
function cp1252(bytes) {
    if (!dec1252) {
        try {
            dec1252 = new TextDecoder('windows-1252');
        }
        catch (_) {
            dec1252 = new TextDecoder('latin1');
        }
    }
...
function utf16le(bytes) {
    if (!decUtf16)
        decUtf16 = new TextDecoder('utf-16le');
...
function readShortStr(dv, bytes, off) {
    ...
    let text;
    if (high) {
        text = utf16le(bytes.subarray(p, p + cch * 2));
        p += cch * 2;
    }
    else {
        text = cp1252(bytes.subarray(p, p + cch));
        p += cch;
    }
    return { text, next: p };
}
...
function parseSST(records, startIdx) {
    ...
    try {
        ...
        for (let i = startIdx + 1; i < records.length; i++) {
            if (records[i].type === 0x003C)
                bufs.push(records[i].data);
            else
                break;
        }
        let bi = 0, bo = 0;
        const advanceBuf = () => { bi++; bo = 0; };
        const readU8 = () => { while (bi < bufs.length && bo >= bufs[bi].length)
            advanceBuf(); const v = bufs[bi][bo]; bo++; return v; };
        ...
        const readChars = (nChars, high) => {
            ...
            while (left > 0) {
                while (bi < bufs.length && bo >= bufs[bi].length) {
                    advanceBuf();
                    if (bi < bufs.length)
                        high = bufs[bi][bo++] & 0x01; // continuation grbit
                }
                if (bi >= bufs.length)
                    break;
        ...
        for (let k = 0; k < cstUnique; k++) {
            ...
            for (let r = 0; r < cRun * 4; r++)
                readU8(); // skip rich-text runs
            for (let e = 0; e < cbExt; e++)
                readU8(); // skip phonetic ext data
        }
    }
...
function extractXlsSheets(cf) {
    ...
    for (let i = 0; i < records.length; i++)
        recAtPos.set(records[i].start - 4, i);
    ...
    for (let s = 0; s < sheetList.length; s++) {
        ...
        for (let i = startRec; i < records.length; i++) {
            const r = records[i];
            if (i > startRec && r.type === 0x0809)
                break; // next substream BOF
            if (i > startRec && r.type === 0x000A)
                break; // EOF
            const d = dvOf(r.data);
            if (r.type === 0x00FD) { // LABELSST
                ...
                try {
                    put(u16(d, 0), u16(d, 2), readShortStr(d, r.data, 6).text);
                }
                catch (_) { }
            }
                ...
                let off = 4, c = cFirst;
                while (c <= cLast && off + 6 <= r.data.length) {
                    put(rr, c, fmtNum(decodeRK(u32(d, off + 2))));
                    off += 6;
                    c++;
                }
            }
                ...
                if (r.data[6] === 0 && r.data[12] === 0xFF && r.data[13] === 0xFF) {
                    ...
                    if (nxt && nxt.type === 0x0207) {
                        try {
                            put(rr, cc, readShortStr(dvOf(nxt.data), nxt.data, 0).text);
                        }
                        catch (_) { }
                    }
                }
...
function scanUrls(bytes, set) {
    if (!bytes || !bytes.length || set.size > 500)
        return;
...
function infoCard(file, appLabel, extraRows, links) {
    ...
    card.appendChild(el('p', { class: 'anr-hint', style: 'font-size:12px;margin:10px 0 0;' }, 'Content is extracted best-effort - original fonts, styling, images and exact layout are not reconstructed.'));
    return card;
}
export async function renderLegacyOffice(file, container, kind) {
    ...
    try {
        ...
        try {
            ...
            for (const sname of scanStreams)
                scanUrls(cf.readStream(sname), urlSet);
        }
        catch (_) { /* ignore */ }
        const links = [...urlSet];
        if (macros)
            extraRows.push(['Macros', '⚠ Contains macros (VBA project)']);
        if (links.length)
            extraRows.push(['External links', String(links.length)]);
...
//# sourceMappingURL=legacy-office.js.map