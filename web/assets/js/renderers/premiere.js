...
function parsePremiere(xml) {
    ...
    for (const e of doc.getElementsByTagName('*')) {
        const uid = e.getAttribute('ObjectUID');
        if (uid)
            byUid.set(uid, e);
    ...
    const derefId = (id, pred) => { const a = id != null && byId.get(id); if (!a)
        return null; return pred ? (a.find(pred) || null) : a[0]; };
    ...
    const mediaPath = (mc) => {
        for (const c of kids(kid(mc, 'Clips'), 'Clip')) {
            ...
            if (p && /[\\/]|\.\w{2,5}$/.test(p))
                return p.replace(/^file:\/+/, '/'); // skip synthetic numeric "paths"
        }
        return '';
    };
    ...
    for (const seqEl of doc.getElementsByTagName('Sequence')) {
        ...
        for (const tg of groups) {
            ...
            if (kind === 'video' && tpf)
                seq.fps = seq.fps || TPS / tpf;
            const rect = txt(tg, 'FrameRect').split(','); // "left,top,right,bottom"
            if (kind === 'video' && rect.length === 4) {
                seq.w = seq.w || num(rect[2]);
                seq.h = seq.h || num(rect[3]);
            }
            ...
            trackEls.forEach((ct, ti) => {
                ...
                for (const it of itemEls) {
                    ...
                    if (!(end > start) || end - start > MAX_CLIP_S)
                        continue; // skip empty / sentinel spans
                ...
                if (clips.length)
                    seq.tracks.push({ kind, idx: isFinite(idx) ? idx : ti, clips });
            });
        }
        ...
        if (!isFinite(seq.dur))
            seq.dur = 0.01; // a NaN / infinite clip end would poison the zoom maths
...
function trackLanesSvg(tracks, dur, H, trackW, pps) {
    ...
    tracks.forEach((t, i) => {
        ...
        t.clips.forEach((c) => {
            ...
            if (bw > 46)
                bars += `<text x="${bx + 5}" y="${y + LH / 2 + 4}" fill="#fff" font-size="9.5" opacity=".95" pointer-events="none" clip-path="inset(0 0 0 0)">${esc(c.file.slice(0, Math.max(3, Math.floor(bw / 6.5))))}</text>`;
        });
    });
...
export async function renderPremiere(file, resultsEl) {
    ...
    tbl.appendChild(rowHelp('Format', isPrel ? 'Premiere Elements project (.prel)' : 'Premiere Pro project (.prproj)', 'A Premiere Pro project (.prproj) is an XML document squeezed down with gzip compression (the PremiereData model). Analyser unpacks it and reads the sequence, track and clip objects to rebuild each timeline.'));
    if (data.projVer)
        tbl.appendChild(rowHelp('Project version', data.projVer, 'The internal version number of the project data (stored in the PremiereData model). It moves in step with the Premiere release that last saved the file, so it points to that version.'));
    if (data.dataVer)
        tbl.appendChild(row('Model version', data.dataVer));
    ...
    if (data.mediaItems)
        tbl.appendChild(row('Media items', String(data.mediaItems)));
    ...
    for (const s of seqs.slice(0, MAX_CARDS))
        resultsEl.insertBefore(buildSequenceTimeline(s), _renderAnchor);
    if (seqs.length > MAX_CARDS) {
        resultsEl.appendChild(el('div', { class: 'anr-card' }, el('p', { class: 'anr-hint', style: 'margin:0' }, `… and ${seqs.length - MAX_CARDS} more sequences not shown.`)));
    }
    // ---- Legend ----
    const [legH, legP] = h3help('Legend', 'Each timeline zooms with ctrl/⌘ + scroll (or the zoom buttons) and pans by dragging. '
    ...
    resultsEl.appendChild(el('div', { class: 'anr-card' }, [
        legH, legP,
        el('p', { class: 'anr-hint', html: 'Each bar is a clip, positioned by its in and out point on the sequence timeline. '
    ...
    if (data.clipNames.length) {
        ...
        if (data.clipNames.length > 200)
            ul.appendChild(el('li', { class: 'anr-hint' }, '… and ' + (data.clipNames.length - 200) + ' more'));
        card.appendChild(ul);
        if (data.mediaPaths.length) {
            ...
            if (data.mediaPaths.length > 200)
                ul2.appendChild(el('li', { class: 'anr-hint' }, '… and ' + (data.mediaPaths.length - 200) + ' more'));
