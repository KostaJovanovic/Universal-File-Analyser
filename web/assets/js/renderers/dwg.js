...
function sniffPreview(data) {
    if (data.length < 16)
        return null;
    if (data[0] === 0x89 && data[1] === 0x50 && data[2] === 0x4E && data[3] === 0x47) {
        return { blob: new Blob([data], { type: 'image/png' }), label: 'PNG', bytes: data.length };
    }
    if (data[0] === 0x42 && data[1] === 0x4D) { // already a complete .bmp
        return { blob: new Blob([data], { type: 'image/bmp' }), label: 'BMP', bytes: data.length };
    }
    const bmp = dibToBmp(data);
    if (bmp)
        return { blob: new Blob([bmp], { type: 'image/bmp' }), label: 'BMP', bytes: data.length };
...
function dwgPreviews(bytes, baseName) {
    const items = [];
    let skipped = 0;
    ...
    if (bytes.length > 0x11) {
        ...
        if (seek > 0 && seek + IMAGE_SENTINEL.length <= bytes.length &&
            IMAGE_SENTINEL.every((b, i) => bytes[seek + i] === b))
            at = seek;
    }
    if (at < 0)
        at = findBytes(bytes, IMAGE_SENTINEL);
    if (at < 0)
    ...
    p += 1;
    if (!count || count > 16)
        return { items, skipped }; // a real directory is 1-3 entries
    for (let i = 0; i < count; i++) {
        ...
        p += 9;
        if (code === 1)
            continue; // header data, not an image
        if (!size || size > PREVIEW_CARVE_MAX)
            continue;
...
function previewCard(file, found, resultsEl) {
...
export async function renderDwg(file, resultsEl) {
    ...
    let found = { items: [], skipped: 0 };
    try {
        found = dwgPreviews(new Uint8Array(buf), baseName);
    }
    catch (_) { /* no preview section */ }
    let db = null, svg = '', ptr = null;
    try {
        ptr = inst.dwg_read_data(buf, FT.DWG);
        if (ptr == null)
            throw new Error('not a readable DWG file');
        db = inst.convert(ptr);
        try {
            svg = inst.dwg_to_svg(db);
        }
        catch (_) {
            svg = '';
        }
    ...
    if (layers && layers.length != null)
        tbl.appendChild(rowHelp('Layers', String(layers.length), 'Named layers the drawing is organised into. Like transparent overlays, each groups related elements (walls, dimensions, text and so on) so they can be shown, hidden or styled together.'));
    ...
    if (topTypes)
        tbl.appendChild(rowHelp('Entity types', topTypes, 'The most common kinds of drawing element (lines, arcs, circles and so on) found in the main drawing area (model space).'));
