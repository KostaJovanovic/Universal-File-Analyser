...
function quantizeFrame(data) {
    ...
    for (const [key, count] of hist)
        uniq.push({ r: (key >> 16) & 255, g: (key >> 8) & 255, b: key & 255, count });
    ...
    let minCodeSize = 2;
    while ((1 << minCodeSize) < usedColors)
        minCodeSize++;
...
export function encodeAnimatedGif(width, height, frames, delaysCs, loop) {
    ...
    const str = (s) => { for (let i = 0; i < s.length; i++)
...
//# sourceMappingURL=gif-encode.js.map