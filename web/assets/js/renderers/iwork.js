...
export async function renderIwork(file, resultsEl) {
    ...
    if (pdfEntry) {
        ...
        try {
            ...
            host.remove();
            subhead.remove(); /* fall through to image / no-preview */
        }
    }
        ...
        if (bytes) {
            ...
            analyse.addEventListener('click', () => {
                if (window._anrHandleFile)
                    window._anrHandleFile(new File([bytes], 'iwork-preview.' + e, { type: mime }), { nested: true });
            });
    ...
    resultsEl.appendChild(el('div', { class: 'anr-info' }, 'This iWork file was saved without an embedded preview, so only its metadata can be shown.'));
    resultsEl.appendChild(integrityCard(file));
}
//# sourceMappingURL=iwork.js.map