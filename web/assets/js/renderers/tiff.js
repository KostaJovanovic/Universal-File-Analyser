...
export function tiffPages(buf) {
    ...
    if (magic !== 0x002A)
        return null; // 0x002B = BigTIFF, not walked here
    ...
    while (ifd && ifd + 2 <= n && !seen.has(ifd) && pages.length < 4096) {
        ...
        for (let i = 0; i < count; i++) {
            ...
            if (tag !== 256 && tag !== 257)
                continue; // ImageWidth / ImageLength
...
export async function buildTiffPagesCard(file, signal, container) {
    let pages = null;
    try {
        pages = tiffPages(await file.arrayBuffer());
    }
    catch (_) {
        pages = null;
    }
    if (!pages || pages.length < 2)
        return null; // single-page TIFF: nothing extra to show
