...
function largestMatch(zip, re) {
    ...
    for (const e of zip.entries) {
        if (re.test(e.name) && (!best || e.uncompSize > best.uncompSize))
            best = e;
    }
    return best;
}
...
function appendPreview(resultsEl, bytes, ext, heading) {
    ...
    analyse.addEventListener('click', () => {
        if (window._anrHandleFile)
            window._anrHandleFile(new File([bytes], 'preview.' + ext, { type: mime }), { nested: true });
    });
...
async function renderKra(file, zip, resultsEl) {
    ...
    if (mainEntry) {
        try {
            const b = await zip.bytes(mainEntry.name);
            if (b)
                main = parseKraMain(new TextDecoder().decode(b));
        }
        catch (_) { }
    }
    ...
    if (main) {
        if (main.width && main.height)
            tbl.appendChild(row('Canvas', main.width + ' × ' + main.height + ' px'));
        if (main.colorspace)
            tbl.appendChild(rowHelp('Colour space', main.colorspace, 'The colour model the artwork is painted in, shown as Krita’s own code - for example ‘RGBA’ (red-green-blue plus transparency) or ‘CMYKA’ (the print inks plus transparency).'));
        if (main.xres)
            tbl.appendChild(row('Resolution', Math.round(parseFloat(main.xres)) + ' dpi'));
        if (main.layers)
            tbl.appendChild(rowHelp('Layers', String(main.layers), 'The artwork is built from stacked layers, like sheets of transparent film. Hidden or switched-off layers can still hold content that is not visible in the flattened preview.'));
        if (main.kritaVersion)
            tbl.appendChild(row('Krita version', main.kritaVersion));
    }
    ...
    if (png) {
        ...
        if (bytes) {
            appendPreview(resultsEl, bytes, 'png', /merged/i.test(png.name) ? 'Merged image' : 'Preview');
            return;
        }
    }
...
function findSizeToken(node, seen) {
    ...
    if (typeof node === 'string')
        return /^\{\s*\d+(\.\d+)?\s*,\s*\d+(\.\d+)?\s*\}$/.test(node) ? node : '';
...
async function renderProcreate(file, zip, resultsEl) {
    ...
    if (archive) {
        try {
            ...
            if (parsed)
                size = findSizeToken(parsed.value, new WeakSet());
        }
        catch (_) { }
    }
    const card = el('div', { class: 'anr-card' });
    ...
    if (size)
        tbl.appendChild(row('Canvas', size.replace(/[{}]/g, '').replace(/\s+/g, '').replace(',', ' × ') + ' px'));
    tbl.appendChild(row('Size', fmtBytes(file.size)));
    card.appendChild(tbl);
    resultsEl.appendChild(card);
    ...
    if (thumb) {
        ...
        if (bytes) {
            appendPreview(resultsEl, bytes, 'png', 'Preview');
            return;
        }
    }
...
async function renderPdn(file, resultsEl) {
    ...
    let head;
    try {
        head = new Uint8Array(await file.slice(0, 7).arrayBuffer());
    }
    catch (_) {
        head = null;
    }
    ...
    let xml = '';
    try {
        xml = new TextDecoder('utf-8').decode(new Uint8Array(await file.slice(7, 7 + xmlLen).arrayBuffer()));
    }
    catch (_) { }
    ...
    if (width && height)
        tbl.appendChild(row('Canvas', width + ' × ' + height + ' px'));
    if (layers)
        tbl.appendChild(rowHelp('Layers', layers, 'The artwork is built from stacked layers, like sheets of transparent film. Hidden or switched-off layers can still hold content that is not visible in the flattened preview.'));
    if (ver)
        tbl.appendChild(row('Saved with version', ver));
    tbl.appendChild(row('Size', fmtBytes(file.size)));
    card.appendChild(tbl);
    resultsEl.appendChild(card);
    ...
    if (m) {
        try {
            appendPreview(resultsEl, b64ToBytes(m[1]), 'png', 'Preview');
            return;
        }
        catch (_) { }
    }
...
export async function renderPaint(file, resultsEl) {
    ...
    if (ext === 'pdn')
        return renderPdn(file, resultsEl);
    let zip;
    try {
    ...
    resultsEl.innerHTML = '';
    if (ext === 'kra')
        return renderKra(file, zip, resultsEl);
