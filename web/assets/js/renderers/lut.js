...
function parseCubeLut(text) {
    ...
    for (let raw of lines) {
        ...
        if (up.startsWith('TITLE')) {
            const m = line.match(/"([^"]*)"/);
            lut.title = m ? m[1] : line.slice(5).trim();
            continue;
        }
        ...
        if (up.startsWith('DOMAIN_MIN')) {
            lut.domainMin = line.split(/\s+/).slice(1, 4).map(Number);
            continue;
        }
        if (up.startsWith('DOMAIN_MAX')) {
            lut.domainMax = line.split(/\s+/).slice(1, 4).map(Number);
            continue;
        }
        if (up.startsWith('LUT_3D_INPUT_RANGE') || up.startsWith('LUT_1D_INPUT_RANGE')) {
            const p = line.split(/\s+/).map(Number);
            lut.domainMin = [p[1], p[1], p[1]];
            lut.domainMax = [p[2], p[2], p[2]];
            continue;
        }
    ...
    if (!lut.type || !(lut.size > 1))
        return null; // not a LUT (Gaussian cube, etc.)
...
function buildLookStack(stages) {
    ...
    const [h, help] = h3help('Grade stack', 'The grading stages this look applies, in order, as built in Adobe SpeedGrade / Premiere Lumetri. Stages left at their defaults are marked so; the baked 3D LUT is the whole stack flattened into one table.');
    card.appendChild(h);
    ...
    for (const op of lookNamedStages(stages)) {
        ...
        let val;
        if (op.name === 'LUT')
            val = 'embedded 3D LUT (shown below)';
        else if (op.visible === '0')
            val = 'hidden';
        else if (active.length)
            val = active.map(([k, v]) => prettyLookParam(k) + ' ' + fmtLookNum(v)).join(', ');
        else
            val = 'default';
...
function hslToRgb(h, s, l) {
    ...
    const p = 2 * l - q;
    const hue = (t) => { t = (t % 1 + 1) % 1; if (t < 1 / 6)
...
function toneCurveSvg(sample, W, H) {
    ...
    for (let c = 0; c < 3; c++)
        g += `<path d="${path(ch[c])}" fill="none" stroke="${cols[c]}" stroke-width="1.8" stroke-linejoin="round"/>`;
...
function buildCubePair(lut, sample) {
    ...
    for (let z = 0; z < n; z += step)
        for (let y = 0; y < n; y += step)
            for (let x = 0; x < n; x += step) {
                ...
                px[i] = inR - 0.5;
                py[i] = 0.5 - inG;
                pz[i] = inB - 0.5; // y flipped (screen y grows down)
    ...
    function paint(buf, cols, half) {
        ...
        for (const [a, b] of edges)
            line(buf, cp[a][0], cp[a][1], cp[b][0], cp[b][1]);
        for (let k = 0; k < M; k++) {
    ...
    function drawAll() {
        ...
        paint(bufN, colN, half);
        ctxN.putImageData(imgN, 0, 0);
        paint(bufL, colL, half);
        ctxL.putImageData(imgL, 0, 0);
    }
    let raf = 0;
    const queue = () => { if (!raf)
        raf = requestAnimationFrame(() => { raf = 0; drawAll(); }); };
    ...
    const attach = (cv) => {
        cv.addEventListener('pointerdown', (e) => { drag = true; lx = e.clientX; ly = e.clientY; cv.style.cursor = 'grabbing'; try {
        ...
        const end = (e) => { drag = false; cv.style.cursor = 'grab'; try {
            cv.releasePointerCapture(e.pointerId);
        }
        catch (_) { } };
        cv.addEventListener('pointerup', end);
        cv.addEventListener('pointercancel', end);
        cv.addEventListener('wheel', (e) => { if (!wheelZoom.enabled())
            return; e.preventDefault(); state.zoom = Math.max(0.5, Math.min(6, state.zoom * (e.deltaY < 0 ? 1.12 : 1 / 1.12))); queue(); }, { passive: false });
        cv.addEventListener('touchmove', (e) => {
            if (e.touches.length === 2) {
                e.preventDefault();
                const d = Math.hypot(e.touches[0].clientX - e.touches[1].clientX, e.touches[0].clientY - e.touches[1].clientY);
                if (pinch) {
                    state.zoom = Math.max(0.5, Math.min(6, state.zoom * d / pinch));
                    queue();
                }
                pinch = d;
            }
...
function seekVideo(v, t) {
    return new Promise((res) => { const on = () => { v.removeEventListener('seeked', on); res(); }; v.addEventListener('seeked', on); try {
        v.currentTime = t;
    }
    catch (_) {
        res();
...
async function loadSampleFrame(maxW) {
    ...
    if (!resp.ok)
        throw new Error('sample image unavailable (' + resp.status + ')');
...
function itemUrl(it) {
    if (!it.url) {
        ...
        c.width = it.img.width;
        c.height = it.img.height;
...
function buildTryout(sample) {
    const card = el('div', { class: 'anr-card' });
    const [h, help] = h3help('See the look', 'Your file is handled on your device - nothing is uploaded. A video is sampled at 8 equally spaced frames at full resolution. Click any frame to open it full-size, then step through with the arrows (or ← / → keys).');
    card.appendChild(h);
    card.appendChild(help);
    card.appendChild(el('p', { class: 'anr-hint', style: 'margin:0 0 10px' }, 'This LUT applied to a sample photo, shown original next to graded. Drop in your own photo or video to replace it.'));
    ...
    function openLightbox(start) {
        ...
        if (!overlay) {
            ...
            const close = () => { if (overlay._backClose)
                overlay._backClose();
            else
                overlay._hide(); };
        ...
        overlay._show = show;
        overlay._prev = () => { if (overlay._i > 0)
            show(overlay._i - 1); };
        overlay._next = () => { if (overlay._i < items.length - 1)
            show(overlay._i + 1); };
    ...
    function renderGrid(frames) {
        ...
        frames.forEach((f, fi) => {
            ...
            const cell = [pair];
            if (f.t != null)
                cell.unshift(el('div', { class: 'anr-lut-cap', style: 'margin:0 0 3px' }, tcLabel(f.t)));
    ...
    input.addEventListener('change', async () => {
        ...
        try {
            ...
            status.textContent = 'Could not process this file: ' + ((e && e.message) || e);
        }
        input.value = '';
    });
...
export async function renderLut(file, resultsEl) {
    ...
    let text;
    try {
        text = await file.slice(0, 64 * 1024 * 1024).text();
    }
    catch (e) {
        resultsEl.innerHTML = '';
    ...
    if (look)
        tbl.appendChild(rowHelp('Grade stages', lookNamedStages(look.stages).length + ' stages', 'The number of separate colour-grading steps stored in this look (exposure, contrast, colour balance and so on), applied one after another.'));
    if (lut.title)
        tbl.appendChild(row('Title', lut.title));
    tbl.appendChild(rowHelp('Type', lut.type === '3D' ? '3D LUT (full colour cube)' : '1D LUT (per-channel curve)', 'A LUT (look-up table) is a colour recipe. A 3D LUT can remap any red/green/blue mix, so it can shift both colour and richness; a 1D LUT only adjusts each colour channel on its own, like a simple brightness curve.'));
    tbl.appendChild(rowHelp('Grid size', lut.type === '3D' ? `${lut.size} x ${lut.size} x ${lut.size}  (${lut.count.toLocaleString()} entries)` : `${lut.size} points`, 'How finely the colour table is sampled. Colours that fall between the stored points are blended smoothly from the nearest ones (trilinear interpolation).'));
    if (!lut.complete)
        tbl.appendChild(rowHelp('Entries', lut.count.toLocaleString() + ' of ' + lut.expected.toLocaleString() + ' (truncated)', 'The table holds fewer entries than it says it should, so the file looks incomplete or cut off.'));
    if (lut.domainMin.some((v, i) => v !== 0 || lut.domainMax[i] !== 1))
        tbl.appendChild(rowHelp('Input domain', `[${lut.domainMin.join(', ')}] - [${lut.domainMax.join(', ')}]`, 'The range of input values the LUT expects for each colour channel, normally 0 to 1 (black to white). A different range means it was built for a special input such as log or HDR footage.'));
    if (lut.range[0] < -0.001 || lut.range[1] > 1.001)
        tbl.appendChild(rowHelp('Output range', lut.range[0].toFixed(3) + ' - ' + lut.range[1].toFixed(3), 'Values reaching outside the normal 0 to 1 range mean this is an extended-range LUT, meant for HDR or scene-linear footage.'));
    if (lut.comments.length)
        tbl.appendChild(row('Source', lut.comments.slice(0, 3).join(' · ')));
    ...
    const [trH, trHelp] = h3help('Tone response (neutral axis)', 'How the LUT reshapes a plain grey ramp running from black to white. The dashed line means no change; a curve bowing above it brightens those tones, below it darkens them. Any gap between the red, green and blue curves is the colour tint the LUT adds.');
    curveCard.appendChild(trH);
    curveCard.appendChild(trHelp);
    ...
    const [baH, baHelp] = h3help('Before / after', 'A hue x brightness field and a neutral grey ramp, shown straight (left) and pushed through this LUT (right).');
    chartCard.appendChild(baH);
    chartCard.appendChild(baHelp);
    const cW = 300, cH = 200;
    ...
    if (lut.type === '3D') {
        ...
        const [ccH, ccHelp] = h3help('Colour cube', 'The left cube is the untouched RGB space; the right is the same lattice recoloured by this LUT, so the difference is exactly what the LUT does. Each point sits at its input R/G/B position.');
        cubeCard.appendChild(ccH);
        cubeCard.appendChild(ccHelp);
        cubeCard.appendChild(el('p', { class: 'anr-hint', style: 'margin:0 0 10px' }, 'Drag either cube to rotate both, scroll (or pinch) to zoom in and look inside.'));
