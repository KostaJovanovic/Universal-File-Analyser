...
export function openDocLightbox(pages, startIndex, label) {
    ...
    function show(idx) {
        ...
        if (overlay._zoomBtn)
            overlay._zoomBtn.textContent = 'Zoom';
    }
...
export function pagedPreviewCard(pages, opts = {}) {
    ...
    function reveal(upTo) {
        for (; shown < upTo && shown < pages.length; shown++)
            addPage(shown);
...
export function pagedTextCard(pageTexts, opts = {}) {
    ...
    function reveal(upTo) {
        for (; shown < upTo && shown < total; shown++)
            block(shown);
        if (shown >= total) {
            btnRow.hidden = true;
        }
            ...
            btnRow.hidden = false;
            moreBtn.textContent = 'Show next ' + batch + ' (' + shown + '/' + total + ')';
        }
    }
    moreBtn.addEventListener('click', () => reveal(shown + batch));
