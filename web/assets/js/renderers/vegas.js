...
function utf16Runs(buf, min) {
    ...
    for (let i = 0; i + 1 < buf.length; i += 2) {
        ...
        if (hi === 0 && ((lo >= 0x20 && lo < 0x7f) || lo === 0x09)) {
            if (!cur.length)
                start = i;
            cur.push(String.fromCharCode(lo));
        }
        else {
            if (cur.length >= min)
                out.push({ off: start, s: cur.join('') });
            cur = [];
        }
    }
    if (cur.length >= min)
...
function parseVegas(buf) {
    ...
    if (anchor >= 0) {
        ...
        for (let i = anchor - 1; i >= 0 && u16[i + 1].off - u16[i].off <= 100; i--)
            block.unshift(u16[i]);
        for (let i = anchor + 1; i < u16.length && u16[i].off - u16[i - 1].off <= 100; i++)
            block.push(u16[i]);
        const seenS = new Set();
        for (const r of block) {
            const s = r.s.trim();
            if (!s || s.length < 3 || seenS.has(s) || /^(Preview|Master|Default|Video|Audio)$/i.test(s))
                continue;
            seenS.add(s);
            let label = '';
            if (/@[\w.-]+\.\w{2,}/.test(s))
                label = 'Contact';
            else if (/all rights reserved/i.test(s))
                label = 'Rights';
            else if (/(©|\(c\)|copyright|\b(19|20)\d\d\b)/i.test(s))
                label = 'Copyright';
            else if (/^https?:|\.(com|net|org|tv|io)\b/i.test(s) && !/\s/.test(s))
                label = 'Website';
...
export async function renderVegas(file, resultsEl) {
    ...
    let buf;
    try {
        buf = new Uint8Array(await file.slice(0, MAX_BYTES).arrayBuffer());
    }
    catch (e) {
        resultsEl.innerHTML = '';
    ...
    tbl.appendChild(rowHelp('Format', isVf ? 'VEGAS Movie Studio project (.vf)' : 'VEGAS Pro project (.veg)', 'A Sonic Foundry project file wrapper (their "RIFF GUID" format) - identified by a lowercase "riff" tag at the start followed by a 16-byte unique id (a GUID) that names the file type. Analyser reads the text, plugin ids and other information stored inside.'));
    if (data.version)
        tbl.appendChild(rowHelp('Version', data.version, 'Worked out from a settings-folder path saved inside the project (for example ...\\Vegas Pro\\11.0\\). It shows the version of VEGAS that wrote the file.'));
    ...
    if (data.texts.length)
        tbl.appendChild(rowHelp('Text events', String(data.texts.length), 'In VEGAS an "event" is a clip placed on the timeline; a text event is a title or caption placed this way.'));
