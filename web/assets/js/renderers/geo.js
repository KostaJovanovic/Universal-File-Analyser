    ...
    for (const pts of tracks) {
        ...
        for (const p of pts) {
            ...
            if (isFinite(p.ele)) {
                ...
                if (delta >= ELE_THRESHOLD) {
                    ascent += delta;
                    refEle = p.ele;
                }
                else if (delta <= -ELE_THRESHOLD) {
                    descent += -delta;
                    refEle = p.ele;
                }
...
function elevationProfileCanvas(profile) {
    ...
    ctx.beginPath();
    ctx.moveTo(x(profile[0].dist), y(profile[0].ele));
    for (const p of profile)
        ctx.lineTo(x(p.dist), y(p.ele));
    ctx.lineTo(x(profile[profile.length - 1].dist), H - padB);
    ctx.lineTo(x(profile[0].dist), H - padB);
    ctx.closePath();
    ctx.fillStyle = 'rgba(68,95,116,0.15)';
    ctx.fill();
    // line
    ctx.beginPath();
    ctx.moveTo(x(profile[0].dist), y(profile[0].ele));
...
function ele(g, v) { if (isFinite(v)) {
    g.eleMin = Math.min(g.eleMin, v);
    g.eleMax = Math.max(g.eleMax, v);
} }
function tstamp(g, t) { const ms = Date.parse(t); if (!isNaN(ms)) {
    g.timeStart = g.timeStart == null ? ms : Math.min(g.timeStart, ms);
    g.timeEnd = g.timeEnd == null ? ms : Math.max(g.timeEnd, ms);
} }
function parseGpx(xml) {
    ...
    const segPts = (nodes) => {
        ...
        for (const pt of nodes) {
            ...
            const eVal = eNode ? parseFloat(eNode.textContent) : NaN;
            if (eNode)
                ele(g, eVal);
            const tNode = pt.querySelector('time');
            const tMs = tNode ? Date.parse(tNode.textContent) : NaN;
            if (tNode)
                tstamp(g, tNode.textContent);
            detail.push({ lat, lon, ele: eVal, time: isNaN(tMs) ? NaN : tMs,
                hr: extVal(pt, 'hr'), cad: extVal(pt, 'cad'), temp: extVal(pt, 'atemp') });
        }
        if (detail.length)
            g.tracks.push(detail);
        return line;
    };
    xml.querySelectorAll('trkseg').forEach((seg) => { const l = segPts(seg.querySelectorAll('trkpt')); if (l.length) {
        g.lines.push(l);
        bump(g, 'track segments');
    } });
    xml.querySelectorAll('rte').forEach((r) => { const l = segPts(r.querySelectorAll('rtept')); if (l.length) {
        g.lines.push(l);
        bump(g, 'routes');
    } });
    xml.querySelectorAll('wpt').forEach((w) => {
        ...
        g.markers.push({ lat, lon, name: nm ? nm.textContent.trim() : '' });
        g.pointCount++;
        bump(g, 'waypoints');
    });
    return g;
...
function parseExtendedData(pm) {
    const ed = pm.querySelector('ExtendedData');
    if (!ed)
        return null;
    const out = {};
    ed.querySelectorAll('Data').forEach((d) => {
        ...
        if (k && v)
            out[k] = v.textContent.trim();
    });
    ed.querySelectorAll('SimpleData').forEach((d) => {
        const k = d.getAttribute('name');
        if (k)
            out[k] = d.textContent.trim();
    });
...
function parseKml(xml) {
    ...
    xml.querySelectorAll('Placemark').forEach((pm) => {
        ...
        if (extended)
            g.features.push({ name, props: extended });
        ...
        pm.querySelectorAll('Point coordinates').forEach((c) => {
            ...
            if (pts.length) {
                g.markers.push({ lat: pts[0][0], lon: pts[0][1], name });
                g.pointCount++;
                bump(g, 'points');
            }
        });
...
function parseGeoJson(text) {
    ...
    const walk = (geom, name) => {
        ...
        switch (geom.type) {
            case 'Point': {
                const p = ll(c);
                g.markers.push({ lat: p[0], lon: p[1], name });
                g.pointCount++;
                bump(g, 'points');
                break;
            }
            case 'MultiPoint':
                c.forEach((p) => { const x = ll(p); g.markers.push({ lat: x[0], lon: x[1], name }); g.pointCount++; });
                bump(g, 'points');
                break;
            case 'LineString': {
                const line = c.map(ll);
                g.lines.push(line);
                g.pointCount += line.length;
                bump(g, 'lines');
                break;
            }
            case 'MultiLineString':
                c.forEach((l) => { const line = l.map(ll); g.lines.push(line); g.pointCount += line.length; });
                bump(g, 'lines');
                break;
            case 'Polygon':
                c.forEach((ring) => { const line = ring.map(ll); g.lines.push(line); g.pointCount += line.length; });
                bump(g, 'polygons');
                break;
            case 'MultiPolygon':
                c.forEach((poly) => poly.forEach((ring) => { const line = ring.map(ll); g.lines.push(line); g.pointCount += line.length; }));
                bump(g, 'polygons');
                break;
            case 'GeometryCollection':
                (geom.geometries || []).forEach((gg) => walk(gg, name));
                break;
        }
    };
    features.forEach((f) => {
...
export async function renderGeo(file, resultsEl) {
    ...
    try {
        if (ext === 'geojson' || (/^\s*[{[]/.test(text) && ext !== 'gpx' && ext !== 'kml')) {
            ...
            if (ext === 'kml' || xml.querySelector('kml, Placemark')) {
                format = 'KML';
                g = parseKml(xml);
            }
            else {
    ...
    for (const line of g.lines)
        for (let i = 1; i < line.length; i++)
            distance += haversine(line[i - 1], line[i]);
    ...
    for (const [k, v] of Object.entries(g.counts))
        tbl.appendChild(row(k.charAt(0).toUpperCase() + k.slice(1), String(v)));
    tbl.appendChild(row('Total points', g.pointCount.toLocaleString()));
    if (distance > 0)
        tbl.appendChild(row('Distance', fmtDist(distance)));
    if (isFinite(g.eleMin))
        tbl.appendChild(row('Elevation', Math.round(g.eleMin) + ' – ' + Math.round(g.eleMax) + ' m'));
    if (g.timeStart != null) {
        ...
        if (span > 0)
            tbl.appendChild(row('Duration', span >= 3600 ? (span / 3600).toFixed(1) + ' h' : Math.round(span / 60) + ' min'));
    }
    ...
    try {
        if (g.tracks && g.tracks.length) {
            ...
            if (ts.ascent >= 1 || ts.descent >= 1) {
                tbl.appendChild(rowHelp('Total ascent', Math.round(ts.ascent) + ' m', 'How much you climbed in total - every uphill height gain along the route added together. Tiny changes under 2 m are ignored as GPS jitter.'));
                tbl.appendChild(row('Total descent', Math.round(ts.descent) + ' m'));
            }
            if (ts.hasTime && ts.movingTime > 0) {
                tbl.appendChild(rowHelp('Moving time', fmtDuration(ts.movingTime), 'How long you were actually moving, with pauses left out - any gap longer than 60 seconds between recorded points counts as a stop.'));
                if (ts.movingDist > 0) {
            ...
            if (ts.hr)
                tbl.appendChild(row('Heart rate', Math.round(ts.hr.avg) + ' avg, ' + Math.round(ts.hr.max) + ' max bpm'));
            if (ts.cad)
                tbl.appendChild(row('Cadence', Math.round(ts.cad.avg) + ' avg, ' + Math.round(ts.cad.max) + ' max'));
            if (ts.temp)
                tbl.appendChild(row('Temperature', ts.temp.avg.toFixed(1) + ' avg, ' + ts.temp.max.toFixed(1) + ' max °C'));
        }
    }
    catch (e) {
        ts = null;
    ...
    if (hasGeo) {
        tbl.appendChild(rowHelp('Bounds', minLat.toFixed(4) + ', ' + minLon.toFixed(4) + '  →  ' + maxLat.toFixed(4) + ', ' + maxLon.toFixed(4), 'The smallest rectangle on the map that contains every point, given as its south-west corner → its north-east corner.'));
    }
    ...
    try {
        if (ts && ts.profile && ts.profile.length > 1) {
            ...
            elevCard.appendChild(eh);
            elevCard.appendChild(ehelp);
    ...
    try {
        if (g.features && g.features.length) {
            ...
            propCard.appendChild(ph);
            propCard.appendChild(phelp);
    ...
    try {
        ...
        mapEl.innerHTML = '';
        mapEl.appendChild(errorCard('Map library failed to load. Offline?'));
        return;
    }
    mapEl.innerHTML = '';
