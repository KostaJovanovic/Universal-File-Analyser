...
export async function detectMoovlessMp4(reader, size) {
    ...
    for (let guard = 0; guard < 1024 && pos + 8 <= size; guard++) {
        ...
        if (type === 'mdat')
            mdat = { start: pos + hsize, declaredEnd: pos + boxSize };
        pos += boxSize;
    }
    if (hasMoov || !mdat)
        return null;
...
export async function extractMp4ParamSets(reader, size) {
    const det = await (async () => {
        ...
        for (let guard = 0; guard < 1024 && pos + 8 <= size; guard++) {
            ...
            if (boxSize < 8)
                break;
            if (type === 'moov')
                return { start: pos + hsize, end: Math.min(pos + boxSize, size) };
            pos += boxSize;
        }
        return null;
    })();
    ...
    if (avcc) {
        ...
        p += 5;
        const nals = [];
        const numSps = moov[p] & 0x1f;
        p += 1;
        for (let i = 0; i < numSps && p + 2 <= avcc.dataEnd; i++) {
            const l = dv.getUint16(p);
            p += 2;
            nals.push(moov.slice(p, p + l));
            p += l;
        }
        const numPps = p < avcc.dataEnd ? moov[p] : 0;
        p += 1;
        for (let i = 0; i < numPps && p + 2 <= avcc.dataEnd; i++) {
            const l = dv.getUint16(p);
            p += 2;
            nals.push(moov.slice(p, p + l));
            p += l;
    ...
    if (hvcc) {
        ...
        for (let a = 0; a < numArrays && p + 3 <= hvcc.dataEnd; a++) {
            ...
            p += 2;
            for (let i = 0; i < numNalus && p + 2 <= hvcc.dataEnd; i++) {
                const l = dv.getUint16(p);
                p += 2;
                nals.push(moov.slice(p, p + l));
                p += l;
...
export async function carveAvccToAnnexB(reader, mdatStart, mdatEnd, opts = {}) {
    ...
    const nalAt = (buf, n, i) => {
        ...
        if (L < 1 || L > MAX_NAL || i + lenSize + L > n)
            return 0;
        const hb = buf[i + lenSize];
        if (hb & 0x80)
            return 0; // forbidden_zero_bit must be 0
    ...
    while (pos < mdatEnd) {
        ...
        while (i < limit) {
            ...
            if (span && (chainOk(buf, n, i, 3) || (atEof && chainOk(buf, n, i, 1)))) {
                ...
                out[op + 3] = 1;
                op += 4;
                out.set(buf.subarray(i + lenSize, i + lenSize + L), op);
                op += L;
                nals++;
                bytes += 4 + L;
                types[t] = (types[t] || 0) + 1;
        ...
        if (onProgress)
            onProgress((pos - mdatStart) / (mdatEnd - mdatStart), { nals, bytes });
    }
    if (onProgress)
        onProgress(1, { nals, bytes });
...
export async function findInbandParamSets(reader, mdatStart, mdatEnd, opts = {}) {
    ...
    const found = new Map();
    const readLen = (i) => { let v = 0; for (let k = 0; k < lenSize; k++)
        v = v * 256 + buf[i + k]; return v; };
    const nalAt = (i) => {
        if (i + lenSize + 1 > n)
            return 0;
        const L = readLen(i);
        if (L < 1 || L > 24 * 1024 * 1024 || i + lenSize + L > n)
            return 0;
        if (buf[i + lenSize] & 0x80)
            return 0;
    ...
    while (i + lenSize + 1 <= n) {
        ...
        if (span) {
            ...
            const L = span - lenSize;
            if (want.includes(t) && !found.has(t) && L < 4096)
                found.set(t, buf.slice(i + lenSize, i + lenSize + L));
            i += span;
            if (want.every((t2) => found.has(t2)))
                break;
        }
        else
            i += 1;
