...
import { parseHevcSps, parseAvcSps, stripEpb, COLOUR_PRIMARIES, TRANSFER_CHARS, MATRIX_COEFFS } from './video-bitstream.js';
...
function walkBoxes(view, start, end) {
    ...
    while (pos + 8 <= end) {
        ...
        if (size < headerSize || pos + size > end + 8)
            break;
...
function metaChildStart(view, off, end) {
    ...
    if (size >= 8 && off + size <= end && IS_ALPHA_FCC(type))
        return off;
    return off + 4;
}
...
function buildChildren(view, box, base, depth) {
    ...
    if (box.type === 'meta')
        childStart = metaChildStart(view, childStart, childEnd);
    const nodes = [];
    for (const c of walkBoxes(view, childStart, childEnd)) {
        ...
        if (CONTAINER_BOXES.has(c.type) && depth < 16) {
            try {
                node.children = buildChildren(view, c, base, depth + 1);
            }
            catch (_) {
                node.children = null;
            }
    ...
    while (stack.length) {
        ...
        for (const b of walkBoxes(view, s, e)) {
            ...
            if (CONTAINER_BOXES.has(b.type)) {
                ...
                if (b.type === 'meta')
                    cs = metaChildStart(view, cs, b.start + b.size);
...
async function readTimecode(file, view, trakStart, trakEnd) {
    ...
    try {
        ...
        if (buf.byteLength >= 4)
            frame = new DataView(buf).getUint32(0);
    }
    catch (_) {
        return null;
    }
...
function computeGopMap(view, trakStart, trakEnd, timescale, fileSize) {
    ...
    const total = Math.min(stsz.sizes ? stsz.sizes.length : stsz.count, stsz.sizes ? Infinity : Math.floor(fileSize / Math.max(1, stsz.fixed)), MP4_SAMPLE_TABLE_MAX);
    if (!total || !timescale)
        return null;
    ...
    let si = 0;
    for (const e of stts) {
        for (let k = 0; k < e.count && si < total; k++)
            durations[si++] = e.delta;
    }
    while (si < total)
        durations[si++] = stts.length ? stts[stts.length - 1].delta : 0;
    ...
    let keyframes;
    if (sync)
        keyframes = [...sync].sort((a, b) => a - b);
    else
        keyframes = null; // all-intra
    ...
    if (keyframes) {
        ...
        for (let i = 1; i < keyframes.length; i++)
            maxGop = Math.max(maxGop, keyframes[i] - keyframes[i - 1]);
...
async function topLevelBoxes(file) {
    ...
    while (pos + 8 <= max && guard++ < 4096) {
        let hdr;
        try {
            hdr = new DataView(await file.slice(pos, pos + 16).arrayBuffer());
        }
        catch (_) {
            break;
        }
...
export async function analyzeMp4Structure(file) {
    ...
    if (!moovBox || moovBox.size > 64 * 1024 * 1024)
        return result;
    // Full track + GOP parse from the moov buffer (buffer-local coordinates).
    let moovView;
    try {
        moovView = new DataView(await file.slice(moovBox.start, moovBox.start + moovBox.size).arrayBuffer());
    }
    catch (_) {
        return result;
    }
    ...
    try {
        ...
        if (mvhd) {
            ...
            if (ts > 0 && dur > 0)
                result.movieDurationSec = dur / ts;
        }
    }
    catch (_) { }
    ...
    for (let ti = 0; ti < traks.length; ti++) {
        ...
        try {
            ...
            let handler = null;
            if (hdlr)
                handler = fourcc(moovView, hdlr.start + hdlr.headerSize + 8);
            ...
            if (mdhdBox) {
                ...
                if (mdhd.timescale > 0)
                    track.durationSec = mdhd.duration / mdhd.timescale;
            }
            ...
            if (stszBox)
                track.sampleCount = readStsz(moovView, stszBox).count;
            ...
            if (handler === 'tmcd') {
                try {
                    ...
                    if (tc) {
                        track.timecode = tc.timecode;
                        track.dropFrame = tc.dropFrame;
                    }
                }
                catch (_) { }
            }
            ...
            if (handler === 'vide' && !videoTrakForGop) {
                videoTrakForGop = trak;
                videoTimescale = track.timescale || 0;
            }
        }
    ...
    if (videoTrakForGop && videoTimescale) {
        try {
            result.gop = computeGopMap(moovView, videoTrakForGop.start + videoTrakForGop.headerSize, videoTrakForGop.start + videoTrakForGop.size, videoTimescale, file.size);
        }
        catch (_) { }
    }
    return result;
...
function spsFacts(nal, h265) {
    ...
    if (!parsed)
        return null;
    const colour = {};
    if (parsed.primaries != null)
        colour.primaries = parsed.primaries;
    if (parsed.transfer != null)
        colour.transfer = parsed.transfer;
    if (parsed.matrix != null)
        colour.matrix = parsed.matrix;
    if (parsed.fullRange !== undefined)
        colour.fullRange = parsed.fullRange;
    return {
...
function readParamSets(view, box, codec) {
    ...
    if (codec === 'avc') {
        ...
        p += 1;
        for (let i = 0; i < numSps && p + 2 <= end; i++) {
            const l = view.getUint16(p);
            p += 2;
            sps.push(new Uint8Array(view.buffer, view.byteOffset + p, l));
            p += l;
        }
        return { lenSize, sps };
    }
    ...
    let p = d + 22;
    const numArrays = view.getUint8(p);
    p += 1;
    for (let a = 0; a < numArrays && p + 3 <= end; a++) {
        const nalType = view.getUint8(p) & 0x3f;
        p += 1;
        const numNalus = view.getUint16(p);
        p += 2;
        for (let i = 0; i < numNalus && p + 2 <= end; i++) {
            const l = view.getUint16(p);
            p += 2;
            if (nalType === 33)
                sps.push(new Uint8Array(view.buffer, view.byteOffset + p, l));
...
function containerVideoFacts(view, ts, te, seRange) {
    ...
    if (stsd) {
        ...
        if (dim + 4 <= view.byteLength) {
            facts.width = view.getUint16(dim);
            facts.height = view.getUint16(dim + 2);
        }
    }
    ...
    if (colr) {
        ...
        if (fourcc(view, d) === 'nclx' && d + 11 <= view.byteLength) {
            facts.primaries = view.getUint16(d + 4);
            facts.transfer = view.getUint16(d + 6);
            facts.matrix = view.getUint16(d + 8);
    ...
    if (mdhdBox && sttsBox) {
        ...
        let ticks = 0, count = 0;
        for (const e of stts) {
            ticks += e.count * e.delta;
            count += e.count;
        }
        if (mdhd.timescale > 0 && ticks > 0)
            facts.fps = mdhd.timescale * count / ticks;
    }
    return facts;
}
...
async function readFirstSample(file, view, ts, te) {
    ...
    try {
        return new Uint8Array(await file.slice(off, off + want).arrayBuffer());
    }
    catch (_) {
        return null;
    }
...
function parseMdcv(view, box) {
    const d = box.start + box.headerSize;
    if (d + 24 > box.start + box.size)
        return null;
    const prim = [];
    for (let i = 0; i < 3; i++)
        prim.push([view.getUint16(d + i * 4) * 0.00002, view.getUint16(d + i * 4 + 2) * 0.00002]);
...
async function detectC2pa(file, top) {
    for (const b of top.filter((x) => x.type === 'uuid')) {
        let buf;
        try {
            buf = new Uint8Array(await file.slice(b.start, b.start + Math.min(b.size, 65536)).arrayBuffer());
        }
        catch (_) {
            continue;
        }
...
export async function analyzeBitstream(file) {
    ...
    let view;
    try {
        view = new DataView(await file.slice(moovBox.start, moovBox.start + moovBox.size).arrayBuffer());
    }
    catch (_) {
        return null;
    }
    ...
    for (const trak of findAll(view, 0, moovEnd, 'trak')) {
        ...
        if (!first(view, s, e, 'vmhd'))
            continue;
        const stsd = first(view, s, e, 'stsd');
        if (stsd)
            codecFcc = fourcc(view, stsd.start + stsd.headerSize + 12);
        ts = s;
        te = e;
        break;
    }
    ...
    if (result.sps) {
        ...
        if (result.sps.colour.primaries != null && cf.primaries != null) {
            ...
            push('Colour', (PRIMARIES[cf.primaries] || cf.primaries) + ' / ' + (TRANSFER[cf.transfer] || cf.transfer), (PRIMARIES[result.sps.colour.primaries] || result.sps.colour.primaries) + ' / ' + (TRANSFER[result.sps.colour.transfer] || result.sps.colour.transfer), match);
        }
    }
    ...
    if (mdcvBox)
        hdr.mdcv = parseMdcv(view, mdcvBox);
    const clliBox = seFind(view, seRange, 'clli') || seFind(view, seRange, 'CoLL');
    if (clliBox)
        hdr.clli = parseClli(view, clliBox);
    const dvBox = seFind(view, seRange, 'dvcC') || seFind(view, seRange, 'dvvC') || seFind(view, seRange, 'dvwC');
    if (dvBox)
        hdr.dolbyVision = parseDvcC(view, dvBox);
    if (/^dv/.test(codecFcc))
        hdr.dolbyVisionCodec = true;
    if (hdr.mdcv || hdr.clli || hdr.dolbyVision || hdr.dolbyVisionCodec)
        result.hdr = hdr;
    // C2PA / Content Credentials in a top-level uuid box.
    try {
        result.c2pa = await detectC2pa(file, top);
    }
    catch (_) { }
    if (!result.sps && !result.hdr && !result.c2pa)
        return null;
    return result;
}
//# sourceMappingURL=video-forensics.js.map