...
const PROP_END = 0, PROP_COLORMAP = 1, PROP_OPACITY = 6, PROP_MODE = 7, PROP_VISIBLE = 8, PROP_OFFSETS = 15, PROP_COMPRESSION = 17, PROP_APPLY_MASK = 11, PROP_FLOAT_OPACITY = 33;
...
(function (B) {
    B[B["Normal"] = 0] = "Normal";
    B[B["Multiply"] = 1] = "Multiply";
    B[B["Screen"] = 2] = "Screen";
    B[B["Overlay"] = 3] = "Overlay";
    B[B["Difference"] = 4] = "Difference";
    B[B["Addition"] = 5] = "Addition";
    B[B["Subtract"] = 6] = "Subtract";
    B[B["Darken"] = 7] = "Darken";
    B[B["Lighten"] = 8] = "Lighten";
    B[B["Divide"] = 9] = "Divide";
    B[B["Dodge"] = 10] = "Dodge";
    B[B["Burn"] = 11] = "Burn";
    B[B["HardLight"] = 12] = "HardLight";
    B[B["SoftLight"] = 13] = "SoftLight";
    B[B["GrainExtract"] = 14] = "GrainExtract";
    B[B["GrainMerge"] = 15] = "GrainMerge";
    B[B["Exclusion"] = 16] = "Exclusion";
    B[B["LinearBurn"] = 17] = "LinearBurn";
    B[B["NonSeparable"] = 18] = "NonSeparable";
})(B || (B = {}));
function modeToBlend(m) {
    switch (m) {
        case 3:
        case 31: return B.Multiply;
        case 4:
        case 32: return B.Screen;
        case 5:
        case 33: return B.Overlay;
        case 6:
        case 34: return B.Difference;
        case 7:
        case 35: return B.Addition;
        case 8:
        case 36: return B.Subtract;
        case 9:
        case 37: return B.Darken;
        case 10:
        case 38: return B.Lighten;
        case 15:
        case 43: return B.Divide;
        case 16:
        case 44: return B.Dodge;
        case 17:
        case 45: return B.Burn;
        case 18:
        case 46: return B.HardLight;
        case 19:
        case 47: return B.SoftLight;
        case 20:
        case 48: return B.GrainExtract;
        case 21:
        case 49: return B.GrainMerge;
...
function rleDecode(src, p, out, stride, count) {
    let written = 0;
...
class XcfReader {
    r;
    wide; // 64-bit pointers (version >= 11)
    constructor(bytes, wide) {
    ...
    str() {
        ...
        try {
            return new TextDecoder().decode(b.subarray(0, Math.max(0, n - 1)));
        }
        catch (_) {
            return '';
        }
...
async function readHierarchy(xr, bytes, at, compression, ew, eh) {
    xr.r.seek(at);
    const w = xr.r.u32(), h = xr.r.u32(), bpp = xr.r.u32();
    if (!w || !h || !bpp || bpp > 4 || w > XCF_EDGE_MAX || h > XCF_EDGE_MAX)
        return null;
    if (w !== ew || h !== eh)
        return null;
    const levelPtr = xr.ptr(); // level 0 = full size
    if (!levelPtr || levelPtr >= bytes.length)
        return null;
    xr.r.seek(levelPtr);
    ...
    if (tilePtrs.length < tilesX * tilesY)
        return null;
    ...
    for (let t = 0; t < tilePtrs.length; t++) {
        ...
        const end = Math.min(t + 1 < tilePtrs.length ? tilePtrs[t + 1] : bytes.length, start + Math.ceil(tile.length * TILE_DATA_FACTOR));
        ...
        if (compression === 0) { // uncompressed, interleaved
            ...
            let p = 0;
            for (let c = 0; c < bpp; c++)
                p = rleDecode(raw, p, tile.subarray(c), bpp, n);
        }
            ...
            try {
                ...
                if (inf)
                    tile.set(inf.subarray(0, Math.min(tile.length, inf.length)));
            }
...
export async function parseXcf(bytes) {
    let magic = '';
    for (let i = 0; i < 14 && i < bytes.length; i++)
        magic += String.fromCharCode(bytes[i]);
    if (!magic.startsWith('gimp xcf '))
        return null;
    ...
    if (!width || !height || width > XCF_EDGE_MAX || height > XCF_EDGE_MAX)
        return null;
    const doc = {
    ...
    for (;;) {
        ...
        if (type === PROP_COMPRESSION)
            doc.compression = xr.r.u8();
        else if (type === PROP_COLORMAP) {
            const n = xr.r.u32();
            if (n > 0 && n <= 256)
                doc.colormap = xr.r.bytes_(n * 3).slice();
        }
        if (next <= xr.r.pos || next > bytes.length)
            break;
        xr.r.seek(next);
    }
    // ---- layer pointer list ----
    ...
    for (;;) {
        ...
        if (!p)
            break;
        if (p >= bytes.length || layerPtrs.length >= XCF_LAYERS_MAX)
            break;
        layerPtrs.push(p);
    }
    ...
    for (const ptr of layerPtrs.slice().reverse()) {
        ...
        for (;;) {
            ...
            if (type === PROP_END)
                break;
            const next = xr.r.pos + len;
            if (type === PROP_OPACITY)
                layer.opacity = xr.r.u32() / 255;
            else if (type === PROP_FLOAT_OPACITY)
                layer.opacity = xr.r.f32();
            else if (type === PROP_VISIBLE)
                layer.visible = xr.r.u32() !== 0;
            else if (type === PROP_MODE)
                layer.mode = xr.r.u32();
            else if (type === PROP_APPLY_MASK)
                layer.applyMask = xr.r.u32() !== 0;
            else if (type === PROP_OFFSETS) {
                layer.offsetX = xr.r.i32();
                layer.offsetY = xr.r.i32();
            }
            if (next <= xr.r.pos || next > bytes.length)
                break;
            xr.r.seek(next);
    ...
    for (const { layer, hierarchyPtr, maskPtr } of pending) {
        ...
        if (!doc.undecodable && hierarchyPtr && hierarchyPtr < bytes.length && lw && lh) {
            ...
            if (hier)
                layer.rgba = hierarchyToRgba(hier, ltype, doc);
...
// image's colormap.
function hierarchyToRgba(hier, ltype, doc) {
...
export function compositeXcf(doc) {
...
export async function renderXcf(file, resultsEl) {
    ...
    loader.remove();
    if (!doc) {
        resultsEl.appendChild(el('div', { class: 'anr-info' }, 'This file has a GIMP extension but its header does not parse as XCF.'));
        return;
    }
    const xcf = doc;
    if (!xcf.undecodable && xcf.layers.some((l) => l.rgba)) {
        ...
        card.appendChild(el('div', { style: 'margin-top:10px;' }, [
            el('button', { type: 'button', class: 'anr-btn', onclick: () => {
                    cv.toBlob((b) => { if (b)
                        downloadBlob((file.name || 'image').replace(/\.[^.]+$/, '') + '.png', b); }, 'image/png');
                } }, 'Save composite (PNG)'),
        ]));
        resultsEl.appendChild(card);
    }
    else {
        resultsEl.appendChild(el('div', { class: 'anr-info' }, xcf.undecodable
    ...
    if (xcf.colormap)
        tbl.appendChild(row('Palette colours', xcf.colormap.length / 3));
    ...
    if (xcf.layers.length) {
        ...
        lc.appendChild(lh);
        lc.appendChild(lhelp);
        const lt = el('table', { class: 'anr-table' });
        for (const l of xcf.layers) {
            const bits = [MODE_NAME[l.mode] || ('mode ' + l.mode)];
            bits.push(Math.round(l.opacity * 100) + '% opacity');
            if (l.width !== xcf.width || l.height !== xcf.height)
                bits.push(l.width + '×' + l.height);
            if (l.offsetX || l.offsetY)
                bits.push('at ' + l.offsetX + ',' + l.offsetY);
            if (l.mask)
                bits.push('masked');
            if (!l.visible)
                bits.push('hidden');
            if (modeToBlend(l.mode) === B.NonSeparable)
                bits.push('drawn as Normal');
