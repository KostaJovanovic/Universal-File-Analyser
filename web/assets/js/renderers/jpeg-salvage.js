...
function decodeHuff(br, tab) {
    ...
    for (let len = 1; len <= 16; len++) {
        ...
        if (br.hitMarker)
            return -1;
        if (tab.maxcode[len] >= 0 && code <= tab.maxcode[len])
            return tab.values[tab.valptr[len] + code - tab.mincode[len]];
    }
...
function colIDCT(blk, o, out, outPos, stride) {
    ...
    out[outPos] = clip(x7 + x1);
    out[outPos + stride] = clip(x3 + x2);
    out[outPos + 2 * stride] = clip(x0 + x4);
    out[outPos + 3 * stride] = clip(x8 + x6);
    out[outPos + 4 * stride] = clip(x8 - x6);
    out[outPos + 5 * stride] = clip(x0 - x4);
    out[outPos + 6 * stride] = clip(x3 - x2);
    out[outPos + 7 * stride] = clip(x7 - x1);
}
...
function parseHeader(d) {
    ...
    while (p + 4 <= d.length) {
        ...
        if (len < 2 || p + len > d.length)
            return null;
        const seg = p + 2, end = p + len;
        if (m === 0xDB) { // DQT
            let q = seg;
            while (q < end) {
                const pq = d[q] >> 4, tq = d[q] & 15;
                q++;
                const t = new Int32Array(64);
                for (let i = 0; i < 64; i++) {
                    t[i] = pq ? ((d[q] << 8) | d[q + 1]) : d[q];
                    q += pq ? 2 : 1;
                }
                qt[tq] = t;
            }
            ...
            let q = seg;
            while (q < end) {
                const tc = d[q] >> 4, th = d[q] & 15;
                q++;
                const counts = d.subarray(q, q + 16);
                q += 16;
                let n = 0;
                for (let i = 0; i < 16; i++)
                    n += counts[i];
                const values = d.subarray(q, q + n);
                q += n;
            ...
            if (m !== 0xC0 && m !== 0xC1)
                return null; // baseline sequential only
            const comps = [];
            const nc = d[seg + 5];
            for (let i = 0; i < nc; i++) {
                const o = seg + 6 + i * 3;
            ...
            for (let i = 0; i < ns; i++) {
                const o = seg + 1 + i * 2;
                scan.push({ id: d[o], dc: d[o + 1] >> 4, ac: d[o + 1] & 15 });
            }
...
function findEmbeddedThumb(d) {
    ...
    while (p + 4 <= d.length) {
        ...
        if (m === 0xDA || m === 0xD9 || (m >= 0xC0 && m <= 0xCF && m !== 0xC4 && m !== 0xC8 && m !== 0xCC))
            break;
...
export function detectCorruptCut(data, w, h, rows = h) {
    ...
    for (let b = Math.floor(nb * 0.75); b < nb; b++)
        if (garb[b] > 0.35) {
            bottomGarb = true;
            break;
        }
    ...
    for (let b = 1; b <= blow; b++) {
        if (sd[b] < 32) {
            let textured = false;
            for (let k = Math.max(0, b - 2); k < b; k++)
                if (sd[k] > 45)
                    textured = true;
            if (textured) {
                onset = b;
    ...
    if (onset < 1)
        return -1; // no real top strip survived - let the normal decode handle it
    return onset * BAND;
}
...
export function decodeJpegPartial(bytes, allowThumb = true) {
    ...
    for (const c of comps) {
        ...
        c.quant = H.qt[c.qId];
        c.pw = mcusPerLine * c.h * 8;
        c.ph = mcusPerCol * c.v * 8;
        c.plane = new Uint8ClampedArray(c.pw * c.ph).fill(128);
        c.pred = 0;
    }
    if (comps.some((c) => !c.dcTab || !c.acTab || !c.quant))
        return bail();
    const br = new BitReader(bytes, H.scanStart);
    const blk = new Int32Array(64);
    let mcuRow = 0;
    outer: for (let my = 0; my < mcusPerCol; my++) {
        for (let mx = 0; mx < mcusPerLine; mx++) {
            for (const c of comps) {
                for (let by = 0; by < c.v; by++) {
                    for (let bx = 0; bx < c.h; bx++) {
                        if (!decodeBlock(br, c, blk))
                            break outer; // bitstream ended/corrupt
                        const px = (mx * c.h + bx) * 8, py = (my * c.v + by) * 8;
                        for (let i = 0; i < 8; i++)
                            rowIDCT(blk, i * 8);
                        for (let i = 0; i < 8; i++)
                            colIDCT(blk, i, c.plane, (py + 0) * c.pw + px + i, c.pw);
                    }
                }
            }
            if (H.dri && ((my * mcusPerLine + mx + 1) % H.dri === 0)) {
                br.restart();
                for (const c of comps)
                    c.pred = 0;
            }
    ...
    if (rows === 0)
        return bail(); // main image gone - try the thumbnail
    ...
    if (cut >= 0 && cut < rows) {
        if (cut === 0) {
            ...
            if (b)
                return b;
        } // garbage from the very top - prefer the thumbnail
...
//# sourceMappingURL=jpeg-salvage.js.map