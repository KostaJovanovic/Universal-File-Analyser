...
export async function detectBPM(samples, sampleRate, tick) {
    ...
    for (let f = 0; f < numFrames; f++) {
        ...
        for (let i = 0; i < halfN; i++)
            curMag[i] = Math.sqrt(re[i] * re[i] + im[i] * im[i]);
        if (f > 0) {
            let sum = 0;
            for (let i = 0; i < halfN; i++) {
    ...
    for (let f = medianW; f < numFrames - medianW; f++) {
        let localMean = 0;
        for (let j = f - medianW; j <= f + medianW; j++)
            localMean += flux[j];
    ...
    if (bpm < 60 || bpm > 200 || !isFinite(bpm))
        return null;
