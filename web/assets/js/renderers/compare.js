...
import { el, fmtBytes, sha256Hex, extraHashRows, errorCard } from '../core/util.js';
...
function mergeReadout(tA, tB) {
    ...
    if (tB)
        for (const tr of tB.rows) {
            ...
            if (th)
                bByLabel.set(th.textContent.trim(), tr);
        }
    const used = new Set();
    if (tA)
        for (const tr of tA.rows) {
            const th = tr.querySelector('th');
            if (!th)
                continue;
            const label = th.textContent.trim();
            const bRow = bByLabel.get(label);
            if (bRow)
                used.add(label);
    ...
    if (tB)
        for (const tr of tB.rows) {
            ...
            if (!th)
                continue;
            if (used.has(th.textContent.trim()))
                continue;
...
function headerNodeOf(card) {
    ...
    for (const n of card.children) {
        if (n.tagName === 'H3')
            return n;
        if (n.querySelector && n.querySelector('h3'))
            return n;
    }
    return null;
}
...
function bodyNodes(card, header) {
    if (!card)
        return [];
    return [...card.children].filter((n) => n !== header && !(n.classList && n.classList.contains('anr-info-panel')));
}
const nodeKind = (n) => (n.matches && n.matches('table.anr-readout')) ? 'table'
...
function tagSplitSameness(root) {
    ...
    for (const split of root.querySelectorAll('.anr-cmp-split')) {
        ...
        if (split.querySelector('canvas, img, audio, video, svg'))
            continue;
        const cols = [...split.children].filter((c) => c.classList && c.classList.contains('anr-cmp-col'));
        if (cols.length >= 2 && colText(cols[0]) === colText(cols[1]))
            split.classList.add('anr-cmp-split-same');
    }
}
...
function mergeCard(cardA, cardB) {
    ...
    const flushRest = () => {
        if (restA.length || restB.length)
            card.appendChild(splitCols(restA, restB));
        restA = [];
        restB = [];
    };
    while (i < A.length || j < B.length) {
        ...
        if (ka === 'table' && kb === 'table') {
            flushRest();
            card.appendChild(mergeReadout(a, b));
            i++;
            j++;
        }
        else if (ka === 'section' && kb === 'section') {
            flushRest();
            card.appendChild(a);
            i++;
            j++;
            ...
            flushRest();
            card.appendChild(mergeReadout(a, null));
            i++;
        }
        else if (kb === 'table') {
            flushRest();
            card.appendChild(mergeReadout(null, b));
            j++;
        }
        else if (ka === 'section') {
            flushRest();
...
function mergeInteractive(aSub, bSub) {
    ...
    if (descEl)
        card.appendChild(el('p', { class: 'anr-info' }, descEl.textContent));
    const aBtn = ctaOf(aSub), bBtn = ctaOf(bSub);
    // Hide each file's own prompt card; the central button drives them.
    [aSub, bSub].forEach((s) => { const p = s && s.querySelector('.anr-card'); if (p)
...
function mergePanels(aBlocks, bBlocks, mount) {
    ...
    for (const b of B.cards)
        if (!usedB.has(b))
            mount.appendChild(mergeCard(null, b));
    // 2. Interactive prompts -> one central button each, paired by heading.
    const subByHeading = new Map();
    for (const s of B.inter) {
        const h = subHeading(s);
        if (!subByHeading.has(h))
            subByHeading.set(h, []);
        subByHeading.get(h).push(s);
    }
    const usedS = new Set();
    ...
    for (const s of B.inter)
        if (!usedS.has(s))
            mount.appendChild(mergeInteractive(null, s));
    ...
    if (A.rest.length || B.rest.length)
        mount.appendChild(splitCols(A.rest, B.rest));
}
...
async function appendHashExtras(mergedRoot, fileA, fileB, shaMatch) {
    ...
    for (const t of mergedRoot.querySelectorAll('table.anr-readout.anr-cmp')) {
        for (const tr of t.rows) {
            const th = tr.querySelector('th');
            if (th && /^sha-?256\b/i.test(th.textContent.trim())) {
                table = t;
                shaRow = tr;
                break;
            }
    ...
    if (shaRow && shaMatch) {
        shaMatch.then((m) => {
            if (m === 'match')
                shaRow.classList.remove('is-diff');
            else if (m === 'differ')
                shaRow.classList.add('is-diff');
        });
    }
...
async function appendFuzzyRows(table, fileA, fileB) {
    try {
        ...
        const cell = (h) => h == null ? el('td', { class: 'anr-cmp-absent' }, 'too large') : el('td', {}, h);
...
function bucketize(staging, mainKey) {
    ...
    for (const n of staging.children) {
        let key = mainKey;
        if (n.classList) {
            if (n.classList.contains('anr-cmp-sub-audio'))
                key = 'sound';
            else if (n.classList.contains('anr-cmp-sub-photo'))
                key = 'photo';
        }
        buckets[key].push(n);
    }
...
export async function renderCompare(fileA, fileB, resultsEl, deps = {}) {
    ...
    const shaMatch = Promise.all([sha256Hex(fileA), sha256Hex(fileB)]).then(([ha, hb]) => (ha && hb ? (ha === hb ? 'match' : 'differ') : 'error'), () => 'error');
    shaMatch.then((sha) => {
    ...
    async function renderInto(file, staging, kind) {
        ...
        let opts;
        if (MEDIA.has(kind))
            opts = { inline: true, compare: true, signal: cmpAbort.signal };
        else if (kind === 'pdf')
            opts = { previewPages: 1 };
        try {
            await Promise.resolve(route.render(file, staging, opts));
        }
        catch (e) {
            staging.appendChild(errorCard('Could not analyse ' + file.name + ': ' + (e && e.message ? e.message : e)));
        }
        ...
        if (typeof deps.renderExtras === 'function') {
            try {
                await deps.renderExtras(file, staging, kind);
            }
            catch (_) { }
        }
    }
    ...
    const order = [];
    const add = (k) => { if (!order.includes(k) && (bucketsA[k].length || bucketsB[k].length))
        order.push(k); };
    add(mainKeyOf(kindA));
    add(mainKeyOf(kindB));
    ...
    if (integ)
        merged.insertBefore(integ, merged.firstChild);
