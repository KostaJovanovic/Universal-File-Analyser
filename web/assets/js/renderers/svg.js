...
function sanitizeSvg(doc) {
    ...
    if (scripts.length)
        findings.push(scripts.length + ' <script> element' + (scripts.length > 1 ? 's' : ''));
    scripts.forEach((n) => n.remove());
    const fo = doc.querySelectorAll('foreignObject');
    if (fo.length)
        findings.push(fo.length + ' <foreignObject> (embedded HTML)');
    ...
    if (styles.length)
        findings.push(styles.length + ' <style> element' + (styles.length > 1 ? 's' : '') + ' (document-wide CSS)');
    styles.forEach((n) => n.remove());
    const smil = doc.querySelectorAll('animate, animateTransform, animateMotion, set');
    if (smil.length)
        findings.push(smil.length + ' SMIL animation element' + (smil.length > 1 ? 's' : ''));
    ...
    if (handlers)
        findings.push(handlers + ' inline event handler' + (handlers > 1 ? 's' : '') + ' (on*)');
    if (jsLinks)
        findings.push(jsLinks + ' script-capable link' + (jsLinks > 1 ? 's' : '') + ' (javascript:, vbscript:, ...)');
    if (extRefs)
        findings.push(extRefs + ' external/remote reference' + (extRefs > 1 ? 's' : ''));
    if (cssRefs)
        findings.push(cssRefs + ' remote CSS url() reference' + (cssRefs > 1 ? 's' : ''));
    let safe;
    try {
        safe = new XMLSerializer().serializeToString(root);
    }
    catch (_) {
        safe = null;
    }
