...
function scanRawPdf(bytes) {
    ...
    let eof = 0, idx = 0;
    while ((idx = text.indexOf('%%EOF', idx)) !== -1) {
        eof++;
        idx += 5;
    }
...
export async function renderPdf(file, resultsEl, opts = {}) {
    ...
    try {
        ...
        try {
            ...
            if (fx.eof > 1) {
                ...
                addRow(rowHelp('Revisions', String(fx.eof) + ' (incrementally updated ' + updates + ' time' + (updates === 1 ? '' : 's') + ')', 'Every time a PDF is saved it adds another end-of-file marker (%%EOF). More than one means the file was edited and saved again without rewriting it from scratch, so older versions of the content are still tucked inside - a strong clue about where the file came from and whether it was altered.'));
            }
            if (fx.linearized) {
                addRow(rowHelp('Linearised', 'yes', 'The PDF is "linearised" (also called "fast web view"): its parts are rearranged so a viewer can show the first page before the whole file has finished downloading. Usually done by tools that optimise PDFs for the web.'));
            }
            if (fx.appended > 0) {
                addRow(rowHelp('Trailing data', '⚠ ' + fx.appended + ' byte' + (fx.appended === 1 ? '' : 's') + ' after final %%EOF', 'There is extra content after the file’s final end-of-file marker. This is often a harmless leftover from re-saving, but it is also a common way to hide data inside a PDF (for example files that are secretly two formats at once, or payloads tacked onto the end) - so it is worth a closer look.'));
            }
        }
        catch (_) { }
        ...
        try {
            ...
            if (Array.isArray(outline) && outline.length) {
                addRow(rowHelp('Outline entries', String(outline.length), 'The main bookmarks in the document’s outline - its built-in table of contents. Any bookmarks nested underneath them appear indented in the list you can open below.'));
        ...
        try {
            ...
            if (names.length) {
                addRow(rowHelp('Embedded files', String(names.length), 'Other files tucked away inside this PDF, which can carry attachments just like a folder can. They are listed below.'));
                const det = el('details');
        ...
        try {
            ...
            try {
                if (typeof pdf.getOpenAction === 'function') {
                    ...
                    if (oa && oa.action === 'JavaScript')
                        pushAction('OpenAction', oa.jsCode || '(script present, source not exposed)');
                }
            }
            catch (_) { }
            if (actions.length) {
                addRow(rowHelp('Embedded JavaScript', '⚠ yes (' + actions.length + ' action' + (actions.length === 1 ? '' : 's') + ')', 'This PDF contains JavaScript - small programs a PDF viewer might run. They can be harmless (for example making a form work), but are also a common way to deliver malware, so treat unexpected ones with caution.'));
                const autoRun = actions.filter((a) => AUTO_TRIGGERS[a.trigger]);
                if (autoRun.length)
                    addRow(rowHelp('Auto-run scripts', '⚠ ' + [...new Set(autoRun.map((a) => AUTO_TRIGGERS[a.trigger]))].join(', '), 'These scripts start on their own when the named thing happens (such as simply opening the file), without you doing anything - the riskiest kind.'));
                const allFlags = [...new Set(actions.flatMap((a) => a.flags))];
                if (allFlags.length)
                    addRow(rowHelp('Suspicious patterns', '⚠ ' + allFlags.join(', '), 'An automated scan of the scripts spotted commands linked to: ' + allFlags.join(', ') + ' - higher-risk actions such as contacting the internet, reading or writing files, launching other programs, or deliberately scrambling the code to hide what it does.'));
                ...
                for (const a of actions) {
                    det.appendChild(el('div', { class: 'anr-readout-section', style: 'margin-top:8px;' }, triggerLabel(a.trigger) + (a.flags.length ? '  -  ⚠ ' + a.flags.join(', ') : '')));
        ...
        try {
            ...
            let perms = null;
            try {
                perms = await pdf.getPermissions().catch(() => null);
            }
            catch (_) { }
            // getPermissions() returns a non-null array only when usage is restricted.
            if (encrypted || Array.isArray(perms)) {
                addRow(rowHelp('Encrypted', encrypted ? 'yes' : 'no', 'Whether the PDF is encrypted (its contents scrambled for protection). An encrypted PDF may still open without asking for a password, yet block certain actions such as printing, copying text, or editing.'));
            }
            if (Array.isArray(perms) && lib.PermissionFlag) {
                const PF = lib.PermissionFlag;
                const has = (flag) => flag != null && perms.indexOf(flag) !== -1;
                const allowed = [];
                if (has(PF.PRINT) || has(PF.PRINT_HIGH_QUALITY))
                    allowed.push('print');
                if (has(PF.COPY))
                    allowed.push('copy');
                if (has(PF.MODIFY_CONTENTS) || has(PF.MODIFY_ANNOTATIONS))
                    allowed.push('modify');
                const allActions = ['print', 'copy', 'modify'];
                const denied = allActions.filter((a) => allowed.indexOf(a) === -1);
                addRow(rowHelp('Allowed actions', allowed.length ? allowed.join(', ') : 'none', 'The things the document’s permissions let you do. Anything left out - such as printing, copying text, or editing - is blocked, and that block is enforced by the PDF’s encryption.'));
                if (denied.length)
                    addRow(row('Restricted actions', denied.join(', ')));
            }
        }
        catch (_) { }
        ...
        try {
            ...
            if (widgets)
                addRow(rowHelp('Form fields', String(widgets) + scope, 'The number of fill-in fields found across the pages checked - text boxes, tick boxes and buttons. Their presence means this is a form you can fill in on screen (an AcroForm).'));
            if (links)
                addRow(row('Links', String(links) + scope));
            if (others)
                addRow(rowHelp('Annotations', String(others) + scope, 'Annotations are marks added on top of the page - highlights, sticky notes, stamps and freehand drawings - rather than part of the original document. This counts them across the pages checked.'));
        }
        catch (_) { }
        // -- XMP metadata (Keywords / Subject / PDF/A) --
        try {
            const getXmp = (key) => {
                if (!metaXmp || typeof metaXmp.get !== 'function')
                    return null;
                try {
                    const v = metaXmp.get(key);
            ...
            if (keywords)
                addRow(row('Keywords', keywords));
            if (subject)
                addRow(row('Subject', subject));
            const part = getXmp('pdfaid:part');
            if (part) {
                const conf = getXmp('pdfaid:conformance');
                addRow(rowHelp('PDF/A', 'PDF/A-' + part + (conf ? conf.toUpperCase() : ''), 'The file says it meets PDF/A, an international standard for long-term archiving. It requires everything the document needs (fonts, images and so on) to be stored inside, so it still displays correctly years from now.'));
            }
        }
        catch (_) { }
        ...
        try {
            ...
            if (fonts.size) {
                ...
                const scope = pdf.numPages > cap ? ' (first ' + cap + ' pages)' : '';
                addRow(rowHelp('Fonts', list.length + (notEmbedded ? ', ' + notEmbedded + ' not embedded' : ', all embedded') + scope, 'How many different fonts the document uses. A font that is not stored inside the file (not embedded) relies on the reader’s device already having it, so the document can look different on another computer - a useful clue to how portable and how genuine the file is.'));
                const det = el('details');
    ...
    function openPageViewer(startPage) {
        ...
        if (!overlay) {
            ...
            function close() { if (overlay._backClose)
                overlay._backClose();
            else
                overlay._hide(); }
            ...
            document.addEventListener('keydown', (e) => {
                ...
                if (e.key === 'Escape')
                    ...
                    overlay._prev && overlay._prev();
                else if (e.key === 'ArrowRight')
                    overlay._next && overlay._next();
            });
            ...
            stage.addEventListener('pointerup', (e) => {
                ...
                if (now - lastTap < 320 && Math.abs(e.clientX - lastX) < 30 && Math.abs(e.clientY - lastY) < 30) {
                    const sel = window.getSelection && window.getSelection();
                    if (!(sel && String(sel).length))
                        overlay._toggleZoom(e.clientX, e.clientY);
                    lastTap = 0;
                }
                else {
                    lastTap = now;
            ...
            stage.addEventListener('pointerdown', (e) => {
                if (e.pointerType === 'touch' || e.button !== 0 || !canPan())
                    return;
                panId = e.pointerId;
                panX = e.clientX;
                panY = e.clientY;
                panL = stage.scrollLeft;
                panT = stage.scrollTop;
            ...
            stage.addEventListener('pointermove', (e) => {
                if (panId === null || e.pointerId !== panId)
                    return;
                if (panAllowX)
                    stage.scrollLeft = panL - (e.clientX - panX);
                if (panAllowY)
                    stage.scrollTop = panT - (e.clientY - panY);
            });
            const endPan = (e) => {
                if (panId === null || e.pointerId !== panId)
                    return;
                try {
                    stage.releasePointerCapture(panId);
                }
                catch (_) { }
                panId = null;
        ...
        async function showPage(num, keepZoom) {
            ...
            try {
                ...
                if (longest > MAX_SIDE)
                    renderScale = MAX_SIDE / Math.max(vp.width, vp.height);
        ...
        prevBtn.addEventListener('click', (e) => { e.stopPropagation(); if (current > 1)
            showPage(current - 1); });
        const nextBtn = el('button', { type: 'button', class: 'lightbox-tool-btn' }, 'Next →');
        nextBtn.addEventListener('click', (e) => { e.stopPropagation(); if (current < pdf.numPages)
            showPage(current + 1); });
        ...
        overlay._prev = () => { if (current > 1)
            showPage(current - 1); };
        overlay._next = () => { if (current < pdf.numPages)
            showPage(current + 1); };
        ...
        if (wasHidden)
            overlay._backClose = openOverlayBack(overlay._hide);
        showPage(startPage);
    }
            ...
            analyseBtn.addEventListener('click', async (e) => {
                ...
                try {
                    ...
                    cv.toBlob((blob) => {
                        ...
                        if (photoResults) {
                            renderPhoto(photoFile, photoResults, { sourceNote: 'Page ' + pageNum + ' of ' + (file.name || 'this PDF') + ', rendered and analysed as an image.' });
                        }
    ...
    thumbMoreBtn.addEventListener('click', async () => {
        ...
        for (let i = thumbsRendered + 1; i <= end; i++)
            await renderThumb(i);
        thumbsRendered = end;
        updateThumbBtns();
    });
    thumbAllBtn.addEventListener('click', async () => {
        ...
        for (let i = thumbsRendered + 1; i <= pdf.numPages; i++)
            await renderThumb(i);
    ...
    imgCard.appendChild(el('p', { class: 'anr-hint', style: 'font-size:12px;margin:0 0 10px;' }, 'The original photos, logos and scans stored inside the PDF - separate from the page previews above.'));
        ...
        try {
            ...
            for (let p = 1; p <= pdf.numPages; p++) {
                ...
                for (let i = 0; i < ops.fnArray.length; i++) {
                    ...
                    aBtn.addEventListener('click', () => cv.toBlob((b) => { if (b && window._anrHandleFile)
                        window._anrHandleFile(new File([b], 'pdf-image.png', { type: 'image/png' }), { nested: true }); }, 'image/png'));
