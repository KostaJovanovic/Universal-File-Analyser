...
import { longAverageSpectrum, detectKey, loudnessR128, truePeakDb, signalHealth, detectDtmf } from './audio-forensics.js';
...
export async function* audioDspPasses({ channels, mono, sampleRate, needBpm, onTick }) {
    ...
    if (!mono)
        mono = mergeMono(channels);
    let spec = null;
    try {
        spec = await longAverageSpectrum(mono, sampleRate, { tick });
    }
    catch (_) { }
    yield ['spec', spec];
    let health = null;
    try {
        health = await signalHealth(channels, tick);
    }
    catch (_) { }
    yield ['health', health];
    let key = null;
    ...
    let r128 = null;
    try {
        r128 = await loudnessR128(channels, sampleRate, tick);
    }
    catch (_) { }
    yield ['r128', r128];
    let tpDb = null;
    try {
        tpDb = await truePeakDb(channels, sampleRate, tick);
    }
    catch (_) { }
    yield ['truePeak', tpDb];
    let dtmf = null;
    try {
        dtmf = await detectDtmf(mono, sampleRate, tick);
    }
    catch (_) { }
    yield ['dtmf', dtmf];
    let centroid = null;
    try {
        centroid = await computeCentroid(mono, sampleRate, tick);
    }
    catch (_) { }
    ...
    let pitch = null;
    try {
        pitch = detectPitch(mono, sampleRate);
    }
    catch (_) { }
    yield ['pitch', pitch];
    let bpm = null;
    if (needBpm) {
        try {
            bpm = await detectBPM(mono, sampleRate, tick);
        }
        catch (_) { }
    }
    yield ['bpm', bpm];
