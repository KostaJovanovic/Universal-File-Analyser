...
class BitReader {
    ...
    bits(n) { let v = 0; for (let i = 0; i < n; i++)
        v = v * 2 + this.bit(); return v; }
    skip(n) { this.pos += n; if (this.pos > this.end) {
        this.pos = this.end;
        this.overrun = true;
    } }
    ue() {
        let zeros = 0;
        while (zeros < 32 && !this.overrun && this.bit() === 0)
            zeros++;
        if (this.overrun || zeros >= 32) {
            this.overrun = true;
            return 0;
        }
...
function findAnnexBNals(buf, hevc, max = 256) {
    ...
    for (let k = 0; k + 1 < nals.length; k++) {
        ...
        while (e > nals[k].start && buf[e - 1] === 0)
            e--;
        nals[k].end = e;
    }
    return nals;
...
function hevcVui(br, out) {
    if (br.bit()) { // aspect_ratio_info_present_flag
        const idc = br.bits(8);
        if (idc === 255) {
            out.sarWidth = br.bits(16);
            out.sarHeight = br.bits(16);
        }
        else
            out.aspectRatioIdc = idc;
    }
    if (br.bit())
        br.bit(); // overscan_info / overscan_appropriate
...
export function parseHevcSps(rbsp) {
    ...
    if (ptl.levelIdc)
        out.level = (ptl.levelIdc / 30).toFixed(1);
    if (ptl.progressive !== undefined)
        out.progressive = ptl.progressive;
    ...
    if (br.overrun || !(out.width > 0) || !(out.height > 0) ||
        out.bitDepthLuma < 8 || out.bitDepthLuma > 16)
        return null;
    ...
    br.ue();
    br.ue();
    br.ue(); // CTB / transform sizes + hierarchy depths
    if (br.bit() && br.bit())
        skipScalingListData(br); // scaling_list_enabled + data present
    ...
    if (br.overrun)
        return essentials;
    if (br.bit())
        hevcVui(br, out); // vui_parameters_present_flag
...
export function parseAvcSps(rbsp) {
    ...
    if (levelIdc)
        out.level = (levelIdc / 10).toFixed(1);
    br.ue(); // seq_parameter_set_id
    let chromaIdc = 1;
    out.bitDepthLuma = 8;
    out.bitDepthChroma = 8;
    if ([100, 110, 122, 244, 44, 83, 86, 118, 128, 138, 139, 134, 135].includes(profileIdc)) {
        ...
        if (br.bit()) { // seq_scaling_matrix_present_flag
            ...
            for (let i = 0; i < n; i++)
                if (br.bit())
                    skipAvcScalingList(br, i < 6 ? 16 : 64);
        }
    }
    out.chromaIdc = chromaIdc;
    out.chroma = CHROMA_FORMATS[chromaIdc] || ('idc ' + chromaIdc);
    ...
    const essentials = { ...out, partial: true };
    if (br.bit())
        avcVui(br, out); // vui_parameters_present_flag
...
export function parseHvcC(u8) {
    ...
    out.levelIdc = u8[12];
    if (out.levelIdc)
        out.level = (out.levelIdc / 30).toFixed(1);
    ...
    if (avgFrameRate > 0) {
        out.fps = avgFrameRate / 256;
        out.fpsSource = 'hvcC average frame rate';
    }
    ...
    for (let a = 0; a < numArrays && p + 3 <= u8.length; a++) {
        const nalType = u8[p] & 0x3f;
        p += 1;
        const numNalus = (u8[p] << 8) | u8[p + 1];
        p += 2;
        for (let n = 0; n < numNalus && p + 2 <= u8.length; n++) {
            ...
            if (nalType === 33 && len > 2 && !out.sps) {
                try {
                    out.sps = parseHevcSps(stripEpb(u8.subarray(p + 2, p + len)));
                }
                catch (_) { /* keep the record */ }
            }
            p += len;
        }
    }
...
export function parseAvcC(u8) {
    ...
    out.levelIdc = u8[3];
    if (out.levelIdc)
        out.level = (out.levelIdc / 10).toFixed(1);
    ...
    if (out.profileIdc === 122)
        out.chroma = '4:2:2';
    else if (out.profileIdc === 244)
        out.chroma = '4:4:4';
    let p = 5;
    ...
    for (let i = 0; i < numSps && p + 2 <= u8.length; i++) {
        ...
        if (len > 1 && !out.sps) {
            try {
                out.sps = parseAvcSps(stripEpb(u8.subarray(p + 1, p + len)));
            }
            catch (_) { /* keep the record */ }
        }
        p += len;
    ...
    if (p < u8.length) {
        const numPps = u8[p++];
        for (let i = 0; i < numPps && p + 2 <= u8.length; i++) {
            p += 2 + ((u8[p] << 8) | u8[p + 1]);
        }
    }
    ...
    if ([100, 110, 122, 144, 244].includes(out.profileIdc) && p + 3 <= u8.length) {
        ...
        if ((b0 & 0xFC) === 0xFC && (b1 & 0xF8) === 0xF8 && (b2 & 0xF8) === 0xF8) {
            ...
            if (luma >= 8 && luma <= 16) {
                out.bitDepthLuma = luma;
                out.bitDepthChroma = chromaDepth;
            }
        }
    }
    return mergeSps(out);
...
function mergeSps(rec) {
    ...
    if (rec.bitDepthLuma === undefined && sps.bitDepthLuma) {
        ...
        rec.bitDepthChroma = sps.bitDepthChroma;
    }
    if (rec.chroma === undefined && sps.chroma) {
        rec.chroma = sps.chroma;
        rec.chromaIdc = sps.chromaIdc;
    }
    return rec;
}
...
export function parseAnnexBStreamInfo(bytes, hevc) {
    ...
    let info = null;
    try {
        info = hevc ? parseHevcSps(rbsp) : parseAvcSps(rbsp);
    }
    catch (_) {
        return null;
    }
...
const uintOf = (b, s, e) => { let v = 0; for (let i = s; i < e; i++)
    v = v * 256 + b[i]; return v; };
const floatOf = (b, s, e) => {
...
function parseMkvTrackEntry(b, start, end) {
    ...
    eachChild(b, start, end, (id, s, e) => {
        switch (id) {
            case EBML.TRACK_NUMBER:
                t.number = uintOf(b, s, e);
                break;
            case EBML.TRACK_TYPE:
                t.type = uintOf(b, s, e);
                break;
            case EBML.CODEC_ID:
                t.codecId = strOf(b, s, e);
                break;
            case EBML.CODEC_NAME:
                t.codecName = strOf(b, s, e);
                break;
            case EBML.NAME:
                t.name = strOf(b, s, e);
                break;
            case EBML.LANGUAGE:
            case EBML.LANGUAGE_BCP47:
                t.language = t.language || strOf(b, s, e);
                break;
            case EBML.FLAG_DEFAULT:
                t.isDefault = uintOf(b, s, e) === 1;
                break;
            case EBML.FLAG_FORCED:
                t.forced = uintOf(b, s, e) === 1;
                break;
            case EBML.DEFAULT_DURATION:
                t.defaultDurationNs = uintOf(b, s, e);
                break;
            case EBML.CODEC_PRIVATE:
                t.codecPrivate = b.subarray(s, e);
                break;
            case EBML.VIDEO:
                eachChild(b, s, e, (vid, vs, ve) => {
                    switch (vid) {
                        case EBML.PIXEL_WIDTH:
                            t.width = uintOf(b, vs, ve);
                            break;
                        case EBML.PIXEL_HEIGHT:
                            t.height = uintOf(b, vs, ve);
                            break;
                        case EBML.DISPLAY_WIDTH:
                            t.displayWidth = uintOf(b, vs, ve);
                            break;
                        case EBML.DISPLAY_HEIGHT:
                            t.displayHeight = uintOf(b, vs, ve);
                            break;
                        case EBML.COLOUR:
                            eachChild(b, vs, ve, (cid, cs, ce) => {
                                switch (cid) {
                                    case EBML.MATRIX_COEFFS:
                                        t.matrix = uintOf(b, cs, ce);
                                        break;
                                    case EBML.BITS_PER_CHANNEL:
                                        t.bitDepthLuma = uintOf(b, cs, ce);
                                        break;
                                    case EBML.RANGE:
                                        t.colourRange = uintOf(b, cs, ce);
                                        break;
                                    case EBML.TRANSFER:
                                        t.transfer = uintOf(b, cs, ce);
                                        break;
                                    case EBML.PRIMARIES:
                                        t.primaries = uintOf(b, cs, ce);
                                        break;
                                    case EBML.MAX_CLL:
                                        t.maxCll = uintOf(b, cs, ce);
                                        break;
                                    case EBML.MAX_FALL:
                                        t.maxFall = uintOf(b, cs, ce);
                                        break;
                                    case EBML.MASTERING:
                                        t.mdcv = true;
                                        break;
            ...
            case EBML.AUDIO:
                eachChild(b, s, e, (aid, as, ae) => {
                    switch (aid) {
                        case EBML.SAMPLING_FREQ:
                            t.sampleRate = floatOf(b, as, ae) || uintOf(b, as, ae);
                            break;
                        case EBML.CHANNELS:
                            t.channels = uintOf(b, as, ae);
                            break;
                        case EBML.BIT_DEPTH:
                            t.audioBitDepth = uintOf(b, as, ae);
                            break;
                        default: break;
                    }
                });
    ...
    while (pos < segEnd && guard++ < 4096 && !(infoDone && tracksDone)) {
        ...
        if (id.val === EBML.INFO && size.val < 8 * 1024 * 1024) {
            ...
            eachChild(b, 0, b.length, (cid, s, e) => {
                switch (cid) {
                    case EBML.TIMECODE_SCALE:
                        timecodeScale = uintOf(b, s, e) || 1000000;
                        break;
                    case EBML.DURATION:
                        rawDuration = floatOf(b, s, e) || 0;
                        break;
                    case EBML.MUXING_APP:
                        out.muxingApp = strOf(b, s, e);
                        break;
                    case EBML.WRITING_APP:
                        out.writingApp = strOf(b, s, e);
                        break;
                    case EBML.TITLE:
                        out.title = strOf(b, s, e);
                        break;
                    case EBML.DATE_UTC: {
                        // Nanoseconds since 2001-01-01, as a signed 8-byte integer.
                        const ns = uintOf(b, s, e);
                        if (ns)
                            out.dateUtc = new Date(Date.UTC(2001, 0, 1) + ns / 1e6);
                        break;
                    }
                    default: break;
                }
            ...
            eachChild(b, 0, b.length, (cid, s, e) => {
                if (cid === EBML.TRACK_ENTRY)
                    out.tracks.push(parseMkvTrackEntry(b, s, e));
            });
            tracksDone = true;
        }
        pos = dataEnd;
    }
    if (rawDuration > 0 && timecodeScale > 0)
        out.durationSec = rawDuration * timecodeScale / 1e9;
    if (!out.tracks.length)
        return (out.writingApp || out.muxingApp || out.durationSec) ? out : null;
        ...
        if (t.type === 1 && !out.video) {
            ...
            try {
                ...
                if (cfg) {
                    ...
                    if (cfg.primaries !== undefined && t.primaries === undefined)
                        t.primaries = cfg.primaries;
                    if (cfg.transfer !== undefined && t.transfer === undefined)
                        t.transfer = cfg.transfer;
                    if (cfg.matrix !== undefined && t.matrix === undefined)
                        t.matrix = cfg.matrix;
                    if (cfg.fullRange !== undefined && t.colourRange === undefined)
                        t.colourRange = cfg.fullRange ? 2 : 1;
                }
            }
            ...
            if (t.bitDepthLuma && !v.bitDepth)
                v.bitDepth = t.bitDepthLuma;
            if (isSpecifiedColourCode(t.primaries))
                v.primaries = COLOUR_PRIMARIES[t.primaries] || ('code ' + t.primaries);
            if (isSpecifiedColourCode(t.transfer))
                v.transfer = TRANSFER_CHARS[t.transfer] || ('code ' + t.transfer);
            if (isSpecifiedColourCode(t.matrix))
                v.matrixName = MATRIX_COEFFS[t.matrix] || ('code ' + t.matrix);
            if (t.colourRange === 1)
                v.range = 'Limited (TV)';
            else if (t.colourRange === 2)
                v.range = 'Full (PC)';
            if (t.transfer === 16)
                v.hdr = 'PQ (' + (v.primaries || 'BT.2020') + ')';
            else if (t.transfer === 18)
...
//# sourceMappingURL=video-bitstream.js.map