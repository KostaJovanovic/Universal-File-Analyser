...
function aspectRatio(row, sideLen, totalArea) {
    if (!row.length || !sideLen || !totalArea)
        return Infinity;
...
function squarify(children, rect) {
    ...
    if (sorted.reduce((s, c) => s + c.size, 0) <= 0)
        return [];
    ...
    suffix[n] = 0;
    for (let k = n - 1; k >= 0; k--)
        suffix[k] = suffix[k + 1] + sorted[k].size;
    const results = [];
    let i = 0;
    let curRect = { ...rect };
    while (i < n) {
...
function layoutNode(node, rect, depth, out) {
    if (depth > MAX_DEPTH || rect.w < 2 || rect.h < 2)
        return;
    const entries = [];
    for (const child of Object.values(node.children)) {
        if (child.totalSize > 0)
            entries.push({ size: child.totalSize, dir: child });
    }
    for (const f of node.files) {
        if (f.size > 0)
            entries.push({ size: f.size, file: f });
    }
    if (!entries.length)
        return;
    ...
    if (tiny.length >= AGG_MIN_FILES) {
        ...
        for (const e of tiny) {
            bytes += e.size;
            const c = e.file.category || 'other';
            catB[c] = (catB[c] || 0) + e.size;
        }
        let cat = 'other', cv = -1;
        for (const k in catB)
            if (catB[k] > cv) {
    ...
    for (const p of placed) {
        ...
        if (p.item.aggregate) {
            ...
            if (inner.w > 3 && inner.h > 3)
                layoutNode(child, inner, depth + 1, out);
        }
    }
}
// ---------- rendering ----------
...
export function renderTreemap(canvas, items, opts) {
    ...
    if (!canvas._hierarchy)
        canvas._hierarchy = buildHierarchy(items);
    ...
    for (const a of out.aggregates) {
        ...
        if (aw > 4 && ah > 4) {
            ...
            ctx.lineWidth = 1;
            ctx.beginPath();
            for (let i = -ah; i < aw; i += 6) {
                ctx.moveTo(x + i, y);
                ctx.lineTo(x + i + ah, y + ah);
            }
            ctx.stroke();
            ctx.restore();
            ctx.fillStyle = 'rgba(255,255,255,0.20)';
            ctx.fillRect(x, y, aw, 1);
            ctx.fillRect(x, y, 1, ah);
            ctx.fillStyle = 'rgba(0,0,0,0.22)';
            ctx.fillRect(x, y + ah - 1, aw, 1);
            ctx.fillRect(x + aw - 1, y, 1, ah);
        ...
        if (aw >= LABEL_MIN_W && ah >= LABEL_MIN_H) {
            ...
            if (ctx.measureText(lab).width > maxW) {
                while (lab.length > 2 && ctx.measureText(lab + '…').width > maxW)
                    lab = lab.slice(0, -1);
                lab += '…';
            }
...
function onOutside(e) { if (_tmMenu && !_tmMenu.contains(e.target))
    closeFileMenu(); }
...
function showFileMenu(clientX, clientY, item, onConfirm) {
    ...
    if (px + mw > window.innerWidth)
        px = clientX - mw - 8;
    if (py + mh > window.innerHeight)
        py = clientY - mh - 8;
...
function onListOutside(e) { if (_tmList && !_tmList.contains(e.target))
    closeFileList(); }
function onListEsc(e) { if (e.key === 'Escape')
    closeFileList(); }
...
function onListScroll(e) { if (_tmList && _tmList.contains(e.target))
...
function showFileListMenu(clientX, clientY, files, opts) {
    ...
    const title = el('div', { class: 'anr-tml-title' }, sorted.length.toLocaleString() + ' files · ' + fmtBytes(total));
    ...
    function render(filter) {
        ...
        for (const f of sorted) {
            ...
            rowEl.addEventListener('click', () => { closeFileList(); if (opts.onFileClick)
                opts.onFileClick(f); });
        ...
        if (!matched) {
            ...
            listEl.appendChild(el('div', { class: 'anr-tml-note' }, (matched - shown).toLocaleString() + ' more — keep typing to narrow it down'));
        }
    }
...
export function attachTreemapEvents(canvas, wrap, items, opts) {
    ...
    function zoomTo(node) {
        ...
        const chain = [];
        for (let n = node; n && n !== canvas._hierarchy; n = n.parent)
            chain.unshift(n);
        zoomStack.length = 0;
    ...
    canvas.addEventListener('mousemove', (e) => {
        ...
        if (header || collapsed) {
            ...
            tooltip.appendChild(el('div', { class: 'anr-tt-meta' }, dir.node.fileCount + (dir.node.fileCount === 1 ? ' file · ' : ' files · ') + fmtBytes(dir.node.totalSize) + '  ·  zoom in'));
            positionTooltip(e);
            ...
            tooltip.appendChild(el('div', { class: 'anr-tt-meta' }, fmtBytes(aggregate.agg.bytes) + '  ·  click to list'));
            positionTooltip(e);
    ...
    function positionTooltip(e) {
        ...
        if (tx + tooltip.offsetWidth > wrapRect.width)
            tx = e.clientX - wrapRect.left - tooltip.offsetWidth - 8;
        if (ty + tooltip.offsetHeight > wrapRect.height)
            ty = e.clientY - wrapRect.top - tooltip.offsetHeight - 8;
    ...
    canvas.addEventListener('click', (e) => {
        ...
        if (collapsed) {
            zoomTo(collapsed.node);
            return;
        }
        const aggregate = hitRects(canvas._aggregateRects, x, y);
        if (aggregate) {
            showFileListMenu(e.clientX, e.clientY, aggregate.agg.files, opts);
            return;
        }
