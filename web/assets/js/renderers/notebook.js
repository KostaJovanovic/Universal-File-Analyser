...
function mdInline(escaped) {
    ...
    s = s.replace(/(^|[^*])\*([^*\n]+)\*/g, '$1<em>$2</em>');
    s = s.replace(/\[([^\]]+)\]\((https?:\/\/[^)\s]+)\)/g, (m, t, h) => '<a class="anr-md-link" href="' + h + '" target="_blank" rel="noopener noreferrer">' + t + '</a>');
    return s;
}
function mdToEl(src) {
    ...
    const flushPara = () => { if (para.length) {
        html += '<p>' + mdInline(esc(para.join(' '))) + '</p>';
        para = [];
    } };
    ...
    for (const ln of lines) {
        if (/^```/.test(ln)) {
            ...
            closeList();
            if (!inCode) {
                html += '<pre class="anr-nb-mdcode"><code>';
                inCode = true;
            }
            else {
                html += '</code></pre>';
...
export async function renderNotebook(file, container) {
    ...
    info.appendChild(buildReadout([
        ...
        rowHelp('Breakdown', counts.code + ' code, ' + counts.markdown + ' markdown' + (counts.raw ? ', ' + counts.raw + ' raw' : ''), 'How the notebook’s cells divide up: code cells that can be run, markdown cells holding the written explanation, and raw cells of plain unformatted text.'),
    ]));
    ...
    function reveal(upTo) {
        for (; shown < upTo && shown < cells.length; shown++)
            addCell(shown);
        if (shown >= cells.length)
            btnRow.hidden = true;
        else {
            btnRow.hidden = false;
            moreBtn.textContent = 'Show more cells (' + shown + '/' + cells.length + ')';
        }
    }
