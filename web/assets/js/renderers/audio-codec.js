...
export async function peekContainer(file) {
    ...
    if (ascii(0, 3) === 'ID3') {
        ...
        try {
            Object.assign(base, await detailMp3(file));
        }
        catch (_) { /* best-effort */ }
        return base;
    }
    ...
    if (head[0] === 0xFF && (head[1] & 0xF0) === 0xF0 && (head[1] & 0x06) === 0x00) {
        ...
        try {
            Object.assign(base, await detailAac(file));
        }
        catch (_) { /* best-effort */ }
        return base;
    }
    ...
    // MP4/M4A
    if (ascii(4, 4) === 'ftyp')
        return { container: 'MP4/M4A', codec: ascii(8, 4).trim() };
...
function detailFlac(b, fileSize, sampleRate, channels, bitDepth) {
    ...
    if (total > 0 && fileSize > 0 && sampleRate > 0) {
        ...
        if (uncompressed > 0)
            out.compressionRatio = uncompressed / fileSize;
    }
    return out;
}
...
async function detailMp3(file) {
    ...
    for (let i = off; i + 4 <= buf.length; i++) {
        ...
        if (verBits === 1 || layerBits === 0 || brIdx === 0 || brIdx === 0x0F || srIdx === 3)
            continue;
        h = i;
        break;
    }
    ...
    if (xt === 'Xing' || xt === 'Info') {
        ...
        let p = xingOff + 8;
        if (flags & 0x01) {
            frameCount = ((buf[p] << 24 | buf[p + 1] << 16 | buf[p + 2] << 8 | buf[p + 3]) >>> 0);
            p += 4;
        }
        if (flags & 0x02)
            p += 4; // bytes field
        ...
        if (p + 9 <= buf.length) {
            ...
            if (/^(LAME|Lavf|Lavc|GOGO|Xing)/i.test(enc)) {
                ...
                if (MP3_LAME_PRESETS[presetCode])
                    encoder += ' (' + MP3_LAME_PRESETS[presetCode] + ')';
            }
    ...
    if (vbr) {
        ...
        if (frameCount > 0 && sampleRate > 0) {
            ...
            if (duration > 0)
                avg = Math.round((file.size * 8) / duration / 1000);
        }
        if (!avg)
            avg = frameBitrate;
        if (avg > 0) {
...
function scanAdts(b, from) {
    for (let start = from; start + 7 <= b.length; start++) {
        if (b[start] !== 0xFF || (b[start + 1] & 0xF6) !== 0xF0)
            continue;
        ...
        while (i + 7 <= b.length) {
            ...
            if (len < 7)
                break;
            if (i + len > b.length)
                break; // frame runs past the window - stop, keep complete frames
            if (!first)
                first = { profile: ((b[i + 2] >> 6) & 3) + 1, freqIdx, chanCfg: ((b[i + 2] & 1) << 2) | ((b[i + 3] >> 6) & 3) };
            frames++;
            bytes += len;
            i += len;
        }
        if (frames >= 3)
            return { first, frames, bytes, start };
    }
    return null;
}
async function detailAac(file) {
    ...
    if (first.chanCfg)
        out.channels = first.chanCfg; // 0 = "in the AudioSpecificConfig" - unknown here
    ...
    if (file.size > WIN * 2) {
        ...
        const wins = await Promise.all(offsets.map((o) => file.slice(o, Math.min(file.size, o + WIN)).arrayBuffer().then((ab) => new Uint8Array(ab)).catch(() => null)));
        for (const w of wins) {
            if (!w)
                continue;
...
export function adtsToM4a(arrayBuffer) {
    ...
    while (i + 7 <= src.length) {
        if (src[i] !== 0xFF || (src[i + 1] & 0xF6) !== 0xF0) {
            i++;
            continue;
        }
        profile = ((src[i + 2] >> 6) & 3) + 1;
        freqIdx = (src[i + 2] >> 2) & 0xF;
        ...
        if (len < 7 || i + len > src.length)
            break;
        const hdr = (src[i + 1] & 1) ? 7 : 9;
        frameData.push(src.slice(i + hdr, i + len));
        frameSizes.push(len - hdr);
        i += len;
    }
    ...
    w1(0x05);
    w1(2);
    w1((profile << 3) | (freqIdx >> 1));
    w1(((freqIdx & 1) << 7) | (chanCfg << 3));
    w1(0x06);
    w1(1);
    w1(0x02);
    bx('stts', 24);
...
export async function readTagBPM(file) {
    ...
    if (head[0] === 0x49 && head[1] === 0x44 && head[2] === 0x33) {
        ...
        while (pos + hdrLen < buf.length && pos < tagSize + 10) {
            ...
            let sz;
            if (ver === 2)
                sz = (buf[pos + 3] << 16) | (buf[pos + 4] << 8) | buf[pos + 5];
            else if (ver === 4)
                sz = ((buf[pos + 4] & 0x7F) << 21) | ((buf[pos + 5] & 0x7F) << 14) |
                    ((buf[pos + 6] & 0x7F) << 7) | (buf[pos + 7] & 0x7F);
            else
                sz = (buf[pos + 4] << 24) | (buf[pos + 5] << 16) | (buf[pos + 6] << 8) | buf[pos + 7];
            if (sz <= 0 || pos + hdrLen + sz > buf.length)
                break;
            if (id === target) {
                ...
                if (enc === 0 || enc === 3)
                    ...
                    text = new TextDecoder(enc === 2 ? 'utf-16be' : 'utf-16').decode(data.slice(1));
...
export async function extractCoverArt(file) {
    ...
    if (head[0] === 0x49 && head[1] === 0x44 && head[2] === 0x33)
        return extractId3Pic(file, head);
    if (head[0] === 0x66 && head[1] === 0x4C && head[2] === 0x61 && head[3] === 0x43)
        return extractFlacPic(file);
    if (head[4] === 0x66 && head[5] === 0x74 && head[6] === 0x79 && head[7] === 0x70)
        return extractMp4Cover(file);
    return null;
}
async function extractId3Pic(file, head) {
    ...
    while (pos + hdrLen < buf.length && pos < tagSize + 10) {
        ...
        if (id === target) {
            ...
            let mime;
            if (target === 'PIC') {
                const fmt = String.fromCharCode(data[p], data[p + 1], data[p + 2]);
                p += 3;
...
async function extractMp4Cover(file) {
    ...
    for (let i = 0; i + 24 < buf.length; i++) {
        ...
        if (buf[i] === 0x63 && buf[i + 1] === 0x6F && buf[i + 2] === 0x76 && buf[i + 3] === 0x72) {
            ...
            if (d + 16 > buf.length)
                continue;
            if (!(buf[d + 4] === 0x64 && buf[d + 5] === 0x61 && buf[d + 6] === 0x74 && buf[d + 7] === 0x61))
                continue;
...
async function extractFlacPic(file) {
    ...
    while (pos + 4 <= buf.length) {
        ...
        if (type === 6 && body + 32 <= buf.length) { // PICTURE
            ...
            p += 4;
            const mime = String.fromCharCode(...buf.slice(p, p + mimeLen));
            p += mimeLen;
            ...
            if (dataLen > 0 && p + dataLen <= buf.length)
                return { bytes: buf.slice(p, p + dataLen), mime: mime || 'image/jpeg' };
        }
        if (last)
            break;
        pos = body + len;
...
export async function readAudioTags(file) {
    try {
        const head = new Uint8Array(await file.slice(0, 12).arrayBuffer());
        if (head[0] === 0x49 && head[1] === 0x44 && head[2] === 0x33)
            return await readId3Tags(file, head);
        if (head[0] === 0x66 && head[1] === 0x4C && head[2] === 0x61 && head[3] === 0x43)
            return await readFlacTags(file);
        if (head[4] === 0x66 && head[5] === 0x74 && head[6] === 0x79 && head[7] === 0x70)
            return await readMp4Tags(file);
        if (head[0] === 0x4F && head[1] === 0x67 && head[2] === 0x67 && head[3] === 0x53)
            return await readVorbisLikeTags(file);
    }
    catch (_) { /* ignore */ }
...
function decodeStr(bytes, enc) {
    try {
        if (enc === 1)
            return new TextDecoder('utf-16').decode(bytes); // UTF-16 w/ BOM
        if (enc === 2)
...
async function readId3Tags(file, head) {
    ...
    while (pos + hdrLen < buf.length && pos < tagSize + 10) {
        const id = String.fromCharCode(...buf.slice(pos, pos + idLen));
        if (id[0] === '\0' || !/^[A-Z0-9]+$/.test(id))
            break;
        let sz;
        if (ver === 2)
            sz = (buf[pos + 3] << 16) | (buf[pos + 4] << 8) | buf[pos + 5];
        else if (ver === 4)
            sz = ((buf[pos + 4] & 0x7F) << 21) | ((buf[pos + 5] & 0x7F) << 14) | ((buf[pos + 6] & 0x7F) << 7) | (buf[pos + 7] & 0x7F);
        else
            sz = (buf[pos + 4] << 24) | (buf[pos + 5] << 16) | (buf[pos + 6] << 8) | buf[pos + 7];
        if (sz <= 0 || pos + hdrLen + sz > buf.length)
            break;
...
function parseVorbisComments(buf, start, dv) {
    ...
    let vendor = '';
    try {
        vendor = clean(new TextDecoder('utf-8').decode(buf.slice(p, p + vlen)));
    }
    catch (_) { /* ignore */ }
    p += vlen;
    let count = dv.getUint32(p, true);
    p += 4;
    for (let i = 0; i < count && p + 4 <= buf.length; i++) {
        ...
        if (len <= 0 || p + len > buf.length)
            break;
        const entry = new TextDecoder('utf-8').decode(buf.slice(p, p + len));
        p += len;
        ...
        if (!val)
            continue;
        if (key === 'LYRICS' || key === 'UNSYNCEDLYRICS' || key === 'LYRICS-XXX')
            lyrics = val;
        else if (VORBIS_NAMES[key])
            tags.push([VORBIS_NAMES[key], val]);
    }
    if (vendor) {
...
async function readFlacTags(file) {
    ...
    while (pos + 4 <= buf.length) {
        ...
        const len = (buf[pos + 1] << 16) | (buf[pos + 2] << 8) | buf[pos + 3];
        const body = pos + 4;
        if (type === 4 && body + 8 <= buf.length)
            return parseVorbisComments(buf, body, dv); // VORBIS_COMMENT
        if (flag & 0x80)
            break;
        pos = body + len;
    }
...
async function readMp4Tags(file) {
    ...
    for (let i = 0; i + 24 < buf.length; i++) {
        let atom = null;
        for (const k of keys) {
            if (buf[i] === k.charCodeAt(0) && buf[i + 1] === k.charCodeAt(1) && buf[i + 2] === k.charCodeAt(2) && buf[i + 3] === k.charCodeAt(3)) {
                atom = k;
                break;
            }
        }
        ...
        if (valLen <= 0 || valStart + valLen > buf.length)
            continue;
