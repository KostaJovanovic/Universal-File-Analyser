...
function detectSamples(font) {
    if (!font || typeof font.charToGlyphIndex !== 'function')
        return [SCRIPT_SAMPLES[0]];
    const has = (probe) => { for (const ch of probe) {
        let i = 0;
        try {
            i = font.charToGlyphIndex(ch);
        }
        catch (_) {
            i = 0;
        }
...
function specimenCard(family, axes, font) {
    ...
    if (axes.length) {
        ...
        const kick = () => { if (!rafId && playing.size)
            rafId = requestAnimationFrame(frame); };
        const toggle = (a) => { if (playing.has(a))
            playing.delete(a);
        else
            startAxis(a, performance.now()); syncButtons(); kick(); };
        playAllBtn.addEventListener('click', () => {
            if (playing.size === axes.length) {
                playing.clear();
            }
            else {
...
function glyphGridCard(font) {
    ...
    if (total > CAP)
        card.appendChild(el('p', { class: 'anr-hint', style: 'margin:0 0 8px;' }, `Showing the first ${CAP} glyph outlines.`));
...
function extractSfnt(buf, offset) {
    try {
        ...
        recs.forEach((r, i) => {
            ...
            odv.setUint32(rec + 8, r.newOff);
            odv.setUint32(rec + 12, r.len);
            if (r.off + r.len <= src.length)
                out.set(src.subarray(r.off, r.off + r.len), r.newOff);
        });
        return out;
    }
    catch (_) {
...
function extractCollection(buf) {
    try {
        const dv = new DataView(buf);
        if (dv.getUint32(0) !== 0x74746366)
            return null; // 'ttcf'
        const numFonts = dv.getUint32(8);
        if (!numFonts || numFonts > 500)
            return null;
        const out = [];
        for (let i = 0; i < numFonts; i++) {
            const f = extractSfnt(buf, dv.getUint32(12 + i * 4));
            if (f)
                out.push(f);
        }
...
function fontMetaCard(font, axes) {
    ...
    const fam = fontName(font, 'fontFamily');
    if (fam)
        tbl.appendChild(row('Family', fam));
    const sub = fontName(font, 'fontSubfamily');
    if (sub)
        tbl.appendChild(row('Style', sub));
    const ver = fontName(font, 'version');
    if (ver)
        tbl.appendChild(row('Version', ver.replace(/^Version\s*/i, '')));
    const designer = fontName(font, 'designer');
    if (designer)
        tbl.appendChild(row('Designer', designer));
    const manuf = fontName(font, 'manufacturer');
    if (manuf)
        tbl.appendChild(rowHelp('Foundry', manuf, 'The type foundry - the company or studio that designs and sells the font.'));
    const lic = fontName(font, 'license');
    if (lic)
        tbl.appendChild(rowHelp('Licence', lic.length > 160 ? lic.slice(0, 160) + '…' : lic, 'The licence text the font maker stored inside the font’s name records.'));
    if (font && font.outlinesFormat)
        tbl.appendChild(rowHelp('Outlines', font.outlinesFormat === 'cff' ? 'PostScript (CFF)' : 'TrueType (glyf)', 'How the letter shapes are drawn inside the font. The two common styles - PostScript (CFF) and TrueType (glyf) - describe curves slightly differently but both render normally.'));
    if (font && font.unitsPerEm)
        tbl.appendChild(rowHelp('Units per em', String(font.unitsPerEm), 'The size of the design grid each letter is drawn on. A bigger number (often 1000 or 2048) allows finer detail; it does not change how large the text appears.'));
    if (font && font.glyphs && font.glyphs.length)
        tbl.appendChild(rowHelp('Glyphs', font.glyphs.length.toLocaleString(), 'How many individual shapes the font contains. A glyph is one drawn form - a letter, digit, accented character, symbol or ligature - so a font usually has far more glyphs than there are letters in the alphabet.'));
    if (axes.length)
        tbl.appendChild(rowHelp('Variable', axes.length + (axes.length === 1 ? ' axis' : ' axes') + ' - ' + axes.map((a) => a.tag).join(', '), 'A variable font packs many styles into one file, adjustable along sliding scales (axes) such as weight or width; the sliders in the sample above blend between them live.'));
    const copyright = fontName(font, 'copyright');
    if (copyright)
        tbl.appendChild(rowHelp('Copyright', copyright.length > 160 ? copyright.slice(0, 160) + '…' : copyright, 'The copyright notice stored inside the font’s name records.'));
    if (!tbl.children.length)
        tbl.appendChild(row('Naming', 'not available'));
...
export async function renderFont(file, resultsEl) {
    ...
    try {
        ...
        resultsEl.innerHTML = '';
        resultsEl.appendChild(errorCard('Could not read this font file.'));
        return;
    }
    ...
    if (members) {
        members.forEach((bytes, i) => {
            ...
            if (ot) {
                try {
                    f = ot.parse(bytes.buffer.slice(0));
                }
                catch (_) {
                    f = null;
                }
    ...
    if (isColl)
        fmtLabel += ' - ' + entries.length + (entries.length === 1 ? ' font' : ' fonts');
    ...
    if (entries.length > 1) {
        ...
        const sel = el('select', { class: 'anr-btn anr-select', style: 'max-width:100%;' }, entries.map((e, i) => el('option', { value: String(i) }, e.label)));
    ...
    async function renderMember() {
        ...
        if (faceOk)
            specimenWrap.appendChild(specimenCard(family, axes, e.font));
        else
            specimenWrap.appendChild(el('div', { class: 'anr-info' }, 'The browser could not load this font for live preview, so only its metadata and glyph outlines are shown.'));
        // Glyph grid via opentype.js.
        if (e.font) {
            const gg = glyphGridCard(e.font);
            if (gg)
                subWrap.appendChild(gg);
        }
        else {
