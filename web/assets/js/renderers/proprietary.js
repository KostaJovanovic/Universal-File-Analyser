...
function forceDeviceViewport(html) {
    ...
    if (vpRe.test(html))
        return html.replace(vpRe, VIEWPORT_META);
    if (/<head\b[^>]*>/i.test(html))
        return html.replace(/<head\b[^>]*>/i, (m) => m + VIEWPORT_META);
...
function parseFbx(buf) {
    ...
    if (!sig.startsWith('Kaydara FBX Binary'))
        return null;
    if (buf.length < 27)
        return { 'Format': 'FBX Binary' };
    const view = new DataView(buf.buffer, buf.byteOffset, buf.byteLength);
...
function parsePe(buf) {
    ...
    if (characteristics & 0x2000)
        ...
        chFlags.push('Executable');
    if (characteristics & 0x0020)
        chFlags.push('Large-address-aware');
    if (characteristics & 0x0001)
        chFlags.push('Relocs-stripped');
    if (characteristics & 0x0100)
        chFlags.push('32-bit-machine');
    if (chFlags.length)
        result['Characteristics'] = chFlags.join(', ');
    ...
    if (secNames.length)
        result['Section names'] = secNames.join(', ');
    if (secTable.length)
        result._peSections = secTable;
    ...
    let installer = null;
    if (secNames.includes('.ndata'))
        installer = 'NSIS (Nullsoft Scriptable Install System)';
    else if (secNames.includes('.wixburn'))
        installer = 'WiX Burn bundle';
    else {
        try {
            const txt = new TextDecoder('latin1').decode(buf.subarray(0, Math.min(buf.length, 65536)));
            if (/Inno Setup/.test(txt))
                installer = 'Inno Setup';
            else if (/InstallShield/.test(txt))
                installer = 'InstallShield';
            else if (/Nullsoft|\bNSIS\b/.test(txt))
                installer = 'NSIS (Nullsoft Scriptable Install System)';
        }
        catch (_) { }
    }
    ...
    try {
        ...
        if (optBase + 72 <= buf.length) {
            ...
            const sec = [];
            if (dc & 0x0020)
                sec.push('High-entropy ASLR');
            if (dc & 0x0040)
                sec.push('ASLR');
            if (dc & 0x0080)
                sec.push('Force-integrity');
            if (dc & 0x0100)
                sec.push('DEP/NX');
            if (dc & 0x0400)
                sec.push('No-SEH');
            if (dc & 0x4000)
                sec.push('Control-Flow-Guard');
            if (dc & 0x8000)
                sec.push('Terminal-server-aware');
            if (sec.length)
                result['Security mitigations'] = sec.join(', ');
        }
        const entryOff = optBase + 16;
        if (entryOff + 4 <= buf.length) {
            const ep = view.getUint32(entryOff, true);
            if (ep)
                result['Entry point'] = '0x' + ep.toString(16).toUpperCase();
        }
...
function readUtf16Value(buf, key) {
    ...
    for (let i = 0; i < key.length; i++) {
        kb.push(key.charCodeAt(i) & 0xFF, 0);
    }
    const dv = new DataView(buf.buffer, buf.byteOffset, buf.byteLength);
    outer: for (let i = 6; i + kb.length + 2 <= buf.length; i++) {
...
async function parseExe(c) {
    ...
    if (c.ext === 'scr')
        result['Screensaver'] = 'Runs with /s (show), /p (preview), /c (configure)';
    ...
    if (rsrc && rsrc.size && c.file) {
        try {
            ...
            if (sig >= 0) {
                ...
                if (fileMS || fileLS)
                    result['File version'] = ver(fileMS, fileLS);
                if (prodMS || prodLS)
                    result['Product version'] = ver(prodMS, prodLS);
            }
            const fields = [
    ...
    delete result._peSections;
    delete result._peMeta;
    if (secTable && secTable.length && peMeta && c.file) {
        try {
            ...
            let lastEnd = 0;
            for (const s of secTable)
                if (s.rawOff && s.rawSize)
                    lastEnd = Math.max(lastEnd, s.rawOff + s.rawSize);
            ...
            if (rep) {
                ...
                if (named.length)
                    result['Packed with'] = named.map((m) => m.name).join(', ');
                if (built.length)
                    result['Built with'] = built.map((m) => m.name).join(', ');
                result['Packing'] = rep.verdict;
                const node = el('div', {});
                node.appendChild(el('p', { class: 'anr-hint', style: 'margin:0 0 8px;' }, 'Packing means the real code is compressed or encrypted in the file and unpacked in memory at start-up. Perfectly ordinary software does it - installers, commercial products and anti-cheat all ship packed - and so does malware trying to defeat a signature scanner. What follows is the evidence, not a verdict on the file.'));
                ...
                for (const s of rep.signals)
                    node.appendChild(el('div', { class: 'anr-pack-signal' }, s));
                ...
                for (const s of rep.sections) {
                    const entCell = el('td', {});
                    if (s.entropy == null)
                        entCell.appendChild(document.createTextNode('-'));
                    else {
                ...
                node.appendChild(el('p', { class: 'anr-hint', style: 'margin:10px 0 4px;' }, 'Entropy is how unpredictable a section’s bytes are, in bits per byte. Compiled code lands near 6; compressed or encrypted data pushes 8.'));
                node.appendChild(t);
...
export async function extractPeIcon(file) {
    ...
    try {
        ...
        const readDir = (dirOff) => {
            if (dirOff < 0 || dirOff + 16 > buf.length)
                return [];
        ...
        const leafData = (leafOff) => {
            if (leafOff < 0 || leafOff + 16 > buf.length)
                return null;
        ...
        if (groupTypeOff < 0 || iconTypeOff < 0)
            return null;
        const iconById = {};
        ...
        for (let i = 0; i < count; i++) {
            ...
            if (!best || score > best.score)
                best = { w, h, planes: dv.getUint16(e + 4, true), bitCount, id: dv.getUint16(e + 12, true), score };
        }
        if (!best || !iconById[best.id])
            return null;
...
async function parseGlif(file) {
    let text;
    try {
        text = await file.slice(0, 4 * 1024 * 1024).text();
    }
    catch (_) {
        return null;
    }
    if (!/<\s*glyph[\s>]/.test(text))
        return null;
    let doc;
    try {
    ...
    if (!glyph || doc.querySelector('parsererror'))
        return null;
    const fields = {};
    const name = glyph.getAttribute('name');
    if (name)
        fields['Glyph name'] = name;
    ...
    if (adv) {
        ...
        if (w != null)
            fields['Advance width'] = w + ' units';
        if (h != null)
            fields['Advance height'] = h + ' units';
    }
    ...
    if (unis.length) {
        fields['Unicode'] = unis.map((h) => {
            ...
            if (isFinite(cp) && cp >= 0x20 && !(cp >= 0xD800 && cp <= 0xDFFF)) {
                try {
    ...
    if (contours.length)
        fields['Contours'] = contours.length;
    if (points.length)
        fields['Points'] = points.length + ' (' + onCurve + ' on-curve, ' + (points.length - onCurve) + ' off-curve)';
    ...
    if (guides.length)
        fields['Guidelines'] = guides.length;
    const imageEl = glyph.querySelector('image');
    if (imageEl)
        fields['Image'] = imageEl.getAttribute('fileName') || 'yes';
    if (glyph.querySelector('note'))
        fields['Note'] = 'present';
    if (glyph.querySelector('lib'))
        fields['Lib data'] = 'present';
    const outline = renderGlifOutline(contours);
    if (outline)
        fields._previewNode = outline;
    else if (components.length)
        fields._previewNode = el('p', { class: 'anr-hint', style: 'margin-top:12px;' }, 'Composed of ' + components.length + ' component' + (components.length > 1 ? 's' : '') + ' - the outline needs the rest of the UFO to draw.');
    else
        fields._previewNode = el('p', { class: 'anr-hint', style: 'margin-top:12px;' }, 'No outline (e.g. a space or an empty glyph).');
...
function glifContourSegments(pts) {
    ...
    if (!isOpen) {
        ...
        if (startIdx < 0)
            return null; // all off-curve - skip
    ...
    for (const p of seq) {
        ...
        if (p.type === 'line' || p.type === 'move') {
            ...
            if (pending.length >= 2)
                ops.push({ t: 'C', p: [pending[pending.length - 2], pending[pending.length - 1], [p.x, p.y]] });
            else if (pending.length === 1)
                ops.push({ t: 'Q', p: [pending[0], [p.x, p.y]] });
            else
                ops.push({ t: 'L', p: [[p.x, p.y]] });
        }
...
function renderGlifOutline(contours) {
    ...
    for (const segs of parsed) {
        ...
        for (const op of segs.ops) {
            if (op.t === 'L')
                ctx.lineTo(tx(op.p[0][0]), ty(op.p[0][1]));
            else if (op.t === 'Q')
                ctx.quadraticCurveTo(tx(op.p[0][0]), ty(op.p[0][1]), tx(op.p[1][0]), ty(op.p[1][1]));
            else if (op.t === 'C')
                ctx.bezierCurveTo(tx(op.p[0][0]), ty(op.p[0][1]), tx(op.p[1][0]), ty(op.p[1][1]), tx(op.p[2][0]), ty(op.p[2][1]));
        }
        if (segs.closed)
            ctx.closePath();
    }
...
function readFvarTable(buf, fvarOff) {
    ...
    for (let i = 0; i < axisCount; i++) {
        ...
        if (max > min)
            axes.push({ tag, name: AXIS_NAMES[tag] || tag, min, def, max });
    }
    return axes;
}
...
async function parseFont(file) {
    ...
    if (magic === 0x00010000 || magic === 0x4F54544F || tag4 === 'true' || tag4 === 'typ1') {
        ...
        if (offs.name)
            names = readNameTable(buf, offs.name);
        if (offs.fvar)
            axes = readFvarTable(buf, offs.fvar);
    }
    else if (tag4 === 'ttcf') {
        ...
        if (t.name)
            names = readNameTable(t.name, 0);
        if (t.fvar)
            axes = readFvarTable(t.fvar, 0);
    }
    else {
...
async function parseFlp(file) {
    ...
    if (buf.length < 10 || ascii(buf, 0, 4) !== 'FLhd')
        return null;
    const view = new DataView(buf.buffer, buf.byteOffset, buf.byteLength);
    ...
    if (channelCount)
        fields['Channels'] = channelCount;
    if (ppq)
        fields['PPQ (ticks/beat)'] = ppq;
    // Locate the FLdt data chunk (right after the FLhd chunk).
    let pos = 8 + headerLen;
    if (pos + 8 > buf.length || ascii(buf, pos, 4) !== 'FLdt')
        return fields;
    ...
    while (pos < end) {
        ...
        if (id < 0x40) { // BYTE event
            ...
            if (id === 0x42 && pos + 2 <= end)
                tempo = view.getUint16(pos, true); // legacy Tempo
            ...
            if (id === 0x41 && pos + 2 <= end)
                maxPattern = Math.max(maxPattern, view.getUint16(pos, true));
            pos += 2;
        }
        else if (id < 0xC0) { // DWORD event
            if (id === 0x9C && pos + 4 <= end)
                tempo = view.getUint32(pos, true) / 1000; // FineTempo
            pos += 4;
        }
            ...
            switch (id) {
                case 199:
                    version = decodeFlpText(data);
                    break; // FLP_Version (ASCII)
                case 194:
                    title = decodeFlpText(data);
                    break; // Title
                case 195:
                    comment = decodeFlpText(data);
                    break; // Comment
                case 206:
                    genre = decodeFlpText(data);
                    break; // Genre
                case 207:
                    author = decodeFlpText(data);
                    break; // Author
                case 192: {
                    const n = decodeFlpText(data);
                    if (n)
                        channelNames.push(n);
                    break;
                } // ChanName
                ...
                case 201:
                case 203: { // DefPluginName / PluginName
                    const n = decodeFlpText(data);
                    if (n && /[a-zA-Z]/.test(n))
                        plugins.add(n);
                    break;
    ...
    if (version)
        fields['FL Studio version'] = version;
    if (tempo)
        fields['Tempo'] = (Math.round(tempo * 100) / 100) + ' BPM';
    if (title)
        fields['Title'] = title;
    if (author)
    ...
    if (genre)
        fields['Genre'] = genre;
    if (comment)
        fields['Comment'] = comment.slice(0, 300);
    if (maxPattern)
        fields['Patterns'] = maxPattern;
    if (channelNames.length)
        fields['Named channels'] = channelNames.slice(0, 30).join(', ');
    if (patternNames.length)
        fields['Named patterns'] = patternNames.slice(0, 30).join(', ');
    if (plugins.size)
        fields['Plugins'] = [...plugins].slice(0, 40).join(', ');
    if (samples.size)
        fields['Samples referenced'] = samples.size;
...
function parseRar(buf) {
    ...
    if (buf[4] === 0x1A && buf[5] === 0x07 && buf[6] === 0x01)
        return { 'RAR version': '5.x' };
    if (buf[4] === 0x1A && buf[5] === 0x07 && buf[6] === 0x00)
        return { 'RAR version': '4.x / earlier' };
    ...
    if (file && file.size < 268435456) {
        try {
            ...
            if (a) {
                ...
                if (a.pragma.encoding)
                    out['Encoding'] = a.pragma.encoding;
                if (a.pragma.page_count)
                    out['Pages'] = Number(a.pragma.page_count).toLocaleString();
                if (a.pragma.user_version)
                    out['user_version'] = a.pragma.user_version;
                if (a.pragma.application_id)
                    out['application_id'] = a.pragma.application_id;
                out['Tables'] = a.tables.length;
                if (a.views.length)
                    out['Views'] = a.views.length;
                if (a.indexes.length)
                    out['Indexes'] = a.indexes.length;
                if (a.triggers.length)
                    out['Triggers'] = a.triggers.length;
                ...
                if (a.tables.length) {
                    ...
                    const cols = el('div', {});
                    for (const tb of a.tables) {
                        cols.appendChild(el('div', { class: 'anr-readout-section' }, tb.name + (tb.rows != null ? ' - ' + Number(tb.rows).toLocaleString() + ' rows' : '')));
                        const ct = el('table', { class: 'anr-readout' });
                        for (const c of tb.cols)
                            ct.appendChild(row(c.name + (c.pk ? ' (PK)' : ''), c.type));
                        cols.appendChild(ct);
                    }
                ...
                if (a.sample && a.sample.rows.length) {
                    ...
                    const hr = el('tr', {});
                    for (const c of a.sample.columns)
                        hr.appendChild(el('th', {}, String(c)));
                    st.appendChild(hr);
                    for (const r of a.sample.rows) {
                        const tr = el('tr', {});
                        for (const cell of r)
                            tr.appendChild(el('td', {}, cell == null ? 'NULL' : String(cell).slice(0, 160)));
                        st.appendChild(tr);
                    }
                    wrap.appendChild(st);
                ...
                // it just drops the copy and re-reads the original bytes.
                {
                    ...
                    const ta = el('textarea', { class: 'anr-sql-input', spellcheck: 'false', rows: '4' });
                    ta.value = firstTable
                        ? 'SELECT * FROM "' + firstTable.replace(/"/g, '""') + '" LIMIT 50;'
                        : 'SELECT name FROM sqlite_master;';
                    const runBtn = el('button', { type: 'button', class: 'anr-btn anr-btn-sm' }, 'Run');
                    const resetBtn = el('button', { type: 'button', class: 'anr-btn anr-btn-sm' }, 'Reset to file');
                    ...
                    const ensureDb = async () => {
                        if (qdb)
                            return qdb;
                        if (!opening) {
                            const { sqliteQuery } = await import('../lib/sqlite.js');
                            opening = sqliteQuery(file);
                        }
                        qdb = await opening;
                        return qdb;
                    };
                    ...
                    const resultTable = (columns, values) => {
                        ...
                        for (const r of values.slice(0, 1000)) {
                            const tr = el('tr', {});
                            for (const cell of r)
                                tr.appendChild(el('td', {}, cell == null ? 'NULL' : String(cell).slice(0, 300)));
                            t.appendChild(tr);
                        }
                        wrap.appendChild(t);
                        return wrap;
                    ...
                    const run = async () => {
                        ...
                        try {
                            ...
                            if (!db) {
                                resultEl.appendChild(el('p', { class: 'anr-hint anr-syn-error' }, 'Could not open the database engine.'));
                                return;
                            }
                            let res;
                            ...
                            try {
                                ...
                                resultEl.appendChild(el('p', { class: 'anr-hint anr-syn-error' }, 'SQL error: ' + (e && e.message)));
                                return;
                            }
                            if (WRITES.test(sql)) {
                            ...
                            res.forEach((r, i) => {
                                ...
                                resultEl.appendChild(el('p', { class: 'anr-hint' }, (res.length > 1 ? 'Statement ' + (i + 1) + ' - ' : '') +
                    ...
                    ta.addEventListener('keydown', (e) => { if ((e.ctrlKey || e.metaKey) && e.key === 'Enter') {
                        e.preventDefault();
                        run();
                    } });
                    const qnode = el('div', {});
                    qnode.appendChild(el('p', { class: 'anr-hint', style: 'margin:0 0 6px;' }, 'Runs against a copy of this database held in memory, entirely in your browser. Any SQL works, writes and schema changes included - the copy is thrown away when you leave, and the file on your device is never written to. Ctrl/Cmd+Enter to run.'));
    ...
    if (file && maxFrames > 0) {
        try {
            ...
            for (let i = 0; i < n; i++) {
                ...
                if (fv.getUint32(8) !== salt1 || fv.getUint32(12) !== salt2)
                    break;
                valid++;
                if (pageNo)
                    pages.add(pageNo);
            ...
            if (lastDbSize)
                out['DB size after last commit'] = lastDbSize.toLocaleString() + ' pages  (' + fmtBytes(lastDbSize * pageSize) + ')';
...
function parseSqliteShm(head) {
    ...
    if (iVersion !== 3007000 && dvv.getUint32(0, false) === 3007000) {
        le = false;
        iVersion = 3007000;
    }
    ...
    if (head.length >= 136) {
        ...
        if (h0.mxFrame)
            out['Frames awaiting checkpoint'] = Math.max(0, h0.mxFrame - nBackfill).toLocaleString();
    }
    return out;
}
// ---------- GIMP XCF ----------
function parseXcf(buf) {
    ...
    if (colorMode != null)
        result['Color mode'] = modes[colorMode] || 'Unknown';
    return result;
}
...
function parseImc(buf) {
    if (!buf || buf.length < 48)
        return null;
    if (buf[0] !== 0x1a || ascii(buf, 1, 20) !== 'RTS COMPRESSED IMAGE')
        return null;
...
function firstUtf16(buf, start, end, minChars = 2) {
    ...
    for (let i = start; i + 1 < end; i += 2) {
        ...
        if (hi === 0 && lo >= 0x20 && lo <= 0x7e)
            ...
            if (cur.length >= minChars && cur.length > best.length)
                best = cur;
            cur = '';
        }
    }
...
async function parsePkf({ file }) {
    ...
    if (!(buf[0] === 0x6b && buf[1] === 0x24 && buf[2] === 0x21 && buf[3] === 0x00))
        return null;
    ...
    if (peakCount)
        out['Peaks'] = peakCount.toLocaleString() + ' (min/max pairs, ' + samplesPerPeak.toLocaleString() + ' samples each)';
    if (totalSamples) {
...
function drawPkfWaveform(dv, start, byteLen) {
    ...
    let absmax = 0;
    for (let i = 0; i < n; i++) {
        const v = dv.getFloat32(start + i * 4, true);
    ...
    try {
        ...
        if (zip.has('meta.xml')) {
            ...
            if (xml) {
                ...
                if (gen)
                    fields['Generator'] = gen;
                if (created && !fields['Created'])
                    fields['Created'] = created;
    ...
    while (pos + 8 <= total) {
        ...
        if (type === 0x0001 && !strings) {
            ...
            for (let i = 0; i < attrCount && ap + 20 <= bytes.length; i++, ap += attrSize) {
                ...
                if (!key && resIds && aNameRef < resIds.length)
                    key = AXML_KNOWN_ATTRS[resIds[aNameRef]] || '';
                if (!key)
                    continue;
                let val;
                if (aRawRef !== 0xffffffff)
                    val = str(aRawRef);
                ...
                else if (dataType === 0x01 || dataType === 0x02)
                    val = '@0x' + data.toString(16);
                else
                    val = String(data | 0);
                attrs[key] = val;
...
async function parseApk(file) {
    let zip = null;
    try {
        zip = await openZip(file, 4 * 1024 * 1024);
    }
    ...
    if (manifest) {
        ...
        for (const ev of manifest.events) {
            if (ev.type === 'start') {
                ...
                if (ev.tag === 'manifest') {
                    if (a.package)
                        fields['Package'] = a.package;
                    if (a.versionName)
                        fields['Version name'] = a.versionName;
                    if (a.versionCode)
                        fields['Version code'] = a.versionCode;
                    if (a.sharedUserId)
                        fields['Shared user ID'] = a.sharedUserId;
                    if (a.installLocation)
                        fields['Install location'] = a.installLocation;
                    if (a.compileSdkVersion)
                        fields['Built with'] = androidApiLabel(a.compileSdkVersion);
                    if (a.platformBuildVersionName && !fields['Built with'])
                        fields['Built with'] = a.platformBuildVersionName;
                }
                else if (ev.tag === 'uses-sdk') {
                    if (a.minSdkVersion)
                        fields['Min Android'] = androidApiLabel(a.minSdkVersion);
                    if (a.targetSdkVersion)
                        fields['Target Android'] = androidApiLabel(a.targetSdkVersion);
                    if (a.maxSdkVersion)
                        fields['Max Android'] = androidApiLabel(a.maxSdkVersion);
                }
                else if (ev.tag === 'uses-permission' || ev.tag === 'uses-permission-sdk-23') {
                    if (a.name)
                        permissions.push(a.name);
                }
                else if (ev.tag === 'uses-feature') {
                    if (a.name)
                        features.push(a.name + (a.required === 'false' ? ' (optional)' : ''));
                    else if (a.glEsVersion) {
                        const n = parseInt(String(a.glEsVersion).replace(/^0x/, ''), 16);
                        if (n)
                            features.push('OpenGL ES ' + (n >> 16) + '.' + (n & 0xffff));
                    }
                }
                else if (ev.tag === 'application') {
                    if (a.label && a.label.charAt(0) !== '@')
                        fields['App label'] = a.label;
                    if (a.debuggable === 'true')
                        fields['Debuggable'] = 'Yes';
                    if (a.usesCleartextTraffic)
                        fields['Cleartext traffic'] = a.usesCleartextTraffic === 'true' ? 'Allowed' : 'Blocked';
                    if (a.allowBackup)
                        fields['Allows backup'] = a.allowBackup === 'true' ? 'Yes' : 'No';
                }
                    ...
                    if (a.name === 'android.intent.category.LAUNCHER')
                        sawLauncher = true;
                }
            }
            else if (ev.type === 'end') {
                if (ev.tag === 'intent-filter') {
                    if (sawMain && sawLauncher && curActivity && !launcher)
                        launcher = curActivity;
                    inIntent = false;
                }
                else if (ev.tag === 'activity' || ev.tag === 'activity-alias') {
                    curActivity = null;
    ...
    if (launcher)
        fields['Launcher activity'] = launcher;
    ...
    if (contentNames) {
        ...
        if (dex.length)
            fields['DEX files'] = String(dex.length);
        const abis = new Set();
        for (const n of contentNames) {
            const m = n.match(/^lib\/([^/]+)\//);
            if (m)
                abis.add(m[1]);
        }
        if (abis.size)
            fields['Native code'] = [...abis].join(', ');
        if (contentNames.includes('resources.arsc'))
            fields['Resource table'] = 'resources.arsc';
        if (info)
            fields['Total entries'] = String(info.count);
    }
    ...
    if (info && info.cdOff >= 16) {
        try {
            ...
            if (findBytes(blk, new Uint8Array(MAGIC)) >= 0) {
                ...
                if (has([0x1a, 0x87, 0x09, 0x71]))
                    schemes.push('v2');
                if (has([0xc0, 0x68, 0x53, 0xf0]))
                    schemes.push('v3');
                if (has([0x61, 0xad, 0x93, 0x1b]))
                    schemes.push('v3.1');
                if (!schemes.length)
                    schemes.push('v2+');
            }
    ...
    if (contentNames && contentNames.some((n) => /^META-INF\/.+\.(RSA|DSA|EC)$/i.test(n)))
        schemes.unshift('v1 (JAR)');
    if (schemes.length)
        fields['Signature'] = 'APK Signature Scheme ' + schemes.join(', ');
    if (permissions.length)
        fields['Permissions'] = String(permissions.length);
...
async function parseTextVersion(file) {
    try {
        ...
        for (const line of lines) {
            ...
            if (ver && !fields['Version'])
                fields['Version'] = ver[1];
            const gen = line.match(/(?:generator|creator|created.?(?:with|by)|application)[:\s=]+(.{2,60})/i);
            if (gen && !fields['Creator'])
                fields['Creator'] = gen[1].trim().replace(/[";]/g, '');
        }
        return Object.keys(fields).length ? fields : null;
    }
    catch (_) {
...
async function parseGzipXmlProject(file, ext) {
    try {
        ...
        if (ext === 'als' || ext === 'alp') {
            ...
            if (ver)
                fields['Creator'] = ver[1];
            const schema = xml.match(/SchemaChangeCount="(\d+)"/);
            if (schema)
                fields['Schema version'] = schema[1];
            return Object.keys(fields).length ? fields : null;
        }
        ...
        if (mediaItems)
            fields['Media items'] = mediaItems;
        const clips = (xml.match(/<(?:Video|Audio)Clip\b/g) || []).length;
        if (clips)
            fields['Timeline clips'] = clips;
    ...
    try {
        ...
        function decode() {
            ...
            if (ch === 'i') {
                pos++;
                let end = pos;
                while (end < raw.length && raw[end] !== 0x65)
                    end++;
        ...
        const fields = {};
        if (torrent.announce)
            fields['Tracker'] = torrent.announce;
        const info = torrent.info;
        if (info) {
            if (info.name)
                fields['Name'] = info.name.slice(0, 200);
            if (info['piece length'])
                fields['Piece size'] = fmtBytes(info['piece length']);
            if (info.pieces)
                fields['Pieces'] = Math.floor(info.pieces.length / 20);
            if (info.length) {
...
async function parseGcode(file) {
    try {
        ...
        if (isPrinting)
            return parseGcodePrinting(text);
...
function parseGcodePrinting(text) {
    ...
    if (fields['Nozzle'])
        fields['Nozzle'] += ' mm';
    if (fields['Layer height'])
        fields['Layer height'] += ' mm';
    if (fields['Bed temp'])
        fields['Bed temp'] += ' °C';
    if (fields['Nozzle temp'])
        fields['Nozzle temp'] += ' °C';
    return fields;
}
...
function parseGcodeCnc(text, truncated) {
    ...
    if (hasGrblCfg)
        fields['Controller'] = 'GRBL';
    else if (hasOprog)
        fields['Controller'] = 'Fanuc / Haas style (O-numbered, %-wrapped)';
    // --- Units ---
    ...
    let machine;
    if (/LightBurn|\blaser\b/i.test(text))
        machine = 'Laser cutter / engraver';
    else if (/plasma|SheetCam/i.test(text))
        machine = 'Plasma / oxy-fuel cutter';
    else if (hasX && hasZ && !hasY)
        machine = 'CNC lathe / turning';
    else if (cannedCycle && !hasArcs)
        machine = 'CNC drilling';
    else if (hasZ && (hasArcs || hasSpindle))
        machine = 'CNC mill / router (3-axis)';
    else if (isLaserPlasma)
        machine = 'Laser / plasma (2-axis)';
    else
        machine = 'CNC machining (generic)';
    ...
    for (let raw of lines) {
        ...
        const sM = line.match(/\bS(\d+\.?\d*)/);
        if (sM)
            maxS = Math.max(maxS, parseFloat(sM[1]));
        const fM = line.match(/\bF(\d+\.?\d*)/);
        if (fM)
            maxF = Math.max(maxF, parseFloat(fM[1]));
        const tM = line.match(/\bT(\d+)/);
        if (tM && /\bM0?6\b/.test(line) || (tM && /^T\d+$/.test(line)))
            tools.add(parseInt(tM[1], 10));
        const wM = line.match(/\bG5[4-9]\b/);
        if (wM)
            offsets.add(wM[0]);
    ...
    if (tools.size)
        fields['Tools used'] = tools.size + ' (T' + [...tools].sort((a, b) => a - b).join(', T') + ')';
    if (offsets.size)
        fields['Work offsets'] = [...offsets].sort().join(', ');
    if (maxS)
        fields['Max spindle / power'] = maxS + (isLaserPlasma ? ' (S)' : ' RPM');
    if (maxF)
        fields['Max feed rate'] = maxF + ' ' + unit + '/min';
    if (hasCoolant)
        fields['Coolant'] = 'Yes (M7/M8)';
    fields['Arc moves'] = hasArcs ? 'Yes (G2/G3)' : 'No';
    if (cannedCycle)
        fields['Canned cycles'] = 'Yes (drilling/boring)';
    if (!relativeSeen && minX !== Infinity && minY !== Infinity) {
        let ext = (maxX - minX).toFixed(2) + ' × ' + (maxY - minY).toFixed(2);
        if (minZ !== Infinity)
            ext += ' × ' + (maxZ - minZ).toFixed(2);
        fields['Extent (X×Y×Z)'] = ext + ' ' + unit;
        if (minZ !== Infinity)
            fields['Z range'] = minZ.toFixed(2) + ' to ' + maxZ.toFixed(2) + ' ' + unit;
    }
...
async function parseLogOrigin(file) {
    try {
        ...
        if (ips.size)
            fields['IPs (sample)'] = ips.size + ' unique';
...
function stepStr(tok) {
    ...
    return m[1].replace(/''/g, "'")
        .replace(/\\X2\\([0-9A-Fa-f]+)\\X0\\/g, (_, h) => { let o = ''; for (let k = 0; k < h.length; k += 4)
        o += String.fromCharCode(parseInt(h.substr(k, 4), 16)); return o; })
...
function stepProtocol(schema) {
    ...
    if (/MANAGED_MODEL_BASED_3D_ENGINEERING/.test(u) || /10303\s+242/.test(u))
        return 'AP242 - Managed model-based 3D engineering';
    if (/AUTOMOTIVE_DESIGN/.test(u) || /10303\s+214/.test(u))
        return 'AP214 - Automotive mechanical design';
    if (/CONFIG_CONTROL_DESIGN/.test(u) || /10303\s+203/.test(u))
        return 'AP203 - Configuration-controlled 3D design';
    if (/STRUCTURAL_ANALYSIS_DESIGN/.test(u) || /10303\s+209/.test(u))
        return 'AP209 - Multidisciplinary analysis & design';
    if (/ELECTRONIC_ASSEMBLY_INTERCONNECT|10303\s+210/.test(u))
        return 'AP210 - Electronic assembly & interconnect';
    if (/SHIP_/.test(u) || /10303\s+21[567]/.test(u))
        return 'AP215/216/217 - Ship structures';
    return null;
}
...
export function parseStepHeader(text) {
    ...
    let descList = [], impl = '';
    if (fdBody) {
        const a = splitStepArgs(fdBody);
        descList = stepStrList(a[0]);
        impl = stepStr(a[1]);
    }
    ...
    if (fnBody) {
        ...
        name = stepStr(a[0]);
        ts = stepStr(a[1]);
        authors = stepStrList(a[2]);
        orgs = stepStrList(a[3]);
        preproc = stepStr(a[4]);
        origSys = stepStr(a[5]);
        auth = stepStr(a[6]);
    }
    let schema = '', proto = '';
    if (fsBody) {
        schema = stepStrList(fsBody).join(', ');
        proto = stepProtocol(schema);
    }
    const app = detectCadApp(origSys + ' ' + preproc);
    const fields = {};
    if (app)
        fields['CAD software'] = app;
    if (origSys)
        fields['Originating system'] = origSys;
    if (preproc)
        fields['Preprocessor'] = preproc;
    if (proto)
        fields['Application protocol'] = proto;
    if (schema)
        fields['Schema'] = schema;
    if (name)
        fields['Model name'] = name;
    if (authors.length)
        fields['Author'] = authors.join(', ');
    if (orgs.length)
        fields['Organisation'] = orgs.join(', ');
    if (ts)
        fields['Exported'] = ts;
    if (auth && auth.toLowerCase() !== 'none')
        fields['Authorization'] = auth;
    if (descList.length)
        fields['Description'] = descList.join('; ');
    if (impl)
        fields['Implementation level'] = impl;
    return Object.keys(fields).length ? fields : null;
}
    ...
    return file.arrayBuffer().then((ab) => {
        ...
        if (headStr.indexOf('-----BEGIN') !== -1) {
            ...
            if (m) {
                ...
                try {
                    der = Uint8Array.from(atob(m[2].replace(/\s+/g, '')), (ch) => ch.charCodeAt(0));
                }
                catch (_) {
                    return fields;
                }
...
function parseX509(der) {
    ...
    if (der[p] === 0xA0) {
        const v = readTLV();
        const vi = readTLV();
        fields['Version'] = 'v' + (der[vi.start] + 1);
    }
    ...
    if (serialHex)
        fields['Serial'] = serialHex.length > 48 ? serialHex.slice(0, 48) + '…' : serialHex;
    ...
    if (algName === 'RSA') {
        ...
        if (der[kp] === 0x30) {
            ...
            p = save;
            let bytes = mod.len;
            if (der[mod.start] === 0x00)
                bytes -= 1; // leading zero
...
async function parseAepx(file) {
    try {
        ...
        if (ver)
            fields['After Effects version'] = ver[1];
        ...
        if (effects.length)
            fields['Effects / plugins'] = effects.length;
        ...
        if (exprs.length)
            fields['Expressions'] = exprs.length;
        ...
        if (names.length)
            fields['Named items'] = names.length;
        const sections = [];
        if (uniqAssets.length) {
    ...
    try {
        ...
        while (p + 8 <= end) {
            ...
            if (fourcc === 'LIST') {
                ...
                if (listType === 'Item' || listType === 'idta')
                    items++;
        ...
        if (vm)
            aeVersion = vm[1];
        if (aeVersion)
            fields['After Effects version'] = aeVersion;
        if (items)
            fields['Items (comps / folders / footage)'] = items;
        if (comps)
            fields['Compositions'] = comps;
        if (layers)
            fields['Layers'] = layers;
        if (folders)
            fields['Folders'] = folders;
        if (effectNames.length)
            fields['Effects / plugins'] = effectNames.length;
        else if (matchNames.size)
            fields['Property match-names'] = matchNames.size;
        if (paths.length)
            fields['Referenced asset paths'] = paths.length;
        if (cap < file.size)
            fields['Scanned'] = fmtBytes(cap) + ' of ' + fmtBytes(file.size);
        const sections = [];
        if (effectNames.length) {
...
function harvestPaths(buf) {
    ...
    for (const m of aRun.matchAll(re)) {
        ...
        if (s.length > 4 && /\.[A-Za-z0-9]{2,5}\b/.test(s))
            out.add(s);
    }
    // UTF-16 pass
...
async function parseVeg(file) {
    try {
        ...
        if (ascii(buf, 0, 4) === 'RIFF')
            fields['Container'] = 'RIFF (little-endian)';
        else if (ascii(buf, 0, 4) === 'RIFX')
            fields['Container'] = 'RIFX (big-endian)';
        else if (buf[0] === 0xD0 && buf[1] === 0xCF)
            fields['Container'] = 'OLE compound document';
        ...
        if (file.size > cap) {
            ...
            for (const p of harvestPaths(tail))
                paths.push(p);
        }
        ...
        if (cap < file.size)
            fields['Scanned'] = fmtBytes(cap) + ' (head) of ' + fmtBytes(file.size);
...
async function parseDrp(file, ext) {
    try {
        ...
        if (verm)
            fields['Resolve version'] = verm[1];
        const namem = blob.match(/(?:ProjectName|TimelineName|"name")\s*[=:]\s*["']?([^"'<>\n\r]{1,80})/i);
        if (namem)
            fields[isTimeline ? 'Timeline name' : 'Project name'] = namem[1].trim();
        const paths = harvestPaths(buf);
        const uniqPaths = [...new Set(paths)];
        if (uniqPaths.length) {
            fields['Referenced media'] = uniqPaths.length;
...
async function parseFilmora(file) {
    try {
        ...
        if (model && typeof model === 'object') {
            ...
            if (ver)
                fields['Filmora version'] = String(ver);
            ...
            if (w && h)
                fields['Resolution'] = w + ' x ' + h + ' px';
            const fpsv = root.fps || root.frameRate || (root.canvas && root.canvas.fps);
            if (fpsv)
                fields['Frame rate'] = parseFloat(fpsv) + ' fps';
            ...
            if (Array.isArray(clipsArr) && !fields['Clips'])
                fields['Clips'] = clipsArr.length;
        }
        else {
...
function buildCapcutFields(obj) {
    ...
    if (platform && typeof platform === 'object') {
        ...
        if (appv)
            fields['CapCut version'] = String(appv);
        if (platform.os)
            fields['Platform'] = String(platform.os) +
    ...
    if (Array.isArray(obj.tracks)) {
        ...
        if (segs)
            fields['Segments (clips)'] = segs;
        const tt = Object.keys(byType);
        if (tt.length)
            fields['Track types'] = tt.map(k => k + ' x' + byType[k]).join(', ');
    }
    ...
    if (m && typeof m === 'object') {
        ...
        const pick = (key, label) => {
            if (Array.isArray(m[key]) && m[key].length)
                counts.push(label + ': ' + m[key].length);
        };
        pick('videos', 'Videos');
        pick('audios', 'Audios');
        pick('texts', 'Texts');
...
async function parseCapcut(file) {
    try {
        ...
        if (!isNamed && !/"canvas_config"|"draft_id"|"last_modified_platform"/.test(text.slice(0, 4096) + text.slice(-2048))) {
            ...
            if (!/"materials"[\s\S]{0,200}"tracks"|"tracks"[\s\S]{0,200}"segments"/.test(text.slice(0, 8192)))
                return null;
        }
        let obj;
        try {
...
async function parsePart(file, head) {
    ...
    if (sig)
        fields['Detected original format'] = sig;
    else
        fields['Detected original format'] = 'Unrecognised (' +
...
function guessFromMagic(b) {
    ...
    if (b[0] === 0xD0 && b[1] === 0xCF)
        return 'OLE compound (legacy Office / MSI)';
    return null;
}
...
function parseEac3(head) {
    ...
    if (acmod === 7)
        fields['Layout'] = lfeon ? '5.1 surround' : '5.0 surround';
    const rates = { 0: '48 kHz', 1: '44.1 kHz', 2: '32 kHz' };
    if (rates[fscod])
        fields['Sample rate'] = rates[fscod];
    return fields;
}
...
async function parseCdp(file, head) {
    ...
    if (hierarchy)
        return parseCriterium(file, head);
    ...
    if (a(0, 15) === 'SQLite format 3') {
        ...
        if (txt[0] === '{' || txt[0] === '[')
            fields['Container'] = 'JSON';
        else if (txt.startsWith('<?xml') || txt[0] === '<')
            fields['Container'] = 'XML';
        else
...
async function parseCriterium(file, head) {
    ...
    if (m) {
        fields['Model type'] = m[1];
        fields['Format version'] = m[2];
    }
    else if (ver)
        fields['Format'] = ver;
    ...
    for (let s of runs) {
        ...
        if (!/[A-Za-z]{3}/.test(s))
            continue; // needs real words
        if (s.length > 48)
            continue; // skip prose and the padded rating scales
        if (/\s{2,}/.test(s))
            continue; // double spaces = padded scale block
        if (/^hierarchy\b/i.test(s))
            continue; // the header version string
        if (s.includes('?'))
            continue; // binary artefacts (e.g. "fff?")
        if (/^default alternative/i.test(s))
            continue; // system-added placeholder
...
function maxWithIndex(a) {
    ...
    a.forEach((x, i) => { const n = Number(x); if (n > bv) {
        bv = n;
        bi = i;
    } });
...
function summariseBepis(cls, o, fields) {
    if (cls === 'GameProgressMoneyAndGear') {
        ...
        if (fam.length)
            fields['Weapon variants unlocked'] = fam.map((f) => `${UK_WEAPONS[f]} ${byFam[f].n}/${byFam[f].t}`).join(', ');
        const flags = [];
        if (o.clashModeUnlocked)
            flags.push('Clash mode');
        if (o.ghostDroneModeUnlocked)
            flags.push('Ghost drone');
        if (o.tutorialBeat)
            flags.push('Tutorial beaten');
        if (flags.length)
            fields['Unlocked'] = flags.join(', ');
        if (isArr(o.newEnemiesFound))
            fields['Enemies in bestiary'] = `${countTruthy(o.newEnemiesFound)} / ${o.newEnemiesFound.length}`;
        if (isArr(o.secretMissions))
            fields['Secret missions found'] = `${countTruthy(o.secretMissions)} / ${o.secretMissions.length}`;
    }
    else if (cls === 'GameProgressData') {
        fields['Save type'] = 'Difficulty progress';
        if (typeof o.difficulty === 'number')
            fields['Difficulty'] = diffName(o.difficulty);
        if (typeof o.levelNum === 'number')
            fields['Furthest level reached'] = String(o.levelNum);
        if (isArr(o.primeLevels))
            fields['Prime sanctums cleared'] = `${countTruthy(o.primeLevels)} / ${o.primeLevels.length}`;
        if (typeof o.encores === 'number')
            fields['Encores'] = String(o.encores);
    }
    else if (cls === 'RankData') {
        fields['Save type'] = 'Per-level rank data';
        if (typeof o.levelNumber === 'number')
            fields['Level number'] = String(o.levelNumber);
        if (isArr(o.ranks)) {
            const played = o.ranks.filter((x) => x !== -1).length;
            fields['Completions (per difficulty)'] = `${played} / ${o.ranks.length}`;
        }
        if (isArr(o.secretsFound))
            fields['Secrets found'] = `${countTruthy(o.secretsFound)} / ${o.secretsAmount != null ? o.secretsAmount : o.secretsFound.length}`;
        if (o.challenge)
            fields['Challenge'] = 'Completed';
        if (isArr(o.majorAssists)) {
            const used = countTruthy(o.majorAssists);
            if (used)
                fields['Major assists used'] = `${used} difficulties`;
        }
        ...
        if (isArr(o.stats)) {
            ...
            if (best.length) {
                ...
                if (k)
                    fields['Best kills'] = String(k);
                if (st)
                    fields['Best style'] = st.toLocaleString();
                if (times.length)
                    fields['Best time'] = fmtTime(Math.min(...times));
            }
        }
    }
        ...
        if (wave) {
            fields['Best wave'] = (Math.floor(wave.value * 100) / 100).toString();
            fields['On difficulty'] = diffName(wave.index);
        }
        else if (typeof o.wave === 'number')
            fields['Best wave'] = String(o.wave);
        if (isArr(o.kills)) {
            const m = maxWithIndex(o.kills);
            if (m)
                fields['Kills'] = String(m.value);
        }
        if (isArr(o.style)) {
            const m = maxWithIndex(o.style);
            if (m)
                fields['Style points'] = m.value.toLocaleString();
        }
        if (isArr(o.time)) {
            const m = maxWithIndex(o.time);
            if (m)
                fields['Time survived'] = fmtTime(m.value);
        }
    }
}
async function parseBepis(file, head) {
    ...
    if (buf[0] === 0x00 && buf[1] === 0x01 && buf[2] === 0x00 && buf[3] === 0x00) {
        ...
        if (res.ok && res.root) {
            ...
            if (res.rootClass)
                fields['Save class'] = res.rootClass;
            ...
            if (dump)
                fields['_readableText'] = dump.slice(0, 20000);
            return fields;
        }
    }
    ...
    if (headStr[0] === '{' || headStr[0] === '[') {
        ...
        try {
            ...
            if (keys.length)
                fields['Fields'] = keys.slice(0, 20).join(', ');
    ...
    for (let i = 0; i < buf.length; i++) {
        ...
        if (c >= 0x20 && c < 0x7F) {
            ...
            if (cur.length >= 4 && /[a-zA-Z]/.test(cur))
                strings.push(cur);
            cur = '';
        }
    }
    ...
    if (known.length)
        fields['Recognised tokens'] = known.slice(0, 25).join(', ');
    if (uniq.length)
        fields['_readableText'] = uniq.slice(0, 300).join('\n');
    return fields;
}
...
async function parseCtg(file) {
    ...
    if (isPath) {
        ...
        let end = 0;
        while (end < buf.length && end < 260 && buf[end] !== 0)
            end++;
        ...
        if (buf.length > 0x84) {
            ...
            if (folderNo >= 100 && folderNo <= 999)
                fields['Folder number'] = String(folderNo);
            if (shots > 0 && shots <= 5000)
                fields['Shots recorded'] = String(shots);
        }
        ...
        if (buf.length > 0xec) {
            ...
            for (let i = 0; i < 32; i++)
                if ((bmp >>> i) & 1)
                    bits++;
            if (bits > 0 && bits <= 1000)
                fields['Frames in use (bitmap)'] = String(bits);
        }
        ...
        if (nImg)
            fields['Photo entries (IMG_)'] = String(nImg);
        if (nMvi)
            fields['Movie entries (MVI_)'] = String(nMvi);
        if (nSnd)
            fields['Voice-memo slots (SND_)'] = String(nSnd);
    }
    else {
        fields['Catalog type'] = 'Master index (D.CTG)';
        if (buf.length >= 4) {
            const folders = dv.getUint32(0, true);
            if (folders > 0 && folders < 100000)
                fields['Folders catalogued'] = String(folders);
        }
    }
...
function lnkHotkey(raw) {
    ...
    if (key >= 0x30 && key <= 0x5A)
        ...
        parts.push('F' + (key - 0x6F));
    else
        parts.push('0x' + key.toString(16));
    return parts.join(' + ');
}
async function parseLnk(file) {
    ...
    if (dv.getUint32(0, true) !== 0x4C)
        return null; // HeaderSize
    for (let i = 0; i < 16; i++)
        if (buf[4 + i] !== LNK_CLSID[i])
            return null;
    ...
    if ((flags & 0x02) && off + 28 <= buf.length) { // HasLinkInfo
        ...
        if ((liFlags & 0x01) && localBaseOff) { // VolumeIDAndLocalBasePath
            ...
            if (suffixOff)
                localPath += lnkCStr(buf, li + suffixOff);
        }
        if (liSize > 0 && li + liSize <= buf.length)
            off = li + liSize;
    }
    const readStr = () => {
        if (off + 2 > buf.length)
        ...
        let s = '';
        if (isUni) {
            for (let i = 0; i < n && off + 1 < buf.length; i++) {
                s += String.fromCharCode(dv.getUint16(off, true));
                off += 2;
            }
        }
        else {
    ...
    if (flags & 0x04)
        name = readStr(); // HasName (description)
    if (flags & 0x08)
        rel = readStr(); // HasRelativePath
    if (flags & 0x10)
        work = readStr(); // HasWorkingDir
    if (flags & 0x20)
        args = readStr(); // HasArguments
    if (flags & 0x40)
        icon = readStr(); // HasIconLocation
    ...
    if (isDir)
        fields['Target type'] = 'Folder';
    if (rel && rel !== target)
        fields['Relative path'] = rel;
    if (args)
        fields['Arguments'] = args;
    if (work)
        fields['Working directory'] = work;
    if (name)
        fields['Description'] = name;
    if (icon)
        fields['Icon location'] = icon;
    if (!isDir && targetSize)
        fields['Target size'] = fmtBytes(targetSize);
    const showMap = { 1: 'Normal window', 3: 'Maximized', 7: 'Minimized' };
    if (showMap[showCmd])
        fields['Window'] = showMap[showCmd];
    const hk = lnkHotkey(hotkey);
    if (hk)
        fields['Hotkey'] = hk;
    const wt = lnkFiletime(dv, 0x2C);
    if (wt)
        fields['Target modified'] = wt;
    return fields;
}
...
async function parseUrlShortcut(file) {
    ...
    if (url)
        fields['URL'] = url;
    if (icon)
        fields['Icon file'] = icon;
    if (idx)
        fields['Icon index'] = idx;
    return fields;
}
...
function parseFatVbr(buf, off) {
    ...
    if (fs52.startsWith('FAT32')) {
        fsType = 'FAT32';
        label = ascii(buf, off + 0x47, 11).trim();
        serial = dv.getUint32(off + 0x43, true);
    }
    else {
        fsType = fs36.startsWith('FAT') ? fs36 : (ascii(buf, off + 3, 5) === 'NTFS' ? 'NTFS' : (ascii(buf, off + 3, 5) === 'EXFAT' ? 'exFAT' : null));
        label = ascii(buf, off + 0x2b, 11).trim();
        serial = dv.getUint32(off + 0x27, true);
    }
    if (fsType)
        out['Filesystem'] = fsType;
    if (label && label !== 'NO NAME')
        out['Volume label'] = label;
    if (oem)
        out['Formatted by'] = oem;
    if (bps && spc)
        out['Cluster size'] = (bps * spc / 1024) + ' KB (' + spc + ' sectors)';
    if (bps)
        out['Bytes/sector'] = String(bps);
    if (totalSec && bps)
        out['Volume size'] = fmtBytes(totalSec * bps);
    if (serial)
        out['Volume serial'] = (serial >>> 0).toString(16).toUpperCase().padStart(8, '0').replace(/(.{4})(.{4})/, '$1-$2');
    return out;
}
            ...
            if (parts.length === 1 && parts[0].type === 0xee) {
                ...
                if (ascii(gpt, 0, 8) === 'EFI PART') {
                    ...
                    for (let i = 0; i + entSz <= arr.length && i / entSz < numEnt; i += entSz) {
                        ...
                        let nm = '';
                        for (let c = 0; c < 36; c++) {
                            const ch = arr[i + 0x38 + c * 2] | (arr[i + 0x38 + c * 2 + 1] << 8);
                            if (!ch)
                                break;
                            nm += String.fromCharCode(ch);
                        }
...
async function parseRec(file) {
    ...
    if (trimmed.startsWith('<?xml') || trimmed[0] === '<') {
        ...
        if (/getdatabackrecovery/i.test(text)) {
            ...
            if (tool)
                fields['Tool'] = tool + (ver ? ' ' + ver : '');
            const t = g(/<recfiletime>([^<]+)/i);
            if (t)
                fields['Saved'] = t;
            const name = g(/<name>([^<]+)/i), kind = g(/<kind>([^<]+)/i);
            if (name || kind)
                fields['Source'] = [name, kind].filter(Boolean).join(' - ');
            const from = g(/<fromsector>(\d+)/i), to = g(/<tosector>(\d+)/i);
            if (to) {
                const sec = (+to) - (+(from || 0)) + 1;
                fields['Imaged range'] = (+(from || 0)) + '–' + to + ' sectors (' + fmtBytes(sec * 512) + ')';
            }
            const id = g(/<recoveryid>([^<]+)/i);
            if (id)
                fields['Recovery ID'] = id;
            return fields;
        }
        if (/reclaime/i.test(text)) {
            fields['Type'] = 'ReclaiMe recovery session';
            return fields;
        }
...
async function renderFontPreview(file, card, fontInfo) {
    ...
    if (fontInfo && fontInfo.variable && Array.isArray(fontInfo.axes) && fontInfo.axes.length) {
        ...
        const makeAxis = (a) => {
            ...
            const startLoop = () => {
                ...
                let t0 = null;
                const step = (ts) => {
                    if (id !== loopId || !playing || !sample.isConnected)
                        return;
                    if (t0 === null)
                        t0 = ts;
            ...
            slider.addEventListener('input', () => {
                if (playing)
                    setPlaying(false); // a manual drag stops this axis
    ...
    if (tbl)
        card.insertBefore(previewCard, tbl);
    else
...
async function parseVdf(file) {
    ...
    while ((m = re.exec(text)) !== null) {
        if (m[2])
            ...
            tokens.push({ t: 's', v: m[1].replace(/\\"/g, '"').replace(/\\\\/g, '\\') });
        else
            tokens.push({ t: 's', v: m[4] });
    }
    if (!tokens.length)
        return null;
    let i = 0;
...
export async function renderProprietary(file, container, extOverride) {
    ...
    if (fn)
        extra = await safe(fn)({ head, file, ext });
    ...
    if (!extra && !fn && fmt.chunk) {
        try {
            ...
            if (lazyFn)
                extra = await lazyFn({ head, file, ext });
        }
    ...
    if (!extra && (ext === 'step' || ext === 'stp'))
        extra = await parseTextCad(file, 'STEP');
    if (!extra && (ext === 'iges' || ext === 'igs'))
        extra = await parseTextCad(file, 'IGES');
    // ZIP-based document formats
    if (!extra && fmt.zip)
        extra = await parseZipMeta(file, ext);
    // Generic text/XML version detection for formats without a dedicated parser
    if (!extra && (fmt.parse === 'text' || fmt.parse === 'xml'))
        extra = await parseTextVersion(file);
    ...
    if (EXT_VARIANTS[ext] && !(extra && extra._app)) {
        ...
        if (vname) {
            ...
            if (v && v.tell)
                tbl.appendChild(rowHelp('Detected as', vname, v.tell));
        }
    }
    ...
    if (extra) {
        ...
        if (extra._app) {
            h3El.textContent = extra._app;
            appRow.lastChild.textContent = extra._app;
        }
        ...
        for (const [k, v] of Object.entries(extra)) {
            ...
            if (k.startsWith('_'))
                continue; // internal payloads (e.g. _font, _readableText)
            if (v !== undefined)
                tbl.appendChild(help && help[k] ? rowHelp(k, String(v), help[k]) : row(k, String(v)));
        }
    }
    ...
    if (file.type)
        tbl.appendChild(rowHelp('MIME type', file.type, "The MIME type is the standard label for the file's format (for example image/jpeg or audio/mpeg). The browser reads it from the extension or the operating system, so it's a hint rather than proof of the real format."));
    // Last modified
                ...
                const contentHeight = () => {
                    try {
                        ...
                        if (d && d.documentElement)
                            return Math.max(d.documentElement.scrollHeight, d.body ? d.body.scrollHeight : 0) || 1600;
                    }
                    catch (_) { }
                    return 1600;
                };
    ...
    if (extra && Array.isArray(extra._sections)) {
        for (const sec of extra._sections) {
            if (!sec || !(sec.node instanceof Node))
                continue;
