...
export async function extractX3fPreview(file) {
    ...
    if (tag(0) !== 'FOVb')
        throw new Error('Not an X3F file');
    if (len < 12)
        throw new Error('X3F too small');
    const dirOff = dv.getUint32(len - 4, true); // last 4 bytes point at the SECd directory
    if (dirOff + 12 > len || tag(dirOff) !== 'SECd')
        throw new Error('X3F directory not found');
    ...
    for (let i = 0; i < count && p + 12 <= len; i++, p += 12) {
        ...
        const etype = tag(p + 8);
        if ((etype !== 'IMA2' && etype !== 'IMAG') || off + 28 > len || tag(off) !== 'SECi')
            continue;
        const fmt = dv.getUint32(off + 12, true);
        if (fmt !== 18)
            continue; // 18 = embedded JPEG (3/11 = non-JPEG, skipped)
        const cols = dv.getUint32(off + 16, true), rows = dv.getUint32(off + 20, true);
        const start = off + 28, end = Math.min(off + elen, len);
        if (start >= end || buf[start] !== 0xFF || buf[start + 1] !== 0xD8)
            continue; // must be a real SOI
        cands.push({ start, end, px: cols * rows });
    }
    if (!cands.length)
        throw new Error('No embedded JPEG preview in X3F');
...
export async function extractRawJpegs(file, { max = 12 } = {}) {
    ...
    if (b0 === 0x4949 || b0 === 0x4D4D)
        ...
        while (p + 4 <= len) {
            ...
            if (sl < 2)
                break;
            if (m === 0xE1 && p + 10 <= len && dv.getUint32(p + 4, false) === 0x45786966 && dv.getUint16(p + 8, false) === 0) {
                base = p + 10;
                break;
            }
            p += 2 + sl;
    ...
    while (queue.length && res.length < 64) {
        ...
        if (ifd <= 0 || ifd + 2 > len || visited.has(ifd))
            continue;
        visited.add(ifd);
        const n = u16(ifd);
        if (n < 0 || n > 4096)
...
export async function readImagesAsPngs(file, container) {
    ...
    try {
        ...
        setBar(1);
        labelEl.textContent = 'decoding pages';
        const { ImageMagick, MagickFormat } = magickReady;
        const data = new Uint8Array(await file.arrayBuffer());
        const out = [];
...
export async function demosaicRaw(file, container) {
    ...
    try {
        setBar(0.1);
        if (!librawMod)
            librawMod = await import(LIBRAW_URL);
        setBar(0.4);
        ...
        if (!img || !img.data || !img.width || !img.height)
            throw new Error('decoder returned no image');
        setBar(0.85);
        ...
        while ((Math.floor(width / step) * Math.floor(height / step) > RAW_DEMOSAIC_MAX_PX) ||
            Math.floor(width / step) > CANVAS_EDGE_MAX || Math.floor(height / step) > CANVAS_EDGE_MAX)
            step++;
...
//# sourceMappingURL=photo-convert.js.map