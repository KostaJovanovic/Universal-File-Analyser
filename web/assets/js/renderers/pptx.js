...
function openSlideLightbox(box, label) {
    ...
    function hide() {
        ...
        if (placeholder.parentNode)
            placeholder.replaceWith(box); // return it to the grid
        else
            box.remove();
        overlay.remove();
...
export async function renderPptx(file, resultsEl) {
            ...
            if (rels[rid]) {
                ...
                if (zip.has(path)) {
                    try {
                        ...
                        if (root && root.getAttribute('show') === '0')
                            hiddenSlides.add(path);
                    }
    ...
    metaTbl.appendChild(rowHelp('Slide size', Math.round(slideW) + ' × ' + Math.round(slideH) + ' px', 'The width and height of each slide, in pixels. The shape of the slides - widescreen 16:9 or the older 4:3 - is worked out from these two numbers.'));
    if (zip.has('docProps/core.xml')) {
        ...
        if (title)
            metaTbl.appendChild(row('Title', title));
    }
    if (zip.has('docProps/app.xml')) {
        ...
        try {
            ...
            if (hs && hs.textContent && hs.textContent !== '0')
                metaTbl.appendChild(row('Hidden slides (declared)', hs.textContent));
        }
        catch (_) { /* ignore */ }
    }
    ...
    for (let i = 0; i < slideOrder.length; i++) {
        ...
        slideBox.addEventListener('click', () => {
            if (!slideBox.classList.contains('anr-pptx-lightboxed'))
                openSlideLightbox(slideBox, 'Slide ' + (i + 1));
        });
        if (xml) {
            ...
            for (const sp of doc.getElementsByTagName('p:sp')) {
                ...
                for (const p of sp.getElementsByTagName('a:p')) {
                    let line = '';
                    for (const t of p.getElementsByTagName('a:t'))
                        line += t.textContent;
                    if (line.trim())
                        paras.push(line);
                }
            ...
            try {
                for (const tbl of doc.getElementsByTagName('a:tbl')) {
                    ...
                    if (grid.length)
                        ...
                        cols = rows[0].getElementsByTagName('a:tc').length;
            ...
            if (zip.has(relsPath)) {
                ...
                for (const r of rels.getElementsByTagName('Relationship')) {
                    ...
                    if (/hyperlink/i.test(type) && (isExt || /^https?:/i.test(target)) && /^(https?:|mailto:)/i.test(target))
                        ...
                        if (bytes) {
                            ...
                            img.addEventListener('click', (e) => {
                                ...
                                if (!slideBox.classList.contains('anr-pptx-lightboxed'))
                                    return;
                                e.stopPropagation();
                                if (slideBox._anrLightboxClose)
                                    slideBox._anrLightboxClose(); // close the lightbox first
                                if (window._anrHandleFile)
                                    window._anrHandleFile(new File([bytes], 'slide-image.' + ext, { type: mime }), { nested: true });
                            });
        ...
        if (zip.has(notesPath)) {
            ...
            let noteText = '';
            for (const t of notes.getElementsByTagName('a:t'))
                noteText += t.textContent + ' ';
    ...
    if (!slideOrder.length)
        slidesCard.appendChild(el('p', { class: 'anr-hint' }, 'No slides found.'));
    ...
    try {
        ...
        if (hasContent) {
            ...
            if (hasMacros)
                t.appendChild(row('Macros', '⚠ Contains macros (ppt/vbaProject.bin)'));
            if (hiddenCount)
                t.appendChild(row('Hidden slides', hiddenCount));
            if (tableReports.length) {
                t.appendChild(row('Tables', tableReports.length + ' (' + tableReports.map((r) => 'slide ' + r.num + ': ' + r.rows + '×' + r.cols).join(', ') + ')'));
            }
            if (totalHyperlinks)
                t.appendChild(row('Hyperlinks', totalHyperlinks));
            if (externalLinks.length)
                t.appendChild(row('External links', externalLinks.length));
            ...
            if (outline.length) {
                ...
                for (const o of outline) {
                    ...
                    if (o.hidden)
                        li.appendChild(el('span', { class: 'anr-hint', style: 'margin-left:6px;' }, '(hidden)'));
                    ol.appendChild(li);
                }
                det.appendChild(ol);
                structCard.appendChild(det);
