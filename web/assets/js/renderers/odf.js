...
function attrNS(elem, nsKey, name) {
    if (!elem || !elem.getAttributeNS)
        return '';
    for (const uri of NS[nsKey]) {
        const v = elem.getAttributeNS(uri, name);
        if (v)
            return v;
    }
    return '';
...
function elsNS(parent, nsKey, local) {
    ...
    if (!parent || !parent.getElementsByTagNameNS)
        return out;
    for (const uri of NS[nsKey])
        for (const e of parent.getElementsByTagNameNS(uri, local))
            out.push(e);
    return out;
}
...
function collectStyles(doc, into) {
    ...
    for (const s of elsNS(doc, 'style', 'style')) {
        ...
        if (tp) {
            ...
            if (w === 'bold' || parseInt(w, 10) >= 600)
                entry.bold = true;
            if (attrNS(tp, 'fo', 'font-style') === 'italic')
                entry.italic = true;
            if (attrNS(tp, 'style', 'text-underline-style') && attrNS(tp, 'style', 'text-underline-style') !== 'none')
                entry.underline = true;
            if (attrNS(tp, 'style', 'text-line-through-style') && attrNS(tp, 'style', 'text-line-through-style') !== 'none')
                entry.strike = true;
            const sz = attrNS(tp, 'fo', 'font-size');
            const m = /^([\d.]+)pt$/.exec(sz);
            if (m)
                entry.sizePt = parseFloat(m[1]);
        }
...
async function buildImageMap(zip) {
    ...
    for (const e of zip.entries) {
        if (!/^Pictures\//i.test(e.name))
            continue;
        if (!/\.(png|jpe?g|gif|bmp|webp|svg)$/i.test(e.name))
            continue;
        try {
...
function imageFor(frame, imageMap) {
    ...
    if (cm)
        im.style.width = Math.round(parseFloat(cm[1]) / 2.54 * 96) + 'px';
    else if (inch)
        im.style.width = Math.round(parseFloat(inch[1]) * 96) + 'px';
    return im;
}
    ...
    for (const child of node.childNodes) {
        ...
        if (isEl(child, 'text', 'span')) {
            ...
            if (st) {
                ...
                if (st.sizePt)
                    span.style.fontSize = st.sizePt + 'pt';
            }
...
function renderParagraph(p, styles, imageMap, tag) {
    ...
    const st = styles[styleName];
    if (st && st.align)
        elem.style.textAlign = st.align;
    renderInline(p, elem, styles, imageMap);
    if (!elem.textContent.trim() && !elem.querySelector('br,img'))
...
function renderList(list, styles, imageMap, level) {
    ...
    for (const item of childEls(list, 'text', 'list-item')) {
        ...
        for (const c of item.children) {
            if (isEl(c, 'text', 'p') || isEl(c, 'text', 'h'))
                li.appendChild(renderParagraph(c, styles, imageMap, 'span'));
            else if (isEl(c, 'text', 'list'))
                li.appendChild(renderList(c, styles, imageMap, (level || 0) + 1));
        }
        ul.appendChild(li);
    }
    return ul;
    ...
    for (const tr of childEls(tbl, 'table', 'table-row')) {
        ...
        for (const tc of childEls(tr, 'table', 'table-cell')) {
            ...
            for (const c of tc.children) {
                if (isEl(c, 'text', 'p') || isEl(c, 'text', 'h'))
                    td.appendChild(renderParagraph(c, styles, imageMap));
                else if (isEl(c, 'text', 'list'))
                    td.appendChild(renderList(c, styles, imageMap, 0));
                else if (isEl(c, 'table', 'table'))
                    td.appendChild(renderTable(c, styles, imageMap));
            }
            rowEl.appendChild(td);
            const rep = parseInt(attrNS(tc, 'table', 'number-columns-repeated'), 10);
            if (rep > 1 && rep < 100)
                for (let i = 1; i < rep; i++)
                    rowEl.appendChild(td.cloneNode(true));
        }
...
function extractMeta(doc) {
    ...
    if (title)
        fields['Title'] = title;
    const creator = dc('creator');
    if (creator)
        fields['Author'] = creator;
    const subject = dc('subject');
    if (subject)
        fields['Subject'] = subject;
    const desc = dc('description');
    if (desc)
        fields['Description'] = desc;
    const created = mt('creation-date');
    if (created)
        fields['Created'] = created.replace('T', ' ').replace(/\..*$/, '');
    const dcDate = dc('date');
    if (dcDate)
        fields['Modified'] = dcDate.replace('T', ' ').replace(/\..*$/, '');
    const gen = mt('generator');
    if (gen)
        fields['Generator'] = gen;
    const stat = elsNS(doc, 'meta', 'document-statistic')[0];
    if (stat) {
        const pc = attrNS(stat, 'meta', 'page-count');
        if (pc)
            fields['Pages'] = pc;
        const wc = attrNS(stat, 'meta', 'word-count');
        if (wc)
            fields['Words'] = wc;
    }
    return fields;
...
function infoCard(file, kindLabel, meta) {
    ...
    card.appendChild(buildReadout([
        ...
        ...Object.entries(meta).map(([k, v]) => k === 'Generator'
...
function renderSheetPage(tableEl) {
    ...
    for (const tr of childEls(tableEl, 'table', 'table-row')) {
        ...
        for (const tc of childEls(tr, 'table', 'table-cell')) {
            ...
            for (let i = 0; i < rep && cells.length < MAX_COLS; i++)
                cells.push(txt);
        }
        // Trim trailing empties on the row.
        while (cells.length && !cells[cells.length - 1])
            cells.pop();
        for (let r = 0; r < rowRep && grid.length < MAX_ROWS; r++)
            grid.push(cells.slice());
    }
    // Trim trailing all-empty rows.
    while (grid.length && grid[grid.length - 1].every((c) => !c))
        grid.pop();
    if (!grid.length) {
        page.appendChild(el('p', { style: 'color:#888;' }, '(empty sheet)'));
        return page;
    }
...
function renderSlidePage(drawPage, styles, imageMap, index) {
    ...
    for (const frame of frames) {
        ...
        if (box) {
            ...
            for (const c of box.children) {
                if (isEl(c, 'text', 'p') || isEl(c, 'text', 'h'))
                    holder.appendChild(renderParagraph(c, styles, imageMap));
                else if (isEl(c, 'text', 'list'))
                    holder.appendChild(renderList(c, styles, imageMap, 0));
            }
    ...
    if (!page.childElementCount)
        page.appendChild(el('div', { class: 'anr-slide-body', style: 'color:#888;' }, 'Slide ' + (index + 1) + ' (no text)'));
    return page;
}
// ---------- main render ----------
