...
function featureId(raw) {
    ...
    if (s.includes('OUTER') || s.includes('EXTERNALPERIM'))
        return 0;
    if (s.includes('OVERHANG'))
        return 1;
    if (s.includes('INNER') || (s.includes('PERIMETER') && !s.includes('EXTERNAL')))
        return 1;
    if (s.includes('BRIDGE'))
        return 6;
    if (s.includes('SUPPORT'))
        return 4;
    if (s.includes('SKIRT') || s.includes('BRIM'))
        return 5;
    if (s.includes('SOLID') || s.includes('SKIN') || s.includes('TOPSURFACE') || s.includes('BOTTOMSURFACE'))
        return 3;
    if (s.includes('GAP'))
        return 7;
    if (s.includes('FILL'))
...
function GrowBuf(stride) {
    ...
    return {
        push(vals) {
            if (n + vals.length > cap) {
                while (n + vals.length > cap)
                    cap *= 2;
...
function parseGcode(text, opts) {
    ...
    const ensureTool = (n) => { let d = toolDefs.get(n); if (!d) {
        d = { n };
        toolDefs.set(n, d);
    } return d; };
    ...
    const emitExtrude = (x0, y0, z0, x1, y1, z1, de) => {
        ...
        if (ext.count < segCap) {
            ext.push([x0, y0, z0, x1, y1, z1, w, h, feed, curType]);
            order.push([0]);
            segFil.push([curFil]);
        }
        nExtrude++;
        sawExtrude = true;
        featureSet.add(curType);
        filUsed.add(curFil);
    ...
    const emitFeed = (x0, y0, z0, x1, y1, z1) => {
        ...
        nFeed++;
        if (ext.count < segCap) {
            ext.push([x0, y0, z0, x1, y1, z1, 0, 0, feed, cncType]);
            order.push([0]);
            segFil.push([curFil]);
        } // width filled later for CNC; type = tool slot
        bump(x0, y0, z0);
        bump(x1, y1, z1);
        if (feed > 0) {
            if (feed < fmin)
    ...
    const emitTravel = (x0, y0, z0, x1, y1, z1) => {
        if (trav.count < travCap) {
            trav.push([x0, y0, z0, x1, y1, z1, feed]);
            order.push([1]);
        }
        nRapid++;
    };
    ...
    const emitPause = (holdSec, label, realSec) => {
        if (holdSec > 0) {
            order.push([2]);
            pause.push([holdSec]);
            pauseInfo.push({ label, realSec: realSec != null ? realSec : holdSec });
        }
    };
    let i = 0;
    while (i < len) {
        ...
        if (skipActive) {
            if (/^;\s*SKIPPABLE_END\s*$/i.test(line))
                skipActive = false;
            else if (/^;\s*SKIPTYPE:/i.test(line) && !/timelapse/i.test(line))
                skipActive = false;
            continue;
        }
        if (/^;\s*SKIPPABLE_START\s*$/i.test(line)) {
            skipActive = true;
            continue;
        }
        ...
        if (semi >= 0) {
            ...
            if (semi === 0) {
                const t = cm.trim();
                if (inHeader && headerComments.length < HDR_CAP)
                    headerComments.push(t);
                footRun.push(t);
            }
            ...
            if (mm) {
                curType = featureId(mm[1]);
                sawTypes = true;
            }
            // Width/height hints (Cura/PrusaSlicer/ideaMaker emit these).
            mm = /WIDTH:\s*([\d.]+)/i.exec(cm);
            if (mm)
                forcedW = parseFloat(mm[1]) || 0;
            mm = /HEIGHT:\s*([\d.]+)/i.exec(cm);
            if (mm)
                forcedH = parseFloat(mm[1]) || 0;
            mm = /Filament\s*Diameter[^:]*:\s*([\d.]+)/i.exec(cm);
            if (mm && !filDia)
                filDia = parseFloat(mm[1]) || 0;
            ...
            if (!filColRaw) {
                mm = /filament[_ ]colou?r\s*[:=]\s*("?)(#[0-9A-Fa-f;,# ]+)/i.exec(cm);
                if (mm)
                    filColRaw = mm[2].trim();
            }
            if (!extColRaw) {
                mm = /extruder[_ ]colou?r\s*[:=]\s*("?)(#[0-9A-Fa-f;,# ]+)/i.exec(cm);
                if (mm)
                    extColRaw = mm[2].trim();
            }
            if (!printTime) {
                mm = /(?:print(?:ing)?\s*time|Print Time)[^:=]*[:=]?\s*([\dhms :]+\d[hms])/i.exec(cm);
                if (mm)
                    printTime = mm[1].trim();
            }
            ...
            if (!printerModel) {
                mm = /\bprinter_model\s*[:=]\s*("?)([^";\r\n]+)/i.exec(cm);
                if (mm)
                    printerModel = mm[2].trim();
            }
            if (!bedShapeRaw) {
                mm = /\b(?:printable_area|bed_shape|machine_bed_shape)\s*[:=]\s*([\d][\dxX.,\- ]+)/i.exec(cm);
                if (mm)
                    bedShapeRaw = mm[1].trim();
            }
            if (!bedType) {
                mm = /\b(?:curr_bed_type|default_bed_type|bed_type)\s*[:=]\s*([A-Za-z][\w .\-/+]*)/i.exec(cm);
                if (mm)
                    bedType = mm[1].trim();
            }
            if (!bedHeight) {
                mm = /\b(?:printable_height|max_print_height|machine_max_height)\s*[:=]\s*([\d.]+)/i.exec(cm);
                if (mm)
                    bedHeight = parseFloat(mm[1]) || 0;
            }
        ...
        if (!sawExtrude && line.indexOf('(') >= 0) {
            ...
            while ((pm = parenRe.exec(line))) {
                ...
                if (!c)
                    continue;
                if (inHeader && headerComments.length < HDR_CAP)
                    headerComments.push(c);
                footRun.push(c);
                const td = /^T(\d+)\b(.*)$/i.exec(c);
                if (td) {
                    const def = ensureTool(+td[1]), rest = td[2];
                    let g;
                    if (def.dia == null && (g = /\bD\s*=?\s*([\d.]+)/i.exec(rest)))
                        def.dia = parseFloat(g[1]);
                    if (def.cr == null && (g = /\bCR\s*=?\s*([\d.]+)/i.exec(rest)))
                        def.cr = parseFloat(g[1]);
                    if (def.taper == null && (g = /\bTAPER\s*=?\s*([\d.]+)\s*DEG/i.exec(rest)))
                        def.taper = parseFloat(g[1]);
                    if (def.zmin == null && (g = /\bZMIN\s*=?\s*(-?[\d.]+)/i.exec(rest)))
                        def.zmin = parseFloat(g[1]);
                    if (!def.desc) {
                        ...
                        if (segs.length)
                            def.desc = segs[segs.length - 1];
                    }
                }
                else if (/^O?\d{1,6}$/.test(c)) {
                    if (!progNum)
                        progNum = c.replace(/^O/i, ''); // Fusion writes the program number as "(1001)"
                }
        ...
        if (cmd.charCodeAt(0) === 84 && !/\bM0?6\b/.test(line)) {
            ...
            if (tn) {
                const ti = +tn[1];
                if (ti <= 32) {
                    if (sawTool && ti !== curFil)
                        toolMarks.push({ at: order.count, kind: 'tool', from: curFil, to: ti, z });
                    sawTool = true;
                    curFil = ti;
                    if (curFil > maxFil)
                        maxFil = curFil;
        ...
        if (cmd === 'M600') {
            toolMarks.push({ at: order.count, kind: 'filament', from: curFil, to: curFil + 1, z });
            emitPause(TOOLCHANGE_PAUSE_S, 'Filament change', TOOLCHANGE_PAUSE_S);
            curFil++;
            if (curFil > maxFil)
                maxFil = curFil;
            continue;
        ...
        if (cmd === 'G17' || cmd === 'G18' || cmd === 'G19') {
            ...
            if (!/\bG0?[0-3]\b/i.test(line) && !(motionMode && /(^|\s)[XYZABCUVWIJKR]-?[\d.]/i.test(line)))
                continue;
        }
        switch (cmd) {
            case 'G90':
        ...
        if (cmd === 'M104' || cmd === 'M109') {
            const s = numAfter(line, 'S');
            if (s > 0 && !temps.nozzle)
                temps.nozzle = s;
            if (cmd === 'M109' && s > 0)
                emitPause(heatSecs(s), 'Heat nozzle ' + Math.round(s) + '°C', realHeat(s));
            continue;
        }
        if (cmd === 'M140' || cmd === 'M190') {
            ...
            if (s > 0 && !temps.bed)
                temps.bed = s;
            if (cmd === 'M190' && s > 0)
                emitPause(heatSecs(s), 'Heat bed ' + Math.round(s) + '°C', realHeat(s));
            continue;
        }
        ...
        if (cmd === 'G4' || cmd === 'G04') {
            const p = numAfter(line, 'P'), s = numAfter(line, 'S');
            const sec = p > 0 ? p / 1000 : (s > 0 ? s : 0);
            emitPause(Math.min(sec, PAUSE_CAP_S), 'Dwell', sec);
            continue;
        }
        if (cmd === 'M0' || cmd === 'M00') {
            emitPause(MANUAL_PAUSE_S, 'Pause', MANUAL_PAUSE_S);
            continue;
        }
        if (cmd === 'M1' || cmd === 'M01') {
            optStops++;
            emitPause(MANUAL_PAUSE_S, 'Optional stop', MANUAL_PAUSE_S);
            continue;
        }
        if (cmd === 'M2' || cmd === 'M02') {
            if (!progEnd)
        ...
        if (cmd === 'M30') {
            progEnd = 'M30 (end + rewind)';
            continue;
        }
        if (!progNum && /^O\d{1,6}$/.test(cmd)) {
            progNum = cmd.slice(1);
            continue;
        }
        ...
        if (!sawExtrude && (line.indexOf('T') >= 0 || line.indexOf('M') >= 0 || line.indexOf('S') >= 0 || line.indexOf('G5') >= 0 || line.indexOf('H') >= 0)) {
            ...
            if (/\bM0?6\b/.test(line) && pendingTool != null) {
                ...
                if (curOp && curOp.moves === 0)
                    curOp.tool = pendingTool;
                ...
                if (curTool != null && pendingTool !== curTool) {
                    toolMarks.push({ at: order.count, kind: 'tool', from: curTool, to: pendingTool, z, desc: (toolDefs.get(pendingTool) || {}).desc });
                    emitPause(TOOLCHANGE_PAUSE_S, 'Tool change', TOOLCHANGE_PAUSE_S);
                }
                curTool = pendingTool;
                lastParen = '';
                let slot = toolSlot.get(pendingTool);
                if (slot == null) {
                    slot = Math.min(7, toolSlot.size);
                    toolSlot.set(pendingTool, slot);
                }
                cncType = slot; // colour subsequent cutting moves by this tool
            }
            if (/\bM0?3\b/.test(line))
                spindleDir = 'Clockwise (M3)';
            else if (/\bM0?4\b/.test(line))
                spindleDir = 'Counter-clockwise (M4)';
            if ((g = /\bS(\d+(?:\.\d+)?)\b/.exec(line))) {
                const rpm = parseFloat(g[1]);
                if (rpm > spindleMax)
                    spindleMax = rpm;
                if (curTool != null) {
                    const d = toolDefs.get(curTool);
                    if (d && d.rpm == null && rpm > 0)
                        d.rpm = rpm;
                }
            }
            if (/\bM0?8\b/.test(line))
                coolant.add('Flood (M8)');
            if (/\bM0?7\b/.test(line))
                coolant.add('Mist (M7)');
            if ((g = /\bG5([4-9])(?:\.(\d+))?\b/.exec(line)))
                workOffsets.add('G5' + g[1] + (g[2] ? '.' + g[2] : ''));
            // G43 tool-length compensation: remember the H offset register per tool.
            if (/\bG43\b/.test(line) && (g = /\bH(\d+)\b/.exec(line)) && curTool != null) {
                const d = toolDefs.get(curTool);
                if (d && d.h == null)
                    d.h = +g[1];
            }
        ...
        if (gMove)
            mo = 'G' + gMove[1];
        else if (motionMode && /(^|\s)[XYZABCUVWIJKR]-?[\d.]/i.test(line))
            mo = motionMode;
        if (!mo)
            continue;
        motionMode = mo;
        ...
        if (isLine) {
            ...
            if (moved) {
                if (extruding) {
                    emitExtrude(x, y, z, nx, ny, nz, de);
                    extrudeMM += de;
                }
                else if (nonExtrudeTravel)
                    emitTravel(x, y, z, nx, ny, nz);
                else
                    emitFeed(x, y, z, nx, ny, nz);
            }
        }
        else {
            nArc++;
            ...
            if (pts) {
                ...
                for (let k = 1; k < pts.length; k++) {
                    ...
                    if (extruding) {
                        emitExtrude(a[0], a[1], a[2], b[0], b[1], b[2], perDe);
                    }
                    else if (nonExtrudeTravel)
                        emitTravel(a[0], a[1], a[2], b[0], b[1], b[2]);
                    else
                        emitFeed(a[0], a[1], a[2], b[0], b[1], b[2]);
                }
                if (extruding)
                    extrudeMM += de;
            }
        ...
        if (curOp) {
            ...
            if (nz < curOp.zmin)
                curOp.zmin = nz;
            if (!extruding && mo !== 'G0')
                curOp.cutLen += Math.hypot(nx - x, ny - y, nz - z);
        }
        x = nx;
        y = ny;
        z = nz;
        if (t.e !== null)
            e = absE ? t.e * unit : e + t.e * unit;
        forcedW = 0;
        forcedH = 0; // hints apply to the next move only
    }
    if (!sawG)
        return { sawG: false };
    ...
    if (!sawExtrude) {
        ...
        for (let p = 0; p < view.length; p += 10) {
            view[p + 6] = wcnc;
            view[p + 7] = wcnc;
        }
    }
    ...
    if (allComments.length > 8000)
        allComments = allComments.slice(0, 8000);
    ...
    for (let k = 0; k < filSlots; k++)
        filamentColors.push(hexList[k] || FIL_FALLBACK[k % FIL_FALLBACK.length]);
    ...
    if (bedShapeRaw) {
        ...
        for (const tok of bedShapeRaw.split(',')) {
            ...
            if (m)
                poly.push([parseFloat(m[1]), parseFloat(m[2])]);
        }
        if (poly.length >= 2) {
            ...
            if (w > 1 && d > 1)
                bed = { x0: bx0, y0: by0, x1: bx1, y1: by1, w, d, h: bedHeight, type: bedType, poly };
        }
    }
    ...
    if (zs.length > 1) {
        ...
        gaps.sort((a, b) => a - b);
        if (gaps.length)
            layerHeight = gaps[Math.floor(gaps.length / 2)];
    }
    return {
        sawG: true,
...
function arcPoints(x0, y0, z0, x1, y1, z1, t, unit, cw, plane) {
    ...
    for (let k = 0; k <= steps; k++) {
        ...
        if (plane === 18)
            pts.push([ss, ww, ff]); // s->X, w->Y, f->Z
        else if (plane === 19)
            pts.push([ww, ff, ss]); // w->X, f->Y, s->Z
        else
            pts.push([ff, ss, ww]); // f->X, s->Y, w->Z
    }
    return pts;
}
...
function buildViewer(data, opts = {}) {
    ...
    const { boundR, boundRMax, ctr } = (() => {
        ...
        if (!segs)
            return { boundR: 0.5, boundRMax: 0.5, ctr: [0, 0, 0] };
    ...
    const makeBedLabelCanvas = (lines) => {
        ...
        for (const ln of lines) {
            ctx.font = fontFor(ln.bold);
            wmax = Math.max(wmax, ctx.measureText(ln.text).width);
        }
        ...
        ctx.lineWidth = 1 * sc;
        ctx.strokeRect(0.5 * sc, 0.5 * sc, cv.width - sc, cv.height - sc);
        ctx.textBaseline = 'top';
        let y = pad;
        for (const ln of lines) {
            ctx.font = fontFor(ln.bold);
            ctx.fillStyle = ln.dim ? 'rgba(238,242,255,0.8)' : '#eef2ff';
    ...
    if (data.bed) {
        ...
        const pushSeg = (arr, ax, ay, bx, by) => {
            ...
            for (const P of [A, B]) {
                const dd = Math.hypot(P[0] - ctr[0], P[1] - ctr[1], P[2] - ctr[2]);
                if (dd > bedBoundR)
                    bedBoundR = dd;
            }
        };
        ...
        for (let gx = Math.ceil(b.x0 / step) * step; gx <= b.x1 + 1e-6; gx += step)
            pushSeg(grid, gx, b.y0, gx, b.y1);
        for (let gy = Math.ceil(b.y0 / step) * step; gy <= b.y1 + 1e-6; gy += step)
            pushSeg(grid, b.x0, gy, b.x1, gy);
        const poly = (b.poly && b.poly.length >= 3) ? b.poly : [[b.x0, b.y0], [b.x1, b.y0], [b.x1, b.y1], [b.x0, b.y1]];
        for (let k = 0; k < poly.length; k++) {
            const p = poly[k], q = poly[(k + 1) % poly.length];
            pushSeg(outline, p[0], p[1], q[0], q[1]);
        }
        bedGrid = new Float32Array(grid);
        bedOutline = new Float32Array(outline);
    ...
    for (let k = 0; k < FIL_MAX; k++) {
        const c = hex2rgb(palette[k % Math.max(1, palette.length)]);
        filCols[k * 3] = c[0];
        filCols[k * 3 + 1] = c[1];
        filCols[k * 3 + 2] = c[2];
    }
    ...
    for (let s = 0; s < segN; s++) {
        ...
        if (data.seg[p + 7] > 0) {
            const hh = instData[q + 7];
            if (hh < minHalfH)
                minHalfH = hh;
        }
    }
    if (!(wHalfMin < wHalfMax)) {
        wHalfMin = 0;
        wHalfMax = 1e-3;
    }
    ...
    let drawBed = () => { };
    if (bedGrid) {
        const bedProg = makeProg('attribute vec3 aPos; uniform mat4 uMVP; void main(){ gl_Position = uMVP*vec4(aPos,1.0); }', 'precision mediump float; uniform vec4 uCol; void main(){ gl_FragColor = uCol; }');
        ...
        gl.bufferData(gl.ARRAY_BUFFER, bedGrid, gl.STATIC_DRAW);
        const oBuf = gl.createBuffer();
        gl.bindBuffer(gl.ARRAY_BUFFER, oBuf);
        gl.bufferData(gl.ARRAY_BUFFER, bedOutline, gl.STATIC_DRAW);
        // Info-label quad pasted flat onto the plate: a textured (printer/size/plate) panel.
        let drawBedLabel = () => { };
        if (bedLabelQuad && bedLabelCanvas) {
            const texProg = makeProg('attribute vec3 aPos; attribute vec2 aUV; uniform mat4 uMVP; varying vec2 vUV; void main(){ vUV = aUV; gl_Position = uMVP*vec4(aPos,1.0); }', 'precision mediump float; varying vec2 vUV; uniform sampler2D uTex; void main(){ vec4 c = texture2D(uTex, vUV); if(c.a < 0.01) discard; gl_FragColor = c; }');
            ...
            gl.bufferData(gl.ARRAY_BUFFER, bedLabelQuad, gl.STATIC_DRAW);
            drawBedLabel = (mvp) => {
                gl.useProgram(texProg);
                gl.uniformMatrix4fv(uTMVP, false, mvp);
                gl.activeTexture(gl.TEXTURE0);
                gl.bindTexture(gl.TEXTURE_2D, tex);
                gl.uniform1i(uTSamp, 0);
                gl.enable(gl.BLEND);
                gl.blendFunc(gl.ONE, gl.ONE_MINUS_SRC_ALPHA); // premultiplied
                gl.bindBuffer(gl.ARRAY_BUFFER, qBuf);
                gl.enableVertexAttribArray(aTPos);
                gl.vertexAttribPointer(aTPos, 3, gl.FLOAT, false, 20, 0);
                if (inst)
                    inst.vertexAttribDivisorANGLE(aTPos, 0);
                gl.enableVertexAttribArray(aTUV);
                gl.vertexAttribPointer(aTUV, 2, gl.FLOAT, false, 20, 12);
                if (inst)
                    inst.vertexAttribDivisorANGLE(aTUV, 0);
        ...
        drawBed = (mvp) => {
            ...
            if (inst)
                inst.vertexAttribDivisorANGLE(aBedPos, 0); // never inherit an instanced divisor
            gl.enable(gl.BLEND);
            gl.blendFunc(gl.SRC_ALPHA, gl.ONE_MINUS_SRC_ALPHA);
            gl.bindBuffer(gl.ARRAY_BUFFER, gBuf);
            gl.vertexAttribPointer(aBedPos, 3, gl.FLOAT, false, 0, 0);
    ...
    if (inst && segN > 0) {
        ...
        if (travData) {
            partTravBuf = gl.createBuffer();
            partTravArr = new Float32Array(STR);
        }
        ...
        const bindTemplate = () => {
            ...
            gl.vertexAttribPointer(aEnd, 1, gl.FLOAT, false, 12, 0);
            inst.vertexAttribDivisorANGLE(aEnd, 0);
            gl.enableVertexAttribArray(aSr);
            gl.vertexAttribPointer(aSr, 1, gl.FLOAT, false, 12, 4);
            inst.vertexAttribDivisorANGLE(aSr, 0);
            gl.enableVertexAttribArray(aSu);
            gl.vertexAttribPointer(aSu, 1, gl.FLOAT, false, 12, 8);
            inst.vertexAttribDivisorANGLE(aSu, 0);
        };
        const bindInstances = (buf) => {
            gl.bindBuffer(gl.ARRAY_BUFFER, buf);
            const S = 44;
            gl.enableVertexAttribArray(aA);
            gl.vertexAttribPointer(aA, 3, gl.FLOAT, false, S, 0);
            inst.vertexAttribDivisorANGLE(aA, 1);
            gl.enableVertexAttribArray(aB);
            gl.vertexAttribPointer(aB, 3, gl.FLOAT, false, S, 12);
            inst.vertexAttribDivisorANGLE(aB, 1);
            gl.enableVertexAttribArray(aHW);
            gl.vertexAttribPointer(aHW, 1, gl.FLOAT, false, S, 24);
            inst.vertexAttribDivisorANGLE(aHW, 1);
            gl.enableVertexAttribArray(aHH);
            gl.vertexAttribPointer(aHH, 1, gl.FLOAT, false, S, 28);
            inst.vertexAttribDivisorANGLE(aHH, 1);
            gl.enableVertexAttribArray(aFeed);
            gl.vertexAttribPointer(aFeed, 1, gl.FLOAT, false, S, 32);
            inst.vertexAttribDivisorANGLE(aFeed, 1);
            if (aType >= 0) {
                gl.enableVertexAttribArray(aType);
                gl.vertexAttribPointer(aType, 1, gl.FLOAT, false, S, 36);
                inst.vertexAttribDivisorANGLE(aType, 1);
            }
            if (aTool >= 0) {
                gl.enableVertexAttribArray(aTool);
                gl.vertexAttribPointer(aTool, 1, gl.FLOAT, false, S, 40);
                inst.vertexAttribDivisorANGLE(aTool, 1);
            }
        };
        ...
        const headProg = makeProg(`attribute vec3 aPos, aNrm; uniform mat4 uProj, uView, uModel; uniform vec3 uHead; uniform float uScale; varying vec3 vN;
       void main(){ vN = mat3(uModel)*aNrm; gl_Position = uProj*uView*uModel*vec4(uHead + aPos*uScale, 1.0); }`, `precision mediump float; varying vec3 vN; uniform vec3 uHeadColor;
       void main(){ vec3 N=normalize(vN); vec3 L=normalize(vec3(0.4,0.78,0.5));
        ...
        const headVariants = [];
        for (let s = 3; s <= 9; s++) {
            const geo = pyramidGeo(s);
            const b = gl.createBuffer();
            gl.bindBuffer(gl.ARRAY_BUFFER, b);
            gl.bufferData(gl.ARRAY_BUFFER, geo, gl.STATIC_DRAW);
            headVariants.push({ buf: b, count: geo.length / 6 });
        }
        ...
        if (isCNC && data.cnc.toolColors)
            for (const tc of data.cnc.toolColors)
                slotToNum.set(tc.slot, tc.n);
        const rawToNum = (raw) => isCNC ? (slotToNum.has(raw) ? slotToNum.get(raw) : raw) : raw;
        const rankMap = new Map();
        for (let s = 0; s < segN; s++) {
            const raw = instData[s * STR + toolOff] | 0;
            if (!rankMap.has(raw))
                rankMap.set(raw, rankMap.size + 1);
        }
        ...
        const digitProg = makeProg(`attribute vec3 aPos; attribute vec2 aUV; uniform mat4 uProj, uView, uModel; uniform vec3 uHead; uniform float uScale; varying vec2 vUV;
       void main(){ vUV = aUV; gl_Position = uProj*uView*uModel*vec4(uHead + aPos*uScale, 1.0); }`, `precision mediump float; varying vec2 vUV; uniform sampler2D uTex;
       void main(){ vec4 c = texture2D(uTex, vUV); if (c.a < 0.02) discard; gl_FragColor = c; }`);
        ...
        const digitBuf = gl.createBuffer();
        gl.bindBuffer(gl.ARRAY_BUFFER, digitBuf);
        gl.bufferData(gl.ARRAY_BUFFER, digitQuad, gl.STATIC_DRAW);
        const digitCache = new Map();
        const digitTex = (num) => {
            let t = digitCache.get(num);
            if (t)
                return t;
            const cv = document.createElement('canvas');
            cv.width = 64;
            ...
            c.lineJoin = 'round';
            c.lineWidth = 8;
            c.strokeStyle = 'rgba(18,20,26,0.95)';
            c.strokeText(str, 32, 34);
            c.fillStyle = '#ffffff';
            c.fillText(str, 32, 34);
            t = gl.createTexture();
            ...
            gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MIN_FILTER, gl.LINEAR);
            gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MAG_FILTER, gl.LINEAR);
            digitCache.set(num, t);
            return t;
        };
        const drawDigit = (proj, view, model, hx, hy, hz, num) => {
            gl.useProgram(digitProg);
            gl.uniformMatrix4fv(uDProj, false, proj);
            gl.uniformMatrix4fv(uDView, false, view);
            gl.uniformMatrix4fv(uDModel, false, model);
            gl.uniform3f(uDHead, hx, hy, hz);
            gl.uniform1f(uDScale, headScale);
            gl.activeTexture(gl.TEXTURE0);
            gl.bindTexture(gl.TEXTURE_2D, digitTex(num));
            gl.uniform1i(uDSamp, 0);
            gl.enable(gl.BLEND);
            gl.blendFunc(gl.ONE, gl.ONE_MINUS_SRC_ALPHA);
            gl.depthMask(false); // premultiplied
            gl.bindBuffer(gl.ARRAY_BUFFER, digitBuf);
            gl.enableVertexAttribArray(aDPos);
            gl.vertexAttribPointer(aDPos, 3, gl.FLOAT, false, 20, 0);
            inst.vertexAttribDivisorANGLE(aDPos, 0);
            gl.enableVertexAttribArray(aDUV);
            gl.vertexAttribPointer(aDUV, 2, gl.FLOAT, false, 20, 12);
            inst.vertexAttribDivisorANGLE(aDUV, 0);
            gl.drawArrays(gl.TRIANGLES, 0, 6);
            gl.depthMask(true);
            gl.disable(gl.BLEND);
        };
        const drawHead = (proj, view, model, hx, hy, hz) => {
            ...
            gl.uniformMatrix4fv(uHProj, false, proj);
            gl.uniformMatrix4fv(uHView, false, view);
            gl.uniformMatrix4fv(uHModel, false, model);
            gl.uniform3f(uHead, hx, hy, hz);
            gl.uniform1f(uHScale, headScale);
            gl.uniform3f(uHColor, col[0], col[1], col[2]);
            gl.bindBuffer(gl.ARRAY_BUFFER, variant.buf);
            gl.enableVertexAttribArray(hPos);
            gl.vertexAttribPointer(hPos, 3, gl.FLOAT, false, 24, 0);
            inst.vertexAttribDivisorANGLE(hPos, 0);
            gl.enableVertexAttribArray(hNrm);
            gl.vertexAttribPointer(hNrm, 3, gl.FLOAT, false, 24, 12);
            inst.vertexAttribDivisorANGLE(hNrm, 0);
            gl.drawArrays(gl.TRIANGLES, 0, variant.count);
            if (multiTool && state.toolMarkers)
                drawDigit(proj, view, model, hx, hy, hz, state.headToolNum | 0);
        };
        ...
        computeHead = () => {
            ...
            if (multiTool) {
                ...
                if (state.isoTool) {
                    if (state.isoNoFade || state.isoCurTool == null) {
                        state.isoCurTool = raw;
                        state.isoFadeTool = null;
                        state.isoFadeT = 1;
                    }
                    else if (raw !== state.isoCurTool) {
                        state.isoFadeTool = state.isoCurTool;
                        state.isoCurTool = raw;
                        state.isoFadeT = 0;
                    }
                }
                if (state.toolMarkers) {
                    state.headToolRank = rankMap.get(raw) || 1;
                    state.headToolNum = rawToNum(raw);
                }
                else {
                    state.headToolRank = 1;
                    state.headToolNum = 0;
                }
            }
            else {
        ...
        drawImpl = (proj, view, model) => {
            ...
            gl.uniform1f(uYspan, ySpan);
            gl.uniform1f(uFmin, fMin);
            gl.uniform1f(uFspan, fSpan);
            gl.uniform1f(uClip, state.clip);
            gl.uniform1f(uMinW, state.minWidth === 'all' ? 1 : 0);
            gl.uniform1f(uMinPx, 1.3);
            gl.uniform1f(uFlat, state.flatten ? 1 : 0);
            gl.uniform1f(uAlpha, 1);
            gl.uniform1f(uDim, 1);
            gl.uniform1f(uIsoUseType, isCNC ? 1 : 0);
            gl.uniform1f(uIsoMode, 0);
            gl.uniform1f(uIsoTool2, -1); // no second exclusion unless a crossfade sets one
            gl.uniform1f(uWmin, wHalfMin);
            gl.uniform1f(uWspan, wSpan);
            if (uFilCols)
                gl.uniform3fv(uFilCols, filCols);
            if (uVis)
                gl.uniform1fv(uVis, state.vis);
            if (uFilVis)
                gl.uniform1fv(uFilVis, state.filVis);
            bindTemplate();
            const shown = Math.max(0, Math.min(segN, state.shown | 0));
            ...
            const drawTravelPass = () => {
                gl.uniform1f(uTravel, 1);
                gl.uniform1f(uMode, 0);
                gl.uniform1f(uTypeBias, 0); // travels are one type, no bias
                ...
                if (state.playKind === 2 && travShown < travN) {
                    ...
                    gl.bindBuffer(gl.ARRAY_BUFFER, partTravBuf);
                    gl.bufferData(gl.ARRAY_BUFFER, partTravArr, gl.DYNAMIC_DRAW);
                    bindInstances(partTravBuf);
                    inst.drawArraysInstancedANGLE(gl.TRIANGLES, 0, 24, 1);
                }
                if (state.translucentTravel) {
                    gl.depthMask(true);
                    gl.disable(gl.BLEND);
            ...
            if (wantTravel && !state.translucentTravel)
                drawTravelPass();
            ...
            gl.uniform1f(uTravel, 0);
            gl.uniform1f(uMode, state.mode);
            gl.uniform1f(uTypeBias, -proj[10] * worldStep);
            ...
            const drawPartial = () => {
                ...
                const q = shown * STR;
                for (let i = 0; i < STR; i++)
                    partArr[i] = instData[q + i];
                partArr[3] = hx;
                partArr[4] = hy;
                partArr[5] = hz; // bead ends at the live tool point
                gl.bindBuffer(gl.ARRAY_BUFFER, partBuf);
                gl.bufferData(gl.ARRAY_BUFFER, partArr, gl.DYNAMIC_DRAW);
                bindInstances(partBuf);
                inst.drawArraysInstancedANGLE(gl.TRIANGLES, 0, 24, 1);
            };
            ...
            if (state.isoTool && multiTool && state.isoEndT < 1) {
                ...
                if (fading) {
                    ...
                    bindInstances(segBuf);
                    inst.drawArraysInstancedANGLE(gl.TRIANGLES, 0, 24, shown);
                    drawPartial();
                    gl.enable(gl.BLEND);
                    gl.blendFunc(gl.SRC_ALPHA, gl.ONE_MINUS_SRC_ALPHA);
                    // Everything except the two crossfading tools, held translucent.
                    gl.uniform1f(uIsoMode, 2);
                    gl.uniform1f(uIsoTool2, state.isoFadeTool | 0);
                    gl.uniform1f(uAlpha, othersA);
                    bindInstances(segBuf);
                    inst.drawArraysInstancedANGLE(gl.TRIANGLES, 0, 24, shown);
                    // Outgoing tool easing from solid down to translucent.
                    gl.uniform1f(uIsoMode, 1);
                    gl.uniform1f(uIsoTool, state.isoFadeTool | 0);
                    gl.uniform1f(uAlpha, aOut);
                    bindInstances(segBuf);
                    inst.drawArraysInstancedANGLE(gl.TRIANGLES, 0, 24, shown);
                    gl.disable(gl.BLEND);
                    gl.uniform1f(uAlpha, 1);
                    gl.uniform1f(uIsoTool2, -1);
                }
                else {
                    gl.uniform1f(uIsoTool, state.headToolRaw | 0);
                    ...
                    drawPartial();
                    gl.uniform1f(uIsoMode, 2);
                    gl.uniform1f(uAlpha, othersA); // pass 2: other tools, translucent (depth writes stay on; othersA ramps to 1 for the end fade-back)
                    gl.enable(gl.BLEND);
                    gl.blendFunc(gl.SRC_ALPHA, gl.ONE_MINUS_SRC_ALPHA);
                    bindInstances(segBuf);
                    inst.drawArraysInstancedANGLE(gl.TRIANGLES, 0, 24, shown);
            ...
            if (wantTravel && state.translucentTravel)
                drawTravelPass();
            if (state.head && haveHead)
                drawHead(proj, view, model, hx, hy, hz);
        };
        drawImpl.hasTravel = !!travBuf;
    }
    else {
        // Fallback: centreline lines.
        const prog = makeProg(vsLine, fsLine);
        gl.useProgram(prog);
        ...
        drawImpl = (proj, view, model) => {
            ...
            if (frac > 0) {
                const q = shown * 6;
                partLine[0] = linePos[q];
                partLine[1] = linePos[q + 1];
                partLine[2] = linePos[q + 2];
                ...
                gl.bindBuffer(gl.ARRAY_BUFFER, partLineBuf);
                gl.bufferData(gl.ARRAY_BUFFER, partLine, gl.DYNAMIC_DRAW);
                gl.enableVertexAttribArray(aPos);
                gl.vertexAttribPointer(aPos, 3, gl.FLOAT, false, 0, 0);
    ...
    function resize() {
        if (exportW) {
            applyExportSize();
            return;
        } // clip export owns the canvas size
    ...
    function applyExportSize() {
        ...
        viewW = exportW;
        viewH = exportH;
        canvas.width = Math.round(exportW * ss);
        canvas.height = Math.round(exportH * ss);
    ...
    function draw() {
        ...
        if (state.follow && state.headValid) {
            pvx = state.headX;
            pvy = state.headY;
            pvz = state.headZ;
        }
        ...
        if (state.showBed && bedGrid)
            r = Math.max(r, Math.min(bedBoundR + 0.05 + pvDisp, (boundRMax * 1.08 + 0.02) * 4));
        const near = Math.max(0.01, state.dist - r);
        const far = state.dist + r + 0.02;
        let proj;
        if (state.ortho) {
            const hh = state.dist * 0.4142;
            proj = mat4Ortho(-hh * aspect, hh * aspect, -hh, hh, near, far);
        }
        else
            proj = mat4Perspective(45 * Math.PI / 180, aspect, near, far);
    ...
    function positionWaitTag(proj, view, model) {
        if (!(state.head && state.paused && state.pauseText && state.headValid)) {
        ...
        if (pauseTag.textContent !== state.pauseText)
            pauseTag.textContent = state.pauseText;
    ...
    function loop(now) {
        ...
        if (state.isoFadeT < 1) {
            state.isoFadeT = Math.min(1, state.isoFadeT + dt / ISO_FADE_S);
            dirty = true;
        }
        ...
        if (state.isoEndRelease) {
            if (state.isoEndT < 1) {
                state.isoEndT = Math.min(1, state.isoEndT + dt / ISO_FADE_S);
                dirty = true;
            }
        }
        else if (state.isoEndT !== 0) {
            state.isoEndT = 0;
            dirty = true;
        }
        isoFadeLast = now || 0;
    ...
    canvas.addEventListener('touchstart', (e) => {
        if (e.touches.length === 2) {
            ...
            pcx = (a.clientX + b.clientX) / 2;
            pcy = (a.clientY + b.clientY) / 2;
        }
        else if (e.touches[0]) {
            twoFinger = false;
            down(e.touches[0].clientX, e.touches[0].clientY, false);
        }
    }, { passive: true });
    canvas.addEventListener('touchmove', (e) => {
        if (twoFinger && e.touches.length === 2) {
            ...
            if (pinchDist > 0)
                state.dist = Math.max(0.04, Math.min(150, state.dist * (pinchDist / (d || 1))));
            state.panX += (mx - pcx) * panK();
            state.panY -= (my - pcy) * panK();
            pinchDist = d;
            pcx = mx;
            pcy = my;
            dirty = true;
        ...
        else if (!twoFinger && e.touches[0]) {
            move(e.touches[0].clientX, e.touches[0].clientY);
            e.preventDefault();
        }
    }, { passive: false });
    canvas.addEventListener('touchend', (e) => { if (!e.touches.length) {
        up();
        twoFinger = false;
    } });
    ...
    paintWheel();
    canvas.addEventListener('wheel', (e) => { if (!wheelOn)
        return; e.preventDefault(); zoomBy(1 + Math.sign(e.deltaY) * 0.1); }, { passive: false });
    ...
    function snapshot() {
        ...
        state.follow = 0;
        state.yaw = Math.PI / 4;
        state.pitch = Math.atan(1 / Math.SQRT2);
    ...
    fsBtn.addEventListener('click', () => { if (document.fullscreenElement)
    ...
    function holdZoom(btn, factor) {
        ...
        btn.addEventListener('pointerdown', (e) => {
            ...
            stop();
            timer = setInterval(() => zoomBy(factor), 60);
        });
    ...
    if (data.hasTypes)
        colourSel.appendChild(el('option', { value: '0' }, 'Colour: line type'));
    else if (cncToolCols.length > 1)
        colourSel.appendChild(el('option', { value: '0' }, 'Colour: tool'));
    colourSel.appendChild(el('option', { value: '1' }, 'Colour: height'));
    colourSel.appendChild(el('option', { value: '2' }, 'Colour: speed'));
    if (isPrintV)
        colourSel.appendChild(el('option', { value: '4' }, 'Colour: width'));
    if (isPrintV && data.multicolour)
        colourSel.appendChild(el('option', { value: '3' }, 'Colour: filament'));
    ...
    const refreshLegend = () => {
        ...
        if (m === 1) {
            ...
            for (const fi of fils) {
                ...
                const row = keyRow();
                setSwatch(row, hex);
                row.lastChild.textContent = 'Filament ' + (fi + 1);
            ...
            for (const id of data.features) {
                const f = FEATURES[id];
                const row = keyRow();
                setSwatch(row, swatchRgb(f.rgb));
                row.lastChild.textContent = f.label;
                legendBody.appendChild(row);
            }
        }
        else if (cncToolCols.length > 1) {
            for (const t of cncToolCols) {
                const f = FEATURES[t.slot];
                const row = keyRow();
                setSwatch(row, swatchRgb(f.rgb));
                row.lastChild.textContent = 'T' + t.n;
                legendBody.appendChild(row);
            }
        }
    ...
    const api = {
        ...
        start: () => { resize(); if (!state.fitted) {
            state.dist = fitDist(0.9);
            state.fitted = true;
        } requestAnimationFrame(loop); }, markDirty: () => { dirty = true; },
...
function detectSlicer(comments) {
    for (const c of comments) {
        let m = c.match(/sliced by\s+(.+)/i);
        if (m)
            return m[1].trim();
        m = c.match(/generated (?:with|by)\s+(.+)/i);
        if (m)
            return m[1].trim();
...
export async function renderGcode(file, resultsEl, opts) {
    ...
    resultsEl.innerHTML = '';
    if (!data || !data.sawG) {
        resultsEl.appendChild(errorCard('This file does not look like G-code (no G0/G1/G2/G3 moves found).'));
        return;
    }
    ...
    if (data.segCount > 0) {
        ...
        if (uncapped) {
            viewer.state.ssaa = false;
            viewer.state.minWidth = 'none';
            viewer.state.translucentTravel = false;
        }
        if (isPrint)
            viewCard.appendChild(el('p', { class: 'anr-hint', style: 'margin:0 0 10px;' }, viewer.instanced
        ...
        if (viewer.ok) {
            ...
            function applyMSAA(on) {
                ...
                if (!next.ok)
                    return; // keep the working viewer if rebuild fails
                viewer = next;
                ...
                viewer.onSpinChange(updateSpinLabel);
                if (viewer.topLeft)
                    viewer.topLeft.appendChild(qWrap); // re-park Quality in the rebuilt overlay
                viewer.start();
            ...
            aaCycleBtn('Minimum line width', [{ v: 'none', label: 'None' }, { v: 'travel', label: 'Travel lines' }, { v: 'all', label: 'All' }], () => viewer.state.minWidth, (v) => { viewer.state.minWidth = v; viewer.markDirty(); });
            ...
            viewRow.appendChild(spinBtn);
            viewRow.appendChild(resetBtn);
            viewRow.appendChild(projBtn);
            viewRow.appendChild(travelBtn);
            if (bedBtn)
                viewRow.appendChild(bedBtn);
            ...
            if (viewer.topLeft)
                viewer.topLeft.appendChild(qWrap);
            ...
            if (data.multicolour && (data.filsUsed || []).length > 1) {
                ...
                for (const idx of data.filsUsed) {
                    ...
                    chip.addEventListener('click', () => {
                        const off = chip.classList.toggle('is-off');
                        chip.setAttribute('aria-pressed', off ? 'false' : 'true');
                        viewer.state.filVis[slot] = off ? 0 : 1;
                        viewer.markDirty();
                    });
            ...
            if (cncTools.length > 1) {
                ...
                for (const t of cncTools) {
                    ...
                    chip.addEventListener('click', () => {
                        ...
                        viewer.state.vis[t.slot] = off ? 0 : 1;
                        viewer.markDirty();
                    });
            ...
            const hudItems = [];
            if (hudShowTime)
                hudItems.push(hudTime);
            if (hudShowLayer)
                hudItems.push(hudLayer);
            if (hudShowTool)
                hudItems.push(hudTool);
            ...
            if (hud)
                updateHud(G); // seed the readout to the finished state (the default full view)
            ...
            const applyGlobal = (gi) => {
                ...
                if (g >= G) {
                    s.shown = data.segCount;
                    s.travShown = data.travelCount;
                    s.partial = 0;
                    s.playKind = 0;
                    s.playFrac = 0;
                    s.paused = false;
                    ...
                    {
                        let k = g;
                        while (k < G && order[k] > 1.5)
                            k++;
                        if (k < G) {
                            if (order[k] < 0.5) {
                                const q = gExt[g] * 10;
                                s.restX = data.seg[q];
                                s.restY = data.seg[q + 1];
                                s.restZ = data.seg[q + 2];
                            }
                            else {
                                const q = gTrav[g] * 7;
                                s.restX = data.travel[q];
                                s.restY = data.travel[q + 1];
                                s.restZ = data.travel[q + 2];
                            }
                            s.restValid = true;
                        }
                    }
            ...
            function stopPlay() { if (!playing)
                return; playing = false; playBtn.textContent = 'Play'; viewer.state.head = false; viewer.markDirty(); if (playRAF)
                cancelAnimationFrame(playRAF); playRAF = 0; }
            function stepPlay(ts) {
                if (!playing)
                    return;
                if (!viewer.wrap.isConnected) {
                    stopPlay();
                    return;
                }
                if (!lastTs)
                ...
                if (realtime) {
                    ...
                    playElapsed += dt;
                    if (playElapsed >= realTotal) {
                        playPos = G;
                        applyGlobal(G);
                        viewer.state.isoEndRelease = true;
                        stopPlay();
                        return;
                    }
                    let g = Math.max(0, Math.min(G - 1, Math.floor(playPos)));
                    while (g < G && gTime[g + 1] <= playElapsed)
                        g++;
                    while (g > 0 && gTime[g] > playElapsed)
                        g--;
            ...
            const choose = (lps, label, btn) => { realtime = false; playLps = lps; setLabel(label); clearActive(); if (btn)
                btn.classList.add('is-active'); rtWarn.hidden = true; };
            const chooseReal = (btn) => { realtime = true; setLabel('real time (' + fmtDur(realTotal) + ')'); clearActive(); if (btn)
                btn.classList.add('is-active'); rtWarn.hidden = false; };
            ...
            for (const [v, l] of LINE_PRESETS) {
                ...
                allPresetBtns.push(b);
                lpsCol.appendChild(b);
            }
            let defBtn = null;
            ...
            if (realTotal > 0) {
                ...
                allPresetBtns.push(rb);
                lenCol.appendChild(rb);
                rtBtn = rb;
            }
            cols.appendChild(lpsCol);
            cols.appendChild(lenCol);
            ...
            const applyCustom = () => {
                ...
                if (!(v > 0))
                    return;
                if (customMode === 'lps')
                    choose(v, Math.round(v).toLocaleString() + ' lines/s', null);
                else
                    choose(lpsForLen(v), 'whole job in ' + v + 's', null);
            };
            ...
            const dockSpd = () => { if (spdPanel.parentNode !== spdWrap) {
                spdPanel.classList.remove('anr-spd-panel--fs');
                spdWrap.appendChild(spdPanel);
            } };
            const openSpd = () => {
                const fs = document.fullscreenElement;
                if (fs && viewer && fs === viewer.wrap) {
                    spdPanel.classList.add('anr-spd-panel--fs');
                    viewer.wrap.appendChild(spdPanel);
                }
                spdPanel.classList.remove('is-hidden');
            };
            const closeSpd = () => { spdPanel.classList.add('is-hidden'); dockSpd(); };
            spdBtn.addEventListener('click', (e) => { e.stopPropagation(); if (spdPanel.classList.contains('is-hidden'))
                openSpd();
            else
                closeSpd(); });
            ...
            const buildMarkPop = (e) => {
                ...
                for (const m of (single ? e.items : e.items)) {
                    ...
                    if (data.multicolour && col) {
                        ...
                        line.appendChild(document.createTextNode(' - ' + titleCase(m.desc)));
                    }
            ...
            progSlider.style.width = '100%';
            progSlider.style.display = 'block';
            ...
            sliderWrap.appendChild(progSlider);
            sliderWrap.appendChild(markLayer);
            sliderWrap.appendChild(markPop);
            sliderWrap.addEventListener('mousemove', (e) => {
                if (!toolMarksOn || !marks.length) {
                ...
                for (const m of marks) {
                    const d = Math.abs(m.pct - frac);
                    if (d < bestd) {
                        bestd = d;
                        best = m;
                    }
                }
                if (best && bestd * rect.width <= 6) {
                    buildMarkPop(best);
                    markPop.style.left = (best.pct * 100).toFixed(3) + '%';
                    markPop.style.display = '';
                }
                else
                    markPop.style.display = 'none';
            });
            ...
            if (!marks.length) {
                ...
                markBtn.title = 'No tool or filament changes in this file';
            }
            ...
            markBtn.addEventListener('click', () => {
                ...
                if (!toolMarksOn)
                    markPop.style.display = 'none';
                viewer.state.toolMarkers = toolMarksOn;
                viewer.markDirty();
            });
            ...
            if (!hasMultiTool) {
                ...
                isoBtn.title = 'This file only uses one tool';
            }
            isoBtn.addEventListener('click', () => {
                ...
                viewer.state.isoFadeTool = null;
                viewer.state.isoFadeT = 1;
            ...
            const fitIsoLabel = () => {
                ...
                if (isoBtn.scrollWidth > isoBtn.clientWidth)
                    isoBtn.textContent = 'Dim tools';
            };
            if (window.ResizeObserver) {
                const ro = new ResizeObserver(fitIsoLabel);
                ro.observe(isoBtn);
            }
            ...
            if (window.VideoEncoder && window.VideoFrame) {
                ...
                const sanitizeAvcC = (desc) => {
                    try {
                        ...
                        if (b.length < 7 || b[0] !== 1)
                            return desc;
                        let o = 6;
                        const read = (n) => { const list = []; for (let i = 0; i < n; i++) {
                            const len = (b[o] << 8) | b[o + 1];
                            o += 2;
                            list.push(dedupeNalHeader(b.subarray(o, o + len)));
                            o += len;
                        } return list; };
                        ...
                        for (const s of spsList)
                            out.push(s.length >> 8, s.length & 0xff, ...s);
                        out.push(ppsList.length & 0xff);
                        for (const p of ppsList)
                            out.push(p.length >> 8, p.length & 0xff, ...p);
                ...
                const stripInbandNals = (buf) => {
                    ...
                    while (i + 4 <= buf.length) {
                        ...
                        if (len === 0 || i + len > buf.length)
                            return buf; // malformed - leave as-is
                        const t = buf[i] & 31;
                        if (t === 7 || t === 8 || t === 9)
                            dropped = true;
                        else {
                            keep.push(buf.subarray(i, i + len));
                            total += 4 + len;
                        }
                        i += len;
                    }
                ...
                const drawClipOverlay = (ctx, W, H, t, withInfo) => {
                    ...
                    const chip = (txt, right) => {
                        ...
                        ctx.fillText(txt, x + pad, y + chipH / 2 + 1);
                    };
                    ...
                    if (withInfo) {
                        ...
                        let name = file.name;
                        while (name.length > 6 && ctx.measureText(name + tail).width + pad * 2 > leftMax)
                            name = name.slice(0, -2);
                        if (name !== file.name)
                            name += '…';
                ...
                async function renderClip() {
                    ...
                    progSlider.disabled = true; // they'd fight the frame loop
                    const [W, H] = clipDims();
                    ...
                    for (const m of SCENE_MIRROR)
                        saved[m.st] = s0[m.st]; // snapshot the mirrored flags so export reverts them
                    ...
                    const arm = () => {
                        const v = viewer;
                        curCanvas = v.canvas;
                        if (statBox.parentNode !== v.wrap)
                            v.wrap.appendChild(statBox);
                    ...
                    try {
                        ...
                        if (cfgEnc) {
                            let gotCfg = false;
                            const probe = new VideoEncoder({ output: (pc, pm) => { if (pm && pm.decoderConfig && pm.decoderConfig.description) {
                                    gotCfg = true;
                                    probeDesc = pm.decoderConfig;
                                } }, error: () => { } });
                            try {
                                probe.configure(cfgEnc);
                                const pf = new VideoFrame(comp, { timestamp: 0, duration: Math.round(1e6 / fps) });
                                probe.encode(pf, { keyFrame: true });
                                pf.close();
                                await probe.flush();
                            }
                            catch (_) {
                        ...
                        if (!mp4 && cfgEnc) {
                            ...
                            if (sup && sup.supported) {
                                const probe = new VideoEncoder({
                                    output: (pc) => {
                                        const d = new Uint8Array(pc.byteLength);
                                        pc.copyTo(d);
                                        for (const nal of splitAnnexB(d)) {
                                            const t = nal[0] & 31;
                                            if (t === 7 && !sps)
                                                sps = nal.slice();
                                            else if (t === 8 && !pps)
                        ...
                        for (let i = 0; i <= NF; i++) {
                            ...
                            let gi;
                            if (i < introF)
                                gi = introF ? (i / introF) * g0 : 0;
                            else
                                gi = g0 + Math.min(1, (i - introF) / N) * (G - g0);
                            const t = G ? gi / G : 1;
                            ...
                            if (i >= mainEnd && viewer.state.isoTool) {
                                ...
                                if (fadeP >= 1) {
                                    viewer.state.isoTool = false;
                                    viewer.state.isoAlpha = 0.22;
                                }
                                else
                                    viewer.state.isoAlpha = 0.22 + (1 - 0.22) * (fadeP * fadeP * (3 - 2 * fadeP)); // smoothstep
                            }
                            ...
                            if (clipCfg.orbit) {
                                viewer.state.yaw = yaw0 + (i / fps) * 0.18;
                                viewer.markDirty();
                            }
                            viewer.draw();
                        ...
                        if (!cancelled) {
                            ...
                            if (!isMp4) {
                                ...
                                try {
                                    ...
                                    ext = 'webm';
                                    mime = 'video/webm'; // the WebM is still a good clip
                                }
                                finally {
                                    if (ff && ff.off && onProg) {
                                        try {
                        ...
                        v.state.isoFadeT = 1;
                        v.state.isoFadeTool = null;
                        v.state.isoCurTool = v.state.isoTool ? (v.state.headToolRaw | 0) : null;
                        scrubTo(savedPos);
                        playBtn.disabled = false;
                        progSlider.disabled = false;
                        clipBusy = false;
                ...
                const openClipPopup = () => {
                    ...
                    for (const m of SCENE_MIRROR)
                        clipCfg[m.cfg] = m.get(viewer.state);
                    const paints = [];
                    ...
                    const togRow = (defs) => {
                        ...
                        for (const [key, lab, title, off] of defs) {
                            ...
                            if (off)
                                ...
                                b.addEventListener('click', () => { clipCfg[key] = !clipCfg[key]; repaint(); });
                    ...
                    const setDur = (d) => { if (isFinite(d) && d > 0)
                        clipCfg.dur = Math.min(DUR_MAX, Math.max(DUR_MIN, d)); };
                    ...
                    const numField = (unit, get, set, step) => {
                        ...
                        inp.addEventListener('change', () => { const v = parseFloat(inp.value); if (isFinite(v) && v > 0)
                            set(v); repaint(); });
                        paints.push(() => { if (document.activeElement !== inp)
            ...
            if (clipBtn)
                playCtrlRow.appendChild(clipBtn);
            ...
            if (hud)
                progRow.appendChild(hud); // live time / layer / tool readout
            ...
            if (legend)
                toolbar.appendChild(legend); // line-type show/hide under Build height
            if (filRow)
                toolbar.appendChild(filRow); // filament show/hide under the line types
            if (toolRow)
                toolbar.appendChild(toolRow); // CNC Tools picker under Build height
            ...
            if (data.capped && !(opts && opts.uncapped)) {
                ...
                const fullWarn = el('p', { class: 'anr-spd-warn' }, `Only the first ${data.segCount.toLocaleString()} segments are drawn - this print exceeds the ${SEG_CAP.toLocaleString()}-segment limit tuned to your device's memory. Showing it in full can use a lot of RAM and may run slowly or crash the tab.`);
                fullBtn.addEventListener('click', () => {
                    fullBtn.disabled = true;
                    fullBtn.textContent = 'Rebuilding…';
    ...
    if (data.printerModel)
        tbl.appendChild(rowHelp('Printer', data.printerModel, 'The printer or machine this file was prepared for, read from the slicing software’s settings saved at the top of the file.'));
    // Headline outcome - the first things you reach for.
    if (data.printTime)
        tbl.appendChild(rowHelp('Est. print time', data.printTime, 'How long the print is expected to take, as estimated by the slicing software and saved at the top of the file.'));
    ...
    if (isPrint) {
        ...
        if (data.extrudeMM)
            tbl.appendChild(rowHelp('Filament used', `${(data.extrudeMM / 1000).toFixed(2)} m  (${Math.round(data.extrudeMM).toLocaleString()} ${u})`, 'The total length of plastic filament pushed through the nozzle, added up from the extruder (E axis) movements in the file.'));
        if (data.layerCount)
            tbl.appendChild(rowHelp('Layers', data.layerCount.toLocaleString(), 'How many horizontal layers the print is built from - each layer is one pass at a distinct height, stacked to form the object.'));
        if (data.layerHeight)
            tbl.appendChild(rowHelp('Layer height', data.layerHeight.toFixed(3) + ' ' + u, 'How thick each layer is - the usual vertical step from one layer up to the next (the median gap between layer heights).'));
        if (data.temps.nozzle)
            tbl.appendChild(rowHelp('Nozzle temp', data.temps.nozzle + ' °C', 'The temperature the nozzle is told to heat to for melting the plastic (set by the M104/M109 commands).'));
        if (data.temps.bed)
            tbl.appendChild(rowHelp('Bed temp', data.temps.bed + ' °C', 'The temperature the heated print bed is told to reach, which helps the first layer stick (set by the M140/M190 commands).'));
        ...
        if (data.bed) {
            ...
            if (b.type)
                tbl.appendChild(rowHelp('Build plate', b.type, 'The type of print-bed surface chosen in the slicing software - for example textured PEI, smooth, or glass.'));
        }
        ...
        const c = data.cnc;
        if (c && c.progNum)
            tbl.appendChild(rowHelp('Program number', 'O' + c.progNum, 'The job number the CAM software wrote into the file (its ‘O’ number) - the identifier the machine’s controller lists.'));
        if (c && c.operations && c.operations.length)
            tbl.appendChild(rowHelp('Operations', c.operations.length.toLocaleString(), 'How many separate machining steps (toolpaths) the program contains. See the operations table below.'));
        if (data.cutMM)
            tbl.appendChild(rowHelp('Cut path length', `${(data.cutMM / 1000).toFixed(2)} m  (${Math.round(data.cutMM).toLocaleString()} ${u})`, 'The total distance the tool travels while actually cutting (its feed moves).'));
        if (c && c.maxDepth != null && c.maxDepth < 0)
            tbl.appendChild(rowHelp('Max cut depth', `${c.maxDepth.toFixed(2)} ${u}  (${Math.abs(c.maxDepth).toFixed(2)} ${u} below Z0)`, 'How far down the tool reaches at its deepest point.'));
        if (c) {
            if (c.changes.length)
                tbl.appendChild(rowHelp('Tool changes', `${c.changes.length.toLocaleString()}  (${c.tools.length} tool${c.tools.length === 1 ? '' : 's'})`, 'How many times the machine swaps its cutting tool (M6 commands), and how many different tools it uses in total. See the tooling table below.'));
            if (c.spindleMax)
                tbl.appendChild(rowHelp('Max spindle speed', `${Math.round(c.spindleMax).toLocaleString()} rpm${c.spindleDir ? ' · ' + c.spindleDir : ''}`, 'The fastest the cutting spindle is told to spin (the S value), and which way it turns (clockwise M3 or anticlockwise M4).'));
            if (c.coolant.length)
                tbl.appendChild(rowHelp('Coolant', c.coolant.join(', '), 'The coolant the program turns on to keep the cut cool - a fine mist (M7) or a heavier flood (M8).'));
            if (c.workOffsets.length)
                tbl.appendChild(rowHelp('Work offsets', c.workOffsets.join(', '), 'The reference zero points the program uses (G54-G59), typically one for each part or setup clamped on the machine.'));
            if (c.optStops)
                tbl.appendChild(rowHelp('Optional stops', c.optStops.toLocaleString(), 'Points where the program can pause (M1), but only if the operator has the "optional stop" switch turned on.'));
            if (c.progEnd)
                tbl.appendChild(rowHelp('Program end', c.progEnd, 'How the program marks its finish - M2 (simply end) or M30 (end and rewind to the start).'));
        }
    }
    // Provenance.
    if (slicer)
        tbl.appendChild(rowHelp(isPrint ? 'Slicer' : 'CAM / sender', slicer, 'The software that created this G-code, named at the top of the file.'));
    ...
    if (data.counts.arc)
        tbl.appendChild(rowHelp('Arc moves', data.counts.arc.toLocaleString(), 'Curved, circular moves (the G2/G3 commands), drawn here as smooth arcs.'));
    ...
    if (data.feedRange.max)
        tbl.appendChild(rowHelp('Feedrate range', `${Math.round(data.feedRange.min).toLocaleString()} - ${Math.round(data.feedRange.max).toLocaleString()} ${u}/min`, 'The range of speeds, from slowest to fastest, at which the tool is told to move while working (the F value).'));
    ...
    if (data.capped)
        tbl.appendChild(row('Note', `Capped at ${SEG_CAP.toLocaleString()} segments for performance; the viewer shows the first ones.`));
    ...
    if (cnc && (cnc.tools.length || cnc.changes.length)) {
        ...
        if (cnc.operations && cnc.operations.length) {
            ...
            const det = el('details', { style: 'margin-top:14px;' });
            if (cnc.operations.length <= 12)
                det.open = true; // short list: open by default
