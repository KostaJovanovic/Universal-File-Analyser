...
function messageCard(text, title) {
    ...
    try {
        walkMime(head, body, acc);
    }
    catch (_) { }
    if (!acc.html && !acc.text)
        acc.text = body; // not MIME / single part
    ...
    const rows = [
        ...
        (hdrs['received'] && hdrs['received'].length) && rowHelp('Received hops', String(hdrs['received'].length), 'The number of mail servers that passed this message along on its way to you. Each server that handles an email stamps a "Received:" line into the hidden headers, so counting those lines shows how many hops the message made.'),
    ];
    const auth = authVerdict(hdrs);
    if (auth.has) {
        const parts = [];
        if (auth.spf)
            parts.push('SPF ' + auth.spf);
        if (auth.dkim)
            parts.push('DKIM ' + auth.dkim);
        if (auth.dmarc)
            parts.push('DMARC ' + auth.dmarc);
        if (parts.length)
            rows.push(rowHelp('Authentication', parts.join('  -  '), 'Checks that help prove an email really came from who it claims. The receiving server runs three: SPF (was this server allowed to send for that domain?), DKIM (does the message carry a valid cryptographic signature?) and DMARC (do those two line up with the domain’s stated policy?). "pass" is good; "fail" or "none" is worth a closer look.'));
    }
    ...
    if (acc.attachments.length) {
        ...
        for (const a of acc.attachments)
            tbl.appendChild(row(a.name, a.type + '  -  ' + fmtBytes(a.size)));
...
export async function renderEml(file, container) {
    ...
    try {
        ...
        if (/\.emlx$/i.test(file.name)) {
            ...
            if (nl > 0 && Number.isFinite(n))
                text = text.slice(nl + 1, nl + 1 + n);
        }
        container.innerHTML = '';
...
export async function renderMbox(file, container) {
    ...
    try {
        ...
        container.appendChild(info);
        if (!chunks.length) {
            container.appendChild(el('div', { class: 'anr-card' }, [el('h3', {}, 'Messages'), el('p', { class: 'anr-hint' }, 'No messages found in this mailbox.')]));
            return;
        }
        let shown = 0;
        const BATCH = 10;
        ...
        function reveal(upTo) {
            for (; shown < upTo && shown < chunks.length; shown++)
                host.appendChild(messageCard(chunks[shown], 'Message ' + (shown + 1)));
            if (shown >= chunks.length)
                btnRow.hidden = true;
            else
                moreBtn.textContent = 'Show more messages (' + shown + '/' + chunks.length + ')';
        }
