...
function getWorker() {
    if (!worker)
        worker = new Worker(new URL('./audio-dsp-worker.js', import.meta.url), { type: 'module' });
    return worker;
}
...
export function runAudioDsp(audioBuffer, { needBpm, signal, onPass }) {
    return new Promise((resolve, reject) => {
        ...
        const onMsg = (e) => {
            const m = e.data;
            if (!m || m.jobId !== jobId)
                return; // a reply belonging to another job
            if (m.type === 'pass') {
                try {
                    onPass(m.name, m.value);
                }
                catch (_) { }
            }
            else if (m.type === 'done') {
                ...
                cleanup();
                reject(new Error(m.message || 'audio DSP failed'));
            }
        };
        ...
        w.postMessage({ type: 'run', jobId, channels, sampleRate: audioBuffer.sampleRate, needBpm }, channels.map((c) => c.buffer));
    });
}
//# sourceMappingURL=audio-dsp-client.js.map