...
function slnxProjectKind(path, typeAttr) {
    ...
    if (typeAttr) {
        ...
        if (PROJECT_TYPES[guid])
            return PROJECT_TYPES[guid];
        if (!/^[0-9A-F-]{30,}$/.test(guid))
            return typeAttr; // a friendly type moniker
    }
...
function parseSln(text) {
    ...
    const configs = [...text.matchAll(/^\s*(.+?)\s*=\s*(?:Debug|Release)[^\n|]*\|[^\n=]+$/gm)]
        .map((x) => x[1].trim())
        .filter((c) => /\|/.test(c));
    const solutionConfigs = [...new Set((text.match(/GlobalSection\(SolutionConfigurationPlatforms\)[\s\S]*?EndGlobalSection/) || [''])[0]
...
export async function renderVsSolution(file, resultsEl) {
    ...
    if (!data && /Microsoft Visual Studio Solution File/i.test(text))
        data = parseSln(text);
    if (!data) {
    ...
    if (data.slnx) {
        tbl.appendChild(rowHelp('Format', 'Visual Studio solution (.slnx)', 'The newer XML form of a Visual Studio solution - the file that groups related code projects together (used by Visual Studio 2022 17.10+ and the dotnet sln tools). It holds a compact <Solution> tree of the projects, folders and build setups (build configurations, such as Debug and Release), replacing the older plain-text .sln.'));
    }
    else {
        tbl.appendChild(rowHelp('Format', 'Visual Studio solution (.sln)', 'A plain-text file that groups one or more code projects together, along with their build setups (build configurations, such as Debug and Release). Opened by Visual Studio, Rider and MonoDevelop; Unity creates one named after the project.'));
    }
    if (data.version)
        tbl.appendChild(row('Format version', data.version));
    if (data.vs)
        tbl.appendChild(row('Visual Studio', data.vs));
    if (data.vsVersion)
        tbl.appendChild(row('VS version', data.vsVersion));
    tbl.appendChild(row('Projects', String(data.projects.length)));
    if (data.folders)
        tbl.appendChild(rowHelp('Solution folders', String(data.folders), 'Named groups used to tidy the projects in Visual Studio’s list. They are just a way to organise the view, not real folders on disk.'));
    if (data.solutionConfigs.length)
        tbl.appendChild(rowHelp('Configurations', data.solutionConfigs.join(', '), 'The named build setups, each pairing a build type with a target platform - for example "Debug | Any CPU" (a build with debugging aids) or "Release | Any CPU" (an optimised build).'));
...
//# sourceMappingURL=vssolution.js.map