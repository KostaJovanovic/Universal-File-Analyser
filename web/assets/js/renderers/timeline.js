...
function parseEdl(text, fpsHint) {
    ...
    for (const raw of lines) {
        ...
        if (/DROP\s*FRAME/i.test(line) && !/NON-?DROP/i.test(line))
            dropFrame = true;
        const cm = /^\*\s*(?:FROM\s+)?CLIP\s+NAME:\s*(.+)$/i.exec(line);
        if (cm && pending) {
            pending.name = cm[1].trim();
            continue;
        }
    ...
    for (const ev of events) {
        ...
        const key = ev.chan;
        if (!trackMap.has(key))
            trackMap.set(key, { kind: ev.kind, name: ev.chan, clips: [] });
...
function parseOtio(json) {
    ...
    for (const trk of (stack.children || [])) {
        ...
        for (const child of (trk.children || [])) {
            ...
            if (sr && sr.duration && sr.duration.rate)
                fps = sr.duration.rate;
...
function parseFcpxml(text) {
    ...
    if (fmt) {
        const fd = fmt.getAttribute('frameDuration');
        const s = rationalSeconds(fd);
        if (s > 0)
            fps = Math.round(1 / s);
    }
    ...
    for (const node of Array.from(spine.children)) {
        ...
        for (const sub of Array.from(node.children)) {
            if (!CLIP_TAGS.has(sub.tagName) || sub.tagName === 'gap')
                continue;
            ...
            if (!laneTracks.has(subLane))
                laneTracks.set(subLane, []);
...
function buildTimelineCard(model) {
    ...
    card.appendChild(el('p', { class: 'anr-hint', style: 'margin-top:10px;' }, `${model.tracks.length} track${model.tracks.length === 1 ? '' : 's'} · ${model.tracks.reduce((n, t) => n + t.clips.length, 0)} clips · ${secToTc(dur, model.fps)} · hover a clip for its name and timecode`));
    return card;
}
function infoCard(file, model) {
    ...
    if (model.fps)
        tbl.appendChild(row('Frame rate', (model.dropFrame ? 'assumed ' : '') + model.fps + ' fps' + (model.dropFrame ? ' (drop-frame)' : '')));
...
export async function renderTimeline(file, resultsEl) {
    ...
    try {
        if (ext === 'otio') {
            model = parseOtio(JSON.parse(await file.text()));
        }
        else if (ext === 'fcpxml') {
            model = parseFcpxml(await file.text());
        }
        else {
            model = parseEdl(await file.text());
        } // edl
    }
    catch (e) {
        resultsEl.innerHTML = '';
