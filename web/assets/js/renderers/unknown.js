...
export function guessFormat(b) {
    ...
    if (a(0, 4) === '%PDF')
        return 'PDF document';
    if (b[0] === 0x89 && a(1, 3) === 'PNG')
        return 'PNG image';
    if (b[0] === 0xFF && b[1] === 0xD8)
        return 'JPEG image';
    if (a(0, 4) === 'GIF8')
        return 'GIF image';
    if (a(0, 4) === 'RIFF' && a(8, 4) === 'WAVE')
        return 'WAV audio';
    if (a(0, 4) === 'RIFF' && a(8, 4) === 'WEBP')
        return 'WebP image';
    if (a(0, 4) === 'RIFF' && a(8, 4) === 'AVI ')
        return 'AVI video';
    if (a(0, 4) === 'OggS')
    ...
    if (b[0] === 0xFF && (b[1] & 0xE0) === 0xE0)
        return 'MPEG audio';
    if (a(4, 4) === 'ftyp')
        return 'MP4 / MOV / M4A (' + a(8, 4).replace(/[^\w]/g, '') + ')';
    if (b[0] === 0x50 && b[1] === 0x4B)
        return 'ZIP container (docx / xlsx / epub / apk / jar / ...)';
    if (a(0, 6) === '7z\xBC\xAF\x27\x1C')
        return '7-Zip archive';
    if (b[0] === 0x1F && b[1] === 0x8B)
        return 'gzip archive';
    if (a(0, 4) === 'Rar!')
        return 'RAR archive';
    if (b[0] === 0x7F && a(1, 3) === 'ELF')
        return 'ELF binary';
    if (a(0, 2) === 'MZ')
        return 'Windows EXE / DLL (MZ)';
    if (a(0, 5) === '<?xml')
        return 'XML document';
    if (a(0, 6) === 'SQLite')
    ...
    if (a(0, 2) === 'BM')
        return 'BMP image';
    if (a(0, 4) === '\x00\x00\x01\x00')
        return 'ICO icon';
    if ((a(0, 2) === 'II' && b[2] === 0x2A) || (a(0, 2) === 'MM' && b[3] === 0x2A))
        return 'TIFF image';
    if (b[0] === 0x1A && b[1] === 0x45 && b[2] === 0xDF && b[3] === 0xA3)
        return 'Matroska / WebM';
    if (b[0] === 0xCA && b[1] === 0xFE && b[2] === 0xBA && b[3] === 0xBE)
        return 'Java class / Mach-O fat binary';
    if (a(0, 12) === 'IDEA - MAKER')
        return 'Raise3D ideaMaker project';
    if (a(0, 14) === 'IEDA - PROFILE')
        return 'Raise3D ideaMaker profile';
    ...
    if (u16)
        return u16 === 'le' ? 'UTF-16 LE text' : 'UTF-16 BE text';
    let printable = 0;
    for (const c of b)
        if (c === 9 || c === 10 || c === 13 || (c >= 0x20 && c <= 0x7E))
            printable++;
    if (printable / b.length > 0.85)
        return 'plain text';
...
function utf16Kind(b) {
    ...
    if (b.length >= 16) {
        ...
        for (let i = 0; i < n; i += 2) {
            ...
            if (lo === 9 || lo === 10 || lo === 13 || (lo >= 0x20 && lo <= 0x7E))
                lowPrintable++;
        }
        const pairs = n / 2;
        if (lowPrintable / pairs > 0.85) {
            if (oddNul / pairs > 0.6 && evenNul / pairs < 0.1)
                return 'le';
            if (evenNul / pairs > 0.6 && oddNul / pairs < 0.1)
                return 'be';
        }
    }
    return null;
...
function highlightJson(val, indent) {
    ...
    if (val === null)
        return '<span class="anr-syn-kw">null</span>';
    if (typeof val === 'boolean')
        return '<span class="anr-syn-kw">' + val + '</span>';
    if (typeof val === 'number')
        return '<span class="anr-syn-num">' + val + '</span>';
...
function formatXml(node, indent) {
    ...
    const meaningful = children.filter(c => c.nodeType === Node.ELEMENT_NODE ||
...
export async function renderUnknown(file, resultsEl, opts) {
    ...
    if (variant) {
        ...
        if (v && v.tell)
            tbl.appendChild(rowHelp('Detected as', variant, v.tell));
    }
        ...
        if (file.size > 2048) {
            ...
            fullBtn.addEventListener('click', async () => {
                ...
                try {
                    ...
                    fullBtn.textContent = 'Show full text';
                }
            });
    ...
    if (showJson) {
        ...
        try {
            ...
            if (parseError) {
                ...
                card.appendChild(el('p', { class: 'anr-hint anr-syn-error' }, 'JSON parse error: ' + parseError.message));
                ...
                if (parsed && typeof parsed === 'object' && 'v' in parsed && typeof parsed.fr === 'number'
                    ...
                    btn.addEventListener('click', async () => {
                        ...
                        try {
                            ...
                            btn.disabled = false;
                            btn.textContent = '▶ Play as Lottie animation';
                        }
                    });
    ...
    if (showXml || (guess === 'XML document' && !showJson)) {
        ...
        try {
            ...
            if (parseErr) {
                ...
                card.appendChild(el('p', { class: 'anr-hint anr-syn-error' }, 'XML parse error - showing raw text above'));
            }
            else {
    ...
    if (!showPlainText && !showJson && !showXml) {
        try {
            await appendEntropyCard(file, resultsEl);
        }
        catch (_) { }
    }
    ...
    if (!extensionless && window._anrSuggest)
        window._anrSuggest.show(fileExt(file.name));
    ...
    scanCard.appendChild(el('p', { class: 'anr-hint', style: 'margin:0 0 12px;' }, 'Recover images hidden inside this file. It reads up to ' + fmtBytes(CARVE_SCAN_CAP) + ', so it can take a moment.'));
    ...
    scanBtn.addEventListener('click', async () => {
        ...
        try {
            await appendEmbeddedImagesCard(file, resultsEl, resultHost);
        }
        catch (_) { }
...
async function appendEntropyCard(file, resultsEl) {
    if (file.size < 256)
        return; // too small for a meaningful profile
    let bytes;
    try {
        bytes = new Uint8Array(await file.slice(0, Math.min(file.size, ENTROPY_SCAN_CAP)).arrayBuffer());
    }
    catch (_) {
        return;
    }
    ...
    let assessment;
    if (max > 7.5 && (max - min) > 1.5)
        assessment = 'Contains a high-entropy region - likely compressed, encrypted or packed data embedded in lower-entropy content.';
    else if (mean > 7.5)
        assessment = 'Uniformly high - the whole file looks compressed or encrypted.';
    else if (mean < 4.5)
        assessment = 'Low - consistent with text or simple structured data.';
    else
        assessment = 'Mixed - typical of a structured binary (headers plus packed payloads).';
    const card = el('div', { class: 'anr-card' });
    ...
    card.appendChild(el('p', { class: 'anr-hint' }, 'Shannon entropy per chunk'
        + (file.size > ENTROPY_SCAN_CAP ? ' (first ' + fmtBytes(ENTROPY_SCAN_CAP) + ' scanned)' : '') + '.'));
    const cv = el('canvas', { class: 'anr-entropy-map' });
    cv.width = prof.length;
    cv.height = 1; // 1px-tall strip, CSS-stretched; columns stay crisp
...
async function appendEmbeddedImagesCard(file, resultsEl, host) {
    ...
    card.appendChild(el('p', { class: 'anr-hint' }, 'Found ' + carved.length + ' image' + (carved.length === 1 ? '' : 's') + ' hidden in this file'
    ...
    for (let k = 0; k < carved.length; k++) {
        ...
        if (c.format === 'jpeg') {
            ...
            if (r && r.data)
                sub = r.data; // close off a cut-off carve
