...
function layerInfo(n) {
    if (LAYER_INFO[n])
        return LAYER_INFO[n];
    if (n >= 56 && n <= 88)
        return { name: 'Mechanical ' + (n - 56), color: MECH_COLORS[(n - 56) % MECH_COLORS.length] };
...
function parseFields(bytes) {
    ...
    for (const kv of txt.split('|')) {
        const i = kv.indexOf('=');
        if (i > 0)
            f[kv.slice(0, i).toUpperCase()] = kv.slice(i + 1);
    }
    return f;
}
    ...
    for (const { f, ord } of records) {
        ...
        switch (r) {
            ...
            case '6': { // line / polyline
                ...
                if (!pts.length)
                    pts.push([x, y], [num(f, 'CORNER.X'), num(f, 'CORNER.Y')]);
            ...
            case '27': { // wire
                ...
                if (pts.length >= 2)
                    objs.push({ kind: 'wire', pts, stroke: bgr(f.COLOR) || '#10559a' });
                break;
            }
            case '4':
            case '25':
            case '34':
            case '17': { // label / net label / designator / power port
                if (f.TEXT)
                    objs.push({ kind: 'text', x, y, text: f.TEXT, fill: bgr(f.COLOR) || '#20283a', power: r === '17' });
                if (r === '34') {
            ...
            case '41': { // parameter (mfr / mpn / datasheet / mouser / sheet field)
                if (!f.NAME || !f.TEXT || f.TEXT === '*')
                    break;
                const owner = f.OWNERINDEX != null ? compByOwner.get(num(f, 'OWNERINDEX', NaN)) : null;
                if (owner)
                    owner.params.push({ name: f.NAME, value: unesc(f.TEXT) });
                else if (f.ISHIDDEN !== 'T')
                    params.push({ name: f.NAME, value: unesc(f.TEXT) }); // document-level field
                break;
            }
            case '209': // text frame (notes)
                ...
                if (f.TEXT)
                    objs.push({ kind: 'text', x: x + 5, y: num(f, 'CORNER.Y') - 8, text: f.TEXT, fill: bgr(f.TEXTCOLOR) || '#20283a' });
                break;
        }
    }
...
function parsePcbStream(bytes, kind) {
    ...
    while (off + 4 <= bytes.length && guard++ < 200000) {
        const ln = dv.getUint32(off, true) >>> 0;
        if (ln === 0 || off + 4 + ln > bytes.length)
            break;
        const p = off + 4;
        const i32 = (k) => dv.getInt32(p + k, true);
        const layer = bytes[p];
        if (kind === 'line')
            prims.push({ kind: 'line', layer, x1: i32(13) / 1e4, y1: i32(17) / 1e4, x2: i32(21) / 1e4, y2: i32(25) / 1e4, w: i32(29) / 1e4 });
        else if (kind === 'arc')
            prims.push({ kind: 'arc', layer, cx: i32(13) / 1e4, cy: i32(17) / 1e4, r: i32(21) / 1e4, a1: dv.getFloat64(p + 25, true), a2: dv.getFloat64(p + 33, true), w: i32(41) / 1e4 });
        off += 4 + ln;
    }
    return prims;
}
...
// helpers live in the shared eda-viewer.js (identical to KiCad's). Altium adapts
// its numeric-id layer model to the viewer's normalised { id, name, color } chips
    ...
    const v = buildViewer((g) => {
        ...
        for (const o of parsed.objs) {
            if (o.kind === 'rect') {
                ...
                if (o.desig) {
                    const t = svg('text', { x: o.x2, y: Y(o.y2) - 1, 'font-size': 6, fill: '#0a5a8a' });
                    t.textContent = o.desig;
                    g.appendChild(t);
                }
                grow(b, o.x, Y(o.y));
                grow(b, o.x2, Y(o.y2));
                ...
                g.appendChild(t);
                grow(b, o.x, Y(o.y));
                grow(b, o.x + o.text.length * 4, Y(o.y) - 8);
            }
        }
        return safeBox(b);
    });
    ...
    return buildViewer((g) => {
        ...
        for (const p of prims) {
            ...
            if (p.kind === 'line') {
                ...
                n = arcPath(p, Y, col);
                grow(b, p.cx - p.r, Y(p.cy) - p.r);
                grow(b, p.cx + p.r, Y(p.cy) + p.r);
            }
            else if (p.kind === 'pad') {
                const x = p.x - p.sx / 2, y = p.y - p.sy / 2;
                if (p.shape === 1)
                    n = svg('ellipse', { cx: p.x, cy: Y(p.y), rx: p.sx / 2, ry: p.sy / 2, fill: col });
                else
                    n = svg('rect', { x, y: Y(y + p.sy), width: p.sx, height: p.sy, rx: p.shape === 3 ? Math.min(p.sx, p.sy) * 0.25 : 0, fill: col });
                if (n && p.hole > 0) {
                    n.setAttribute('fill-opacity', '0.85');
                }
                grow(b, x, Y(y));
                grow(b, x + p.sx, Y(y + p.sy));
                if (p.hole > 0) {
                    const hole = svg('circle', { cx: p.x, cy: Y(p.y), r: p.hole / 2, fill: '#0b0b0f', 'data-layer': p.layer });
                    g.appendChild(group1(n, p.layer));
                    g.appendChild(hole);
                    if (p.name)
                        g.appendChild(padLabel(p, Y));
                    continue;
                }
            }
            else if (p.kind === 'frect') {
                const x = Math.min(p.x1, p.x2), y = Math.min(p.y1, p.y2), w = Math.abs(p.x2 - p.x1), h = Math.abs(p.y2 - p.y1);
                n = svg('rect', { x, y: Y(y + h), width: w, height: h, fill: col });
                grow(b, x, Y(y));
                grow(b, x + w, Y(y + h));
            }
            if (n) {
                n.setAttribute('data-layer', p.layer);
                g.appendChild(n);
                if (p.kind === 'pad' && p.name)
                    g.appendChild(padLabel(p, Y));
            }
        }
        return safeBox(b);
...
function metaCard(title, helpText, rows, file, extra) {
    ...
    for (const [k, v, htxt] of rows)
        if (v != null && v !== '')
            tbl.appendChild(htxt ? rowHelp(k, String(v), htxt) : row(k, String(v)));
...
export async function renderAltium(file, resultsEl) {
    ...
    if (ext === 'epw') {
        await renderEpw(file, resultsEl);
        return;
    }
    if (ext === 'prjpcb' || ext === 'prjpcbstructure') {
        await renderPrjPcb(file, resultsEl);
        return;
    }
    if (/preview$/.test(ext)) {
        await renderPreview(file, resultsEl);
        return;
    }
    ...
    try {
        if (ext === 'schdoc' || ext === 'schlib') {
            ...
            if (reader.readStream('FileHeader') && reader.readStream((e) => /Board6\/Data$/i.test(e.path)))
                renderPcbDoc(file, reader, resultsEl);
            else if (reader.readStream((e) => /Board6\/Data$/i.test(e.path)) || reader.readStream((e) => /\/Data$/.test(e.path)))
                renderPcbLib(file, reader, resultsEl);
            else
                renderSch(file, reader, resultsEl, ext);
        }
...
function renderSch(file, reader, resultsEl, ext) {
    ...
    resultsEl.appendChild(metaCard(isLib ? 'Altium schematic library' : 'Altium schematic', 'An Altium Designer circuit diagram (schematic), stored inside an OLE compound file - a single file that holds many internal streams, like a mini filesystem. Analyser reads the drawing’s lines and shapes from its FileHeader stream and redraws them as a crisp, zoomable (vector) view in the browser.', [
    ...
    if (part) {
        ...
        if (part.desc)
            tbl.appendChild(row('Description', part.desc));
        for (const p of (part.params || [])) {
            if (/^https?:/i.test(p.value)) {
                ...
                tbl.appendChild(row(p.name, p.value));
        }
        card.appendChild(tbl);
        resultsEl.appendChild(card);
    }
...
function parsePcbLibData(reader) {
    ...
    for (const e of reader.entries) {
        ...
        if (!m || /^(Library|FileVersionInfo)$/i.test(m[1]))
            continue;
        if (e.size > best) {
            best = e.size;
            fpName = m[1];
    ...
    if (fpName) {
        const db = reader.readStream(fpName + '/Data');
        if (db)
            ({ prims, layers } = parseFootprintData(db));
    }
...
function renderPcbDoc(file, reader, resultsEl) {
    ...
    if (outline && outline.length >= 2) {
        ...
        if (!prims.length)
            dcard.appendChild(el('div', { class: 'anr-info' }, 'This board has no routed copper, components or polygons - only the board outline is defined.'));
    }
    else {
...
export async function buildAltiumProjectCard(altFiles, folderName) {
    ...
    for (const af of altFiles) {
        ...
        if (ext === 'prjpcb' || ext === 'prjpcbstructure') {
            ...
            epwF.push(af);
        else if (/preview$/.test(ext))
            prevF.push(af);
    }
    let prj = null;
    if (prjFile) {
        try {
            prj = parsePrj(await prjFile.file.text(), base(prjFile.path));
        }
        catch (_) { }
    }
    ...
    async function addDoc(af, kind) {
        try {
            ...
            if (!reader)
                return;
            if (kind === 'sch')
                docs.push({ kind: 'sch', name: base(af.path), file: af.file, parsed: parseSchematic(reader) });
            else if (kind === 'pcbdoc')
                docs.push({ kind: 'pcbdoc', name: base(af.path), file: af.file, data: parsePcbDocData(reader) });
            else if (kind === 'pcblib')
                docs.push({ kind: 'pcblib', name: base(af.path), file: af.file, data: parsePcbLibData(reader) });
        }
        catch (_) { }
    }
    for (const af of schF)
        await addDoc(af, 'sch');
    for (const af of pcbF)
        await addDoc(af, 'pcbdoc');
    for (const af of libF)
        await addDoc(af, 'pcblib');
    ...
    for (const d of schDocs) {
        for (const c of (d.parsed ? d.parsed.parts : []) || []) {
            if (c.libref || c.designator)
                bom.push({ comp: c, doc: d });
        }
    }
    ...
    card.appendChild(el('p', { class: 'anr-hint', style: 'margin:0 0 12px;' }, 'Every document in this folder, opened together as one design.'));
    ...
    function buildDocPanel(panel, d) {
        ...
        if (d.kind === 'pcbdoc') {
            ...
            if (outline && outline.length >= 2) {
                ...
                if (!prims.length)
                    panel.appendChild(el('div', { class: 'anr-info' }, 'Only the board outline is defined - no routed copper or placed components.'));
            }
            else
                panel.appendChild(el('div', { class: 'anr-info' }, 'No board outline or geometry could be decoded.'));
            return null;
        }
        // pcblib
        ...
        if (pads.length)
            panel.appendChild(padsTable(pads));
        if (prims.length)
            panel.appendChild(pcbView(prims, layers, null).wrap);
        else
            panel.appendChild(el('div', { class: 'anr-info' }, 'No footprint geometry could be decoded.'));
        return null;
    }
    function buildOverview(panel) {
        ...
        if (bom.length) {
            ...
            bomCard.appendChild(bh);
            bomCard.appendChild(bhp);
            ...
            for (const { comp, doc } of bom) {
                ...
                desigBtn.addEventListener('click', () => {
                    ...
                    if (v && v.focus && comp.designator)
                        v.focus(comp.designator);
        ...
        const roleOf = (n) => {
            ...
            if (e === 'epw')
                return 'SamacSys model';
            if (/preview$/.test(e))
                return 'Preview cache';
            return 'Document';
        };
        for (const af of altFiles)
            filesCard.appendChild(row(roleOf(base(af.path)), base(af.path)));
    }
    setActive(overviewIdx);
    return card;
}
//# sourceMappingURL=altium.js.map