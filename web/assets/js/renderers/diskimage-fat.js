...
export function looksLikeFatBoot(b, o) {
    if (o + 512 > b.length)
        return false;
    if (!(b[o] === 0xEB || b[o] === 0xE9))
        return false;
    const bps = u16(b, o + 0x0B);
    if (bps !== 512 && bps !== 1024 && bps !== 2048 && bps !== 4096)
        return false;
    const spc = b[o + 0x0D];
    if (![1, 2, 4, 8, 16, 32, 64, 128].includes(spc))
        return false;
    if (u16(b, o + 0x0E) === 0)
        return false; // reserved sectors must be >= 1
    return true;
}
...
function chain(img, fatStart, type, first, maxClusters, st) {
    ...
    while (cl >= 2 && !isEoc(type, cl) && out.length < maxClusters) {
        if (seen.has(cl))
            break; // loop in the FAT
        seen.add(cl);
        out.push(cl);
        const nxt = nextCluster(img, fatStart, type, cl, st);
        if (nxt < 2 || nxt === cl || nxt === 0x0FFFFFFF)
            break;
        cl = nxt;
    }
    return out;
...
function readDir(raw) {
    ...
    for (let p = 0; p + 32 <= raw.length; p += 32) {
        ...
        const attr = raw[p + 11];
        if ((attr & 0x3F) === 0x0F) {
            lfn = lfnPart(raw, p) + lfn;
            continue;
        } // LFN slot (reverse order)
        if (attr & 0x08) { // volume-label entry (also the FAT32 root label)
            if (!(attr & 0x10))
                volumeLabel = trimAscii(raw, p, 11);
            lfn = '';
            continue;
        }
...
export function parseFatVolume(img, partStart) {
    if (!looksLikeFatBoot(img, partStart))
        return null;
    ...
    if (isFat32) {
        fatSize = u32(img, partStart + 0x24);
        rootCluster = u32(img, partStart + 0x2C);
    }
    if (!fatSize || !numFats || !totalSectors)
        return null;
    ...
    function clustersToBytes(clusters) {
        ...
        for (const cl of clusters) {
            ...
            if (start < 0 || start >= img.length) {
                st.short = true;
                break;
            }
            if (start + bytesPerCluster > img.length)
                st.short = true;
            if (off + bytesPerCluster > out.length)
                break;
    ...
    if (isFat32) {
        ...
        if (rootStart + rootEntries * 32 > img.length)
            st.short = true;
    ...
    function walk(raw, path, depth) {
        ...
        for (const d of dirs) {
            if (d.startCl < 2 || visitedDirs.has(d.startCl))
                continue; // loop / bad-pointer guard
    ...
    if (!volumeLabel)
        volumeLabel = trimAscii(img, partStart + (isFat32 ? 0x47 : 0x2B), 11);
    ...
    for (let cl = 2; cl < lastCl; cl++) {
        if (nextCluster(img, fatStart, type, cl, stFree) === 0)
            freeClusters++;
        if (stFree.short) {
            st.short = true;
            break;
...
export function otherFsLabel(img, off) {
    const at = (o, s) => { for (let i = 0; i < s.length; i++)
        if (img[off + o + i] !== s.charCodeAt(i))
            return false; return true; };
    if (off + 0x8006 <= img.length && at(0x8001, 'CD001'))
        return 'ISO 9660 (CD/DVD)';
    if (at(0x03, 'NTFS'))
        return 'NTFS';
    if (at(0x03, 'EXFAT'))
        return 'exFAT';
    if (off + 0x43A <= img.length && img[off + 0x438] === 0x53 && img[off + 0x439] === 0xEF)
        return 'ext2/3/4';
    return null;
}
//# sourceMappingURL=diskimage-fat.js.map