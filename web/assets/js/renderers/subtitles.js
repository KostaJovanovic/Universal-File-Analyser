...
function parseSrtVtt(text, isVtt) {
    ...
    for (let block of blocks) {
        ...
        if (!lines.length)
            continue;
        if (isVtt && /^WEBVTT/.test(lines[0]))
            continue;
        if (/^(NOTE|STYLE|REGION)\b/.test(lines[0]))
            continue; // VTT metadata blocks
...
function parseAss(text) {
    ...
    const info = {}; // [Script Info] keys (PlayResX/Y, ScriptType, ...)
    ...
    for (const line of lines) {
        ...
        if (/^Format:/i.test(line)) {
            ...
            if (section.includes('style'))
                styleFmt = cols;
            else if (section.includes('event') || (/start/i.test(line) && /text/i.test(line)))
                dlgFmt = cols;
            continue;
        }
        ...
        if (/^Dialogue:/i.test(line)) {
            ...
            const mg = (v) => { const n = parseFloat(String(v)); return isFinite(n) && n > 0 ? n : null; };
            if (start != null)
                cues.push({ start, end, text: clean, raw: obj.text || '', style: (obj.style || '').trim(),
...
function assColor(v) {
    ...
    if (!m)
        return null;
    let hex = m[1];
    if (/^\d+$/.test(hex) && !/[a-f]/i.test(hex) && hex.length < 6)
...
const num = (v, dflt) => { const n = parseFloat(String(v)); return isFinite(n) ? n : dflt; };
const flag = (v) => /-?1/.test(String(v || '').trim());
...
function legacyToAn(a) {
    const col = (a & 3) || 2;
    if (a & 8)
        return 3 + col; // middle row -> 4,5,6
    if (a & 4)
        return 6 + col; // top row    -> 7,8,9
    return col; // bottom row -> 1,2,3
}
// Reduce a parsed Style row to everything the stage needs to draw it.
function assBaseStyle(s, legacy) {
...
function parseAssRuns(raw, base) {
    const segs = [];
    const layout = { pos: null, move: null, align: base.align, hasK: false };
    ...
    for (const part of raw.split(/(\{[^}]*\}|\\N|\\n|\\h)/)) {
        ...
        if (/^\{[^}]*\}$/.test(part)) {
            ...
            while ((m = TAG.exec(body))) {
                ...
                if (tag.charAt(0) === 'r') {
                    state.colour = base.primary;
                    state.bold = base.bold;
                    state.italic = base.italic;
                    state.underline = base.underline;
                    state.strike = base.strike;
                }
                else if (tag === 'b')
                    state.bold = arg !== '0' && arg !== '';
                else if (tag === 'i')
                    state.italic = arg === '1';
                else if (tag === 'u')
                    state.underline = arg === '1';
                else if (tag === 's')
                    state.strike = arg === '1';
                else if (tag === 'c' || tag === '1c') {
                    const c = assColor(arg);
                    if (c)
                        state.colour = c;
                }
                else if (tag === 'an') {
                    ...
                    const n = parseInt(arg, 10);
                    if (n > 0)
                        layout.align = legacyToAn(n);
                }
                else if (tag === 'pos') {
                    const p = /\(\s*(-?[\d.]+)\s*,\s*(-?[\d.]+)/.exec(arg);
                    if (p)
                        layout.pos = { x: parseFloat(p[1]), y: parseFloat(p[2]) };
                }
                else if (tag === 'move') {
                    ...
                    if (isFinite(cs)) {
                        ...
                        const type = (tag === 'kf' || rawTag === 'K') ? 'kf' : tag === 'ko' ? 'ko' : 'k';
        ...
        if (/^\\[Nn]$/.test(part)) {
            segs.push({ text: '', br: true, colour: state.colour, bold: state.bold, italic: state.italic, underline: state.underline, strike: state.strike, k: null });
            continue;
        }
        if (part === '\\h') {
            segs.push({ text: ' ', br: false, ...state, k: null });
            continue;
        }
...
function buildAssLine(segs, base) {
    ...
    for (const s of segs) {
        ...
        if (s.k) {
            span.style.color = base.secondary;
            kSpans.push({ node: span, k: s.k });
        }
...
function paintKaraoke(kSpans, base, t) {
...
function buildAssStage(cues, styleMap, styleDefs, info, legacy) {
    ...
    for (const c of cues) {
        ...
        const node = el('div', { class: 'anr-ass-cue' });
        node.style.fontSize = base.size + 'px';
        if (base.font)
            node.style.fontFamily = '"' + base.font.replace(/"/g, '') + '", sans-serif';
        ...
        if (base.borderStyle === 3) {
            ...
            node.style.webkitTextStrokeWidth = (base.outlineW * 2) + 'px';
            node.style.webkitTextStrokeColor = base.outline;
            node.style.paintOrder = 'stroke fill';
        }
        if (base.shadowW > 0)
            node.style.textShadow = base.shadowW + 'px ' + base.shadowW + 'px 0 ' + base.back;
    ...
    const place = (t) => {
        ...
        for (const s of staged) {
            ...
            if (s.kSpans.length)
                paintKaraoke(s.kSpans, s.base, t - s.start);
        }
    };
    // ---- transport ----
    const playBtn = el('button', { class: 'anr-btn anr-btn-sm', type: 'button' }, 'Play');
    const slider = el('input', { class: 'anr-range', type: 'range', min: '0', max: String(duration), step: '0.02', value: '0' });
    slider.style.flex = '1';
    ...
    const stop = () => { playing = false; last = 0; playBtn.textContent = 'Play'; if (raf)
...
export async function renderSubtitles(file, resultsEl) {
    ...
    try {
        ...
        resultsEl.appendChild(errorCard('Could not read this subtitle file.'));
        return;
    }
    const ext = (file.name.split('.').pop() || '').toLowerCase();
    let format, cues = [], styles = 0, fps = null, styleDefs = null;
    let assInfo = {}, assLegacy = false;
    if (ext === 'sub') {
        ...
        if (/^\{\d+\}\{\d+\}/m.test(text)) {
            ...
            tbl.appendChild(rowHelp('Note', binary ? 'VobSub stores subtitles as bitmaps' : 'No MicroDVD or SubViewer cues found', binary ? 'VobSub (.sub) stores its subtitles as ready-made pictures, paired with an .idx file that holds the timings - there is no actual text to read out. The matching .idx file is the readable half.' : 'The file did not match any of the text subtitle formats Analyser can read.'));
    ...
    if (fps != null)
        tbl.appendChild(rowHelp('Frame rate', fps.toFixed(3).replace(/\.?0+$/, '') + ' fps', 'MicroDVD subtitles are timed by frame number rather than by the clock, so a frame rate (frames per second) is needed to turn those into real times. Assumed to be 23.976 fps unless the file states its own.'));
    if (styles)
        tbl.appendChild(row('Styles', String(styles)));
    if (cues.length) {
        ...
        tbl.appendChild(rowHelp('On-screen time', fmtTime(covered), 'The total time that at least one subtitle line is showing, adding up how long every line stays on screen.'));
    ...
    if (styleDefs && styleDefs.length) {
        ...
        for (const s of styleDefs) {
            ...
            const colTd = el('td', {});
            colTd.appendChild(swatch);
            colTd.appendChild(document.createTextNode(s.primarycolour || '-'));
    ...
    if (styleDefs && styleDefs.length && cues.length) {
        try {
            ...
            const card = el('div', { class: 'anr-card' });
            const [sh, shelp] = h3help('Preview', 'The subtitles drawn where the file actually places them, at the resolution the author positioned against (' + stage.resX + ' x ' + stage.resY + '). Lines land at their own alignment, margins and \\pos coordinates, move along a \\move path, and karaoke lines fill in syllable by syllable as they are sung. There is no video behind them - only the subtitle layer.');
            card.appendChild(sh);
            card.appendChild(shelp);
            card.appendChild(stage.controls);
            card.appendChild(stage.wrap);
            if (stage.anyKaraoke) {
                card.appendChild(el('p', { class: 'anr-hint' }, 'This file carries karaoke timing: each syllable has its own duration, so the words change colour in time with the singing. Press play to see it.'));
            }
            card.appendChild(el('p', { class: 'anr-hint' }, 'Vector drawing commands (\\p), rotation and blur are not drawn - a line built from those shows as its text.'));
            resultsEl.appendChild(card);
        }
