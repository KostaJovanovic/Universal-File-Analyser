...
function parseRpp(text) {
    const lines = text.split(/\r?\n/);
    if (!/^\s*<REAPER_PROJECT\b/.test(lines[0] || ''))
        return null;
    let version = '', tempo = null, sig = null;
    const tracks = [];
    const media = [];
    ...
    for (const raw of lines) {
        ...
        if (line.startsWith('<')) {
            ...
            if (name === 'REAPER_PROJECT')
                ...
                cur = { name: '', kind: 'Track', clips: [], muted: false, devices: [] };
        ...
        if (key === 'TEMPO' && stack.length <= 1) {
            ...
            if (key === 'NAME')
                cur.name = tok[1] || '';
            else if (key === 'MUTESOLO')
                cur.muted = tok[1] === '1';
        }
        else if (item && inside === 'ITEM') {
            if (key === 'POSITION')
                item.start = parseFloat(tok[1]) || 0;
            else if (key === 'LENGTH')
                item.end = item.start + (parseFloat(tok[1]) || 0);
            else if (key === 'NAME')
                item.name = tok[1] || '';
        }
            ...
            media.push(tok[1]);
            if (item)
                item.name = item.name || (tok[1].split(/[\\/]/).pop() || '');
        }
    }
    if (cur)
        tracks.push(cur);
...
function buildTimeline(p) {
    ...
    for (const t of p.tracks) {
        ...
        for (const c of t.clips) {
            ...
            if (c.name)
                block.appendChild(el('span', { class: 'anr-daw-cliplabel' }, c.name));
    ...
    const zoom = el('input', { class: 'anr-range', type: 'range', min: '100', max: '1200', step: '10', value: '100' });
    zoom.style.flex = '1';
...
export async function renderDaw(file, resultsEl) {
    ...
    // ---- Info ----
    const [h, help] = h3help(p.app.includes('Reaper') ? 'Reaper project' : 'Ableton Live set', 'A project file contains no audio - it is the arrangement: which clip sits on which track at which moment, and the plugins that play it back. What follows is read from those same numbers.');
    ...
    if (p.tempo)
        tbl.appendChild(row('Tempo', (Math.round(p.tempo * 100) / 100) + ' BPM'));
    else
        tbl.appendChild(rowHelp('Tempo', 'not stated', 'Without a tempo, clip positions can only be given in beats - there is nothing to convert them to seconds with.'));
    if (p.sig)
        tbl.appendChild(row('Time signature', p.sig));
    tbl.appendChild(row('Tracks', String(p.tracks.length)));
    tbl.appendChild(rowHelp('Clips', clipTotal.toLocaleString() + (p.truncated ? ' (stopped counting here)' : ''), 'Every positioned clip in the arrangement. Session-view clips in a Live set are not counted: they have no position on a timeline.'));
    if (timeline.end)
        tbl.appendChild(row('Length', fmtClock(timeline.end) + (p.units === 'beats' ? ' (in beats)' : '')));
    if (p.media.length)
        tbl.appendChild(rowHelp('Media files referenced', String(p.media.length), 'A project points at its audio rather than containing it, so these are the files it needs alongside it to open complete.'));
    card.appendChild(tbl);
    resultsEl.appendChild(card);
    // ---- Timeline ----
    {
        const c = el('div', { class: 'anr-card' });
        const [th, thelp] = h3help('Arrangement', p.units === 'beats'
        ...
        if (timeline.controls)
            c.appendChild(timeline.controls);
        c.appendChild(timeline.wrap);
        if (p.truncated)
            c.appendChild(el('p', { class: 'anr-hint' }, 'Stopped after ' + DAW_CLIP_MAX.toLocaleString() + ' clips; the rest of the project was not read.'));
    ...
    if (p.media.length) {
        const c = el('div', { class: 'anr-card' });
        const [mh, mhelp] = h3help('Media files (' + p.media.length + ')', 'The recordings and samples this project plays. They are not inside the file - a project that has been moved without them opens with silent tracks, which is what this list is for checking.');
        c.appendChild(mh);
        c.appendChild(mhelp);
