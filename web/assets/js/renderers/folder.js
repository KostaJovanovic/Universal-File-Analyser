...
function extKnown(file) {
    ...
    if (typeof window !== 'undefined' && typeof window._anrClassify === 'function') {
        try {
            return window._anrClassify(file) !== 'unknown';
        }
        catch (_) { }
    }
...
async function sniffKnown(file) {
    try {
        ...
        if (headStr.trimStart().startsWith('<svg') || (headStr.includes('<svg') && headStr.includes('xmlns')))
            return true;
        if (/^\s*(<!doctype html|<html[\s>])/i.test(headStr))
            return true;
...
async function probeOpenable(file) {
    if (!file)
        return { ok: false, reason: 'No file data available', cloud: true };
    if (file.size === 0)
        return { ok: true }; // opens, just nothing to show
    ...
    if (HEIC_EXTS.has(ext)) {
        ...
        if (fast === true)
            return { ok: true };
        if (await decodes(file))
            return { ok: true }; // Safari decodes HEIC natively
    ...
    if (RAW_EXTS.has(ext)) {
        ...
        if (ext === 'x3f') {
            try {
                const p = await m.extractX3fPreview(file);
                if (await decodes(p))
                    return { ok: true };
            }
            catch (_) { }
        ...
        let prev = null;
        try {
            const j = await m.extractRawJpegs(file, { max: 1 });
            if (j && j[0])
                prev = j[0].blob;
        }
        catch (_) { }
        if (!prev) {
            try {
                prev = await m.extractRawPreview(file);
            }
            catch (_) { }
        }
        if (prev && await decodes(prev))
            return { ok: true };
    ...
    if (typeof window !== 'undefined' && window._anrResolveContent) {
        try {
            const r = await window._anrResolveContent(file);
            if (r && r.kind && r.kind !== 'unknown')
                return { ok: true };
        }
        catch (_) { }
    }
    else {
        if (await sniffKnown(file))
            return { ok: true };
        try {
            const { sniffGitObject } = await import('./gitobject.js');
            if (await sniffGitObject(file))
                return { ok: true };
        }
        catch (_) { }
    }
...
function classifyOf(file) {
    if (typeof window !== 'undefined' && typeof window._anrClassify === 'function') {
        try {
            return window._anrClassify(file) || '';
        }
        catch (_) { }
    }
    return '';
...
async function looksLikeText(file) {
    if (typeof window !== 'undefined' && window._anrReadableText) {
        try {
            return await window._anrReadableText(file);
        }
        catch (_) { }
    }
    try {
...
function renderScanReport(host, total, failures, cancelled, checked) {
    host.innerHTML = '';
    if (!failures.length) {
        host.appendChild(el('p', { class: 'anr-scan-ok' }, cancelled
            ? 'Checked ' + checked + ' of ' + total + ' files before stopping - all of them open.'
            : 'All ' + total + ' files open in Analyser.'));
        return;
    }
    host.appendChild(el('p', { class: 'anr-scan-bad' }, cancelled
    ...
    if (samplePaths.length) {
        const copyBtn = el('button', { type: 'button', class: 'anr-btn anr-scan-copy' }, 'Copy paths (' + samplePaths.length + ' format' + (samplePaths.length === 1 ? '' : 's') + ')');
...
async function walkTree(roots, opts = {}) {
    ...
    while (level.length && !shouldStop() && out.length < FOLDER_ENTRY_CAP) {
        ...
        const listings = await mapLimited(level, WALK_CONCURRENCY, async (d) => {
            try {
                return { d, children: await readEntries(d.entry.createReader()) };
            }
            catch (_) {
                return { d, children: [] };
            }
        ...
        for (const listing of listings) {
            ...
            for (const c of listing.children) {
                if (c.isDirectory)
                    nextLevel.push({ entry: c, path: base });
                else if (c.isFile)
                    fileEntries.push({ entry: c, path: base + c.name });
            }
        }
        ...
        for (let i = 0; i < fileEntries.length; i += WALK_CONCURRENCY * 8) {
            if (shouldStop())
                break;
            if (out.length >= FOLDER_ENTRY_CAP) {
                truncated = true;
                break;
            }
    ...
    if (level.length && out.length >= FOLDER_ENTRY_CAP)
        truncated = true;
...
export function renderFolder(files, resultsEl) {
    ...
    if (files.truncated) {
        resultsEl.appendChild(el('div', { class: 'anr-info' }, 'This folder holds more than ' + FOLDER_ENTRY_CAP.toLocaleString()
    ...
    const fileByPath = Object.create(null);
    for (const f of files)
        fileByPath[f.path] = f.file;
    ...
    if (files.length) {
        ...
        scanCard.appendChild(el('p', { class: 'anr-folder-scan-intro' }, 'Check every file and flag the ones Analyser can’t open or doesn’t recognise - unknown file types, cloud-only files that won’t download, undecodable HEIC, and RAW with no usable preview. Large RAW folders may load the full decoder, so this can take a while.'));
        ...
        scanBtn.addEventListener('click', async () => {
            ...
            for (const f of files) {
                ...
                let real = null;
                try {
                    real = await desktopFile(f.file);
                }
                catch (e) {
                    res = { ok: false, reason: 'Could not be read - access to the folder has expired. Open the folder again to re-check it.' };
                }
                if (real) {
                    try {
                        res = await probeOpenable(real);
                    }
                    catch (e) {
                        res = { ok: false, reason: 'Unexpected error: ' + ((e && e.message) || e) };
                    }
                }
                if (res && !res.ok)
                    failures.push({ path: f.path, size: f.size, reason: res.reason, cloud: !!res.cloud });
    ...
    if (contentsSlot)
        host.insertBefore(pendingCard, contentsSlot);
    else
    ...
    requestAnimationFrame(() => requestAnimationFrame(() => {
        ...
        for (const f of files) {
            ...
            for (let i = 0; i < parts.length - 1; i++) {
                if (!node[parts[i]] || typeof node[parts[i]] !== 'object' || node[parts[i]][LEAF])
