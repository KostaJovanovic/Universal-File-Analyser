...
const DEFAULT_VCTX = {
    ...
    afterPhoto: () => { const sec = document.getElementById('photo'); if (sec)
...
function applyVideoControls(playerEl, opts = {}) {
    if (isIOS()) {
        ...
        playerEl.addEventListener('click', () => { if (playerEl.paused)
            playerEl.play();
        else
            playerEl.pause(); });
    }
...
function stepLoader(text) {
    ...
    return {
        node,
        set(frac, t) { if (t)
            label.textContent = t; bar.set(frac); },
    };
}
...
function buildFrameControls(playerEl, getFps, file) {
    ...
    let editing = false;
    function refresh() { if (editing)
        return; const p = parts(playerEl.currentTime); display.textContent = `${pad(p.h)}:${pad(p.m)}:${pad(p.s)}:${pad(p.f)}`; }
    function enterEdit() {
        editing = true;
        playerEl.pause();
    ...
    function commit() {
        ...
        if (isFinite(playerEl.duration))
            t = Math.min(t, playerEl.duration);
        exitEdit();
        playerEl.currentTime = Math.max(0, t);
        refresh();
    }
    tc.addEventListener('click', (e) => { if (!editing && !editWrap.contains(e.target))
        enterEdit(); });
    tc.addEventListener('keydown', (e) => { if (!editing && (e.key === 'Enter' || e.key === ' ')) {
        e.preventDefault();
        enterEdit();
    } });
    editWrap.addEventListener('focusout', (e) => { if (!editWrap.contains(e.relatedTarget))
        commit(); });
    ...
    let raf = 0;
    function tick() { refresh(); if (!playerEl.paused)
    ...
    function stepFrame(delta) {
        ...
        if (isFinite(dur) && dur > 0)
            target = Math.min(target, dur - 0.5 / f);
    ...
    const analyseBtn = el('button', { type: 'button', class: 'anr-btn', onclick: async () => {
            ...
            analyseBtn.textContent = 'Capturing…';
            try {
                ...
                if (pr) {
                    renderPhoto(frameFile, pr, ctx.photoOpts(undefined));
                    ctx.afterPhoto();
                }
            }
            catch (_) { }
            analyseBtn.disabled = false;
            analyseBtn.textContent = 'Analyse frame';
        } }, 'Analyse frame');
...
function canPlayHevc() {
    if (_hevcPlayable !== null)
        return _hevcPlayable;
    let ok = false;
    try {
...
async function ffmpegReverseVideo(file, onLoad, onEnc, signal) {
    ...
    const report = (frac) => { const v = Math.max(lastP, Math.max(0, Math.min(1, frac))); lastP = v; if (onEnc)
        onEnc(v); };
    const phase = (base, span) => { pBase = base; pSpan = span; };
    const onProg = ({ progress }) => { if (isFinite(progress))
        report(pBase + Math.max(0, Math.min(1, progress)) * pSpan); };
    ff.on('log', onLog);
    ...
    try {
        ...
        for (let i = 0; i < segs.length; i++) {
            ...
            await rm(segs[i]);
            if (!ok) {
                for (const n of [...segs.slice(i + 1), ...revs, rev])
                    await rm(n);
                detachAll();
                if (crashed)
                    killFFmpeg();
        ...
        for (const n of [...revs, listName, out])
            await rm(n);
        if (data)
            report(1);
        detachAll();
...
async function ffmpegTranscodeToH264(file, onLoad, onEnc, signal, opts = {}) {
    ...
    const onProg = ({ progress }) => { if (onEnc && isFinite(progress))
        onEnc(Math.max(0, Math.min(1, progress))); };
    ff.on('progress', onProg);
    ...
    if (preset === 'turbo') {
        inOpts.push('-skip_loop_filter', 'all');
...
function buildReverseVideoCard(file, signal) {
    ...
    card.appendChild(el('p', { class: 'anr-hint' }, 'Re-encode this video playing backwards - picture and sound - in your browser with FFmpeg. This can take a while.'));
    ...
    btn.addEventListener('click', async () => {
        ...
        if (!blob) {
            btn.disabled = false;
            btn.textContent = '↺ Reverse video';
            out.innerHTML = '';
            out.appendChild(el('p', { class: 'anr-hint', style: 'color:var(--accent);' }, 'Could not reverse this video this time - a very high resolution (4K) or a low-memory device can run the browser out of memory mid-encode. The engine has been reset, so pressing Reverse again will retry from scratch; a shorter or lower-resolution clip is the most reliable.'));
            return;
        }
        ...
        analyseBtn.addEventListener('click', () => {
            ...
            if (window._anrPushNav)
                window._anrPushNav(file.name || 'video', () => { if (window._anrHandleFile)
                    window._anrHandleFile(file, {}); });
            if (window._anrHandleFile)
                window._anrHandleFile(revFile, { nested: true });
        });
...
async function extractRawParamSets(file, h265, signal) {
    ...
    while (i + 4 <= buf.length) {
        if (buf[i] === 0 && buf[i + 1] === 0 && buf[i + 2] === 1) {
            const nalStart = i + 3;
            let j = nalStart;
            while (j + 3 <= buf.length && !(buf[j] === 0 && buf[j + 1] === 0 && buf[j + 2] === 1))
                j++;
            const nalEnd = (j + 3 <= buf.length) ? j : buf.length;
            let end = nalEnd;
            while (end > nalStart && buf[end - 1] === 0)
                end--; // drop the next SC's leading zeros
            const t = nalTypeOf(buf[nalStart], h265);
            if (isParamNal(t, h265) && !seen.has(t)) {
                seen.add(t);
                sets.push({ t, payload: buf.slice(nalStart, end) });
            }
            i = nalEnd;
        }
        else
...
function videoTrackFromStream(info) {
    ...
    if (info.profile)
        v.profile = (hevc && info.tier) ? info.profile + ' (' + info.tier + ')' : info.profile;
    if (info.level)
        v.level = info.level.replace(/\.0$/, '');
    if (info.chroma)
        v.chroma = info.chroma;
    if (info.bitDepthLuma)
        v.bitDepth = Math.max(info.bitDepthLuma, info.bitDepthChroma || 0);
    if (info.fps) {
        v.fps = info.fps;
        v.fpsSource = info.fpsSource;
    }
    if (info.temporalLayers > 1)
        v.temporalLayers = info.temporalLayers;
    if (info.sarWidth && info.sarHeight && info.sarWidth !== info.sarHeight)
        v.pixelAspect = info.sarWidth + ':' + info.sarHeight;
    if (isSpecifiedColourCode(info.primaries))
        v.primaries = COLOUR_PRIMARIES[info.primaries] || ('code ' + info.primaries);
    if (isSpecifiedColourCode(info.transfer))
        v.transfer = TRANSFER_CHARS[info.transfer] || ('code ' + info.transfer);
    if (isSpecifiedColourCode(info.matrix))
        v.matrixName = MATRIX_COEFFS[info.matrix] || ('code ' + info.matrix);
    if (info.fullRange !== undefined)
        v.range = info.fullRange ? 'Full (PC)' : 'Limited (TV)';
    if (info.transfer === 16)
        v.hdr = 'PQ (' + (v.primaries || 'BT.2020') + ')';
    else if (info.transfer === 18)
        v.hdr = 'HLG (' + (v.primaries || 'BT.2020') + ')';
...
async function planRawSegments(file, h265, signal) {
    ...
    for (let k = 1; k < count; k++) {
        ...
        if (signal && signal.aborted)
            return null;
        if (idr != null && idr > boundaries[boundaries.length - 1] + 4096)
            boundaries.push(idr);
    }
    boundaries.push(file.size);
    if (boundaries.length < 3)
        return null; // couldn't actually split it
    ...
    let info = null;
    try {
        info = parseAnnexBStreamInfo(paramSets, h265);
    }
    catch (_) { }
...
function buildRawSceneCard(playerEl, signal) {
    const card = el('div', { class: 'anr-card' });
    const [scH, scHelp] = h3help('Scene changes', 'Scans only the part currently loaded in the player. It scrubs through the video, so it can see just the segment that is loaded.');
    card.appendChild(scH);
    card.appendChild(scHelp);
    const out = el('div');
    ...
    runBtn.addEventListener('click', async () => {
        ...
        try {
            changes = await detectSceneChanges(playerEl, 55, signal, contentSamples, (f) => prog.set(f, 'Scanning for scene changes… ' + Math.round(f * 100) + '%'));
        }
        catch (_) { }
        try {
        ...
        if (signal && signal.aborted)
            return;
        out.innerHTML = '';
        out.appendChild(el('p', { class: 'anr-hint', style: 'margin-bottom:10px;' }, changes.length ? changes.length + ' scene change' + (changes.length > 1 ? 's' : '') + ' detected in this part' : 'No scene changes detected in this part'));
        ...
        try {
            ...
            if (ctCard) {
                ctCard.appendChild(el('p', { class: 'anr-hint', style: 'margin:10px 0 0;' }, 'Covers only the part currently loaded in the player.'));
                card._anrCt = ctCard;
                card.after(ctCard);
            }
        }
...
async function renderSegmentedRawVideo(file, header, resultsEl, kind, signal) {
    ...
    resultsEl.innerHTML = '';
    resultsEl.appendChild(el('div', { class: 'anr-info' }, 'Large raw ' + kind + ' stream (' + fmtBytes(file.size) + ') - scanning for keyframes to split it into playable parts…'));
    ...
    const brRow = rowHelp('Bitrate (total)', 'measuring…', 'How much data the file uses per second of playback. A raw stream carries no length, so this is measured from the first part that plays - its byte range divided by its running time - and applied across the whole file.');
    tbl.appendChild(brRow);
    // Bit depth, chroma, colour, HDR and the parameter sets, straight from the SPS.
    try {
        appendTrackRows(tbl, { video: videoTrackFromStream(info) });
    }
    catch (_) { }
    tbl.appendChild(row('Parts', N + ' × ~' + fmtBytes(Math.round(file.size / N)) + ', split at keyframes'));
    infoCard.appendChild(tbl);
    infoCard.appendChild(el('p', { class: 'anr-hint' }, 'Too big to convert in one piece, so it’s split at keyframes into ' + N + ' parts, each remuxed to MP4 on demand and '
        + 'played back-to-back.'));
    resultsEl.appendChild(infoCard);
    // Integrity: hashing a multi-GB file reads the whole thing, so keep it on-demand.
    const hashCard = el('div', { class: 'anr-card' });
    const [hashH, hashHelp] = h3help('Integrity', '<strong>SHA-256</strong> is a cryptographic fingerprint of the file’s exact contents. Computing it reads the whole file (' + fmtBytes(file.size) + '), so for a video this size it is left to a button rather than run automatically.');
    hashCard.appendChild(hashH);
    hashCard.appendChild(hashHelp);
    ...
    async function ensureSegment(i, loaderLabel) {
        ...
        for (const key of [...cache.keys()]) {
            if (Math.abs(key - i) > 1) {
                try {
                    URL.revokeObjectURL(cache.get(key).url);
                }
                catch (_) { }
                cache.delete(key);
            }
    ...
    async function goTo(i, autoplay) {
        ...
        let entry = null;
        try {
            entry = await ensureSegment(i, 'Preparing part ' + (i + 1) + ' / ' + N + '…');
        }
        catch (_) {
            entry = null;
        }
        if (myGen !== gen || signal.aborted)
            return;
        if (!entry) {
            status.textContent = 'Part ' + (i + 1) + ' couldn’t be prepared.';
            return;
        }
        ...
        if (autoplay)
            playerEl.play().catch(() => { });
        if (i + 1 < N)
            ensureSegment(i + 1).catch(() => { }); // prefetch the next part
    }
    playerEl.addEventListener('ended', () => { if (cur + 1 < N)
        goTo(cur + 1, true); });
...
function formatDuration(sec) {
    ...
    const s = sec % 60;
    if (h > 0)
        return h + ':' + String(m).padStart(2, '0') + ':' + s.toFixed(0).padStart(2, '0');
...
function findSampleEntryBox(view, entryStart, entryEnd, type, isAudio) {
    ...
    for (let p = entryStart + 12; p + 8 <= entryEnd; p++) {
        if (view.getUint8(p) !== want[0] || view.getUint8(p + 1) !== want[1] ||
            view.getUint8(p + 2) !== want[2] || view.getUint8(p + 3) !== want[3])
            continue;
        const size = view.getUint32(p - 4);
        if (size >= 8 && p - 4 + size <= entryEnd)
            return { type, offset: p - 4, size, headerSize: 8 };
    }
    return null;
}
...
function extractPcmFromMp4(arrayBuffer) {
    ...
    for (const trak of traks) {
        ...
        if (stsdData + 8 > fileEnd)
            continue;
        const codecFcc = String.fromCharCode(view.getUint8(stsdData + 4), view.getUint8(stsdData + 5), view.getUint8(stsdData + 6), view.getUint8(stsdData + 7));
        const pcmCodecs = new Set(['twos', 'sowt', 'lpcm', 'in16', 'in24', 'in32', 'raw ', 'NONE', 'ulaw', 'alaw']);
        if (!pcmCodecs.has(codecFcc))
            continue;
        ...
        if (!stcoBoxes.length && !co64Boxes.length)
            continue;
        const chunkOffsets = [];
        if (stcoBoxes.length) {
        ...
        if (stscBoxes.length) {
            ...
            const count = view.getUint32(d + 4);
            if (count > 0)
                samplesPerChunk = view.getUint32(d + 8 + 4);
        }
        if (stszBoxes.length) {
        ...
        for (let ch = 0; ch < channels; ch++)
            chans.push(audioBuf.getChannelData(ch));
...
function audioBufferToWavUrl(audioBuf) {
    ...
    o += 2;
    view.setUint16(o, channels, true);
    o += 2;
    view.setUint32(o, sr, true);
    o += 4;
    view.setUint32(o, sr * block, true);
    o += 4;
    view.setUint16(o, block, true);
    o += 2;
    ...
    for (let c = 0; c < channels; c++)
        ch.push(audioBuf.getChannelData(c));
...
async function sniffMp4AudioCodec(file) {
    ...
    async function findMoov() {
        ...
        for (let guard = 0; guard < 4096 && off + 8 <= file.size; guard++) {
            ...
            if (size === 1) { // 64-bit size
                size = Number((BigInt(u32(h, 8)) << 32n) | BigInt(u32(h, 12)));
                hdr = 16;
            }
            if (size < hdr)
                return null;
    ...
    try {
        ...
        for (const trak of findAllBoxes(view, 0, moovBuf.byteLength, 'trak')) {
            ...
            if (!hdlr)
                continue;
            const handler = String.fromCharCode(view.getUint8(hdlr.offset + hdlr.headerSize + 8), view.getUint8(hdlr.offset + hdlr.headerSize + 9), view.getUint8(hdlr.offset + hdlr.headerSize + 10), view.getUint8(hdlr.offset + hdlr.headerSize + 11));
            if (handler !== 'soun')
                continue;
...
async function attachPcmAudioCompanion(file, playerCard, signal) {
    ...
    try {
        ...
        if (!BROWSER_UNPLAYABLE_AUDIO.has(codec))
            return; // browser plays it natively
        if (signal && signal.aborted)
            return;
        ...
        if (!audioBuf) {
            try {
                audioBuf = await ffmpegExtractAudio(file, playerCard);
            }
            catch (_) { }
        }
        if (!audioBuf || (signal && signal.aborted))
            return;
...
async function readMatroskaApps(file) {
    ...
    const out = {};
    if (mkv.writingApp)
        out.writingApp = mkv.writingApp;
    if (mkv.muxingApp)
        out.muxingApp = mkv.muxingApp;
    if (mkv.title)
        out.title = mkv.title;
    if (mkv.dateUtc)
        out.created = mkv.dateUtc;
    return out;
}
...
async function readMp4Encoder(file) {
    const find = (b) => {
        for (let i = 0; i + 24 < b.length; i++) {
            if (b[i] !== 0xA9 || b[i + 1] !== 0x74 || b[i + 2] !== 0x6F || b[i + 3] !== 0x6F)
                continue; // '©too'
            if (!(b[i + 8] === 0x64 && b[i + 9] === 0x61 && b[i + 10] === 0x74 && b[i + 11] === 0x61))
                continue; // 'data'
    ...
    if (!s && file.size > headN)
        s = find(new Uint8Array(await file.slice(Math.max(0, file.size - 4 * 1024 * 1024)).arrayBuffer()));
    return s ? { software: s } : {};
}
async function readContainerSoftware(file, container) {
    try {
        if (/Matroska|WebM/i.test(container || ''))
            return await readMatroskaApps(file);
        if (container === 'AVI')
            return await readAviSoftware(file);
        if (/MP4|MOV|M4V|QuickTime|3GP|3G2/i.test(container || ''))
            return await readMp4Encoder(file);
    }
    catch (_) { /* best-effort */ }
    return {};
}
...
function appendCreatorRows(tbl, header) {
    ...
    if (created)
        tbl.appendChild(rowHelp('Created with', created, 'The app or software that created this file, as noted inside the file itself (the Matroska WritingApp or AVI ISFT field).'));
    if (header.muxingApp && header.muxingApp !== header.writingApp)
        tbl.appendChild(rowHelp('Muxer', header.muxingApp, 'The tool that packaged (multiplexed) the separate video and audio streams together into this container file. It can differ from the app that created the content - for example, an edit finished in one program but wrapped into the file by another.'));
    if (header.title)
        tbl.appendChild(rowHelp('Title', header.title, 'A name stored inside the file itself, separate from the filename. Players show it instead of the filename when it is set.'));
    if (header.created)
        tbl.appendChild(rowHelp('Created', fmtDate(header.created), 'When the file was packaged, as recorded inside the file. This is the muxing date, not necessarily when the video was filmed, and it survives copying and renaming - unlike the date the operating system shows.'));
}
...
async function detectFpsFromContainer(file) {
    ...
    if (moovOffset < 0 || moovSize > 20 * 1024 * 1024)
        return null;
    ...
    for (const trak of traks) {
        ...
        if (!findAllBoxes(view, trakStart, trakEnd, 'vmhd').length)
            continue;
        ...
        if (sttsData + 16 > moovSize)
            continue;
        if (view.getUint32(sttsData + 4) < 1)
            continue;
        const sampleDuration = view.getUint32(sttsData + 12);
        if (sampleDuration <= 0 || timescale <= 0)
            continue;
    ...
    for (const trak of traks) {
        ...
        if (isVideo && !result.video) {
            ...
            if (cfg) {
                ...
                if (cfg.level)
                    v.level = cfg.level.replace(/\.0$/, '');
                if (cfg.chroma)
                    v.chroma = cfg.chroma;
                if (cfg.bitDepthLuma)
                    v.bitDepth = Math.max(cfg.bitDepthLuma, cfg.bitDepthChroma || 0);
                if (cfg.fps) {
                    v.fps = cfg.fps;
                    v.fpsSource = cfg.fpsSource;
                }
                if (cfg.temporalLayers > 1)
                    v.temporalLayers = cfg.temporalLayers;
                ...
                if (codecFcc === 'hev1' || codecFcc === 'hvc1')
                    v.paramSets = codecFcc === 'hvc1' ? 'out-of-band (hvc1)' : 'in-band (hev1)';
            }
            ...
            try {
                ...
                if (btrt) {
                    ...
                    if (d + 12 <= moovSize) {
                        ...
                        if (avgBitrate > 0 && avgBitrate < 4e9)
                            v.avgBitrate = avgBitrate;
                        if (maxBitrate > 0 && maxBitrate < 4e9)
                            v.maxBitrate = maxBitrate;
                    }
                }
            }
            catch (_) { }
            ...
            try {
                ...
                if (colr) {
                    ...
                    if ((colourType === 'nclx' || colourType === 'nclc') && d + 10 <= moovSize) {
                        ...
                        if (colourType === 'nclx' && d + 11 <= moovSize)
                            fullRange = !!(view.getUint8(d + 10) & 0x80);
                    }
                }
                if (cfg) {
                ...
                if (isSpecifiedColourCode(primaries))
                    v.primaries = COLOUR_PRIMARIES[primaries] || ('code ' + primaries);
                if (isSpecifiedColourCode(transfer))
                    v.transfer = TRANSFER_CHARS[transfer] || ('code ' + transfer);
                if (isSpecifiedColourCode(matrix))
                    v.matrixName = MATRIX_COEFFS[matrix] || ('code ' + matrix);
                if (fullRange !== undefined)
                    v.range = fullRange ? 'Full (PC)' : 'Limited (TV)';
                ...
                if (clli) {
                    ...
                    if (d + 4 <= moovSize) {
                        v.maxCll = view.getUint16(d);
                        v.maxFall = view.getUint16(d + 2);
                    }
                }
            }
            catch (_) { }
    ...
    try {
        ...
        if (mvhd) {
            ...
            if (timescale > 0 && duration > 0)
                result.durationSec = duration / timescale;
        }
    }
    catch (_) { }
...
function appendTrackRows(tbl, tracks) {
    ...
    if (v) {
        if (v.codecName) {
            ...
            const extra = [];
            if (v.profile)
                extra.push(v.profile);
            if (v.level)
                extra.push('L' + v.level);
            if (extra.length)
                label += '  (' + extra.join(', ') + ')';
            tbl.appendChild(rowHelp('Video codec', label, 'The recipe used to squeeze the picture down to a manageable size, such as H.264, and where available its profile and level. Read from the file’s codec settings (the MP4/MOV sample-description and codec-config boxes, or the Matroska CodecPrivate), the profile and level show which encoding features and quality ceiling were used.'));
        }
        if (v.avgBitrate || v.maxBitrate) {
            const parts = [];
            if (v.avgBitrate)
                parts.push(fmtBitrate(v.avgBitrate) + ' average');
            if (v.maxBitrate && v.maxBitrate !== v.avgBitrate)
                parts.push(fmtBitrate(v.maxBitrate) + ' peak');
            tbl.appendChild(rowHelp('Bitrate (video track)', parts.join('  ·  '), 'How much data the picture alone uses per second, as the encoder itself recorded it in the file. This is the video track on its own, so it is lower than the whole-file bitrate, which also counts the sound and the packaging.'));
        }
        if (v.bitDepth) {
            let depthText = v.bitDepth + '-bit';
            if (v.chroma)
                depthText += '  ·  ' + v.chroma + ' chroma';
            tbl.appendChild(rowHelp('Bit depth', depthText, 'How many shades of colour each pixel can hold, plus how much colour detail is kept (the chroma subsampling), read from the codec settings. 8-bit is standard; 10-bit (e.g. Sony XAVC HS, HLG/HDR) stores smoother gradients. 4:2:0 is normal for delivery, 4:2:2 keeps more colour detail for editing. Browsers have no built-in player for 10-bit 4:2:2, so those files can be identified here but not played.'));
        }
        if (v.pixelAspect) {
            tbl.appendChild(rowHelp('Pixel aspect', v.pixelAspect, 'The shape of each stored pixel. Normally pixels are square (1:1); anything else means the picture is squeezed in the file and stretched back out on playback, so the stored width and height are not the shape you see.'));
        }
        if (v.rotation) {
            const orient = (v.rotation === 90 || v.rotation === 270) ? 'portrait' : 'landscape';
            tbl.appendChild(rowHelp('Rotation', v.rotation + '°  (' + orient + ')', 'A note stored inside the file (in the track header transform matrix) telling players to turn the picture the right way up. Phones film in the sensor’s native orientation and add this flag so the video plays upright.'));
        }
        if (v.hdr) {
            let hdrText = v.hdr;
            if (v.mdcv || v.clli)
                hdrText += '  · ' + [v.mdcv ? 'mastering display' : '', v.clli ? 'content-light' : ''].filter(Boolean).join(' + ') + ' metadata';
            if (v.maxCll)
                hdrText += '  · MaxCLL ' + v.maxCll + ' nits';
            if (v.maxFall)
                hdrText += ', MaxFALL ' + v.maxFall + ' nits';
            tbl.appendChild(rowHelp('HDR', hdrText, 'A flag marking the video as HDR (High Dynamic Range), which allows brighter highlights and a wider range of colour than normal video. The type is PQ (HDR10/Dolby Vision) or HLG, usually with the wide BT.2020 colour range; mastering-display and content-light data describe the HDR grade in more detail. MaxCLL is the brightest single pixel in the whole video and MaxFALL the brightest average frame, both in nits.'));
        }
        if (v.dvProfile != null) {
            tbl.appendChild(rowHelp('Dolby Vision', 'Profile ' + v.dvProfile + ', level ' + v.dvLevel
                + (v.dvBlCompatible ? '  ·  plays on non-Dolby screens' : '  ·  needs a Dolby Vision display'), 'Dolby Vision adds scene-by-scene brightness and colour instructions on top of the picture. The profile says how it is packaged; a base-layer-compatible profile still looks correct on an ordinary HDR or SDR screen, while the others need Dolby Vision support to show properly.'));
        }
        if (v.primaries || v.transfer || v.matrixName || v.range) {
            const bits = [];
            if (v.primaries)
                bits.push(v.primaries);
            if (v.transfer)
                bits.push(v.transfer);
            if (v.matrixName)
                bits.push(v.matrixName);
            if (v.range)
                bits.push(v.range + ' range');
            tbl.appendChild(rowHelp(v.hdr ? 'Colour details' : 'Colour', bits.join(' · '), 'Instructions that tell a player how to turn the stored numbers into the colours you see on screen: the colour primaries, the gamma (transfer), the maths used to split colour from brightness, and whether the values use the full 0-255 range or the narrower broadcast range. BT.709 is standard HD; BT.2020 is the wider range used for UHD. Getting these wrong is what makes a video look washed out or over-contrasty.'));
        }
        if (v.temporalLayers > 1) {
            tbl.appendChild(rowHelp('Temporal layers', String(v.temporalLayers), 'The video is encoded in nested layers so a player can drop frames and show it at a lower frame rate without re-encoding - used for streaming and for slow-motion recordings that must also play at normal speed.'));
        }
        if (v.paramSets) {
            tbl.appendChild(rowHelp('Parameter sets', v.paramSets, 'Where the decoder settings that describe the picture are kept. Out-of-band means the file header holds them, so a chunk cut out of the middle will not decode on its own; in-band means they repeat through the stream, so any part can be decoded standalone. A raw stream has no header, so it always carries them inline.'));
        }
        if (v.partialParse) {
            tbl.appendChild(rowHelp('Note', 'The stream header parsed only partially - some details may be missing.', 'Analyser reads the picture settings straight out of the video stream. This one did not follow the expected layout all the way through, so what is shown is what was read before that point; the rest is left out rather than guessed at.'));
        }
    }
    if (a && a.codecName) {
        ...
        const bits = [];
        if (a.channels)
            bits.push(a.channels === 1 ? 'mono' : a.channels === 2 ? 'stereo' : a.channels + 'ch');
        if (a.sampleRate)
            bits.push((a.sampleRate / 1000).toFixed(a.sampleRate % 1000 ? 1 : 0) + ' kHz');
        if (a.bitDepth)
            bits.push(a.bitDepth + '-bit');
        if (a.avgBitrate)
            bits.push(fmtBitrate(a.avgBitrate));
        if (bits.length)
            label += '  (' + bits.join(', ') + ')';
        tbl.appendChild(rowHelp('Audio codec', label, 'The recipe used to compress the sound, such as AAC, with its channel count, sample rate and - where the file records them - bit depth and bitrate. Read from the file’s own audio settings.'));
    }
    ...
    if (Array.isArray(tracks.tracks) && tracks.tracks.length > 2) {
        const lines = tracks.tracks.map((t) => {
            ...
            if (t.width && t.height)
                bits.push(t.width + '×' + t.height);
            if (t.channels)
                bits.push(t.channels + 'ch');
            if (t.language && t.language !== 'und')
                bits.push(t.language);
            if (t.name)
                bits.push('“' + t.name + '”');
            if (t.isDefault)
                bits.push('default');
        ...
        const trackRow = rowHelp('Tracks (' + tracks.tracks.length + ')', '-', 'Every stream packaged inside this file. Matroska files often hold more than one soundtrack (different languages or mixes) and several subtitle tracks alongside the picture.');
        const cell = trackRow.lastChild;
        cell.textContent = '';
        for (const line of lines)
            cell.appendChild(el('div', {}, line));
...
function clearVideoPreviewBoot() {
    ...
    if (pv && pv.querySelector('.anr-inline-loader'))
        pv.innerHTML = '';
}
async function renderMoovlessRecovery(file, header, det, resultsEl, signal) {
    ...
    tbl.appendChild(rowHelp('Index (moov atom)', 'missing', 'MP4/MOV files keep a table of contents - the "moov" atom - that every player needs to locate the frames. Cameras write it last, so a recording that was cut off or a copy that did not finish leaves it out. The video itself is still inside the file; it just has no table of contents, which is why no player will open it.'));
    if (det.truncated) {
        const expect = det.declaredMdatEnd - det.mdatStart;
        tbl.appendChild(rowHelp('Media data', 'truncated - ' + fmtBytes(det.missingBytes) + ' short of the ' + fmtBytes(expect) + ' the header expects', 'The file says it should hold more video data (in its mdat box) than it actually does, so the recording or copy stopped early. Everything up to the cut-off point can still be salvaged.'));
    }
    diag.appendChild(tbl);
    diag.appendChild(el('p', { class: 'anr-hint' }, 'Analyser can scan the leftover data, gather every video frame it can decode and stitch them into a playable clip. Audio can’t be recovered without the index.'));
    ...
    let inband = null;
    try {
        inband = await findInbandParamSets(reader, det.mdatStart, det.mdatEnd, { codec, scanBytes: 128 * 1024 * 1024 });
    }
    catch (_) { }
    if (signal.aborted)
        return;
    ...
    // Reference-clip path.
    {
        const [refH, refHelp] = h3help('Reference clip needed', 'This recording stored its codec setup (the SPS/PPS) only inside the missing index, so Analyser has to borrow that setup from a healthy clip before it can rebuild the picture. Everything stays on your device.');
        action.appendChild(refH);
        action.appendChild(refHelp);
    }
    action.appendChild(el('p', { class: 'anr-hint' }, 'Choose a healthy, complete clip shot on the same camera in the same mode - matching resolution and codec (for example another clip from the same memory card).'));
    ...
    inp.addEventListener('change', async () => {
        ...
        let rp = null;
        try {
            rp = await extractMp4ParamSets(fileRangeReader(ref), ref.size);
        }
        catch (_) { }
        if (!rp || !rp.paramSets) {
...
async function renderUnplayableVideoInfo(file, header, resultsEl, signal, rawInfo) {
    ...
    if (declaredFps) {
        tbl.appendChild(rowHelp('Frame rate', roundFps(declaredFps) + ' fps' + (fpsFrom ? '   (' + fpsFrom + ')' : ''), 'How many pictures per second the file is meant to play at, read from the video’s own settings rather than measured by playing it. Where the file states it in more than one place the source is named, since those can disagree - a mismatch is itself a tell that the file was re-wrapped or re-timed.'));
    }
    if (dispW && dispH) {
    ...
    try {
        await appendTelemetryCards(file, resultsEl);
    }
    catch (_) { }
    let unplayableAdvCard = null;
    try {
        const adv = await buildVideoAdvancedCard(file);
        if (adv && !(signal && signal.aborted))
            unplayableAdvCard = adv; // appended last, below every other card
    }
    catch (_) { }
    ...
    const convSettings = el('div', { class: 'anr-conv-settings', hidden: '' }, [
        ...
        el('p', { class: 'anr-hint', style: 'margin:8px 0 0; flex-basis:100%;' }, 'Lower resolution and frame rate convert much faster. "Speed" trades encode time for a slightly smaller/cleaner file.'),
    ]);
    ...
    convBtn.addEventListener('click', async () => {
        ...
        try {
            ...
            blob = await queueFFmpeg(() => {
                convBtn.textContent = 'Converting…';
                return ffmpegTranscodeToH264(file, (p) => { convLabel.textContent = 'loading ffmpeg'; convSetBar(p); }, (p) => { convLabel.textContent = 'converting'; convSetBar(p); }, signal, convOpts);
        ...
        if (!blob) {
            ...
            convBtn.textContent = 'Convert to H.264 and play';
            convErr.appendChild(el('p', { class: 'anr-hint', style: 'color:var(--accent);' }, 'Could not convert this video - it may be corrupt, or too large to hold in memory.'));
            return;
        }
        const base = (file.name || 'video').replace(/\.[^/.]+$/, '');
        ...
        if (ctx.inline) {
            ...
            [convBtn.closest('.anr-btn-row'), convSettings, convWrap, convErr].forEach((n) => { if (n && n.parentElement)
                n.remove(); });
...
async function detectSceneChanges(video, threshold, signal, collect, onProgress) {
    ...
    for (let i = 0; i <= sampleCount; i++) {
        ...
        if (collect)
            collect.push({ time: t, r: ar, g: ag, b: ab, luma, diff: prevData ? meanDiff : 0 });
        prevData = frame;
    }
    return changes;
}
...
function buildContentTimelineCard(samples, dur, playerEl) {
    if (!samples || samples.length < 2 || !isFinite(dur) || dur <= 0)
        return null;
    ...
    const groupRuns = (test) => {
        ...
        for (let i = 0; i < n; i++) {
            if (test(i)) {
                if (!run)
                    run = { from: samples[i].time, to: samples[i].time, count: 0 };
                run.to = samples[i].time;
                run.count++;
            }
            else if (run) {
                segs.push(run);
    ...
    const card = el('div', { class: 'anr-card' });
    const [h, help] = h3help('Content timeline', 'Uses the same sampled frames as scene detection. The barcode squeezes each frame down to a single stripe of its average colour, so the whole video reads as a colour-over-time fingerprint. The curve below plots average brightness, flagging near-black frames and frozen or still stretches. Click the barcode to jump to that moment.');
    card.appendChild(h);
    card.appendChild(help);
    card.appendChild(el('p', { class: 'anr-hint', style: 'margin:0 0 10px;' }, n + ' frames sampled across ' + formatDuration(dur) + '.'));
    ...
    if (bctx) {
        ...
        for (let i = 0; i < n; i++) {
            const o = i * 4;
            img.data[o] = samples[i].r;
            img.data[o + 1] = samples[i].g;
            img.data[o + 2] = samples[i].b;
            img.data[o + 3] = 255;
        }
    ...
    tbl.appendChild(rowHelp('Brightness (mean)', pct(meanL) + '%  (luma ' + meanL.toFixed(0) + '/255)', 'The average brightness of the sampled frames (measured as luma, the Rec. 709 standard). A very low value suggests the video is dark or underexposed overall.'));
    ...
    tbl.appendChild(rowHelp('Black frames', blackSegs.length
        ...
        : 'none', 'Sampled frames that are almost completely dark (brightness under ' + BLACK_LUMA + ' out of 255) - usually fades to black, cuts between shots, or black at the start or end.'));
    if (blackSegs.length)
        tbl.appendChild(row('', segList(blackSegs, dur)));
    tbl.appendChild(rowHelp('Freeze / still', freezeSegs.length
        ? freezeSegs.length + ' segment' + (freezeSegs.length > 1 ? 's' : '')
        : 'none', 'Stretches where the picture barely changes from one sampled frame to the next - a frozen frame, a held title card, or a motionless shot. Only stretches longer than the sampling gap show up.'));
    if (freezeSegs.length)
        tbl.appendChild(row('', segList(freezeSegs, dur)));
...
async function renderVisibleVideoFallback(file, url, header, resultsEl, signal) {
    ...
    if (vw && vh)
        tbl.appendChild(rowHelp('Frame size', ((vw * vh) / 1_000_000).toFixed(2) + ' MP', 'How many pixels make up each frame, in megapixels (width × height ÷ 1,000,000). A rough guide to how much detail each frame holds before compression.'));
    ...
    if (exif) {
        const metaRows = [];
        if (exif.Make)
            metaRows.push(['Make', exif.Make]);
        if (exif.Model)
            metaRows.push(['Model', exif.Model]);
        if (exif.Software)
            metaRows.push(['Software', exif.Software]);
        if (exif.DateTimeOriginal)
            metaRows.push(['Taken', new Date(exif.DateTimeOriginal).toISOString().replace('T', ' ').slice(0, 19)]);
        if (exif.CreateDate)
            metaRows.push(['Created', new Date(exif.CreateDate).toISOString().replace('T', ' ').slice(0, 19)]);
        if (metaRows.length) {
            const mc = el('div', { class: 'anr-card' });
            mc.appendChild(el('h3', {}, 'Metadata'));
            const mt = el('table', { class: 'anr-readout' });
            for (const [k, v] of metaRows)
                mt.appendChild(row(k, v));
            mc.appendChild(mt);
    ...
    if (vw && vh && isFinite(dur) && dur > 0) {
        ...
        sheetBtn.addEventListener('click', async () => {
            ...
            for (let i = 0; i < total; i++) {
                ...
                if (!ok)
                    ok = await seekAndPaint(playerEl, Math.min(safeDur, t + 0.05));
            ...
            if (missed)
                sheetOut.appendChild(el('p', { class: 'anr-hint' }, missed + ' of ' + total + ' frames could not be captured - the video stalled seeking to them.'));
        ...
        const runScenes = async () => {
            ...
            if (signal && signal.aborted)
                return;
            sceneOut.innerHTML = '';
            sceneOut.appendChild(el('p', { class: 'anr-hint', style: 'margin-bottom:10px;' }, changes.length ? changes.length + ' scene change' + (changes.length > 1 ? 's' : '') + ' detected' : 'No scene changes detected'));
            try {
                const ctCard = buildContentTimelineCard(contentSamples, dur, playerEl);
                if (ctCard)
                    sceneCard.after(ctCard);
            }
            catch (_) { }
            if (changes.length && isFinite(dur) && dur > 0) {
                const timeline = el('div', { class: 'anr-scene-timeline' });
        ...
        if (bigVideo) {
            sceneBadge.remove();
            sceneOut.innerHTML = '';
            sceneOut.appendChild(el('p', { class: 'anr-hint', style: 'margin-bottom:8px;' }, 'Skipped automatically for large videos (' + (file.size / 1048576).toFixed(0) + ' MB). '
    ...
    if (audioResultsEl)
        mountAudioAnalyseButton(audioResultsEl, async () => {
            audioResultsEl.hidden = false;
...
function renderBitrateGraph(perSecKbps) {
    ...
    for (let i = 0; i < bars; i++) {
        let v = 0;
        for (let j = Math.floor(i * step); j < Math.floor((i + 1) * step) && j < n; j++)
            v = Math.max(v, perSecKbps[j]);
...
async function buildVideoAdvancedCard(file) {
    let s = null;
    try {
        s = await analyzeMp4Structure(file);
    }
    catch (_) {
        return null;
    }
    ...
    const [advH, advHelp] = h3help('Advanced', 'Container structure and stream forensics read straight from the MP4/MOV boxes, with nothing decoded. The headline provenance signs are shown up top; the deeper panels fold open when you want them.');
    card.appendChild(advH);
    card.appendChild(advHelp);
    // -- Provenance tells (the forensic headline - shown open, on top) --
    {
        const rows = [];
        if (s.faststart !== null) {
            rows.push(rowHelp('Faststart', s.faststart ? 'Yes - moov before mdat (progressive / web-optimised)'
                : 'No - mdat before moov (typical camera-original layout)', 'Faststart puts the file’s index (the moov) before the video data so playback can start before the whole file has downloaded. Editors and upload tools add it; most cameras write the index last, so an original camera file often will not have it.'));
        }
        if (s.ftyp && s.ftyp.majorBrand) {
            const brands = (s.ftyp.brands || []).filter((b) => b && b !== s.ftyp.majorBrand);
            rows.push(rowHelp('Brand', s.ftyp.majorBrand + (brands.length ? '  (' + brands.join(', ') + ')' : ''), 'Codes near the start of the file (the ftyp brands) that say which format standard it follows, such as mp42, isom, qt or M4V. They hint at which tool wrote the file and which device it was meant for.'));
        }
        ...
        if (edited.length)
            rows.push(rowHelp('Edit lists', edited.join(', ') + '  (multi-segment)', 'An edit list (elst) with several segments splices or trims a track’s timeline, which is a sign of editing. The plain single-segment version found in most MP4s is not flagged here.'));
        if (s.mdatCount > 1)
            rows.push(rowHelp('Media segments', s.mdatCount + ' mdat boxes', 'More than one block of media data usually means the file was joined together or exported by an editor, rather than recorded in one continuous pass.'));
        if (s.padBytes > 0)
            rows.push(rowHelp('Padding', fmtBytes(s.padBytes) + ' in ' + s.padCount + ' free/skip box' + (s.padCount === 1 ? '' : 'es'), 'Empty reserved space inside the file (free or skip boxes), often left behind when a faststart tool moved the index, or added as slack when the file was assembled.'));
        if (s.fragmented)
            rows.push(rowHelp('Fragmented', 'Yes - moof fragments', 'The video is broken into many small fragments (moof) instead of one continuous block. This is how streaming formats (DASH/CMAF/HLS-fMP4) and some recorders lay out a file.'));
        if (s.trailing)
            rows.push(rowHelp('Trailing data', s.trailing.type + '  (' + fmtBytes(s.trailing.size) + ')', 'Extra data sitting after the main media. Sometimes a marker added by an editor or app (a uuid box), sometimes leftover data tacked on after the file was first written.'));
        if (rows.length) {
            const { det, body } = vAdvPanel('Provenance tells', 'Structural signs of how the file was produced - camera-original, re-muxed, edited or streamed.', true);
    ...
    // -- Tracks --
    if (s.tracks.length) {
        const { det, body } = vAdvPanel('Tracks (' + s.tracks.length + ')', 'Every track in the file, not just the first video and audio - including timecode and timed-metadata streams (GoPro, CAMM, Sony gyro).');
        const tt = el('table', { class: 'anr-readout' });
        for (const t of s.tracks) {
            const parts = [];
            if (t.codecName)
                parts.push(t.codecName);
            if (t.language && t.language !== 'und')
                parts.push(t.language);
            if (isFinite(t.durationSec) && t.durationSec > 0)
                parts.push(formatDuration(t.durationSec));
            if (t.sampleCount)
                parts.push(t.sampleCount.toLocaleString() + ' sample' + (t.sampleCount === 1 ? '' : 's'));
            if (t.timecode)
                parts.push('start ' + t.timecode + (t.dropFrame ? ' (drop-frame)' : ''));
            ...
            if (t.editList && t.editList.entries > 1)
                parts.push('edit list (' + t.editList.entries + ' segments)');
    ...
    if (s.gop) {
        const g = s.gop;
        const { det, body } = vAdvPanel('Frames & bitrate', 'Keyframe structure and data-rate over time, computed from the sample tables (sizes, sync samples and durations) - no frames are decoded.');
        const ft = el('table', { class: 'anr-readout' });
        ft.appendChild(rowHelp('Frame rate', g.cfr ? g.avgFps.toFixed(3).replace(/\.?0+$/, '') + ' fps (constant)'
            : 'Variable - avg ' + g.avgFps.toFixed(2) + ' fps (' + g.minFps.toFixed(2) + ' - ' + g.maxFps.toFixed(2) + ')', 'How many frames play each second, added up from the timing of every frame. One steady value means a constant frame rate; several means it varies (variable frame rate), which is common in screen recordings and phone footage.'));
        ft.appendChild(rowHelp('Keyframe interval', g.allIntra ? 'All-intra (every frame a keyframe)'
            : 'avg ' + g.avgGop.toFixed(1) + ' frames  ·  max ' + g.maxGop + '  ·  ' + g.keyCount.toLocaleString() + ' keyframes', 'The average and longest gap between keyframes - full, self-contained frames (also called the GOP length). Short, regular gaps suit streaming and editing; a single keyframe, or every frame being a keyframe, points to camera-original or editing-friendly formats.'));
        if (g.pAvg > 0)
            ft.appendChild(rowHelp('Frame size', 'keyframe ~' + fmtBytes(g.iAvg) + '  ·  inter ~' + fmtBytes(g.pAvg), 'The average stored size of keyframes compared with in-between frames. Keyframes are usually several times larger because each one holds a complete picture, while the others only store what changed.'));
        ft.appendChild(rowHelp('Bitrate (video)', (g.avgBitrateKbps / 1000).toFixed(2) + ' Mbps avg  ·  ' + (g.peakKbps / 1000).toFixed(2) + ' Mbps peak', 'How much data the picture uses per second, on average and at its busiest, measured second by second. This is the video on its own, unlike the whole-file bitrate shown in File info.'));
        body.appendChild(ft);
        if (g.perSecKbps && g.perSecKbps.length > 1) {
            ...
            body.appendChild(el('p', { class: 'anr-hint', style: 'margin:6px 0 0;' }, 'Per-second video bitrate across the clip. Peaks mark high-motion or scene-change sections.'));
        }
        card.appendChild(det);
    }
    ...
    let bs = null;
    try {
        bs = await analyzeBitstream(file);
    }
    catch (_) { }
    if (bs)
        appendBitstreamPanel(card, bs);
    // -- Box tree (last) --
    ...
    {
        const { det, body } = vAdvPanel('Box tree (' + s.tree.length + ' top-level box' + (s.tree.length === 1 ? '' : 'es') + ')', 'Every atom (box) in the file: its 4-character type, size and byte offset. Expand a container to see what it holds.');
...
function appendBitstreamPanel(card, bs) {
    const { det, body } = vAdvPanel('Bitstream & authenticity', 'Read from the actual H.264/H.265 stream, not just the container: the codec’s own SPS, an encoder fingerprint, HDR mastering values and any Content Credentials.');
    // Stream (SPS)
    if (bs.sps) {
        const p = bs.sps;
        body.appendChild(el('div', { class: 'anr-readout-section' }, 'Stream (from the codec SPS)'));
        const st = el('table', { class: 'anr-readout' });
        st.appendChild(rowHelp('Codec', p.codec + '  ·  ' + p.profile + '  ·  L' + p.level, 'The profile and level read from deep inside the video stream itself (the sequence parameter set) - the true record of what the encoder actually produced, regardless of what the file’s outer labels claim.'));
        st.appendChild(rowHelp('Coded size', p.width + ' × ' + p.height + ' px', 'The full frame size the encoder actually worked on, which can be slightly larger than the displayed resolution. Video is encoded in blocks, so the picture is padded up to the next block boundary (for example a 1080-tall video is often coded as 1088) and the extra rows are cropped off on playback.'));
        st.appendChild(rowHelp('Chroma / depth', p.chroma + '  ·  ' + p.bitDepth + '-bit', 'How the colour is stored. Chroma subsampling (4:2:0, 4:2:2 or 4:4:4) is how much colour detail is kept relative to brightness - the eye notices brightness more, so most video keeps less colour to save space. Bit depth is how many shades each channel has (8-bit is standard; 10-bit gives smoother gradients and is common for HDR).'));
        st.appendChild(rowHelp('Scan', p.progressive ? 'Progressive' : 'Interlaced', 'Whether each frame is stored complete (progressive) or as two interlaced half-frames. Read from the frame_mbs_only flag in the stream’s settings (the SPS).'));
        if (p.colourText)
            st.appendChild(rowHelp('Colour (VUI)', p.colourText, 'The colour instructions and signal range carried inside the video stream itself (its VUI). If these disagree with the labels on the outer container, the file was re-tagged after encoding.'));
        if (p.fps)
            st.appendChild(rowHelp('Stream frame rate', p.fps.toFixed(3).replace(/\.?0+$/, '') + ' fps', 'The frame rate read from the video stream itself, shown separately from the container’s stated frame rate. Usually they agree; if they disagree, the file was re-tagged or re-timed after it was encoded.'));
        body.appendChild(st);
    }
    // Consistency verdict
    if (bs.consistency && bs.consistency.length) {
        ...
        body.appendChild(el('p', { class: 'anr-hint', style: 'margin:0 0 8px;' }, mism.length
    ...
    if (bs.encoder) {
        body.appendChild(el('p', { class: 'anr-hint', style: 'margin:0 0 6px;' }, (bs.encoder.tool ? bs.encoder.tool + ' - from' : 'From') + ' an unregistered SEI message in the first frame. Pins the exact encoder build and its settings.'));
        ...
        body.appendChild(el('p', { class: 'anr-hint', style: 'margin:0;' }, 'None found. Hardware and camera encoders (phones, GoPro, cameras) rarely embed one; software encoders like x264/x265 do, so its absence is itself a mild camera-original tell.'));
    }
    // HDR
    if (bs.hdr) {
        const h = bs.hdr;
        ...
        if (h.mdcv)
            ht.appendChild(rowHelp('Mastering display', h.mdcv.maxLum.toFixed(0) + ' cd/m² peak  ·  ' + h.mdcv.minLum.toFixed(4) + ' cd/m² black', 'The brightness range of the professional screen the video was colour-graded on (recorded as SMPTE ST 2086 data).'));
        if (h.clli)
            ht.appendChild(rowHelp('Content light', 'MaxCLL ' + h.clli.maxCLL + '  ·  MaxFALL ' + h.clli.maxFALL, 'The brightest single point and the brightest whole-frame average in the video, measured in cd/m² (the CEA-861.3 standard).'));
        if (h.dolbyVision)
            ht.appendChild(rowHelp('Dolby Vision', 'profile ' + h.dolbyVision.profile + '  ·  level ' + h.dolbyVision.level, 'Dolby Vision is a premium HDR format. The profile says which variant is used (which decides how it plays on non-Dolby screens) and the level indicates the resolution and frame-rate range it targets.'));
        else if (h.dolbyVisionCodec)
            ht.appendChild(rowHelp('Dolby Vision', 'signalled by codec', 'Dolby Vision is a premium HDR format. Here it is declared only by the container’s codec label rather than by full stream metadata, so the profile and level details are not present to read.'));
        body.appendChild(ht);
    }
    // C2PA
    body.appendChild(el('div', { class: 'anr-readout-section' }, 'Content Credentials (C2PA)'));
    body.appendChild(el('p', { class: 'anr-hint', style: 'margin:0;' }, bs.c2pa && bs.c2pa.present
...
export async function renderVideo(file, resultsEl, opts = {}) {
    ...
    if (!inline) {
        ...
        if (bootPv) {
            bootPv.innerHTML = '';
            bootPv.appendChild(inlineLoader('Decoding video…'));
        }
    }
    let header = {};
    try {
        header = await peekVideoContainer(file);
    }
    catch (_) { }
    ...
    if (!opts.remuxed && looksRaw) {
        ...
        resultsEl.innerHTML = '';
        resultsEl.appendChild(el('div', { class: 'anr-info' }, 'Raw ' + kind + ' elementary stream - remuxing to MP4 with FFmpeg so it plays in the browser…'));
        ...
        if (!renderSignal.aborted) {
            ...
            diag.appendChild(el('pre', { style: 'white-space:pre-wrap; word-break:break-word; font-size:12px; margin:8px 0 0; overflow:auto;' }, tail || 'FFmpeg produced no output and emitted no log (it may be offline or blocked).'));
            resultsEl.appendChild(diag);
        }
        return;
    }
    ...
    if (!opts.remuxed && header.container === 'MPEG-TS') {
        ...
        resultsEl.innerHTML = '';
        resultsEl.appendChild(el('div', { class: 'anr-info' }, 'AVCHD / MPEG-TS video - remuxing to MP4 with FFmpeg so it plays in the browser…'));
        let mp4Blob = null, remuxLog = '';
        try {
    ...
    try {
        ...
        if (avi) {
            ...
            appendCreatorRows(tbl, header);
            if (avi.codec)
                tbl.appendChild(row('Video codec', avi.codec.toUpperCase()));
            if (avi.audioCodec)
                tbl.appendChild(row('Audio codec', avi.audioCodec.toUpperCase()));
            tbl.appendChild(row('Resolution', `${avi.width} × ${avi.height} px`));
            tbl.appendChild(row('Aspect ratio', aspectRatio(avi.width, avi.height)));
            if (avi.duration)
                tbl.appendChild(row('Duration', formatDuration(avi.duration)));
            if (avi.fps)
                tbl.appendChild(row('Frame rate', avi.fps + ' fps'));
            if (avi.totalFrames)
                tbl.appendChild(row('Total frames', avi.totalFrames.toLocaleString()));
            ...
            if (framesAreJpeg) {
                ...
                const seekToFrame = (idx) => {
                    ...
                    if (hasAudio) {
                        try {
                            frameAudioEl.currentTime = Math.min(audioDur, frameTimeOf(idx));
                        }
                        catch (_) { }
                    }
                    showFrame(idx);
                ...
                if (frames.length === 1) {
                    ...
                    const bumpDrops = (n) => {
                        if (!playing || n <= 0)
                            return;
                        if (n > fps * 2)
                            return; // a multi-second leap is a tab-switch/seek, not a decode hiccup
                        droppedFrames += n;
                    ...
                    playBtn.addEventListener('click', () => {
                        ...
                        if (hasAudio) {
                            if (frameAudioEl.currentTime >= audioDur - 0.05) {
                                try {
                                    frameAudioEl.currentTime = 0;
                                }
                                catch (_) { }
                            }
                    ...
                    const onTMove = (e) => { if (dragging && e.touches[0]) {
                        e.preventDefault();
                    ...
                    frameCard.appendChild(el('p', { class: 'anr-hint', style: 'margin-top:4px; text-align:center;' }, [frameLabel, document.createTextNode(` · ${fps} fps${hasAudio ? '' : ' · loop'}`), dropOut]));
                    ...
                    if (heavy) {
                        frameCard.appendChild(el('p', { class: 'anr-hint', style: 'margin-top:8px; color: var(--accent);' }, '⚠ Heavy playback: this clip is large enough (' +
    ...
    if (previewSlot && (posterUrl || vw)) {
        ...
        const miniPlayer = el('div', { class: 'section-meta-player anr-video-hoverui' }, [mini, makePlayer(mini, undefined, { noVolume: true, mutedPreview: true })]);
        thumb.appendChild(miniPlayer);
        thumb.appendChild(el('p', { class: 'section-meta-preview-caption' }, `${vw} × ${vh} · ${formatDuration(dur)} · ${fmtBytes(file.size)}`));
    ...
    if (photoResultsEl && vw && vh) {
        ...
        fcv.toBlob(blob => { if (blob)
            firstFrameFile = new File([blob], 'frame_0.000s.png', { type: 'image/png' }); }, 'image/png');
    }
    ...
    if (photoResultsEl && vw && vh) {
        mountPhotoAnalyseButton(photoResultsEl, () => {
            ...
            if (live) {
                const cv = document.createElement('canvas');
                cv.width = playerEl.videoWidth;
                cv.height = playerEl.videoHeight;
                try {
                    ...
                    cv.toBlob(blob => {
                        ...
                        if (f)
                            renderPhoto(f, photoResultsEl, vctx.photoOpts({ sourceNote: 'Frame captured at ' + t.toFixed(3) + 's from ' + (file.name || 'the video') + '.' }));
                    }, 'image/png');
                    return;
                }
            ...
            if (firstFrameFile)
                renderPhoto(firstFrameFile, photoResultsEl, vctx.photoOpts({ sourceNote: 'First frame of ' + (file.name || 'the video') + '.' }));
        });
    }
    // ---- File info ----
    ...
    if (opts.sourceFile && opts.converted)
        tbl.appendChild(rowHelp('Source', opts.sourceCodec || 'Original codec', 'Your browser could not play the original format, so Analyser converted it to H.264 (MP4) on your device using FFmpeg. Converting loses a little quality, so this copy is for viewing and analysis, not for keeping as a master.'));
    else if (opts.sourceFile)
        tbl.appendChild(rowHelp('Source', 'Raw ' + (opts.sourceKind || 'H.264') + ' (Annex B)', 'A raw ' + (opts.sourceKind || 'H.264') + ' stream has no container to hold it, so Analyser wrapped it in an MP4 on your device without re-encoding, just to play it. '
    ...
    if (header.container)
        tbl.appendChild(row('Container', (opts.sourceFile && opts.converted ? (opts.sourceCodec || 'Original') + ' → H.264 / MP4 (converted)'
    ...
    try {
        appendTrackRows(tbl, rawStream ? { video: videoTrackFromStream(rawStream), audio: isoTracks && isoTracks.audio } : isoTracks);
    }
    catch (_) { }
    infoCard.appendChild(tbl);
    resultsEl.appendChild(infoCard);
    ...
    if (exif) {
        ...
        if (exif.Software)
            metaRows.push(['Software', exif.Software]);
        if (exif.DateTimeOriginal)
            metaRows.push(['Taken', fmtDate(exif.DateTimeOriginal)]);
        if (exif.CreateDate)
            metaRows.push(['Created', fmtDate(exif.CreateDate)]);
        if (exif.ModifyDate)
            metaRows.push(['Modified', fmtDate(exif.ModifyDate)]);
    ...
    if (vw && vh) {
        ...
        sheetCard.appendChild(shH);
        sheetCard.appendChild(shHelp);
        ...
        const sceneCard = el('div', { class: 'anr-card' });
        const [scH, scHelp] = h3help('Scene changes', 'Checks the video at regular intervals and measures how much each frame differs from the one before. When the change is big enough it marks a scene change, with a score for how clear-cut it was. Runs automatically; click any thumbnail or timeline marker to jump there.');
        sceneCard.appendChild(scH);
        sceneCard.appendChild(scHelp);
        const sceneOut = el('div');
        sceneOut.appendChild(el('p', { class: 'anr-hint' }, 'Detecting scene changes…'));
        sceneCard.appendChild(sceneOut);
        resultsEl.appendChild(sceneCard);
        ...
        function renderSceneResults(changes) {
            sceneOut.innerHTML = '';
            sceneOut.appendChild(el('p', { class: 'anr-hint', style: 'margin-bottom:10px;' }, changes.length
        ...
        async function detectAndRender(videoEl, removeAfter) {
            ...
            const contentSamples = [];
            try {
                changes = await detectSceneChanges(videoEl, 55, renderSignal, contentSamples, (f) => prog.set(f, 'Scanning for scene changes… ' + Math.round(f * 100) + '%'));
            }
            catch (_) { }
            if (removeAfter) {
                try {
        ...
        if (bigVideo) {
            ...
            sceneBadge.remove();
            sceneOut.innerHTML = '';
            sceneOut.appendChild(el('p', { class: 'anr-hint', style: 'margin-bottom:8px;' }, 'Skipped automatically for ' + skipWhy + '. '
            ...
            function startScan() {
                ...
                let started = false;
                const go = () => { if (started)
                    return; started = true; detectAndRender(v, true); };
            ...
            const [ctH, ctHelp] = h3help('Content timeline', 'A movie barcode - every sampled frame squeezed to a single stripe of its average colour, so the whole video reads as a colour-over-time fingerprint - plus a brightness curve flagging near-black frames and frozen or still stretches.');
            ctWait.appendChild(ctH);
            ctWait.appendChild(ctHelp);
            ctWait.appendChild(el('p', { class: 'anr-hint', style: 'margin-bottom:8px;' }, 'Read from the same scan as Scene changes, which is skipped automatically for '
    ...
    const audioResultsEl = vctx.audioTarget();
    if (audioResultsEl && !opts.noAudio)
        mountAudioAnalyseButton(audioResultsEl, async () => {
            audioResultsEl.hidden = false;
            ...
            try {
                ...
                if (!audioCard.isConnected)
                    audioResultsEl.appendChild(audioCard);
                audioCard.appendChild(el('p', { class: 'anr-hint' }, 'Audio decode failed: ' + (e && e.message || 'unknown error') + '. Try converting to MP4 (H.264 + AAC).'));
            }
        });
    // ---- Reverse playback (re-encode the video backwards, on demand) ----
    // Sits just above Integrity, below the scene-change card.
    ...
    if (hashFile.size <= HASH_FILE_MAX) {
        ...
        hashCard.appendChild(vhH);
        hashCard.appendChild(vhHelp);
    ...
    if (full && !renderSignal.aborted) {
        try {
            ...
            if (adv && !renderSignal.aborted)
                resultsEl.appendChild(adv);
        }
