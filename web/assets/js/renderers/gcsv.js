...
function parseGcsv(text) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (!ln)
            continue;
        if (/imu\s*log/i.test(ln) && i < 3)
            continue; // "GYROFLOW IMU LOG" banner
        const parts = ln.split(/[,\t]/).map((s) => s.trim());
        if (parts[0].toLowerCase() === 't') {
            cols = parts.map((s) => s.toLowerCase());
            dataStart = i + 1;
            break;
        }
        if (parts.length >= 2) {
            ...
            if (k in meta)
                meta[k] = (k === 'tscale' || k === 'gscale' || k === 'ascale') ? (parseFloat(v) || 1) : v;
        }
    }
    if (!cols)
        return null;
    ...
    for (let i = dataStart; i < lines.length; i++) {
        ...
        if (hasGyro)
            for (let a = 0; a < 3; a++)
                rg[a].push((parseFloat(p[ig[a]]) || 0) * meta.gscale * RAD2DEG);
        if (hasAccel)
            for (let a = 0; a < 3; a++)
                ra[a].push((parseFloat(p[ia[a]]) || 0) * meta.ascale);
    }
    ...
    for (let i = 0; i < n; i += stride) {
        ...
        for (let a = 0; a < 3; a++) {
            if (hasGyro)
                gyro[ax[a]].push(rg[a][i]);
            if (hasAccel)
                accel[ax[a]].push(ra[a][i]);
        }
    }
...
export async function renderGcsv(file, resultsEl) {
    ...
    const [h, help] = h3help('Gyro log (.gcsv)', 'A motion-sensor log from Gyroflow: moment by moment it records how fast the camera was turning (gyroscope) and the forces acting on it (accelerometer), the readings used to smooth out shaky footage. '
    ...
    if (m.version)
        tbl.appendChild(row('Version', m.version));
    if (m.id)
        tbl.appendChild(row('Logger / camera', m.id));
    if (m.orientation)
        tbl.appendChild(rowHelp('IMU orientation', m.orientation, 'Which way round Gyroflow treats the sensor’s three directions (up/down, left/right, forward/back) so they line up with the picture.'));
    ...
    if (parsed.rate)
        tbl.appendChild(rowHelp('Sample rate', '~' + Math.round(parsed.rate).toLocaleString() + ' Hz', 'How many readings were taken each second, estimated from the typical time gap between them.'));
    if (parsed.durationSec)
        tbl.appendChild(row('Duration', parsed.durationSec.toFixed(2) + ' s'));
...
export function looksLikeGyroCsv(text) {
    ...
    if (/sony rtmd imu export/i.test(head))
        return true;
    for (const ln of head.split(/\r?\n/)) {
        ...
        if ((has('gyro_x') && has('gyro_y') && has('gyro_z')) ||
            ...
            (has('accel_x') && has('accel_y') && has('accel_z')))
            return true;
...
function parsePlainGyroCsv(text) {
    ...
    if (!header)
        return null;
    const ix = (names) => { for (const n of names) {
    ...
    for (let i = dataStart; i < lines.length; i++) {
        ...
        if (hasGyro)
            for (let a = 0; a < 3; a++)
                G[a].push(parseFloat(p[ig[a]]) || 0);
        if (hasAccel)
            for (let a = 0; a < 3; a++)
                A[a].push(parseFloat(p[ia[a]]) || 0);
    }
    const n = T.length;
    if (!n)
        return null;
    ...
    for (let i = 0; i < n; i += stride) {
        ...
        for (let a = 0; a < 3; a++) {
            if (hasGyro)
                gyro[ax[a]].push(G[a][i]);
            if (hasAccel)
                accel[ax[a]].push(A[a][i]);
        }
    }
...
export async function renderGyroCsv(file, resultsEl, preText) {
    ...
    try {
        if (text == null)
            text = await file.slice(0, MAX_TEXT).text();
    }
    catch (e) {
        resultsEl.innerHTML = '';
        resultsEl.appendChild(errorCard('Could not read this file: ' + (e && e.message)));
    ...
    resultsEl.innerHTML = '';
    const card = el('div', { class: 'anr-card' });
    const [h, help] = h3help('Gyro log (CSV)', 'A spreadsheet-style (CSV) log of motion-sensor readings - how fast the camera turned (gyroscope) and the forces on it (accelerometer) - like the one Analyser can pull from a video’s built-in motion track. '
    ...
    tbl.appendChild(row('Samples', parsed.samples.toLocaleString()));
    if (parsed.rate)
        tbl.appendChild(rowHelp('Sample rate', '~' + Math.round(parsed.rate).toLocaleString() + ' Hz', 'How many readings were taken each second, estimated from the typical time gap between rows.'));
    if (parsed.durationSec)
        tbl.appendChild(row('Duration', parsed.durationSec.toFixed(2) + ' s'));
    tbl.appendChild(row('Size', fmtBytes(file.size)));
    card.appendChild(buildImuTimeline(parsed.d, null));
