...
export async function longAverageSpectrum(mono, sampleRate, opts = {}) {
    ...
    for (let i = 0; i < N; i++)
        win[i] = 0.5 - 0.5 * Math.cos((2 * Math.PI * i) / (N - 1)); // Hann
    ...
    for (let b = 0; b < half; b++)
        if (power[b] > peak)
            peak = power[b];
    for (let b = 0; b < half; b++)
        db[b] = 10 * Math.log10(power[b] / peak + 1e-30);
...
export function analyzeTranscode(spec, opts = {}) {
    ...
    if (brickwall) {
        if (cutoffHz < 11500)
            sourceGuess = 'MP3 96 kbps or lower';
        else if (cutoffHz < 16500)
            sourceGuess = 'MP3 128 kbps / AAC ~96 kbps';
        else if (cutoffHz < 18500)
            sourceGuess = 'MP3 160-192 kbps / AAC 128 kbps';
        else if (cutoffHz < 19700)
            sourceGuess = 'MP3 256 kbps / AAC ~192 kbps';
        else
            sourceGuess = 'MP3 320 kbps / AAC ~256 kbps';
    }
...
export function analyzeUltrasonic(spec, opts = {}) {
    const startHz = opts.startHz || 18000;
    const nyquist = spec.sampleRate / 2;
    if (nyquist <= startHz + 500)
        return { supported: false, nyquist };
    const { power } = spec;
    ...
    for (let b = startBin; b < power.length - nb; b++) {
        ...
        if (prom > 12)
            peaks.push({ hz: hzOfBin(spec, b), promDb: prom });
    }
...
export function analyzeMainsHum(spec) {
    ...
    const prominence = (hz) => {
        ...
        for (let k = -win; k <= win; k++) {
            const bb = pk + k;
            if (bb < 1 || bb >= spec.power.length || Math.abs(k) <= 2)
                continue;
            med += spec.power[bb];
            n++;
        }
...
async function kWeight(samples, sampleRate, tick) {
    ...
    const st1 = await applyBiquad(samples, (A1 * ((A1 + 1) + (A1 - 1) * c1 + 2 * sq * al1)) / a0s, (-2 * A1 * ((A1 - 1) + (A1 + 1) * c1)) / a0s, (A1 * ((A1 + 1) + (A1 - 1) * c1 - 2 * sq * al1)) / a0s, (2 * ((A1 - 1) - (A1 + 1) * c1)) / a0s, ((A1 + 1) - (A1 - 1) * c1 - 2 * sq * al1) / a0s);
...
export async function loudnessR128(mono, sampleRate, tick) {
    ...
    const series = [];
    for (const b of mom) {
        const l = msToLufs(b.ms);
        if (l > momentaryMax)
            momentaryMax = l;
        series.push({ t: b.t, lufs: l });
    }
    for (const b of shrt) {
        const l = msToLufs(b.ms);
        if (l > shortTermMax)
    ...
    if (n) {
        ...
        let s2 = 0, n2 = 0;
        for (const b of gate) {
            if (msToLufs(b.ms) > relGate && msToLufs(b.ms) > ABS) {
                s2 += b.ms;
                n2++;
            }
        }
...
export async function signalHealth(channels, tick) {
    ...
    let maxDc = 0;
    for (const d of dcPerCh)
        if (Math.abs(d) > Math.abs(maxDc))
            maxDc = d;
...
export async function detectDtmf(mono, sampleRate, tick) {
    ...
    const frameDigit = (start) => {
        ...
        for (let i = 0; i < 4; i++) {
            if (i !== li && mag[i] > secLow)
                secLow = mag[i];
            if (4 + i !== hi && mag[4 + i] > secHigh)
                secHigh = mag[4 + i];
        }
        // Twist + presence + purity checks.
        const rowColSum = low + high;
        if (rowColSum < total * 0.30)
            return null; // tones must dominate the frame
        if (low < secLow * 4 || high < secHigh * 4)
            return null; // one clear tone per group
        const twist = 10 * Math.log10((low + 1e-30) / (high + 1e-30));
        if (Math.abs(twist) > 8)
            return null; // ITU-T Q.24 receivers accept ~8 dB twist; looser than that admits music transients
    ...
    let runLen = 0;
    const flush = () => { if (cur && runLen >= MIN_FRAMES)
        digits.push({ digit: cur, tStart: runStart, tEnd: runEnd }); };
    for (const f of raw) {
        if (f.d && f.d === cur) {
            runEnd = f.t;
            runLen++;
