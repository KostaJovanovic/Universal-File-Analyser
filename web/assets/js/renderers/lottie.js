...
async function readLottieData(file, ext) {
    ...
    if (ext === 'tgs' || isGzip) {
        ...
        if (!out)
            throw new Error('could not decompress this .tgs');
...
function neutraliseLottie(data) {
    ...
    for (const f of fonts) {
        ...
        delete f.fOrigin;
        if (f.fFamily != null)
            f.fFamily = String(f.fFamily).replace(/[^\w -]/g, '').slice(0, 100);
    }
    const assets = data && Array.isArray(data.assets) ? data.assets : [];
    for (const a of assets) {
        // Precomps (a.layers) are nested timelines, not files.
        if (!a || typeof a !== 'object' || Array.isArray(a.layers) || (a.p == null && a.u == null))
            continue;
        const p = typeof a.p === 'string' ? a.p.replace(/[\x00-\x20]+/g, '') : '';
        if (/^data:(?:image|audio)\//i.test(p)) {
            a.p = p;
            a.u = '';
            a.e = 1;
            continue;
...
export async function renderLottieData(data, resultsEl, file) {
    ...
    if (!lib) {
        resultsEl.appendChild(errorCard('Could not load the Lottie engine.'));
        return;
    }
    if (!isLottie(data)) {
        resultsEl.appendChild(errorCard('This JSON is not a Lottie animation.'));
        return;
    }
    // ---- Metadata ----
    const fr = data.fr || 0;
    ...
    if (data.nm)
        t.appendChild(row('Name', String(data.nm)));
    ...
    if (Array.isArray(data.assets) && data.assets.length)
        t.appendChild(row('Assets', data.assets.length));
    ...
    loopBtn.addEventListener('click', () => { anim.loop = !anim.loop; loopBtn.classList.toggle('is-on', anim.loop); if (anim.loop && playing)
        anim.play(); });
    ...
    const lottieStopper = () => {
        ...
        setTimeout(() => {
            const stash = window._anrHomeMain;
