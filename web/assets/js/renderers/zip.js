...
async function readCentralDirectory(file) {
    ...
    if (count === 0xFFFF || cdSize === 0xFFFFFFFF || cdOff === 0xFFFFFFFF)
        return null;
    if (cdOff + cdSize > file.size || cdSize < 46)
        return null;
    // The central directory is usually inside the tail we already read; otherwise
    // fetch just those bytes.
    let cd;
    if (cdOff >= tailStart)
        cd = tail.subarray(cdOff - tailStart, cdOff - tailStart + cdSize);
    else
        cd = new Uint8Array(await file.slice(cdOff, cdOff + cdSize).arrayBuffer());
...
async function readEntryRaw(file, buf, entry) {
    ...
    if (lh.length < 30 || !(lh[0] === 0x50 && lh[1] === 0x4B && lh[2] === 0x03 && lh[3] === 0x04))
        return null;
...
export async function openZip(file, maxBytes) {
    ...
    if (!entries) {
        const seq = await readZipEntries(file, maxBytes);
        entries = seq.entries;
        buf = seq.buf;
    }
    const map = new Map();
    ...
    return {
        ...
        names: () => [...map.keys()],
        text: async (name) => { const e = map.get(name); if (!e)
            return null; const b = await decodeEntry(file, buf, e); return b ? new TextDecoder().decode(b) : null; },
