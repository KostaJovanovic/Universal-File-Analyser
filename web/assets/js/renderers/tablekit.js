...
function csvQuote(v) {
    ...
    if (/^[=+\-@\t\r]/.test(s) && !/^[+-]?(\d+\.?\d*|\.\d+)(e[+-]?\d+)?$/i.test(s))
        s = "'" + s;
...
const WORKBENCH_HELP_HTML = '<div class="anr-tk-help-item">Everything here runs in your browser - nothing is uploaded. The grid is a live, filterable view of your data, and the tools below summarise and chart whatever you select.</div>' +
...
const GROUPBY_HELP_HTML = '<div class="anr-tk-help-item">This builds a <strong>summary table</strong>: it takes every row, sorts them into buckets by the first column you pick, and shows one line per bucket.</div>' +
...
function decimateSpec(spec) {
    ...
    if (n <= MAX_POINTS || !['line', 'scatter', 'area'].includes(spec.type))
        return spec;
    const stride = Math.ceil(n / MAX_POINTS);
    const xLabels = [], xValues = spec.xValues ? [] : undefined;
    for (let i = 0; i < n; i += stride) {
        xLabels.push(spec.xLabels[i]);
        if (xValues)
            xValues.push(spec.xValues[i]);
    }
...
function drawXY(ctx, spec, W, H, top, th, hits) {
    ...
    if (stacked) {
        ...
        for (const s of series)
            for (let i = 0; i < n; i++)
                sums[i] += isFinite(s.data[i]) ? s.data[i] : 0;
        yMax = Math.max(0, ...sums);
        yMin = Math.min(0, ...sums);
    }
    else {
        const all = [];
        for (const s of series)
    ...
    if (xTitle)
        PAD_B += 18; // room for the x-axis title beneath the tick labels
    ...
    if (rotateX) {
        ...
        for (const i of shownIdx)
            ctx.fillText(truncateLbl(rawLbl(i)), xAt(i), plotY + plotH + 14);
    }
    ...
    if (spec.type === 'bar' || spec.type === 'histogram') {
        ...
        if (stacked || series.length === 1) {
            ...
            for (let i = 0; i < n; i++) {
                ...
                series.forEach((s, si) => {
                    ...
                    if (hits)
                        hits.push({ type: 'rect', x: bx, y: by, w: bw, h: bh, color, tip: [String(spec.xLabels[i]), (series.length > 1 ? s.name + ' = ' : '') + fmtNumFull(v)] });
                    base += v;
                });
            }
        }
            ...
            series.forEach((s, si) => {
                for (let i = 0; i < n; i++) {
                    ...
                    if (hits)
                        hits.push({ type: 'rect', x: bx, y: by, w: bwm, h: bh, color, tip: [String(spec.xLabels[i]), s.name + ' = ' + fmtNumFull(v)] });
                }
            });
        }
    }
        ...
        series.forEach((s, si) => {
            ...
            for (let i = 0; i < n; i++) {
                ...
                if (!started) {
                    ...
                    ctx.lineTo(x, y);
                if (hits)
                    hits.push({ type: 'circle', cx: x, cy: y, r: 3, fill: true, color, tip: [String(spec.xLabels[i]), (series.length > 1 ? s.name + ' = ' : '') + fmtNumFull(v)] });
            }
        ...
        series.forEach((s, si) => {
            ...
            for (let i = 0; i < n; i++) {
                ...
                ctx.fill();
                if (hits)
                    hits.push({ type: 'circle', cx: x, cy: y, r: 2.5, fill: true, color, tip: [String(spec.xLabels[i]), (series.length > 1 ? s.name + ' = ' : '') + fmtNumFull(v)] });
            }
        });
    }
    ...
    if (xTitle) {
        ctx.textAlign = 'center';
        ctx.textBaseline = 'alphabetic';
        ctx.fillText(truncateLbl(xTitle, 40), plotX + plotW / 2, H - 5);
    }
    if (yTitle) {
        ctx.save();
...
function drawBox(ctx, spec, W, H, top, th, hits) {
    ...
    boxes.forEach((b, i) => {
        ...
        ctx.lineWidth = 1.5;
        ctx.beginPath();
        ctx.moveTo(cx, yOf(b.min));
        ctx.lineTo(cx, yOf(b.q1));
        ctx.moveTo(cx, yOf(b.q3));
        ctx.lineTo(cx, yOf(b.max));
        ctx.moveTo(cx - bw / 4, yOf(b.min));
        ctx.lineTo(cx + bw / 4, yOf(b.min));
        ctx.moveTo(cx - bw / 4, yOf(b.max));
        ctx.lineTo(cx + bw / 4, yOf(b.max));
        ctx.stroke();
        ...
        ctx.beginPath();
        ctx.moveTo(cx - bw / 2, yOf(b.median));
        ctx.lineTo(cx + bw / 2, yOf(b.median));
        ctx.stroke();
...
function drawHeatmap(ctx, spec, W, H, top, th, hits) {
    ...
    ctx.fillStyle = th.muted;
    for (let r = 0; r < n; r++)
        ctx.fillText(truncateLbl(labels[r], 16), plotX - 6, plotY + r * size + size / 2);
    for (let c = 0; c < n; c++) {
        ctx.save();
    ...
    for (let r = 0; r < n; r++) {
        for (let c = 0; c < n; c++) {
            ...
            if (hits)
                hits.push({ type: 'rect', x: plotX + c * size, y: plotY + r * size, w: size - 1, h: size - 1, color: th.fg, tip: [truncateLbl(labels[r], 24) + ' × ' + truncateLbl(labels[c], 24), 'r = ' + (valid ? v.toFixed(3) : 'n/a')] });
            if (size > 22) {
    ...
    if (lx < W - 20 && lh > 10) {
        for (let i = 0; i < lh; i++) {
            ctx.fillStyle = heatColor(1 - i / lh, true);
            ctx.fillRect(lx, ly + i, 14, 1);
        }
        ctx.strokeStyle = th.hair;
...
function drawPie(ctx, spec, W, H, top, th, hits) {
    ...
    data.forEach((v, i) => {
        ...
        ctx.fill();
        if (hits && frac > 0)
            hits.push({ type: 'wedge', cx, cy, r, rInner: donut ? r * 0.55 : 0, a0, a1, color: PALETTE[i % PALETTE.length], tip: [String(labels[i]), fmtNumFull(isFinite(v) ? v : 0) + ' (' + Math.round(frac * 100) + '%)'] });
        a0 = a1;
    });
    if (donut) {
        ctx.fillStyle = th.bg;
...
function drawChart(canvas, spec) {
    ...
    const hits = [];
    if (spec.type === 'pie')
        drawPie(ctx, spec, W, H, top, th, hits);
    else if (spec.type === 'box')
        drawBox(ctx, spec, W, H, top, th, hits);
    else if (spec.type === 'heatmap')
        drawHeatmap(ctx, spec, W, H, top, th, hits);
    else
        drawXY(ctx, spec, W, H, top, th, hits);
    const note = spec.meta && spec.meta.sampledNote;
    if (note) {
        ctx.fillStyle = th.muted;
        ctx.textAlign = 'right';
        ctx.font = '10px ' + th.fontMono;
        ctx.fillText(note, W - 8, H - 6);
    }
    return hits;
}
...
function drawHitHighlight(ctx, hit, th) {
    ...
    if (hit.type === 'rect') {
        ...
        if (hit.fill) {
            ctx.fillStyle = hit.color || th.fg;
            ctx.beginPath();
            ctx.arc(hit.cx, hit.cy, (hit.r || 3) + 0.5, 0, Math.PI * 2);
            ctx.fill();
        }
        ctx.beginPath();
        ctx.arc(hit.cx, hit.cy, (hit.r || 3) + 3, 0, Math.PI * 2);
        ctx.stroke();
    }
    else if (hit.type === 'wedge') {
        ctx.beginPath();
        ctx.moveTo(hit.cx, hit.cy);
        ctx.arc(hit.cx, hit.cy, hit.r, hit.a0, hit.a1);
        ctx.closePath();
        ctx.stroke();
    }
    ctx.restore();
...
export function mountTableKit(host, model, opts = {}) {
    ...
    function renderColsList() {
        ...
        view.colOrder.forEach((c, pos) => {
            ...
            upBtn.addEventListener('click', () => {
                if (pos > 0) {
                    const t = view.colOrder[pos - 1];
                    view.colOrder[pos - 1] = view.colOrder[pos];
                    view.colOrder[pos] = t;
                    renderColsList();
                    structuralRefresh();
                    refreshPickers();
    ...
    function applySplit(colIdx, delimChoice) {
        ...
        for (let i = 0; i < rows.length; i++) {
            const p = cut(rows[i][colIdx]).length;
            if (p > maxParts)
                maxParts = p;
        }
        ...
        if (maxParts < 2)
            return null;
        if (!splitBackup)
            splitBackup = { headers: headers.slice(), rows: rows.map((r) => r.slice()) };
    ...
    function resetSplit() {
        if (!splitBackup)
            return;
        headers.length = 0;
        splitBackup.headers.forEach((h) => headers.push(h));
        rows.length = 0;
        splitBackup.rows.forEach((r) => rows.push(r.slice()));
        splitBackup = null;
        reinferAndReset();
    }
    ...
    splitDetails.addEventListener('toggle', () => { if (splitDetails.open)
        populateSplitCols(); });
    splitBtn.addEventListener('click', () => {
        ...
        if (res) {
            splitHint.textContent = 'Split into ' + res.added + ' columns on ' + res.label + '.';
            resetSplitBtn.hidden = false;
            populateSplitCols();
        }
        else
            splitHint.textContent = 'No repeating delimiter found in that column.';
    });
    ...
    function buildGrid() {
        ...
        function renderColgroup() {
            ...
            for (let i = 0; i < n; i++)
                colgroup.appendChild(el('col', { style: `width:${COL_W}px` }));
            table.style.width = (ROWNUM_W + COL_W * n) + 'px';
        }
        let scrollRaf = null;
        const onScroll = () => { if (scrollRaf)
            return; scrollRaf = requestAnimationFrame(() => { scrollRaf = null; renderBody(); }); };
            ...
            applyBtn.addEventListener('click', () => {
                if (type === 'number') {
                    ...
                    if (min == null && max == null)
                        delete view.colFilters[c];
                    else
                        view.colFilters[c] = { min, max };
                }
                else if (!input1.value)
                    delete view.colFilters[c];
                else
                    view.colFilters[c] = { text: input1.value };
        ...
        function renderHeader() {
            ...
            for (const c of gridCols()) {
                ...
                filterBtn.addEventListener('click', (e) => {
                    e.stopPropagation();
                    document.querySelectorAll('.anr-tk-filterpop').forEach((p) => { if (p !== pop)
        ...
        function applySelectionHighlight() {
            ...
            tbody.querySelectorAll('td.anr-tk-cell').forEach((td) => {
                ...
                if (r >= rect.r0 && r <= rect.r1 && c >= rect.c0 && c <= rect.c1)
        ...
        function onWinMove(e) { if (!dragging)
            return; const cell = cellFromEvent(e); if (cell) {
            dragCur = cell;
            selRect = currentSelectionRect();
            applySelectionHighlight();
            activeTarget = { kind: 'range', ...selRect };
            updateStatsBar();
        } }
        function onWinUp() {
            if (!dragging)
            ...
            if (dragStart && dragCur && (dragStart.r !== dragCur.r || dragStart.c !== dragCur.c))
                justDragged = true;
            const rect = currentSelectionRect();
            selRect = rect;
            if (rect) {
                activeTarget = { kind: 'range', ...rect };
                updateStatsBar();
                showChartSelBtn(true);
            }
        }
        ...
        function renderBody() {
            ...
            for (let i = first; i < last; i++) {
                ...
                cols.forEach((c, cp) => {
                    ...
                    td.dataset.c = String(cp);
                    td.addEventListener('click', () => { if (justDragged) {
                        justDragged = false;
                        return;
                    } showCellPopup(val, headers[c]); });
                    tr.appendChild(td);
                });
                frag.appendChild(tr);
            }
    ...
    function showChartSelBtn(show) { if (chartSelBtn)
        chartSelBtn.hidden = !show; }
    ...
    function updateStatsBar() {
        ...
        if (activeTarget.kind === 'col') {
            ...
            for (let rp = r0; rp <= r1 && rp < view.rowIndex.length; rp++) {
                ...
                for (let cp = c0; cp <= c1 && cp < cols.length; cp++) {
                    const n = toNumber(r[cols[cp]]);
                    if (!isNaN(n))
                        vals.push(n);
                }
            }
    ...
    function refreshGroupByOptions() {
        ...
        gbKeySelect.innerHTML = '';
        gbKeySelect.appendChild(el('option', { value: '' }, 'N/A'));
        cols.forEach((c) => gbKeySelect.appendChild(el('option', { value: String(c) }, headers[c])));
        gbValSelect.innerHTML = '';
        cols.filter((c) => colTypes[c] === 'number').forEach((c) => gbValSelect.appendChild(el('option', { value: String(c) }, headers[c])));
        runGroupBy();
    }
    function buildGroupBy() {
        ...
        runGroupBy = function () {
            syncValEnabled();
            gbResult.innerHTML = '';
            if (gbKeySelect.value === '')
                return; // "N/A" - summary off
    ...
    function renderFields() {
        ...
        if (t === 'heatmap') {
            dataFields.appendChild(el('span', { class: 'anr-hint' }, 'Correlates every numeric column - nothing to pick.'));
            return;
        }
        if (t === 'histogram') {
            dataFields.appendChild(field('Column', yList));
            dataFields.appendChild(field('Bins', binsInput));
            return;
        }
        if (t === 'box') {
            dataFields.appendChild(field('Columns', yList));
            return;
        }
        if (t === 'pie') {
            dataFields.appendChild(field('Category', xList));
            dataFields.appendChild(field('Value', yList));
            return;
        }
        if (t === 'bar') {
            dataFields.appendChild(field('Category', xList));
            dataFields.appendChild(field('Y axis', yList));
            dataFields.appendChild(field('Options', stackedToggle));
            return;
        }
        if (t === 'area') {
            dataFields.appendChild(field('X axis', xList));
            dataFields.appendChild(field('Y axis', yList));
            dataFields.appendChild(field('Options', stackedToggle));
            return;
        }
        dataFields.appendChild(field('X axis', xList));
        dataFields.appendChild(field('Y axis', yList)); // line, scatter
    }
    ...
    function onChartMove(e) {
        if (!currentSpec || !lastHits.length) {
    ...
    function buildChartBuilder() {
        ...
        chartSelBtn.addEventListener('click', () => { const spec = buildSpecFromSelection(); if (spec)
            loadSpecIntoBuilder(spec); });
        const chartRO = new ResizeObserver(() => { if (currentSpec)
            renderChart(currentSpec); });
    ...
    function refreshPickers() {
        ...
        xList.innerHTML = '';
        cols.forEach((c) => {
            const rb = el('input', { type: 'radio', name: xRadioName });
            rb.dataset.col = String(c);
        ...
        if (xDefault != null) {
            const rb = xList.querySelector(`input[data-col="${xDefault}"]`);
            if (rb)
                rb.checked = true;
        }
        const firstNum = cols.find((c) => colTypes[c] === 'number');
        if (firstNum != null) {
            const cb = yList.querySelector(`input[data-col="${firstNum}"]`);
            if (cb)
                cb.checked = true;
        }
    ...
    function xySpec(type, xCol, yCols, extraMeta) {
        ...
        const xKey = (r) => {
            ...
            if (xType === 'date') {
                const t = parseDateValue(raw, monthFirst);
    ...
    function histogramSpec(col, binsWanted) {
        ...
        let bins = binsWanted;
        if (!bins || bins < 5 || bins > 40)
            bins = Math.min(40, Math.max(5, Math.ceil(Math.sqrt(vals.length))));
        const width = (max - min) / bins || 1;
        const counts = new Array(bins).fill(0);
        vals.forEach((v) => { let idx = Math.floor((v - min) / width); if (idx >= bins)
            idx = bins - 1; if (idx < 0)
            idx = 0; counts[idx]++; });
    ...
    function buildSpecFromSelection() {
        if (!activeTarget || activeTarget.kind !== 'range')
            return null;
        const { r0, r1, c0, c1 } = activeTarget;
        const cols = visCols();
        const rowsSel = [];
        for (let rp = r0; rp <= r1 && rp < view.rowIndex.length; rp++)
            rowsSel.push(rows[view.rowIndex[rp]]);
        const colsSel = [];
        for (let cp = c0; cp <= c1 && cp < cols.length; cp++)
            colsSel.push(cols[cp]);
        if (!rowsSel.length || !colsSel.length)
            return null;
        ...
        if (topRowNonNumeric) {
            seriesNames = colsSel.map((c) => firstRow[c]);
            startRow = 1;
        }
        const dataRows = rowsSel.slice(startRow);
        let xLabels;
        if (leftColNonNumeric) {
            xLabels = dataRows.map((r) => r[colsSel[0]]);
            startCol = 1;
        }
        else
            xLabels = dataRows.map((_, i) => String(i + 1));
        const series = [];
        for (let ci = startCol; ci < colsSel.length; ci++)
            series.push({ name: seriesNames[ci] || headers[colsSel[ci]], data: dataRows.map((r) => toNumber(r[colsSel[ci]])) });
        if (!series.length)
            return null;
    ...
    function buildExportRow() {
        ...
        pngBtn.addEventListener('click', () => {
            if (!currentSpec)
                return;
            chartCanvas.toBlob((blob) => { if (blob)
                downloadBlob(baseFileName() + '-' + currentSpec.type + '.png', blob); }, 'image/png');
        });
        ...
        csvBtn.addEventListener('click', () => {
            ...
            for (const idx of view.rowIndex) {
                const r = rows[idx];
                lines.push(cols.map((c) => csvQuote(r[c] || '')).join(','));
            }
    ...
    function buildSuggestions() {
        ...
        if (numCols.length >= 2) {
            ...
            for (let i = 0; i < pool.length; i++)
                for (let j = i + 1; j < pool.length; j++) {
                    ...
                    if (isFinite(r))
                        pairs.push({ a: pool[i], b: pool[j], r });
                }
        ...
        shown.forEach((s) => {
            ...
            btn.addEventListener('click', () => { const spec = s.build(); if (spec)
                loadSpecIntoBuilder(spec); });
    ...
    function structuralRefresh() { if (gridApi) {
        gridApi.renderHeader();
        gridApi.renderBody();
    } }
    function computeRowIndex() {
        ...
        for (let i = 0; i < rows.length; i++) {
            ...
            for (const [colStr, f] of filters) {
                ...
                const raw = r[c] || '';
                if (f.text != null) {
                    if (!raw.toLowerCase().includes(f.text.toLowerCase())) {
                        ok = false;
                        break;
                    }
                }
            ...
            if (ok && search) {
                let hit = false;
                for (const c of cols) {
                    if ((r[c] || '').toLowerCase().includes(search)) {
                        hit = true;
                        break;
                    }
                }
