...
function parseWebpAnim(bytes) {
    const ascii = (o, n) => { let s = ''; for (let i = 0; i < n; i++)
        s += String.fromCharCode(bytes[o + i]); return s; };
...
export async function decodeWebpFrames(file, budget = 120e6) {
    ...
    for (let i = 0; i < count; i++)
        delaysMs.push(info.durationsMs[i] || 100);
    ...
    let loop = info.loop;
    if (track.repetitionCount === Infinity)
        loop = 0;
    else if (typeof track.repetitionCount === 'number')
        loop = track.repetitionCount;
    ...
    const get = async (idx) => {
        ...
        let data;
        if (closed)
            return new Uint8ClampedArray(px * 4);
        try {
...
//# sourceMappingURL=webp-frames.js.map