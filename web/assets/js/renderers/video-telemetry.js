...
function readTrackChunks(dv, ts, te, fileSize) {
    ...
    const fits = (b, at, entry) => Math.max(0, Math.floor((b.offset + b.size - at) / entry));
    let o = P(stsz) + 4;
    ...
    for (let i = 0; i < count; i++)
        sizes[i] = uniform || dv.getUint32(o + i * 4);
    const offsets = [];
    if (stco) {
        let q = P(stco) + 4;
        const n = Math.min(dv.getUint32(q), fits(stco, q + 4, 4));
        q += 4;
        for (let i = 0; i < n; i++)
            offsets.push(dv.getUint32(q + i * 4));
    }
    else {
        let q = P(co64) + 4;
        const n = Math.min(dv.getUint32(q), fits(co64, q + 4, 8));
        q += 4;
        for (let i = 0; i < n; i++)
            offsets.push(dv.getUint32(q + i * 8) * 0x100000000 + dv.getUint32(q + i * 8 + 4));
    }
    let s = P(stsc) + 4;
    const sc = Math.min(dv.getUint32(s), fits(stsc, s + 4, 12));
    s += 4;
    const runs = [];
    for (let i = 0; i < sc; i++)
        runs.push([dv.getUint32(s + i * 12), dv.getUint32(s + i * 12 + 4)]);
    let timescale = 0;
    if (mdhd) {
        const d = P(mdhd);
        const ver = dv.getUint8(d);
        timescale = ver === 1 ? dv.getUint32(d + 20) : dv.getUint32(d + 12);
    }
    const durs = new Array(count).fill(0);
    if (stts) {
        let t = P(stts) + 4;
        const n = Math.min(dv.getUint32(t), fits(stts, t + 4, 8));
        t += 4;
        let si = 0;
        for (let i = 0; i < n; i++) {
            const c = dv.getUint32(t + i * 8), del = dv.getUint32(t + i * 8 + 4);
            for (let k = 0; k < c && si < count; k++)
                durs[si++] = del;
        }
    }
    const chunks = [];
...
function gpsStats(gps) {
    ...
    for (let i = 1; i < gps.length; i++)
        distance += haversine([gps[i - 1].lat, gps[i - 1].lon], [gps[i].lat, gps[i].lon]);
    let maxSpeed = 0, altLo = Infinity, altHi = -Infinity;
    for (const p of gps) {
        if (isFinite(p.speed))
            maxSpeed = Math.max(maxSpeed, p.speed);
        if (isFinite(p.alt)) {
            altLo = Math.min(altLo, p.alt);
            altHi = Math.max(altHi, p.alt);
        }
    }
...
function gpmfValues(dv, dp, type, structSize, repeat) {
    ...
    for (let r = 0; r < repeat; r++) {
        const struct = [];
        for (let c = 0; c < per; c++)
            struct.push(gpmfRead(dv, dp + (r * per + c) * es, type));
        out.push(struct);
    }
    return out;
}
...
function gpmfWalk(dv, start, end, scope, acc, baseT, dur) {
    ...
    while (p + 8 <= end) {
        ...
        if (typeCode === 0) { // nested container
            ...
            for (const v of gpmfValues(dv, dp, type, structSize, repeat))
                if (isFinite(v[0]))
                    acc.iso.push(v[0]);
        }
        else if (key === 'SHUT') {
            for (const v of gpmfValues(dv, dp, type, structSize, repeat))
                if (isFinite(v[0]) && v[0] > 0)
                    acc.shutter.push(v[0]);
        }
        else if (key === 'WBAL') {
            for (const v of gpmfValues(dv, dp, type, structSize, repeat))
                if (isFinite(v[0]) && v[0] > 1000)
                    acc.wbal.push(v[0]);
        }
        else if (key === 'GPSF') {
            ...
            let str = '';
            for (let i = 0; i < Math.min(dataLen, 16); i++)
                str += String.fromCharCode(dv.getUint8(dp + i));
...
export async function extractGpmf(file) {
    ...
    for (const chunk of table.chunks) {
        ...
        let buf;
        try {
            buf = await file.slice(chunk.fileOffset, chunk.fileOffset + chunk.sizeBytes).arrayBuffer();
        }
        catch (_) {
            continue;
        }
        for (const samp of chunk.samples) {
            if (samp.rel + samp.len > buf.byteLength)
                continue;
            const sdv = new DataView(buf, samp.rel, samp.len);
            const dur = 1; // GPMF payloads are ~1 s; sub-sample offset spreads within it
            try {
                gpmfWalk(sdv, 0, samp.len, { scal: null }, acc, samp.t, dur);
            }
            catch (_) { }
        }
    }
    if (!acc.gps.length && !acc.accl.length && !acc.gyro.length)
        return null;
    return finishTelemetry('GoPro GPMF', acc, table, file, 'Gyroscope (rad/s)', 'Accelerometer (m/s²)');
}
// ---------- CAMM ----------
export async function extractCamm(file) {
        ...
        for (const samp of chunk.samples) {
            ...
            try {
                if (type === 2 && o + 16 <= buf.byteLength) { // gyro (rad/s)
                    ...
                    if (Math.abs(lat) <= 90 && Math.abs(lon) <= 180 && !(lat === 0 && lon === 0))
                        acc.gps.push({ lat, lon, alt, speed: NaN, t: samp.t });
                }
                    ...
                    acc.fix = fix;
                    if (Math.abs(lat) <= 90 && Math.abs(lon) <= 180 && !(lat === 0 && lon === 0))
                        acc.gps.push({ lat, lon, alt, speed, t: samp.t });
                }
            }
            catch (_) { }
        }
    ...
    if (!acc.gps.length && !acc.accl.length && !acc.gyro.length)
        return null;
    return finishTelemetry('CAMM', acc, table, file, 'Gyroscope (rad/s)', 'Accelerometer (m/s²)');
}
...
function finishTelemetry(source, acc, table, file, gyroName, accelName) {
    ...
    if (hasGyro || hasAccel) {
        ...
        const resample = (arr) => {
            ...
            for (let k = 0; k < N; k++) {
                const tt = grid[k];
                while (j + 1 < arr.length && Math.abs(arr[j + 1].t - tt) <= Math.abs(arr[j].t - tt))
                    j++;
                x.push(arr[j].x);
                y.push(arr[j].y);
                z.push(arr[j].z);
    ...
    const scalars = {};
    if (acc.temps && acc.temps.length)
        scalars.temperature = acc.temps.reduce((s, v) => s + v, 0) / acc.temps.length;
    if (acc.fix != null)
        scalars.fix = acc.fix;
    if (acc.gpsu)
        scalars.gpsu = acc.gpsu;
    const stat = (arr) => {
        if (!arr || !arr.length)
            return null;
    ...
    const exposure = {};
    const isoS = stat(acc.iso);
    if (isoS)
        exposure.iso = isoS;
    const shutS = stat(acc.shutter);
    if (shutS)
        exposure.shutter = shutS;
    const wbS = stat(acc.wbal);
    if (wbS)
        exposure.wb = wbS;
...
export async function extractContainerLocation(file) {
    ...
    for (const b of findAllBoxes(dv, 8, moov.size, '©xyz')) {
        ...
        let str = '';
        for (let i = 0; i < len && d + 4 + i < b.offset + b.size; i++)
            str += String.fromCharCode(dv.getUint8(d + 4 + i));
        const p = parseISO6709(str);
        if (p)
            return { ...p, source: 'QuickTime ©xyz' };
    }
    ...
    try {
        ...
        if (meta) {
            ...
            if (keys && ilst) {
                ...
                const kcount = dv.getUint32(kp);
                kp += 4;
                let locIndex = -1;
                for (let i = 0; i < kcount; i++) {
                    const ksize = dv.getUint32(kp);
                    let name = '';
                    for (let j = 8; j < ksize; j++)
                        name += String.fromCharCode(dv.getUint8(kp + j));
                    if (name.indexOf('location.ISO6709') >= 0) {
                        locIndex = i + 1;
                        break;
                    }
                    kp += ksize;
                ...
                if (locIndex > 0) {
                    ...
                    if (item) {
                        ...
                        if (data) {
                            ...
                            let str = '';
                            for (let j = dd; j < data.offset + data.size; j++)
                                str += String.fromCharCode(dv.getUint8(j));
                            const p = parseISO6709(str);
                            if (p)
                                return { ...p, source: 'Apple QuickTime location' };
                        }
                    }
                }
            }
...
export function buildTelemetryCard(d) {
    ...
    const [h, help] = h3help('Telemetry - ' + d.source, 'Action cameras and phones can record a hidden data track alongside the video: where you were and how fast you moved (GPS), plus movement sensed by the gyroscope and accelerometer. '
    ...
    const g = d.gps;
    if (g) {
        tbl.appendChild(rowHelp('GPS points', g.count.toLocaleString(), 'How many times the camera recorded its GPS position during the clip.'));
        tbl.appendChild(row('Track distance', (g.distance >= 1000 ? (g.distance / 1000).toFixed(2) + ' km' : g.distance.toFixed(0) + ' m')));
        if (g.maxSpeed > 0)
            tbl.appendChild(rowHelp('Max speed', (g.maxSpeed * 3.6).toFixed(1) + ' km/h  (' + g.maxSpeed.toFixed(1) + ' m/s)', 'The fastest speed recorded during the clip, taken from the GPS.'));
        if (isFinite(g.altLo) && isFinite(g.altHi))
            tbl.appendChild(row('Altitude', g.altLo.toFixed(0) + ' - ' + g.altHi.toFixed(0) + ' m'));
        const start = g.pts[0];
        tbl.appendChild(rowHelp('Start', fmtDeg(start.lat) + ', ' + fmtDeg(start.lon), 'Where the clip began, from the first GPS reading. Use the map link to see the whole route.'));
    }
    else {
        tbl.appendChild(rowHelp('GPS', d.source === 'GoPro GPMF' ? 'No satellite lock in this clip' : 'Not found', 'This clip has movement and exposure data but no usable GPS position - the camera had GPS turned off, or never managed to lock onto satellites (common indoors or in a short clip).'));
    }
    tbl.appendChild(rowHelp('Gyroscope', d.motion && d.motion.hasGyro ? 'Present' : 'Not found', 'Readings of how fast the camera was turning on each of its three axes, used by stabilisation tools such as Gyroflow and ReelSteady.'));
    tbl.appendChild(rowHelp('Accelerometer', d.motion && d.motion.hasAccel ? 'Present' : 'Not found', 'Readings of the camera’s acceleration along its three axes. When it is still, one axis reads about 9.8 m/s², which is gravity.'));
    if (d.scalars.temperature != null)
        tbl.appendChild(rowHelp('Temperature', d.scalars.temperature.toFixed(1) + ' °C', 'A temperature the camera measured from its own internal sensors while recording, in degrees Celsius. This is the hardware’s heat, not the white-balance colour temperature.'));
    if (isFinite(d.durationSec) && d.durationSec > 0)
        tbl.appendChild(row('Duration', d.durationSec.toFixed(1) + ' s'));
    ...
    if (ex.iso || ex.shutter || ex.wb) {
        ...
        if (ex.iso)
            et.appendChild(rowHelp('ISO', Math.round(ex.iso.min) + ' - ' + Math.round(ex.iso.max) + '  (avg ' + Math.round(ex.iso.avg) + ')', 'How sensitive to light the camera set its sensor, noted for each frame. A wide range means the light was changing; a high ISO means the scene was dark.'));
        if (ex.shutter)
            et.appendChild(rowHelp('Shutter', fmtShutter(ex.shutter.min) + ' - ' + fmtShutter(ex.shutter.max) + '  (avg ' + fmtShutter(ex.shutter.avg) + ')', 'How long each frame was exposed to light. A fast shutter (e.g. 1/1000 s) freezes motion; a slow one blurs it. It cannot go slower than the frame rate allows.'));
        if (ex.wb)
            et.appendChild(rowHelp('White balance', Math.round(ex.wb.min) + ' - ' + Math.round(ex.wb.max) + ' K  (avg ' + Math.round(ex.wb.avg) + ' K)', 'The colour of light the camera adjusted for, measured in Kelvin. Low values are warm indoor light; high values are cool daylight or shade.'));
        card.appendChild(et);
    }
    if (g && g.pts.length > 1) {
...
function buildLocationCard(loc) {
    const card = el('div', { class: 'anr-card' });
    const [h, help] = h3help('Location', 'A single GPS location saved inside the file itself (the QuickTime ©xyz or Apple location field). '
    ...
    if (isFinite(loc.alt))
        tbl.appendChild(row('Altitude', loc.alt.toFixed(1) + ' m'));
    card.appendChild(tbl);
...
export async function appendTelemetryCards(file, resultsEl, opts = {}) {
    try {
        let d = null;
        try {
            d = await extractGpmf(file);
        }
        catch (_) { }
        if (!d) {
            try {
                d = await extractCamm(file);
            }
            catch (_) { }
        }
        if (d) {
        ...
        if (!opts.hasExifGps) {
            let loc = null;
            try {
                loc = await extractContainerLocation(file);
            }
            catch (_) { }
            if (loc) {
                resultsEl.appendChild(buildLocationCard(loc));
                return true;
            }
        }
    }
...
//# sourceMappingURL=video-telemetry.js.map