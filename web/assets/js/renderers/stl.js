...
function buildViewer(geo, opts = {}) {
    ...
    function draw() {
        ...
        if (state.ortho) {
            ...
            proj = mat4Ortho(-hh * aspect, hh * aspect, -hh, hh, 0.005, 1000);
        }
        else
            proj = mat4Perspective(45 * Math.PI / 180, aspect, 0.005, 1000);
        ...
        if (state.upZ)
            model = mat4Multiply(model, mat4RotX(-Math.PI / 2));
    ...
    function snapshot() {
        ...
        state.ortho = false;
        state.panX = 0;
        state.panY = 0; // centre the framing
...
export function buildViewerCard(geo, title = '3D model', opts = {}) {
    ...
    if (viewer.ok) {
        ...
        resetBtn.addEventListener('click', () => {
            ...
            let t0 = 0;
            if (resetBtn._anim)
                cancelAnimationFrame(resetBtn._anim);
            const tick = (ts) => {
                if (!t0)
                    t0 = ts;
        ...
        if (!(geo.colors || geo.textureImage))
...
export function analyzeMeshIntegrity(positions, step) {
    ...
    if (!triN)
        return null;
    if (triN > BODY_SPLIT_CAP)
        return { tooLarge: true, triN };
    const s = step || 1;
    ...
    if (boundaryEdges && boundaryEdges <= 500000) {
        const comp = new Map();
        const find = (x) => { let r = x; while (comp.get(r) !== r)
            r = comp.get(r); while (comp.get(x) !== r) {
            const n = comp.get(x);
            comp.set(x, r);
            x = n;
        } return r; };
        const ensure = (x) => { if (!comp.has(x))
            comp.set(x, x); };
...
export function meshIntegrityCard(geo, opts = {}) {
    ...
    if (!a) {
        card.appendChild(el('p', { class: 'anr-hint' }, 'No mesh geometry to check.'));
        return card;
    }
    if (a.tooLarge) {
        card.appendChild(el('p', { class: 'anr-hint' }, `This mesh has ${a.triN.toLocaleString()} triangles - too many to scan for topological errors quickly, so the integrity check was skipped.`));
        return card;
    }
    const problems = a.boundaryEdges || a.nonManifoldEdges || a.degenerate || a.duplicate || a.flipped;
    heading.appendChild(el('span', { class: 'anr-mesh-verdict ' + (problems ? 'is-flag' : 'is-ok') }, problems ? 'Faults found' : 'Watertight'));
    const tbl = el('table', { class: 'anr-readout' });
    tbl.appendChild(rowHelp('Watertight', a.isWatertight ? 'Yes' : 'No', 'The surface is fully sealed with no holes or gaps, so it forms a solid object - every edge joins exactly two triangles. Only a sealed model has a real inside volume and prints reliably.'));
    tbl.appendChild(rowHelp('Manifold', a.isManifold ? 'Yes' : 'No', 'The surface is joined cleanly, with no edge where more than two triangles meet. Edges that break this rule confuse slicers and CAD tools, and can make cut-and-combine (boolean) operations fail.'));
    if (a.boundaryEdges)
        tbl.appendChild(rowHelp('Open edges', a.boundaryEdges.toLocaleString() + (a.holes ? ` (${a.holes.toLocaleString()} ${a.holes === 1 ? 'hole' : 'holes'})` : ''), 'Edges that belong to only one triangle instead of two. Each one is the border of a hole or gap, so the surface is not fully sealed.'));
    if (a.nonManifoldEdges)
        tbl.appendChild(rowHelp('Non-manifold edges', a.nonManifoldEdges.toLocaleString(), 'A place where too many surfaces meet along the same line. On a normal solid object, exactly two surfaces meet at each edge - like two walls meeting in the corner of a room. Here a third surface joins in, which cannot happen on a real object, so the shape has no clear inside and outside. 3D printers and modelling tools get confused by this and often fail.'));
    if (a.flipped)
        tbl.appendChild(rowHelp('Inconsistent winding', a.flipped.toLocaleString() + (a.flipped === 1 ? ' edge' : ' edges'), 'Neighbouring triangles disagree about which side faces outward, so some are effectively inside-out. This causes odd shading and throws off the inside/outside and volume calculations.'));
    if (a.degenerate)
        tbl.appendChild(rowHelp('Degenerate faces', a.degenerate.toLocaleString(), 'Collapsed triangles with no area, where two or more corners sit on the same spot. They cover no surface and are best removed.'));
    if (a.duplicate)
        tbl.appendChild(rowHelp('Duplicate faces', a.duplicate.toLocaleString(), 'Triangles that sit exactly on top of another triangle with the same three corners - redundant duplicate copies.'));
    tbl.appendChild(rowHelp('Euler characteristic', String(a.euler), 'A single number that captures the model’s overall shape, worked out from its corners, edges and faces (V - E + F), and staying the same however the shape is bent or stretched. A single sealed solid gives 2 (genus 0); each hole passing right through the body - like the hole in a doughnut - lowers it by 2.'));
    card.appendChild(tbl);
    if (!problems) {
        card.appendChild(el('p', { class: 'anr-sig-flag-note' }, 'No topological errors were detected, so the reported volume is reliable.'));
    }
    else {
        const pl = (n, one, many) => `${n.toLocaleString()} ${n === 1 ? one : (many || one + 's')}`;
        const issues = [];
        if (a.boundaryEdges)
            issues.push(pl(a.boundaryEdges, 'open edge') + (a.holes ? ` across ${pl(a.holes, 'hole')}` : ''));
        if (a.nonManifoldEdges)
            issues.push(pl(a.nonManifoldEdges, 'non-manifold edge'));
        if (a.flipped)
            issues.push(pl(a.flipped, 'inconsistently wound edge'));
        if (a.degenerate)
            issues.push(pl(a.degenerate, 'degenerate face'));
        if (a.duplicate)
            issues.push(pl(a.duplicate, 'duplicate face'));
        const list = issues.length > 1 ? issues.slice(0, -1).join(', ') + ' and ' + issues[issues.length - 1] : issues[0];
        card.appendChild(el('p', { class: 'anr-sig-flag-note' }, `This mesh has ${list}, which can cause slicing failures and an unreliable volume or inside-outside test. `
...
export function splitBodiesIndexed(verts, tris, step) {
    ...
    const union = (a, b) => { a = find(a); b = find(b); if (a !== b)
        parent[a] = b; };
    for (let t = 0; t < triN; t++) {
...
export function renderPartsViewer(file, resultsEl, { metaCard, parts, format, unitLabel, partsTitle, partsHint, zUp }) {
    resultsEl.innerHTML = '';
    if (!parts.length) {
        resultsEl.appendChild(errorCard('No models found in this file.'));
        return;
    }
    ...
    async function showPart(part, chip) {
        ...
        viewCardEl.replaceWith(loading);
        viewCardEl = loading;
        statsCardEl.replaceWith(blankStats);
        statsCardEl = blankStats;
        integCardEl.replaceWith(blankInteg);
        integCardEl = blankInteg;
