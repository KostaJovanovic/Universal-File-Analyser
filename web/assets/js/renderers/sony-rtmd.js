...
function sampleTable(dv, trakStart, trakEnd, fileSize) {
...
function scalarFields(dv, tags) {
    ...
    const iso = u32(0xe301);
    if (iso)
        f.ISO = iso;
    const wb = u8(0xe303);
    if (wb != null && WB[wb])
        f['White balance'] = WB[wb];
    if (tags[0xe304]) { // DateTime: skip 1, BCD yyyy mm dd hh mm ss
        const d = tags[0xe304][0], hx = (o, n) => { let s = ''; for (let i = 0; i < n; i++)
            s += dv.getUint8(o + i).toString(16).padStart(2, '0'); return s; };
        const yr = hx(d + 1, 2), mo = hx(d + 3, 1), da = hx(d + 4, 1), hh = hx(d + 5, 1), mm = hx(d + 6, 1), ss = hx(d + 7, 1);
        if (/^\d{4}$/.test(yr))
            f['Recorded'] = `${yr}-${mo}-${da} ${hh}:${mm}:${ss}`;
    }
    return f;
}
...
async function openRtmd(file) {
    ...
    for (const trak of findAllBoxes(dv, 8, moov.size, 'trak')) {
        ...
        if (entry + 8 > moov.size)
            continue;
        if (fcc(dv, entry + 4) !== 'rtmd')
            continue;
        const table = sampleTable(dv, ts, te, file.size);
        if (table && table.samples.length)
            return table;
    }
    return null;
}
...
async function readImuSample(file, off, len) {
    let buf;
    try {
        buf = new Uint8Array(await file.slice(off, off + len).arrayBuffer());
    }
    catch (_) {
        return null;
    }
...
export async function extractSonyGyro(file) {
    ...
    for (let fi = 0; fi < total; fi += stride) {
        ...
        for (let i = 0; i < n; i += sub) {
            ...
            const gi = g && g[i];
            const ai = a && a[i];
            gyro.x.push(gi ? gi[0] : 0);
            gyro.y.push(gi ? gi[1] : 0);
            gyro.z.push(gi ? gi[2] : 0);
            accel.x.push(ai ? ai[0] / ACCEL_LSB_PER_G : 0);
            accel.y.push(ai ? ai[1] / ACCEL_LSB_PER_G : 0);
            accel.z.push(ai ? ai[2] / ACCEL_LSB_PER_G : 0);
        }
    }
    if (!hasGyro && !hasAccel)
        return null;
...
const range = (arrs) => { let lo = Infinity, hi = -Infinity; for (const a of arrs)
    for (const v of a) {
        if (v < lo)
            lo = v;
        if (v > hi)
...
function drawGyroCanvas(canvas, d, trackW, H, duration) {
    ...
    const regions = [];
    if (d.hasGyro)
        regions.push({ top: 0, h: both ? H / 2 : H, series: [d.gyro.x, d.gyro.y, d.gyro.z], name: d.gyroName || 'Gyroscope (raw counts)' });
    if (d.hasAccel)
        regions.push({ top: both ? H / 2 : 0, h: both ? H / 2 : H, series: [d.accel.x, d.accel.y, d.accel.z], name: d.accelName || 'Accelerometer (g)' });
    ...
    for (const r of regions) {
        ...
        if (r.top > 0) {
            ...
            ctx.lineWidth = 1;
            ctx.beginPath();
            ctx.moveTo(0, r.top + 0.5);
            ctx.lineTo(trackW, r.top + 0.5);
            ctx.stroke();
        }
        ...
        for (const tk of ticks) {
            ...
            ctx.beginPath();
            ctx.moveTo(gx, padT);
            ctx.lineTo(gx, padB + 2);
            ctx.stroke();
            ctx.fillStyle = 'rgba(255,255,255,.4)';
            ctx.fillText(fmtSec(tk), gx + 3, r.top + r.h - 3);
        }
        // zero baseline
        if (ylo < 0 && yhi > 0) {
            ...
            ctx.beginPath();
            ctx.moveTo(0, y(0));
            ctx.lineTo(trackW, y(0));
            ctx.stroke();
        }
        // series polylines
        for (let si = 0; si < 3; si++) {
            const data = r.series[si];
            ctx.strokeStyle = TRACE_COLORS[si];
            ctx.lineWidth = 1;
            ctx.beginPath();
            for (let i = 0; i < data.length; i++) {
                const px = x(d.t[i]), py = y(data[i]);
                if (i === 0)
                    ctx.moveTo(px, py);
                else
                    ctx.lineTo(px, py);
...
export function buildImuTimeline(d, file) {
    ...
    if (file) {
        ...
        video.addEventListener('click', () => { if (video.paused)
            video.play();
        else
            video.pause(); });
    ...
    const clock = () => {
        if (!video)
            return null;
        const a = audioClock();
        if (a && (a.currentTime > 0.01 || video.currentTime < 0.01))
            return a;
        return video;
    };
    ...
    function place(follow) {
        ...
        if (follow && video && !video.paused) {
            ...
            if (x < viewLeft + 20 || x > viewRight - 20)
                scroller.scrollLeft = parked;
            playhead.style.left = x + 'px';
        }
    }
    ...
    if (video) {
        const loop = () => { place(true); if (!video.paused && !video.ended)
            requestAnimationFrame(loop); };
        video.addEventListener('play', () => requestAnimationFrame(loop));
        video.addEventListener('timeupdate', () => { if (video.paused)
            place(false); });
    ...
    scroller.addEventListener('wheel', (e) => { if (e.ctrlKey || e.metaKey) {
        e.preventDefault();
        setZoom(zoom * (e.deltaY < 0 ? 1.2 : 1 / 1.2), e.clientX);
    } }, { passive: false });
    ...
    if (video) {
        ...
        const seekAt = (clientX, precise) => {
            ...
            if (transportFill)
                setPlayerFill(transportFill, frac);
        };
        scroller.addEventListener('pointerdown', (e) => { scrubbing = true; try {
            scroller.setPointerCapture(e.pointerId);
        }
        catch (_) { } seekAt(e.clientX, false); });
        scroller.addEventListener('pointermove', (e) => { if (scrubbing)
            seekAt(e.clientX, false); });
        const endScrub = (e) => {
            if (scrubbing && typeof video.fastSeek === 'function') {
                try {
    ...
    if (typeof ResizeObserver !== 'undefined') {
        let lw = -1;
        new ResizeObserver(() => { if (scroller.clientWidth !== lw) {
            lw = scroller.clientWidth;
            layout();
        } }).observe(scroller);
    }
...
export function buildGyroCard(d, file, playFile = file) {
    ...
    const [h, help] = h3help('Gyro & motion metadata', 'Sony cameras save a small burst of motion-sensor readings for every frame - from a gyroscope (which senses rotation) and an accelerometer (which senses movement and tilt) - inside the MP4 video file’s "rtmd" timed-metadata track (metadata tied to the timeline). '
    ...
    tbl.appendChild(rowHelp('Gyroscope', d.hasGyro ? 'Present' : 'Not found', 'How fast the camera is rotating, measured on three axes (pitch, roll and yaw - tilting up and down, rocking side to side, and turning left and right). Used to steady footage in Gyroflow and Catalyst Browse.'));
    tbl.appendChild(rowHelp('Accelerometer', d.hasAccel ? 'Present' : 'Not found', 'The camera’s acceleration, measured on three axes. When held still, one axis reads about 1 g - the constant pull of gravity.'));
    tbl.appendChild(row('Frames', d.frames.toLocaleString() + (d.sampled < d.frames ? ` (${d.sampled.toLocaleString()} sampled)` : '')));
    if (d.perFrame)
        tbl.appendChild(rowHelp('IMU samples / frame', String(d.perFrame), 'How many motion-sensor readings the camera packs into each video frame.'));
    if (d.imuRate)
        tbl.appendChild(rowHelp('IMU rate', '~' + Math.round(d.imuRate).toLocaleString() + ' Hz', 'Roughly how many motion-sensor readings are taken per second (samples per frame × frames per second).'));
    if (d.fps)
        tbl.appendChild(row('Frame rate', (Math.round(d.fps * 100) / 100) + ' fps'));
    if (d.durationSec)
        tbl.appendChild(row('Duration', d.durationSec.toFixed(1) + ' s'));
    for (const k in d.scalars)
        tbl.appendChild(row(k, d.scalars[k]));
    ...
    if (file && (d.hasGyro || d.hasAccel)) {
        ...
        const run = async (btn, kind) => {
            ...
            try {
                ...
                if (!cached) {
                    status.textContent = 'no IMU data to export';
                    return;
                }
                if (kind === 'csv')
                    downloadText(baseName + '-gyro.csv', toCSV(cached));
                else
                    downloadText(baseName + '.gcsv', toGCSV(cached, baseName));
        ...
        const [exH, exHp] = h3help('Export motion data', 'CSV holds the raw gyro plus accelerometer in g for any tool or spreadsheet. The .gcsv loads into Gyroflow and other stabilisers, '
        ...
        card.appendChild(el('p', { class: 'anr-hint', style: 'margin-top:6px' }, 'CSV opens in any tool; the .gcsv loads into Gyroflow for stabilisation.'));
    }
    return card;
}
...
export async function appendSonyGyroCard(file, resultsEl, playFile) {
    try {
        ...
        if (d)
            resultsEl.appendChild(buildGyroCard(d, file, playFile || file));
        return !!d;
    }
    catch (_) {
        return false;
...
//# sourceMappingURL=sony-rtmd.js.map