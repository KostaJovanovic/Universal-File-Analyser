...
function imageToMagnitude(imageData, o, ear) {
    ...
    for (let b = 0; b < bins; b++) {
        ...
        if (hz < o.minHz || hz > o.maxHz) {
            active[b] = 0;
            continue;
        }
        let frac; // 0 at minHz (bottom), 1 at maxHz (top)
        if (o.scale === 'log')
            frac = (Math.log10(Math.max(1, hz)) - logMin) / (logMax - logMin);
        else
            frac = (hz - o.minHz) / (o.maxHz - o.minHz);
    ...
    const toMag = (t) => {
        let v = t < 0 ? 0 : t > 1 ? 1 : t;
        if (o.invert)
            v = 1 - v; // negative: dark becomes loud, light becomes quiet
...
async function synthOscillator(mag, o, onProgress) {
    ...
    for (let b = 0; b < bins; b++) {
        ...
        if (fr <= 0 || fr >= o.sampleRate / 2)
            continue;
        let any = false;
        for (let f = 0; f < frames; f++) {
            if (mag[f * bins + b] > 1e-4) {
                any = true;
                break;
            }
        }
...
function stftPhase(samples, o, win) {
    ...
    for (let f = 0; f < frames; f++) {
        const start = f * hop;
        for (let n = 0; n < N; n++) {
            const s = start + n < samples.length ? samples[start + n] : 0;
            re[n] = s * win[n];
            im[n] = 0;
        }
        fft(re, im);
        for (let b = 0; b <= half; b++)
            phase[f * pStride + b] = Math.atan2(im[b], re[b]);
    }
    return phase;
}
...
function encodeWav(channels, sampleRate) {
    ...
    const wr = (off, s) => { for (let i = 0; i < s.length; i++)
...
function rangeCtl(label, min, max, step, value, fmt, editable) {
    ...
    if (editable)
        out.style.cssText = 'cursor:text;text-decoration:underline dotted;text-underline-offset:2px;';
...
function makeToggle(options, value) {
    ...
    for (const [v, t] of options) {
        ...
        b.addEventListener('click', () => { wrap._value = v; for (const x of wrap.children)
...
const SETTINGS_HELP = 'Analyser reads your picture as a spectrogram: left to right is time, bottom to top is pitch, and how bright a spot is sets how loud that pitch sounds at that moment. Everything runs on your own device.<br><br>' +
...
export async function renderSonify(file, mountEl, opts = {}) {
    ...
    if (!source) {
        try {
            ...
            card.appendChild(el('p', { class: 'anr-hint' }, 'Could not decode this image for sonification.'));
            return;
        }
    }
    const { imageData, resized, srcW, srcH, w: workW, h: workH } = sourceToImageData(source, MAX_COLS, MAX_ROWS);
    if (resized)
        card.appendChild(el('p', { class: 'anr-hint' }, `Resized for synthesis: ${srcW} x ${srcH} to ${workW} x ${workH}.`));
    ...
    function cubeValue(r, g, b) {
        ...
        if (cubeName !== name) {
            cubeName = name;
            cubeLut = new Float32Array(CUBE * CUBE * CUBE);
            const inv = makeColormapInverter(name);
            for (let ri = 0; ri < CUBE; ri++)
                for (let gi = 0; gi < CUBE; gi++)
                    for (let bi = 0; bi < CUBE; bi++)
    ...
    function redrawImage() {
        ...
        if (colour)
            for (let i = 0; i < 256; i++)
                gammaLut[i] = Math.round(Math.pow(i / 255, g) * 255);
        ...
        for (let i = 0; i < src.length; i += 4) {
            ...
            let d;
            if (spectro && dbInvert) {
                const db = DEFAULTS.dbFloor + t * (DEFAULTS.dbCeil - DEFAULTS.dbFloor);
                d = Math.pow(10, db / 20) / ceilAmp;
            }
            else
                d = g !== 1 ? Math.pow(t, g) : t;
    ...
    for (const c of [modeSel, leftSel, cmapSel])
        c.addEventListener('change', redrawImage);
    for (const t of [invTog, dbTog])
        t.addEventListener('click', redrawImage);
    redrawImage();
    ...
    function stopCursor() { if (cursorRaf) {
    ...
    async function doRender() {
        ...
        try {
            ...
            if (channels.length === 2) {
                ...
                for (let c = 0; c < 2; c++)
                    if (channels[c].length < len) {
                        const padded = new Float32Array(len);
                        padded.set(channels[c]);
                        channels[c] = padded;
                    }
            }
    ...
    if (opts.signal)
        opts.signal.addEventListener('abort', teardownAudio, { once: true });
}
