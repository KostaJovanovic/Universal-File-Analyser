...
import { renderTracker } from '../lib/openmpt-loader.js';
...
function nameList(title, list, total) {
...
/** Render a tracker module: the tracker card, then the full audio analysis of
    the rendered song. */
export async function renderTrackerModule(file, resultsEl) {
    ...
    loader.remove();
    if (!result) {
        resultsEl.appendChild(el('div', { class: 'anr-info' }, failure
    ...
    const [h, help] = h3help('Tracker module', 'A module stores instrument samples plus a pattern score, not recorded audio. The sound below is rendered from that score on your device by libopenmpt - the same engine OpenMPT uses - so it is a performance of the file, not a decode of it.');
    card.appendChild(h);
    card.appendChild(help);
    const tbl = el('table', { class: 'anr-table' });
    if (info.title)
        tbl.appendChild(row('Title', info.title));
    if (info.artist)
        tbl.appendChild(row('Artist', info.artist));
    tbl.appendChild(row('Format', info.typeLong || info.type || 'tracker module'));
    if (info.tracker)
        tbl.appendChild(row('Made with', info.tracker));
    ...
    if (info.instruments.length)
        tbl.appendChild(row('Instruments', info.instruments.length + (namedInst ? ' (' + namedInst + ' named)' : '')));
    if (info.samples.length)
        tbl.appendChild(row('Samples', info.samples.length + (namedSamp ? ' (' + namedSamp + ' named)' : '')));
    tbl.appendChild(row('File size', fmtBytes(file.size)));
    card.appendChild(tbl);
    if (truncated) {
        card.appendChild(el('div', { class: 'anr-info' }, 'This module runs longer than the render ceiling (or loops without end), so the audio below is the first '
