...
function cellText(v) {
    ...
    if (typeof v === 'object') {
        if (v.length != null)
            return '[' + v.length + ' bytes]'; // Buffer/Uint8Array (OLE/attachment)
...
export async function renderMdb(file, resultsEl) {
    ...
    try {
        ...
        try {
            created = reader.getCreationDate();
        }
        catch (_) {
            created = null;
        }
    ...
    if (created) {
        try {
            metaTbl.appendChild(row('Created', created.toISOString().slice(0, 19).replace('T', ' ')));
        }
        catch (_) { }
    }
    ...
    const tables = names.map((name) => {
        try {
            ...
            return { name, cols: [], rowCount: 0, table: null };
        }
    });
    ...
    function renderTable(idx) {
        ...
        if (!t.table || !t.cols.length) {
            wrap.appendChild(el('p', { class: 'anr-hint' }, 'This table could not be read.'));
            return;
        }
        let data = [];
        try {
            data = t.table.getData({ rowLimit: ROW_CAP });
        }
        catch (_) {
            data = [];
        }
        ...
        for (const c of t.cols)
            thead.appendChild(el('th', {}, c));
        table.appendChild(thead);
        data.forEach((r, i) => {
            const tr = el('tr', {}, [el('th', { class: 'anr-xlsx-rownum' }, String(i + 1))]);
            for (const c of t.cols)
                tr.appendChild(el('td', {}, cellText(r[c])));
            table.appendChild(tr);
        });
        wrap.appendChild(table);
        if (t.rowCount > ROW_CAP)
            wrap.appendChild(el('p', { class: 'anr-hint', style: 'margin-top:8px;' }, `Showing the first ${ROW_CAP} of ${t.rowCount.toLocaleString()} rows.`));
    }
    tables.forEach((t, i) => {
