...
export async function renderF3d(file, resultsEl) {
    ...
    if (subType)
        t.appendChild(row('Subtype', SUBTYPES[subType] || prettify(subType)));
    if (docVer)
        t.appendChild(row('Fusion document version', docVer));
    if (structVer)
        t.appendChild(row('Document structure', 'v' + structVer));
    if (bodies)
        t.appendChild(rowHelp('Solid bodies', String(bodies), 'The number of distinct solid shapes that make up the design. A simple part is often a single body, but a design can hold several separate solids.'));
    if (appearances)
        t.appendChild(rowHelp('Appearance assets', String(appearances), 'The number of surface finishes stored in the design - the colours, metals, plastics and other materials Fusion 360 (which calls them appearances) applies to give surfaces their look.'));
    ...
    const [nh, nhelp] = h3help('About the geometry', 'A Fusion 360 design stores its solid model as Autodesk ShapeManager BREP data'
    ...
    note.appendChild(el('p', { class: 'anr-hint', style: 'margin:0;' }, 'To open it as a viewable mesh, export it from Fusion 360 as STL, OBJ, STEP or 3MF and drop that here for the full 3D viewer.'));
