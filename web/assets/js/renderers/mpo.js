...
export function parseMpfEntries(buf) {
    ...
    while (p + 4 <= n) {
        ...
        if (marker === 0xD9 || marker === 0xDA)
            break; // EOI / start of scan - no MPF
    ...
    if (bo === 0x4949)
        ...
        return null;
    if (dv.getUint16(mpStart + 2, le) !== 0x002A)
        return null;
    ...
    for (let i = 0; i < count; i++) {
        ...
        if (tag === 0xB001)
            numImages = dv.getUint32(e + 8, le); // NumberOfImages
        else if (tag === 0xB002)
            entryOffset = mpStart + dv.getUint32(e + 8, le); // MP Entry -> offset
        void valCount;
    }
    if (entryOffset < 0)
        return null;
    if (!numImages || numImages > 64)
        numImages = 0; // sanity / unknown
    const ranges = [];
    ...
    for (let i = 0; i < maxEntries; i++) {
        ...
        if (fileOff < 0 || fileOff + size > n) {
            if (numImages)
                continue;
            else
                break;
        }
        if (!(bytes[fileOff] === 0xFF && bytes[fileOff + 1] === 0xD8)) {
            if (numImages)
                continue;
            else
                break;
