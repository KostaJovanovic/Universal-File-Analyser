...
export function makePlayer(mediaEl, knownDuration, opts = {}) {
    ...
    function dur() {
        if (opts.preferKnown && typeof knownDuration === 'number' && isFinite(knownDuration) && knownDuration > 0)
    ...
    const unregisterVol = opts.noVolume && !opts.mutedPreview
        ...
        : null;
    if (opts.signal && unregisterVol)
        opts.signal.addEventListener('abort', unregisterVol, { once: true });
    function playMedia() {
    ...
    mediaEl.addEventListener('play', () => {
        if (controller)
            return; // the controller owns the glyph/clock while attached
        playBtn.textContent = '❚❚';
    ...
    mediaEl.addEventListener('pause', () => { if (!controller && !mediaEl.ended) {
        playBtn.textContent = '▶';
    ...
    function onMouseMove(e) { if (dragging) {
        scrub(e.clientX);
        tick();
    } }
    function onMouseUp() {
    ...
    function onTouchMove(e) { if (dragging && e.touches[0]) {
    ...
    if (opts.signal)
        opts.signal.addEventListener('abort', () => { onMouseUp(); onTouchEnd(); }, { once: true });
    let lastLabel = '';
    function tick() {
        if (controller)
            return; // while delegated, the controller pushes UI via update()
        const d = dur();
...
export function playerAudioNode(mediaEl) {
    if (mediaEl._anrAudioNode)
        return mediaEl._anrAudioNode;
    try {
        ...
        const resume = () => { if (ctx.state === 'suspended')
            ctx.resume().catch(() => { }); };
...
function applyVolumeTo(mediaEl, level) {
    ...
    if (!node && level > 1)
        node = playerAudioNode(mediaEl);
    try {
        if (node) {
            mediaEl.volume = 1;
            node.boostGain.gain.value = level;
        }
        else
            mediaEl.volume = Math.min(1, level);
    }
    catch (_) { }
}
...
function applyShared() {
    ...
    if (comp) {
        try {
            applyVolumeTo(comp, sharedVol);
            comp.muted = sharedMuted;
        }
        catch (_) { }
    }
    for (const p of volPlayers) {
        ...
        try {
            ...
            if (isSynced(p.mediaEl)) {
                ...
                if (owner)
                    p.mediaEl.muted = sharedMuted || (p.mediaEl !== owner);
            }
            else {
...
function setShared(v, m) {
    ...
    if (sharedVol > 0)
        lastNonZero = sharedVol;
    sharedMuted = !!m;
    // Deliberately not persisted - see the load block above (every load starts 100%).
    applyShared();
    for (const cb of volListeners) {
        try {
            cb(sharedVol, sharedMuted);
        }
        catch (_) { }
    }
}
...
function makeVolume(mediaEl, signal) {
    ...
    function open() { if (closeTimer) {
        clearTimeout(closeTimer);
        closeTimer = null;
    } wrap.classList.add('is-open'); }
    function closeNow() { if (!dragging)
    ...
    muteBtn.addEventListener('click', () => {
        if (!IS_IOS && !isOpen()) {
            open();
            return;
        } // touch first tap: reveal only
        if (!sharedMuted && sharedVol > 0)
            setShared(sharedVol, true); // mute
        else if (sharedMuted)
            setShared(sharedVol, false); // unmute, keep level
        else
            setShared(lastNonZero, false); // was 0 -> restore
        open();
    });
    function setVol(clientY) {
        ...
        let v = frac * MAX_VOL;
        if (Math.abs(v - 1) <= 0.06)
            v = 1; // detent at the 100% slit (see the ::after tick)
        setShared(v, false);
    }
    ...
    function onTMove(e) { if (dragging && e.touches[0]) {
        setVol(e.touches[0].clientY);
        e.preventDefault();
    } }
    function onTEnd() {
        dragging = false;
    ...
    if (signal)
        signal.addEventListener('abort', () => { onUp(); onTEnd(); }, { once: true });
    ...
    mediaEl.addEventListener('volumechange', () => {
        ...
        if (Math.abs(mediaEl.volume - sharedVol) > 0.001 || muteChanged)
            setShared(mediaEl.volume, isSynced(mediaEl) ? sharedMuted : mediaEl.muted);
        else
            sync();
