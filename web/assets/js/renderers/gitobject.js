...
export async function sniffGitObject(file) {
    ...
    if (head[0] === 0x50 && head[1] === 0x41 && head[2] === 0x43 && head[3] === 0x4B)
        return 'pack'; // "PACK"
    if (head[0] === 0xFF && head[1] === 0x74 && head[2] === 0x4F && head[3] === 0x63)
        return 'idx'; // \377tOc
    if (hasInflate() && (head[0] & 0x0f) === 8 && ((((head[0] << 8) | head[1]) % 31) === 0)) { // zlib stream
        try {
            ...
            if (sp > 0 && nul > sp && TYPES.has(s.slice(0, sp)) && /^\d+$/.test(s.slice(sp + 1, nul)))
                return 'loose';
        }
        catch (_) { }
    }
...
function parseTree(content) {
    ...
    while (i < content.length && entries.length < 100000) {
        let sp = i;
        while (sp < content.length && content[sp] !== 0x20)
            sp++;
        if (sp >= content.length)
            break;
        let nul = sp + 1;
        while (nul < content.length && content[nul] !== 0x00)
            nul++;
        if (nul + 21 > content.length)
            break;
...
function renderBlobObject(file, content, resultsEl) {
    ...
    if (textual) {
        ...
        c.appendChild(el('p', { class: 'anr-hint', style: 'margin:0 0 10px;' }, 'Binary blob - not readable as text.'));
    }
...
async function renderPackOrIdx(file, resultsEl, kind) {
    ...
    const version = dv.getUint32(4, false);
    let count = 0;
    if (version === 2 && buf.length >= 8 + 256 * 4)
        count = dv.getUint32(8 + 255 * 4, false);
...
export async function renderGitObject(file, resultsEl) {
    ...
    if (kind === 'pack' || kind === 'idx') {
        await renderPackOrIdx(file, resultsEl, kind);
        return;
    }
    if (kind !== 'loose') {
    ...
    let nul = 0;
    while (nul < data.length && data[nul] !== 0x00)
        nul++;
    ...
    if (capped) {
        rows.push(rowHelp('⚠ Truncated', 'Only the first ' + fmtBytes(DECOMP_OUTPUT_MAX) + ' was unpacked', 'This object unpacks to more than the browser can safely hold, so only its beginning is shown here and no SHA-1 is worked out.'));
    }
    if (sha) {
        ...
        if (fn.length === 38 && sha.slice(2) === fn)
            rows.push(row('Filename match', 'Yes - the filename is this object hash'));
    }
    resultsEl.appendChild(card('Git ' + (type || 'object'), rows));
    if (type === 'commit' || type === 'tag')
        renderTextObject(content, type, resultsEl);
    else if (type === 'tree')
        renderTreeObject(content, resultsEl);
    else
        renderBlobObject(file, content, resultsEl);
}
