...
export async function renderDiskImage(file, resultsEl, opts = {}) {
    ...
    if (file.size > WALL_INDEX) {
        resultsEl.innerHTML = '';
        resultsEl.appendChild(errorCard('This disk image is ' + fmtBytes(file.size) + ' - too large to open in the browser without exhausting memory. The file was not read.'));
        return;
    }
    ...
    try {
        ...
        if (candidate < 0) {
            ...
            for (;;) {
                ...
                if (vol)
                    volStart = candidate;
                if (prefix >= file.size || (vol && !vol.shortRead))
                    break;
    ...
    if (!vol || !vol.entries.length) {
        ...
        if (otherFs)
            t.appendChild(rowHelp('Filesystem', otherFs, 'The way files are organised on the disc inside this image (its filesystem). Analyser can currently open and browse only FAT12, FAT16 and FAT32 discs; this one is recognised but not opened.'));
        note.appendChild(t);
    ...
    if (vol.volumeLabel)
        summaryRows.push(row('Volume label', vol.volumeLabel));
    ...
    if (vol.capacityBytes) {
        ...
        summaryRows.push(rowHelp('Used space', fmtBytes(vol.capacityBytes - vol.freeBytes) + ' of ' + fmtBytes(vol.capacityBytes) + '  (' + usedPct + '%)', 'How much of the disc’s space is taken up, worked out from how many storage chunks the disc lists as still free.'));
    }
    if (vol.truncated)
        summaryRows.push(rowHelp('⚠ Listing truncated', 'over ' + MAX_ENTRIES.toLocaleString() + ' entries', 'The disc holds more files and folders than are shown here; the list was capped to keep the page quick to use.'));
...
function carvedImageGallery(readFull, file, resultsEl, vol) {
    ...
    body.appendChild(el('p', { class: 'anr-hint', style: 'margin:0 0 12px;' }, 'Recover deleted and orphaned images from the raw sectors. It reads the whole image, so it can take a few seconds.'));
    ...
    scanBtn.addEventListener('click', async () => {
        ...
        let carved = [];
        try {
            carved = carveImages(img, { max: SCAN_CARVE });
        }
        catch (_) {
            carved = [];
        }
        ...
        carved = carved.filter((c) => {
            ...
            if (len < 512 || len > MAX_CARVE_BYTES)
                return false;
            if (c.format === 'bmp' && !plausibleBmp(img, c.start))
                return false;
            return true;
        });
        body.innerHTML = '';
        ...
        for (const c of list) {
            const raw = carveExifDate(img, c.start, c.end);
            c._dateFmt = fmtExifDate(raw);
            c._date = c._dateFmt ? raw : null;
        }
        ...
        if (frameShown) {
            let hidden = false;
            const toggle = el('button', { type: 'button', class: 'anr-btn anr-btn-sm', style: 'margin:0;' }, 'Hide frames & thumbnails (' + frameShown + ')');
            toggle.addEventListener('click', () => {
                hidden = !hidden;
                for (const cell of grid.children) {
                    if (cell.dataset && cell.dataset.frame === '1')
        ...
        for (let k = 0; k < list.length; k++) {
            const cell = carveCell(img, file, list[k], k, io, vol);
            cells.push({ c: list[k], cell });
        ...
        const applySort = (mode) => {
            ...
            arr.sort(mode === 'size' ? bySize : (a, b) => {
                ...
                if (da && db)
                    return da === db ? bySize(a, b) : (newest ? (da < db ? 1 : -1) : (da < db ? -1 : 1));
                if (da)
                    return -1;
                if (db)
                    return 1;
...
function showCarveInLightbox(pos, n, c, f, img, file, vol, url) {
...
function analyseCarve(img, c, idx, file, vol) {
    ...
    if (window._anrPushNav)
        window._anrPushNav(file.name || 'disk image', () => { if (window._anrHandleFile)
...
function carvedFile(img, c, idx, vol) {
    ...
    return new File([sub], 'carved_' + String(idx + 1).padStart(4, '0') + '_0x' + c.start.toString(16) + '.' + ext, { type: CARVE_MIME[c.format] || 'application/octet-stream' });
}
...
function recoverViaChain(img, vol, c) {
    const g = vol.geom;
    if (!g || (g.type !== 'FAT12' && g.type !== 'FAT16' && g.type !== 'FAT32'))
        return null;
    const dataStart = g.partStart + g.firstDataSector * g.bps;
    if (c.start < dataStart)
        return null;
    const rel = c.start - dataStart;
    if (rel % g.bytesPerCluster !== 0)
        return null; // signature not at a cluster start
    const startCl = rel / g.bytesPerCluster + 2;
    const next = (cl) => {
        if (g.type === 'FAT16') {
            const o = g.fatStart + cl * 2;
            return (o + 1 < img.length) ? (img[o] | (img[o + 1] << 8)) : 0x0FFFFFFF;
        }
        if (g.type === 'FAT32') {
            const o = g.fatStart + cl * 4;
            return (o + 3 < img.length) ? ((img[o] | (img[o + 1] << 8) | (img[o + 2] << 16) | (img[o + 3] << 24)) & 0x0FFFFFFF) : 0x0FFFFFFF;
        }
        const o = g.fatStart + Math.floor(cl * 3 / 2);
        if (o + 1 >= img.length)
            return 0x0FFFFFFF;
    ...
    while (cur >= 2 && len < maxLen) {
        if (seen.has(cur))
            return null; // loop -> untrustworthy
        seen.add(cur);
        ...
        if (nx < 2)
            return null; // free/bad -> deleted, chain lost
        cur = nx;
    }
    if (!terminated || len <= 1 || contiguous)
        return null;
...
function carveCell(img, file, c, idx, io, vol) {
    ...
    if (c.frame)
        cell.dataset.frame = '1';
    const thumb = el('div', { class: 'anr-carve-thumb', title: 'Click to view full size' }, el('span', { class: 'anr-hint', style: 'margin:0;' }, c.format.toUpperCase()));
    ...
    thumb.addEventListener('click', () => { if (!c.undecodable)
        openCarveLightboxAt(idx); });
    ...
    const dims = (c.width && c.height) ? c.width + ' × ' + c.height : '';
    cell.appendChild(el('div', { class: 'anr-carve-cap' }, c.format.toUpperCase() + (dims ? '  ' + dims : '') + (c.complete ? '' : '  · partial') + (c.frame ? '  · frame' : '')));
    cell.appendChild(el('div', { class: 'anr-carve-cap', style: 'margin-top:2px;' }, 'offset 0x' + c.start.toString(16) + ' · ' + fmtBytes(c.end - c.start)));
    if (c._dateFmt)
        cell.appendChild(el('div', { class: 'anr-carve-cap', style: 'margin-top:2px;' }, c._dateFmt));
    io.observe(thumb);
    return cell;
}
...
function carveExifDate(img, start, end) {
    ...
    if (start + 4 > lim || img[start] !== 0xFF || img[start + 1] !== 0xD8)
        return null;
    ...
    for (let i = start + 2; i + 6 < lim; i++) {
        if (img[i] === 0x45 && img[i + 1] === 0x78 && img[i + 2] === 0x69 && img[i + 3] === 0x66 && img[i + 4] === 0 && img[i + 5] === 0) {
            tiff = i + 6;
            break;
        }
    }
    if (tiff < 0 || tiff + 8 > lim)
        return null;
    ...
    if (u16(tiff + 2) !== 0x002A)
        return null;
    const readStr = (o, cnt) => { let s = ''; for (let j = 0; j < cnt - 1 && o + j < lim; j++) {
        const ch = img[o + j];
        if (!ch)
            break;
    ...
    const readIFD = (ifdOff, wantTag) => {
        ...
        for (let k = 0; k < n; k++) {
            const ent = base + 2 + k * 12;
            if (ent + 12 > lim)
                break;
            const tag = u16(ent), type = u16(ent + 2), cnt = u32(ent + 4);
            if (tag === 0x8769)
                exifPtr = u32(ent + 8);
            else if (tag === wantTag && type === 2 && cnt >= 8) {
                const vo = cnt <= 4 ? ent + 8 : tiff + u32(ent + 8);
                dt = readStr(vo, cnt);
            }
        }
        return { exifPtr, dt };
    ...
    if (ifd0.exifPtr) {
        const ex = readIFD(ifd0.exifPtr, 0x9003);
        if (ex.dt)
            return ex.dt;
    }
    return ifd0.dt || null;
    ...
    return new Promise((resolve) => {
        ...
        im.onload = () => {
            ...
            if (frac >= 0.99 && format === 'jpeg') { // browser drew nothing - salvage may still recover a strip or the thumbnail
                ...
                if (s) {
                    done(s.canvas, s.cat, true, s.thumb, s.corrupt);
                    return;
                }
            }
            ...
            if (format === 'jpeg' && frac < 0.99) {
                try {
                    corrupt = detectCorruptCut(ctx.getImageData(0, 0, cw, ch).data, cw, ch) >= 0;
                }
                catch (_) { }
            }
...
function renderCarveThumb(thumbEl) {
    ...
    return decodeCarveToCanvas(sub, c.format, CARVE_THUMB_EDGE).then(({ canvas, cat, salvaged, thumb, corrupt }) => {
        c._cat = cat;
        c._decoded = cat !== 'none';
        c.undecodable = cat === 'none';
        c._salvaged = salvaged;
        c._thumb = thumb;
        c._corrupt = corrupt;
        if (!canvas) {
            // Nothing at all decodes - keep the text placeholder, but the cell stays in
            // the gallery; nothing is removed.
            thumbEl.classList.add('is-plain');
            thumbEl.title = '';
            if (placeholder)
                placeholder.textContent = c.format.toUpperCase() + ' · no preview';
        }
        else {
            if (placeholder)
            ...
            if (thumb)
                thumbEl.title = 'Embedded thumbnail - the full-size image was overwritten';
            else if (corrupt)
                thumbEl.title = 'Only the top strip is the real picture - below the break the scan is corrupt (decoder noise, not the actual image)';
            else if (cat !== 'ok')
                thumbEl.title = 'Click to view full size (' + cat + ' - recovered data is incomplete)';
        }
    });
}
