...
export function makeFft(N) {
    ...
    for (let k1 = 0; k1 < N1; k1++) {
        DR.push(new Float64Array(N1));
        DI.push(new Float64Array(N1));
        for (let n1 = 0; n1 < N1; n1++) {
            const ang = -2 * Math.PI * (n1 * k1) / N1;
            DR[k1][n1] = Math.cos(ang);
            DI[k1][n1] = Math.sin(ang);
        }
    }
    // Outer twiddles TW[k1][n2] = exp(-2pi i n2 k1 / N) (forward sign).
    const TWR = [], TWI = [];
    for (let k1 = 0; k1 < N1; k1++) {
        TWR.push(new Float64Array(N2));
        TWI.push(new Float64Array(N2));
        for (let n2 = 0; n2 < N2; n2++) {
            const ang = -2 * Math.PI * (n2 * k1) / N;
            TWR[k1][n2] = Math.cos(ang);
            TWI[k1][n2] = Math.sin(ang);
        }
    }
    ...
    const br = [], bi = [];
    for (let k1 = 0; k1 < N1; k1++) {
        br.push(new Float64Array(N2));
        bi.push(new Float64Array(N2));
    }
    ...
    return function transform(inRe, inIm, inv) {
        ...
        for (let k1 = 0; k1 < N1; k1++)
            fftRadix2(br[k1], bi[k1], inv);
        // Recombine: X[N1*k2 + k1] = Y_k1[k2].
        for (let k2 = 0; k2 < N2; k2++) {
            const base = N1 * k2;
            for (let k1 = 0; k1 < N1; k1++) {
                outRe[base + k1] = br[k1][k2];
                outIm[base + k1] = bi[k1][k2];
            }
        }
...
export function makeStftEngine(nFft, hop) {
    ...
    function stft(signal) {
        ...
        if (padded.length !== paddedLen)
            padded = new Float64Array(paddedLen);
        else
            padded.fill(0);
        ...
        for (let mIdx = 0; mIdx < frames; mIdx++) {
            ...
            for (let i = 0; i < nFft; i++) {
                stftFr[i] = padded[off + i] * window[i];
                stftFi[i] = 0;
            }
            const spec = fft(stftFr, stftFi, false);
            const base = mIdx * nBins;
            for (let b = 0; b < nBins; b++) {
                stftRe[base + b] = spec.re[b];
                stftIm[base + b] = spec.im[b];
            }
        }
...
//# sourceMappingURL=mdx-stft.js.map