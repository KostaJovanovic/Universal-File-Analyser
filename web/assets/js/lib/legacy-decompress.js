...
function makeSink() {
    ...
    return {
        ...
        get pos() { return len; },
        byte(i) { return buf[i]; },
        copy(from, count) { this.ensure(count); let s = from; for (let k = 0; k < count; k++)
            buf[len++] = buf[s++]; },
...
export function unlz4(input) {
    try {
        ...
        if (((flg >> 6) & 0x03) !== 1)
            return null; // version must be 01
...
export function unlzw(input) {
    try {
        const b = input instanceof Uint8Array ? input : new Uint8Array(input);
        if (b.length < 3 || b[0] !== 0x1F || b[1] !== 0x9D)
            return null;
...
//# sourceMappingURL=legacy-decompress.js.map