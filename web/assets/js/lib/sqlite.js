...
export async function getSQL() {
    ...
    _sqlPromise = (async () => {
        try {
            if (!window.initSqlJs)
                await loadScript('assets/vendor/sqljs/sql-wasm.js');
            if (!window.initSqlJs)
                return null;
...
export async function sqliteQuery(file) {
    try {
        ...
        if (file && file.size > 300 * 1024 * 1024 && isLowMemoryDevice())
            return null;
...
export async function sqliteAnalysis(file) {
    ...
    try {
        ...
        const master = exec("SELECT type,name,sql FROM sqlite_master WHERE name NOT LIKE 'sqlite\\_%' ESCAPE '\\' ORDER BY type,name");
        ...
        if (biggest) {
            ...
            if (s)
                sample = { table: biggest.name, columns: s.columns, rows: s.values };
        }
