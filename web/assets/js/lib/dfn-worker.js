...
async function fetchWithProgress(url, expectedBytes, onProg) {
    ...
    for (;;) {
        ...
        if (onProg)
            onProg(Math.min(1, received / expectedBytes));
    }
...
async function ensureModel(report) {
    ...
    if (bytes) {
        ...
        bytes = await fetchWithProgress(DFN_MODEL.url, DFN_MODEL.bytes, (frac) => { if (report)
            report('model', frac); });
        if (report)
            report('cache', 0);
...
async function handleDenoise(msg) {
    ...
    try {
        ...
        const transfer = [];
        for (const a of result.clean)
            transfer.push(a.buffer);
        for (const a of result.noise)
            transfer.push(a.buffer);
        self.postMessage({
            type: 'done',
            clean: result.clean,
