...
function looksLikeDate(val) {
    if (YYYY_DOT_MM.test(val) || YYYY_SEP_MM_DD.test(val))
        return true;
...
export function parseDateValue(val, monthFirst) {
    ...
    let m;
    if ((m = YYYY_DOT_MM.exec(s)))
        return Date.UTC(+m[1], +m[2] - 1, 1);
    if ((m = YYYY_SEP_MM_DD.exec(s)))
...
export function describe(nums) {
    const n = nums.length;
    if (n === 0)
        return { count: 0, sum: 0, mean: NaN, min: NaN, max: NaN, median: NaN, stddev: NaN, distinct: 0 };
...
export function groupBy(rows, keyCol, valCol, agg) {
    ...
    for (const key of order) {
        ...
        if (agg === 'count')
            ...
            value = percentile(vals.slice().sort((a, b) => a - b), 0.5);
        out.push([key, value]);
    }
    return out;
}
//# sourceMappingURL=table-stats.js.map