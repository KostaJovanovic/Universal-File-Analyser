...
function getWorker() {
    if (!worker)
        worker = new Worker(new URL('./dfn-worker.js', import.meta.url), { type: 'module' });
    return worker;
}
function abortError() {
...
function runWorker(channels, sampleRate, { onProgress, signal }) {
    ...
    return new Promise((resolve, reject) => {
        ...
        const armStallTimer = (phase) => {
            clearTimeout(stallTimer);
            stallTimer = setTimeout(() => failWorker(new Error('Denoise worker stopped responding during model initialisation or processing')), phase === 'runtime' ? RUNTIME_STALL_MS : STALL_MS);
        };
        const onMessage = (e) => {
            const msg = e.data;
        ...
        armStallTimer('');
        try {
            w.postMessage({ type: 'denoise', channels, sampleRate, jobId }, channels.map((channel) => channel.buffer));
        }
        catch (err) {
 ...
 */
export function enhanceAudio(audioBuffer, { onProgress, signal } = {}) {
...
//# sourceMappingURL=dfn-client.js.map