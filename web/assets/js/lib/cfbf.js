...
export async function openCfbf(input) {
    try {
        ...
        if (sectorShift !== 9 && sectorShift !== 12)
            return null; // 512 or 4096
        if (miniSectorShift !== 6)
            return null; // 64-byte mini-sectors
        ...
        const sectorOffset = (id) => {
            ...
            if (off < 0 || off + sectorSize > bytes.length + 1)
                return -1;
            return off;
        };
        ...
        const readMiniStream = (start, size) => {
            ...
            while (s !== ENDOFCHAIN && s <= MAXREGSECT && written < size && guard++ < cap) {
                ...
                if (base < 0 || base >= miniStream.length)
                    break;
        ...
        const readEntry = (e) => {
            if (!e || e.type !== T_STREAM)
                return null;
            if (e.size < cutoff)
                return readMiniStream(e.startSect, e.size);
        ...
        const assignPaths = (nodeIndex, prefix, depth) => {
            if (nodeIndex === NOSTREAM || nodeIndex >= rawEntries.length)
                return;
            if (depth > rawEntries.length)
                return; // safety
            const visit = (idx, seen) => {
                if (idx === NOSTREAM || idx >= rawEntries.length || seen.has(idx))
                    return;
                seen.add(idx);
