...
async function deleteCachedUrl(url) {
    try {
        ...
        await Promise.all(keys.map(async (key) => {
            try {
                await (await caches.open(key)).delete(url);
            }
            catch (_) { }
        }));
    }
...
async function fetchWithProgress(url, expectedBytes, onProgress) {
    ...
    while (true) {
        ...
        if (onProgress && progressTotal)
            onProgress(Math.min(1, received / progressTotal));
    }
...
async function ensureModel(model, report, forceWasm = false) {
    if (session && loadedModelId === model.id && (!forceWasm || loadedProvider === 'wasm'))
        return;
    releaseSession();
    let bytes = await cachedModel(model);
    if (bytes) {
        if (report)
            report('model-cache', 1);
    }
    else {
        bytes = await fetchWithProgress(model.url, model.bytes, (frac) => { if (report)
            report('model', frac); });
        if (report)
            report('cache', 0);
        const stored = await storeModel(model.url, bytes);
        if (report)
            report(stored ? 'cache' : 'cache-warning', stored ? 1 : 0);
    }
...
async function runModel(input, dims) {
    ...
    if (out.data && out.data.length)
        return out.data;
    if (typeof out.getData === 'function')
...
self.onmessage = (e) => {
    const msg = e.data;
    if (!msg || (msg.type !== 'prepare' && msg.type !== 'separate'))
        return;
