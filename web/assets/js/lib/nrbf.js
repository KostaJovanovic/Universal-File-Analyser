...
class Reader {
    ...
    need(n) { if (this.p + n > this.b.length)
        throw new Error('unexpected end of NRBF stream'); }
...
function utf8(bytes) {
    try {
        (_dec = _dec || new TextDecoder('utf-8'));
...
export function parseNrbf(bytes) {
    try {
        ...
        if (r.u8() !== REC.Header)
            return { ok: false, error: 'not a .NET BinaryFormatter stream' };
        const rootId = r.i32();
        r.i32(); // headerId (-1)
        ...
        let elements = 0;
        const spend = (n) => { elements += n; if (elements > NRBF_ELEMENT_MAX)
            throw new Error('object graph too large'); };
        // Objects with id 0 are never stored in the map but still count.
        const register = (id, val) => { if (++count > MAX_OBJECTS)
            throw new Error('object graph too large'); spend(1); if (id)
            objects.set(id, val); return val; };
        // Read the AdditionalInfo that follows a member's BinaryTypeEnum.
        function readAdditional(bt) {
            if (bt === 0 || bt === 7)
                return r.u8(); // Primitive / PrimitiveArray -> PrimitiveTypeEnum
            if (bt === 3)
                return r.str(); // SystemClass -> class name
            if (bt === 4) {
                const name = r.str();
                const lib = r.i32();
                return { name, lib };
        ...
        function readPrimitive(pt) {
            switch (pt) {
                ...
                case 12: {
                    const t = r.i64();
                    return typeof t === 'number' ? (t / 1e7) + ' s' : t + ' ticks';
                } // TimeSpan
                case 13: {
                    r.need(8);
                    const v = decodeDateTime(r.dv, r.p);
                    r.p += 8;
                    return v;
                } // DateTime
        ...
        function readMemberValue(m) {
            if (m.bt === 0)
                return readPrimitive(m.ai); // Primitive: inline, no record tag
        ...
        function readClassWithMembersAndTypes(system) {
            ...
            const n = r.i32();
            if (n < 0 || n > 4096)
                throw new Error('absurd member count');
            const memberNames = [];
            for (let i = 0; i < n; i++)
                memberNames.push(r.str());
        ...
        function fillArray(arr, length, readEl) {
            ...
            while (i < length) {
                ...
                if (peek === REC.ObjectNullMultiple256) {
                    r.u8();
                    const c = r.u8();
                    spend(Math.min(c, length - i));
                    for (let k = 0; k < c && i < length; k++, i++)
                        arr.push(null);
                    continue;
                }
                if (peek === REC.ObjectNullMultiple) {
                    r.u8();
                    const c = r.i32();
                    spend(Math.max(0, Math.min(c, length - i)));
                    for (let k = 0; k < c && i < length; k++, i++)
                        arr.push(null);
                    continue;
                }
                ...
                if (v && typeof v === 'object' && v.__ref) {
                    const ref = v.__ref;
                    pendingRefs.push({ id: ref, set: (val) => { arr[idx] = val; } });
                }
                arr.push(v);
                i++;
            }
        ...
        function readRecord() {
            ...
            switch (t) {
                ...
                case REC.ClassWithId: {
                    ...
                    if (!meta)
                        throw new Error('ClassWithId references unknown metadata ' + metaId);
                    return readClassValues(meta, id);
                }
                ...
                case REC.ArraySinglePrimitive: {
                    ...
                    if (length < 0 || length > MAX_ARRAY)
                        throw new Error('absurd array length');
                    spend(length);
                    const arr = [];
                    for (let i = 0; i < length; i++)
                        arr.push(readPrimitive(pt));
                ...
                case REC.BinaryArray: {
                    ...
                    const rank = r.i32();
                    if (rank < 1 || rank > 32)
                        throw new Error('absurd array rank');
                    const lengths = [];
