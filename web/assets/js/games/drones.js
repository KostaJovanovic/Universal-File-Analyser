...
export function addDrone(weapon) {
    ...
    if (g.drones.length < DRONE_MAX)
        g.drones.push(makeDrone(weapon));
}
...
export function droneFire(d, ang) {
    ...
    if (d.weapon === 'machine') {
        spawnBulletAt(d.x, d.y, ang + rand(-3, 3) * Math.PI / 180, 1080 * S, 0.9);
        return 0.1;
    }
    if (d.weapon === 'sniper') {
        spawnBulletAt(d.x, d.y, ang, 1080 * S, Infinity, true, 1);
        return 0.5;
    }
    if (d.weapon === 'homing') {
        spawnMissileFrom(d.x, d.y, ang);
        return 0.5;
    } // flat 2/sec
    spawnBulletAt(d.x, d.y, ang, 540 * S, 0.9);
    return 0.22; // normal
...
export function droneHurt(d) {
    if (immortal())
        return; // sandbox invuln shields the drones along with the player
    d.hp--;
...
export function updateDrones(dt) {
    ...
    if (g.droneHomingLock > 0)
        g.droneHomingLock -= dt;
                ...
                if (tgt) {
                    ...
                    if (d.weapon === 'homing' || Math.hypot(tdx, tdy) < 520 * S) {
                        ...
                        if (d.weapon === 'homing' && g.droneHomingLock > 0) {
                            ...
                            d.fireCd = droneFire(d, ang);
                            fired = true;
                            if (d.weapon === 'homing')
                                g.droneHomingLock = 0.1; // hold the gate so the next one staggers
                        }
                    }
                }
            }
