...
function menuShell() {
    if (!g.menuDim)
        g.menuDim = makeMenuDim();
    if (!g.menuPanel) {
        g.menuPanel = document.createElement('div');
        g.menuPanel.className = 'anr-score-panel';
        g.overlay.appendChild(g.menuPanel);
    }
...
function statChip(k, v, acc) {
    ...
    c.className = 'anr-menu-chip';
    const kk = document.createElement('span');
    kk.className = 'k';
    kk.textContent = k;
    const vv = document.createElement('span');
    vv.className = 'v' + (acc ? ' acc' : '');
    vv.textContent = v;
    c.append(kk, vv);
    return c;
}
function menuButton(icon, label, onClick, primary) {
    ...
    b.type = 'button';
    b.className = 'anr-menu-btn' + (primary ? ' anr-menu-btn--primary' : '');
...
function toggleRow(def) {
    ...
    row.type = 'button';
    row.className = 'anr-menu-toggle';
    row.setAttribute('role', 'switch');
    const lab = document.createElement('span');
    lab.className = 'lab';
    lab.append(menuLine('t', def.t));
    if (def.d)
        lab.append(menuLine('d', def.d));
    const sw = document.createElement('span');
    sw.className = 'anr-menu-sw' + (g.settings[def.key] ? ' on' : '');
...
function selectRow(def) {
    ...
    for (const opt of def.options) {
        ...
        b.addEventListener('click', () => { g.settings[def.key] = opt.v; sync(); saveSettings(); if (def.apply)
            def.apply(); });
        btns.push({ b, v: opt.v });
        seg.append(b);
    }
...
function renderSplash() {
    const p = menuShell();
    const title = menuLine('anr-score-go', 'ASTEROIDS');
    title.style.fontSize = '30px';
    p.append(title, menuLine('anr-score-title', 'SUPPORTED FILE TYPES'));
    if (g.highScore > 0)
        p.append(menuLine('anr-score-sub', 'HIGH ' + String(g.highScore).padStart(5, '0')));
...
export function startGame() { if (g.splash)
    restart(); }
...
function renderPauseRoot() {
    ...
    chips.className = 'anr-menu-chips';
    chips.append(statChip('SCORE', String(g.score), true), statChip('WAVE', String(g.wave)), statChip('HIGH', String(g.highScore)));
...
function renderSettings(back) {
    ...
    for (const def of SETTING_DEFS)
        p.append(def.type === 'select' ? selectRow(def) : toggleRow(def));
    p.append(menuRule());
    p.append(menuButton('←', 'Back', back));
}
export function openPause() {
    if (g.splash || g.gameOver || g.menuOpen)
        return;
    g.menuOpen = true;
