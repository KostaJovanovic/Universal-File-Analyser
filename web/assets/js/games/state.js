...
export function saveHi() {
    try {
        localStorage.setItem(HI_KEY, String(g.highScore));
    }
    catch (_) { }
}
export function saveBestWave() {
    try {
        localStorage.setItem(BESTWAVE_KEY, String(g.bestWave));
    }
    catch (_) { }
}
...
export function initState() {
    ...
    g.highScore = 0;
    try {
        g.highScore = parseInt(localStorage.getItem(HI_KEY) || '0', 10) || 0;
    }
    catch (_) { }
    g.newHigh = false;
    // Persistent boss-beaten unlock + remembered start-wave preference (1..maxStartWave).
    g.bossEverBeaten = false;
    g.startWavePref = 1;
    g.bestWave = 0;
    try {
        g.bossEverBeaten = localStorage.getItem(BOSS_UNLOCK_KEY) === '1';
    }
    catch (_) { }
    try {
        g.bestWave = parseInt(localStorage.getItem(BESTWAVE_KEY) || '0', 10) || 0;
    }
    catch (_) { }
    try {
        g.startWavePref = Math.max(1, parseInt(localStorage.getItem(STARTWAVE_KEY) || '1', 10) || 1);
    }
    catch (_) { }
