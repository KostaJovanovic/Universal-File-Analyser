...
function trackTouchCombo(tok) {
    ...
    if (g.comboPos === TOUCH_COMBO.length) {
        g.comboPos = 0;
        if (g.revealSandbox)
            g.revealSandbox();
    }
...
function onKeyDown(e) {
    ...
    if (k === 'r' || k === 'R') {
        ...
        restart();
        return;
    } // restart the run anytime
    if (k === 'p' || k === 'P') {
        e.preventDefault();
        openPause();
        return;
    } // pause anytime mid-run
    if (g.gameOver && (k === ' ' || k === 'Enter')) {
        e.preventDefault();
        restart();
        return;
    }
...
function buildControls() {
    ...
    base.addEventListener('pointerdown', (e) => { e.preventDefault(); joyId = e.pointerId; try {
        base.setPointerCapture(e.pointerId);
    }
    catch (_) { } onMove(e); });
    base.addEventListener('pointermove', (e) => { if (e.pointerId === joyId) {
        e.preventDefault();
        onMove(e);
    } });
    base.addEventListener('pointerup', (e) => { if (e.pointerId === joyId) {
        e.preventDefault();
        onUp();
    } });
    ...
    if (coarseInput) {
        ...
        const setFire = (v) => (e) => { e.preventDefault(); if (v)
            trackTouchCombo('fire'); if (g.gameOver && !g.nameEntry && v) {
            restart();
            return;
        } input.fire = v; };
        ...
        const mkArrow = (label, prop, tok) => {
            ...
            const set = (v) => (e) => { e.preventDefault(); if (v)
                trackTouchCombo(tok); input[prop] = v; };
