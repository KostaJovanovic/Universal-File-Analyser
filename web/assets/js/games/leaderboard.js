...
export function rememberedName() {
    try {
        ...
        if (raw && raw.name)
            return String(raw.name); // never expires
    }
    catch (_) { }
    return '';
}
export function rememberName(name) {
    try {
        localStorage.setItem(NAME_KEY, JSON.stringify({ name, ts: Date.now() }));
    }
    catch (_) { }
}
...
function submitRecord() {
    try {
        return JSON.parse(localStorage.getItem(SUBMIT_KEY) || 'null');
    }
    catch (_) {
        return null;
    }
...
function markSubmitted() {
    try {
        localStorage.setItem(SUBMIT_KEY, JSON.stringify({ day: dayBucket(), count: submitsToday() + 1, ts: Date.now() }));
    }
    catch (_) { }
}
...
function endHeaderNodes() {
    ...
    go.className = 'anr-score-go';
    go.textContent = 'GAME OVER';
    const sub = document.createElement('div');
    sub.className = 'anr-score-sub';
    sub.textContent = 'score ' + g.score + ' · wave ' + g.wave;
    const hi = document.createElement('div');
    hi.className = 'anr-score-sub';
    if (g.newHigh) {
        hi.textContent = '★ NEW HIGH SCORE';
        hi.style.color = POWERUP_DEF.health.color;
    }
    else {
        hi.textContent = 'high ' + g.highScore;
        hi.style.color = g.MUTED;
    }
    return [go, sub, hi];
}
...
function showLeaderboardView(top, mineName) {
    ...
    title.className = 'anr-score-title';
    title.textContent = 'HIGH SCORES';
    const mineIdx = mineName && top ? top.findIndex((s) => s.name === mineName) : -1;
    const row = document.createElement('div');
    row.className = 'anr-score-row';
    const again = document.createElement('button');
    again.type = 'button';
    again.className = 'anr-game-btn';
    ...
    if (g.bossEverBeaten && maxWave > 1) {
        ...
        second.addEventListener('click', () => {
            ...
            try {
                localStorage.setItem(STARTWAVE_KEY, String(maxWave));
            }
            catch (_) { }
            if (g.startToggleBtn) {
                g.startToggleBtn.textContent = 'START W' + maxWave;
                g.startToggleBtn.classList.add('on');
            }
            restart();
        });
    }
...
async function skipToLeaderboard() {
    ...
    if (g.endPanel)
        showLeaderboardView(g.leaderboard, null);
}
...
async function submitScore(name, msgEl, submitBtn) {
    ...
    try {
        ...
        if (!resp.ok || !data.ok) {
            if (msgEl) {
                msgEl.className = 'anr-score-msg err';
                msgEl.textContent = data.error || 'Could not send score.';
            }
            if (submitBtn)
        ...
        if (msgEl) {
            msgEl.className = 'anr-score-msg err';
            msgEl.textContent = 'Offline - score not sent.';
        }
        if (submitBtn)
            submitBtn.disabled = false;
    }
...
function showSubmitView() {
    ...
    hint.className = 'anr-score-msg';
    hint.textContent = '5 letters or numbers';
    const input = document.createElement('input');
    input.type = 'text';
    input.className = 'anr-score-input';
    input.maxLength = 5;
...
export function endGame() {
    ...
    if (g.pauseBtn)
        g.pauseBtn.style.display = 'none';
    g.input.left = g.input.right = g.input.thrust = g.input.fire = false;
    g.joy.active = false;
    g.joy.mag = 0;
    if (g.newHigh && !g.scoreDone && !g.sandboxUsed && canSubmitToday())
        showSubmitView();
    else {
        g.endPanel = document.createElement('div');
        g.endPanel.className = 'anr-score-panel';
