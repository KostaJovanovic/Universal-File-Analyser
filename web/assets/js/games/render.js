...
import { LINE, MONO, POWERUP_DEF, TAU, SPAWN_INVULN, WAVE_GRACE, WRECK_FADE, NUKE_TOTAL, NUKE_WHITE, NUKE_FADE, BOSS_COLOR, SING_CORE } from './config.js';
...
const F10 = '10px ' + MONO, F11 = '11px ' + MONO, F12 = '12px ' + MONO, F13 = '13px ' + MONO, F14 = '14px ' + MONO, F15 = '15px ' + MONO, F18 = '18px ' + MONO, F34 = '34px ' + MONO;
...
function scopeFrameLayer() {
    ...
    if (g._scopeKey !== key) {
        ...
        c.save();
        c.shadowColor = hard ? '#fff' : ACCENT;
        c.shadowBlur = hard ? 26 : 18;
        c.strokeStyle = hard ? '#fff' : ACCENT;
        c.lineWidth = hard ? 3 : 2;
        c.strokeRect(pad, pad, fw, fh);
        c.restore();
        c.strokeStyle = hard ? 'rgba(255,255,255,0.5)' : 'rgba(255,255,255,0.18)';
        c.lineWidth = 1;
...
function drawUfo(u) {
    ...
    for (let i = -1; i <= 1; i++) {
        if ((Math.floor(clock * 4) + i) & 1) {
            ctx.beginPath();
            ctx.arc(i * r * 0.42, r * 0.30, r * 0.1, 0, TAU);
            ctx.fill();
        }
    }
...
function drawShipAt(x, y) {
    ...
    if (g.shield > 0 && !(g.shield < 1 && (Math.floor(g.shield * 8) & 1))) {
        ...
        ctx.strokeStyle = POWERUP_DEF.health.color;
        ctx.lineWidth = 2;
        ctx.beginPath();
...
function drawShip() {
    if (g.ship.dead || g.nuke > 0 || g.splash)
        return;
...
function drawDroneIcon(s) {
    ...
    for (let i = 1; i < pts.length; i++)
        ctx.lineTo(pts[i][0] * f, pts[i][1] * f);
    ctx.closePath();
    ctx.stroke();
}
...
function drawPowerupAt(p, x, y) {
    if (p.life < 3 && (Math.floor(p.life * 8) & 1))
        return; // blink as it nears expiry
...
function drawLightning() {
    if (g.weapon !== 'lightning' || !g.lightningEnd || !g.lightningMid || g.ship.dead || g.gameOver || g.nuke > 0)
        return;
    ...
    ctx.shadowColor = POWERUP_DEF.lightning.color;
    ctx.shadowBlur = 10;
    ctx.lineWidth = 2;
    ctx.lineJoin = 'round';
    ...
    if (!hardEdges()) {
        let ox = 0, oy = 0;
        if (lightningEnd.x > cx + HW)
            ox = -2 * HW;
        else if (lightningEnd.x < cx - HW)
            ox = 2 * HW;
        if (lightningEnd.y > cy + HH)
            oy = -2 * HH;
        else if (lightningEnd.y < cy - HH)
            oy = 2 * HH;
        if (ox || oy) {
            strokeAt(ox, oy);
            if (ox && oy) {
                strokeAt(ox, 0);
                strokeAt(0, oy);
            } // corner wrap: cover both single-axis images too
        }
    }
    ctx.restore();
}
...
function drawUltrasound() {
    if (g.weapon !== 'ultrasound' || g.ship.dead || g.gameOver || g.nuke > 0)
        return;
    const ctx = g.ctx, ship = g.ship, S = g.S, R = ultrasoundRadius();
    const paint = (x, y) => {
        for (const rp of g.ripples) {
            ctx.globalAlpha = (1 - rp.p) * 0.6;
            ctx.strokeStyle = POWERUP_DEF.ultrasound.color;
            ctx.lineWidth = 2;
            ctx.beginPath();
            ctx.arc(x, y, 6 * S + (R - 6 * S) * rp.p, 0, TAU);
            ctx.stroke();
        }
        ctx.globalAlpha = 0.8;
        ctx.strokeStyle = LINE;
    ...
    if (hardEdges())
        ...
        withWrap(ship.x, ship.y, R, paint);
}
function drawLasers() {
...
function drawLaserSight() {
    if (g.weapon !== 'laser' || g.ship.dead || g.gameOver || g.nuke > 0)
        return;
    ...
    ctx.save();
    ctx.strokeStyle = POWERUP_DEF.laser.color;
    ctx.fillStyle = POWERUP_DEF.laser.color;
    ...
    for (const s of g.singularities) {
        ...
        withWrap(s.x, s.y, rr * 2.2, (x, y) => {
            ...
            for (let i = 0; i < 3; i++) {
                ...
                ctx.beginPath();
                ctx.arc(0, 0, rr * 2.05, a0, a0 + 0.85);
                ctx.stroke();
            }
            ctx.restore();
        });
...
function drawBulletSet(sniper, color) {
    ...
    for (const b of g.bullets) {
        ...
        if (ox) {
            ctx.moveTo(b.x + ox + r, b.y);
            ctx.arc(b.x + ox, b.y, r, 0, TAU);
        }
        if (oy) {
            ctx.moveTo(b.x + r, b.y + oy);
            ctx.arc(b.x, b.y + oy, r, 0, TAU);
        }
        if (ox && oy) {
            ctx.moveTo(b.x + ox + r, b.y + oy);
            ctx.arc(b.x + ox, b.y + oy, r, 0, TAU);
        }
    }
    if (any) {
        ctx.fillStyle = color;
...
function hud() {
    ...
    ctx.font = F12;
    ctx.fillStyle = MUTED;
    ctx.fillText('SCORE', cx - HW, top - 17);
    ctx.font = F18;
    ctx.fillStyle = ACCENT;
    ctx.fillText(String(g.score).padStart(5, '0'), cx - HW, top);
    ctx.font = F11;
    ctx.fillStyle = MUTED;
    ctx.fillText('HIGH ' + String(g.highScore).padStart(5, '0'), cx - HW, top + 15);
    // Wave - top-right (always the plain wave number; the boss's name shows in the centre banner).
    ctx.textAlign = 'right';
    ctx.font = F12;
    ctx.fillStyle = MUTED;
    ctx.fillText('WAVE', cx + HW, top - 17);
    ctx.font = F18;
    ctx.fillStyle = ON_DARK;
    ctx.fillText(String(g.wave), cx + HW, top);
...
function waveBanner(graceLeft) {
    ...
    if (tw > maxW) {
        size = Math.max(18, Math.round(size * maxW / tw));
        ctx.font = '900 ' + size + 'px ' + MONO;
    }
...
function drawWeaponTimer() {
    if (g.weapon === 'normal' || g.ship.dead || g.gameOver || g.nuke > 0)
        return;
...
function drawBuffTimers() {
    if (g.timeWarp <= 0 || g.ship.dead || g.gameOver || g.nuke > 0)
        return;
...
function nukeFlash() {
    ...
    if (g.nuke > 0) {
        ...
        if (elapsed < NUKE_WHITE)
            a = 1;
        else if (elapsed < NUKE_WHITE + NUKE_FADE)
            a = 1 - (elapsed - NUKE_WHITE) / NUKE_FADE;
    }
    const bg = g.settings.reduceFlash ? '#000' : '#fff';
    if (bg !== g.nukeTint) {
        g.nukeEl.style.background = bg;
        g.nukeTint = bg;
    }
    const v = String(Math.max(0, a));
    if (g.nukeEl.style.opacity !== v)
        g.nukeEl.style.opacity = v;
}
function gameOverScreen() {
    ...
    if (g.newHigh) {
        ctx.fillStyle = POWERUP_DEF.health.color;
        ctx.font = F14;
        ctx.fillText('★ NEW HIGH SCORE', cx, cy + 36);
    }
    else {
        ctx.fillStyle = MUTED;
        ctx.font = F13;
        ctx.fillText('high ' + g.highScore, cx, cy + 36);
    }
}
...
function drawLeaderboard() {
    ...
    for (let i = 0; i < g.leaderboard.length; i++) {
        ...
        ctx.fillStyle = LINE;
        ctx.fillText(String(s.name), x + 22, y);
        ctx.textAlign = 'right';
        ctx.fillStyle = ACCENT;
        ctx.fillText(Number(s.score).toLocaleString(), x + colW, y);
        y += 20;
    }
    ctx.textAlign = 'left';
}
...
function drawFps() {
    ...
    if (bodies)
        parts.push('BODIES ' + bodies);
    if (g.bullets.length)
        parts.push('BULLETS ' + g.bullets.length);
    if (g.powerups.length)
        parts.push('POWERUPS ' + g.powerups.length);
    ctx.save();
    ctx.font = F11;
    ctx.fillStyle = MUTED;
    ctx.textAlign = 'left';
...
export function render() {
    ...
    ctx.save();
    ctx.beginPath();
    ctx.rect(cx - HW, cy - HH, 2 * HW, 2 * HH);
    ctx.clip();
    ...
    if (settings.bgDetail) {
        ...
        for (const f of g.flyers)
            drawFlyer(f); // background squadrons, behind the action
    }
    ...
    if (!g.gameOver && !g.splash) {
        ...
        if (boss && boss.grace > 0)
            graceLeft = Math.max(graceLeft, boss.grace); // boss waves get the banner too
        if (graceLeft > 0)
            waveBanner(graceLeft);
    }
    drawWeaponTimer();
    drawBuffTimers();
    if (!g.splash)
        hud();
    drawBossBar();
    if (g.megaMsgT > 0 && boss && !boss.dying)
        drawMegaMessage();
    drawLeaderboard();
    if (settings.showFps)
        drawFps();
