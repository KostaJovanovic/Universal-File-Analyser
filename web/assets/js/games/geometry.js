...
export function layout() {
    ...
    if (!g.stars.length) {
        for (let i = 0; i < 90; i++)
            g.stars.push({ x: rand(-1, 1), y: rand(-1, 1), b: rand(0.15, 0.6) });
    }
}
...
export function rescaleScene(oldS, oldCx, oldCy, oldHW, oldHH) {
    ...
    for (const a of g.asteroids) {
        remap(a);
        a.font = fitFont(a.label, a.radius);
        a.fontStr = a.font + 'px ' + MONO;
    }
    ...
    if (g.lightningMid) {
        g.lightningMid.ox *= sr;
        g.lightningMid.oy *= sr;
    }
}
...
export function wrap(o) {
    if (hardEdges())
        return; // mega fight: the walls are solid, nothing wraps
    const { cx, cy, HW, HH } = g;
    if (o.x < cx - HW)
        o.x += 2 * HW;
    else if (o.x > cx + HW)
...
export function edgeBounceShip() {
    ...
    if (ship.x < cx - HW + m) {
        ship.x = cx - HW + m;
        ship.vx = Math.abs(ship.vx) * e;
    }
    else if (ship.x > cx + HW - m) {
        ship.x = cx + HW - m;
        ship.vx = -Math.abs(ship.vx) * e;
    }
    if (ship.y < cy - HH + m) {
        ship.y = cy - HH + m;
        ship.vy = Math.abs(ship.vy) * e;
    }
    else if (ship.y > cy + HH - m) {
        ship.y = cy + HH - m;
        ship.vy = -Math.abs(ship.vy) * e;
    }
}
...
export function rayToRim(px, py, dx, dy) {
    ...
    let t = Infinity;
    if (dx > 1e-9)
        t = Math.min(t, (cx + HW - px) / dx);
    else if (dx < -1e-9)
        t = Math.min(t, (cx - HW - px) / dx);
    if (dy > 1e-9)
        t = Math.min(t, (cy + HH - py) / dy);
    else if (dy < -1e-9)
        t = Math.min(t, (cy - HH - py) / dy);
