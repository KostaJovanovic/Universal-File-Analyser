/* Analyser - Asteroids easter egg: world lifecycle + spawning.
   ...
   shared state `g`. */
import { TAU, rand, pick, ARCHIVE_POOL, FILE_POOL, LINE, MONO, POWERUP_DEF, POWERUP_TYPES, WAVE_GRACE, MAX_POWERUPS, MAX_LIVES, SHIELD_DUR, POWERUP_LIFE, NUKE_TOTAL, SPAWN_INVULN, WRECK_FADE, SING_DUR, SING_DRIFT } from './config.js';
...
export function makeAsteroid(x, y, size, label) {
    ...
    const verts = [];
    for (let i = 0; i < n; i++)
        verts.push({ a: (i / n) * TAU, r: rand(0.72, 1.12) });
...
export function spawnWave() {
    ...
    g.wave++;
    if (!g.sandbox && g.wave > g.bestWave) {
        g.bestWave = g.wave;
        saveBestWave();
    } // remember the deepest scored run
    if (isBossWave(g.wave)) {
        spawnBoss();
        return;
    } // boss wave: just the boss; advances when it dies
    ...
    if (g.wave >= 3) {
        ...
        if (g.wave === 3)
            ...
            rewards = 1;
            while (rewards < ufoCap && Math.random() < Math.min(0.6, g.wave * 0.05))
                rewards++;
        }
        for (let k = 0; k < rewards; k++) {
            if (ufos.filter((u) => u.kind === 'reward').length >= ufoCap)
                break; // cap on-screen reward saucers
            ufos.push(makeUfo('reward'));
            if (Math.random() < 0.25)
                ufos.push(makeUfo('ambient'));
        }
    }
}
...
export function applyPowerup(type) {
    if (type === 'health') {
        ...
        g.weapon = type;
        g.weaponTimer = POWERUP_DEF[type].dur;
        g.homingLeft = 0;
    }
}
...
export function triggerNuke() {
    if (!immortal())
        g.lives--; // sandbox invulnerability spares the bomb's life cost
...
export function restart() {
    ...
    g.nuke = 0;
    g.wreck = null;
    g.overlay.style.cursor = '';
    clearEndPanel();
    g.scoreDone = false;
    g.splash = false;
    g.menuOpen = false;
    clearMenus(); // leave the splash / pause overlays
    if (g.pauseBtn) {
        g.pauseBtn.style.display = '';
        g.pauseBtn.textContent = '❚❚';
    }
...
export function destroyAsteroid(ai) {
    ...
    burst(a.x, a.y, a.size === 3 ? ACCENT : LINE, { count: 5 + a.size * 3, speed: 60 + a.size * 38, life: 0.4 + a.size * 0.06 });
    // Power-up drop: 5% from a red (archive) asteroid, 1% from a white one - within the cap.
    if (!g.puSpawnOff && powerups.length < MAX_POWERUPS && Math.random() < (a.size === 3 ? 0.05 : 0.01))
        powerups.push(makePowerup(a.x, a.y));
    asteroids.splice(ai, 1);
    if (a.size > 1) {
        for (let k = 0; k < 2; k++)
            asteroids.push(makeAsteroid(a.x, a.y, a.size - 1, pick(FILE_POOL)));
    }
    if (!asteroids.length && !g.boss)
        spawnWave(); // boss fights spawn their own asteroids - don't advance mid-fight
}
export function loseLife() {
    ...
    g.ship.dead = true;
    g.deathTimer = 0.9; // animate the wreck before respawning / game over
}
...
export function updateFlyers(dt) {
    ...
    g.flyerTimer -= dt;
    if (g.flyerTimer <= 0 && flyers.length === 0) {
        spawnFlyers();
        g.flyerTimer = rand(5, 12);
    }
    for (let i = flyers.length - 1; i >= 0; i--) {
        const f = flyers[i];
        f.x += f.vx * dt;
        f.y += f.vy * dt;
        if (Math.hypot(f.x - cx, f.y - cy) > Math.hypot(HW, HH) + 120)
            flyers.splice(i, 1);
    }
}
