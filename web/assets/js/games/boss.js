...
import { TAU, rand, pick, ARCHIVE_POOL, FILE_POOL, WAVE_GRACE, MAX_POWERUPS, MAX_LIVES, SHIELD_DUR, SPAWN_INVULN, NUKE_WHITE, NUKE_FADE, BOSS_COLOR, UFO_REWARD_COLOR, MONO, POWERUP_DEF, BOSS_UNLOCK_KEY } from './config.js';
...
export function nextBossType() {
    if (!g.bossBag.length) {
        ...
        for (let i = g.bossBag.length - 1; i > 0; i--) {
            const j = (Math.random() * (i + 1)) | 0;
            const t = g.bossBag[i];
            g.bossBag[i] = g.bossBag[j];
            g.bossBag[j] = t;
        }
    }
...
export function updateMegaRadar(b, dt) {
    ...
    if (b.pingCd <= 0) {
        b.pingCd = 2.6;
        b.pings.push({ r: b.r * 0.2, life: 1.7, max: 1.7 });
    }
    ...
    for (let ai = asteroids.length - 1; ai >= 0; ai--) {
        ...
        if (Math.abs(Math.atan2(Math.sin(ab - b.sweepAng), Math.cos(ab - b.sweepAng))) < 0.16)
            destroyAsteroid(ai);
    }
        ...
        if (Math.abs(d) < 0.20) { // beam is over the ship
            ...
            if (first) {
                ...
                for (const dr of g.drones)
                    burst(dr.x, dr.y, POWERUP_DEF.drone.color, { count: 12, speed: 130, life: 0.5, lines: true });
                g.drones.length = 0;
                g.bullets.length = 0;
                for (let pi = g.powerups.length - 1; pi >= 0; pi--)
                    if (g.powerups[pi].type !== 'ram')
                        g.powerups.splice(pi, 1); // wipe everything but the ram
            }
            const power = first ? 2 : 1; // the first sweep hits twice as hard as the rest
            ship.vx += dirx * 780 * S * power;
            ship.vy += diry * 780 * S * power; // shove toward the far wall
        }
    }
}
...
export function drawMegaRadar(b) {
    ...
    ctx.strokeStyle = UFO_REWARD_COLOR;
    ctx.lineWidth = 2;
    for (const p of b.pings) {
        ctx.globalAlpha = 0.55 * (p.life / p.max);
        ctx.beginPath();
    ...
    ctx.beginPath();
    ctx.moveTo(0, 0);
    ctx.arc(0, 0, reach, a - wedge, a);
    ctx.closePath();
    ctx.fill(); // trailing fade
    ctx.globalAlpha = 0.75;
    ctx.beginPath();
    ctx.moveTo(0, 0);
    ctx.lineTo(Math.cos(a) * reach, Math.sin(a) * reach);
    ctx.stroke(); // leading edge
    ctx.globalAlpha = 1;
    ctx.restore();
}
...
export function ensureRamPickup() {
    if (g.weapon === 'ram' || g.ship.dead)
        return;
    if (g.powerups.some((p) => p.type === 'ram'))
        return;
...
export function startMegaOutro() {
    ...
    for (let i = g.powerups.length - 1; i >= 0; i--)
        if (g.powerups[i].life === Infinity)
            g.powerups.splice(i, 1);
    g.hideShip = true;
    ship.invuln = Math.max(ship.invuln, 12); // ship vanishes into the blast; returns next wave
    ...
    if (!g.bossEverBeaten) {
        ...
        try {
            localStorage.setItem(BOSS_UNLOCK_KEY, '1');
        }
        catch (_) { }
        if (g.startToggleBtn)
            g.startToggleBtn.style.display = '';
    }
...
export function updateMegaOutro(dt) {
    ...
    if (b.ringCd <= 0 && t < DET) {
        b.ringCd = 0.8;
        b.outroRings.push({ r: b.r * 0.2, life: 2, max: 2 });
    } // shockwaves
    for (let i = b.outroRings.length - 1; i >= 0; i--) {
        const r = b.outroRings[i];
        r.r += 380 * S * dt;
        r.life -= dt;
        if (r.life <= 0)
            b.outroRings.splice(i, 1);
    }
    if (!b.detonated && t >= DET) { // the blast
        b.detonated = true;
        for (let k = 0; k < 5; k++)
            burst(cx + rand(-HW, HW) * 0.4, cy + rand(-HH, HH) * 0.4, '#fff', { count: 24, speed: 240, life: 1, lines: true });
    }
    if (t >= DET + NUKE_WHITE + NUKE_FADE)
        finishMegaOutro(); // flash matches a nuke's length; never sooner than 10s
}
export function finishMegaOutro() {
    const { cx, cy, HW, HH } = g;
    g.boss = null; // clearing the boss releases the white walls / wrap
    g.hideShip = false;
    resetShip(SPAWN_INVULN); // ship returns fresh - no life lost
    if (g.lives < MAX_LIVES)
        g.lives++;
    else
        g.shield = Math.max(g.shield, SHIELD_DUR);
    for (let k = 0; k < 3; k++)
        g.powerups.push(makePowerup(cx + rand(-HW, HW) * 0.5, cy + rand(-HH, HH) * 0.5));
    spawnWave();
}
...
export function drawMegaOutro(b) {
    ...
    ctx.strokeStyle = UFO_REWARD_COLOR;
    ctx.lineWidth = 2;
    for (const r of b.outroRings) {
        ctx.globalAlpha = 0.6 * (r.life / r.max);
        ctx.beginPath();
        ctx.arc(b.x, b.y, r.r, 0, TAU);
        ctx.stroke();
    }
    ctx.globalAlpha = 1;
    ...
    if (t >= DET) {
        ...
        if (el < NUKE_WHITE)
            fa = 1;
        else if (el < NUKE_WHITE + NUKE_FADE)
            fa = 1 - (el - NUKE_WHITE) / NUKE_FADE;
    }
    if (fa > 0) {
        ctx.globalAlpha = Math.min(1, fa);
        ctx.fillStyle = g.settings.reduceFlash ? '#000' : '#fff';
        ctx.fillRect(cx - HW - 30, cy - HH - 30, 2 * HW + 60, 2 * HH + 60);
        ctx.globalAlpha = 1;
    }
    ctx.restore();
}
...
export function drawMegaMessage() {
    ...
    for (let i = 0; i < lines.length; i++)
        ctx.fillText(lines[i], cx, by + padY + lh * (i + 1) - 5);
    ctx.restore();
}
...
export function spawnBossAsteroid() {
    ...
    if (g.boss && g.boss.type === 'megastructure' && g.asteroids.length >= 12)
        return;
    const size = Math.random() < 0.5 ? 3 : 2;
    let x, y, tries = 0;
    do {
        x = cx + rand(-HW, HW) * 0.92;
        y = cy + rand(-HH, HH) * 0.92;
...
export function spawnBossPowerup() {
    const { cx, cy, HW, HH, S, ship } = g;
    if (g.puSpawnOff || g.powerups.length >= MAX_POWERUPS)
        return;
    let x, y, tries = 0;
    do {
        x = cx + rand(-HW, HW) * 0.85;
        y = cy + rand(-HH, HH) * 0.85;
...
export function spawnBoss(forcedType) {
    ...
    if (type === 'mothership') {
        ...
        b.r = R * 1.56;
        b.spin = 0.22;
        b.advance = 0; // 1.2 x 1.3: the original +20%, then +30% more
        b.rimState = 'fighting';
        b.finaleT = 0;
        b.rimGone = false; // satellite-destruction finale, then the slide-in
        b.sweepAng = 0;
        b.pings = [];
        b.pingCd = 0.5;
        b.sweepHitCd = 0; // radar attack once the core is exposed
        b.coreReady = false;
        b.gunsKilled = false;
        b.dying = false;
        b.stage2T = 0; // ram-the-core endgame, then the cinematic outro
        megaAnchor(b);
        ...
        b.r = su * 0.5;
        b.spacing = su * 0.44;
        b.headAngle = rand(0, TAU);
        b.steerT = 1;
        b.turn = 0;
        b.eatCd = 0; // drive straight for 1s before the first turn
        const M = Math.min(25, 13 + Math.max(0, Math.floor((g.wave - 11) / 7)) * 2);
        const shp = Math.round(18 * buff);
        const hx = cx, hy = cy - HH * 0.4;
        for (let i = 0; i < M; i++)
            b.nodes.push({ ax: hx - i * b.spacing, ay: hy, r: su * 0.34, hp: i === 0 ? Infinity : shp, maxhp: i === 0 ? Infinity : shp, kind: i === 0 ? 'head' : 'segment', dead: false });
    }
    g.boss = b;
}
...
export function bossNodePos(b, n) {
    ...
    _np[0] = b.x + n.ox * c - n.oy * s;
    _np[1] = b.y + n.ox * s + n.oy * c;
    return _np;
}
...
export function bossNodeVulnerable(b, n) {
    if (n.dead || b.grace > 0)
        return false; // no hitbox while it's still the arrival outline
    if (n.kind === 'head')
        return false; // serpent head is invulnerable (but lethal to touch)
    if (n.kind === 'core' && b.type === 'megastructure')
        return b.nodes.every((x) => x.kind !== 'weak' || x.dead); // core sealed until the rim is cleared
    return true;
}
export function bossDead(b) {
    if (b.type === 'mothership')
        return b.nodes[0].dead;
    if (b.type === 'segmented')
        return b.nodes.every((n) => n.kind === 'head' || n.dead); // all body segments gone
    return b.nodes.some((n) => n.kind === 'core' && n.dead); // megastructure: core destroyed
}
export function damageBossNode(b, n, dmg, hx, hy, byRam) {
    if (n.dead)
        return;
    if (b.type === 'megastructure' && n.kind === 'core' && !byRam)
        return; // core destroyed only by the ram
    n.hp -= dmg;
    burst(hx, hy, BOSS_COLOR, { count: 3, speed: 70, life: 0.25 });
    if (n.hp <= 0) {
        n.dead = true;
        burst(hx, hy, BOSS_COLOR, { count: 10, speed: 120, life: 0.5, lines: true });
    }
}
...
export function hitBossAt(x, y, padR, dmg) {
    ...
    for (const n of boss.nodes) {
        if (!bossNodeVulnerable(boss, n))
            continue;
        if (boss.type === 'megastructure' && n.kind === 'core')
            continue; // bullets pass through the core (ram-only)
        const [nx, ny] = bossNodePos(boss, n);
        const rr = n.r + padR;
        if ((x - nx) * (x - nx) + (y - ny) * (y - ny) < rr * rr) {
            damageBossNode(boss, n, dmg, nx, ny);
            return true;
        }
    }
    return false;
...
export function serpentEat(b, dt) {
    if (b.eatCd > 0) {
        b.eatCd -= dt;
        return;
    } // still digesting the last meal
    const head = b.nodes[0];
    for (let ai = g.asteroids.length - 1; ai >= 0; ai--) {
        const a = g.asteroids[ai];
        if (a.grace > 0)
            continue; // no hitbox while it's still the arrival outline
        ...
        if (a.size === 3) {
            ...
            if (g.powerups.length < MAX_POWERUPS)
                g.powerups.push(makePowerup(a.x, a.y)); // drop loot where it fed
        }
        else {
            let max = 0;
            ...
            for (const n of b.nodes) {
                if (heal <= 0)
                    break;
                if (n.kind !== 'segment' || n.dead || n.hp >= n.maxhp)
                    continue;
                const add = Math.min(heal, n.maxhp - n.hp);
                n.hp += add;
                heal -= add;
            }
        }
...
export function updateSnakeOutro(dt) {
    ...
    if (!b.exploded) {
        ...
        if (b.dieSpark <= 0) {
            b.dieSpark = 0.07;
            burst(head.ax, head.ay, BOSS_COLOR, { count: 2, speed: 55, life: 0.4 });
        } // glowing trail as it coils
        if (b.dieDist >= b.dieLen) {
            snakeOutroExplode(b);
            b.explodeT = b.dieT;
        } // whole spine now on the ring - detonate
    }
...
function snakeOutroExplode(b) {
    ...
    for (const n of b.nodes)
        burst(n.ax, n.ay, '#fff', { count: 10, speed: 320, life: 1.2, lines: true });
    const head = b.nodes[0];
...
export function updateBoss(dt) {
    ...
    if (bossDead(boss)) {
        if (boss.type === 'megastructure')
            startMegaOutro(); // epic 10s+ send-off before the next wave
        else if (boss.type === 'segmented')
            startSnakeOutro(); // coil-up, violent shake, then a big blast
        else
            bossDefeated();
        return;
    }
    ...
    b.t += dt;
    if (b.grace > 0)
        b.grace = Math.max(0, b.grace - dt); // arrival outline: inert until it expires
    ...
    if (b.type === 'mothership') {
        ...
        if (active && b.spawnCd <= 0) {
            ...
            if (g.ufos.filter((u) => u.fromBoss && !u.leaving).length < 6) {
                ...
                u.fromBoss = true;
                u.x = b.x;
                u.y = b.y; // emerge from the carrier, then ease onto its path
                g.ufos.push(u);
            }
        }
    }
        ...
        if (b.rimState !== 'fighting')
            b.stage2T += dt; // second-stage clock: the ram appears 5s after the rim falls
        if (b.rimState === 'fighting') {
            ...
            if (b.finaleT >= 0.85) {
                ...
                for (let k = 0; k < PTS; k++) {
                    ...
                    if (k % 2 === 0)
                        burst(b.x + Math.cos(a) * b.r * 0.58, b.y + Math.sin(a) * b.r * 0.58, UFO_REWARD_COLOR, { count: 4, speed: 150, life: 0.7, lines: true });
                }
                b.rimGone = true;
                b.rimState = 'sliding';
            }
        ...
        if (b.stage2T >= 5)
            ensureRamPickup(); // 5s into the second stage the ram appears and is kept parked permanently
        megaAnchor(b); // re-pin to the shorter edge (resize-proof), advancing inward when sliding
    }
    else {
        updateSnake(b, dt);
        if (active)
            serpentEat(b, dt); // head swallows asteroids it passes over, regrowing a lost segment
    }
        ...
        for (const n of b.nodes) {
            ...
            if (ramming) {
                if (g.ramHitCd <= 0 && bossNodeVulnerable(b, n) && ship.vx * dx + ship.vy * dy > 0) {
                    damageBossNode(b, n, 2, nx, ny, true);
                    g.ramHitCd = 0.2;
                    ship.invuln = Math.max(ship.invuln, 0.2);
                }
                continue;
            }
            if (ship.invuln <= 0 && g.shield <= 0 && !immortal()) {
                g.cause = 'boss';
                loseLife();
            }
            break;
...
export function bossDefeated() {
    ...
    for (const n of b.nodes) {
        const [nx, ny] = bossNodePos(b, n);
        burst(nx, ny, BOSS_COLOR, { count: 8, speed: 130, life: 0.6, lines: true });
    }
    for (let k = 0; k < 3; k++)
        g.powerups.push(makePowerup(cx + rand(-HW, HW) * 0.5, cy + rand(-HH, HH) * 0.5));
    if (g.lives < MAX_LIVES)
    ...
    if (b.type === 'segmented') {
        ...
        for (const n of b.nodes) {
            if (n.kind === 'core') {
                ...
                ctx.strokeRect(n.ox - n.r, n.oy - n.r, n.r * 2, n.r * 2);
        }
    }
...
function megaShellSprite(b) {
    ...
    for (const n of b.nodes)
        if (n.kind === 'weak')
            mask += n.dead ? '1' : '0'; // re-bake when a satellite dies
    ...
    if (b._shellKey !== key) {
        ...
        c.lineJoin = 'round';
        c.shadowBlur = 10;
        c.shadowColor = UFO_REWARD_COLOR;
        c.strokeStyle = UFO_REWARD_COLOR;
        c.beginPath();
        c.arc(0, 0, b.r, 0, TAU);
        c.stroke(); // outer orbit
        c.globalAlpha = 0.5;
        c.beginPath();
        c.arc(0, 0, b.r * 0.58, 0, TAU);
        c.stroke();
        c.globalAlpha = 1; // inner orbit
        if (!finale) {
            ...
            for (const n of b.nodes) {
                ...
                if (!n.dead) {
                    c.globalAlpha = 0.3;
                    c.fillRect(n.ox - n.r, n.oy - n.r, n.r * 2, n.r * 2);
                }
                c.globalAlpha = 1;
            }
        }
...
function drawSerpentHunt(b) {
    const prey = b.prey;
    if (!prey || !g.asteroids.includes(prey))
        return; // nothing to hunt, or it was just eaten
    ...
    for (let i = 0; i < 100; i++) {
        ...
        x += Math.cos(ang) * spd * step;
        y += Math.sin(ang) * spd * step;
        ctx.lineTo(x, y);
    ...
    ctx.beginPath();
    ctx.arc(prey.x, prey.y, prey.radius + 5 * S, 0, TAU);
    ctx.stroke();
    ctx.restore();
}
...
function drawSnakeOutro(b) {
    if (b.exploded)
        return; // nothing left but the burst (drawn as particles)
    ...
    for (const n of b.nodes) {
        if (n.ax < cx - HW + mg)
            lx = true;
        else if (n.ax > cx + HW - mg)
            rx = true;
        if (n.ay < cy - HH + mg)
            ty = true;
        else if (n.ay > cy + HH - mg)
            by = true;
    }
    const xs = [0];
    if (lx)
    ...
    for (const ox of xs)
        for (const oy of ys) {
            ...
            for (let i = 1; i < b.nodes.length; i++) { // spine line through every node position (shortest-wrapped, no streaks)
                ...
                ctx.lineTo(a.ax + dx, a.ay + dy);
                ctx.stroke();
            }
            for (const n of b.nodes) {
                if (n.dead)
                    continue; // destroyed segments stay gone - never recovered for the death animation
...
export function drawBoss() {
    ...
    if (b.type === 'segmented') {
        ...
        ctx.restore();
        if (b.rimState === 'sliding')
            drawMegaRadar(b); // radar sweep + pings (world space)
    }
    else { // mothership - detailed carrier saucer
        const core = b.nodes[0], frac = Math.max(0, core.hp / core.maxhp);
        ctx.save();
        ctx.translate(b.x, b.y); // no rotation - it hovers level
        const r = b.r;
        ctx.beginPath();
        ctx.moveTo(-r, 0);
        ctx.lineTo(-r * 0.55, -r * 0.32);
        ...
        ctx.quadraticCurveTo(0, -r * 0.85, r * 0.45, -r * 0.32);
        ctx.stroke();
        ctx.save();
        ctx.fillStyle = UFO_REWARD_COLOR;
        ctx.shadowColor = UFO_REWARD_COLOR;
        ctx.shadowBlur = 8;
        const bay = 0.6 + 0.4 * Math.sin(b.t * 4);
        for (let i = -2; i <= 2; i++) {
            ctx.globalAlpha = bay;
            ctx.beginPath();
            ctx.arc(i * r * 0.32, r * 0.18, 2.6 * S, 0, TAU);
            ctx.fill();
        }
        ctx.restore();
        ...
        ctx.beginPath();
        ctx.arc(0, -r * 0.05, r * 0.15 + pulse * r * 0.05, 0, TAU);
        ctx.fill();
        ctx.restore();
        ...
        if (cracks > 0) {
            ...
            for (let i = 0; i < cracks; i++) {
                ...
                ctx.beginPath();
                ctx.moveTo(x0, y0);
                ctx.lineTo((x0 + x1) / 2 + r * 0.05, (y0 + y1) / 2);
                ctx.lineTo(x1, y1);
                ctx.stroke();
            }
            ctx.globalAlpha = 1;
...
export function drawBossBar() {
    const boss = g.boss;
    if (!boss || boss.grace > 0 || boss.dying)
        return; // hidden during the arrival outline and the death cinematic
    const ctx = g.ctx, { cx, cy, HW, HH } = g;
    let hp = 0, max = 0;
    for (const n of boss.nodes) {
        if (!isFinite(n.maxhp))
        ...
        hp += Math.max(0, n.hp);
        max += n.maxhp;
    } // skip the invulnerable serpent head
    ...
    ctx.shadowBlur = 8;
    ctx.fillStyle = BOSS_COLOR;
    ctx.fillText(BOSS_NAMES[boss.type] || 'BOSS', cx, y - 8);
    ctx.shadowBlur = 0;
    ...
    if (frac > 0) {
        ...
        grd.addColorStop(0, '#c78cff');
        grd.addColorStop(1, BOSS_COLOR);
        ctx.fillStyle = grd;
