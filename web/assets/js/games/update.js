...
import { RIPPLE_DUR, ULTRASOUND_TICK, SPAWN_INVULN, LIGHTNING_HALF, TAU, POWERUP_DEF, rand, TIMEWARP_SCALE, SING_REACH, SING_PULL, SING_CORE, SING_SHIP_PULL, SING_BOSS_DPS } from './config.js';
...
export function update(dt) {
    ...
    if (g.nuke > 0) {
        ...
        if (g.nuke <= 0) {
            ...
            if (g.lives <= 0)
                ...
                if (!g.boss)
                    spawnWave();
            } // a surviving boss keeps the wave going
        }
        return;
    }
    g.clock += dt;
    ...
    if (g.shield > 0 && !g.sbInfinite)
        g.shield -= dt;
    if (g.timeWarp > 0 && !g.sbInfinite)
        g.timeWarp -= dt; // bullet-time buff winds down
    if (g.ramHitCd > 0)
        g.ramHitCd -= dt;
    ...
    if (g.megaMsgT > 0)
        g.megaMsgT -= dt; // narrator box counts down
    // Decay power-up drop heat so recently-dropped types gradually become likely again.
    for (const t in g.dropHeat)
        g.dropHeat[t] *= Math.exp(-0.06 * dt);
    ...
    if (ship.dead) {
        ...
        if (joy.active) {
            ...
            if (g.settings.legacyControls) {
                ...
                if (dx || dy) {
                    ...
                    let d = desired - ship.angle;
                    d = Math.atan2(Math.sin(d), Math.cos(d));
                    const max = 6.9 * dt;
                    ship.angle += Math.max(-max, Math.min(max, d)); // 1.5x the base rotate rate - WASD/arrow steering turns snappier
                    let rem = desired - ship.angle;
                    rem = Math.atan2(Math.sin(rem), Math.cos(rem));
                    if (Math.abs(rem) < 0.15)
                        input.thrust = true; // reached the heading -> engines on
                }
                else {
        ...
        if (input.thrust) {
            ship.vx += Math.cos(ship.angle) * accel * S * dt;
            ship.vy += Math.sin(ship.angle) * accel * S * dt;
        }
        ...
        if (g.gunsOff) {
            ...
            if (input.fire) {
                ...
                if (reroll || g.lightningAirAngle === null)
                    g.lightningAirAngle = rand(-LIGHTNING_HALF, LIGHTNING_HALF);
                ...
                if (reroll)
                    g.lightningMidTimer = 0.1;
                if (g.lightningTarget && g.fireCd <= 0) {
                    const ai = asteroids.indexOf(g.lightningTarget);
                    if (ai >= 0)
                        destroyAsteroid(ai);
                    else if (g.lightningTarget._bossNode) {
                        if (g.boss)
                            damageBossNode(g.boss, g.lightningTarget._bossNode, 1, g.lightningTarget.x, g.lightningTarget.y);
                    }
                    else {
                        const ui = ufos.indexOf(g.lightningTarget);
                        if (ui >= 0)
                            damageUfo(ui, 1);
                    }
                    g.fireCd = 0.18;
                ...
                g.lightningTarget = null;
                g.lightningEnd = null; // not firing: no lock, no bolt
            }
        }
            ...
            if (g.homingLeft > 0) {
                ...
                if (g.homingLeft === 0) {
                    g.fireCd = 3;
                    g.homingTrickle = 1;
                } // start the 3s cooldown
            }
            else if (g.fireCd > 0) {
                // During the cooldown, trickle a single forward missile once a second.
                g.homingTrickle -= dt;
                if (g.homingTrickle <= 0) {
                    if (input.fire)
                        spawnMissile(ship.angle);
                    g.homingTrickle += 1;
                }
            }
            else if (input.fire) {
                g.homingBase = ship.angle;
                g.homingIdx = 0;
                g.homingLeft = 12;
                g.homingGap = 0; // begin a burst
            }
        }
    ...
    for (const a of asteroids) {
        a.x += a.vx * wdt;
        a.y += a.vy * wdt;
        a.angleR += a.spin * wdt; // wdt: slowed under Time Warp
        if (hard)
            edgeReflect(a);
        else
            wrap(a); // ping-pong off the solid walls during the mega fight
        if (a.grace > 0)
            a.grace -= dt; // count down the spawn-grace (no hitbox) - on the real clock
    }
        ...
        for (let ai = asteroids.length - 1; ai >= 0; ai--) {
            ...
            if (dx * dx + dy * dy < a.radius * a.radius) {
                ...
                if (b.pierce > 0) {
                    b.pierce--;
                    break;
                } // survive the hit, one less pierce left
                bullets.splice(bi, 1);
                break;
            }
        }
...
function updateSingularities(dt) {
    ...
    for (let si = sings.length - 1; si >= 0; si--) {
        ...
        s.life -= dt;
        if (s.life <= 0) {
            burst(s.x, s.y, c, { count: 16, speed: 150, life: 0.5, lines: true });
            sings.splice(si, 1);
            continue;
        }
        s.x += s.vx * dt;
        s.y += s.vy * dt;
        wrap(s); // wander slowly across the field
        ...
        if (!g.ship.dead && !g.gameOver) {
            ...
            if (d < reach) {
                const pull = SING_SHIP_PULL * S * dt * (1.15 - d / reach);
                g.ship.vx += (dx / d) * pull;
                g.ship.vy += (dy / d) * pull;
            }
        }
        for (let ai = g.asteroids.length - 1; ai >= 0; ai--) {
            ...
            const d = Math.hypot(dx, dy) || 1;
            if (d < core) {
                burst(a.x, a.y, c, { count: 6, speed: 90, life: 0.35 });
                destroyAsteroid(ai);
                continue;
            }
            if (d < reach) {
                const pull = SING_PULL * S * dt * (1.15 - d / reach);
                a.x += (dx / d) * pull;
                a.y += (dy / d) * pull;
            }
        }
        for (let ui = g.ufos.length - 1; ui >= 0; ui--) {
            const u = g.ufos[ui];
            if (u.kind !== 'reward' || u.appear < 1)
                continue; // ambient escorts are indestructible - leave them
        ...
        if (g.boss)
            for (const n of g.boss.nodes) {
                if (!bossNodeVulnerable(g.boss, n) || !isFinite(n.maxhp) || n.hp <= 1)
                    continue;
                const [nx, ny] = bossNodePos(g.boss, n);
                const [dx, dy] = wrapDelta(nx, ny, s.x, s.y);
                if (dx * dx + dy * dy <= reach * reach)
                    n.hp = Math.max(1, n.hp - SING_BOSS_DPS * dt);
            }
    }
}
