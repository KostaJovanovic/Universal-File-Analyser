...
export function fireLaser() {
    ...
    for (let ai = g.asteroids.length - 1; ai >= 0; ai--) {
        ...
        if (a.grace > 0)
            continue;
        if (distToSeg(a.x, a.y, ship.x, ship.y, ex, ey) < a.radius + laserWidth() / 2)
            destroyAsteroid(ai);
    }
    ...
    for (let ui = g.ufos.length - 1; ui >= 0; ui--) {
        ...
        if (u.kind !== 'reward' || u.appear < 1)
            continue;
        if (distToSeg(u.x, u.y, ship.x, ship.y, ex, ey) < u.radius + laserWidth() / 2)
            damageUfo(ui, 2);
    }
    if (boss)
        for (const n of boss.nodes) {
            ...
            if (distToSeg(nx, ny, ship.x, ship.y, ex, ey) < n.r + laserWidth() / 2)
                damageBossNode(boss, n, 2, nx, ny);
        }
}
...
export function findLightningTarget() {
    ...
    const consider = (o) => {
        ...
        if (dist > lightningRange() || dist >= bestD)
            return;
        let d = Math.atan2(dy, dx) - ship.angle;
        d = Math.atan2(Math.sin(d), Math.cos(d));
        if (Math.abs(d) > LIGHTNING_HALF)
            return;
        best = o;
        bestD = dist;
    };
...
export function fireWeapon() {
    ...
    if (g.weapon === 'sniper') {
        spawnBullet(ship.angle, 1080 * S, Infinity, true, 1);
        g.fireCd = 0.4;
        return;
    } // punches through one into a second
    spawnBullet(ship.angle, 540 * S, 0.9, false);
    g.fireCd = 0.18; // normal
}
...
function targetAlive(t) {
    if (!t)
        return false;
    if (g.asteroids.includes(t))
        return t.grace <= 0;
    if (g.ufos.includes(t))
        return t.kind === 'reward' && t.appear >= 1 && t.hp > 0;
    const boss = g.boss;
    if (boss && boss.nodes.includes(t))
        return bossNodeVulnerable(boss, t) && !(boss.type === 'megastructure' && t.kind === 'core');
    return false;
}
...
function collectTargets() {
    ...
    for (const a of g.asteroids) {
        if (a.grace > 0)
            continue;
        out.push({ obj: a, need: 1, x: a.x, y: a.y, boss: false });
    }
    for (const u of g.ufos) {
        if (u.kind !== 'reward' || u.appear < 1 || u.hp <= 0)
            continue;
        out.push({ obj: u, need: Math.max(1, Math.ceil(u.hp)), x: u.x, y: u.y, boss: false });
    }
    const boss = g.boss;
    if (boss)
        for (const n of boss.nodes) {
...
function retargetMissiles() {
    ...
    if (!targets.length)
        return; // nothing to lock onto - free missiles keep their heading
    ...
    for (const t of targets)
        cap.set(t.obj, t.need);
    for (const m of missiles)
        if (m.target && cap.has(m.target))
            cap.set(m.target, cap.get(m.target) - 1);
    // Greedy fill: sort every (free missile, eligible target-with-capacity) pair by distance, closest claims first.
    const pairs = [];
    for (const m of free)
        for (const t of targets) {
            if (cap.get(t.obj) <= 0 || !eligible(m, t))
                continue;
    ...
    for (const p of pairs) {
        if (p.m.target || cap.get(p.obj) <= 0)
            continue;
        p.m.target = p.obj;
        cap.set(p.obj, cap.get(p.obj) - 1);
    }
...
export function updateMissiles(dt) {
    ...
    for (let i = missiles.length - 1; i >= 0; i--) {
        ...
        m.life -= dt;
        if (m.life <= 0) {
            burst(m.x, m.y, homingColor, { count: 6, speed: 80, life: 0.3 });
            missiles.splice(i, 1);
            continue;
        }
        if (m.target && targetAlive(m.target)) {
        ...
        if (hit) {
            burst(m.x, m.y, homingColor, { count: 8, speed: 110, life: 0.35 });
            missiles.splice(i, 1);
        }
    }
}
